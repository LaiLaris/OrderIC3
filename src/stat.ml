...
type stat_item = I of int_item | F of float_item | L of int_list_item

(* Create a statistics item *)
let empty_item display default =
  { display; value = default; default; temp = default }
...
let unpause_time item = item.temp <- Unix.gettimeofday ()
...
let stop_all_timers stats =
  List.iter (function F i -> record_time i | _ -> ()) stats
...
let pp_print_item width ppf = function
  ...
  | L { display; value } ->
      Format.fprintf ppf "%-*s: @[<hov>%a@]" width display
...
let pp_print_item_xml ppf = function
  ...
  | L { display; value } ->
      Format.fprintf ppf
        ...
        (Lib.pp_print_list
           (function
             | ppf -> Format.fprintf ppf "<value type=\"int\">%d</value>")
           "@,")
        value
...
let pp_print_item_json ppf = function
  ...
  | L { display; value } ->
      let pp_print_value_json ppf value =
        Format.fprintf ppf "{@[<v 1>@,\"type\" : \"int\",@,\"value\" : %d@]@,}"
          value
      in
      ...
      Format.fprintf ppf
        ...
        display
        (pp_print_list_attrib pp_print_value_json)
        value
...
let bmc_k = empty_item "k" 0
let bmc_total_time = empty_item "Total time" 0.

(* Title for BMC statistics *)
let bmc_stats_title = "BMC"

(* All BMC statistics *)
let bmc_stats = [ I bmc_k; F bmc_total_time ]
...
let pp_print_bmc_stats ppf =
  Format.fprintf ppf "@[<v>@,[%s]@,%a@]" bmc_stats_title pp_print_stats
    bmc_stats

(* ********** Inductive step statistics ********** *)

let ind_k = empty_item "k" 0
...
let ind_restarts = empty_item "Restarts" 0
let ind_lazy_invariants_count = empty_item "Asserted invariants at one state" 0
let ind_lazy_invariants_time = empty_item "Lazy invariants time" 0.
let ind_total_time = empty_item "Total time" 0.
...
let pp_print_ind_stats ppf =
  Format.fprintf ppf "@[<v>@,[%s]@,%a@]" ind_stats_title pp_print_stats
    ind_stats

(* ********** IC3 statistics ********** *)

let ic3_k = empty_item "k" 0
let ic3_restarts = empty_item "Restarts" 0
let ic3_solver = empty_item "Solver" 0
let ic3_neg_state = empty_item "Neg_state" 0
let ic3_frame_sizes = empty_item "Frame sizes" []
let ic3_fwd_propagated = empty_item "Forward propagations" 0

let ic3_fwd_gen_propagated =
  empty_item "Forward propagations before generalization" 0

let ic3_fwd_subsumed = empty_item "Forward subsumed clauses" 0
let ic3_back_subsumed = empty_item "Backward subsumed clauses" 0
let ic3_inductive_blocking_clauses = empty_item "Inductive blocking clauses" 0
let ic3_fwd_fixpoint = empty_item "Fixpoint at" 0
let ic3_total_time = empty_item "Total time" 0.
let ic3_fwd_prop_time = empty_item "Forward propagation time" 0.
let ic3_strengthen_time = empty_item "Frame strengthening time" 0.
let ic3_generalize_time = empty_item "Generalization time" 0.
let ic3_find_cex_time = empty_item "Counterexample search time" 0.
let ic3_ind_gen_time = empty_item "Inductive generalization time" 0.
let ic3_inductive_check_time = empty_item "Inductiveness check time" 0.
let ic3_activation_literals = empty_item "Activation literals" 0
let ic3_stale_activation_literals = empty_item "Stale activation literals" 0
...
let pp_print_ic3_stats ppf =
  Format.fprintf ppf "@[<v>@,[%s]@,%a@]" ic3_stats_title pp_print_stats
    ic3_stats

let ic3ia_refinements = empty_item "Refinements per index" []

let ic3ia_refinements_end =
  empty_item "Refinenements per index relative to end" []

let ic3ia_num_simulations = empty_item "Number of concrete simulations" 0
let ic3ia_interpolation_time = empty_item "Total time for interpolation" 0.
...
let pp_print_ic3ia_stats ppf =
  Format.fprintf ppf "@[<v>@,[%s]@,%a@]" ic3ia_stats_title pp_print_stats
    ic3ia_stats

(* ********** INVGENOS statistics ********** *)

let invgengraph_os_k = empty_item "k" 0
...
let invgengraph_os_graph_rewriting_time = empty_item "Graph rewriting time" 0.
let invgengraph_os_total_time = empty_item "Total time" 0.
...
let pp_print_invgengraph_os_stats ppf =
  Format.fprintf ppf "@[<v>@,[%s]@,%a@]" invgengraph_os_stats_title
    pp_print_stats invgengraph_os_stats

(* ********** INVGENTS statistics ********** *)

let invgengraph_ts_k = empty_item "k" 0
...
let invgengraph_ts_graph_rewriting_time = empty_item "Graph rewriting time" 0.
let invgengraph_ts_total_time = empty_item "Total time" 0.
...
let pp_print_invgengraph_ts_stats ppf =
  Format.fprintf ppf "@[<v>@,[%s]@,%a@]" invgengraph_ts_stats_title
...
let pp_print_c2i_stats ppf =
  Format.fprintf ppf "@[<v>@,[%s]@,%a@]" c2i_stats_title pp_print_stats
    c2i_stats

(* ********** Testgen statistics ********** *)

(* Number of testcases generated. *)
let testgen_testcases = empty_item "testcases" 0

(* Number of deadlocks found. *)
let testgen_deadlocks = empty_item "deadlocks" 0

(* Number of restarts performed. *)
let testgen_restarts = empty_item "restarts" 0

(* Time spent going forward. *)
let testgen_forward_time = empty_item "forward" 0.

(* Time spent going backward. *)
let testgen_backward_time = empty_item "backward" 0.

(* Time spent enumerating. *)
let testgen_enumerate_time = empty_item "enumerate" 0.

(* Total runtime for testgen. *)
let testgen_total_time = empty_item "Total time" 0.
...
let pp_print_testgen_stats ppf =
  Format.fprintf ppf "@[<v>@,[%s]@,%a@]" testgen_stats_title pp_print_stats
    testgen_stats

(* ********** SMT statistics ********** *)

let smt_check_sat_time = empty_item "check-sat time" 0.
let smt_get_value_time = empty_item "get-value time" 0.
let smt_get_unsat_core_time = empty_item "get-unsat-core time" 0.
...
(* All SMT statistics *)
let smt_stats =
  [ F smt_check_sat_time; F smt_get_value_time; F smt_get_unsat_core_time ]
...
let pp_print_smt_stats ppf =
  Format.fprintf ppf "@[<v>@,[%s]@,%a@]" smt_stats_title pp_print_stats
    smt_stats

(* ********** Certificate statistics ********** *)

let certif_gen_time = empty_item "generation time" 0.
let certif_min_time = empty_item "minimization time" 0.
let certif_frontend_time = empty_item "frontend time" 0.
let certif_cvc5_time = empty_item "cvc5 proof-gen time" 0.
let certif_k = empty_item "k" (-1)
let certif_size = empty_item "size" 0
let certif_old_k = empty_item "Old k" (-1)
let certif_old_size = empty_item "Old size" 0
...
let pp_print_certif_stats ppf =
  Format.fprintf ppf "@[<v>@,[%s]@,%a@]" certif_stats_title pp_print_stats
    certif_stats

(* ********** Misc statistics ********** *)

let total_time = empty_item "Total time" 0.
let analysis_time = empty_item "Analysis time" 0.
let clause_of_term_time = empty_item "clause_of_term time" 0.
let smtexpr_of_term_time = empty_item "smtexpr_of_term time" 0.
let term_of_smtexpr_time = empty_item "term_of_smtexpr time" 0.
...
let pp_print_misc_stats ppf =
  Format.fprintf ppf "@[<v>%a@]" pp_print_stats misc_stats
