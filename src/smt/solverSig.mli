...
module type Inst = sig
  ...
  val define_fun :
    string -> SMTExpr.var list -> SMTExpr.sort -> SMTExpr.t -> decl_response
  ...
  val execute_custom_command :
    string -> SMTExpr.custom_arg list -> int -> custom_response
  (** Execute a custom command and return its result

      [execute_custom_command s c a r] sends a custom command [s] with the
      arguments [a] to the solver instance [s]. The command expects [r]
      S-expressions as result in case of success and returns a pair of the
      success response and a list of S-expressions. *)
...
module type S = sig
  ...
  (** The functor [Create] creates a new instance of the SMT solver with
      paramters passed as its arguments. The parameter argument [P] conataints a
      unique identifier [id], initialized to the logic [l] and produces models
      if the optional labelled argument [produce_models] is [true], proofs if
      [produce_proofs] is true, unsatisfiable cores if [produce_unsat_cores] is
      true, and unsatisfiable sets of assumptions if [produce_unsat_assumptions]
      is true *)
