...
let cmd_line _ (* logic *) timeout _ (* produce_models *) _ (* produce_proofs *)
  ...
  | None -> [| yices_bin |]
  | Some timeout ->
      let timeout = Format.sprintf "--timeout=%.0f" (timeout |> ceil) in
      [| yices_bin; timeout |]

(* Command to limit check-sat to run for the given numer of ms at most *)
let check_sat_limited_cmd _ = failwith "Yices.check_sat_limited_cmd"
...
let prelude =
  [
    ...
    "(define to_int::(-> x::real (subtype (y::int) (and (<= y x) (< x (+ y \
     1))))))";
    "(define to_real::(-> x::int (subtype (y::real) (= y x))))";
    (* Define xor operator *)
    "(define xor :: (-> bool bool bool) (lambda (x::bool y::bool) (and (or x \
     y) (not (and x y)) )))";
  ]
...
(* Pretty-print a type *)
let rec pp_print_type ppf t = pp_print_type_node ppf (Type.node_of_type t)
...
and pp_print_type_node ppf = function
  ...
  | Type.UBV i -> (
      ...
      | _ -> raise (Invalid_argument "pp_print_type_node: BV size not allowed"))
  | Type.BV i -> (
      match i with
      | 8 -> Format.pp_print_string ppf "(bitvector 8)"
      | 16 -> Format.pp_print_string ppf "(bitvector 16)"
      ...
      | _ -> raise (Invalid_argument "pp_print_type_node: BV size not allowed"))
  | Type.IntRange (i, j) ->
      Format.fprintf ppf "(subrange %a %a)" pp_print_bound_opt i
        pp_print_bound_opt j
  | Type.Enum (i, j) ->
      Format.fprintf ppf "(subrange %a %a)" Numeral.pp_print_numeral i
        Numeral.pp_print_numeral j
  | Type.Real -> Format.pp_print_string ppf "real"
  | Type.Abstr s -> Format.pp_print_string ppf s
  | Type.Array (te, ti) ->
      Format.fprintf ppf "(-> %a %a)" pp_print_type ti pp_print_type te
...
let rec interpr_type t =
  ...
  | Type.IntRange _ (* -> Type.mk_int () *) | Type.Enum _ | Type.Bool | Type.Int
  | Type.UBV 8
  | Type.UBV 16
  | Type.UBV 32
  | Type.UBV 64
...
let type_of_string_sexpr = function
  ...
  | HStringSExpr.List
      [ HStringSExpr.Atom s; HStringSExpr.Atom i; HStringSExpr.Atom j ]
    when s == s_subrange ->
      Type.mk_int_range
        (Some (Numeral.of_string (HString.string_of_hstring i)))
...
let reserved_word_list =
  List.map HString.mk_hstring [ "maxsat"; "mk-tuple"; "tuple"; "record" ]
...
let rec pp_print_symbol_node ?arity ppf = function
  ...
  | `DIVISIBLE _ -> failwith "divisible not implemented for yices"
  ...
  | `BVEXTRACT (i, j) ->
      Format.fprintf ppf "bv-extract %a %a" Numeral.pp_print_numeral j
        Numeral.pp_print_numeral i
  | `BVSIGNEXT i ->
      Format.fprintf ppf "bv-sign-extend %a" Numeral.pp_print_numeral i
...
let rec pp_print_term' db ppf t =
  ...
  | Term.T.Node (s, [ a; i; v ]) when s == Symbol.s_store ->
      Format.fprintf ppf "@[<hv 1>(%a@ %a (%a)@ %a)@]"
        (pp_print_symbol ?arity:(Some 3))
        s (pp_print_term' db) a (pp_print_term' db) i (pp_print_term' db) v
  (* Print a function application as S-expression *)
  | Term.T.Node (s, a) ->
      Format.fprintf ppf "@[<hv 1>(%a@ %a)@]"
        (pp_print_symbol ?arity:(Some (List.length a)))
        s
        (pp_print_list (pp_print_term' db) "@ ")
        a
  (* Print a let binding *)
  | Term.T.Let (l, b) ->
      let (Term.T.L (x, t)) = Term.T.node_of_lambda l in
      Format.fprintf ppf "@[<hv 1>(let@ @[<hv 1>(%a)@]@ %a)@]"
        (pp_print_let_bindings 0 db)
        (b, x)
        (pp_print_term' (db + List.length b))
        t
  ...
  | (Term.T.Forall l | Term.T.Exists l) as nt ->
      ...
      let quant =
        ...
        | _ -> assert false
      in
      Format.fprintf ppf "@[<hv 1>(%s@ @[<hv 1>(%a)@ %a@])@]" quant
        (pp_print_typed_var_list db)
        x
        (pp_print_term' (db + List.length x))
        t
  | _ ->
      Term.T.pp_print_term_w pp_print_symbol Var.pp_print_var pp_print_sort ~db
        ppf t
...
and pp_print_let_bindings i db ppf = function
  ...
  | t :: tl, s :: sl ->
      ...
      Format.fprintf ppf "@[<hv 1>(X%i::%a@ %a)@]" db' pp_print_type s
        (pp_print_term' (db - i))
        t;
