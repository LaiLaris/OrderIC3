...
let check_logic logic =
  ...
  | `Inferred l when mem Q l ->
      let msg =
        Format.asprintf "In %a: OpenSMT does not support quantifiers"
          Lib.pp_print_kind_module (KEvent.get_module ())
      in
      failwith msg
  | `Inferred l when mem A l ->
      let msg =
        Format.asprintf "In %a: OpenSMT does not support arrays"
          Lib.pp_print_kind_module (KEvent.get_module ())
      in
      failwith msg
  | `Inferred l when mem NA l ->
      let msg =
        Format.asprintf "In %a: OpenSMT does not support non-linear arithmetic"
          Lib.pp_print_kind_module (KEvent.get_module ())
      in
      failwith msg
  | `Inferred l when mem BV l ->
      let msg =
        Format.asprintf "In %a: OpenSMT does not support machine integers"
          Lib.pp_print_kind_module (KEvent.get_module ())
      in
      failwith msg
  | `Inferred l when mem IA l && mem RA l ->
      let msg =
        Format.asprintf
          "In %a: OpenSMT does not support the combination of integer and real \
           arithmetic"
          Lib.pp_print_kind_module (KEvent.get_module ())
      in
      failwith msg
  | `Inferred l when mem UF l && (mem IA l || mem RA l) ->
      let msg =
        Format.asprintf
          "In %a: OpenSMT does not support the combination of uninterpreted \
           functions and integer/real arithmetic"
          Lib.pp_print_kind_module (KEvent.get_module ())
      in
      failwith msg
  | _ -> ()
...
let cmd_line logic timeout _ (* produce_assignments *) _ (* produce_proofs *) _
  ...
  match timeout with None -> cmd | Some _timeout -> cmd
(*let timeout = 
