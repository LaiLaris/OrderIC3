...
let check_logic logic =
  ...
  | `Inferred l ->
      ...
      (if mem BV l && (mem IA l || mem RA l) then
         let msg =
           Format.asprintf
             "In %a: Yices 2 does not support programs with both \
              integers/reals and machine integers"
             Lib.pp_print_kind_module (KEvent.get_module ())
         in
         failwith msg);
      if mem NA l then (
        (if mem A l then
           let msg =
             Format.asprintf
               "In %a: Yices 2 does not support non-linear arithmetic problems \
                with arrays"
               Lib.pp_print_kind_module (KEvent.get_module ())
           in
           failwith msg);
        if Flags.Smt.check_sat_assume () then (
          ...
          KEvent.log Lib.L_warn
            "In %a: Yices 2 does not support check-sat-assuming with \
             non-linear problems.@ check_sat_assume option disabled"
            Lib.pp_print_kind_module (KEvent.get_module ())))
  | _ -> ()
