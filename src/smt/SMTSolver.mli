...
val create_instance :
  ...
  Flags.Smt.solver ->
  t
(** Create a new instance of an SMT solver of the given kind and with the given
    flags *)
...
(** Name a term, convert a term to an SMT expression and assert, and return the
    name *)
...
(** Return an unsatisfiable core of named expressions if the current context is
    unsatisfiable. Interpret unsatisfiable core as names and return corresponing
    terms *)
...
val check_sat_assuming :
  ...
  Term.t list ->
  'a
(** Checks satisfiability of the current context assuming the given list of
    literals, and evaluate one of two continuation functions depending on the
    result

    [check_sat_assuming s t f l] assumes each of the literals in [l] to be true,
    and checks satisfiablilty of the context of the SMT solver instance [s]. If
    the solver returns satisfiable, the continuation [t] is evaluated, and if
    the solver returns unsatisfiable, the continuation [f] is evaluated.

    {e Important:} If a solver does not support check-sat with assumptions, it
    will be simulated by checking satisfiability on a new context level with the
    literals asserted. This context is removed after the continuations have been
    evaluated, hence all operations performed in the continuations affecting the
    context will be undone upon return from this function. Do not rely on
    context-modifying operations in the continuations being persistent, and keep
    the continuations as short as possible. Moreover, any call to
    [get_unsat_core_lits] should be done INSIDE the continutation [if_unsat],
    and NOT in AFTER the call to [check_sat_assuming].

    The list [l] should contain only positive Boolean constants, although this
    is not enforced. If the solver does not support the [check-sat-assuming]
...
val check_sat_and_get_term_values :
  ...
  Term.t list ->
  'a
(** Checks satisfiability of the current context, and evaluate one of two
    continuation functions depending on the result

    [check_sat_and_get_term_values s t f l] checks satisfiablilty of the context
    of the SMT solver instance [s]. If the solver returns satisfiable, the
    continuation [t] is evaluated with the values of the terms of [l], and if
    the solver returns unsatisfiable, the continuation [f] is evaluated. *)
...
val check_sat_assuming_and_get_term_values :
  ...
  Term.t list ->
  'a
(** Check satisfiability under assumptions as with {!check_sat_assuming}, but if
    the solver returns satisfiable, the values of the terms in the current
    context are given to the continuation [t] as its second argument *)

(** Alternative between type 'a and 'b *)
type ('a, 'b) sat_or_unsat = Sat of 'a | Unsat of 'b

val check_sat_assuming_ab :
  t -> (t -> 'a) -> (t -> 'b) -> Term.t list -> ('a, 'b) sat_or_unsat
(** Check satisfiability under assumptions as with {!check_sat_assuming}, but
    the two continuations can return different values that are wrappen in the
    {!sat_or_unsat} type *)

val check_sat_assuming_tf : t -> Term.t list -> bool
(** Check satisfiability under assumptions as with {!check_sat_assuming}, but
    return [true] or [false] without continuations as arguments *)
...
(** Execute the a custom command with the given arguments, and expect the given
    number of S-expressions as a result *)
