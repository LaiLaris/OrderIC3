...
module Converter (Driver : SolverDriver.S) : Conv = struct
  include Driver
  ...
  let var_term_of_smtexpr e =
    ...
    if Term.is_bound_var e then invalid_arg "var_of_smtexpr: Bound variable"
    else
      ...
      | Term.T.Const sym -> (
          ...
          with Not_found ->
            invalid_arg
              (Format.asprintf
                 "var_of_smtexpr: %aNo state variable found for uninterpreted \
                  function symbol"
      ...
      | Term.T.App (uf_sym, args) when Symbol.is_uf uf_sym -> (
          try
            ...
            List.fold_left Term.mk_select (Term.mk_var v) args
          with Not_found ->
            invalid_arg
              (Format.asprintf
                 "var_of_smtexpr: %aNo state variable found for uninterpreted \
      ...
      (* Other expressions *)
      | Term.T.App _ ->
          invalid_arg "var_of_smtexpr: Must be an uninterpreted function"
  ...
  let term_of_smtexpr term =
    Term.map
      (function
        | _ -> (
            function
            | t -> ( try var_term_of_smtexpr t with Invalid_argument _ -> t)))
      term
          ...
          | _ -> (
              ...
              | t when Term.is_free_var t -> (
                ...
                uninterpreted function *)
                  try Term.mk_var (List.assq v var_to_temp_var)
  ...
  let smtexpr_of_term term = quantified_smtexpr_of_term false [] term
  ...
  let rec pp_print_custom_arg ppf = function
    ...
    | ArgList l ->
        Format.fprintf ppf "(%a)" (Lib.pp_print_list pp_print_custom_arg " ") l

  (* Return a string representation of a custom argument *)
  let string_of_custom_arg t = string_of_t pp_print_custom_arg t
end
