...
type t = {
  ...
  solver_trace_res : response -> unit; (* Tracing function for responses *)
...
let next_id solver =
  solver.solver_last_id |> YicesResponse.int_of_yices_id |> succ
  |> YicesResponse.yices_id_of_int

let name_of_yices_id solver id = Hashtbl.find solver.solver_id_names id
  ...
  let model =
    List.filter
      (fun (e, _) ->
        match e with
        | HStringSExpr.Atom x -> (
            try HString.sub x 0 4 <> "aux:" with Invalid_argument _ -> true)
        | _ -> true)
      model
  in
  let m =
    List.fold_left
      (fun acc (e, v) ->
        ...
        let v_smte =
          if Term.is_numeral v_smte && Type.is_real (Term.type_of_term e_smte)
          then
            v_smte |> Term.numeral_of_term |> Numeral.to_big_int
            |> Decimal.of_big_int |> Term.mk_dec
          else v_smte
        in

        (* assert (Term.type_of_term e_smte = Term.type_of_term v_smte); *)
        SMTExprMap.add e_smte v_smte acc)
      SMTExprMap.empty model
  in
...
let[@ocaml.warning "-27"] get_command_response solver timeout =
  ...
  | YicesResponse.YSuccess -> `Success
  | YicesResponse.YNoResp -> `NoResponse (* or success maybe *)
  | YicesResponse.YError ->
      solver.solver_state <- YError;
      `Error (get_yices_errmsg solver)
  | _ -> failwith "Yices returned an unexpected response"
...
let[@ocaml.warning "-27"] get_custom_command_response num_res solver timeout =
  ...
  | YicesResponse.YError ->
      solver.solver_state <- YError;
      `Error (get_yices_errmsg solver)
  | _ -> failwith "Yices get_custom_command_response got unexpected answer"
...
let send_command cmd_type ({ solver_stdin } as solver) command timeout =
      ...
      Format.out_newline =
        (fun () ->
          fmt_out_fun.Format.out_string
            (" " ^ cl ^ "\n" ^ op ^ " ")
            0
            (3 + String.length op + String.length cl));
      Format.out_flush =
        (fun n ->
...
let pp_print_bindings ppf vars =
  ...
  | _ ->
      Format.fprintf ppf "(@[<hv 1>%a@])"
        (pp_print_list pp_print_binding "@ ")
        vars

(* Print lambda expression *)
let pp_print_lambda ppf (arg_vars, defn) =
  if arg_vars = [] then Format.fprintf ppf "%a" pp_print_expr defn
  else
    Format.fprintf ppf "(lambda@ @[<hv 1>%a@]@ %a)" pp_print_bindings arg_vars
      pp_print_expr defn
...
let ensure_symbol_qf_lira s =
  ...
  | `TRUE | `FALSE | `NOT | `IMPLIES | `AND | `OR | `XOR | `EQ | `DISTINCT
  | `ITE | `NUMERAL _ | `DECIMAL _ | `MINUS | `PLUS | `TIMES | `DIV | `LEQ | `LT
  | `GEQ | `GT | `TO_REAL | `TO_INT | `IS_INT | `UF _ ->
      ()
  (* | `UF f when UfSymbol.arg_type_of_uf_symbol f = [] -> () *)
  | `UBV _ | `BV _ | `INTDIV | `DIVISIBLE _ | `MOD | `ABS | `BVNOT | `BVNEG
  | `BVAND | `BVOR | `BVADD | `BVSUB | `BVMUL | `BVUDIV | `BVSDIV | `BVUREM
  | `BVSREM | `BVSHL | `BVLSHR | `BVASHR | `BVULT | `BVULE | `BVUGT | `BVUGE
  | `BVSLT | `BVSLE | `BVSGT | `BVSGE | `BVEXTRACT _ | `BVCONCAT | `BVSIGNEXT _
  | `BVZEROEXT _ | `UINT8_TO_INT | `UINT16_TO_INT | `UINT32_TO_INT
  | `UINT64_TO_INT | `INT8_TO_INT | `INT16_TO_INT | `INT32_TO_INT
  | `INT64_TO_INT | `TO_UINT8 | `TO_UINT16 | `TO_UINT32 | `TO_UINT64 | `TO_INT8
  | `TO_INT16 | `TO_INT32 | `TO_INT64 | `BV2NAT | `SELECT _ | `STORE ->
      let msg =
        Format.sprintf
          "Yices was run with set-arith-only, but the symbol %s is not \
           interpreted correctly in this mode. Run Kind 2 with --smt_logic \
           none instead."
...
let rec ensure_lambda_qf_lira l =
  let open Term.T in
  match node_of_lambda l with L (_, t) -> ensure_term_qf_lira t
...
let fail_declare_when_arith solver f arg_sorts res_sort =
  if solver.solver_config.solver_arith_only && arg_sorts <> [] then (
    let msg =
      Format.asprintf
        "Yices was run with set-arith-only, but the symbol %s has type %a." f
...
let declare_sort solver sort =
  match Type.node_of_type sort with
  | Type.Abstr _ ->
      let cmd =
        Format.sprintf "@[<hv 1>(define-type@ %s)@]" (string_of_sort sort)
      in
...
let declare_fun solver fun_symbol arg_sorts res_sort =
  ...
  let cmd =
    Format.asprintf "@[<hv 1>(define@ %s ::@ @[<hv 1>%a@])@]" fun_symbol
      pp_print_function_type (arg_sorts, res_sort)
  in
...
let define_fun solver fun_symbol arg_vars res_sort defn =
  ...
  let cmd =
    Format.asprintf "@[<hv 1>(define@ %s ::@ @[<hv 1>%a@]@ @[<hv 1>%a@])@]"
      fun_symbol pp_print_function_type (arg_sorts, res_sort) pp_print_lambda
      (arg_vars, defn)
  in
...
let assert_removable_expr ?id solver expr =
  ...
  let t', name_info =
    if Term.is_named t then (
      ...
      ( Term.term_of_named t,
        Format.asprintf "[id: %a, name: %s]" YicesResponse.pp_print_yices_id id
          name ))
    else (t, Format.asprintf "[id: %a]" YicesResponse.pp_print_yices_id id)
  in
  let expr = Conv.smtexpr_of_term t' in

  let cmd =
    Format.asprintf "@[<hv 1>(assert+@ @[<hv>%s@])@]\n;; %s"
      (string_of_expr expr) name_info
  in
...
let retract solver id =
  let cmd =
    Format.asprintf "@[<hv 1>(retract %a)@]" YicesResponse.pp_print_yices_id id
  in
...
let fast_pop solver = function
  ...
  | n when n > 0 -> (
      try
        ...
        for
          i = YicesResponse.int_of_yices_id solver.solver_last_id
          downto YicesResponse.int_of_yices_id id + 1
        do
...
let default_of_type t = TermLib.default_of_type t |> Conv.smtexpr_of_term
    ...
    List.fold_left
      (fun _ expr ->
        ...
        | (Term.T.App (s, []) | Term.T.Const s) when Symbol.is_uf s ->
            ...
            let name =
              s |> Symbol.uf_of_symbol |> UfSymbol.string_of_uf_symbol
            in
    ...
    (fun e v ->
      try
        ...
        if Term.is_free_var t then
          Var.VarHashtbl.add vars_assign (Term.free_var_of_term t)
          ...
          let args =
            List.map (fun x -> Numeral.to_int (Term.numeral_of_term x)) args_t
          in

          let vt = Conv.term_of_smtexpr v in

          let map_var =
            match Var.VarHashtbl.find vars_assign var with
            | Model.Map m -> m
            | _ -> assert false
            | exception Not_found -> Model.MIL.empty
          in
...
let get_value solver expr_list =
  ...
  let cmd =
    Format.asprintf "@[<hv 1>(get-value@ @[<hv 1>(%a)@])@]"
            ...
            let v =
              ...
              with Not_found -> (
                ...
                with Invalid_argument _ ->
                  ...
                  let ve =
                    Eval.eval_term [] vars_assign (Conv.term_of_smtexpr e)
                  in
...
let get_model solver =
  ...
  (* The fake SMTLIB command  *)
  let cmd = Format.asprintf "@[<hv 1>(get-model)@]" in
...
let execute_custom_command solver cmd args num_res =
  ...
  let cmd =
    if args = [] then Format.sprintf "@[<hv 1>(%s)@]" cmd
    else
      Format.sprintf "@[<hv 1>(%s@ %s)@]" cmd
...
let create_trace_ppf id =
  ...
  if Flags.Smt.trace () then (
    ...
    try
      ...
      KEvent.log L_debug "Tracing output of SMT solver instace to %s"
        trace_filename;
...
let trace_res solver_ppf res =
  match solver_ppf with
  | Some ppf ->
...
let create_instance ?produce_models ?produce_proofs ?produce_unsat_cores
    ?produce_unsat_assumptions logic id =
  let arith_only =
    let open TermLib in
  ...
  let solver_cmd =
    YicesDriver.cmd_line logic 0 produce_models produce_proofs
      produce_unsat_cores produce_unsat_assumptions false false
  in

  let config = { solver_cmd; solver_arith_only = arith_only } in
  ...
  let solver_pid =
    Unix.create_process solver_executable solver_cmd solver_stdin_in
      solver_stdout_out solver_stderr_out
  in
  ...
  let header_logic solver =
    if solver.solver_config.solver_arith_only then [ "(set-arith-only! true)" ]
    else []
  in

  let headers =
    (Format.sprintf "(set-evidence! %B)" evidence :: header_logic solver)
    @ headers false
  in
...
let delete_instance
    ({ solver_pid; solver_stdin; solver_stdout; solver_stderr } as solver) =
  ...
  (try ignore (execute_command_no_response solver "(exit)" 0)
   with Signal s when s = Sys.sigpipe ->
     KEvent.log L_warn
       "[Warning] Got broken pipe when trying to exit %s instance PID %d.It \
  ...
  (* Exit with code *)
  | Unix.WEXITED c -> Debug.smt "Solver exited with code %d" c
  (* Killed by signal *)
  | Unix.WSIGNALED s -> Debug.smt "Solver killed with signal %d" s
  (* Stopped by signal *)
  | Unix.WSTOPPED s -> Debug.smt "Solver stopped by signal %d" s);
...
let trace_comment solver comment = solver.solver_trace_coms comment

module Create (P : SolverSig.Params) : SolverSig.Inst = struct
  module Conv = Conv

  let solver =
    create_instance ~produce_models:P.produce_models
      ~produce_unsat_cores:P.produce_unsat_cores
      ~produce_unsat_assumptions:P.produce_unsat_assumptions
      ~produce_proofs:P.produce_proofs P.logic P.id
