...
module Make (Driver : SMTLIBSolverDriver) : SolverSig.S = struct
  ...
  type t = {
    ...
    solver_lexbuf : Lexing.lexbuf;
        (* Lexing buffer on solver's
                                           stdout *)
    solver_stdout : Unix.file_descr;
        (* File descriptor of solver's
                                           stdout *)
    solver_stderr : Unix.file_descr;
        (* File descriptor of solver's
                                           stderr *)
    solver_trace_cmd : string -> unit; (* Tracing function for commands *)
  ...
  let response_error e =
    match e with
    | HStringSExpr.Atom s when s == s_timeout -> `Timeout
    | e ->
        raise
          (Failure ("Invalid solver response " ^ HStringSExpr.string_of_sexpr e))
  ...
  let response_of_sexpr : HStringSExpr.t -> decl_response = function
    ...
    (* Error *)
    | HStringSExpr.List [ HStringSExpr.Atom s; HStringSExpr.Atom e ]
      when s == s_error ->
  ...
  let rec get_value_response_of_sexpr' accum = function
    ...
    | HStringSExpr.List [ e; v ] :: tl ->
        Debug.smtexpr "get_value_response_of_sexpr: %a is %a"
          HStringSExpr.pp_print_sexpr e HStringSExpr.pp_print_sexpr v;
    ...
    | HStringSExpr.List [ e; v; HStringSExpr.Atom d ] :: tl
      when d == HString.mk_hstring ".0" ->
        get_value_response_of_sexpr' accum (HStringSExpr.List [ e; v ] :: tl)
    | e :: _ ->
        Debug.smtexpr "get_value_response_of_sexpr: %a"
  ...
  let rec get_model_response_of_sexpr' accum = function
    | [] -> `Model accum
    | e :: tl -> (
        Debug.smtexpr "get_model_response_of_sexpr: %a"
          HStringSExpr.pp_print_sexpr e;
        ...
        try
          ...
          let u = UfSymbol.uf_symbol_of_string (HString.string_of_hstring s) in
  ...
  let ignore_model_item = function
    (* (declare-datatype ...) *)
    | HStringSExpr.List (HStringSExpr.Atom s :: _) when s == s_declare_datatypes
      ->
        true
    (* (define-fun __compress_equal_mod_input ...) *)
    | HStringSExpr.List (HStringSExpr.Atom s :: HStringSExpr.Atom c :: _)
      when s == s_define_fun && c == s__compress_equal_mod_input () ->
        true
    | _ -> false
  ...
  let get_model_response_of_sexpr = function
    ...
    | HStringSExpr.List (HStringSExpr.Atom s :: l) when s == s_model ->
  ...
  let rec get_custom_command_result solver accum = function
    ...
    | i ->
        get_custom_command_result solver
  ...
  let send_command cmd_type ({ solver_stdin; solver_stderr } as solver) command
      timeout =
  ...
  let declare_sort solver sort =
    ...
    | Type.Abstr _ ->
        let cmd =
          Format.sprintf "@[<hv 1>(declare-sort@ %s 0)@]" (string_of_sort sort)
        in
  ...
  let declare_fun solver fun_symbol arg_sorts res_sort =
    let cmd =
      Format.sprintf "@[<hv 1>(declare-fun@ %s@ @[<hv 1>%s@]@ %s)@]" fun_symbol
  ...
  let define_fun solver fun_symbol arg_vars res_sort defn =
    let cmd =
      Format.asprintf "@[<hv 1>(define-fun@ %s@ @[<hv 1>(%a)@]@ %s@ %a)@]"
        fun_symbol
        (pp_print_list
           (fun ppf var ->
           ...
           "@ ")
        arg_vars (string_of_sort res_sort) pp_print_expr defn
    in
  ...
  let assert_expr solver expr =
    let cmd =
      Format.sprintf "@[<hv 1>(assert@ @[<hv>%s@])@]" (string_of_expr expr)
    in
  ...
  let assert_soft_expr solver expr weight =
    let cmd =
      Format.sprintf "@[<hv 1>(assert-soft@ @[<hv>%s@] :weight %d)@]"
        (string_of_expr expr) weight
    in
  ...
  let get_unsat_assumptions solver =
    ...
    let cmd =
      ...
      else Format.sprintf "@[<hv 1>(get-unsat-core)@]"
    in
  ...
  let create_trace_ppf id =
    ...
    if Flags.Smt.trace () then (
      ...
      let trace_filename =
        Filename.concat tdir
          (Format.sprintf "%s.%s.%s%d.%s"
             ...
             !trace_suffix id trace_extension)
      in
      ...
      try
        ...
        KEvent.log L_debug "Tracing output of SMT solver instance to %s"
          trace_filename;
  ...
  let set_commented_formatter ppf =
    ...
    let rec iter_line f g s p i n =
      ...
      if n = 0 then () (* Apply [f] at the end of the string *)
      else if i >= n then f s p n
      else
    ...
    let out_string s p n =
      iter_line fmt_out_fun.Format.out_string out_newline s p 0 n
    in
  ...
  let create_instance ?(timeout = 0) ?(produce_models = false)
      ?(produce_proofs = false) ?(produce_unsat_cores = false)
      ?(produce_unsat_assumptions = false) ?(minimize_cores = false)
      ?(produce_interpolants = false) logic id =
    (* Get autoconfigured configuration *)
    let solver_cmd =
      Driver.cmd_line logic timeout produce_models produce_proofs
        produce_unsat_cores produce_unsat_assumptions minimize_cores
        produce_interpolants
    in
    let config = { solver_cmd } in
    ...
    let define_bv2int =
      match logic with
      | `Inferred l ->
          if TermLib.FeatureSet.mem BV l && TermLib.FeatureSet.mem IA l then
            true
          else false
      | _ -> false
    in
    ...
    let header_bv2int =
      [
        "(define-fun int8_to_int ((x (_ BitVec 8))) Int (ite (bvsle x (_ bv0 \
         8)) (- (bv2nat (bvneg x))) (bv2nat x)))";
        "(define-fun int16_to_int ((x (_ BitVec 16))) Int (ite (bvsle x (_ bv0 \
         16)) (- (bv2nat (bvneg x))) (bv2nat x)))";
        "(define-fun int32_to_int ((x (_ BitVec 32))) Int (ite (bvsle x (_ bv0 \
         32)) (- (bv2nat (bvneg x))) (bv2nat x)))";
        "(define-fun int64_to_int ((x (_ BitVec 64))) Int (ite (bvsle x (_ bv0 \
         64)) (- (bv2nat (bvneg x))) (bv2nat x)))";
      ]
    in

    let headers =
      ("(set-option :print-success true)" :: headers minimize_cores)
      @ (if produce_models then [ "(set-option :produce-models true)" ] else [])
      @ (if
           produce_unsat_cores
           || (produce_unsat_assumptions && not (Flags.Smt.check_sat_assume ()))
         then [ "(set-option :produce-unsat-cores true)" ]
         else [])
      @ (if produce_unsat_assumptions && Flags.Smt.check_sat_assume () then
           [ "(set-option :produce-unsat-assumptions true)" ]
         else [])
      @ (if produce_interpolants then
           [
             Format.sprintf "(set-option :produce-interpolants %B)"
               produce_interpolants;
           ]
         else [])
      @ header_logic @ header_farray
  ...
  module Create (P : SolverSig.Params) : SolverSig.Inst = struct
    ...
    let solver =
      create_instance ~timeout:P.timeout ~produce_models:P.produce_models
        ~produce_unsat_cores:P.produce_unsat_cores
        ~produce_unsat_assumptions:P.produce_unsat_assumptions
        ~minimize_cores:P.minimize_cores ~produce_proofs:P.produce_proofs
        ~produce_interpolants:P.produce_interpolants P.logic P.id
