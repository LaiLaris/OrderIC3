...
(** All solver instances created are stored in this map from solver id to
    solver. The main goal of this is to have all the live solver instances in
    one place so that we can kill everyone easily in case of unexpected
    shutdown.
...
(** Forgets a solver. *)
let drop_solver { id } = all_solvers := IntMap.remove id !all_solvers
...
let fail_on_smt_error s = function
  ...
  | `Unsupported -> raise (Failure "SMT solver reported not implemented")
  | `NoResponse -> raise (Failure "SMT solver did not produce a reply")
  | _ -> ()
...
let bool_of_bool_option = function None -> false | Some b -> b
let bool_of_int_option = function None -> 0 | Some i -> i

(* Create a new instance of an SMT solver, declare all currently created
   uninterpreted function symbols *)
let create_instance ?timeout ?produce_models ?produce_proofs
    ?produce_unsat_cores ?produce_unsat_assumptions ?minimize_cores
    ?produce_interpolants l kind =
  ...
  let module Params = struct
    ...
    let produce_unsat_assumptions =
      bool_of_bool_option produce_unsat_assumptions
  ...
  let fomodule =
    match kind with
    | `Bitwuzla_SMTLIB ->
        (module BitwuzlaSMTLIB.Create (Params) : SolverSig.Inst)
    ...
    | `SMTInterpol_SMTLIB ->
        (module SMTInterpolSMTLIB.Create (Params) : SolverSig.Inst)
...
let destroy_all () =
  !all_solvers |> IntMap.iter (fun _ s -> destroy s);
  all_solvers := IntMap.empty

(** Delete instance entries (should be called after forking, on child
    processes). *)
let delete_instance_entries () = all_solvers := IntMap.empty
    ...
    (function
      | v, e ->
         ...
         to a term *)
          ...
          let tv', te' = (Var.type_of_var v', Term.type_of_term e') in

          (* Hack to make integer values match reals *)
          let e'' =
            if
              (* Rhs is of type real, variable is of type integer? *)
              Type.is_real tv' && (Type.is_int te' || Type.is_int_range te')
            then
...
let range (l, u) =
  let rec aux acc u = if u < l then acc else aux (u :: acc) (u - 1) in
  aux [] u
...
let cross_2 l1 l2 =
  List.fold_left
    (fun acc i1 -> List.fold_left (fun acc i2 -> (i1 :: i2) :: acc) acc l2)
    [] l1
...
let get_var_values s state_var_indexes vars =
  ...
  let sexpr_vars, array_vars =
    List.fold_left
      (fun (sexpr_vars, array_vars) v ->
        if Var.type_of_var v |> Type.is_array then (sexpr_vars, v :: array_vars)
        else (S.Conv.smtexpr_of_var v [] :: sexpr_vars, array_vars))
      ([], []) vars
  in
  ...
  List.iter
    (fun v ->
      ...
      in
      let indexes =
        StateVar.StateVarHashtbl.find state_var_indexes
          ...
          List.map
            (function
              | LustreExpr.Unbound _ ->
                  raise Exit
                  (* assert false *)
                  (* no models for unbounded arrays *)
              | LustreExpr.Fixed eu | LustreExpr.Bound eu -> (
                  if LustreExpr.is_numeral eu then
                    ...
                    let ub =
                      LustreExpr.unsafe_term_of_expr eu
                      |> Term.bump_state offset |> Eval.eval_term [] model
                    in
                    match ub with
      ...
      let sexprs =
        List.map
          (fun args ->
            List.fold_left Term.mk_select vt
              (List.rev_map Term.mk_num_of_int args)
            |> Term.convert_select |> S.Conv.smtexpr_of_term)
          args_list
      in

      let values =
        if sexprs = [] then []
          (* when the size of the array is 0 in the model *)
        else
...
let get_unsat_core_lits s =
  ...
  | `Unsat_core c ->
      ...
      if S.check_sat_assuming_supported () then
        ...
        List.fold_left
          (fun a s ->
            try Term.mk_uf (UfSymbol.uf_symbol_of_string s) [] :: a
...
let check_sat_assuming s if_sat if_unsat literals =
  ...
  else (* Solver does not support check-sat-assuming, doing
       push/pop. *)
    (* Pushing. *)
    let _ = push s in
    ...
    let next_assumption_id' =
      List.fold_left
        (fun i l ->
          ...
          let l' = Term.mk_named_unsafe l unsat_core_namespace i in
...
type ('a, 'b) sat_or_unsat = Sat of 'a | Unsat of 'b
...
let check_sat_assuming_tf s literals =
  check_sat_assuming s (fun _ -> true) (fun _ -> false) literals
  ...
  | `MathSAT_SMTLIB ->
      List.init
        ...
        (fun i ->
          ...
          match
            execute_custom_command solver "get-interpolant" [ groups ] 1
          with
          | `Custom r ->
...
let rec conj_of_goal accum = function
  ...
  | HStringSExpr.Atom a :: _ when HString.sub a 0 1 = ":" -> List.rev accum
...
let goals_to_terms solver = function
  ...
  | HStringSExpr.List [ HStringSExpr.Atom s; HStringSExpr.List g ]
    when s == s_goals ->
      let module S = (val solver.solver_inst) in
      goal_to_term (HStringSExpr.List g)
      |> List.map (fun t ->
             S.Conv.term_of_smtexpr (GenericSMTLIBDriver.expr_of_string_sexpr t))
  (* Invalid S-expression as result *)
...
let get_qe_expr solver quantified_expr =
  ...
  | _ ->
      failwith
        "Quantifier elimination is not supported by SMT solver or \
...
let normalize_if_inconsistent solver term =
  ...
  let term' = if check_sat solver then term else Term.t_false in
  pop solver;
  term'
...
let get_qe_interpolants fwd solver groups =
  let get_interpolant solver t1 t2 =
    ...
    | _ ->
        ...
        get_qe_term solver forall_term |> Term.mk_and |> simplify_term solver
  in

  let get_interpolant =
    if fwd then fun s t1 t2 -> get_interpolant s t2 t1 |> Term.negate
    else get_interpolant
  in
  ...
  List.fold_left
    (fun itps (a, b) ->
      ...
      let i =
        get_interpolant solver (Term.mk_and [ prev_itp; a ]) (Term.mk_and b)
        |> normalize_if_inconsistent solver
      in
      let i = try Simplify.simplify_term ~split_eq:true [] i with _ -> i in
      i :: itps)
    [ Term.t_true ] pairs
  |> List.rev |> List.tl
