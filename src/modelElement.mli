...
(** Manpulation of sets of equations and model elements (equations, assertions,
    assumptions, guarantees, node calls). Make the link between TransSys-level
    equations and Lustre model elements.
...
(** Represents an equation of the transition system. It is not specific to the
    'equation' model elements of the source lustre program (any model element
    can be represented by this 'equation' type). Intuitively, a ts_equation
    corresponds to a conjunct of the transition system. However, some conjuncts
    might be grouped together if they are related. *)

type core
(** Represents a set of equations at the level of the transition system. The
    equations are separated by scope. The equations inside are not localized
    (i.e. they are not mapped to the initial Lustre model). This type can be
    used when manipulating sets of equations: many operations are implemented
    (cores can be filtered, you can get an activation litteral or state variable
    for each equation, etc.) *)
...
val add_from_other_core :
  core (* src *) -> Scope.t -> UfSymbol.t -> core (* dst *) -> core
...
type model_element
(** Represents an equation at the level of the transition system together with
    localisation data that relates it to the original Lustre model. *)

type loc_core
(** Represents a set of localized equations ([model_element]). The equations are
    separated by scope. This type can be used to store a set of equations
    together with their localization data and to perform some basic
    manipulations such as filtering the equations by Lustre category. However,
    the type [core] should be prefered for manipulations at the level of the
    transition system. *)
...
val ts_equation_to_model_element :
  'a InputSystem.t -> ts_equation -> model_element
...
val filter_loc_core_by_categories :
  Scope.t (* Toplevel scope *) ->
  category list ->
  loc_core ->
  loc_core * loc_core

(* Separate contract guarantees and mode elements from the rest of the model elements *)
val partition_loc_core_elts_by_guarantees_and_mode_elts :
  loc_core -> loc_core * loc_core
...
val attach_approx_to_print_data : core_print_data -> bool -> core_print_data
