...
let list_transpose =
  let rec list_transpose' accum =
    let rec list_transpose'' accum = function
      | [] -> (
          function [] -> List.rev accum | _ -> invalid_arg "list_transpose")
      | h :: tla -> (
          function
  ...
  | h :: _ as l -> list_transpose' (List.map (fun _ -> []) h) l
...
let fold_pairs f accum l =
  let rec fold_pairs' f accum l = function
    ...
    | h :: s :: tl -> fold_pairs' f (f accum h s) l (h :: tl)
  in
...
let function_symbol_name () = !equal_mod_input_string
...
let only_bv trans_sys =
  ...
  | `SMTLogic "QF_AUFBV" ->
      true
  | `SMTLogic _ | `None -> false
  | `Inferred l when TermLib.FeatureSet.(subset l (of_list [ Q; UF; A ])) -> (
      match Flags.Smt.solver () with `Bitwuzla_SMTLIB -> true | _ -> false)
...
let init_equal_mod_input declare_fun trans_sys =
  ...
  let unconstrained_inputs = TransSys.unconstrained_inputs trans_sys in

  let uf_distinct =
    UfSymbol.mk_uf_symbol !equal_mod_input_string
      (List.fold_left
         (fun a sv ->
         ...
         []
         (List.sort StateVar.compare_state_vars (TransSys.state_vars trans_sys)))
...
let equal_mod_input only_bv unc_inputs accum s1 s2 =
  let uf_distinct = UfSymbol.uf_symbol_of_string !equal_mod_input_string in
  ...
  if
    ...
    List.for_all2
      (fun (v1, val1) (v2, val2) ->
        ...
        assert (
          StateVar.equal_state_vars sv1 (Var.state_var_of_state_var_instance v2));
  ...
  then (
    ...
    let term =
      ...
      if true then
        ...
        let term_of_state s =
          let n =
            if only_bv then Term.mk_ubv (Bitvector.num_to_ubv64 (aux s))
            else Term.mk_num (aux s)
          in
          Term.mk_eq
            [
              Term.mk_uf uf_distinct
                (List.fold_right
                   (fun (v, _) a ->
                     if
                       not
          ...
          (List.fold_left2
             (fun a (v1, _) (v2, _) ->
               ...
               if not (StateVar.StateVarSet.mem sv1 unc_inputs) then
                 ...
                 Term.negate (Term.mk_eq [ Term.mk_var v1; Term.mk_var v2 ])
                 :: a
               else a)
             [] s1 s2)
    in

    Debug.compress "Compression clause@ %a" Term.pp_print_term term;
    ...
    (fun _ t ->
      ...
      if Term.is_free_var t then
        ...
        if
          ...
          && Numeral.equal (Var.offset_of_state_var_instance v) Numeral.zero
        then
          ...
          Var.is_state_var_instance v
          && Numeral.equal (Var.offset_of_state_var_instance v) Numeral.one
        then
          (* State variable *)
...
let same_successors declare_fun uf_defs trans accum sj si =
  ...
  then (
    KEvent.log L_debug "Not considering state pair (%a,%a)"
    ...
    | Eval.ValBool false ->
        ...
        accum
    | Eval.ValTerm t ->
        KEvent.log L_debug "@[<v>Transition system evaluates to@,@[<hv>%a@]@]"
...
let init declare_fun trans_sys = init_equal_mod_input declare_fun trans_sys
...
let check_and_block declare_fun trans_sys path =
  ...
  let states =
    list_transpose
      (List.map
         (fun (sv, t) ->
           List.mapi
             (fun i t -> (Var.mk_state_var_instance sv (Numeral.of_int i), t))
             t)
         (List.sort
  ...
  let block_terms =
    if Flags.BmcKind.compress_equal () then
      let unc_inputs = TransSys.unconstrained_inputs trans_sys in

      fold_pairs
        (equal_mod_input (only_bv trans_sys) unc_inputs)
        block_terms states
    else block_terms
  in
