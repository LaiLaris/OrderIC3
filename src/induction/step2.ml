...
let deactivate solver actlit = Term.mk_not actlit |> Smt.assert_term solver

(* Asserts an invariant at [k]. *)
let assert_invariant_at solver k inv = unroll k inv |> Smt.assert_term solver
...
let mk_ctx in_sys param sys =
  ...
  Sys.define_and_declare_of_bounds sys (Smt.define_fun solver)
    (Smt.declare_fun solver) (Smt.declare_sort solver) Numeral.zero
    Numeral.(succ one);
...

  {
    ...
    map =
      Sys.get_prop_status_all_unknown sys
      |> List.fold_left
           (fun map (name, _) ->
...
let rec check_new_things new_stuff ({ solver; sys; map } as ctx) =
  ...
  in
  let new_stuff = new_stuff || new_stuff_invs in
  ...
  | _ ->
      let map, new_stuff =
        map
        |> List.fold_left
             (fun (* Go through map and inspect property status. *)
                    (map, new_stuff) ((name, (pos, _)) as p) ->
               ...
               | Prop.PropInvariant _ ->
                   ...
                   deactivate solver pos;
                   (map, true)
               | Prop.PropKTrue n -> (p :: map, new_stuff || n >= 1)
               | _ ->
                   (* Still unknown. *)
                   (p :: map, new_stuff))
             ([], new_stuff)
...
let split { solver; map } =
  let rec loop falsifiable =
    if List.length falsifiable = List.length map then
      ...
      let actlits, unknowns, map_back =
        ...
        |> List.fold_left
             (fun (actlits, terms, map_back) (prop, (pos, term)) ->
               ...
               if List.mem prop falsifiable |> not then
      ...
      match
        Smt.check_sat_assuming_and_get_term_values solver
          (fun _ term_values ->
            ...
            |> List.fold_left
                 (fun l (term, value) ->
                   if value == Term.t_false then List.assq term map_back :: l
                   else l)
                 []
            |> fun l -> Some l)
          (fun _ ->
      ...
      with
      | None ->
          (* Unsat, remaining properties are unfalsifiable. *)
          deactivate ();
          unknowns |> List.map (fun t -> List.assq t map_back)
      | Some [] -> failwith "got empty list of falsifiable properties"
      | Some nu_falsifiable ->
          deactivate ();
  ...
  let rec loop confirmed = function
    | prop :: tail -> (
        let ok_cert =
          ...
          | Prop.PropKTrue n when n >= 1 -> Some (2, Sys.get_prop_term sys prop)
          | Prop.PropInvariant ((k, _) as cert) ->
              if k <= 2 then Some cert else Some (2, Sys.get_prop_term sys prop)
          | _ -> None
        in
        match ok_cert with
        | Some cert ->
...
let rec run ctx =
  ...
  let new_stuff =
    ...
    | [] ->
        KEvent.log L_info "%s@[<v>%d falsifiable properties@]" prefix
          (List.length ctx.map);
        false
    | unfalsifiable ->
        KEvent.log L_info "%s@[<v>Proved %d properties%t@]" prefix
          (List.length unfalsifiable) (fun fmt ->
            match ctx.map with
            | [] -> ()
            | _ ->
                Format.fprintf fmt "@ %d falsifiable properties"
                  (List.length ctx.map));
  ...
  | [] ->
      ...
      KEvent.log L_info "%s@[<v>No more properties to analyze.@]" prefix;
      ()
  | _ ->
...
let main in_sys param sys =
  ...
  | [] ->
      ...
      KEvent.log L_info "%s@[<v>No properties to analyze.@]" prefix;
      ()
  | _ ->
      KEvent.log L_info "%s@[<v>%d properties to check@]" prefix
        (List.length ctx.map);
      ctx |> run
