...
let shall_keep trans (s, _) =
  ...
  | Property.PropInvariant _ | Property.PropFalse _ -> false
  | _ -> true
...
let split trans solver k to_split actlit =
  ...
  let if_sat _ =
    ...
    let cex = Model.path_from_model (TransSys.state_vars trans) model k in
    ...
    let new_to_split, new_falsifiable =
      List.partition (fun (_, term) -> Term.bump_state k term |> eval) to_split
    in
    (* Building result. *)
  ...
  Format.asprintf "Looking for falsification at %a." Numeral.pp_print_numeral k
...
let split_closure trans solver k to_split =
  let rec loop falsifiable list =
    ...
    let deactivate () = actlit |> Term.mk_not |> SMTSolver.assert_term solver in
    (* Asserting implication. *)
    Term.mk_implies [ actlit; term ] |> SMTSolver.assert_term solver;
    (* Splitting. *)
...
let skip_steps_next trans solver k unknowns =
  let step = ref k in
  let llb = Numeral.of_int (lowest_lower_bound trans unknowns) in
  while Numeral.(!step < llb) do
    step := Numeral.succ !step;

    (* Declaring unrolled vars at k+1. *)
    TransSys.declare_vars_of_bounds trans
    ...
    | Some ctr ->
        SMTSolver.assert_term solver
          (Term.mk_eq
             [
               Term.mk_var (Var.mk_state_var_instance ctr !step);
               Term.mk_num !step;
             ])
    | None -> ()
  done;
...
let rec next (input_sys, aparam, trans, solver, k, unknowns, skip) =
  ...
  let all_invs =
    TransSys.invars_of_bound ~one_state_only:Numeral.(equal k zero) trans k
    |> Term.mk_and
  in
  if all_invs != Term.t_true then SMTSolver.assert_term solver all_invs;
  ...
  | _ ->
      ...
      if Numeral.(k > zero) then
        ...
        |> List.filter (fun (name, _) ->
               ...
               | Reachable (Some (From b)) | Reachable (Some (At b)) ->
                   b < k_int
      ...
      let unknowns_at_k, k_true =
        ...
        |> List.partition (fun (name, _) ->
               match TransSys.get_prop_status trans name with
      ...
      let unfalsifiable =
        ...
        | [] ->
            KEvent.log L_info "BMC @[<v>at k = %i@,skipping@]" k_int;
            nu_unknowns
        | _ ->
            (* Output current progress. *)
            KEvent.log L_info "BMC @[<v>at k = %i@,%i properties.@]" k_int
              (List.length unknowns_at_k);

            Format.asprintf "%a unrolling satisfiability check."
            ...
            if Flags.BmcKind.check_unroll () then
              if SMTSolver.check_sat solver |> not then (
                KEvent.log L_warn
                  "BMC @[<v>Unrolling of the system is unsat at %a, the system \
                   has no more reachable states.@]"
                  Numeral.pp_print_numeral k;
            ...
            |> List.iter (fun (s, _) ->
                   ...
                   | Reachable (Some (Within b))
                     when b <= Numeral.to_int k ->
                       KEvent.prop_status
                         (Property.PropInvariant
                            ( Numeral.to_int k,
                              (TransSys.property_of_name trans s)
                                .Property.prop_term ))
                         input_sys aparam trans s
      ...
      | Some ctr ->
          SMTSolver.assert_term solver
            (Term.mk_eq
               [
                 Term.mk_var (Var.mk_state_var_instance ctr k_p_1);
                 Term.mk_num k_p_1;
               ])
      | None -> ());
      ...
      if Flags.BmcKind.max () > 0 && k_p_1_int > Flags.BmcKind.max () then
        KEvent.log L_info "BMC @[<v>reached maximal number of iterations.@]"
      else
        (* Looping. *)
        next (input_sys, aparam, trans, solver, k_p_1, unfalsifiable, skip)
...
let init input_sys aparam trans skip =
  ...
  let unknowns =
    if skip then TransSys.props_list_of_bound_skip trans Numeral.zero
    else TransSys.props_list_of_bound_no_skip trans Numeral.zero
  in

  (* Creating solver. *)
  let solver =
    SMTSolver.create_instance ~produce_models:true (TransSys.get_logic trans)
      (Flags.Smt.solver ())
  in

  (* Defining uf's and declaring variables. *)
  TransSys.define_and_declare_of_bounds trans
  ...
  |> List.iter (fun (name, _) ->
         ...
         | Reachable (Some (At lower_bound)) ->
             broadcast_k_true lower_bound name
         | _ -> ());
  ...
  let _ =
    ...
    | Some ctr ->
        SMTSolver.assert_term solver
          (Term.mk_eq
             [
               Term.mk_var (Var.mk_state_var_instance ctr Numeral.zero);
...
let main skip input_sys aparam trans =
  try init input_sys aparam trans skip |> next
