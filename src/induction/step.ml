...
let split_unfalsifiable_rm_proved trans k =
  List.fold_left
    (fun (dis, true_k, others) ((s, t) as p) ->
      match TransSys.get_prop_status trans s with
      | Property.PropInvariant _ -> (dis, true_k, others)
      | Property.PropFalse _ -> (p :: dis, true_k, others)
      | Property.PropKTrue n when n >= k -> (dis, (s, t, k) :: true_k, others)
      | _ -> (dis, true_k, p :: others))
    ([], [], [])
...
let eval_terms_assert_first_false trans solver eval k =
  ...
  let rec loop_all_k k' =
    if Numeral.(k' > k) then false
      ...
      let blocked =
        (if Numeral.(equal k' zero) then os else ts) |> loop_at_k k'
      in
      if blocked
      (* Blocked, returning. *)
      then true
...
let split (input_sys, analysis, trans) solver k to_split actlits =
  ...
  let if_sat _ =
    ...
    let model =
      ...
      then
        ...
        TransSys.vars_of_bounds trans k k |> SMTSolver.get_var_values solver svi
    in
  ...
  let rec loop () =
    match
      ...
      SMTSolver.check_sat_assuming solver if_sat if_unsat all_actlits
    with
    | Some model -> (
        (* Evaluation function. *)
        let term_to_val = Eval.eval_term (TransSys.uf_defs trans) model in
        (* Bool evaluation function. *)
        let eval term = term_to_val term |> Eval.bool_of_value in
        ...
        then loop ()
          ...
          in
          let cex = if print_cex then Model.path_to_list path else [] in

          (* Attempting to compress path. *)
          match
            if Flags.BmcKind.compress () then
              Compress.check_and_block (SMTSolver.declare_fun solver) trans path
            else []
          with
          | [] ->
          ...
          | compressor ->
              ...
              Term.mk_or
                [ path_comp_act_term |> Term.mk_not; compressor |> Term.mk_and ]
...
(* Splits its input list of properties between those that can be
   ...
   are not overloaded. *)
let split_closure ((_, _, _) as sys) solver k optimistic_actlits
    optimistic_terms to_split =
  let rec loop falsifiable list =
    ...
    let neg_term =
      list
      |> List.map (fun pair -> snd pair |> snd)
    ...
    let term =
      neg_term
      :: (optimistic_terms
         |> List.map (fun (_, (_, t)) -> t |> Term.bump_state k))
      |> Term.mk_and
    in
    ...
    Term.mk_implies [ actlit_term; term ] |> SMTSolver.assert_term solver;
...
let rec next input_sys aparam trans solver k unfalsifiables unknowns =
  ...
  | _ when Flags.BmcKind.max () > 0 && k_int + 1 > Flags.BmcKind.max () ->
      KEvent.log L_warn "IND @[<v>reached maximal number of iterations.@]"
  | _ ->
      ...
      if Flags.BmcKind.compress () then Compress.incr_k ();
      ...
      unfalsifiable_impls |> Term.mk_and |> SMTSolver.assert_term solver;

      (* Output current progress. *)
      KEvent.log L_info "IND @[<v>at k = %i@,%i unknowns@,%i unfalsifiables.@]"
        k_p_1_int (List.length unknowns')
        (List.length unfalsifiable_props);

      (* Splitting. *)
      let unfalsifiables_at_k, falsifiables_at_k =
        split_closure (input_sys, aparam, trans) solver k_p_1
          unfalsifiable_actlits unfalsifiable_props unknowns'
      in
      ...
      (* Looping. *)
      next input_sys aparam trans solver k_p_1
  ...
  let logic =
    ...
    | `Inferred fs as l when Flags.BmcKind.compress () ->
        ...
        *)
          ...
          l)
        else if Compress.only_bv trans then
...
let main input_sys aparam trans =
  if not (List.mem `BMC (Flags.enabled ())) then
    KEvent.log L_warn
      "@[<v>Inductive step without BMC will not be able to prove or@ disprove \
       any properties.@,\
