...
type value = Term of Term.t | Lambda of Term.lambda | Map of Term.t MIL.t
...
let pp_print_term ppf term =
  ...
  if Term.is_numeral term then
    ...
    Numeral.pp_print_numeral ppf (Term.numeral_of_term term)
    (* Constant is a decimal? *)
  else if Term.is_decimal term then
    (* Pretty-print as a decimal *)
    pp_print_decimal ppf (Term.decimal_of_term term)
  else
    (LustreExpr.pp_print_expr false) ppf (LustreExpr.unsafe_expr_of_term term)
...
let width_val_of_map m =
  MIL.fold (fun _ v -> max (width_of_string (string_of_t pp_print_term v))) m 0

let pp_print_value_term as_type ppf t =
  match as_type with
  | Some ty when Term.is_numeral t && Type.is_enum ty -> (
      try
        Term.numeral_of_term t |> Type.get_constr_of_num
    ...
    MIL.iter
      (fun l v ->
        ...
        let ty =
          ...
          | _ -> None
        in
        Format.fprintf ppf "%*s%a" (val_width - w) "" (pp_print_value_term ty) v;
        first := false)
      m;
    for _ = 1 to dim do
      Format.fprintf ppf "]@]"
...
type array_model = ItemValue of Term.t | ItemArray of int * array_model array

let rec allocate_model sizes =
  match sizes with
  | [] -> ItemValue Term.t_false
  | s :: rs -> ItemArray (s, Array.init s (fun _ -> allocate_model rs))

let dimension_of_map m =
  MIL.fold (fun l _ acc -> List.map2 max acc l) m (fst @@ MIL.choose m)
  |> List.map succ
...
let map_to_array_model m =
  allocate_model (dimension_of_map m) |> MIL.fold add_at_indexes m

let pp_print_value_term_xml as_type ppf t =
  match as_type with
  | Some ty when Term.is_numeral t && Type.is_enum ty -> (
      try
        Term.numeral_of_term t |> Type.get_constr_of_num
  ...
  | Some ty when Type.is_bitvector ty || Type.is_ubitvector ty ->
      ...
      let bv_num =
        ...
        with Bitvector.NonStandardBVSize -> raise LustreExpr.Type_mismatch
      in
...
let rec pp_print_array_model_as_xml as_type top_level ppf index it =
  if not top_level then Format.fprintf ppf "@[<hv 2><Item index=\"%d\">@," index;
  (match it with
  | ItemValue v ->
      Format.fprintf ppf "@[<hv 2>%a@]" (pp_print_value_term_xml as_type) v
  | ItemArray (s, a) ->
      Format.fprintf ppf "@[<hv 2><Array size=\"%d\">@,%a@;<0 -2></Array>@]" s
        (pp_print_listi (pp_print_array_model_as_xml as_type false) "@,")
        (Array.to_list a));
...
let pp_print_map_as_xml as_type ppf m =
  if MIL.is_empty m then
    ...
    let as_type =
      match as_type with
      | Some ty when Type.is_array ty -> Some (Type.last_elem_type_of_array ty)
      | _ -> None
    in
    pp_print_array_model_as_xml as_type true ppf 0 arm

let pp_print_value_term_json as_type ppf t =
  match as_type with
  | Some ty when Term.is_numeral t && Type.is_enum ty ->
      let num_str =
...
let rec pp_print_array_model_as_json as_type ppf _ it =
  ...
  | ItemArray (_, a) ->
      Format.fprintf ppf "[%a]"
        (pp_print_listi (pp_print_array_model_as_json as_type) ", ")
        (Array.to_list a)
  | ItemValue v -> Format.fprintf ppf "%a" (pp_print_value_term_json as_type) v
...
let pp_print_value ?as_type ppf = function
  ...
  | Map m ->
      Format.fprintf ppf "@[<hov 0>%a@]" (pp_print_map_as_array as_type) m

let pp_print_value_xml ?as_type ppf = function
  | Term t -> pp_print_value_term_xml as_type ppf t
  | Lambda l -> Term.pp_print_lambda ppf l
  | Map m -> ( try pp_print_map_as_xml as_type ppf m with Not_found -> ())

let pp_print_value_json ?as_type ppf = function
  | Term t -> pp_print_value_term_json as_type ppf t
  | Lambda l -> Term.pp_print_lambda ppf l
  | Map m -> ( try pp_print_map_as_json as_type ppf m with Not_found -> ())

(* Pretty-print a model *)
let pp_print_model ppf model =
  Var.VarHashtbl.iter
    (fun v value ->
      Format.fprintf ppf "@[<hv 2>%a =@ %a@]@." Var.pp_print_var v
        (pp_print_value ~as_type:(Var.type_of_var v))
        value)
    model
...
let pp_print_path ppf path =
  SVT.iter
    (fun sv l ->
      Format.fprintf ppf "@[<hv 2>%a =@ %a@]@ " StateVar.pp_print_state_var sv
        (pp_print_list
           (pp_print_value ~as_type:(StateVar.type_of_state_var sv))
           ", ")
        l)
    path
...
let import_value = function
  ...
  | Map m ->
      Map (MIL.fold (fun l v acc -> MIL.add l (Term.import v) acc) m MIL.empty)
...
let of_list l =
  ...
  List.iter (fun (v, t) -> VT.add model v t) l;

  (* Return hash table *)
  model

(* Return an association list with the assignments in the model *)
let to_list m = VT.fold (fun v t_or_l a -> (v, t_or_l) :: a) m []
...
let path_of_list l =
  ...
  (* Add associations from list to hash table *)
  List.iter (fun (v, t) -> SVT.add path v t) l;
...
let path_of_term_list l =
  ...
  (* Add associations from list to hash table *)
  List.iter (fun (v, t) -> SVT.add path v (List.map (fun t -> Term t) t)) l;

  (* Return hash table *)
  path

(* Return an association list with the assignments of the path *)
let path_to_list p = SVT.fold (fun sv t_or_l a -> (sv, t_or_l) :: a) p []
...
let find_value_vi vi model =
  let rec find prev vi =
    try
      ...
      | _ -> va
    with Not_found -> (
      match prev with None -> raise Not_found | Some va -> va)
  in
  find None vi
          ...
          (fun state_var ->
            ...
            let value =
              ...
              with Not_found -> (
                ...
                | _ -> Term (TermLib.default_of_type ty))
            in
...
let model_at_k_of_path path k =
  ...
  SVT.iter
    (fun sv p ->
      ...
      let t_or_l =
        try List.nth p k_int
        with Failure _ -> raise (Invalid_argument "model_from_path")
      in

      (* Create variable at instant *)
      let var = Var.mk_state_var_instance sv k in
    ...
    (fun sv sv_path accum ->
      ...
      let models =
        if accum = [] then
          ...
          List.map (fun _ -> VT.create (SVT.length path)) sv_path
        else (
...
let map f_var f_val model =
  ...
  (* Apply functions to each variable and assignment and add results
     to fresh table *)
  VT.iter (fun v t_or_l -> VT.add model' (f_var v) (f_val t_or_l)) model;
...
let bump_var k model =
  map (fun v -> Var.bump_offset_of_state_var_instance v k) identity model
...
(* Combine assignments of two models into one. If a variable has an
    assignment in both models, it gets the assignment in the second
    model. *)
let merge model1 model2 = apply_and_merge (fun a b -> (a, b)) model1 model2
