  ...
  (try
   ...
   with Invalid_argument _ ->
     ...
     if not !flat_apply_warning_reported then (
       KEvent.log L_warn "Cannot mine invariants in quantified terms";
      ...
      | sys :: tail ->
          ...
          if List.mem_assoc sys result then
            ...
            let candidates =
              ...
              if Flags.Certif.only_user_candidates () then candidates
              else
                ...
                let candidates, flat_info =
                  ...
                  |>
                  if Flags.Invgen.mine_trans () then flat_apply trans flat_rules
                  else identity
                in
          ...
          in *)
            ...
            let result =
              ...
              |> (if Flags.Invgen.lift_candidates () then
                    Set.fold
                      (fun term map ->
                        Sys.instantiate_term_all_levels top_sys
                          TransSys.trans_base scope term two_state
                        |> fun (top, others) ->
                        top :: others
                        |> List.fold_left
                             (fun map (sys, terms) ->
                               insert_sys_terms (sys, Set.of_list terms) map)
                             map)
                      candidates
                  else identity)
            ...
            mk_sys_map result (List.concat [ Sys.get_subsystems sys; tail ])
      | [] ->
          (if top_only then get_last result else result)
          |>
          if two_state then identity
          else
            List.map (fun (sys, set) ->
...
let is_var_or_const term =
  match Term.destruct term with
  (* | Term.T.Attr (kid, _) -> is_var_or_const kid *)
  | Term.T.Var var ->
      Var.is_state_var_instance var
      && Var.state_var_of_state_var_instance var |> SVar.for_inv_gen
  | Term.T.Const _ -> true
  | _ -> false
...
module BoolRules = struct
  ...
  let enum_rule lo hi svar =
    let var = var_of svar in
    let rec loop lo set =
      if Num.(lo > hi) then set
      else loop (Num.succ lo) (Set.add (Term.mk_eq [ var; Term.mk_num lo ]) set)
    in
    loop lo
  ...
  let range_rule lo hi svar =
    ...
    | Some lo, None ->
        Set.union
          (Set.of_list
             [
               Term.mk_eq [ var; Term.mk_num lo ];
               Term.mk_leq [ Term.mk_num lo; var ];
             ])
    | None, Some hi ->
        Set.union
          (Set.of_list
             [
               Term.mk_eq [ var; Term.mk_num hi ];
               Term.mk_leq [ var; Term.mk_num hi ];
             ])
  ...
  let bool_term_rule flat set =
    ...
    | Term.T.App (sym, kids) ->
        if (not only_depth_one) || List.for_all is_var_or_const kids then
          ...
          | `OR | `XOR | `IMPLIES | `AND | `ITE | `IS_INT | `DISTINCT ->
              to_term_add flat set
          | _ -> set
        else set
    | _ -> set
  ...
  [lhs <= rhs]. *)
  let arith_op_synth flat set =
    match flat with
    | Term.T.App (sym, ([ kid1; kid2 ] as kids))
      when (is_var kid1 && is_const kid2) || (is_var kid2 && is_const kid1)
           (* ||
    ( (is_var kid1) && (is_var kid2) ) *) -> (
        let add_ineqs () =
          set |> Set.add (Term.mk_geq kids) |> Set.add (Term.mk_leq kids)
  ...
  let svar_rules svars set =
    ( svars
      |> List.fold_left
           (fun set svar ->
             if SVar.for_inv_gen svar |> not then set
             else
  ...
  let flat_rules flat (set, _) =
    ...
    | Type.Bool ->
        ( (arith_op_synth flat set
          |> if Flags.Invgen.all_out () then bool_term_rule flat else identity),
          () )
    | _ -> (set, ())
...
let eval term = Eval.eval_term [] empty_model term |> Eval.term_of_value

let generic_octagons mk_plus mk_minus oct3 f =
  let rec octagons_3 terms tail' set =
    match tail' with
    | term :: tail ->
    ...
    | [] -> set
  in
  let rec octagons_2_3 term terms set =
    match terms with
    | term' :: tail ->
...
module IntRules = struct
  ...
  let svar_rules svars set =
    ...
    let svars, set =
      ...
      |> List.fold_left
           (fun (svars, set) svar ->
             match type_of_svar svar with
             | Type.Int ->
                 if SVar.for_inv_gen svar then (var_of svar :: svars, set)
                 else (svars, set)
             | Type.IntRange (lo, hi) ->
                 let set =
                   match lo with
                   | None -> set
                   | Some lo -> Set.add (Term.mk_num lo) set
                 in
                 let set =
                   match hi with
                   | None -> set
                   | Some hi -> Set.add (Term.mk_num hi) set
                 in
                 (svars, set)
             | _ -> (svars, set))
           ([], set)
    ...
    let set = octagons (Flags.Invgen.all_out ()) identity svars set in
  ...
  let flat_rules flat (set, constants) =
    ...
    | Type.Int | Type.IntRange _ -> (
        match flat with
        | Term.T.App (_, kids) ->
            if Flags.Invgen.all_out () || kids |> List.for_all is_var_or_const
            then (Set.add term set, constants)
            else (set, constants)
        | Term.T.Const sym -> (
            match Symbol.node_of_symbol sym with
        ...
        | Term.T.Var _ -> (Set.add term set, constants))
    | _ -> (set, constants)

  let post_rules _ constants set =
    let zero, one = (Term.mk_num zero, Term.mk_num one) in
    let set =
      Set.add zero set |> Set.add one
      |> octagons true eval (Set.add one constants |> Set.elements)
    in
    let set =
      Set.fold
...
module MachineIntegerRules (M : MachineIntegerSig) = struct
  ...
  let svar_rules svars set =
    ...
    let set = octagons_bv (Flags.Invgen.all_out ()) identity svars set in
    (* Format.printf "set: @[<v>%a@]@.@."
      (pp_print_list fmt_term "@ ")
      (Set.elements set) ; *)
    (set, ())
  ...
  let flat_rules flat (set, constants) =
    ...
    if M.is_type (Term.type_of_term term) then
      ...
      | Term.T.Const sym ->
          if M.is_symbol sym then (Set.add term set, Set.add term constants)
          else failwith "Type of constant and term does not match"
      (* | Term.T.Attr (term, _) ->
        flat_rules (Term.destruct term) (set, constants)*)
      | Term.T.Var _ -> (Set.add term set, constants)
    else (set, constants)
  ...
  let post_rules _ constants set =
    ...
    let set =
      Set.add zero set |> Set.add one
      |> octagons_bv true eval (Set.add one constants |> Set.elements)
    in
    let set =
      Set.fold
