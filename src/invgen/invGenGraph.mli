...
(** Graph representing equivalence classes and ordering between some terms.
    ...
    There's one graph module per domain we are interested in: bool, int and
    real. They're generated by a common functor using the domains from
    [InvGenDomain]. *)
...
val write_dot_to :
  string -> string -> string -> (Format.formatter -> 'a -> unit) -> 'a -> unit
(** [write_dot_to path name suff fmt graph] Writes a graph in graphviz to file
    [<path>/<name>_<suff>.dot]. *)
