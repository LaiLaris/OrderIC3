...
let fmt_term_dot fmt term = Term.string_of_term term |> Format.fprintf fmt "%s"

(* Opens a file in write mode, creating it if needed. *)
let openfile path =
  Unix.openfile path [ Unix.O_TRUNC; Unix.O_WRONLY; Unix.O_CREAT ] 0o640
...
let write_dot_to path name suff fmt_graph graph =
  ...
  let desc =
    (* Create descriptor for log file. *)
...
module type Graph = sig
  ...
  (** Mines a system and creates the relevant graphs.
      ...
      First boolean is [top_only], then [two_state]. Input function is applied
      to each subsystem. It is used to create the pruning checkers. *)
  ...
  (** Minimal list of terms encoding the current state of the graph. Contains
      ...
      Used when querying the base instance of the LSD (graph stabilization). See
      also [equalities_of] and [relations_of], used for the step instance
      (induction check). *)
  ...
  (** Equalities coming from the equivalence classes of a graph.
      ...
      candidate invariant, while the second element stores the representative of
      the class the candidate comes from, and the term that can be dropped from
      it if the candidate is indeed invariant. *)
  ...
  (** Appends the relations from a graph to the input term list.
      ...
      function. This is to accomodate with the information we need to keep
      around for the equalities of a graph (see [equalities_of]). *)
...
module Make (Dom : DomainSig) : Graph = struct
  ...
  let has_svars { classes } =
    Map.fold
      (fun rep clss acc ->
        acc
        || Term.state_vars_of_term rep |> StateVar.StateVarSet.is_empty |> not
        || Set.exists
             (fun term ->
  ...
  let drop_class_member { classes } rep term =
    try Map.find classes rep |> Set.remove term |> Map.replace classes rep
    with Not_found ->
      KEvent.log L_fatal
  ...
  let fmt_graph_dot fmt { map_up; map_down; classes; values } =
    ...
    |> Map.iter (fun key ->
           ...
           Set.iter (fun kid ->
               ...
               in
               Format.fprintf fmt
                 "\"%a (%d, %s)\" -> \"%a (%d, %s)\" [constraint=false] ;@ "
                 fmt_term_dot key key_len key_value fmt_term_dot kid kid_len
                 kid_value));
  ...
  let fmt_graph_classes_dot fmt { classes; values } =
    ...
    classes
    |> Map.iter (fun rep set ->
           let rep_value =
             try Map.find values rep |> Format.asprintf "%a" Domain.fmt
             with Not_found -> "_mada_"
           in
           Format.fprintf fmt "\"%a (%s)\" ->\"%a\" ;@ " fmt_term_dot rep
             rep_value
             (pp_print_list
                (fun fmt term ->
                  Format.fprintf fmt "@[<h>%a@]" fmt_term_dot term)
                "\n")
             (Set.elements set));
  ...
  let check_graph ({ map_up; map_down; classes } as graph) =
    ...
    Map.fold
      (fun rep reps ok ->
        ...
        if
          (* Fail if [rep] has no kids and is not [true] or [false]. *)
          Set.is_empty reps && rep <> Term.t_false && rep <> Term.t_true
        then (
          KEvent.log L_fatal
            "Inconsistent graph:@   @[<v>representative [%a] has no kids@]"
            Term.pp_print_term rep;
          is_ok := false);

        (try
           let _ = Map.find classes rep in
           ()
         with Not_found ->
           KEvent.log L_fatal
             "Inconsistent graph:@   @[<v>representative [%a] has no \
              equivalence class@]"
             Term.pp_print_term rep;
           is_ok := false);
        ...
        |> Set.iter (fun kid ->
               try
                 ...
                 if Set.mem rep kid_parents |> not then (
                   ...
                   KEvent.log L_fatal
                     "Inconsistent graph:@   @[<v>representative [%a] is a kid \
                      of [%a]@ but [%a] is not a parent of [%a]@]"
               ...
               with Not_found ->
                 ...
                 KEvent.log L_fatal
                   "Inconsistent graph:@   @[<v>representative [%a] does not \
                    appear in [map_down]@]"
    ...
    | false ->
        ...
        write_dot_to dump_path "inconsistent" "graph" fmt_graph_dot graph;
        KEvent.log L_fatal
          "Dumping current classes as graphviz in current directory";
        write_dot_to dump_path "inconsistent" "classes" fmt_graph_classes_dot
          graph;
        false
  ...
  (** Minimal list of terms encoding the current state of the graph. Contains
      ...
      Used when querying the base instance of the LSD (graph stabilization). See
      also [all_terms_of], used for the step instance (induction check). *)
  let terms_of { map_up; classes } known =
    let cond_cons cand l = if known cand then l else cand :: l in
    let eqs =
      Map.fold
        (fun rep set acc ->
          if Set.is_empty set then acc
          else cond_cons (rep :: Set.elements set |> Term.mk_eq) acc)
        classes []
    in
    Map.fold
      (fun rep above acc ->
        if Domain.is_bot rep then acc
        else
          above |> Set.elements
          |> List.fold_left
               (fun acc rep' ->
  ...
  (** Maximal list of terms encoding the current state of the graph. Contains
      ...
      Used when querying the step instance of the LSD (induction check). The
      idea is that while the comparison terms between representatives may not be
  ...
  let equalities_of { classes } known =
    let cond_cons l cand info = if known cand then l else (cand, info) :: l in

    let rec loop rep pref suff = function
      | term :: tail ->
          let pref = cond_cons pref (Domain.mk_eq rep term) (rep, term) in
          let suff =
            List.fold_left
              (fun suff term' ->
    ...
    Map.fold
      (fun rep terms acc ->
        if Set.cardinal terms < 10 then Set.elements terms |> loop rep [] acc
        else
          Set.fold
            (fun term acc -> cond_cons acc (Domain.mk_eq rep term) (rep, term))
            terms acc)
      classes []
  ...
  (** Applies a function [f] to the value [key] is bound to in [map].
      ...
      - if it's [Some default], then a binding between [key] and [f default]
        will be created *)
  let apply ?(not_found = None) f key map =
    try Map.find map key |> f |> Map.replace map key
    with Not_found -> (
      match not_found with
      | None ->
          Format.asprintf "could not find %a in map" fmt_term key |> failwith
    ...
    let rec insert ?(is_rep = false) pref sorted term value =
      if is_rep || Domain.eq value rep_val |> not then (
        ...
        | head :: tail ->
            ...
            insert ~is_rep (head :: pref) tail term value)
      else sorted
    in
    ...
    | _ ->
      ...
      class. *)
        ...
        let chain =
          ...
          |> List.map (fun (rep, value, set) ->
                 (* TODO: add [is_bot] and [is_top] to input modules and use that
            instead. *)
                 let rep, set =
                   if Set.mem Term.t_true set then
                     (Term.t_true, set |> Set.add rep |> Set.remove Term.t_true)
                   else (rep, set)
                 in
    ...
    let rec insert known continuation chain node =
      ...
      | [] ->
          ...
          if Set.mem node below then (
            ...
            above
            |> Set.iter (fun above -> map_down |> apply (Set.add node) above))
      | lowest :: _ ->
  ...
  let next_of_continuation { map_down; values } continuation =
    if Set.is_empty continuation then None
    else
      try
          ...
          Set.partition
            (fun rep ->
              try
                ...
                |> Set.for_all (fun rep -> Map.mem values rep)
              with Not_found ->
                Format.asprintf "could not find rep %a in map down" fmt_term rep
                |> failwith)
            continuation
        in
  ...
  let split_of_model sys new_reps model ({ map_down; map_up } as graph) =
    let rec loop new_reps continuation next =
      ...
      let new_reps, continuation =
        Set.fold
          (fun rep (new_reps, continuation) ->
            ...
            the current rep. *)
            let continuation = Map.find map_up rep |> Set.union continuation in
      ...
      | None -> new_reps
      | Some (next, continuation) -> loop new_reps continuation next
    in
  ...
  (** Stabilizes the equivalence classes. Stabilizes classes one by one to send
      relatively few candidates to lsd. *)
  let[@ocaml.warning "-27"] stabilize_classes sys known stable_action query
      ({ classes } as graph) =
    let rec loop count reps_to_update =
      try
        ...
        try
          ...
          let eqs, _ =
            Set.fold
              (fun rep (acc, last) -> (Domain.mk_eq last rep :: acc, rep))
              cl4ss ([], rep)
          in
          match
    ...
    Map.fold (fun rep _ set -> Set.add rep set) classes Set.empty |> loop 0

  (** Stabilizes the relations. *)
  let rec stabilize_rels sys known query count ({ map_up } as graph) =
    (* Checking if we should terminate before doing anything. *)
    KEvent.check_termination ();
    ...
    | Some model ->
        ...
        clear graph;
        (* Stabilize graph. *)
        let reps_to_update = split_of_model sys Set.empty model graph in
        if Set.is_empty reps_to_update |> not then
          KEvent.log L_warn
            "[graph splitting] @[<v>Some classes were split during relation \
             stabilization.@ This should not be possible.";
        (* Loop after adding new representatives (includes old one). *)
  ...
  (** Queries the lsd and updates the graph. Iterates until the graph is stable.
      That is, when the lsd returns unsat. *)
  ...
  let step_stabilize two_state graph sys known lsd comm =
    ...
    stabilize_classes sys known
      (fun (* Action to perform on stable eq classes. *)
             rep cl4ss ->
        Set.fold
          (fun term acc ->
...
module MakeEq (Dom : DomainSig) : Graph = struct
  ...
  let term_count graph =
    Map.fold (fun _ _ sum -> sum + Map.length graph + 1) graph 0
  ...
  let drop_class_member graph rep term =
    try Map.find graph rep |> Set.remove term |> Map.replace graph rep
    with Not_found ->
      KEvent.log L_fatal
  ...
  let fmt_graph_classes_dot fmt classes =
    ...
    classes
    |> Map.iter (fun rep set ->
           Format.fprintf fmt "\"%a\" ->\"%a\" ;@ " fmt_term rep
             (pp_print_list
  ...
  let terms_of graph known =
    ...
    Map.fold
      (fun rep ->
        Set.fold (fun term acc -> Domain.mk_eq rep term |> cond_cons acc))
      graph []
  ...
  let stabilize graph sys known base =
    ...
    let split graph rep set eval =
      ...
      Set.iter
        (fun term ->
          ...
          try
            let _, rep =
              !val_map |> List.find (fun (v, _) -> Domain.eq v value)
            in
            add rep term
          with Not_found ->
    ...
    let model_stabilize graph eval =
      ...
      behavior. *)
      Map.fold (fun rep set acc -> (rep, set) :: acc) graph []
      (* Extract info and modify afterwards. *)
      |> List.iter (fun (rep, set) -> split graph rep set eval)
    in

    (* Loops as long as the graph is unstable in base. *)
    let rec loop () =
      match terms_of graph known |> has_cex with
      | None -> ()
      | Some model ->
