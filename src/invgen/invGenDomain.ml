...
module type Domain = sig
  ...
  val mine : bool -> bool -> Analysis.param -> Sys.t -> (Sys.t * Set.t) list
  (** Mines a transition system for candidate terms. *)

  val first_rep_of : Set.t -> Term.t * Set.t
  (** Representative of the first equivalence class. [False] for bool, a random
      term in the set for arith. *)
...
module Bool : Domain = struct
  ...
  let eval_bool sys model term =
    Eval.eval_term (Sys.uf_defs sys) model term |> Eval.bool_of_value
  ...
  let mk_eq rep term =
    if rep == Term.t_true then term
    else if rep == Term.t_false then Term.negate term
    else Term.mk_eq [ rep; term ]
...
module Int : Domain = struct
  ...
  let eval_int sys model term =
    Eval.eval_term (Sys.uf_defs sys) model term |> Eval.num_of_value
...
module MakeSigned (M : MachineIntegerMiner) : MachineIntegerParam = struct
  ...
  let eval sys model term =
    Eval.eval_term (Sys.uf_defs sys) model term |> Eval.ubv_of_value
...
module Real : Domain = struct
  ...
  let eval_real sys model term =
    Eval.eval_term (Sys.uf_defs sys) model term |> Eval.dec_of_value
