...
(** Lock Step Driver for graph-based invariant generation.
    ...
    Provides structures to abstract SMT-solvers for the base / step case, as
    well as trivial invariant pruning. *)
...
let fresh_actlit_of uid =
  UfSymbol.mk_uf_symbol (Format.sprintf "%%actlit_%d" uid) [] (Type.mk_bool ())
...
let mk_base_checker_solver sys k =
  let solver =
    (* Creating solver. *)
    Smt.create_instance ~produce_models:true (Sys.get_logic sys)
      (Flags.Smt.solver ())
  in

  Format.asprintf (* Logging stuff in smt trace. *)
    "[Base/Step] Setting up system [%s], k = %a." (sys_name sys)
    Num.pp_print_numeral k
  |> Smt.trace_comment solver;

  let init_actlit =
    (* Creating actlit for initial predicate. *)
    fresh_actlit ()
  in
  ...
  Sys.define_and_declare_of_bounds sys
  ...
  Sys.invars_of_bound ~one_state_only:true sys
    Numeral.zero (* Invariants at [0]. *)
  ...
  let rec unroll i =
    if Num.(i <= k) then (
      ...
      Format.asprintf "Asserting transition relation at [%a]."
        Num.pp_print_numeral i
      |> Smt.trace_comment solver;

      Sys.trans_of_bound (Some (SMTSolver.declare_fun solver)) sys i
      |> Smt.assert_term solver;

      Format.asprintf "Asserting invariants at [%a]." Num.pp_print_numeral i
      |> Smt.trace_comment solver;
...
let conditional_base_solver_reset ({ solver; sys; k } as base_checker) =
  if shall_reset () then (
...
let query_base base_checker candidates =
  ...
  Format.asprintf "Querying base with actlit [%a] (%d candidates)."
    Uf.pp_print_uf_symbol actlit (List.length candidates)
  |> Smt.trace_comment solver;

  Smt.declare_fun solver actlit;
  ...
  let res =
    Smt.check_sat_assuming solver
      (fun (* If sat, get model and return that. *)
             solver ->
        let minus_k = Numeral.(~-k) in
        (* Variables we want to know the value of. *)
        (if Numeral.(equal k zero) then TransSys.vars_of_bounds sys k k
         else TransSys.vars_of_bounds sys (Numeral.pred k) k)
        (* Getting their value. *)
        |> SMTSolver.get_var_values solver (TransSys.get_state_var_bounds sys)
        (* Bumping to -k. *)
        |> Model.bump_var minus_k
        (* Making an option out of it. *)
        |> fun model -> Some model)
      (fun (* If unsat then no model. *)
             _ -> None)
      [ init_actlit; actlit ]
  in
...
(* A step checker. *)
type step = { mutable solver : solver; sys : sys; k : num }
...
let to_step_solver { solver; sys; k; init_actlit } =
  ...
  Format.asprintf "Asserting transition relation at [%a]." Num.pp_print_numeral
    kp1
  |> Smt.trace_comment solver;
  Sys.trans_of_bound (Some (Smt.declare_fun solver)) sys kp1
  |> Smt.assert_term solver;

  Format.asprintf "Asserting invariants at [%a]." Num.pp_print_numeral kp1
  |> Smt.trace_comment solver;
...
let conditional_step_solver_reset ({ solver; sys; k } as step_checker) =
  if shall_reset () then (
...
(** Queries step.
    ...
    [candidates] is a list of elements of type [(Term.t, 'a)]. The second
    element is understood as some information about the candidate.

    The "information" represented by ['a] is used when checking equality
    candidate invariants coming from equivalence classes from the graph. The
    info is then a pair representative / eq class member meaning that if the
    candidate is indeed invariant, we can drop the class member from the
    equivalence class.
...
let query_step two_state step_checker candidates =
  ...
  let rec loop candidates =
    ...
    Format.asprintf "Querying step with actlit [%a] (%d candidates)."
      Uf.pp_print_uf_symbol actlit (List.length candidates)
    |> Smt.trace_comment solver;
    ...
    let unfalsified_opt =
      ...
      Smt.check_sat_assuming_and_get_term_values solver
        (fun (* If sat, get values and remove falsified candidates. *)
               _ term_values ->
          Some
            (term_values
            |> List.fold_left
...
type pruning = { mutable solver : solver; sys : sys; mutable actlit_uid : int }
(** A pruning checker. *)
...
let mk_pruning_checker_solver sys two_state =
  ...
  if two_state then (
    ...
    Format.asprintf "Asserting transition relation." |> Smt.trace_comment solver;
...
let query_pruning pruning_checker two_state =
  ...
  let rec loop non_trivial candidates =
    ...
    Format.asprintf "Querying pruning with actlit [%a] (%d candidates)."
      Uf.pp_print_uf_symbol actlit (List.length candidates)
    |> Smt.trace_comment solver;
    ...
    Term.mk_implies [ actlit; cands |> Term.mk_and |> Term.mk_not ]
    |> Smt.assert_term solver;
          ...
          Some
            ...
            |> List.fold_left
                 (fun (non_triv, rest) (cand, b_val) ->
                   ...
                   else (Term.bump_state minus_k cand :: non_triv, rest))
                 ([], [])))
        (fun (* If unsat. *)
               _ -> None)
        [ actlit ] cands
    ...
    | Some (non_triv, rest) -> loop (List.rev_append non_triv non_trivial) rest
