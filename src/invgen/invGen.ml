...
let sys_name sys = Sys.scope_of_trans_sys sys |> Scope.to_string
...
(** Constructs an invariant generation module from a value module.
    ...
    The central notion used in the graph splitting algorithm is "chains". A
    chain is just a list of representative / value pairs that represent the new
    nodes obtained after splitting an old node.

    Throughout the different algorithms, a chain is ordered by decreasing
    values.

    Once a node is split, the chain is inserted in all the parents of the old
    node. The update algorithm is designed such that a node is split iff all its
    parents have already be split (that is, their value is known).

    Inserting a chain in the parents consists in extracting the longest prefix
    from the chain such that all the values are greater than that of the parent.
    So for instance when inserting the chain [7, 3, 2, -1] (omitting the
    representatives) for a parent with value 1, then the longest prefix is
    [7, 3, 2]. The graph is thus updated by linking the parent to the
    representative with value 2. The rest of the chain ([-1]) is inserted in all
    the parents of the parent. *)
module Make (Graph : GraphSig) : Out = struct
  ...
  let no_more_lsd () =
    ...
    !prune_ref |> List.iter (fun lsd -> try Lsd.kill_pruning lsd with _ -> ())
  ...
  let communicate_invariants top_sys sys_map two_state sys invs =
    ...
    let pruner_add_of sys =
      try
        ...
        fun inv -> Lsd.pruning_add_invariants pruning_checker two_state [ inv ]
      with Not_found ->
    ...
    | invars_certs when top_sys == sys ->
        ...
        |> List.fold_left
             (fun (cnt, invs) (inv, cert) ->
               ...
               KEvent.invariant
                 (Sys.scope_of_trans_sys top_sys)
                 inv cert two_state;
               (cnt, inv :: invs))
             (0, [])
    | invars_certs ->
        ...
        instantiating invariant at all levels. *)
               let top_invariants, intermediary_invariants =
                 Sys.instantiate_term_cert_all_levels top_sys Sys.prop_base
        ...
        [os] and [ts] for [sys]. *)
               top_invariants :: intermediary_invariants
               |> List.fold_left
                    (fun (cnt, invs) (sub_sys, term_certs) ->
                      ...
                      |> List.fold_left
                           (fun (cnt, invs) (inv, cert) ->
                             ...
                             let inv =
                               Sys.add_invariant sub_sys inv cert two_state
                             in
                             (* Add to pruner. *)
                             pruner_add inv;
                             (* Broadcast. *)
                             KEvent.invariant
                               (Sys.scope_of_trans_sys sub_sys)
                               inv cert two_state;
                             (* Remember if at top sys. *)
                             if at_top_sys then (cnt + 1, inv :: invs)
                             else (cnt, invs))
                           (cnt, invs))
                    (cnt, invs))
             (0, [])
  ...
  let communicate_and_add two_state top_sys sys_map sys k blah non_trivial
      trivial =
    ...
    | _, [] ->
        KEvent.log L_info
          "%s @[<v>On system [%a] at %a: %s@ found %d non-trivial \
           invariants:@   @[<v>%a@]@]"
          (pref_s two_state) Scope.pp_print_scope
          (Sys.scope_of_trans_sys sys)
          Num.pp_print_numeral k blah (List.length non_trivial)
          (pp_print_list fmt_term "@ ")
          non_trivial
    | [], _ ->
        KEvent.log L_info
          "%s @[<v>On system [%a] at %a: %s@ found %d trivial invariants@]"
          (pref_s two_state) Scope.pp_print_scope
          (Sys.scope_of_trans_sys sys)
          Num.pp_print_numeral k blah (List.length trivial)
    | _, _ ->
        KEvent.log L_info
          "%s @[<v>On system [%a] at %a: %s@ found %d non-trivial invariants \
           and %d trivial ones:@   @[<v>%a@]@]"
          (pref_s two_state) Scope.pp_print_scope
          (Sys.scope_of_trans_sys sys)
          Num.pp_print_numeral k blah (List.length non_trivial)
          (List.length trivial)
  ...
  let find_invariants blah two_state lsd sys_map top_sys sys pruner k f
      candidates =
    ...
    let top_level_inc, invs =
      communicate_and_add two_state top_sys sys_map sys k blah non_trivial
        trivial
    in
  ...
  let controlled_find_invariants blah two_state lsd sys_map top_sys sys pruner k
      f =
    let rec loop non_trivial trivial non_invs top_level_inc candidates =
      ...
      let non_trivial', trivial', top_level_inc' =
        find_invariants blah two_state lsd sys_map top_sys sys pruner k f
          candidates
      in
      let non_invs =
        candidates
        |> List.fold_left
             (fun acc ((eq, _) as pair) ->
               if List.memq eq non_trivial' || List.memq eq trivial' then acc
               else pair :: acc)
             []
      in
      let non_trivial, trivial, top_level_inc =
  ...
  let kill_solvers sys_map =
    SysMap.iter (fun _ pruner -> Lsd.kill_pruning pruner) sys_map;
    no_more_lsd ()

  (** Goes through all the (sub)systems for the current [k]. Then loops after
      incrementing [k]. *)
  let rec system_iterator max_depth two_state input_sys param top_sys res memory
      k sys_map top_level_count = function
    | (sys, graph, non_trivial, trivial) :: graphs ->
        ...
        KEvent.log L_info
          ...
          (pref_s two_state) Scope.pp_print_scope
          (Sys.scope_of_trans_sys sys)
          blah Num.pp_print_numeral k (Graph.term_count graph)
    ...
    haven't create the base/step checker yet. *)
        ...
        let prune =
          if two_state then fun cand ->
            is_inv cand
            ||
            match Term.var_offsets_of_term cand with
            | Some lo, Some hi
              when Num.(lo = hi) && (one_state_running || Num.(lo = ~-one)) ->
                true
            | None, None -> true
            | _ -> false
          else is_inv
    ...
    in *)
        ...
        let trivial =
          trivial_eqs
          |> List.fold_left (fun trivial inv -> Set.add inv trivial) trivial
        in
        ...
        let relations =
          Graph.relations_of graph
            (List.map (fun (eq, _) -> (eq, ())) non_inv_eqs)
            prune
        in
        ...
        let non_trivial_rels, trivial_rels, _, top_level_inc =
          controlled_find_invariants
            (Format.asprintf "graph relations (%d candidates)"
               (List.length relations))
            two_state lsd sys_map top_sys sys pruning_checker k (List.map fst)
            relations
        in
    ...
    exit () ; *)
        ...
        (* Looping. *)
        system_iterator max_depth two_state input_sys param top_sys res memory k
          sys_map top_level_count graphs
    | [] -> (
        ...
        | _ -> (
            match memory with
            | [] ->
                KEvent.log L_info "%s No more system to run on, stopping."
                  (pref_s two_state);
                kill_solvers sys_map;
                res
            | _ ->
          ...
          (List.length memory) ; *)
                List.rev memory
                |> system_iterator max_depth two_state input_sys param top_sys
                     res [] k sys_map top_level_count))
  ...
  let main max_depth top_only _modular two_state input_sys aparam sys =
    try
      ...
      invariants. *)
      Graph.mine top_only two_state aparam sys (fun sys ->
      ...
      |> List.filter (fun (_, graph, _, _) -> Graph.has_svars graph)
      ...
      | [] ->
          ...
          exit ()
      | graphs ->
          system_iterator max_depth two_state input_sys aparam sys [] [] k
            sys_map 0 graphs
    with
...
let exit _ =
  if Flags.Invgen.bool_eq_only () then EqOnly.BoolInvGen.exit ()
  else BoolInvGen.exit ();
