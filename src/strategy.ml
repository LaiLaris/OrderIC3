  ...
  let rec loop refined abstraction = function
    | ((system, { opacity }) :: tail) :: lower ->
        if
          ...
          Scope.Map.find system abstraction
        then
          (* Is system refineable? *)
          if opacity <> Opaque then
            try
              match A.results_find system results with
              | ({ A.param } as result) :: _ ->
            ...
            with Not_found ->
              (* Case of imported nodes (they have no result) *)
              tail :: lower |> loop refined abstraction
          else tail :: lower |> loop refined abstraction
        else
          (* System is not abstracted, remembering its subsystems and
            looping. *)
          (tail :: lower) @ [ subs_of_scope system ] |> loop refined abstraction
...
let get_refinement_abstraction results subs_of_scope result =
  ...
  | Some false ->
      (* Requirements could not be proved, aborting. *)
      (* KEvent.log L_info
      ...
      let rec loop = function
        | ((candidate, { opacity }) :: tail) :: lower ->
            ...
            then
              ...
              (* Candidate is not abstracted, remembering its subsystems and
                looping. *)
...
let first_param_of ass _results all_nodes scope =
  let rec loop abstraction = function
    | (sys, { opacity; has_impl; has_contract; has_modes }) :: tail ->
        ...
        let is_abstract =
          if sys = scope then has_modes && Flags.Contracts.check_modes ()
          else
            (not has_impl)
            || has_contract
               && (opacity = O.Opaque
                  || Flags.Contracts.compositional ()
                     && opacity <> O.Transparent)
        in
  ...
  let has_first_analysis =
    try is_candidate_for_analysis (List.assoc scope all_nodes)
    with Not_found ->
...
let first_analysis_of_contract_check ass top ({ A.abstraction_map } as info) =
  function
...
let next_monolithic_analysis results main_syss = function
  | [] -> failwith "[strategy] no system to analyze (empty list of scopes)"
  | all_syss ->
      let check_sys (top, { has_impl }) =
        try
          match A.results_find top results with
          | [] -> failwith "unreachable"
          | [ { A.param = A.ContractCheck info; A.contract_valid } ]
  ...
  | all_syss ->
      ...
      let rec go_up = function
        ...
        | sys :: tail -> (
            ...
            try match A.results_find sys results with _ -> None
            with Not_found -> (
              match
                first_param_of (last_assumptions ()) results all_syss sys
              with
              | None ->
      ...
      let rec go_down prefix = function
        | (sys, { has_impl }) :: tail -> (
            try
              ...
              match A.results_find sys results with
              | [] -> assert false
              | _ when Flags.Contracts.check_implem () |> not || not has_impl ->
                  go_up prefix
              | [
               {
               ...
               (* A.sys = trans_sys ; *)
                  ...
                  (* ^^-- Invariants generated during ContractCheck analysis are discarded.
               They were generated assuming the contract! *)
                  first_analysis_of_contract_check (last_assumptions ()) sys
                    info contract_valid
              | result :: _ ->
                  last_trans_sys := Some result.A.sys;
                  if A.result_is_all_inv_proved result then
                    match
                      get_reachability_abstraction results subs_of_scope result
                    with
                    | None ->
                    ...
                    match
                      get_refinement_abstraction results subs_of_scope result
                    with
                    | None ->
                        (* Cannot refine, going up. *)
