...
let rec list_eq l1 l2 =
  match (l1, l2) with
  | [], [] -> true
  | h1 :: t1, h2 :: t2 -> if h1 != h2 then false else list_eq t1 t2
  | _ -> false
...
let mk_context data sys declare actlit_implications checksat_getvalues
    trace_comment =
  { sys; declare; actlit_implications; checksat_getvalues; trace_comment; data }
...
let cex_to_inputs_csv fmt sys cex k =
  match TransSys.get_source sys with
  | TransSys.Lustre nodes ->
      let path = Model.path_from_model (TransSys.state_vars sys) cex k in
...
let cex_to_outputs_csv fmt sys cex k =
  match TransSys.get_source sys with
  | TransSys.Lustre nodes ->
      let path = Model.path_from_model (TransSys.state_vars sys) cex k in
      Format.fprintf fmt "@[<v>%a@]@."
        (LustrePath.pp_print_path_outputs_csv nodes true)
        path
  (* Not supporting non lustre sources. *)
  | _ -> assert false
...
let pp_print_contract_testcase fmt testcase =
  Format.fprintf fmt "@[<v>%a@]"
    (pp_print_list
       (fun fmt (k, svar_list) ->
         Format.fprintf fmt "%3a | %a" Numeral.pp_print_numeral k
  ...
  Format.fprintf fmt "%a"
    (pp_print_list
       (fun fmt (actlit, k, testcase) ->
         Format.fprintf fmt "(%a) %a | @[<v>%a@]" Numeral.pp_print_numeral k
           UfSymbol.pp_print_uf_symbol actlit pp_print_contract_testcase
           testcase)
       "@,")
    testcases
...
let pp_print_contract_testcases_named sys fmt testcases =
  Format.fprintf fmt "%a"
    (pp_print_list
       (fun fmt (actlit, k, testcase) ->
         Format.fprintf fmt "length: %a | @[<v>%a@]" Numeral.pp_print_numeral k
           (pp_print_contract_testcase_named sys)
           testcase)
       "@,")
    testcases
...
let description_of_contract_testcase sys (_, k, trace) =
  ...
  let rec testcase_tail_to_strings res last_k = function
    | [ (k, svars) ] when Numeral.(k = last_k) ->
        Format.asprintf "and at last step (%a), activates %a"
          Numeral.pp_print_numeral k pp_names svars
        :: res
        |> List.rev
    | (k, svars) :: tail ->
        testcase_tail_to_strings
          (Format.asprintf "then at step %a, activates %a"
             Numeral.pp_print_numeral k pp_names svars
          :: res)
          last_k tail
    | [] ->
        Format.asprintf "until the last step (%a)" Numeral.pp_print_numeral
          last_k
        :: res
        |> List.rev
  in
  ...
  | (head_k, head_svars) :: tail ->
      ...
      Format.asprintf "initial state: activates %a" pp_names head_svars
      :: testcase_tail_to_strings [] k tail

(* Tree structure to display the tree of activable modes. *)
type tree = Branch of (string list * tree) list | Leaf of string list list
...
let rec pp_print_tree_pair ppf = function
  ...
  | key, (Branch [ _ ] as value) ->
      Format.fprintf ppf "[%a], %a"
        (pp_print_list Format.pp_print_string ",")
        key pp_print_tree value
  | key, value ->
      Format.fprintf ppf "@[<v 3>[%a],@,%a@]"
        (pp_print_list Format.pp_print_string ",")
        key pp_print_tree value

and pp_print_tree ppf = function
  | Branch map ->
      Format.fprintf ppf "%a" (pp_print_list pp_print_tree_pair "@,") map
  | Leaf [] -> ()
  | Leaf l ->
...
let testcases_to_tree sys testcases =
  ...
  let rec list_unord_eq l1 l2 =
    match (l1, l2) with
    | h1 :: t1, _ :: _ ->
        if l2 |> List.mem h1 then
          l2 |> List.filter (fun s -> s <> h1) |> list_unord_eq t1
        else false
    | [], [] -> true
    | _ -> false
  in
  ...
  let rec insert todo t l =
    ...
    | Leaf (h1 :: t1), h2 :: t2 ->
        if list_unord_eq h1 h2 then insert ((h1, []) :: todo) (Leaf t1) t2
        else Branch [ (h1, Leaf t1); (h2, Leaf t2) ] |> zip_up todo
    | Leaf [], _ -> Leaf l
    | _ ->
        Format.printf "List length is %d, tree:@.%a@." (List.length l)
          pp_print_tree t;
        assert false
  and zip_up todo tree =
    match todo with
    | [] -> tree
    | (key, map) :: tail -> Branch (update_assoc [] key tree map) |> zip_up tail
  in
...
let extend_contract_testcases ({ sys; data } as context) k =
  ...
  let contracts =
    match TransSys.mode_req_svars sys with
    | None -> assert false
    | Some l ->
        l
        |> List.fold_left
             (fun l svar ->
               if
                 ...
                 |> List.exists (fun (sv, _, _) ->
                        StateVar.equal_state_vars svar sv)
               then l
               else
                 let term0 =
  ...
  let conj_of_contracts l =
    l
    |> List.map (fun svar -> Var.mk_state_var_instance svar k |> Term.mk_var)
    |> Term.mk_and
  in
  ...
  let split path_actlit =
    ...
    Format.asprintf "Declaring and constraining split actlit at %a."
    ...
    (* Extracts all the conjunction of branches it is possible to go to from
       [path_actlit]. *)
    let rec split_branches paths =
      match get_values [ path_actlit; split_actlit ] contracts_k with
      ...
      | Some values ->
          ...
          let active_contracts_svars, active_contracts_terms =
            ...
            |> List.fold_left
                 (fun (l_svar, l_k) (svar, term0, termk) ->
                   (* Check the value. *)
                   if List.assq termk values == Term.t_true then
                     (svar :: l_svar, termk :: l_k)
                   else (l_svar, l_k))
                 ([], [])
          in
          ...
          activate
            [
              (split_actlit, Term.mk_and active_contracts_terms |> Term.mk_not);
            ];
  ...
  let split_testcase (actlit, old_k, path) =
    ...
    | (_, contracts) :: _, [ contracts' ] when list_eq contracts contracts' ->
    ...
    | path, _ :: _ ->
        ...
        |> List.map (fun contracts ->
               ...
               (* Declaring it. *)
               declare actlit';
               Format.asprintf "Activates the same path as %a."
  ...
  |> List.fold_left
       (fun nu_data testcase ->
         List.rev_append (split_testcase testcase) nu_data)
       []
...
let testcase_gen strat_out_dir dir pp_testcase context get_model =
  ...
  Format.fprintf gv_fmt "strict digraph mode_graph {@.  @[<v>";

  let names_of =
    List.fold_left
      (fun l svar ->
        match TransSys.mode_names_of_req_svar context.sys svar with
        | None -> assert false
        | Some names -> names @ l)
      []
  in
  ...
  context.data
  |> List.fold_left
       (fun cpt ((actlit, k, testcase) as tc) ->
             ...
             | ((kay', svars') as pair) :: tail ->
                 ...
                 if Numeral.(kay = pred kay') then (
                   ...
                   Format.fprintf gv_fmt
                     "  %s__%a -> %s__%a [label = \"%d\"];@," name
                     Numeral.pp_print_numeral kay name' Numeral.pp_print_numeral
                     kay' cpt;
                   loop pair tail)
                 else
  ...
  let cmd = Format.sprintf "dot -Tpdf %s -o %s" gv_file_name gv_file_out_name in

  Format.printf "  Generating graph:@,  > %s@." cmd;
  (match Unix.system cmd with
  | Unix.WEXITED 0 -> Format.printf "  > done.@."
  | Unix.WEXITED n -> Format.printf "  > error (%d).@." n
  | Unix.WSIGNALED n -> Format.printf "  > /!\\ killed by signal %d.@." n
  | Unix.WSTOPPED n -> Format.printf "  > /!\\ stopped by signal %d.@." n);
