...
let oracle_path dir sys =
  ...
  | TransSys.Lustre nodes ->
      nodes |> List.rev |> List.hd |> oracle_name
      |> Format.sprintf "%s/%s" oracle_dir

module Xml = struct
  let pp_header fmt = Format.fprintf fmt "<?xml version=\"1.0\"?>@."

  let pp_necker fmt sys =
    Format.fprintf fmt "<data system=\"%s\">@.@.  @[<v>" sys

  let pp_oracle fmt oracle_path outputs =
    Format.fprintf fmt "@[<v><oracle path=\"%s\">@,  @[<v>%a@]@,</oracle>@]@,@,"
      oracle_path
      (pp_print_list
         (fun fmt (is_global, output) ->
           Format.fprintf fmt "<output global=\"%b\">%s</output>" is_global
             output)
         "@,")
      outputs
...
let actlit_implications solver ?(eq = false) = function
  ...
  | impls ->
      impls
      |> List.map (fun (uf, term) ->
             (if eq then Term.mk_eq else Term.mk_implies) [ term_of uf; term ])
...
let delete_solver () =
  try
    ...
    | Some solver ->
        ...
        solver_ref := None
  with e ->
    KEvent.log L_debug "TestGen @[<v>Error while deleting solver:@ %s@]"
...
let run_strategy out_dir sys strategy =
  ...
  Format.printf "Starting run for strategy %s.@." Strat.name;
  ...
  let solver =
    S.create_instance ~produce_models:true (TransSys.get_scope sys)
  ...
  (* Initializing solver. *)
  TransSys.init_solver sys (comment solver) (S.define_fun solver)
    (S.declare_fun solver) (S.assert_term solver) Numeral.zero Numeral.zero;
  ...
  let context =
    Strat.mk_context sys (declare solver)
  ...
  let rec loop_strategy k =
    ...
    if not is_done then (
      ...
      TransSys.declare_vars_of_bounds sys
  ...
  Format.printf "Strategy %s is done at %a.@.@." Strat.name
  ...
  in
  checksat_getmodel solver |> Strat.testcase_gen out_dir pp_testcase context;
...
let oracle_of_nodes out_dir nodes =
  ...
  Format.printf "@[<v>top: %a@,subs:@,  @[<v>%a@]@]@.@."
    ...
    (pp_print_list
       (fun ppf n ->
         Format.fprintf ppf "%a" (LustreIdent.pp_print_ident false) n.N.name)
       "@,")
    subs;
  ...
  let contracts =
    match top.N.contract_spec with
    | None -> assert false
    | Some (_, _, globl, modes, _) -> (
        ...
        |> List.fold_left
             (fun l (m : N.contract) ->
               ...
               :: (m, mk_out_ident_req m.N.name, mk_and m.N.reqs)
               :: l)
             []
        |> List.rev
        |> fun l ->
        ...
        | Some c ->
            ...
            :: (c, mk_out_ident c.N.name, mk_and c.N.enss)
            :: l)
  in
  ...
  (* Format.printf
    ...
    (pp_print_list
      ...
      "@,")
    contracts ; *)
  let oracle_ident = oracle_name top |> LustreIdent.mk_string_ident in
         ...
         (fun (out, eqs) (_, id, expr) ->
           let sv =
             LustreExpr.mk_state_var_of_ident
               ...
               id (Type.mk_type Type.Bool)
           in
           ((sv, LustreIdent.index_of_ident id) :: out, (sv, expr) :: eqs))
         ([], [])
  ...
  let sliced = N.reduce_to_coi (oracle :: subs) oracle_ident oracle_out_svs in
  ...
  (* ( match List.rev sliced with
    ...
    | _ -> assert false ) ; *)
  let out_file =
    Format.asprintf "%s/%a.lus" out_dir
      (LustreIdent.pp_print_ident false)
      oracle_ident
  in
  ...
  let file =
    Unix.openfile out_file [ Unix.O_TRUNC; Unix.O_WRONLY; Unix.O_CREAT ] 0o640
  in
  ...
  Format.fprintf out_fmt "%a@?"
    (pp_print_list (N.pp_print_node ~no_subrange:true true) "@.@.")
    sliced;
...
let main sys =
  let out_dir =
    Format.asprintf "%s/%a" (Flags.testgen_out_dir ())
      TransSys.pp_print_trans_sys_name sys
  in
  ...
  let subfiles =
    try strategies |> List.map (run_strategy out_dir sys)
    with e ->
      delete_solver ();
      raise e
  in
  ...
  let xml_file =
    Unix.openfile xml_file [ Unix.O_TRUNC; Unix.O_WRONLY; Unix.O_CREAT ] 0o640
  in
  let xml_fmt =
    Unix.out_channel_of_descr xml_file |> Format.formatter_of_out_channel
  in
  ...
  Xml.pp_header xml_fmt;
  TransSys.get_name sys |> Xml.pp_necker xml_fmt;
  (* Oracle things. *)
  TransSys.get_contracts sys
  |> List.map (fun (_, _, name) ->
