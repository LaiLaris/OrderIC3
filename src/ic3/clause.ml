...
let get_actlit_status_array solver clause_id =
  ...
  try
    (* Get array of activation literal status from hash table for
       solver instance *)
    let actlit_status = IntegerHashtbl.find solver_actlit_status solver_id in
    ...
    if Array.length actlit_status > clause_id then
      ...
      (* Double array size *)
      let actlit_status' =
        Array.make (Array.length actlit_status * 2) actlit_status_default
      in

      (* Copy entries from previous array *)
      Array.blit actlit_status 0 actlit_status' 0 (Array.length actlit_status);

      (* Replace previous array with grown array *)
      IntegerHashtbl.replace solver_actlit_status solver_id actlit_status';
  ...
  with Not_found ->
    ...
    IntegerHashtbl.add solver_actlit_status solver_id actlit_status;
...
let set_actlit_status_of_clause solver actlit_bit clause_id =
  ...
  let actlit_status_array = get_actlit_status_array solver clause_id in
...
let assert_actlits solver actlits terms =
  ...
  List.iter2
    (fun t l ->
      ...
      SMTSolver.assert_term solver (Term.mk_implies [ t; l ]))
    actlits terms
...
let actlit_p1_of_clause solver { clause_id; actlit_p1; literals } =
  ...
  if actlit_undefined solver actlit_p1_bit clause_id then (
    ...
    assert_actlits solver [ actlit_p1 ]
...
let actlits_n0_of_clause solver { clause_id; actlits_n0; literals } =
  ...
  if actlit_undefined solver actlit_n0_bit clause_id then (
    (* Assert activation literal for negative unprimed clause
         literal *)
    assert_actlits solver actlits_n0 (List.map Term.negate literals);
...
let actlits_n1_of_clause solver { clause_id; actlits_n1; literals } =
  ...
  if actlit_undefined solver actlit_n1_bit clause_id then (
    (* Assert activation literal for negative primed clause
         literal *)
    assert_actlits solver actlits_n1
      (List.map
         (fun t -> Term.negate t |> Term.bump_state Numeral.one)
         literals);
...
let mk_clause_of_literals source literals =
  let literals =
    if Flags.IC3QE.eq_canonicalize () then
      literals
...
let deactivate_clause solver
    { clause_id; literals; actlit_p0; actlit_p1; actlits_n0; actlits_n1 } =
  SMTSolver.trace_comment solver "Deactivating activation literals for clause";
  ...
  if not (actlit_undefined solver actlit_p1_bit clause_id) then (
    (* Assert negation of activation literal *)
    Term.mk_not actlit_p1 |> SMTSolver.assert_term solver;
  ...
  if not (actlit_undefined solver actlit_n0_bit clause_id) then (
    ...
    (* Increment statistics for activation literals *)
    Stat.incr ~by:(List.length literals) Stat.ic3_stale_activation_literals);
...
let rec undo_ind_gen = function
  ...
  | { source = PropSet } | { source = BlockFrontier } | { source = BlockRec _ }
    ->
      None
...
(* Return activation literal for property, and lazily assert in solver
   instance *)
let actlit_p0_of_prop_set solver { clause } = actlit_p0_of_clause solver clause

(* Return activation literal for property, and lazily assert in solver
   instance *)
let actlit_p1_of_prop_set solver { clause } = actlit_p1_of_clause solver clause
...
let actlit_and_symbol_of_frame k =
  (* Name of uninterpreted function symbol *)
  let uf_symbol_name = Format.asprintf "%s_frame_%d" actlit_prefix k in

  (* Create or retrieve uninterpreted constant *)
  let uf_symbol = UfSymbol.mk_uf_symbol uf_symbol_name [] Type.t_bool in
...
let create_and_assert_fresh_actlit solver tag term actlit_type =
  ...
  (* SMTSolver.trace_comment solver
    (Format.sprintf
       ...
       tag !actlit_count_ref); *)

  (* Name of uninterpreted function symbol primed negative *)
  let uf_symbol_name =
    Format.asprintf "%s_%s_%d" actlit_prefix tag !actlit_count_ref
  in
  ...
  SMTSolver.assert_term solver (Term.mk_implies [ actlit; term' ]);
