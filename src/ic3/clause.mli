...
type source =
  ...
  | BlockRec of t
      (** Negtion of clause reaches a state outside the negation of the clause
          to block *)
...
(** For every clause [C = {L1, ..., Ln}] two activation literals [p0] and [p1]
    are generated per clause, in addition two activation literals [n0_i] and
    [n1_i] per literal Li. The following terms are then lazily asserted on the
    first access of the actiation:
    ...
    where the C' and Li' are the clause and the literals, respectively, at the
    next instant. *)

val mk_clause_of_literals : source -> Term.t list -> t
val mk_clause_of_literals_l1l2 : source -> Term.t list -> t
val mk_clause_of_literals_l2l1 : source -> Term.t list -> t

(** Create a clause from a set of literals

    The activation literals are only created in the Kind 2 term database, but
    nothing is sent to the solver, instead they are only made on the first
    access of the activation literals in {!actlit_p0_of_clause},
    {!actlit_p1_of_clause}, {!actlits_n0_of_clause} and {!actlits_n1_of_clause}.
*)
...
(** Return the number of literals in the clause
    ...
    Since duplicate literals are eliminated, this is not necessarily equal to
    the number of literals given when creating the clause. *)
...
(** Return the literals in the clause
    ...
    Since duplicate literals are eliminated and ordered, this is not necessarily
    equal to the list of literals given when creating the clause. *)
...
(** Return the activation literal for the positive unprimed clause
    ...
    Declare the activation literal and assert the term [p0 => C] on the first
    access of the activation literal in the given solver instance. Fail with
    [Invalid_argument] if {!deactivate_clause} has been called for the clause in
...
(** Return the activation literal for the positive primed clause
    ...
    Declare the activation literal and assert the term [p1 => C'] on the first
    access of the activation literal in the given solver instance. Fail with
    [Invalid_argument] if {!deactivate_clause} has been called for the clause in
    the given solver instance. *)
...
(** Return the activation literals for the negated clause literals
    ...
    Declare the activation literals and assert the terms [n0_i => L_i] for each
    literal [L_i] in the clause on the first access of the activation literal in
    the given solver instance. Fail with [Invalid_argument] if
    {!deactivate_clause} has been called for the clause in the given solver
    instance. *)

val actlits_n1_of_clause : SMTSolver.t -> t -> Term.t list
(** Return the activation literals for the negated, primed literals

    Declare the activation literals and assert the terms [n1_i => L_i'] for each
    literal [L_i] in the clause on the first access of the activation literal in
    the given solver instance. Fail with [Invalid_argument] if
    {!deactivate_clause} has been called for the clause in the given solver
    instance. *)
...
(** Assert the negation of all activation literals of clause in the solver
    instance

    The clause is marked as unusable in the solver instance, hence all calls to
    {!actlit_p0_of_clause}, {!actlit_p1_of_clause}, {!actlits_n0_of_clause},
    {!actlits_n1_of_clause} fail. *)

val undo_ind_gen : t -> t option
(** If the clause is an inductive generalization, return the clause before
    generalization

    Only return one step of inductive generalization, repeat to obtain possible
    chains of generalizations. *)
...
(** Create a property set from a list of named properties
    ...
    The conjunction of properties is viewed as a single literal of a clause, and
    this clause is asserted with activation literals in the given solver
    instance. *)

val clause_of_prop_set : prop_set -> t
(** Return the unit clause containing the conjunction of the properties as a
    literals *)
...
(** Return the activation literal for the positive primed conjunction of
    properties *)
...
(** Return the (singleton list of) activation literals for the negated primed
    conjunction of properties *)
...
(** Create or return the uninterpreted functoin symbol for the activation
    literal for the given frame *)
...
val create_and_assert_fresh_actlit :
  SMTSolver.t -> string -> Term.t -> actlit_type -> Term.t
(** Create a fresh activation literal, declare it in the solver, and assert a
    term guarded with it

    [create_and_assert_fresh_actlit s h t a] declares a fresh uninterpreted
    Boolean constant in the solver instance [s], and asserts the term [t]
    guarded by this activation literal in the same solver instance. The
    parameter [h] is a tag used to name the constant, together with a counter
    that is maintained per tag. *)
