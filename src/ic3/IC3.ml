...
(* let record_safe_frontier_first_built_frame solver frontier_level frames =
  if Flags.IC3QE.first_frame_order () then
    let rec first_non_empty level = function
      | [] -> None
      | frame :: tl -> (
          match F.values frame with
          | [] -> first_non_empty (pred level) tl
          | clauses -> Some (level, clauses))
    in
    match first_non_empty frontier_level frames with
    | None -> ()
    | Some (frame_level, clauses) ->
        let literals =
          clauses
          |> List.to_seq
          |> Seq.flat_map (fun clause -> C.literals_of_clause clause |> List.to_seq)
          |> Term.TermSet.of_seq
        in
        first_built_frame_literals := Some literals;
        SMTSolver.trace_comment solver
          (Format.asprintf
             "@[<v>first-built-frame candidate after safe R_%d:@,\
              frame R_%d clauses:@,%a@,\
              literals (%d):@,%a@]"
             frontier_level frame_level
             (pp_print_list
...
let print_stats () =
  KEvent.stat
    ...
    @ (if Flags.IC3QE.abstr () = `IA then
       ...
       else [ (Stat.ic3_stats_title, Stat.ic3_stats) ])
...
type tree_node = { node_clause : C.t; mutable children : tree_node list }

let reuse_tree = ref None

let add_root_to_tree root_node =
  match !reuse_tree with
  | None -> reuse_tree := Some root_node
  | Some _ ->
      (* 如果全局树已经有根节点了，什么都不做 *)
      (* let()=print_endline("已经有根节点了") in  *)
      ()

(*找到某个id对应的孩子节点*)
let find_node_by_id tree id =
  let rec find_node_by_id_recursive node id =
    if C.id_of_clause node.node_clause = id then Some node
    else
      match node.children with
      | [] -> None
      | hd :: tl -> (
          match find_node_by_id_recursive hd id with
          | None -> find_node_by_id_recursive { hd with children = tl } id
          | Some _ as found -> found)
  in
  match !tree with
  | None -> None
  | Some root -> find_node_by_id_recursive root id

(* 向树中添加子节点（根除外，直接给出）*)
let add_node_to_tree id clause =
  match find_node_by_id reuse_tree id with
  | None -> ()
  (*print_endline("找不到对应的父节点，什么也不做")  (* 找不到对应的父节点，什么也不做 *)*)
  | Some parent ->
...
let handle_events solver input_sys aparam trans_sys props =
  ...
  List.iter
    (fun (p, _) ->
      match TransSys.get_prop_status trans_sys p with
...
let rec check_frames' solver prop_set accum = function
  ...
  | (r_i : C.t F.t) :: tl ->
      ...
      let is_rel_ind () =
        (* SMTSolver.trace_comment solver
          (Format.sprintf "check_frames: Does R_%d & T |= R_%d hold?"
             (List.length tl)
             (List.length tl |> succ)); *)

        (* Activation literal for conjunction of clauses *)
        let actlit_n1 =
          C.create_and_assert_fresh_actlit solver "check_frames"
            (List.map C.term_of_clause
      ...
      let rec is_initial = function
        ...
        | c :: ctl ->
            (*SMTSolver.trace_comment solver
              (Format.sprintf "check_frames: Does I |= C for C in R_%d hold?"
                 (List.length tl |> succ)); *)
            if
      ...
      (* SMTSolver.trace_comment solver
        (Format.sprintf "check_frames: Does R_%d |= P hold?"
           (List.length tl |> succ)); *)
...
let check_frames solver prop_set clauses frames =
  SMTSolver.trace_comment solver
    (Format.asprintf "@[<v>check_frames: frames k to 1@,%a@]"
       (pp_print_list
          (fun ppf r_i ->
...
let count_subsumed solver (c, f) =
  ...
  if num > 0 then (
    SMTSolver.trace_comment solver
      (Format.sprintf "@[<v>Backward subsumed %d clauses in R_k@]" num);
...
let deactivate_subsumed solver (subsumed, frame') =
  ...
  List.iter (fun (_, c) -> C.deactivate_clause solver c) subsumed;
    ...
    if Flags.IC3QE.freq_sort () && frame <> [] then
      ...
      SMTSolver.trace_comment solver
        (Format.asprintf
           ...
           literals
           (if Flags.IC3QE.ast_complexity () then "freq+ast"
            else "freq")
           (pp_print_list pp_literal_frequency "@,")
           reordered);
      reordered
...
*)
  ...
  let primed_vars =
    Var.VarSet.elements (Term.vars_at_offset_of_term Numeral.one term)
  in

  Stat.unpause_time Stat.ic3_generalize_time;

  (* Generalize term by quantifying over and eliminating primed
     variables *)
  let gen_term =
    try
      QE.generalize trans_sys
        (TransSys.uf_defs trans_sys)
        state primed_vars term
    with QE.QuantifiedTermFound _ ->
  ...
  let intrpo =
    ...
    | Some s ->
        if List.length trace > !max_unrolling then (
          TransSys.declare_vars_of_bounds trans_sys (SMTSolver.declare_fun s)
    ...
    | None ->
        let solver =
          SMTSolver.create_instance ~produce_interpolants:true
  ...
  let names =
    ...
    | `MathSAT_SMTLIB ->
        List.map
          ...
          interpolizers
    | `OpenSMT_SMTLIB | `SMTInterpol_SMTLIB ->
        List.map
          (fun t -> SMTExpr.ArgString (SMTSolver.assert_named_term_wr intrpo t))
          interpolizers
    | _ -> failwith "Interpolating solver not found or unsupported"
  in
  Stat.start_timer Stat.ic3ia_interpolation_time;

  if SMTSolver.check_sat intrpo then raise_cex ()
  else
    let interpolants = SMTSolver.get_interpolants intrpo names in
    ...
    let interpolants =
      List.mapi
        ...
        interpolants
      |> List.filter (fun t -> not (t == Term.t_false || t == Term.t_true))
      |> List.fold_left
...
let rec block solver input_sys aparam trans_sys prop_set term_tbl predicates =
  ...
  | [] -> (
      ...
      | r_k_orig :: frames_tl as frames -> (
          ...
          | SMTSolver.Sat cti ->
                ...
                | `IA ->
                    ...
                    List.map
                      (fun p ->
                        match
                          (* Evaluate predicate *)
                          Eval.eval_term (TransSys.uf_defs trans_sys) cti p
                        with
              ...
              let clause =
                try
                  C.mk_clause_of_literals C.BlockFrontier
                    (List.map Term.negate cti_gen)
                with Invalid_argument _ as e ->
                  if List.exists (fun t -> Term.has_quantifier t) cti_gen then
                    raise
                      (UnsupportedFeature
                         "Shutting down IC3QE: QE failed during generalization \
                          step.")
                  else raise e
              in
              ...
              block solver input_sys aparam trans_sys prop_set () predicates
  ...
  | ([], r_i) :: block_tl -> (
      ...
      | frames ->
          SMTSolver.trace_comment solver
            (Format.sprintf "block: All counterexamples blocked in R_%d"
  ...
  | ((block_clause_orig, block_trace) :: block_clauses_tl, r_i) :: block_tl as
    trace -> (
      function
      | frames -> (
            ...
            SMTSolver.check_sat_assuming_ab solver
              ...
              (fun _ ->
                ...
                | _ ->
                    SMTSolver.get_var_values solver
                      (TransSys.get_state_var_bounds trans_sys)
                      (TransSys.vars_of_bounds trans_sys Numeral.zero
                         Numeral.one))
          ...
          | SMTSolver.Unsat core_actlits_trans ->
              ...
              let r_i', frames', block_tl' =
                ...
                ( (try
                   ...
                   with Invalid_argument _ ->
                     SMTSolver.trace_comment solver
                       (Format.asprintf
                          "@[<v>Clause@ @[<hv>{%a@}@]@ subsumes a clause in \
                           frame, must do subsumption before adding@ \
                           @[<hv>%a@]@]"
                          (pp_print_list Term.pp_print_term ";@ ")
                  ...
                  if Flags.IC3QE.block_in_future () then
                    add_to_block_tl solver block_clause_orig block_trace
                      block_tl
                  else block_tl )
              in
              ...
              block solver input_sys aparam trans_sys prop_set term_tbl
                ...
                (if
                   ...
                   match block_tl' with [] -> false | _ -> true
                 then
          ...
          | SMTSolver.Sat cti -> (
              ...
              | [] -> (
                  SMTSolver.trace_comment solver
                    (Format.asprintf "@[<hv>~P reachable from I:@ @[<hv>%a@]@]"
                       (pp_print_list
                          (fun ppf c ->
                            Format.fprintf ppf "%d" (C.id_of_clause c))
                          ",@ ")
                  ...
                  | `IA ->
                      SMTSolver.trace_comment solver
                        (Format.sprintf "block: generating interpolants.");

                      let cex_trace =
                        List.map
                          (fun bcl ->
                            Term.mk_and
                              (List.map Term.negate
                                 (Clause.literals_of_clause bcl)))
                          (block_clause :: block_trace)
                      in

                      let interpolants =
                        abstr_simulate cex_trace trans_sys raise_cex
                      in
                      ...
                      List.iteri
                        (fun _ t ->
                          SMTSolver.assert_term solver
                            (Term.mk_eq
                               [ t; Term.bump_state (Numeral.of_int 2) t ]);
              ...
              | r_pred_i :: frames_tl ->
                    ...
                    | `IA ->
                        List.map
                          (fun p ->
                            ...
                            | _ ->
                                raise
                                  (Invalid_argument "abstract cex evaluation"))
                          predicates
                  in
                  ...
                  (* Create a clause with activation literals from generalized
                    counterexample *)
                  let block_clause' =
                    C.mk_clause_of_literals (C.BlockRec block_clause)
                      (List.map Term.negate cti_gen)
                  in
...
(* Split list of clauses into those that are inductive and those that
   are not *)
let rec partition_inductive solver trans_sys frame not_inductive maybe_inductive
    =
  ...
  SMTSolver.trace_comment solver "Checking for inductiveness of clauses";
  ...
  | SMTSolver.Sat model ->
        ...
        List.partition
          (function
            | c ->
                ...
                |> Eval.eval_term [] model |> Eval.bool_of_value)
          maybe_inductive
      in
      ...
      if maybe_inductive = [] then (not_inductive', [])
      else
        (* Continue checking if remaining clauses are inductive *)
        partition_inductive solver trans_sys frame not_inductive'
          maybe_inductive'
...
let partition_fwd_prop solver trans_sys prop_set frame clauses =
  ...
  let rec partition_fwd_prop' keep maybe_prop =
    ...
    (* Assert n1 => ~(C_1' & ... & C_n') *)
    let actlit_n1 =
      C.create_and_assert_fresh_actlit solver "fwd_prop"
    ...
    | SMTSolver.Sat model ->
        ...
        if maybe_prop' = [] then (keep', [])
        else
          (* Continue checking if remaining clauses are inductive *)
          partition_fwd_prop' keep' maybe_prop'
...
let fwd_propagate solver input_sys aparam trans_sys prop_set frames =
  let subsume_and_add a c =
    ...
    if Flags.IC3QE.fwd_prop_subsume () then
      ...
      if F.is_subsumed a l then (
        SMTSolver.trace_comment solver
          (Format.asprintf "@[<v>Clause is subsumed in frame@,%a@]"
             (pp_print_list Term.pp_print_term "@,")
             (F.values a |> List.map (C.actlit_p0_of_clause solver)));
  ...
  let rec fwd_propagate' solver input_sys aparam trans_sys prop frames =
    ...
    | [] ->
        ...
        if Flags.IC3QE.check_inductive () && prop <> [] then (
          SMTSolver.trace_comment solver
            "fwd_propagate: Checking for inductiveness of clauses in last \
             frame.";
          ...
          let non_inductive_clauses, inductive_clauses =
            partition_inductive solver trans_sys [] [] prop
          in
    ...
    | frame :: frames_tl ->
        ...
        SMTSolver.trace_comment solver
          (Format.sprintf
             "fwd_propagate: Checking forward propagation of clauses in frame \
              %d."
             (succ (List.length frames)));
        ...
        (* DEBUG *)
        if debug_assert then
          assert (check_frames' solver prop_set frames_tl_full (frame' :: frames));
        ...
        if debug_assert then
          assert (
            check_frames' solver prop_set (frames_tl_full @ fwd)
              (List.fold_left
                 (fun a c -> F.add (C.literals_of_clause c) c a)
                 frame' keep
              :: frames));
          ...
          let ind_inv_conjuncts =
            let rec collect acc t =
              if Term.is_node t then
                ...
                match Symbol.node_of_symbol s with
                | `AND -> List.fold_left collect acc (Term.node_args_of_term t)
                | _ -> t :: acc
              else t :: acc
            in
                        ...
                        SMTSolver.trace_comment solver
                          (Format.asprintf
                             "@[<hv>Copied clause #%d in forward propagation:@ \
                              #%d @[<hv 1>{%a}@]@]"
                             (C.id_of_clause p) (C.id_of_clause p')
                             (pp_print_list Term.pp_print_term ";@ ")
  ...
  fwd_propagate' solver input_sys aparam trans_sys [] [] (List.rev frames)
...
let rec ic3 solver input_sys aparam trans_sys prop_set frames predicates =
  ...
  let frames' =
    try
      (* Forward propagate clauses in all frames *)
      fwd_propagate solver input_sys aparam trans_sys prop_set frames
      (* Fixed point reached *)
  ...
  let frames'', predicates =
    block solver input_sys aparam trans_sys prop_set () predicates [] frames'
  in
...
let add_to_path model path state_vars i =
    ...
    List.fold_left
      (fun (m, eq) sv ->
        ...
        let t =
          ...
          | Model.Term t -> t
          | exception Not_found ->
              TermLib.default_of_type (StateVar.type_of_state_var sv)
          | _ -> assert false
        in
        ...
        ( (sv, Model.Term t) :: m,
          Term.mk_eq
            [
              Term.mk_var (Var.set_offset_of_state_var_instance v Numeral.zero);
              t;
            ]
          :: eq ))
      ([], []) state_vars
  ...
  let path' =
    list_join StateVar.equal_state_vars
      (List.sort
...
let extract_cex_path solver trans_sys trace =
  ...
  let rec extract_cex_path' path pre_state = function
    | [] ->
        ...
        List.map (fun (sv, vl) -> (sv, List.rev vl)) path
    ...
    | r_i :: tl -> (
        ...
        | SMTSolver.Sat (path', state) ->
            ...
            let actlit_p0_state =
              C.create_and_assert_fresh_actlit solver "cex_path" state
                C.Actlit_p0
            in
    ...
    | r_1 :: _ -> (
        match
          ...
          SMTSolver.check_sat_assuming_ab solver
            ...
            (C.actlit_of_frame 0 :: C.actlits_n1_of_clause solver r_1)
        with
        | SMTSolver.Sat r -> r
        (* Counterexample trace must be satisfiable *)
        | SMTSolver.Unsat _ -> assert false)
  ...
  let actlit_p0_state_init =
    C.create_and_assert_fresh_actlit solver "cex_path" state_init C.Actlit_p0
  in
  ...
  if props = [] then ()
    ...
    let props' =
      try
        ...
        let prop_set = C.prop_set_of_props props in
      ...
      | Success (_, ind_inv) ->
          ...
          List.iter
            (fun (p, _) ->
              KEvent.prop_status (Property.PropInvariant cert) input_sys aparam
                trans_sys p)
            props;
      ...
      | Counterexample trace ->
          (* Extract counterexample from sequence of blocking
               clauses *)
          Stat.unpause_time Stat.ic3_find_cex_time;
          let cex_path = extract_cex_path solver trans_sys trace in
          Stat.record_time Stat.ic3_find_cex_time;
          (*
            debug ic3
              ...
              (fun (props', props_false) (p, t) ->
                if
                  ...
                  Model.exists_on_path
                    (fun m ->
                      match Eval.eval_term (TransSys.uf_defs trans_sys) m t with
                      | Eval.ValBool false -> true
                      | _ -> false)
                    (Model.path_of_list cex_path)
                then (
                  KEvent.prop_status (Property.PropFalse cex_path) input_sys
                    aparam trans_sys p;

                  KEvent.log L_info "Property %s disproved by IC3QE" p;

                  (props', p :: props_false))
                else (
                  KEvent.log L_info "Property %s not disproved by IC3QE" p;
...
*)
          ...
          props'
      | Disproved _ ->
          KEvent.log L_info "Some properties are disproved";
            ...
            List.fold_left
              (fun accum (p, t) ->
                ...
                if TransSys.is_disproved trans_sys p then (
                  KEvent.log L_info "Removing disproved property %s" p;
      ...
      | Presburger.Not_in_LIA ->
          let qe_solver_is_available =
            match Flags.Smt.qe_solver () with `detect -> false | _ -> true
          in

          if not qe_solver_is_available then
            raise
              (UnsupportedFeature
                 "Shutting down IC3QE: Z3 or cvc5 is required for inputs with \
                  reals or machine integers.");
    ...
    if not (props' = []) then (
      KEvent.log L_info "@[<h>Restarting IC3QE with properties @[<h>%a@]@]"
        (pp_print_list (fun ppf (n, _) -> Format.fprintf ppf "%s" n) "@ ")
        props';
...
let bmc_checks solver input_sys aparam trans_sys props bound =
  ...
  let not_entailed props k _ =
    ...
    let cex = Model.path_from_model (TransSys.state_vars trans_sys) model k in
    ...
    let not_falsified, falsifiable =
      List.partition (fun (_, term) -> Term.bump_state k term |> eval) props
    in
  ...
  let rec bmc_check check_primed = function
    ...
    | props -> (
        ...
        (* Check satsifiability of I & ~P for all not falsified
         ...
         falsifiable *)
        SMTSolver.trace_comment solver
          (Format.sprintf "bmc_checks: Check for %s-step counterexample"
        ...
        let props', props_falsifiable =
          SMTSolver.check_sat_assuming solver
            ...
            (if check_primed then
               ...
               :: C.actlit_p0_of_prop_set solver prop_set
               :: C.actlits_n1_of_prop_set solver prop_set
             else actlit_R0 :: C.actlits_n0_of_prop_set solver prop_set)
        in
        ...
        | None ->
            ...
            List.iter
              (fun (s, _) ->
                KEvent.prop_status
                  (Property.PropKTrue (if check_primed then 1 else 0))
                  input_sys aparam trans_sys s)
              props';
...
let main_ic3 input_sys aparam trans_sys =
  ...
  let bound = match Flags.IC3QE.abstr () with `None -> 1 | `IA -> 3 in
  ...
  let trans_sys_props = TransSys.props_list_of_bound trans_sys Numeral.zero in
  let predicates =
    match Flags.IC3QE.abstr () with
    | `IA ->
  ...
  List.iter
    (fun p ->
      SMTSolver.assert_term solver
        (Term.mk_eq [ p; Term.bump_state (Numeral.of_int 2) p ]);

      SMTSolver.assert_term solver
        (Term.mk_eq
           [
             Term.bump_state Numeral.one p; Term.bump_state (Numeral.of_int 3) p;
           ]))
    predicates;
  ...
  let props' =
    ...
    if List.mem `BMC (Flags.enabled ()) && not debug_assert then (
      ...
      *)
      ...
      KEvent.log L_info
        "Delegating check for zero and one step counterexamples to BMC process.";

      trans_sys_props)
    else
      (* BMC is not running, must check here *)
      bmc_checks solver input_sys aparam trans_sys trans_sys_props bound
  in
...
let main input_sys aparam trans_sys =
  ...
  | `Yices2_SMTLIB -> (
      ...
      | `Inferred l when mem NA l ->
          raise
            (UnsupportedFeature
               "Shutting down IC3QE: Yices 2 does not support unsat-cores with \
                non-linear models.")
      | _ -> ())
  | _ -> ());
