...
let term_partition vars_of_term var_lst term_lst =
  ...
  term_lst
  |> List.partition (fun c -> VS.inter (vars_of_term c) var_set |> VS.is_empty)
...
let realizability_check ?(include_invariants = false) vars_of_term sys
    controllable_vars_at_0 vars_at_1 controllable_vars_at_1 =
  let term_partition = term_partition vars_of_term in

  (* Solver for term simplification *)
  let solver =
    SMTSolver.create_instance (TSys.get_logic sys) (Flags.Smt.solver ())
  in
  ...
  let free_of_controllable_vars_at_1, contains_controllable_vars_at_1 =
    let trans =
      if include_invariants then
        ...
        in
        Term.mk_and (TSys.trans_of_bound None sys Numeral.one :: invars)
      else TSys.trans_of_bound None sys Numeral.one
    in
  ...
  *)
  ...
  let rec loop fp1 fp =
    ...
    | QE.Unknown -> Unknown
    | QE.Valid _ -> (
        Debug.realiz "@[<hv>Computed fixpoint:@ @[<hv>%a@]@]@."
          Term.pp_print_term fp;
    ...
    | QE.Invalid valid_region ->
        Debug.realiz "@[<hv>Valid region:@ @[<hv>%a@]@]@." Term.pp_print_term
          valid_region;

        if uncontrollable_varset_is_non_empty then (
          let premises' =
            Term.mk_and (fp :: free_of_uncontrollable_vars_at_1)
          in

          let neg_region =
            SMTSolver.simplify_term solver (Term.negate valid_region)
          in
          ...
          *)
              let fp1' =
                if fst fp1 == Term.t_true then (Term.t_false, valid_region)
                else fp1
              in

              Unrealizable (InductiveCase fp1')
          | QE.Invalid violating_region ->
              Debug.realiz "@[<hv>Violating region:@ @[<hv>%a@]@]@."
              ...
              let fp1' =
                if fst fp1 == Term.t_true then (fp', valid_region) else fp1
              in

              loop fp1' fp')
        else
          let fp' =
            Term.mk_and [ valid_region; fp ] |> SMTSolver.simplify_term solver
          in

          let fp1' = if fst fp1 == Term.t_true then (fp', fp') else fp1 in
  ...
  (* Required when Flags.QE.ae_val_use_ctx is false *)
  let res =
    match QE.ae_val sys premises controllable_vars_at_0 conclusion with
    | QE.Unknown -> Unknown
    | QE.Valid r ->
        if r == Term.t_false then (
          (* Premises are inconsistent *)
          Debug.realiz "@[<hv>Initial state premises are inconsistent@]@.";
          Realizable Term.t_true)
        else loop (Term.t_true, Term.t_true) Term.t_true
    | QE.Invalid valid_region ->
        Debug.realiz "@[<hv>Valid initial region:@ @[<hv>%a@]@]@."
          Term.pp_print_term valid_region;
...
let get_contract_terms init sys guarantee_core other_core =
      ...
      List.fold_left
        (fun acc s ->
          let eqs =
            ...
            |> List.map (fun a ->
                   let eq = ME.eq_of_actlit_uf core ~with_act a in
  ...
  terms_of init guarantee_core other_core |> fun (guarantee_sc, other_sc) ->
  ( ScMap.find scope guarantee_sc |> List.map (fun t -> Term.bump_state offset t),
    ScMap.find scope other_sc |> List.map (fun t -> Term.bump_state offset t) )
...
let filter_and_mk_assign model pred =
  Model.to_list model |> List.filter pred
  |> List.map (fun (v, vl) ->
         match vl with
         | Model.Term e -> Term.mk_eq [ Term.mk_var v; e ]
         | _ -> assert false)
...
let mk_assign_uncontr model contr_vars =
  ...
  filter_and_mk_assign model (fun (v, _) -> VS.mem v var_set |> not)
      ...
      |> List.map (fun (sv, values) ->
        ...
        Format.printf "VAR: %a = %a@."
          Var.pp_print_var v (Model.pp_print_value ~as_type:t) vl; *)
                   sv,
                   List.rev (vl :: List.rev values) )
             | None -> assert false)

let compute_deadlocking_trace_max_smt_impl solver sys cex offset act_terms =
  List.iter (fun a -> SMTSolver.assert_soft_term solver a 1) act_terms;
...
let compute_dependent_conflict_and_deadlocking_trace solver in_sys sys cex
    offset act_terms guarantee_mode_elts_core contr_vars =
  match Flags.Smt.solver () with
  | `Z3_SMTLIB ->
      SMTSolver.push solver;
...
let compute_deadlocking_trace_and_conflict analyze in_sys param sys
  ...
  let sys, slice_to_prop =
    ...
    | Some { LustreNode.is_function } ->
        if is_function && Flags.Contracts.enforce_func_congruence () then
          ...
          let sys, _ =
            InputSystem.trans_sys_of_analysis ~add_functional_constraints:true
              in_sys param
          in
          (sys, false)
        else (sys, true)
  ...
  let vr, cex, is_base, offset, contr_vars =
    ...
    | InductiveCase (vr_wo, vr_w) -> (
        ...
        | Property.PropKTrue _ | Property.PropUnknown ->
            raise
              (Trace_or_conflict_computation_failed
  ...
  let solver =
    SMTSolver.create_instance ~produce_unsat_assumptions:true
      ~produce_models:true ~minimize_cores:true (TSys.get_logic sys)
      (Flags.Smt.solver ())
  in
  ...
  let act_terms =
    let actlits = get_and_declare_actlits solver sys guarantee_mode_elts_core in
    List.map Actlit.term_of_actlit actlits
  in

  let constraints =
    let neg_vr = SMTSolver.simplify_term solver (Term.negate vr) in
    Term.mk_and (neg_vr :: (cex_assigns @ guarantee_mode_elts_term @ other_term))
  in
  ...
  in*)
  let cex', conflict_set =
    compute_dependent_conflict_and_deadlocking_trace solver in_sys sys cex
      offset act_terms guarantee_mode_elts_core contr_vars
  in
