...
let conv = { D.smtlib_string_sexpr_conv with D.prime_symbol = Some s_prime }
...
let init_pred_hname pred =
  Format.sprintf "%s_%s_0" Ids.init_uf_string (HString.string_of_hstring pred)

(* Return the name of the transition relation predicate *)
let trans_pred_hname pred =
  Format.sprintf "%s_%s_0" Ids.trans_uf_string (HString.string_of_hstring pred)

(* Return a type of an S-expression *)
let type_of_sexpr = function
  (* Integer range type *)
  | HS.List [ HS.Atom c; HS.Atom l; HS.Atom u ] when c == s_intrange ->
      (* Numeral of lower bound *)
      let nl =
        try Numeral.of_string (HString.string_of_hstring l)
      ...
      let nu =
        try Numeral.of_string (HString.string_of_hstring u)
        with Invalid_argument _ ->
          failwith "Invalid argument for integer range type"
      in
...
let state_var_of_sexpr = function
  ...
  | HS.List (HS.Atom v :: t :: opts) ->
      ...
      let var_name, scope =
        Lib.extract_scope_name (HString.string_of_hstring v)
      in
      ...
      ( StateVar.mk_state_var ~is_input ~is_const ~for_inv_gen var_name scope
          var_type,
        is_init_flag )
...
let trans_args_of_state_vars state_vars =
  ...
  let at1 =
    List.map (fun v -> Var.bump_offset_of_state_var_instance v Numeral.one) at0
  in
...
let state_var_of_atom = function
  | HS.Atom v -> StateVar.state_var_of_long_string (HString.string_of_hstring v)
...
let subsystems_of_sexpr = function
  | HS.List
      [
        ...
        HS.List m;
        HS.List
          [ HS.Atom f; HS.List [ HS.List [ HS.Atom v; ty ] ]; HS.List [ g ] ];
      ]
    when f == s_lambda ->
      (* Get name of caller *)
      let name = c in
      ...
      let subsys =
        ...
        with Not_found ->
          failwith
            (Format.sprintf "Undefined subsystem %s"
...
let prop_source_of_sexpr prop_term = function
  ...
  | [ HS.Atom c; HS.Atom scopedprop ] ->
      let p, scope =
        Lib.extract_scope_name (HString.string_of_hstring scopedprop)
      in
      if c == s_inst then
        let rec prop =
          {
  ...
  | [ HS.Atom c; HS.Atom pos; HS.Atom scopedprop ] ->
      ...
      if c == s_assumption then
        Property.Assumption (ppos, (scope, Lib.dummy_pos))
...
let rec optional_fields_of_sexprs (subsystems, props) = function
  (* Subsystems *)
  | HS.List (HS.Atom s :: subs_l) :: rs when s == s_subsystems ->
      let subsystems =
        List.map subsystems_of_sexpr subs_l |> merge_subsystem_insts
      in
      optional_fields_of_sexprs (subsystems, props) rs
  (* Propeties *)
  | HS.List [ HS.Atom p; HS.List ps ] :: rs when p == s_props ->
...
let node_def_of_sexpr = function
  ...
  | HS.List
      ...
      :: others)
    when c == s_define_node && ci == s_init && ct = s_trans ->
      ...
      let init_flag =
        match List.filter snd state_vars_b with
      ...
      ( node_name,
        ...
        subsystems,
        props )
  | HS.Atom _ | HS.List _ -> failwith "Invalid format of node definition"
...
let rec mk_subsys_structure sys =
  {
    ...
    has_impl = true;
    has_modes = false;
    subsystems = TransSys.get_subsystems sys |> List.map mk_subsys_structure;
  }
    ...
    List.map
      (fun sexp ->
        let ( node_name,
              ...
              subsystems,
              props ) =
          node_def_of_sexpr sexp
        in

        (* Scope from name *)
        let node_scope =
          let n, s =
            Lib.extract_scope_name (HString.string_of_hstring node_name)
          in
          s @ [ n ]
        in
        ...
        let sys, _ =
          TransSys.mk_trans_sys node_scope [] None init_flag
            (* [] *)
            state_vars StateVar.StateVarSet.empty (* underapproximation *)
            (StateVar.StateVarHashtbl.create 7)
            [] (* global consts *)
            [] (* global constraints *)
            [] (* ufs *)
            init_uf_symbol init_args init_term trans_uf_symbol trans_args
            trans_term subsystems props (None, []) (Invs.empty ())
        in
...
let pp_print_var ppf v =
  if Var.is_state_var_instance v then
    ...
    let rec add_primes ppf = function
      ...
      | n when n > 0 -> Format.fprintf ppf "(prime %a)" add_primes (n - 1)
      | _ -> assert false
    in
...
let pp_print_state_var sys ppf state_var =
  Format.fprintf ppf "@[<hv 1>(%a@ %a%t%t%t%t)@]" StateVar.pp_print_state_var
...
let pp_print_prop_source sys ppf = function
  ...
  | Property.Generated (_, state_vars) ->
      Format.fprintf ppf ":generated@ (@[<v>%a@])"
        (pp_print_list (pp_print_state_var sys) "@ ")
        state_vars
  ...
  | Property.Candidate _ -> Format.fprintf ppf ":candidate"
  | _ -> () (* TODO *)

let pp_print_property sys ppf { Property.prop_name; prop_source; prop_term } =
  Format.fprintf ppf "@[<hv 1>(\"%s\"@ %a@ %a)@]" prop_name pp_print_term
    prop_term (pp_print_prop_source sys) prop_source
...
let pp_print_subsystem name_c ppf { TransSys.map_down; guard_clock } =
  Format.fprintf ppf "(%s@ @[<hv 1>(%a)@,@[<v>%a@]@])" name_c
    (pp_print_list
       (fun ppf (s, t) ->
         Format.fprintf ppf "@[<hv 1>(@[<hv>%a@]@ @[<hv>%a@])@]"
           StateVar.pp_print_state_var s StateVar.pp_print_state_var t)
       "@ ")
...
let pp_print_subsystems ppf (t, inst) =
  ...
  Format.fprintf ppf "@[<hv 1>%a@]"
    (pp_print_list (pp_print_subsystem name_s) "@ ")
    inst
...
let pp_print_props ppf sys =
  ...
  in
  if props <> [] then
    Format.fprintf ppf "@[<hv 2>(props@ (@[<v>%a@]))@]\n@,"
      (pp_print_list (pp_print_property sys) "@ ")
      props

let pp_print_subsystems ppf sys =
  let subs = TransSys.get_subsystem_instances sys in
  if subs <> [] then
    Format.fprintf ppf "@[<hv 2>(subsystems@ @[<v>%a@])@]\n@,"
      (pp_print_list pp_print_subsystems "@ ")
      subs

let eq_sys s1 s2 =
  TransSys.equal_scope s1 s2
  && List.for_all2 StateVar.equal_state_vars (TransSys.state_vars s1)
       (TransSys.state_vars s2)
  && Term.equal
...
let all_systems sys =
  let rec all_systems_rec acc sys =
    let acc =
      List.fold_left all_systems_rec acc (TransSys.get_subsystems sys)
    in
    let acc = if List.exists (eq_sys sys) acc then acc else sys :: acc in
    acc
  in
...
let pp_print_one_native ppf sys =
  Format.fprintf ppf
    ...
    (pp_print_list (pp_print_state_var sys) "@ ")
    (TransSys.state_vars sys) pp_print_term
    (TransSys.init_of_bound None sys TransSys.init_base)
    pp_print_term
    (TransSys.trans_of_bound None sys TransSys.trans_base)
    pp_print_subsystems sys pp_print_props sys
...
let dump_native sys =
  ...
  let filename =
    Filename.concat dirname
      (Format.sprintf "%s.kind2" (Filename.basename (Flags.input_file ())))
  in
