...
(** Child processes forked. This is an association list of PID to process type.
    We need a reference here, because we may need to terminate asynchronously
    after an exception. *)
let child_pids = ref []
...
let renice () =
  ...
  if nice < 0 then
    KEvent.log L_info "[renice] ignoring negative niceness value."
  else if nice > 0 then
...
let main_of_process = function
  | GenericCall m -> (
      ...
      | `IC3 -> IC3.main
      | `IC3QE -> IC3.main
      | `IC3QEl1l2 -> IC3l1l2.main
      | `IC3QEl2l1 -> IC3l2l1.main
      | `IC3IA -> assert false
      | `BMC -> BMC.main false
      ...
      | `INVGEN ->
          renice ();
          InvGen.main_bool true
      | `INVGENOS ->
          renice ();
          InvGen.main_bool false
      | `INVGENINT ->
          renice ();
          InvGen.main_int true
      | `INVGENINTOS ->
          renice ();
          InvGen.main_int false
      | `INVGENINT8 ->
          renice ();
          InvGen.main_int8 true
      | `INVGENINT8OS ->
          renice ();
          InvGen.main_int8 false
      | `INVGENINT16 ->
          renice ();
          InvGen.main_int16 true
      | `INVGENINT16OS ->
          renice ();
          InvGen.main_int16 false
      | `INVGENINT32 ->
          renice ();
          InvGen.main_int32 true
      | `INVGENINT32OS ->
          renice ();
          InvGen.main_int32 false
      | `INVGENINT64 ->
          renice ();
          InvGen.main_int64 true
      | `INVGENINT64OS ->
          renice ();
          InvGen.main_int64 false
      | `INVGENUINT8 ->
          renice ();
          InvGen.main_uint8 true
      | `INVGENUINT8OS ->
          renice ();
          InvGen.main_uint8 false
      | `INVGENUINT16 ->
          renice ();
          InvGen.main_uint16 true
      | `INVGENUINT16OS ->
          renice ();
          InvGen.main_uint16 false
      | `INVGENUINT32 ->
          renice ();
          InvGen.main_uint32 true
      | `INVGENUINT32OS ->
          renice ();
          InvGen.main_uint32 false
      | `INVGENUINT64 ->
          renice ();
          InvGen.main_uint64 true
      | `INVGENUINT64OS ->
          renice ();
          InvGen.main_uint64 false
      | `INVGENREAL ->
          renice ();
          InvGen.main_real true
      | `INVGENREALOS ->
          renice ();
          InvGen.main_real false
      | `C2I ->
          renice ();
          C2I.main
      | `Interpreter -> Flags.Interpreter.input_file () |> Interpreter.main
      | `Supervisor -> assert false
      | `INVGENMACH | `INVGENMACHOS | `MCS | `CONTRACTCK | `Parser | `Certif ->
          fun _ _ _ -> ())
...
let on_exit_of_process mdl =
  ...
  | `IC3QEl1l2 -> IC3.on_exit None
  | `IC3QEl2l1 -> IC3.on_exit None
  | `IC3IA -> IC3IA.on_exit None
  | `BMCSKIP | `BMC -> BMC.on_exit None
...
let status_of_safety_results in_sys =
  ...
  | `RESULTS_AND_ERRORS -> (
      ...
      | Some true ->
          ...
          if l1 = l2 then ExitCodes.success else report_incomplete_analysis ())
...
let status_of_realiz_results in_sys =
  ...
  let report_incomplete_analysis () =
    KEvent.log L_note
      "Incomplete analysis result: Not all imported nodes could be proven \
       realizable";
    ExitCodes.incomplete_analysis
  in
  match Flags.exit_code_mode () with
...
let status_of_exn process status = function
  ...
  | Exit -> status
  (* Parser error *)
  | LustreAst.Parser_error | Parsing.Parse_error -> ExitCodes.parse_error
  (* Got unknown, issue error but normal termination. *)
  | SMTSolver.Unknown ->
      KEvent.log L_warn
        "In %a: a check-sat resulted in `unknown`. This is most likely due to \
         non-linear expressions in the model, usually multiplications `v_1 * \
         v_2` or divisions `v_1 / v_2`.%t"
        pp_print_kind_module process (fun fmt ->
  ...
  (* Signal caught. *)
  | Signal s ->
      if s = Sys.sigint then InvarManager.print_stats !latest_trans_sys;
      KEvent.log_interruption s;
      (* Return exit status and signal number. *)
      ExitCodes.kid_status + s
  (* Runtime failure. *)
  | IC3.UnsupportedFeature msg | IC3IA.UnsupportedFeature msg ->
      KEvent.log L_warn "%s" msg;
      status
  | Failure msg ->
      if
        msg
        = "SMT solver failed: non-linear arithmetic is not allowed in logic ALL"
      then (
        (* Yices 2 error *)
        KEvent.log L_error
          "In %a: no logic has been provided, but Yices 2 requires one when@ \
           the model contains non-linear expressions.@ Consider running Kind 2 \
           with `--smt_logic detect`"
          pp_print_kind_module process;
        status)
      else (
        InvarManager.print_stats !latest_trans_sys;
        KEvent.log L_fatal "Runtime failure in %a: %s" pp_print_kind_module
          process msg;
        ExitCodes.error)
  (* Other exception, return exit status for error. *)
  | e ->
      InvarManager.print_stats !latest_trans_sys;
      KEvent.log L_fatal "Runtime error in %a: %s" pp_print_kind_module process
...
let slaughter_kids process sys =
  Signals.ignoring_sigalrm (fun _ ->
      ...
      (try
         while !child_pids <> [] do
           try
             ...
             KEvent.log L_debug "Process %d %a" pid pp_print_process_status
               status
           with
      ...
      if !child_pids <> [] then KEvent.log L_fatal "Some children did not exit.";
      (* Cleaning kids list. *)
      child_pids := [];
      (* Draining mailbox. *)
      KEvent.recv () |> ignore);
...
let on_exit_success process exn = on_exit None process ExitCodes.success exn
...
(** Call cleanup function of process and exit. Give the exception [exn] that was
    raised or [Exit] on normal termination. *)
let on_exit_child ?(_alone = false) messaging_thread process exn =
  ...
  (match messaging_thread with Some t -> KEvent.exit t | None -> ());
...
let run_process in_sys param sys messaging_setup process =
  ...
  | 0 -> (
      ...
      try
        ...
        KEvent.log L_debug "Starting new process %a with PID %d"
          pp_print_kind_module kind_module pid;
        ...
        | Some f -> (
            try
              ...
              let f' =
                Format.sprintf "%s.%s-%d" f
                  (debug_ext_of_process kind_module)
                  pid
              in
      ...
      | e ->
          ...
          if Printexc.backtrace_status () then
            KEvent.log L_fatal "Caught %s in %a.@ Backtrace:@ %a"
              (Printexc.to_string e) pp_print_kind_module kind_module
...
let create_processes slice_to_prop modules sys =
  let ic3ia_module, other_modules =
    modules |> List.partition (function `IC3IA -> true | _ -> false)
  in
  ...
  | _ ->
      if Flags.IC3IA.max_processes () > 0 then
        let qe_itp_support_model =
          let check_system_is_supported itp_solver =
            ...
            if not supported then
              KEvent.log L_warn
                "IC3IA (%s) disabled: system includes an abstract function. \
          ...
          List.fold_left
            (fun acc p ->
              ...
              | (`cvc5_QE | `Z3_QE) when qe_itp_support_model ->
                  IC3IA_Call
                    (false, slice_to_prop, p, fresh_ic3ia_instance_name ())
                  :: IC3IA_Call
                       (true, slice_to_prop, p, fresh_ic3ia_instance_name ())
                  :: acc
              | `cvc5_QE | `Z3_QE -> acc
              | _ ->
                  IC3IA_Call
        ...
        in
        let to_run, pending =
          list_split (Flags.IC3IA.max_processes ()) pending
        in
        (List.rev_append to_run processes, pending)
      else (processes, [])

let process_invgen_mach_modules sys (modules : Lib.kind_module list) :
    Lib.kind_module list =
  let invgenmach_modules, other_modules =
    modules
    |> List.partition (function
         | `INVGENMACH | `INVGENMACHOS -> true
         | _ -> false)
  in
      ...
      | `Inferred fs when mem BV fs ->
          let other_modules =
            if List.mem `INVGENMACHOS invgenmach_modules then
              `INVGENINT8OS :: `INVGENINT16OS :: `INVGENINT32OS
              :: `INVGENINT64OS :: `INVGENUINT8OS :: `INVGENUINT16OS
              :: `INVGENUINT32OS :: `INVGENUINT64OS :: other_modules
            else other_modules
          in
          let other_modules =
...
let process_bmc_modules sys (modules : Lib.kind_module list) :
    Lib.kind_module list =
  if List.mem `BMC modules then
    let has_lb_queries = TSys.props_list_of_bound_skip sys Numeral.zero <> [] in
    if has_lb_queries && not (List.mem `BMCSKIP modules) then
      `BMCSKIP :: modules
    else modules
  else modules
...
let process_ic3_modules (modules : Lib.kind_module list) : Lib.kind_module list
  ...
  if List.mem `IC3 modules then
    let _, other_modules =
      modules
      |> List.partition (function `IC3 | `IC3QE | `IC3IA -> true | _ -> false)
    in
    `IC3QE :: `IC3IA :: other_modules
  else modules

(** Performs an analysis. *)
let analyze msg_setup save_results ignore_props stop_if_falsified slice_to_prop
    modules in_sys param sys =
  Stat.start_timer Stat.analysis_time;

  (if TSys.has_real_properties sys |> not && not ignore_props then
     KEvent.log L_note "System %a has no property, skipping verification step."
       fmt_sys sys
   else
     ...
     to_run |> List.iter (fun p -> run_process in_sys param sys msg_setup p);
     ...
     InvarManager.main
       ...
       pending ignore_props stop_if_falsified child_pids in_sys param sys;

     (* Killing kids when supervisor's done. *)
     Some sys |> slaughter_kids `Supervisor);

  let result = Stat.get_float Stat.analysis_time |> Anal.mk_result param sys in
...
let handle_exception process e =
  ...
  if Printexc.backtrace_status () then
    KEvent.log L_fatal "Caught %s in %a.@\nBacktrace:@\n%a"
      (Printexc.to_string e) pp_print_kind_module process print_backtrace
      backtrace
...
let run in_sys =
  ...
  | [ m ] when m = `Interpreter -> (
      ...
      try
        ...
        let sys, _ =
          ISys.trans_sys_of_analysis ~preserve_sig:true ~slice_nodes:false
            in_sys param
        in
        (* Run interpreter. *)
        Interpreter.main (Flags.Interpreter.input_file ()) in_sys param sys;
  ...
  | [ m ] when m = `CONTRACTCK -> (
      ...
      *)
      ...
      try
        ...
        KEvent.run_im msg_setup [] (on_exit_realiz_results in_sys `CONTRACTCK)
        |> ignore;
        ...
        | [] ->
            KEvent.log L_note
              "No imported nodes found, skipping contract checking."
        | params ->
            Flags.Arrays.set_smt true;
            (* Uninterpreted functions are not supported *)
            params
            |> List.iter (fun (param, has_contract) ->
                   ...
                   let result =
                     ...
                     else if satisfy_input_requirements in_sys param then
                       ContractChecker.check_contract_realizability in_sys sys
                     else Realizability.Unknown
                   in
                   ...
                   (try
                      Log.log_result
                        (ContractChecker.pp_print_realizability_result_pt
                           (analyze msg_setup false false false)
                           in_sys param sys)
                        ...
                        result
                    with
                    | Realizability.Trace_or_conflict_computation_failed msg ->
                      KEvent.log L_warn "%s" msg);
                   ...
                   | Unrealizable _ ->
                       if Flags.Contracts.check_contract_is_sat () then (
                         KEvent.log L_note
                           "Checking satisfiability of contract...";
                         let result =
  ...
  | modules when List.mem `MCS modules -> (
      try
        ...
        | [] ->
            KEvent.log L_note
              "No analyzable nodes found, skipping MCS analysis."
  ...
  | modules -> (
      KEvent.log L_info "@[<hov>Running in parallel mode: @[<v>- %a@]@]"
      ...
      let rec loop ac () =
        ...
        | Some param ->
            ...
            | Some old when TSys.equal_scope old sys |> not ->
                PostAnalysis.run in_sys
                  (TSys.scope_of_trans_sys old)
                  (analyze msg_setup false) !all_results
            | _ -> ());
    ...
    No kids yet. *)
      KEvent.run_im msg_setup [] (on_exit_safety_results in_sys `Supervisor)
      |> ignore;
      KEvent.log L_debug "Messaging initialized in supervisor.";
      ...
      try
        ...
        if
          Analysis.results_is_empty !all_results
          && InputSystem.analyzable_subsystems in_sys = []
        then KEvent.log L_note "No analyzable nodes found, skipping analysis.";

        let results = !all_results |> Analysis.results_clean in
        ...
        |> List.fold_left
             (fun l sys ->
               try
                 ...
                 | [] ->
                     Format.asprintf
                       "Unreachable: no results at all for system %a."
