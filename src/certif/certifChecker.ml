...
let names_bare = { vars = []; init = "I"; prop = "P"; trans = "T"; phi = "PHI" }

let names_kind2 vars =
  { vars; init = "I1"; prop = "P1"; trans = "T1"; phi = "PHI1" }

let names_jkind vars =
  { vars; init = "I2"; prop = "P2"; trans = "T2"; phi = "PHI2" }

let names_obs =
  { vars = []; init = "IO"; prop = "PO"; trans = "TO"; phi = "PHIO" }
    ...
    (fun _ term ->
      if Term.is_free_var term then
        ...
        if Var.is_state_var_instance v then
          ...
          in
          Term.mk_uf vf [ arg ]
        else if Var.is_const_state_var v then
          let sv = Var.state_var_of_state_var_instance v in
          let vf = StateVar.uf_symbol_of_state_var sv in
          Term.mk_uf vf []
        else term
  ...
  Term.map
    (fun _ t ->
      ...
      | Term.T.Node (cmp, (_ :: _ :: _ :: _ as l)) -> (
          match Symbol.node_of_symbol cmp with
          | `LEQ | `LT | `GEQ | `GT -> split_cmp [] cmp l
          | _ -> t)
      | _ -> t)
    term
...
let assert_expr fmt expr =
  fprintf fmt "@[<hv 1>(assert@ @[<hov>%a@])@]@." SMT.pp_print_expr
    (preproc expr)

(* Declare a new function symbol *)
let declare_fun fmt fun_symbol arg_sorts res_sort =
  fprintf fmt "@[<hv 1>(declare-fun@ %s@ @[<hv 1>%s@]@ %a)@]@." fun_symbol
...
let define_fun ?(trace_lfsc_defs = false) fmt fun_symbol arg_vars res_sort defn
    =
  fprintf fmt "@[<hov 1>(define-fun@ %s@ @[<hv 1>(%a)@]@ %s@ %a)@]\n@."
...
let extract_props_terms sys =
  List.fold_left
    (fun p_acc -> function { Property.prop_term = p } -> p :: p_acc)
    []
...
let extract_props_certs sys =
  ...
  let certs, invs =
    ...
    KEvent.log_uncond "[INV] %a -----> %i:%a" Term.pp_print_term i k Term.pp_print_term p' ; *)
        (c :: c_acc, i :: i_acc))
      ([], [])
      (TS.get_invariants sys |> Invs.flatten)
  in

  let certs, props =
    List.fold_left
      (fun ((c_acc, p_acc) as acc) -> function
        ...
        KEvent.log_uncond "[PROP] %a -----> %i:%a" Term.pp_print_term p k Term.pp_print_term p' ; *)
            ( (if List.exists (Term.equal p) invs then c_acc else c :: c_acc),
              p :: p_acc )
        | { Property.prop_name } ->
    ...
    List.fold_left
      (fun certs -> function
        | {
            ...
            prop_source = Property.Candidate _;
          } ->
            c :: certs
        | { Property.prop_name } ->
            ...
            certs)
      certs
      (TS.get_candidate_properties sys)
  in
...
let create_acts solver k inv =
  ...
  let not_act_k =
    Term.bump_state (Numeral.of_int k) inv |> Term.mk_not |> actlitify solver
  in
...
let at_most_one_false solver acts =
  let acts_cptl =
    List.map
      (fun t -> Term.mk_ite t (Term.mk_num_of_int 0) (Term.mk_num_of_int 1))
      acts
  in
...
*)
let rec trim solver uinvs_acc invs_acts prev_props_act prop'act neg_prop'act
    trans_acts =
  let if_sat _ =
  ...
  let if_unsat _ =
    ...
    SMTSolver.check_sat_assuming solver
      (fun _ ->
        ...
        Debug.certif "[Fixpoint] could not verify inductiveness";

        trim solver uinvs_acc invs_acts new_prop_act new_prop'act
          neg_new_prop'act trans_acts)
      (fun _ ->
...
let rec check_ind_and_trim ~just_check_ind sys k prop invs_terms solver
    invs_acts prev_props_act prop'act neg_prop'act trans_acts =
  (* Get invariants at k - 1 *)
  let invs, invs' = List.split invs_acts in

  let neg_invs'prop'act =
    prop'act :: invs' |> Term.mk_and |> Term.mk_not |> actlitify solver
  in
  ...
  SMTSolver.check_sat_assuming solver
      ...
      if Flags.BmcKind.compress () then
        ...
        let model =
          SMTSolver.get_var_values solver svi
            (TransSys.vars_of_bounds sys Numeral.zero (Numeral.of_int k))
        in
        let path =
          Model.path_from_model (TransSys.state_vars sys) model
            (Numeral.of_int k)
        in
        match
          Compress.check_and_block (SMTSolver.declare_fun solver) sys path
        with
        | [] ->
            Debug.certif "[Fixpoint] no compression, failure";
            raise Exit
        | compressor ->
            let compr_act = actlitify solver (Term.mk_and compressor) in
            (* try again *)
            check_ind_and_trim ~just_check_ind sys k prop invs_terms solver
              invs_acts prev_props_act prop'act neg_prop'act
    ...
    (fun _ ->
      ...
      let uc = SMTSolver.get_unsat_core_lits solver in
      let invs_acts =
        List.filter (fun (a, _) -> List.exists (Term.equal a) uc) invs_acts
      in
      ...
      let cont () =
        ...
        | _ ->
            ...
            ( trim solver [] invs_acts prev_props_act prop'act neg_prop'act
                trans_acts,
              must )
      in

      if just_check_ind then raise (Reduce_cont cont) else cont ())
...
*)
let rec cherry_pick solver trans needed_invs remaining_invs prev_props_act
    prop'act neg_prop'act trans_acts =
  let needed_invs_acts, needed_invs'acts = List.split needed_invs in

  let neg_invs'prop'act =
    prop'act :: needed_invs'acts
    |> Term.mk_and |> Term.mk_not |> actlitify solver
  in
  ...
  SMTSolver.check_sat_assuming solver
    (fun _ ->
      ...
      (* Evaluation function. *)
      let eval term =
        Eval.eval_term (TransSys.uf_defs trans) model term |> Eval.bool_of_value
      in
        ...
        let needed_invs =
          List.fold_left
            (fun acc (inv, inv') ->
              ...
              (invact, inv'act) :: acc)
            needed_invs extra_needed
        in

        Debug.certif "Hard minimization identified %d" (List.length needed_invs);

        (* recursive call to find out if we have found an inductive set or not *)
        cherry_pick solver trans needed_invs other_invs prev_props_act prop'act
          neg_prop'act trans_acts)
    (fun _ ->
      (* UNSAT *)
...
let try_at_bound ?(just_check_ind = false) sys solver k invs prop trans_acts =
  ...
  let cherry_pick_min (useful_acts, must_invs) =
    ...
    let needed, to_consider =
      List.fold_left
        (fun acc a ->
          List.fold_left
            (fun (needed, to_consider) (inv, _, prev_inv, a', inv') ->
              if Term.equal a a' then
                ...
                else (needed, (prev_inv, inv') :: to_consider)
    ...
    cherry_pick solver sys
      (* [] useful_infos *)
      needed to_consider prev_props_act prop'act neg_prop'act trans_acts
  ...
  try
    ...
    let useful_acts, must_invs =
      check_ind_and_trim ~just_check_ind sys k prop invs solver invs_acts
        prev_props_act prop'act neg_prop'act trans_acts
    in

    (* If fixpoint returned a list of useful invariants we just return them *)
    Inductive
      (follow_up (useful_acts, must_invs) (* map_back_to_invs useful_acts *))
  with
  | Exit ->
  ...
  if k > kmax then
    raise
      (CouldNotProve
         (fun fmt ->
           Format.fprintf fmt
             "[Certification] simplification of inductive invariant went over \
              bound %d"
             kmax));
...
let unroll_trans_actlits sys solver kmax =
  let rec fill acc prev = function
    ...
    | k ->
        ...
        in
        let tuptok =
          match prev with Some p -> Term.mk_and [ p; tk ] | _ -> tk
        in
        ...
        | _, Not_inductive ->
            raise
              (CouldNotProve
                 (fun fmt ->
                   Format.fprintf fmt
                     "[Certification] Could not verify %d-inductiveness of \
                      invariant"
                     k))
...
let rec loop_dicho sys solver kmax invs prop trans_acts_map acc k_l k_u =
  if k_l > k_u then
    ...
    match
      let res =
        try_at_bound ~just_check_ind:false sys solver k_mid invs prop
          [ trans_act ]
      in
      res
    with
    | Not_inductive ->
        ...
        loop_dicho sys solver kmax invs prop trans_acts_map acc (k_mid + 1) k_u
    | res ->
        (* Inductive, register and Look for non-inductiveness on the left *)
        loop_dicho sys solver kmax invs prop trans_acts_map (k_mid, res) k_l
          (k_mid - 1)
...
let find_bound_frontier_dicho sys solver kmax invs prop =
  ...
  let res_kmax_m1 =
    if kmax - 1 = 0 then Not_inductive
      ...
      try_at_bound ~just_check_ind:true sys solver (kmax - 1) invs prop
        [ trans_act ]
  in
  ...
  | Not_inductive, _ ->
      raise
        (CouldNotProve
           (fun fmt ->
             Format.fprintf fmt
               "[Certification, frontier dicho] Could not verify \
                inductiveness@ of invariant"))
  | Inductive useful, Not_inductive -> (kmax, useful)
  | Inductive_to_reduce f, Not_inductive -> (kmax, f ())
  | (Inductive _ | Inductive_to_reduce _), (Inductive _ | Inductive_to_reduce _)
    ->
...
let minimize_invariants sys props invs_predicate =
  ...
  let props', certs = extract_props_certs sys in
  let props = match props with Some props -> props | None -> props' in
  let certs =
    match invs_predicate with
    | None -> certs
    | Some filter -> certs |> List.filter (fun (_, inv) -> filter inv)
  in
  ...
  let k, invs =
    List.fold_left
      (fun (m, invs) (k, i) ->
        ( max m k,
          if List.exists (Term.equal i) props || List.exists (Term.equal i) invs
          then invs
          else i :: invs ))
      (0, []) certs
  in
  ...
  if Flags.Smt.check_sat_assume () && invs <> [] then (
    (* For stats *)
    ...
    let solver =
      SMTSolver.create_instance ~produce_unsat_assumptions:true
        ~produce_models:true logic (Flags.Smt.solver ())
    in
    ...
    let min_strategy =
      match Flags.Certif.mink () with
      | `No -> assert false
      | (`Fwd | `Bwd | `Dicho | `FrontierDicho) as s -> s
      | `Auto ->
          (* Heuristic to find best strategy *)
          if k <= 3 then `Fwd else if k <= 20 then `Dicho else `FrontierDicho
    in
...
let minimize_certificate sys = minimize_invariants sys None None
...
let is_fec sys =
  match TransSys.scope_of_trans_sys sys with "OBS" :: _ -> true | _ -> false

(* Add a set-info header *)
let set_info fmt tag str =
  fprintf fmt "@[<hv 1>(set-info@ :%s@ @[<hv>%s@])@]@." tag str

(* System specific header for definitions *)
let add_system_header fmt prefix =
  set_info fmt "system"
    (sprintf "\"Logical transition system generated by %s\"" prefix);
...
let add_logic ?(compliant = false) fmt sys =
  ...
  let logic =
    ...
    | `Inferred l ->
        `Inferred
          (l |> FeatureSet.add UF |> FeatureSet.add IA
...
let export_system_defs fmt ?(recursive = true) ?(trace_lfsc_defs = false) prefix
    consts sys =
  (* add headers for info *)
  ...
  if consts <> [] then (
    add_section fmt "Constants";
    List.iter
      (fun sv -> declare_const fmt (StateVar.uf_symbol_of_state_var sv))
      consts;
  ...
  let defs =
    if recursive then TS.uf_defs sys
      ...
      match TS.uf_defs sys |> List.rev with
      | t :: i :: _ -> [ i; t ]
      | _ -> assert false
  in
  List.iter
    (fun (f, (a, d)) -> define_fun ~trace_lfsc_defs fmt f a Type.t_bool d)
    defs
...
let s_define_pred ?(trace_lfsc_defs = false) fmt fun_symbol args defn =
  ...
  fprintf fmt "(define-fun %s (%a) Bool\n   %s)\n@." fun_symbol
    (pp_print_list (fun fmt f -> fprintf fmt "(%s %s)" f sindex_sort) " ")
    args defn;
  ...
  if trace_lfsc_defs then (
    ...
    fprintf fmt
      "@[<hov 1>(assert@ @[<hov 1>(=@ @[<hv 1>(%s@ %a)@]@ @[<hv 1>(%s@ \
       %a)@])@])@]\n\
       @."
      fun_def_sy
      (pp_print_list pp_print_string "@ ")
      fun_def_args fun_symbol
      (pp_print_list pp_print_string "@ ")
      fun_def_args)
...
let mononames_system fmt ~trace_lfsc_defs sys name_sys prop names =
  ...
  export_system_defs fmt ~recursive:true ~trace_lfsc_defs:false name_sys consts
    sys;

  (* Declaring state variables upto k *)
  add_section fmt "State variables";
  List.iter
    (fun sv -> declare_state_var fmt (StateVar.uf_symbol_of_state_var sv))
    svars;
  ...
  add_section fmt "Transition_relation";
  let trans_s =
    UfSymbol.mk_uf_symbol names.trans [ ty_index (); ty_index () ] Type.t_bool
  in
  let t01 = TransSys.trans_fun_of sys Numeral.zero Numeral.one in
  let trans_def = roll sigma_0i1j t01 in
...
let export_system ~trace_lfsc_defs dirname file names sys name_sys =
...
let smk s l =
  asprintf "@[<hov 1>(%s@ %a)@]" s (pp_print_list pp_print_string "@ ") l
...
let mononames_base_check sys dirname file definitions_files k names =
  ...
  for i = k - 1 downto 0 do
    ...
    for j = i - 1 downto 0 do
      l :=
        smk names.trans [ index_sym_of_int j; index_sym_of_int (j + 1) ] :: !l
    done;

    let conj =
      smk_and
        [
          smk_and (smk names.init [ index_sym_of_int 0 ] :: !l);
          smk "not" [ smk names.phi [ index_sym_of_int i ] ];
        ]
    in
...
let mononames_induction_check sys dirname file definitions_files k names =
  ...
  for i = k - 1 downto 0 do
    l :=
      smk_and
        [
          smk names.phi [ index_sym_of_int i ];
          smk names.trans [ index_sym_of_int i; index_sym_of_int (i + 1) ];
        ]
      :: !l
  done;

  let g =
    smk_and [ smk_and !l; smk "not" [ smk names.phi [ index_sym_of_int k ] ] ]
  in
...
let mononames_implication_check sys dirname file definitions_files names =
  ...
  let v = "0" in
  let f = smk "=>" [ smk names.phi [ v ]; smk names.prop [ v ] ] in
...
let generate_split_certificates sys dirname =
  ...
  export_phi ~trace_lfsc_defs:false dirname kind2_phi_f [ kind2_defs_path ]
    names_kind2 sys phi;

  export_phi ~trace_lfsc_defs:true dirname kind2_phi_lfsc_f [ kind2_defs_path ]
    names_kind2 sys phi;
  ...
  let base =
    mononames_base_check sys dirname base_f smt2_definitions k names_kind2
  in

  let induction =
    mononames_induction_check sys dirname induction_f smt2_definitions k
      names_kind2
  in

  let implication =
    mononames_implication_check sys dirname implication_f smt2_definitions
      names_kind2
  in
  ...
  Debug.certif "SMT-LIB2 intermediate certificates were written in %s" dirname;
...
let generate_certificate sys dirname =
  ...
  let certificate_filename =
    Filename.concat dirname
      (if is_fec sys then "FECC.smt2" else "certificate.smt2")
  in
  ...
  (* add headers for info *)
  add_header fmt_header sys k names_bare.init names_bare.prop names_bare.trans
    names_bare.phi;
  ...
  add_section fmt "Initial states";
  let init_s =
    UfSymbol.mk_uf_symbol names_bare.init [ ty_index () ] Type.t_bool
  in
  let i0 = TransSys.init_fun_of sys Numeral.zero in
  let init_def = roll sigma_0i i0 in
  define_fun fmt init_s [ fvi ] Type.t_bool init_def;
  ...
  add_section fmt "Transition_relation";
  let trans_s =
    UfSymbol.mk_uf_symbol names_bare.trans
      [ ty_index (); ty_index () ]
      Type.t_bool
  in
  let t01 = TransSys.trans_fun_of sys Numeral.zero Numeral.one in
  ...
  add_section fmt "Original property";
  let prop_s =
    UfSymbol.mk_uf_symbol names_bare.prop [ ty_index () ] Type.t_bool
  in
  let prop_def = roll sigma_0i prop in
  define_fun fmt prop_s [ fvi ] Type.t_bool prop_def;
  ...
  let phi_s =
    UfSymbol.mk_uf_symbol names_bare.phi [ ty_index () ] Type.t_bool
  in
  let phi_def = roll sigma_0i (guard_two_state_term_list phi t_fvi) in
  ...
  if monolithic_base then (
    if simple_base then (* Incorrect base case checking *)
      (
      let l = ref [] in
  ...
  (* Show which file contains the certificate *)
  KEvent.log L_note "Certificate checker was written in @{<b>%s@}"
...
let add_scope_state_var scope sv =
  ...
  StateVar.mk_state_var ~is_input:(StateVar.is_input sv)
    ~is_const:(StateVar.is_const sv) ~for_inv_gen:(StateVar.for_inv_gen sv)
...
let mk_init_term init_flag scope sys =
  ...
  Term.mk_uf (TS.init_uf_symbol sys)
    (List.map
       (fun v ->
         if Var.equal_vars v vinit then Term.mk_var viflag
...
let mk_trans_term init_flag scope sys =
  ...
  in
  let viflag =
    Var.mk_state_var_instance init_flag Numeral.(TransSys.trans_base - one)
  in
  let vinit' =
    Var.mk_state_var_instance
      (TransSys.init_flag_state_var sys)
  ...
  Term.mk_uf (TS.trans_uf_symbol sys)
    (List.map
       (fun v ->
         if Var.equal_vars v vinit' then Term.mk_var viflag'
         else if Var.equal_vars v vinit then Term.mk_var viflag
         else Term.mk_var (add_scope_var scope v))
...
let mk_obs_eqs kind2_sys ?(prime = false) ?(prop = false) lustre_vars
    orig_kind2_vars =
  let term_state_var =
    if prime then term_state_var1 [ obs_name ] else term_state_var0 [ obs_name ]
  in

  List.fold_left
    (fun acc sv ->
      let jkind_vars =
        JkindParser.jkind_vars_of_kind2_statevar kind2_sys lustre_vars sv
      in

      Debug.fec "(Kind2->JKind): %a -> [ %a ]" StateVar.pp_print_state_var sv
        (pp_print_list StateVar.pp_print_state_var ", ")
        jkind_vars;

      (* Fail if variables of properties do not have a jKind equivalent *)
      if jkind_vars = [] then (
        KEvent.log L_info "Could not find a match for the%s variable %a."
          (if StateVar.is_input sv then " INPUT" else "")
          StateVar.pp_print_state_var sv;

        if prop (* && jkind_vars = [] *) then (
      ...
      List.fold_left
        (fun acc jv ->
          global_jkind_vars :=
            List.filter
              (fun sv -> not (StateVar.equal_state_vars sv jv))
              !global_jkind_vars;
...
let mk_multiprop_obs ~only_out lustre_vars kind2_sys =
  ...
  let prop_vs =
    List.fold_left
      ...
      SVS.empty
      (TS.get_real_properties kind2_sys)
  in

  let other_vars =
    List.filter
      (fun sv ->
        (not (SVS.mem sv prop_vs))
  ...
  let props_eqs =
    mk_obs_eqs kind2_sys ~prime:false ~prop:true lustre_vars prop_vars
  in
  let others_eqs =
    mk_obs_eqs kind2_sys ~prime:false ~prop:false lustre_vars other_vars
  in
...
let same_inputs kind2_sys ?(prime = false) lustre_vars orig_kind2_vars =
  mk_obs_eqs kind2_sys ~prime lustre_vars
    (List.filter
       (fun sv -> StateVar.is_input sv || is_nondet sv)
       orig_kind2_vars)
  |> Term.mk_and
...
let mk_inst init_flag sys formal_vars =
  let map_down, map_up =
    List.fold_left
      ...
      StateVar.StateVarMap.(empty, empty)
      formal_vars
  in
  ( sys,
    [
...
let add_scope_and_register_bounds scope orig_tbl dest_tbl sv =
  ...
  (try SVH.add dest_tbl sv' (SVH.find orig_tbl sv) with Not_found -> ());
  sv'
...
let merge_systems lustre_vars kind2_sys jkind_sys =
  ...
  let init_flag =
    TS.init_flag_state_var kind2_sys |> add_scope_state_var [ obs_name ]
  in

  (* Create versions of variables with the new scope *)
  let kind2_vars =
    List.map
      (add_scope_and_register_bounds [ obs_name ] kind2_bounds bounds)
      orig_kind2_vars
  in
  let jkind_vars =
    List.map
      (add_scope_and_register_bounds [ obs_name ] jkind_bounds bounds)
      orig_jkind_vars
  in
  let state_vars =
    (* init_flag :: *)
    ...
    |> List.filter (fun sv ->
           not
             (StateVar.equal_state_vars sv
                (TransSys.init_flag_state_var kind2_sys)))
  in
  let vars_types = List.map StateVar.type_of_state_var state_vars in

  let state_vars0 =
    List.map (fun sv -> Var.mk_state_var_instance sv Numeral.zero) state_vars
  in

  let state_vars1 =
    List.map (fun sv -> Var.mk_state_var_instance sv Numeral.one) state_vars
  in
  ...
  let trans_uf =
    UfSymbol.mk_uf_symbol
      ...
      (vars_types @ vars_types) Type.t_bool
  in
  ...
  let init_term =
    Term.mk_and
      [
        ...
        mk_init_term init_flag [ obs_name ] jkind_sys;
      ]
  in
  ...
  let trans_term =
    Term.mk_and
      [
        ...
        mk_trans_term init_flag [ obs_name ] jkind_sys;
      ]
  in
  ...
  Debug.fec "@[<hv 4>Unmatched JKind vars:@,%a@]@."
    (pp_print_list StateVar.pp_print_state_var "@,")
    !global_jkind_vars;
  ...
  let obs_sys, _ =
    TS.mk_trans_sys [ obs_name ] [] None init_flag
      (* [] *)
      state_vars unconstrained_inputs bounds [] (* Global consts *)
      [] (* Global constraints *)
      [] (* Ufs *)
      init_uf init_args init_term trans_uf trans_args trans_term
      [ kind2_subsys_inst; jkind_subsys_inst ]
      props (None, []) (Invs.empty ())
  in
...
let export_obs_system ~trace_lfsc_defs dirname file definitions_files names_obs
    names_kind2 names_jkind kind2_sys same_inputs_term =
  let filename = Filename.concat dirname file in
  ...
  let sip_s =
    UfSymbol.mk_uf_symbol same_inputs_pred [ ty_index () ] Type.t_bool
  in
  ...
  s_define_pred ~trace_lfsc_defs fmt names_obs.init [ "i" ]
    (sprintf "(and (%s i) (and (%s i) (%s i)))" same_inputs_pred
       names_kind2.init names_jkind.init);

  add_section fmt "Transition relation for observer";

  s_define_pred ~trace_lfsc_defs fmt names_obs.trans [ "i"; "j" ]
    (sprintf "(and (%s j) (and (%s i j) (%s i j)))" same_inputs_pred
       names_kind2.trans names_jkind.trans);
...
let generate_frontend_obs node kind2_sys dirname =
  ...
  Debug.fec "Lustre vars:@,%a"
    (fun fmt ->
      StateVar.StateVarMap.iter (fun sv l ->
          List.iter
            (fun (sv', l') ->
              Format.fprintf fmt "%a -> %a : %a@," StateVar.pp_print_state_var
                sv StateVar.pp_print_state_var sv'
                (pp_print_list
                   (fun fmt (lid, n, cond) ->
  ...
  let jkind_props =
    List.fold_left
      (fun acc p ->
        ...
        match
          p.prop_term |> Term.free_var_of_term
        ...
        | jksvs ->
            let jkp =
              List.map
                (fun jksv ->
                  Var.mk_state_var_instance jksv TS.prop_base |> Term.mk_var)
                jksvs
              |> Term.mk_and
            in
        ...
        | exception _ -> acc)
      []
      (TransSys.get_properties kind2_sys)
  in
  ...
  let same_inputs_term =
    same_inputs kind2_sys lustre_vars (TS.state_vars kind2_sys)
  in

  (* Export Observer system in SMT-LIB2 format for use in proof *)
  export_obs_system ~trace_lfsc_defs:false dirname obs_defs_f observer_deps
    names_obs names_kind2 names_jkind kind2_sys same_inputs_term;

  export_obs_system ~trace_lfsc_defs:true dirname obs_defs_lfsc_f observer_deps
    names_obs names_kind2 names_jkind kind2_sys same_inputs_term;
  ...
  (* Show which file contains the certificate *)
  Debug.fec "Frontend eq-observer was written in %s, run Kind 2 on it" filename;
...
let generate_frontend_certificates sys dirname =
  ...
  let deps =
    [ kind2_defs_f; jkind_defs_f; obs_defs_f ]
  ...
  export_phi ~trace_lfsc_defs:true dirname obs_phi_lfsc_f deps names_obs sys phi;
  ...
  let base =
    mononames_base_check sys dirname frontend_base_f smt2_definitions k
      names_obs
  in

  let induction =
    mononames_induction_check sys dirname frontend_induction_f smt2_definitions
      k names_obs
  in

  let implication =
    mononames_implication_check sys dirname frontend_implication_f
      smt2_definitions names_obs
  in
  ...
  (* Show which file contains the certificate *)
  Debug.certif "SMT-LIB2 frontend certificates were written in %s" dirname;
...
let certificate_checker_script =
  "#!/bin/sh\n" ^ "set -e\n" ^ select_solver_script ^ goto_cert_dir
...
let generate_smt2_certificates input sys =
  ...
  let certif_script_name =
    Filename.concat dirname
      (if is_fec sys then "FECC_checker" else "certificate_checker")
  in
  let csoc =
    openfile certif_script_name [ O_WRONLY; O_CREAT; O_TRUNC ] 0o755
  ...
  (* Recursive call *)
  if (not (is_fec sys)) && call_frontend && gen_frontend then (
    KEvent.log L_note "@{<b>Generating frontend certificate@}";
    let cmd_l =
      Array.to_list Sys.argv |> List.filter (fun s -> s <> Flags.input_file ())
    in
...
let generate_all_proofs uid input sys =
  ...
  let dirname =
    if is_fec sys then Filename.dirname (Flags.input_file ())
      ...
      Flags.output_dir () |> mk_dir;
      Filename.concat (Flags.output_dir ()) ("certificates." ^ string_of_int uid))
  in
  create_dir dirname;
  ...
  if not (is_fec sys) then (
    ...
    let final_lfsc =
      Filename.concat (Flags.output_dir ())
        (String.concat "."
           [
             Filename.basename (Flags.input_file ()); string_of_int uid; "lfsc";
           ])
    in
    let final_trust =
      Filename.concat (Flags.output_dir ())
    ...
    if call_frontend then (
      ...
      (* fix_A0 final_lfsc; *)
      (* temporary *)
      KEvent.log L_note "Final @{<green>LFSC proof@} written to @{<b>%s@}"
        final_lfsc;
      if Sys.file_exists final_trust then
        KEvent.log L_warn
          "Some trusted assumptions remain to be proven in @{<b>%s@}"
          final_trust))
  else
    let frontend_inv = generate_frontend_certificates sys dirname in

    (try Proof.generate_frontend_proof frontend_inv
     with e ->
       (* Send statistics *)
       KEvent.stat Stat.[ (certif_stats_title, certif_stats) ];
       raise e);
