...
type lfsc_decl = { decl_symb : H.t; decl_type : lfsc_type }

(* Type of LFSC definitions *)
type lfsc_def = { def_symb : H.t; def_body : lfsc_term }

(* Type of LFSC lemmas *)
type lfsc_lem = { lem_symb : H.t; lem_ty : lfsc_type; lem_body : lfsc_term }
...
(* Empty context *)
let mk_empty_proof_context () = { lfsc_decls = []; lfsc_defs = [] }
...
let print_decl fmt { decl_symb; decl_type } =
  fprintf fmt "@[<hov 1>(declare@ %a@ %a)@]" H.pp_print_hstring decl_symb
...
let rec print_hyps_type ty fmt = function
  ...
  | { decl_symb; decl_type } :: rhyps ->
      fprintf fmt "@[<hov 0>(! %a@ %a@ %a)@]" H.pp_print_hstring decl_symb
        print_type decl_type (print_hyps_type ty) rhyps
...
let rec print_proof_term term fmt = function
  | [] -> print_term fmt term
  | { decl_symb; _ } :: rhyps ->
      fprintf fmt "@[<hov 0>(\\ %a@ %a)@]" H.pp_print_hstring decl_symb
...
and extract_trusts_list acc = function
  | [] -> acc
...
let log_trusted ~frontend dirname =
  if Flags.Certif.log_trust () && !trusted <> [] then (
    let o_flags =
    ...
    KEvent.log L_warn "%s proof contains %d trusted assumptions.@."
...
let proof_from_chan ctx prefix in_ch =
  ...
  | [ Atom a ] when a == s_sat || a == s_unknown ->
      failwith
        (sprintf "Certificate cannot be checked by smt solver (%s)@."
  ...
  | _ ->
      failwith
        (asprintf "No proofs, instead got:\n%a@." HS.pp_print_sexpr_list sexps)
...
let context_from_chan in_ch =
  ...
  | [ Atom a ] ->
      failwith (sprintf "No proofs, instead got:\n%s@." (H.string_of_hstring a))
  | Atom u :: dummy_proof when u == s_unsat -> parse_context_dummy dummy_proof
  | _ ->
      failwith
        (asprintf "No proofs, instead got:\n%a@." HS.pp_print_sexpr_list sexps)
...
let rec pp_print_sort ppf t =
  ...
  | Type.Array (te, ti) ->
      if Flags.Arrays.smt () then
        ...
        Format.fprintf ppf "(cvc.FArray %a %a)" pp_print_sort ti pp_print_sort
          te
...
let context_from_vars vars =
  let to_decl i v =
    ...
    in
    { def_symb = H.mk_hstring ("cvc." ^ symb); def_body = sort }
  in
...
let write_proof_and_check fmt ?(check = true) pname ptype pterm =
  fprintf fmt "@[<hov 1>(define@ %a@ @[<hov 1>(: @[<hov 0>%a@]@ %a)@])@]@.@."
    H.pp_print_hstring pname print_type ptype print_term pterm;

  if check then
    fprintf fmt "@[<hov 1>(check@ %a@])@]@.@." H.pp_print_hstring pname
...
let write_inv_proof fmt ?(check = true) (s_implication, s_base, s_induction)
    name_proof c =
  let open HS in
...
let generate_inv_proof inv =
  ...
  fprintf proof_fmt
    ";;------------------------------------------------------------------\n\
     ...
     ;;------------------------------------------------------------------\n\
     @."
    Version.package_name Version.version (get_cvc5_version ())
    (Flags.input_file ());
...
let generate_frontend_proof inv =
  ...
  fprintf proof_fmt ";; System generated by JKind\n@.%a\n@." print_context
    (intersect_contexts (merge_contexts ctx_vars ctx_obs) ctx_jk);

  fprintf proof_fmt ";; System generated for Observer\n@.%a\n@."
    (print_delta_context (merge_contexts ctx_vars ctx_jk))
    ctx_obs;

  let ctx_phi = context_from_file inv.phi_lfsc_trace_file in
  fprintf proof_fmt ";; k-Inductive invariant for observer system\n@.%a\n@."
    (print_delta_context (merge_contexts ctx_jk ctx_obs))
    ctx_phi;

  let ctx_k2 = context_from_file inv.kind2_system.smt2_lfsc_trace_file in

  let ctx =
    ctx_phi |> merge_contexts ctx_obs |> merge_contexts ctx_jk
    |> merge_contexts ctx_k2 |> merge_contexts ctx_vars
  in

  Debug.certif "Extracting LFSC frontend proof of base case from cvc5";
  let base_proof = proof_from_file ctx "front.base" inv.base in
  fprintf proof_fmt ";; Additional symbols@.%a@." (print_delta_context ctx)
    base_proof.proof_context;
  fprintf proof_fmt ";; Proof of base case\n@.%a\n@." (print_proof obs_base)
    base_proof;

  Debug.certif "Extracting LFSC frontend proof of inductive case from cvc5";
  let induction_proof = proof_from_file ctx "front.ind" inv.induction in
  fprintf proof_fmt ";; Additional symbols@.%a@." (print_delta_context ctx)
    induction_proof.proof_context;
  fprintf proof_fmt ";; Proof of inductive case\n@.%a\n@."
    (print_proof obs_induction)
    induction_proof;

  Debug.certif "Extracting LFSC frontend proof of implication from cvc5";
  let implication_proof = proof_from_file ctx "front.imp" inv.implication in
  fprintf proof_fmt ";; Additional symbols@.%a@." (print_delta_context ctx)
    implication_proof.proof_context;
  fprintf proof_fmt ";; Proof of implication\n@.%a\n@."
    (print_proof obs_implication)
    implication_proof;

  fprintf proof_fmt ";; Proof of invariance by %d-induction\n@." inv.k;
  write_inv_proof proof_fmt ~check:false
    (obs_implication, obs_base, obs_induction)
    proof_obs_name inv;
