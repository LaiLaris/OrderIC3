...
let is_property sys sv =
  ...
  List.exists
    (fun { Property.prop_term } -> Term.equal tsv prop_term)
...
let jkind_var_of_lustre sys kind_sv (li, parents) =
  let base_li =
    match (parents, List.rev parents) with
...
let match_condact_clock lustre_vars sv =
  ...
  List.fold_left
    (fun acc (sv, base_li, parents) ->
      try build_call_base sv base_li parents :: acc with Not_found -> acc)
    [] clocks_calls
  |> List.rev
...
let jkind_vars_of_kind2_statevar sys lustre_vars sv =
  ...
  List.fold_left
    (fun acc lv ->
      try jkind_var_of_lustre sys sv lv :: acc with Not_found -> acc)
    [] lus_vs
...
let jkind_empty = { jk_init_flag; jk_statevars = []; jk_trans_lambda = None }
...
let rec parse acc = function
  (* Ignore set-option *)
  | HS.List (HS.Atom s :: _) :: r when s == s_set_option -> parse acc r
  (* Definition of transition relation *)
  | HS.List
      [ HS.Atom s; HS.Atom t; HS.List args; HS.Atom _ (* return type *); hdef ]
    :: r
    when s == s_define_fun && t == s_T ->
  ...
  | HS.List [ HS.Atom s; HS.Atom i; HS.List []; _ (* ty *) ] :: r
    when s == s_declare_fun && i == s_pinit ->
      parse acc r
  ...
  | HS.List
    ...
    :: r
    when ass == s_assert && conj == s_and && leq1 == s_leq && leq2 == s_leq
         && t1 == t2 ->
      ...
      (fun _ t ->
        if Term.is_free_var t then
          ...
          then (
            ...
            with Not_found ->
              let svc =
                StateVar.mk_state_var ~is_const:true ~is_input:true
                  ~for_inv_gen:true
                  (StateVar.name_of_state_var sv ^ "~1")
                  (StateVar.scope_of_state_var sv)
                  (StateVar.type_of_state_var sv)
              in
              let tc = svc |> Var.mk_const_state_var |> Term.mk_var in
...
let simplify_trivial_ites =
  Term.map (fun _ t ->
      ...
      | Node (s_ite, [ cond; t_then; t_else ]) when s_ite == Symbol.s_ite ->
...
let of_channel in_ch =
  ...
  let statevars0 =
    List.map (fun sv -> Var.mk_state_var_instance sv Numeral.zero) statevars
  in

  let statevars1 =
    List.map (fun sv -> Var.mk_state_var_instance sv Numeral.one) statevars
  in

  let statevars_m1 =
    List.map
      (fun sv -> Var.mk_state_var_instance sv Numeral.(neg one))
      statevars
  in
  ...
  let init_term, consts =
    ...
    |> unlet_term |> simplify_trivial_ites
  ...
  let trans_args =
    vconsts @ (initflag1 :: statevars1) @ (initflag0 :: statevars0)
  in
  ...
  let sys, _ =
    TransSys.mk_trans_sys jkind_scope [] None jk_init_flag
      (* [] *)
      allstatevars StateVar.StateVarSet.empty (* underapproximation *)
      (StateVar.StateVarHashtbl.create 3)
      [] (* Global consts *)
      [] (* Global constraints *)
      [] (* Ufs *)
      init_uf_symbol init_args init_term trans_uf_symbol trans_args trans_term
