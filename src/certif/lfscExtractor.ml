...
let rec uncurry = function
  | HS.List (HS.List f :: args) -> HS.List (List.map uncurry (f @ args))
  | e -> e
...
let parse_type = function
  ...
  | HS.List (HS.Atom t :: ty) when t == s_term -> (
      match ty with [ (HS.Atom _ as ty) ] -> Decl ty | _ -> Decl (HS.List ty))
...
let rec parse_smt2 acc (others, asserts) = function
  (* Fail on push / pop *)
  | HS.List (HS.Atom p :: _) :: _ when p == s_push || p == s_pop ->
  ...
  (* ... from the rest *)
  | se :: r -> parse_smt2 acc (se :: others, asserts) r
...
let () =
  ...
  (* open file given as argument or read from stdin *)
  let in_ch_lfsc =
    try open_in Sys.argv.(2) with Invalid_argument _ -> stdin
  in
  ...
  (try
     List.iter2
       (fun formula proven ->
         let equiv =
           HS.List [ HS.Atom s_not; HS.List [ HS.Atom s_eq; formula; proven ] ]
         in
