...
let rec pp_print_nusmv_symbol_node ppf = function
  ...
  | `DECIMAL f ->
      pp_print_decimal ppf f (*| `BV b -> pp_print_bitvector_b ppf b *)
  ...
  | `MOD ->
      Format.pp_print_string ppf "mod"
...
let rec pp_print_nusmv_type_node ppf = function
  ...
  | Type.Int ->
      pp_print_nusmv_type ppf
        (Type.mk_int_range
           (numeral_of_int !int_lowerbound)
           (numeral_of_int !int_upperbound))
  | Type.IntRange (i, j) ->
      Format.fprintf ppf "%a .. %a" pp_print_numeral i pp_print_numeral j
  (*
  ...
  *)
  | Type.Array (s, t) ->
      Format.fprintf ppf "array %a of %a"
  ...
  | t ->
      Format.fprintf ppf "unsupported type: %a\n" Type.pp_print_type_node t;
      assert false
...
let pp_print_nusmv_var ofs ppf term =
  ...
  | H.Var v when ofs = numeral_of_int 1 ->
      Format.fprintf ppf "next(%a)" StateVar.pp_print_state_var
...
let rec pp_print_nusmv_term ppf term =
  ...
  | H.App (s, l) when s = Symbol.mk_symbol `PLUS -> (
      ...
      | [] -> ()
      | h :: [] -> Format.fprintf ppf "%a" pp_print_nusmv_term h
      | h :: t ->
  ...
  | H.App (s, l) when s = Symbol.mk_symbol `AND -> (
      ...
      | [] -> Format.fprintf ppf "%a" pp_print_nusmv_term (Term.mk_false ())
      | [ t ] -> Format.fprintf ppf "%a" pp_print_nusmv_term t
      | h :: t ->
          Format.fprintf ppf "(%a & %a)" pp_print_nusmv_term
            (Term.mk_and (List.rev t))
            pp_print_nusmv_term h)
  ...
  | H.App (s, l) -> (
      ...
      | [ cond; cons; alt ] -> (
          ...
          | s ->
              Format.fprintf ppf "invalid symbol: %a" Symbol.pp_print_symbol
                (Symbol.mk_symbol s);
              assert false)
      | [ lhs; rhs ] ->
          Format.fprintf ppf "(%a %a %a)"
      ...
      | [] -> ()
      | _ ->
          Format.fprintf ppf "invalid term %a" Term.pp_print_term term;
          assert false)
  ...
  | h :: tl -> (
      ...
      | H.App (s, [ l; r ]) when s = Symbol.mk_symbol `EQ ->
          Format.fprintf ppf "\tinit(%a) := %a;\n"
            (pp_print_nusmv_var (numeral_of_int 0))
            (Term.mk_term l) pp_print_nusmv_term (Term.mk_term r);

          pp_print_nusmv_init ppf tl
      | _ ->
          Format.fprintf ppf "invalid term %a" Term.pp_print_term h;
          assert false)
...
let pp_print_nusmv_prop ppf (s, t) =
  ...
  Format.fprintf ppf "@[<hv 1>(%a)@]" pp_print_nusmv_term t

let pp_print_nusmv_trans_sys ppf
    {
      TransSys.init = i;
      TransSys.constr = c;
      TransSys.trans = g;
  ...
  Format.fprintf ppf
    "MODULE main\nVAR@\n@[<v>%a@]@\nASSIGN@\n@[<v>%a@]@[<v>%a@]@\n"
    (pp_print_list pp_print_nusmv_var_declaration "")
    v
    (pp_print_list pp_print_nusmv_init "")
    [ i ]
    (pp_print_list pp_print_nusmv_constr "")
    [ c ];
  if p <> [] then
    Format.fprintf ppf "LTLSPEC G@\n(@[<v>%a@]);\n"
      (pp_print_list pp_print_nusmv_prop " & ")
      p
