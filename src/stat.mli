...
(** Record the time since the call to {!start_timer} of this statistics item,
    stop the timer *)
...
(** Record the time since the call to {!start_timer} of this statistics item, do
    not stop the timer *)
...
(** Number of solver called *)

val ic3_neg_state : int_item
...
(** Total number of invariants discovered by invariant generation for all
    systems. *)

val invgengraph_os_implication_count : int_item
(** Total number of invariants discovered by invariant generation which were
    implications. *)
