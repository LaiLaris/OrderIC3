...
(* Bool formatter. *)
let fmt_bool fmt b = Format.fprintf fmt "%s" (if b then "true" else "false")
...
let fmt_flag fmt (flag, spec, desc) =
  Format.fprintf fmt "@[<v>%t@[<v>%t@]@]"
    (fun fmt ->
      match spec with
      | Arg.Unit _ | Arg.Set _ | Arg.Clear _ ->
          Format.fprintf fmt "\x1b[1m%-10s\x1b[0m " flag
      | Arg.Bool _ -> Format.fprintf fmt "\x1b[1m%s <bool>\x1b[0m@   " flag
      ...
      | _ -> failwith "Unsupported Arg.spec (Tuple, Symbol, or Rest).")
    desc

(* Prints all the flags in the input list, given a triples of the flag,
its spec, and its description. *)
let print_flags = Format.printf "@[<v>%a@]" (pp_print_list fmt_flag "@ ")
...
module Make_Spec (_ : sig end) = struct
  ...
  let add_format_spec flag parse desc =
    add_spec flag parse desc;
    format_specs := (flag, parse, desc) :: !format_specs
...
module type FlagModule = sig
  ...
  val all_specs : unit -> (string * Arg.spec * (Format.formatter -> unit)) list
end
...
module Smt = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>SMT solvers are the tools Kind 2 relies on to verify systems. \
       Flags@ in this module control which solver Kind 2 uses, the path to \
       the@ binary, and tweak how Kind 2 interacts with the solver at \
       runtime.@]"
  ...
  let solver_values =
    "Z3, cvc5, Yices2, MathSAT, SMTInterpol, OpenSMT, Bitwuzla, Yices"
  ...
  let _ =
    add_spec "--smt_solver"
      ...
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>where <string> can be %s@ Set the main SMT solver@ Default: \
           %s@]"
          solver_values
  ...
  let _ =
    add_spec "--smt_qe_solver"
      (Arg.String (fun str -> qe_solver := qe_solver_of_string str))
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>where <string> can be %s@ Set the SMT solver used for \
           quantifier elimination@ Default: %s@]"
          qe_solver_values
  ...
  let _ =
    add_spec "--smt_logic"
      ...
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>where <string> is none, detect, or a legal SMT-LIB logic@ \
           Select logic for SMT solvers@ \"none\" for no logic@ \"detect\" to \
           detect with the input system@ Other SMT-LIB logics will be passed \
           to the solver@ Default: %s@]"
          (string_of_logic logic_default))

  let set_logic l = logic := l
  let detect_logic_if_none () = if !logic = `None then logic := `detect
  let logic () = !logic
  ...
  let _ =
    add_spec "--check_sat_assume" (bool_arg check_sat_assume) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Use check-sat-assuming, or simulate with push/pop@ when false@ \
           Default: %a@]"
  ...
  let _ =
    add_spec "--smt_short_names" (bool_arg short_names) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Send short variables names to SMT solver, send full names if \
           false@ Default: %a@]"
  ...
  let _ =
    add_spec "--mathsat_bin" (Arg.Set_string mathsat_bin) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--opensmt_bin" (Arg.Set_string opensmt_bin) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--smtinterpol_jar" (Arg.Set_string smtinterpol_jar) (fun fmt ->
        Format.fprintf fmt "@[<v>JAR of SMTInterpol solver@ Default: \"%s\"@]"
  ...
  let _ =
    add_spec "--bitwuzla_bin" (Arg.Set_string bitwuzla_bin) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--z3_bin" (Arg.Set_string z3_bin) (fun fmt ->
        Format.fprintf fmt "@[<v>Executable of Z3 solver@ Default: \"%s\"@]"
          z3_bin_default)
  ...
  let _ =
    add_spec "--cvc5_bin" (Arg.Set_string cvc5_bin) (fun fmt ->
        Format.fprintf fmt "@[<v>Executable of cvc5 solver@ Default: \"%s\"@]"
          cvc5_bin_default)
  ...
  let _ =
    add_spec "--yices_bin" (Arg.Set_string yices_bin) (fun fmt ->
        Format.fprintf fmt "@[<v>Executable of Yices solver@ Default: \"%s\"@]"
          yices_bin_default)
  ...
  let _ =
    add_spec "--yices2_bin" (Arg.Set_string yices2smt2_bin) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--smt_trace" (Arg.Set trace) (fun fmt ->
        Format.fprintf fmt "@[<v>Write all SMT commands to files@]")
  ...
  let check_smtsolver () =
    ...
    (* User chose cvc5 *)
    | `cvc5_SMTLIB -> find_solver ~fail:true "cvc5" (cvc5_bin ()) |> ignore
    (* User chose OpenSMT *)
    | `OpenSMT_SMTLIB ->
    ...
    | `SMTInterpol_SMTLIB ->
        let full_path =
          find_solver ~filetype:"JAR" ~fail:true "SMTInterpol"
            (smtinterpol_jar ())
        in
    ...
    (* User chose Yices *)
    | `Yices_native -> find_solver ~fail:true "Yices" (yices_bin ()) |> ignore
    (* User chose Z3 *)
    | `Z3_SMTLIB -> find_solver ~fail:true "Z3" (z3_bin ()) |> ignore
    ...
    | `detect -> (
        ...
        with Not_found -> (
          ...
          with Not_found -> (
            try
              let exec =
                find_solver ~fail:false "Yices2 SMT2" (yices2smt2_bin ())
              in
            ...
            with Not_found -> (
              ...
              with Not_found -> (
                try
                  let exec =
                    find_solver ~filetype:"JAR" ~fail:false "SMTInterpol"
                      (smtinterpol_jar ())
                  in
                ...
                with Not_found -> (
                  try
                    let exec =
                      find_solver ~fail:false "OpenSMT" (opensmt_bin ())
                    in
                  ...
                  with Not_found -> (
                    try
                      let exec =
                        find_solver ~fail:false "Bitwuzla" (bitwuzla_bin ())
                      in
                    ...
                    with Not_found -> (
                      try
                        let exec =
                          find_solver ~fail:false "Yices" (yices_bin ())
                        in
        ...
        | _ -> (
            ...
            with Not_found -> (
              try
                ...
                set_cvc5_bin exec
              with Not_found ->
                () (* Ẃe keep `detect to know no qe solver was found *))))

  let check_itp_solver () =
    match itp_solver () with
    (* User chose cvc5qe *)
    | `cvc5_QE -> (
        match solver () with
                ...
                with Not_found -> (
                  ...
                  with Not_found -> (
                    try
                      ...
                      set_opensmt_bin exec
                    with Not_found ->
                      () (* Ẃe keep `detect to know no itp solver was found *)))))
            ))
end
...
module BmcKind = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>K-induction is the primary verification technique in Kind 2. It@ \
       consists of two processes:@ > bounded model checking (BMC) explores the \
       reachable states of the@   system looking for a falsification of one of \
       the properties.@ > the (k-inductive) step tries to prove that from `k` \
       succeeding states@   verifying the properties, it not possible to reach \
       a state violating@   any of them. If this holds, then if BMC proved \
       these properties to@   hold in the `k` first states, then by \
       (k-)induction the properties@   hold.@ Internally many techniques such \
       as path compression, multi-property,@ etc. are used to improve the \
       basic scheme described above.@ Flags in this module deal with the \
       behavior of BMC and step.@]"

  let max_default = 0
  let max = ref max_default

  let _ =
    add_spec "--unroll_max" (Arg.Set_int max) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Maximal number of iterations for BMC and k-induction@ Default: \
           %d (unlimited: 0)@]"
          max_default)
  ...
  let _ =
    add_spec "--bmc_check_unroll" (bool_arg check_unroll) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Check that the unrolling alone is satisfiable@ Default: %a@]"
  ...
  let _ =
    add_spec "--ind_print_cex" (bool_arg ind_print_cex) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Print counterexamples to induction@ Default: %a@]" fmt_bool
          ind_print_cex_default)
  ...
  let _ =
    add_spec "--ind_compress" (bool_arg compress) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Compress inductive counterexamples@ Default: %a@]" fmt_bool
          compress_default)
  ...
  let _ =
    add_spec "--ind_compress_equal" (bool_arg compress_equal) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Compress inductive counterexamples for states equal modulo \
           inputs@ Default: %a@]"
  ...
  let _ =
    add_spec "--ind_compress_same_succ" (bool_arg compress_same_succ)
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Compress inductive counterexamples for states with same \
           successors@ Default: %a@]"
  ...
  let _ =
    add_spec "--ind_compress_same_pred" (bool_arg compress_same_pred)
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Compress inductive counterexamples for states with same \
           predecessors@ Default: %a@]"
  ...
  let _ =
    add_spec "--ind_lazy_invariants" (bool_arg lazy_invariants) (fun fmt ->
        Format.fprintf fmt "@[<v>Asserts invariants lazily@ Default: %a@]"
...
module IC3QE = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>The IC3-QE engine is an SMT-based extension of IC3 (a.k.a. PDR) \
       that@ uses quantifier elimination (QE) to generalize counterexamples.@ \
       The QE procedure has its own flags (see module \"qe\").\n\
      \    @]"

  let cex_inv_map_default = true
  let cex_inv_map = ref cex_inv_map_default

  let _ =
    add_spec "--ic3qe_cex_inv_map" (bool_arg cex_inv_map) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Check inductiveness of blocking clauses@ Default: %a@]" fmt_bool
          cex_inv_map_default)
  ...
  let _ =
    add_spec "--ic3qe_check_inductive" (bool_arg check_inductive) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Check inductiveness of blocking clauses@ Default: %a@]" fmt_bool
          check_inductive_default)
  ...
  let _ =
    add_spec "--ic3qe_inductively_generalize"
      (Arg.Set_int inductively_generalize) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Inductively generalize blocking clauses before forward \
  ...
  let _ =
    add_spec "--ic3qe_block_in_future" (bool_arg block_in_future) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Block counterexample in future frames@ Default: %a@]" fmt_bool
          block_in_future_default)
  ...
  let _ =
    add_spec "--ic3qe_block_in_future_first" (bool_arg block_in_future_first)
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Block counterexample in future frames first before returning \
           to@ frame@ Default: %a@]"
  ...
  let _ =
    add_spec "--ic3qe_fwd_prop_non_gen" (bool_arg fwd_prop_non_gen) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--ic3qe_fwd_prop_ind_gen" (bool_arg fwd_prop_ind_gen) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Inductively generalize all clauses after forward propagation@ \
           Default: %a@]"
  ...
  let _ =
    add_spec "--ic3qe_fwd_prop_subsume" (bool_arg fwd_prop_subsume) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Subsumption in forward propagation@ Default: %a@]" fmt_bool
  ...
  let _ =
    add_spec "--ic3qe_use_invgen" (bool_arg use_invgen) (fun fmt ->
        Format.fprintf fmt
...
module IC3IA = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>The IC3-IA engine is an SMT-based extension of IC3 (a.k.a. PDR) \
       that@ uses implicit (predicate) abstraction (IA).\n\
      \    @]"
  ...
  let _ =
    add_spec "--ic3ia_max" (Arg.Set_int max_processes) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Maximum number of IC3IA parallel processes.@ Each process \
           checks an individual property.@ Default: %d@]"
...
module QE = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>Quantifier elimination (QE) is a technique that, given some \
       variables@ `v_1`, ..., `v_n` and a quantifier-free formula `f`, returns \
       a@ quantifier-free formula equivalent to `(exists (v_1, ..., v_n) f)`.@ \
       The QE method implemented in Kind 2 supports integer arithmetic,@ but \
       not real or machine integer arithmetic. If the solver used is@ Z3 or \
       cvc5 then the solver's QE will be used instead of the internal@ one for \
       systems with real or machine integer variables.@ IC3 and CONTRACTCK are \
       the only Kind 2 technique that uses QE.@]"
  ...
  let _ =
    add_spec "--qe_method"
      ...
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>where <string> can be %s@ Choose quantifier elimination \
           method@ Default: %s@]"
          qe_method_values
          (string_of_qe_method qe_method_default))
  ...
  let string_of_extract = function `First -> "first" | `Vars -> "vars"

  let extract_values =
    [ `First; `Vars ] |> List.map string_of_extract |> String.concat ", "
  ...
  let _ =
    add_spec "--qe_extract"
      ...
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>where <string> can be %s@ Heuristics for extraction of \
           implicant@ Default: %s@]"
          extract_values
          (string_of_extract extract_default))
  ...
  let _ =
    add_spec "--ae_val_use_ctx" (bool_arg ae_val_use_ctx) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Use context (premises) in ae_val procedure@ Default: %a@]"
  ...
  let _ =
    add_spec "--order_var_by_elim" (bool_arg order_var_by_elim) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Order variables in polynomials by order of elimination@ \
           Default: %a@]"
  ...
  let _ =
    add_spec "--general_lbound" (bool_arg general_lbound) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Choose lower bounds containing variables@ Default: %a@]"
...
module Contracts = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>Kind 2 extends Lustre with a syntax for assume-guarantee-like@ \
       contracts. The syntax and semantics of contracts are described in@ Kind \
       2 documentation.@ Contracts allow to perform compositional \
       verification, where sub-nodes@ with a contract are abstracted away by \
       their specification. Also, the@ test generation technique (see module \
       \"testgen\") relies on contracts@ to generate the testcases.@]"

  let compositional_default = false
  let compositional = ref compositional_default

  let _ =
    add_spec "--compositional" (bool_arg compositional) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Abstract subnodes with a contract@ Default: %a@]" fmt_bool
  ...
  let _ =
    add_spec "--translate_contracts"
      (Arg.String (fun str -> translate := Some str))
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Translates a contracts in assertions / properties \
           (experimental)@ Default: %t@]" (fun fmt ->
            Format.fprintf fmt "%s"
              (match translate_default with None -> "off" | Some f -> f)))
  ...
  let _ =
    add_spec "--check_modes" (bool_arg check_modes) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Checks the modes of a contracts are exhaustive@ Default: %a@]"
  ...
  let _ =
    add_spec "--check_environment" (bool_arg check_environment) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Check realizability of contract environments@ Default: %a@]"
  ...
  let _ =
    add_spec "--check_implem" (bool_arg check_implem) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Checks the implementation of nodes@ Default: %a@]" fmt_bool
  ...
  let _ =
    add_spec "--contract_gen" (bool_arg contract_gen) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Uses invariant generation to infer contracts for a lustre \
           system.@ Providing contracts, properties and assertion helps but is \
           not@ mandatory. Contracts will be written to the folder specified \
           by@ --output_dir.@ See also --contract_gen_depth and \
           --contract_gen_fine_grain.@ Default: %a@]"
  ...
  let _ =
    add_spec "--contract_gen_depth"
      (Arg.Int (fun n -> contract_gen_depth := n))
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Controls the depth of exploration used to generate contracts.@ \
           Note that invariant generation is expected to go faster as it@ \
           unrolls (explores) the system.@ Default: %d@]"
  ...
  let _ =
    add_spec "--assumption_gen" (bool_arg assumption_gen) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Generates an assumption that blocks all possible violations \
           of@ the invalid properties.@ Default: %a@]"
  ...
  let _ =
    add_spec "--two_state_assumption" (bool_arg two_state_assumption)
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Generated assumptions may include references@ to previous \
           values of input and output streams.@ Default: %a@]"
  ...
  let _ =
    add_spec "--assumption_gen_iter"
      (Arg.Int (fun n -> assumption_gen_iter := n))
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Controls the number of iterations used for the generalization \
           of@ the assumption candidate.@ Default: %d@]"
  ...
  let _ =
    add_spec "--refinement" (bool_arg refinement) (fun fmt ->
        Format.fprintf fmt
          "@[<v>(De)activates refinement in compositional reasoning@ Default: \
           %a@]"
  ...
  let _ =
    add_spec "--enforce_func_congruence" (bool_arg enforce_func_congruence)
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Add constraints to ensure imported functions,@ partially \
           defined functions, and functions abstracted by@ their contracts \
           behave as mathematical functions:@ given the same inputs the \
           functions provide the same outputs@ Default: %a@]"
  ...
  let _ =
    add_spec "--print_deadlock" (bool_arg print_deadlock) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Print a deadlocking trace and a conflict when@ a contract is \
           proven unrealizable.@ Default: %a@]"
  ...
  let _ =
    add_spec "--dump_deadlock" (bool_arg dump_deadlock) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--check_contract_is_sat" (bool_arg check_contract_is_sat)
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Check whether a contract proven unrealizable is satisfiable@ \
           Default: %a@]"
  ...
  let _ =
    add_spec "--print_viable_states" (bool_arg print_viable_states) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Print a lustre-like constraint representing the set of@ viable \
           states of a realizable contract.@ Default: %a@]"
...
module Certif = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>Kind 2 generates (intermediate) certificates in the SMT-LIB 2 \
       format and@ produces proofs in the LFSC format.@]"
  ...
  let _ =
    add_spec "--certif"
      ...
      (fun fmt ->
        Format.fprintf fmt "@[<v>Produce SMT-LIB 2 certificates.@ Default: %a@]"
  ...
  let _ =
    add_spec "--proof"
      ...
      (fun fmt ->
        Format.fprintf fmt "@[<v>Produce LFSC proofs.@ Default: %a@]" fmt_bool
          proof_default)
  ...
  let _ =
    add_spec "--smaller_holes" (bool_arg smaller_holes) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--flatten_proof" (bool_arg flatten_proof) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Breakdown proofs into smaller steps, where each step is \
           checked independently.@ Substantially reduces LFSC memory \
           footprint. @ Default:%a@]"
          fmt_bool smaller_holes_default)
  ...
  let _ =
    add_spec "--log_trust" (bool_arg log_trust) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Log trusted parts of the proof in a separate file for users to \
           fill.@ Default: %a@]"
  ...
  let _ =
    add_spec "--certif_mink"
      (Arg.String (fun str -> mink := mink_of_string str))
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>where <string> is no, fwd, bwd, dicho, frontierdicho or auto.@ \
           Select strategy for minimizing k of certificates@ \"no\" for no \
           minimization@ \"fwd\" for a search starting at 1 up to k@ \"bwd\" \
           for a search starting at k and going down to 1@ \"dicho\" for a \
           binary search of the minimum k@ \"frontierdicho\" tries the \
           frontier k/k-1 then employs the dicho stractegy@ \"auto\" to \
           heuristically select the best strategy among the previous ones \
           (default)@]")
  ...
  let _ =
    add_spec "--certif_mininvs"
      (Arg.String (fun str -> mininvs := mininvs_of_string str))
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>where <string> is easy, medium, mediumonly, hard, hardonly.@ \
           Select strategy for minimizing the invariants of certificates@ \
           \"easy\" to only do unsat-core based trimming@ \"medium\" does easy \
           + coarse couter-example based minimization (default)@ \
           \"mediumonly\" does only coarse couter-example based minimization@ \
           \"hard\" does easy + cherry-pick invariants based on \
           couter-examples@ \"hardonly\" only cherry-picks invariants based on \
           couter-examples@]")
  ...
  let _ =
    add_spec "--jkind_bin" (Arg.Set_string jkind_bin) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Executable of JKind for frontend certificates.@ Default: \
           \"%s\"@]"
          jkind_bin_default)
  ...
  let _ =
    add_spec "--only_user_candidates" (bool_arg only_user_candidates)
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Only use user provided candidates for invariants.@ Default: \
           %a@]"
...
module IVC = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>Kind 2 generates a minimal inductive validity core,@ that is a \
       minimal subset of the model elements sufficient to prove the \
       properties.@]"
  ...
  let _ =
    add_spec "--ivc" (bool_arg compute_ivc) (fun fmt ->
        Format.fprintf fmt
          "Enable inductive validity core generation@ Default: %a" fmt_bool
          compute_ivc_default)
  ...
  let ivc_element_of_string = function
    ...
    | unexpected ->
        Arg.Bad
          (Format.sprintf "Unexpected value \"%s\" for flag --ivc_category"
             unexpected)
        |> raise
  ...
  let _ =
    add_spec "--ivc_category"
      (Arg.String
         (fun str ->
           let elt = ivc_element_of_string str in
           if List.mem elt !ivc_category |> not then
             ivc_category := elt :: !ivc_category))
      (fun fmt ->
        Format.fprintf fmt
          "where <string> can be 'node_calls', 'contracts', 'equations', \
           'assertions' or 'annotations'@ Minimize only a specific category of \
           elements, repeat option to minimize multiple categories@ Default: \
           minimize all categories of elements")
  ...
  let _ =
    add_spec "--ivc_all" (bool_arg ivc_all) (fun fmt ->
        Format.fprintf fmt
          "Compute all the Minimal Inductive Validity Cores.@ Default: %a"
  ...
  let _ =
    add_spec "--ivc_approximate" (bool_arg ivc_approximate) (fun fmt ->
        Format.fprintf fmt
          "Compute an approximation (superset) of a MIVC.@ Ignored if \
           --ivc_all or --ivc_must_set is true.@ Default: %a"
  ...
  let _ =
    add_spec "--ivc_smallest_first" (bool_arg ivc_smallest_first) (fun fmt ->
        Format.fprintf fmt
          "Compute a smallest IVC first.@ If --ivc_all is false, the computed \
           IVC will be a smallest one.@ Default: %a"
  ...
  let _ =
    add_spec "--ivc_only_main_node" (bool_arg ivc_only_main_node) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--ivc_must_set" (bool_arg ivc_must_set) (fun fmt ->
        Format.fprintf fmt
          "Compute the MUST set in addition to the IVCs@ Default: %a" fmt_bool
          ivc_must_set_default)
  ...
  let _ =
    add_spec "--print_ivc" (bool_arg print_ivc) (fun fmt ->
        Format.fprintf fmt
          "Print the inductive validity core computed@ Default: %a" fmt_bool
          print_ivc_default)
  ...
  let _ =
    add_spec "--print_ivc_complement" (bool_arg print_ivc_compl) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--minimize_program"
      (Arg.String (fun str -> minimize_program := mm_of_string str))
      (fun fmt ->
        Format.fprintf fmt
          "Minimize the source Lustre program according to the inductive \
           validity core(s) computed@ \"no\" to disable this feature \
           (default)@ \"valid_lustre\" to replace useless expressions by a \
           valid node call@ \"concise\" to replace useless expressions by a \
           '_'")
  ...
  let _ =
    add_spec "--ivc_output_dir" (Arg.Set_string minimized_program_dir)
      (fun fmt ->
        Format.fprintf fmt
          "Output directory for the minimized programs@ Default: \
           <INPUT_FILENAME>")
  ...
  let _ =
    add_spec "--ivc_precomputed_mcs" (Arg.Set_int ivc_precomputed_mcs)
      (fun fmt ->
        Format.fprintf fmt
          "When computing all MIVCs,@ set a cardinality upper bound for the \
           precomputed MCSs@ (helps prune space of candidates).@ Default: %n"
  ...
  let _ =
    add_spec "--ivc_uc_timeout" (Arg.Set_int ivc_uc_timeout) (fun fmt ->
        Format.fprintf fmt
          "Set a timeout for each unsat core check sent to the solver.@ Set to \
           0 to disable timeout.@ Default: %n"
  ...
  let _ =
    add_spec "--ivc_disable_must_opt" (bool_arg ivc_disable_must_opt)
      (fun fmt ->
        Format.fprintf fmt
          "Disable the must-set optimization.@ Ignored if --ivc_must_set is \
           true.@ Default: %a"
...
module MCS = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>Kind 2 generates a minimal cut set,@ that is a minimal subset of \
       the model elements@ whose no satisfaction leads to the violation of a \
       property.@]"
  ...
  let mcs_element_of_string = function
    ...
    | unexpected ->
        Arg.Bad
          (Format.sprintf "Unexpected value \"%s\" for flag --mcs_category"
             unexpected)
        |> raise
  ...
  let _ =
    add_spec "--mcs_category"
      (Arg.String
         (fun str ->
           let elt = mcs_element_of_string str in
           if List.mem elt !mcs_category |> not then
             mcs_category := elt :: !mcs_category))
      (fun fmt ->
        Format.fprintf fmt
          "where <string> can be 'node_calls', 'contracts', 'equations', \
           'assertions' or 'annotations'@ Consider only a specific category of \
           elements, repeat option to consider multiple categories@ Default: \
           annotations")
  ...
  let _ =
    add_spec "--mcs_only_main_node" (bool_arg mcs_only_main_node) (fun fmt ->
        Format.fprintf fmt
          "Only elements of the main node are considered in the computation@ \
           Default: %a"
  ...
  let _ =
    add_spec "--mcs_all" (bool_arg mcs_all) (fun fmt ->
        Format.fprintf fmt
          "Specify whether all the Minimal Cut Sets must be computed or just \
           one@ Default: %a"
  ...
  let _ =
    add_spec "--mcs_approximate" (bool_arg mcs_approximate) (fun fmt ->
        Format.fprintf fmt
          "Compute an approximation of a MCS.@ Ignored if --mcs_all is true.@ \
           Default: %a"
  ...
  let _ =
    add_spec "--mcs_max_cardinality" (Arg.Set_int mcs_max_cardinality)
      (fun fmt ->
        Format.fprintf fmt
          "Only search for MCSs of cardinality lower or equal to this \
           parameter.@ If -1, all MCSs will be considered.@ Default: %i"
  ...
  let _ =
    add_spec "--print_mcs" (bool_arg print_mcs) (fun fmt ->
        Format.fprintf fmt "Print the minimal cut set computed@ Default: %a"
  ...
  let _ =
    add_spec "--print_mcs_complement" (bool_arg print_mcs_compl) (fun fmt ->
        Format.fprintf fmt
          "Print the complement of the minimal cut set computed@ (this is \
           equivalent to computing a Maximal Unsafe Abstraction)@ Default: %a"
  ...
  let _ =
    add_spec "--print_mcs_legacy" (bool_arg print_mcs_legacy) (fun fmt ->
        Format.fprintf fmt
          "Print the minimal cut set using the legacy format@ Default: %a"
  ...
  let _ =
    add_spec "--print_mcs_cex" (bool_arg print_mcs_cex) (fun fmt ->
        Format.fprintf fmt
          "Print a counterexample for each MCS found.@ Ignored if \
           --print_mcs_legacy is true@ Default: %a"
  ...
  let _ =
    add_spec "--mcs_per_property" (bool_arg mcs_per_property) (fun fmt ->
        Format.fprintf fmt
...
module Arrays = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>Kind 2 extends Lustre with a syntax for recursive whole arrays@ \
       definitions. The syntax and semantics are described in@ Kind 2 \
       documentation.@]"
  ...
  let _ =
    add_spec "--smt_arrays" (bool_arg smt) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--inline_arrays" (bool_arg inline) (fun fmt ->
        Format.fprintf fmt
  ...
  let recdef_action b =
    ...
    | _ ->
        raise
          (Arg.Bad
             "Recursive encoding of arrays can only be used with cvc5. Use the \
              flag --smtsolver cvc5")

  let _ =
    add_spec "--arrays_rec" (Arg.Bool recdef_action) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--var_array_size" (bool_arg var_size) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Allow variable arrays size (Dangerous).@ Default: %a@]" fmt_bool
          var_size_default)
...
module Testgen = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>Test generation uses the contract (see module \"contract\") \
       attached@ to a node to explore the behavior of a node. For each \
       behavior, a@ trace of inputs triggering it is generated and logged as a \
       testcase in@ CSV.@]"
  ...
  let _ =
    add_spec "--testgen" (bool_arg active) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Activates test generation for systems proved correct@ Default: \
           %a@]"
  ...
  let _ =
    add_spec "--testgen_graph_only" (bool_arg graph_only) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Only draw the graph of reachable modes, do not log testcases.@ \
           Default: %a@]"
  ...
  let _ =
    add_spec "--testgen_len" (Arg.Set_int len) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Maximimum length for test generation@ Default: %d@]" len_default)
...
module Invgen = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>Invariant generation attempts to discover invariants of the form@ \
  ...
  let _ =
    add_spec "--invgen_prune_trivial" (bool_arg prune_trivial) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Invariant generation will only communicate one-state \
           invariants not implied by@ previous one-state invariants, and \
           two-state invariants not implied by@ previous two-state invariants \
           or the transition relation@ Default: %a@]"
  ...
  let _ =
    add_spec "--invgen_max_depth"
      (Arg.Int (fun n -> max_depth := Some n))
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Maximal depth for graph-based invariant generation \
  ...
  let _ =
    add_spec "--invgen_max_succ"
      (Arg.Int (fun b -> max_succ := b))
      (fun fmt ->
        Format.fprintf fmt
          "@[<v>Maximal number of successive iterations for subsystems@ \
           Default: %d@]"
          max_succ_default)
  ...
  let _ =
    add_spec "--invgen_lift_candidates" (bool_arg lift_candidates) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Invariant generation will instantiate candidates from \
           sub-nodes@ Default: %a@]"
  ...
  let _ =
    add_spec "--invgen_top_only" (bool_arg top_only) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Only generate invariants for the top level@ Default: %a@]"
  ...
  let _ =
    add_spec "--invgen_all_out" (bool_arg all_out) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Forces invariant generation to consider a huge number of \
           candidates@ Slower, but more likely to succeed.@ Default: %a@]"
  ...
  let _ =
    add_spec "--invgen_mine_trans" (bool_arg mine_trans) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Invariant generation will extract candidate terms from the@ \
           transition predicate@ Default: %a@]"
  ...
  let _ =
    add_spec "--invgen_two_state" (bool_arg two_state) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Run invariant generion in two state modeDefault: %b@]"
          two_state_default)
  ...
  let _ =
    add_spec "--invgen_bool_eq_only" (bool_arg bool_eq_only) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Forces bool invgen to look for equalities onlyDefault: %b@]"
  ...
  let _ =
    add_spec "--invgen_arith_eq_only" (bool_arg arith_eq_only) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Forces arith invgen to look for equalities onlyDefault: %b@]"
  ...
  let _ =
    add_spec "--invgen_renice" (Arg.Set_int renice) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Renice invariant generation process. Give a positive argument \
           to@ lower priority@]")
...
module C2I = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>C2I is an invariant generation technique based on \
       machine-learning.@ The Kind 2 implementation is still experimental and \
  ...
  let _ =
    add_spec "--c2i_dnf" (Arg.Set_int dnf_size) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Number of disjuncts in the DNF constructed by C2I@ Default %d@]"
          dnf_size_default)
  ...
  let _ =
    add_spec "--c2i_int_cubes" (Arg.Set_int int_cube_size) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Number of int cubes in the DNF constructed by C2I@ Default %d@]"
  ...
  let _ =
    add_spec "--c2i_real_cubes" (Arg.Set_int real_cube_size) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Number of real cubes in the DNF constructed by C2I@ Default \
           %d@]"
  ...
  let _ =
    add_spec "--c2i_modes" (bool_arg modes) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Activates mode subcandidates. Subsumes \"c2i_dnf_size\"@ \
           Default %a@]"
...
module Interpreter = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>The interpreter is a special mode where Kind 2 reads inputs from \
       a@ file and prints the outputs of the system at each step.@]"

  let input_file_default = ""
  let input_file = ref input_file_default

  let _ =
    add_spec "--interpreter_input_file" (Arg.Set_string input_file) (fun fmt ->
  ...
  let _ =
    add_spec "--interpreter_steps" (Arg.Set_int steps) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Run number of steps, override the number of steps given in \
           the@ input file@ Default: %d@]"
          steps_default)
...
module Lsp = struct
  ...
  let fmt_explain fmt =
    Format.fprintf fmt
      "@[<v>Flags in this module allow LSP servers to request information,@ \
  ...
  let _ =
    add_spec "--lsp" (bool_arg lsp) (fun fmt ->
        Format.fprintf fmt
          "Provide AST info for language-servers. Only in JSON output.@ \
           Default: %a"
  ...
  let _ =
    add_spec "--fake_filepath"
      (Arg.String (fun str -> fake_filepath := str))
      (fun fmt ->
        Format.fprintf fmt "@[<v>Fake filepath for error messages.\n        @]")
...
let usage_msg_header =
  Filename.basename Sys.executable_name
  |> Format.sprintf "Usage: \x1b[1m%s [options] [input_file]\x1b[0m"

let all_modules_id = "all"

let module_map =
  [
    (Smt.id, (module Smt : FlagModule));
    (BmcKind.id, (module BmcKind : FlagModule));
    (IC3QE.id, (module IC3QE : FlagModule));
    (IC3IA.id, (module IC3IA : FlagModule));
    (Invgen.id, (module Invgen : FlagModule));
    (C2I.id, (module C2I : FlagModule));
    (Testgen.id, (module Testgen : FlagModule));
    (Interpreter.id, (module Interpreter : FlagModule));
    (QE.id, (module QE : FlagModule));
    (Contracts.id, (module Contracts : FlagModule));
    (IVC.id, (module IVC : FlagModule));
    (MCS.id, (module MCS : FlagModule));
    (Arrays.id, (module Arrays : FlagModule));
    (Certif.id, (module Certif : FlagModule));
    (Lsp.id, (module Lsp : FlagModule));
  ]
...
let fmt_module_info fmt (id, m0d) =
  ...
  Format.fprintf fmt
    "|===| Module \x1b[1m%s\x1b[0m:@.@.@[<v>%t@]@.@.  @[<v>%a@]@." id
    Flags.fmt_explain
...
let print_module_info = function
  ...
  | hd :: tl as ids ->
      ...
      if List.mem all_modules_id ids then
        Format.printf "%a" (pp_print_list fmt_module_info "@.@.") module_map
      else (
        (try
           let m0d = module_map |> List.assoc hd in
           Format.printf "%a" fmt_module_info (hd, m0d)
         with Not_found ->
           Format.printf "Unknown module \"%s\", legal values are@.  all, %a@."
             hd
             (pp_print_list
        ...
        |> List.iter (fun id ->
               ...
               with Not_found ->
                 Format.printf
                   "Unknown module \"%s\", legal values are@.  all, %a@." id
                   (pp_print_list
                      (fun fmt (key, _) -> Format.fprintf fmt "%s" key)
                      ", ")
                   module_map;
...
module Global = struct
  ...
  let usage_msg =
    Format.sprintf
      "%s@.Prove properties over the Lustre program in <input_file> or from \
       standard@ input (in Lustre) if no file is given.@.Global options \
       follow, use \"--help_of\" for module-specific information."
      usage_msg_header
  ...
  let clear_input_files () =
    all_input_files := [];
    all_input_ids := FileId.FileIdSet.empty
  ...
  let add_input_file file =
    ...
    if not (FileId.FileIdSet.mem id !all_input_ids) then (
      ...
      all_input_ids := FileId.FileIdSet.add (FileId.get_id file) !all_input_ids;
      true)
    else false
  ...
  let help_of_desc fmt =
    Format.fprintf fmt
      "Explains and shows the flags for a Kind 2 module@ <string> should be a \
       legal module identifier among@   @[<v>%a@ %-15s explanation and flags \
       for all Kind 2 modules@]"
      (pp_print_list
         (fun fmt (key, m0d) ->
           let module Flags = (val m0d : FlagModule) in
           Format.fprintf fmt "%-15s %s" (Format.sprintf "%s" key) Flags.desc)
         "@ ")
      module_map all_modules_id
           ...
           if str = all_modules_id || List.mem_assoc str module_map then (
             ...
             BadArg
               ( Format.sprintf "unknown module identifier \"%s\"" str,
                 ("--help_of", Arg.String (fun _ -> ()), help_of_desc) )
             |> raise))
      help_of_desc
      ...
      (fun fmt ->
        Format.fprintf fmt
          "where <string> is a node identifier from the Lustre input file@ \
           Designate a node as the top node for the analysis or the \
           interpretation@ Default: all nodes with \"--%%MAIN\" annotations in \
           source if any, otherwise@ all nodes that are not depended on by \
           another node")
  ...
  let _ =
    add_spec "--lus_main_type"
      (Arg.String (fun str -> lus_main_type := Some str))
      (fun fmt ->
        Format.fprintf fmt
          "where <string> is a type alias identifier from the Lustre input \
           file@ Designate a type declaration for which the corresponding \
           generated node@ is the top node for realizability analysis@ \
  ...
  let input_format_values =
    ...
    |> List.map string_of_input_format
    |> String.concat ", "
  ...
  let _ =
    add_spec "--output_dir" (Arg.String output_dir_action) (fun fmt ->
        Format.fprintf fmt
          "Output directory for the files generated:@ SMT traces, compilation, \
           testgen, certification...@ Default: \"<path_to_filename>.out\"")

  let set_output_dir s =
    match !output_dir with "" -> s ^ ".out" |> output_dir_action | _ -> ()

  let output_dir () = !output_dir
  let include_dirs = ref []

  let _ =
    add_spec "--include_dir"
      (Arg.String (fun dir -> include_dirs := dir :: !include_dirs))
      (fun fmt ->
        Format.fprintf fmt
          "where <string> is a directory to be included in the search path@ \
           The directory will be searched after the current include \
           directory,@ and any other directory added before it (left-to-right \
           order) when@ an include directive is found")
  ...
  let real_precision_values =
    [ `Rational; `Float ]
    |> List.map string_of_real_precision
    |> String.concat ", "
  ...
  let _ =
    add_spec "--real_precision"
      (Arg.String (fun str -> real_precision := real_precision_of_string str))
      (fun fmt ->
        Format.fprintf fmt
          "where <string> can be %s@ Adjust precision of real values in model \
           output@ In floating-point format f<nn> means a relative error less \
           than 2^-nn@ Default: %s"
  ...
  let _ =
    add_spec "--log_invs" (bool_arg log_invs) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--print_invs" (bool_arg print_invs) (fun fmt ->
        Format.fprintf fmt
          "Prints list of discovered invariants after minimization.@ Default: \
           %b"
          print_invs_default)
  ...
  let _ =
    add_spec "--print_cex" (bool_arg print_cex) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Print counterexamples to (disproven) invariant properties.@ \
  ...
  let _ =
    add_spec "--print_witness" (bool_arg print_witness) (fun fmt ->
        Format.fprintf fmt
          "@[<v>Print witnesses of proven reachability properties.@ Only in \
           plain text output. Default: %a@]"
  ...
  let _ =
    add_spec "--dump_cex" (bool_arg dump_cex) (fun fmt ->
        Format.fprintf fmt
          "Dump counterexamples to a file instead of printing them in stdout.@ \
           Only in plain text output. Implies --print_cex true.@ Default: %b"
          dump_cex_default)
  ...
  let _ =
    add_spec "--dump_witness" (bool_arg dump_witness) (fun fmt ->
        Format.fprintf fmt
          "Dump witnesses to a file instead of printing them in stdout.@ Only \
           in plain text output. Implies --print_witness true.@ Default: %b"
    ...
    add_specs
      [
        ( "--timeout",
          ...
          fun fmt ->
            Format.fprintf fmt "Wallclock timeout in seconds@ Default: %1.0f"
              timeout_wall_default );
        ( "--timeout_wall",
          Arg.Set_float timeout_wall,
          fun fmt -> Format.fprintf fmt "Same as \"--timeout\", for legacy" );
      ]
  ...
  let _ =
    add_spec "--only_parse" (bool_arg only_parse) (fun fmt ->
        Format.fprintf fmt
          "Only parse the Lustre program. No analysis is performed.@ Default: \
           %a"
  ...
  let _ =
    add_spec "--old_frontend" (bool_arg old_frontend) (fun fmt ->
        Format.fprintf fmt "Use the old Lustre front-end.@ Default: %a" fmt_bool
  ...
  let disable modul3 = enabled := !enabled |> List.filter (fun m -> m <> modul3)
  let disabled = ref disable_default_init

  let finalize_enabled () =
    (* If [enabled] is unchanged, set it do default after init. *)
    if !enabled = enable_default_init then enabled := enable_default_after
    else if !enabled = `MCS :: enable_default_init then
      enabled := `MCS :: enable_default_after;
    (* Remove disabled modules. *)
    enabled :=
      !enabled |> List.filter (fun mdl -> List.mem mdl !disabled |> not)
    ...
    add_spec "--enable"
      (Arg.String
         (fun str ->
           ...
           if List.mem mdl !enabled |> not then enabled := mdl :: !enabled))
      (fun fmt ->
        Format.fprintf fmt
          "@[<hov>where <string> can be %a@]@ Enable Kind module, repeat \
           option to enable several modules.@ IC3 is a virtual module that \
           enables both IC3QE and IC3IA.@ Default: %s"
  ...
  let invgen_enabled () =
    ...
    |> List.filter (function
         | `INVGEN | `INVGENOS | `INVGENINT | `INVGENINTOS | `INVGENMACH
         | `INVGENMACHOS | `INVGENREAL | `INVGENREALOS ->
             true
         | _ -> false)
  ...
  let _ =
    add_spec "--disable"
      (Arg.String
         (fun str ->
           let mdl = kind_module_of_string str in
           if List.mem mdl !disabled |> not then disabled := mdl :: !disabled))
      (fun fmt ->
        Format.fprintf fmt
          "@[<hov>where <string> can be %a@]@ Disable a Kind module"
          (pp_print_list Format.pp_print_string ",@ ")
          enable_values)
  ...
  let _ =
    add_spec "--modular" (bool_arg modular) (fun fmt ->
        Format.fprintf fmt "Bottom-up analysis of each node@ Default: %a"
  ...
  let _ =
    add_spec "--slice_nodes" (bool_arg slice_nodes) (fun fmt ->
        Format.fprintf fmt
          "Only equations that are relevant for checking the contract and@ \
           properties of a node are considered during the analysis@ Default: \
           %a"
  ...
  let _ =
    add_spec "--check_reach" (bool_arg check_reach) (fun fmt ->
        Format.fprintf fmt
  ...
  let _ =
    add_spec "--check_nonvacuity" (bool_arg check_nonvacuity) (fun fmt ->
        Format.fprintf fmt
          "Check non-vacuity of contract modes and conditional properties.@ \
           Ignored if --check_reach is false.@ Default: %a"
  ...
  let _ =
    add_spec "--check_subproperties" (bool_arg check_subproperties) (fun fmt ->
        Format.fprintf fmt
          "Check properties of subnodes in addition to properties of the main \
           node.@ Ignored if --modular is true@ Default: %a"
  ...
  let _ =
    add_spec "--compile" (bool_arg lus_compile) (fun fmt ->
        Format.fprintf fmt
          "Nodes proved correct will be compiled to Rust@ Note that \
           uninitialized pre's are not allowed in this mode@ Default: %a"
  ...
  let _ =
    add_spec "--lus_strict" (bool_arg lus_strict) (fun fmt ->
        Format.fprintf fmt
          "Strict mode in Lustre: uninitialized pre and undefined@ Local \
           variables are not allowed when this flag is present@ Default: %a"
  ...
  let _ =
    add_spec "--lus_push_pre" (bool_arg lus_push_pre) (fun fmt ->
        Format.fprintf fmt
          "Optional transformation of pre expressions to push them@ deeper \
           into the AST, e.g. `pre (x + y)` becomes `pre x + pre y`@ Default: \
           %a"
  ...
  let _ =
    add_spec "--debug_log"
      ...
      (fun fmt ->
        Format.fprintf fmt
          "where <string> is a file path in an existing directory.@ Output \
           debug messages to file@ Default: stdout")
  ...
  let exit_code_convention_of_string = function
    ...
    | unexpected ->
        Arg.Bad
          (Format.sprintf "Unexpected value \"%s\" for flag --exit_code"
             unexpected)
        |> raise
  ...
  let _ =
    add_spec "--exit_code_mode"
      (Arg.String
         (fun str -> exit_code_mode := exit_code_convention_of_string str))
      (fun fmt ->
        Format.fprintf fmt
          "where <string> can be results_and_errors, only_errors@ Select the \
           exit code convention that Kind 2 should follow@ results_and_errors \
           = use exit code to report analysis results and errors@ only_errors \
           = only use a non-zero exit code for errors@ See user documentation \
           for a description of the exit code conventions@ Default: %s"
    ...
    add_specs
      [
        ( "-qq",
          ...
          fun fmt -> Format.fprintf fmt "Disable output completely" );
        ( "-q",
          Arg.Unit (fun () -> set_log_level L_fatal),
          fun fmt -> Format.fprintf fmt "Disable output, fatal errors only" );
        ( "-s",
          Arg.Unit (fun () -> set_log_level L_error),
          fun fmt -> Format.fprintf fmt "Silence output, errors only" );
        ( "-v",
          Arg.Unit (fun () -> set_log_level L_info),
          fun fmt -> Format.fprintf fmt "Output informational messages" );
        ( "-vv",
  ...
  let _ =
    add_format_spec "-xml"
      (Arg.Unit (fun () -> Log.set_log_format_xml ()))
  ...
  (* JSON log. *)
  let _ =
    add_format_spec "-json"
      (Arg.Unit (fun () -> Log.set_log_format_json ()))
  ...
  let _ =
    add_spec "--color" (bool_arg color) (fun fmt ->
        Format.fprintf fmt
          "Display colors in ascii output (deactivated when using -xml or \
           -json)@ Default: %a"
  ...
  let _ =
    add_spec "--" (Arg.Set only_filename) (fun fmt ->
        Format.fprintf fmt "What follows on the command line is a file name.")
end
...
let subdir_for scope = Filename.concat (output_dir ()) (String.concat "_" scope)
...
let arg_bool_of_string tuple s =
  ...
  else BadArg (Format.sprintf "expected bool but got \"%s\"" s, tuple) |> raise

let arg_int_of_string tuple s =
  try int_of_string s
  with _ ->
    BadArg (Format.sprintf "expected int but got \"%s\"" s, tuple) |> raise

let arg_float_of_string tuple s =
  try float_of_string s
  with _ ->
    BadArg (Format.sprintf "expected float but got \"%s\"" s, tuple) |> raise
  ...
  | _ :: args -> (
        ...
        | ((flag', spec, _) as tuple) :: _ when flag = flag' -> (
            ...
            | _ -> (
                ...
                | arg :: clas ->
                    ...
                    | Arg.Set_float f_ref ->
                        f_ref := arg_float_of_string tuple arg
                    | _ ->
                        failwith
                          "unsupported specification (Tuple, Symbol, or Rest)");
                    clas))
      ...
      let check_format_flags args =
        if Log.get_log_format () = Global.log_format_default then
          ...
          let format_arg_specs =
            List.fold_left
              (fun acc arg ->
                try
                  List.find (fun (flag', _, _) -> arg = flag') format_specs
                  :: acc
                with Not_found -> acc)
              [] args
          in
      ...
      try args |> cla_loop with
      | UnknownFlag flag ->
          check_format_flags args;
          (match Log.get_log_format () with
          | Log.F_pt | Log.F_relay ->
              Global.print_help ();
              Format.printf "\n\x1b[31;1mError\x1b[0m: unknown flag \"%s\".@."
                flag
          | Log.F_xml | Log.F_json -> Log.log L_error "Unknown flag '%s'" flag);
          exit ExitCodes.usage_error
      | BadArg (error, spec) ->
          check_format_flags args;
          (match Log.get_log_format () with
          | Log.F_pt | Log.F_relay ->
              Format.printf "\x1b[31;1mError on flag\x1b[0m@.@[<v>%a@]@.%s@."
                fmt_flag spec error
      ...
      | Arg.Bad expl ->
          ...
          (match Log.get_log_format () with
          | Log.F_pt | Log.F_relay ->
              Format.printf "\x1b[31;1mBad argument\x1b[0m: @[<v>%s.@]@." expl
          | Log.F_xml -> Log.log L_error "Bad argument:@ @[<v>%s@]@." expl
          | Log.F_json -> Log.log L_error "Bad argument: %s" expl);
          exit ExitCodes.usage_error)
  | [] -> failwith "expected at least one argument, got zero"

let solver_dependant_actions solver =
  let get_version with_patch cmd =
    let get_rev output idx = int_of_string (Str.matched_group idx output) in
    let version_re =
  ...
  | `MathSAT_SMTLIB -> (
      ...
      | Some (major_rev, minor_rev, patch_rev) ->
          if
            major_rev < 5
            || major_rev = 5
               && (minor_rev < 5 || (minor_rev = 5 && patch_rev < 4))
          then
            if Smt.check_sat_assume () then (
              Log.log L_warn
                "Detected MathSAT 5.5.3 or older: disabling check_sat_assume";
  ...
  | `Z3_SMTLIB -> (
      ...
      | Some (major_rev, minor_rev, _) ->
          if major_rev < 4 || (major_rev = 4 && minor_rev < 6) then
            if Smt.check_sat_assume () then (
              Log.log L_warn
                "Detected Z3 4.5.x or older: disabling check_sat_assume";
              Smt.set_check_sat_assume false)
  ...
  | `cvc5_SMTLIB -> (
      ...
      | Some (major, minor, patch) ->
          if major < 1 then (
            Log.log L_error
              "Kind 2 requires cvc5 1.0.0 or later. Found version: %d.%d.%d"
              major minor patch;
            raise UnsupportedSolver);
          if
            Certif.proof ()
          ...
          then (
            Log.log L_error
              "LFSC proof production requires cvc5 >= 1.0.3 and <= 1.1.0. \
               Found version: %d.%d.%d"
              major minor patch;
            raise UnsupportedSolver))
  | `Yices_native -> (
      ...
      | Some (major_rev, _, _) ->
          if major_rev > 1 then (
            Log.log L_error
              "Selected Yices 1 (native format), but found Yices 2 or later";
            raise UnsupportedSolver)
...
let print_xml_options () =
  ...
  Format.fprintf !log_ppf
    "@[<v><Results xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" \
     enabled=\"%a\" timeout=\"%f\" bmc_max=\"%d\" compositional=\"%b\" \
     modular=\"%b\"%s%s>@.@.@."
    (pp_print_list Log.pp_print_kind_module_xml_src ",")
    (Global.enabled ()) (Global.timeout_wall ()) (BmcKind.max ())
    (Contracts.compositional ())
    (Global.modular ())
    (if IVC.compute_ivc () then
       Format.asprintf
         " ivc_category=\"%a\" ivc_all=\"%b\" ivc_approximate=\"%b\" \
          ivc_smallest_first=\"%b\" ivc_only_main_node=\"%b\" \
          ivc_must_set=\"%b\""
         (pp_print_list pp_print_category_str ",")
         (IVC.ivc_category ()) (IVC.ivc_all ()) (IVC.ivc_approximate ())
         (IVC.ivc_smallest_first ())
         (IVC.ivc_only_main_node ())
         (IVC.ivc_must_set ())
     else "")
    (if
       MCS.compute_mcs ()
       || Global.enabled () |> List.exists (fun m -> m = `MCS)
     then
       Format.asprintf
         " mcs_category=\"%a\" mcs_all=\"%b\" mcs_only_main_node=\"%b\""
         (pp_print_list pp_print_category_str ",")
         (MCS.mcs_category ()) (MCS.mcs_all ())
         (MCS.mcs_only_main_node ())
     else "")
...
let print_json_options () =
  ...
  Format.fprintf !log_ppf
    "{@[<v 1>@,\
     ...
     \"compositional\" : %b,@,\
     \"modular\" : %b%s%s@]@.}@."
    (pp_print_list pp_print_module_str ",@,")
    (Global.enabled ()) (Global.timeout_wall ()) (BmcKind.max ())
    (Contracts.compositional ())
    (Global.modular ())
    (if IVC.compute_ivc () then
...
let post_argv_parse_actions () =
  if Lsp.fake_filepath () <> "" then set_stdin_id (Lsp.fake_filepath ());
...
let parse_argv () =
  ...
  let open Format in
  if color () && not (log_format_xml () || log_format_json ()) then (
  ...
  if Global.check_reach () then (
    ...
    if Certif.certif () || Certif.proof () then (
      Log.log L_warn
        "Certification post-analysis enabled: disabling reachability checks";
      Global.set_check_reach false);
    if print_invs () then (
      Log.log L_warn "Invariant printing enabled: disabling reachability checks";
      Global.set_check_reach false));

  if not (Global.check_reach ()) then Global.set_check_nonvacuity false
