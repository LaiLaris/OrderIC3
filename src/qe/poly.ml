...
let pp_print_psummand ppf = function
  ...
  | c, Some x when Numeral.(c = one) -> Var.pp_print_var ppf x
  | c, Some x when Numeral.(c = neg one) -> Var.pp_print_var ppf x
...
let pp_print_poly ppf = function
  | ((c, _) as s) :: tl ->
      if Numeral.(c < zero) then Format.pp_print_string ppf "-";
      pp_print_psummand ppf s;
      Format.pp_print_string ppf " ";
      pp_print_poly' ppf tl
...
let rec pp_print_cformula ppf cf =
  ...
  | [ pret ] -> pp_print_preAtom ppf pret
  | pret :: cf' ->
...
let compare_psummands (c : Var.t -> Var.t -> int) (ps1 : psummand)
    (ps2 : psummand) : int =
  match (ps1, ps2) with
  | (_, None), (_, None) -> 0
  | (_, None), (_, Some _) -> 1
...
let negate_poly (pt : poly) : poly = List.map negate_psummand pt

(* Multiply a presburger summand with a integer. *)
let psummand_times_num ((c, v) : psummand) (i : Numeral.t) : psummand =
  (Numeral.(c * i), v)
...
let psummand_contains_variable (v : Var.t) (ps : psummand) : bool =
  match ps with
  | _, None -> false
  | _, Some ps_v -> if Var.compare_vars ps_v v = 0 then true else false
...
let get_coe_in_poly_obv (v : Var.t) (pl : poly) : Numeral.t =
  match pl with
  | (i, Some v1) :: _ -> if Var.equal_vars v1 v then i else Numeral.zero
  | _ -> Numeral.zero
...
let preAtom_contains_variable (v : Var.t) (pret : preAtom) : bool =
  ...
  | DIVISIBLE (_, pl) -> Numeral.(get_coe_in_poly v pl <> zero)
  | INDIVISIBLE (_, pl) -> Numeral.(get_coe_in_poly v pl <> zero)

(* Check if a presburger formula cf contains the variable v. 
   Return true when pf contains v.
   Return false otherwise. *)
let cformula_contains_variable (v : Var.t) (cf : cformula) : bool =
  List.exists (preAtom_contains_variable v) cf
...
let rec add_two_polys (c : Var.t -> Var.t -> int) (accum : poly) (pt1 : poly)
    (pt2 : poly) : poly =
  match (pt1, pt2) with
