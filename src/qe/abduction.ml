...
let assert_context solver premises =
  ...
  | `cvc5_SMTLIB -> SMTSolver.assert_term solver premises
  | _ -> ()
...
let normalize_if_inconsistent solver premises term =
  ...
  let term' = if SMTSolver.check_sat solver then term else Term.t_false in
...
let simplify_abduct solver premises term =
  ...
  if SMTSolver.kind solver = `cvc5_SMTLIB then
    ...
    let res =
      ...
      |> List.filter (fun dj ->
             let actlit_symb =
               UfSymbol.mk_fresh_uf_symbol [] (Type.mk_bool ())
             in
...
let abduce_simpl solver forall_vars premises conclusion =
  ...
  | _ ->
      ...
      res |> simplify_abduct solver premises |> Simplify.remove_ite
