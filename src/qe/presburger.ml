...
(* Intermediate formula in the bottom-up conversion from term to
   Presburger atoms*)
type iformula = Poly of poly | Formula of cformula
...
let combine_singleton_to_iformula (if1 : iformula) (if2 : iformula) : iformula =
  ...
  | _ ->
      failwith
        "The first Presburger formula must be a singleton in \
...
let multiply_iformula_list (ifl : iformula list) : poly =
  ...
  | Poly pl :: ifl' ->
      ...
      List.fold_left
        ...
        pl ifl'
  | _ -> failwith "multiply_iformula_list can only multiply polynomials."

(* Convert equality, which may have arity greater than two, to a
   conjunction of binary equalities *)
let unchain_EQ_to_iformula (c : Var.t -> Var.t -> int) (ifl : iformula list) :
    iformula =
  match ifl with
  | _ :: [] | [] -> failwith "The chain of EQ must have at least two elements."
  | if1 :: ifl' ->
           ...
           (fun (prev, accum) e ->
             ...
             | Poly pl1, Poly pl2 ->
                 ...
                 ( e,
                   ...
                   combine_singleton_to_iformula
                     (Formula [ EQ (add_two_polys c [] (negate_poly pl1) pl2) ])
                     accum )
             | _ -> failwith "EQ can only work with list of polynomials")
           (if1, Formula []) ifl')
...
let unchain_LEQ_to_iformula (c : Var.t -> Var.t -> int) (ifl : iformula list) :
    iformula =
  match ifl with
  | _ :: [] | [] -> failwith "The chain of LEQ must have at least two elements."
  | if1 :: ifl' ->
             ...
             | Poly pl1, Poly pl2 ->
                 ...
                 ( e,
                   ...
                   combine_singleton_to_iformula
                     ...
                     accum )
             | _ -> failwith "LEQ can only work with list of polynomials.")
           (if1, Formula []) ifl')

(* Convert a list of presburger formulas chained by LT into an iformula. *)
let unchain_LT_to_iformula (c : Var.t -> Var.t -> int) (ifl : iformula list) :
    iformula =
  match ifl with
  | _ :: [] | [] -> failwith "The chain of LT must have at least two elements."
  | if1 :: ifl' ->
      (* Convert variadic relation to conjunction of binary
         greater-than relations *)
      snd
           ...
           (fun (prev, accum) e ->
             ...
             | Poly pl1, Poly pl2 ->
                 ...
                 ( e,
                   (* Turn x < y into y - x > 0 *)
                   combine_singleton_to_iformula
                     (Formula [ GT (add_two_polys c [] (negate_poly pl1) pl2) ])
                     accum )
             | _ -> failwith "LT can only work with list of polynomials.")
           (if1, Formula []) ifl')

(* Convert a list of presburger formulas chained by GEQ into an iformula. *)
let unchain_GEQ_to_iformula (c : Var.t -> Var.t -> int) (ifl : iformula list) :
    iformula =
  match ifl with
  | _ :: [] | [] -> failwith "The chain of GEQ must have at least two elements."
  | if1 :: ifl' ->
      (* Convert variadic relation to conjunction of binary
         greater-than relations *)
      snd
             ...
             | Poly pl1, Poly pl2 ->
                 ...
                 ( e,
                   ...
                   combine_singleton_to_iformula
                     ...
                     accum )
             | _ -> failwith "GEQ can only work with list of polynomials.")
           (if1, Formula []) ifl')

(* Convert a list of presburger formulas chained by GT into an iformula. *)
let unchain_GT_to_iformula (c : Var.t -> Var.t -> int) (ifl : iformula list) :
    iformula =
  match ifl with
  | _ :: [] | [] -> failwith "The chain of GT must have at least two elements."
  | if1 :: ifl' ->
      (* Convert variadic relation to conjunction of binary
         greater-than relations *)
      snd
           ...
           (fun (prev, accum) e ->
             ...
             | Poly pl1, Poly pl2 ->
                 ...
                 ( e,
                   (* Turn x > y into x - y > 0 *)
                   combine_singleton_to_iformula
                     (Formula [ GT (add_two_polys c [] pl1 (negate_poly pl2)) ])
                     accum )
             | _ -> failwith "GT can only work with list of polynomials.")
           (if1, Formula []) ifl')
    ...
    Term.eval_t
      (fun fterm args ->
        ...
        | Term.T.Var var when Var.type_of_var var = Type.t_real ->
            raise Not_in_LIA
        | Term.T.Var var -> Poly [ (Numeral.one, Some var) ]
        | Term.T.Const sym | Term.T.App (sym, _) -> (
            ...
            | `NOT, [ Formula _ ] ->
                failwith
                  "NOT only take one argument, and can only appear in the atom \
                   level."
            (* Fail on implication *)
            | `IMPLIES, _ ->
                failwith "Presburger atoms may not conatin Boolean operators."
            (* Fail on empty conjunction *)
            | `AND, [] -> failwith "AND must take at least one argument."
            ...
            | `AND, ifm :: l' ->
                ...
                List.fold_left
                  (fun ifm1 ifm2 ->
                    ...
                    | _ ->
                        failwith
                          "AND only takes formula as arguments, not polynomial.")
                  ifm l'
            (* Fail on disjunction *)
            | `OR, _ ->
                failwith "Presburger atoms may not conatin Boolean operators."
            ...
            | `EQ, ifl -> unchain_EQ_to_iformula c ifl
            (* Fail on distinct *)
            | `DISTINCT, _ ->
                failwith "Presburger atoms may not conatin Boolean operators."
            ...
            | `LEQ, ifl -> unchain_LEQ_to_iformula c ifl
            (* Variadic less-than *)
            | `LT, ifl -> unchain_LT_to_iformula c ifl
            (* Variadic greater-than-or-equal *)
            | `GEQ, ifl -> unchain_GEQ_to_iformula c ifl
            (* Variadic greater-than *)
            | `GT, ifl -> unchain_GT_to_iformula c ifl
            ...
            | `DIVISIBLE i, [ Poly pl ] -> Formula [ DIVISIBLE (i, pl) ]
            | `NOT, _
            | `MINUS, _
            | `DIVISIBLE _, _
            | `BV _, _
...
let term_of_preAtom = function
  ...
  | DIVISIBLE (i, poly) -> (
      ...
      (* i | 0 becomes true *)
      | [] -> Term.mk_true ()
      | _ -> Term.mk_divisible i (term_of_poly poly))
  (* Polynomial not divisible by constant *)
  | INDIVISIBLE (i, poly) -> (
      match poly with
      (* i !| 0 becomes false *)
      | [] -> Term.mk_false ()
      | _ -> Term.mk_not (Term.mk_divisible i (term_of_poly poly)))
