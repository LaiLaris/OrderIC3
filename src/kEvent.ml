...
let div_by_zero_text prop_name =
  [
    ...
    Format.sprintf "Counterexample for property '%s' may be inconsistent."
      prop_name;
  ]
...
let pp_print_event ppf = function
  ...
  | PropStatus (p, Property.PropFalse cex) ->
      Format.fprintf ppf "@[<hv>Property %s false at step %d@]" p
        (Property.length_of_cex cex - 1)
  | StepCex (p, cex) ->
      Format.fprintf ppf "@[<hv>Step cex for property %s at step %d@]" p
        (Property.length_of_cex cex)

(* Module as input to Messaging.Make functor *)
module EventMessage = struct
  type t = event
  ...
  let strings_of_message = function
    ...
    | PropStatus (p, Property.PropUnknown) -> [ "PROP_UNKNOWN"; p ]
    | PropStatus (p, Property.PropKTrue k) ->
...
type messaging_setup =
  (EventMessaging.ctx * EventMessaging.pub_socket * EventMessaging.pull_socket)
  * (string * string)
...
(* Map of kind_module *)
module MdlMap = Map.Make (struct
  type t = kind_module

  let compare m1 m2 = compare (int_of_kind_module m1) (int_of_kind_module m2)
end)
...
let all_stats () =
  List.rev
    (MdlMap.fold (fun mdl stats accum -> (mdl, stats) :: accum) !last_stats [])

(* ********************************************************************** *)
(* Plain text output                                                      *)
(* ********************************************************************** *)
(* Pretty-print kind module for plain text output *)
let pp_print_kind_module_pt = pp_print_kind_module
...
let tag_pt level tag str =
  (ignore_or_fprintf level) !log_ppf "@[<hov>%t %s@.@." tag str
...
let proved_pt mdl level trans_sys k prop =
  ...
  if not (Property.prop_status_known (Property.get_prop_status property)) then
    let prop_type =
      match property.prop_source with
    ...
    let k_val = function
      | ppf -> (
          match k with None -> () | Some k -> Format.fprintf ppf "for k=%d " k)
    in
    ...
    | Property.Invariant ->
        (ignore_or_fprintf level) !log_ppf
          ...
          success_tag prop_type
          (LGR.get_node_type_and_name prop |> snd)
          k_val pp_print_kind_module_pt mdl
          (Stat.get_float Stat.analysis_time)
    | Property.Reachable (Some (From ts)) ->
        (ignore_or_fprintf level) !log_ppf
          "@[<hov>%t %s @{<blue_b>%s@} is unreachable in %d steps or more %tby \
           %a after %.3fs.@.@."
          failure_tag prop_type
          (LGR.get_node_type_and_name prop |> snd)
          ts k_val pp_print_kind_module_pt mdl
          (Stat.get_float Stat.analysis_time)
    | Property.Reachable (Some (Within ts)) ->
        (ignore_or_fprintf level) !log_ppf
          "@[<hov>%t %s @{<blue_b>%s@} is unreachable in %d steps or less %tby \
           %a after %.3fs.@.@."
          failure_tag prop_type
          (LGR.get_node_type_and_name prop |> snd)
          ts k_val pp_print_kind_module_pt mdl
    ...
    | Property.Reachable (Some (At ts)) ->
        (ignore_or_fprintf level) !log_ppf
          "@[<hov>%t %s @{<blue_b>%s@} is unreachable at step %d %tby %a after \
           %.3fs.@.@."
          failure_tag prop_type
          (LGR.get_node_type_and_name prop |> snd)
          ts k_val pp_print_kind_module_pt mdl
    ...
    | Property.Reachable (Some (FromWithin (ts1, ts2))) ->
        (ignore_or_fprintf level) !log_ppf
          "@[<hov>%t %s @{<blue_b>%s@} is unreachable between steps %d and %d \
           %tby %a after %.3fs.@.@."
          failure_tag prop_type
          (LGR.get_node_type_and_name prop |> snd)
          ts1 ts2 k_val pp_print_kind_module_pt mdl
          (Stat.get_float Stat.analysis_time)
...
let unknown_pt mdl level trans_sys prop =
  ...
  if not (Property.prop_status_known (TransSys.get_prop_status trans_sys prop))
  then
    (ignore_or_fprintf level) !log_ppf
      "@[<hov>%t %s @{<blue_b>%s@} is unknown by %a after %.3fs.@.@."
      warning_tag
      (if TransSys.is_candidate trans_sys prop then "Candidate" else "Property")
      (LGR.get_node_type_and_name prop |> snd)
...
let slice_trans_sys_and_cex_to_property input_sys analysis trans_sys prop_name
    cex =
  ...
  | Some prop_name ->
      ...
      let trans_sys, instances, cex, _, input_sys =
        InputSystem.slice_to_abstraction_and_property input_sys analysis
          trans_sys cex prop
      in
  ...
  | None ->
      ( trans_sys,
        [],
        cex,
        InputSystem.slice_to_abstraction input_sys analysis trans_sys )

(* Pretty-print a trace *)
let pp_print_trace_pt ?(title = "Counterexample") ?(color = "red") dump level
    input_sys analysis trans_sys prop_name disproved ppf = function
  | [] -> ()
  | trace ->
      (* Slice trace and transitions system to property *)
      let trans_sys, _, trace, input_sys =
        slice_trans_sys_and_cex_to_property input_sys analysis trans_sys
          prop_name trace
      in

      let print_trace fmt =
        Format.fprintf fmt "@{<%s>%s@}:@,  @[<v>%a@]" color title
          (InputSystem.pp_print_path_pt input_sys trans_sys disproved)
          (Model.path_of_list trace)
      in

      if dump then (
        let dirname = Filename.concat (Flags.output_dir ()) "trace" in
        ...
        | Some prop_name ->
            (ignore_or_fprintf level) !log_ppf
              "@[<hov>%t %s of @{<blue_b>%s@} written to '%s'@." note_tag title
              prop_name path
        | None ->
            (ignore_or_fprintf level) !log_ppf "@[<hov>%t %s written to '%s'@."
              note_tag title path)
      else print_trace ppf
...
let execution_path_pt level input_sys trans_sys path =
  (ignore_or_fprintf level) !log_ppf "@[<v>@{<b>Execution@}:@,%a@]@."
    (pp_print_path_pt input_sys trans_sys)
    path

(* Output cex for a property as plain text *)
let cex_pt ?(wa_model = []) mdl level input_sys analysis trans_sys prop cex
    disproved =
  (* Only ouptut if status was unknown *)
  if not (Property.prop_status_known (TransSys.get_prop_status trans_sys prop))
  then (
    ...
    if TransSys.is_candidate trans_sys prop then (
      if disproved then
        (ignore_or_fprintf level) !log_ppf
          ...
          warning_tag prop pp_print_kind_module_pt mdl
          (function
            | ppf -> (
                match cex with
                | [] -> ()
      ...
      let title, color, print, dump =
        ...
        | Property.Invariant ->
            ( "Counterexample",
              "red",
              Flags.print_cex () || Flags.dump_cex (),
              Flags.dump_cex () )
        | Property.Reachable _ ->
            ( "Witness",
              "green",
              Flags.print_witness () || Flags.dump_witness (),
              Flags.dump_witness () )
      in
      (* Output cex. *)
      (ignore_or_fprintf level) !log_ppf
        ...
        (if disproved then
           if kind = Property.Invariant then failure_tag else success_tag
         else warning_tag)
        prop
        ...
        | true, Property.Reachable (Some (From ts)) ->
            Format.asprintf "is reachable in %d steps or more by %a" ts
              pp_print_kind_module_pt mdl
        | true, Property.Reachable (Some (Within ts)) ->
            Format.asprintf "is reachable in %d steps or less by %a" ts
              pp_print_kind_module_pt mdl
        | true, Property.Reachable (Some (At ts)) ->
            Format.asprintf "is reachable at step %d by %a" ts
              pp_print_kind_module_pt mdl
        | true, Property.Reachable (Some (FromWithin (ts1, ts2))) ->
            Format.asprintf "is reachable between steps %d and %d by %a" ts1 ts2
              pp_print_kind_module_pt mdl
        | true, Property.Reachable None ->
            Format.asprintf "is reachable by %a" pp_print_kind_module_pt mdl
        ...
        (fun ppf ->
          ...
          let pp_print_weak_assumptions color title assumps ppf =
            Format.fprintf ppf "@{<%s>%s weak assumptions:@}@,%a@," color title
              (Format.pp_print_list ~pp_sep (fun ppf (id, _) ->
                   Format.fprintf ppf "@{<blue_b>%s@}" id))
              assumps
          in
          ...
          | [], [] -> ()
          | [], _ -> Format.fprintf ppf "%t@," pp_print_unsatisfied_wa
          | _, [] -> Format.fprintf ppf "%t@," pp_print_satisfied_wa
          | _, _ ->
              Format.fprintf ppf "%t@,%t@," pp_print_satisfied_wa
                pp_print_unsatisfied_wa)
        (function
          | ppf ->
          if print then
            Format.fprintf ppf "%a@."
              (pp_print_trace_pt ~title ~color dump level input_sys analysis
                 trans_sys (Some prop) disproved)
              cex);
      ...
      if Simplify.has_division_by_zero_happened () then
        ...
        |> printf_pt L_warn "%t @[<v> %a@]" warning_tag
             (pp_print_list Format.pp_print_string "@,"))
  else Debug.event "Status of property %s already known" prop

(* Output statistics section as plain text *)
let stat_pt mdl level stats =
  (ignore_or_fprintf level) !log_ppf "@[<v>@{<b>Statistics for %a@}@,@,%a@]@."
    ...
    (pp_print_list
       (function
         | ppf -> (
             function
             | section, items ->
                 Format.fprintf ppf "[%s]@,%a@," section Stat.pp_print_stats
                   items))
       "@,")
    stats
...
let prop_status_pt level prop_status_kind =
  (ignore_or_fprintf level) !log_ppf
    "@[<v>%a@{<b>Summary of properties@}:@,%a%a@,%a@]@." Pretty.print_line ()
    Pretty.print_line ()
    (pp_print_list
       (fun ppf (p, s, k) ->
         Format.fprintf ppf "@[<h>@{<blue_b>%s@}: %a@]"
           ...
           (function
             | ppf -> (
                 ...
                 | Property.PropKTrue n, Property.Reachable _ ->
                     Format.fprintf ppf "@{<yellow>unreachable up to %d steps@}"
                       n
                 ...
                 | ( Property.PropInvariant (n, _),
                     Property.Reachable (Some (From ts)) ) ->
                     Format.fprintf ppf
                       "@{<red_b>unreachable in %d steps or more (k=%d)@}" ts n
                 | ( Property.PropInvariant (n, _),
                     Property.Reachable (Some (Within ts)) ) ->
                     Format.fprintf ppf
                       "@{<red_b>unreachable in %d steps or less (k=%d)@}" ts n
                 | ( Property.PropInvariant (n, _),
                     Property.Reachable (Some (At _)) ) ->
                     Format.fprintf ppf "@{<red_b>unreachable at step %d@}" n
                 | ( Property.PropInvariant (n, _),
                     Property.Reachable (Some (FromWithin (ts1, ts2))) ) ->
                     Format.fprintf ppf
                       "@{<red_b>unreachable between steps %d and %d (k=%d)@}"
                       ts1 ts2 n
           ...
           (s, k))
       "@,")
    prop_status_kind Pretty.print_double_line ()
...
let prop_attributes_xml trans_sys prop_name =
  ...
  let rec get_attributes = function
    | Property.PropAnnot pos ->
        ...
        Format.asprintf " line=\"%d\" column=\"%d\" source=\"PropAnnot\"%a" lnum
          cnum pp_print_fname fname
    ...
    | Property.Assumption (pos, (scope, _)) ->
        let fname, lnum, cnum = file_row_col_of_pos pos in
        Format.asprintf
          " line=\"%d\" column=\"%d\" scope=\"%s\" source=\"Assumption\"%a" lnum
          cnum (String.concat "." scope) pp_print_fname fname
    | Property.Guarantee (pos, scope) ->
        let fname, lnum, cnum = file_row_col_of_pos pos in
        Format.asprintf
          " line=\"%d\" column=\"%d\" scope=\"%s\" source=\"Guarantee\"%a" lnum
          cnum (String.concat "." scope) pp_print_fname fname
    | Property.GuaranteeOneModeActive (pos, scope) ->
        let fname, lnum, cnum = file_row_col_of_pos pos in
        Format.asprintf
          " line=\"%d\" column=\"%d\" scope=\"%s\" source=\"OneModeActive\"%a"
          lnum cnum (String.concat "." scope) pp_print_fname fname
    | Property.GuaranteeModeImplication (pos, scope) ->
        let fname, lnum, cnum = file_row_col_of_pos pos in
        Format.asprintf
          " line=\"%d\" column=\"%d\" scope=\"%s\" source=\"Ensure\"%a" lnum
          cnum (String.concat "." scope) pp_print_fname fname
    | Property.NonVacuityCheck (pos, scope) ->
        let fname, lnum, cnum = file_row_col_of_pos pos in
        Format.asprintf
          " line=\"%d\" column=\"%d\" scope=\"%s\" source=\"NonVacuityCheck\"%a"
          lnum cnum (String.concat "." scope) pp_print_fname fname
  in
...
let proved_xml mdl level trans_sys k prop_name =
  let prop = TransSys.property_of_name trans_sys prop_name in
  (* Only ouptut if status was unknown *)
  if not (Property.prop_status_known (Property.get_prop_status prop)) then
    let comment =
  ...
  if not (Property.prop_status_known (Property.get_prop_status prop)) then
    (ignore_or_fprintf level) !log_ppf
      "@[<hv 2><Property name=\"%s\"%s>@,\
       ...
       <Answer source=\"%a\">unknown</Answer>@;\
       <0 -2></Property>@]@."
      (Lib.escape_xml_string (LGR.get_node_type_and_name prop_name |> snd))
      (prop_attributes_xml trans_sys prop_name)
      (Stat.get_float Stat.analysis_time)
...
(* Pretty-print a trace *)
let pp_print_trace_xml ?(tag = "CounterExample") input_sys analysis trans_sys
    prop_name disproved ppf = function
  | [] -> ()
  | trace -> (
      (* Slice trace and transitions system to property *)
      let trans_sys', _, trace', input_sys' =
      ...
      try
        (* Output trace *)
        Format.fprintf ppf "@[<hv 2> <%s>%a@]@,</%s>" tag
...
let execution_path_xml level input_sys trans_sys path =
  (ignore_or_fprintf level) !log_ppf
    "@[<hv 2><Execution>@,%a@;<0 -2></Execution>@]@."
    (pp_print_path_xml input_sys trans_sys)
    path

(* Output disproved property as XML *)
let cex_xml ?(wa_model = []) mdl level input_sys analysis trans_sys prop_name
    (cex : (StateVar.t * Model.value list) list) disproved =
  let prop = TransSys.property_of_name trans_sys prop_name in
  (* Only ouptut if status was unknown *)
  if not (Property.prop_status_known (Property.get_prop_status prop)) then (
    (* Reset division by zero indicator. *)
      ...
      (function
        | ppf -> (
            ...
            | _ ->
                let pp_sep ppf () = Format.fprintf ppf "@," in
                Format.fprintf ppf
                  "@[<hv 2><WeakAssumptions>@,%a@]@,</WeakAssumptions>@,"
                  (Format.pp_print_list ~pp_sep (fun ppf (id, vl) ->
                       Format.fprintf ppf
                         "<WeakAssumption name=\"%s\" satisfied=\"%b\" />" id vl))
                  wa_model))
      (pp_print_trace_xml ~tag input_sys analysis trans_sys (Some prop_name)
         disproved)
      cex;
         ...
         | ppf -> (
             ...
             | section, items ->
                 Format.fprintf ppf
                   ...
                   section Stat.pp_print_stats_xml items))
       "@,")
    stats
...
let progress_xml mdl level k =
  (ignore_or_fprintf level) !log_ppf
    ...
    pp_print_kind_module_xml_src mdl k

(* Pretty-print a list of properties and their status *)
let prop_status_xml level trans_sys prop_status_kind =
  (* Filter unknown properties. *)
  prop_status_kind
  |> List.filter (fun (_, status, _) -> not (Property.prop_status_known status))
  |> (ignore_or_fprintf level) !log_ppf "@[<v>%a@]@."
       (pp_print_list
          (fun ppf (p, s, k) ->
            Format.fprintf ppf
              "@[<hv 2><Property name=\"%s\"%s>@,\
              ...
              (function
                | ppf -> (
                    function
                    | Property.PropUnknown | Property.PropKTrue _ ->
                        Format.fprintf ppf "unknown"
                    | Property.PropInvariant _ when k = Property.Invariant ->
                        Format.fprintf ppf "valid"
                    | Property.PropInvariant _ ->
                        Format.fprintf ppf "unreachable"
                    | (Property.PropFalse [] | Property.PropFalse _)
                      when k = Property.Invariant ->
                        Format.fprintf ppf "falsifiable"
                    | Property.PropFalse _ -> Format.fprintf ppf "reachable"))
              s
              (function
                | ppf -> (
                    function
                    | Property.PropUnknown | Property.PropInvariant _
                    | Property.PropFalse [] ->
                        ()
                    | Property.PropKTrue n when k = Property.Invariant ->
                        Format.fprintf ppf
                          "@,@[<hv 2><TrueFor>@,%d@;<0 -2></TrueFor>@]" n
...
let pp_print_list_attrib pp ppf = function
  | [] -> Format.fprintf ppf " []"
  | lst -> Format.fprintf ppf "@,[@[<v 1>@,%a@]@,]" (pp_print_list pp ",@,") lst
...
let prop_attributes_json ppf trans_sys prop_name =
  ...
  let rec get_attributes = function
    ...
    | Property.NonVacuityCheck (pos, scope) ->
        print_attributes pos scope "NonVacuityCheck"
    | Property.Generated (pos, _) -> (
        match pos with
  ...
  then
    ...
    (ignore_or_fprintf level) !log_ppf
      ",@.{@[<v 1>@,\
       \"objectType\" : \"property\",@,\
       \"name\" : \"%s\",@,\
       %t\"runtime\" : {\"unit\" : \"sec\", \"timeout\" : false, \"value\" : \
       %.3f},@,\
       %t\"answer\" : {\"source\" : \"%s\", \"value\" : \"%s\"}@]@.}@."
  ...
  then
    (ignore_or_fprintf level) !log_ppf
      ",@.{@[<v 1>@,\
       \"objectType\" : \"property\",@,\
       \"name\" : \"%s\",@,\
       %t\"runtime\" : {\"unit\" : \"sec\", \"timeout\" : true, \"value\" : \
       %.3f},@,\
       \"answer\" : {\"source\" : \"%s\", \"value\" : \"unknown\"}@]@.}@."
      (Lib.escape_json_string (LGR.get_node_type_and_name prop |> snd))
      (function ppf -> prop_attributes_json ppf trans_sys prop)
      (Stat.get_float Stat.analysis_time)
      (short_name_of_kind_module mdl)
...
let pp_print_trace_json ?(object_name = "counterExample") input_sys analysis
    trans_sys prop_name disproved ppf = function
  | [] -> ()
  | trace -> (
      (* Slice trace and transitions system to property *)
      let trans_sys', _, trace', input_sys' =
      ...
      try
        (* Output trace *)
        Format.fprintf ppf "\"%s\" :%a" object_name
...
let cex_json ?(wa_model = []) mdl level input_sys analysis trans_sys prop cex
    disproved =
  (* Only output if status was unknown *)
  if not (Property.prop_status_known (TransSys.get_prop_status trans_sys prop))
  then (
                ...
                Format.fprintf ppf
                  ...
                  (Format.pp_print_list ~pp_sep (fun ppf (id, vl) ->
                       Format.fprintf ppf
                         ...
                         id vl))
                  wa_model))
      (pp_print_trace_json ~object_name input_sys analysis trans_sys
         (Some (LGR.get_node_type_and_name prop |> snd))
         disproved)
      cex;
         ...
         (fun ppf (p, s, k) ->
           Format.fprintf ppf
             ...
             (function
               ...
               | Property.PropFalse cex when k = Property.Invariant ->
                   Format.fprintf ppf "\"falseAt\" : %d,@,"
                     (Property.length_of_cex cex - 1)
               | Property.PropFalse cex ->
                   Format.fprintf ppf "\"reachableAt\" : %d,@,"
                     (Property.length_of_cex cex - 1)
               | _ -> ())))
         "@,")
      unknown_props
...
let stat_json mdl level stats =
  (ignore_or_fprintf level) !log_ppf
    ...
    (pp_print_list_attrib (fun ppf (section, items) ->
         Format.fprintf ppf "{@[<v 1>@,\"name\" : \"%s\",@,\"items\" :%a@]@,}"
...
let[@ocaml.warning "-27"] log (mdl : kind_module) (lvl : log_level)
    (msg : string) =
  try
...
module ELog = Log.Make (struct
  let printf_relay = printf_relay
end)
...
(* Log a message with source and log level *)
let log_cex ?(wa_model = []) disproved mdl level input_sys analysis trans_sys
    prop cex =
  match get_log_format () with
  | F_pt ->
      cex_pt ~wa_model mdl level input_sys analysis trans_sys prop cex disproved
  | F_xml ->
      cex_xml ~wa_model mdl level input_sys analysis trans_sys prop cex
        disproved
  | F_json ->
      cex_json ~wa_model mdl level input_sys analysis trans_sys prop cex
        disproved
  | F_relay -> ()
...
let log_contractck_analysis_start scope =
  let node_type, node_name =
    LGR.get_node_type_and_name (Scope.to_string scope)
  in
  if Flags.log_level () <> L_off then
    match get_log_format () with
    | F_pt ->
        Format.fprintf !log_ppf "@.%a@{<b>Checking@} %s @{<blue>%s@}@.@."
    ...
    | F_xml ->
        ...
        in
        Format.fprintf !log_ppf
          "@.@.<AnalysisStart top=\"%s\" context=\"%s\" />@.@." node_name
          (match node_type with
  ...
  if Flags.log_level () <> L_off then
    ...
    | F_pt ->
        Format.fprintf !log_ppf
          ...
          Pretty.print_double_line () Scope.pp_print_scope sc
          (Analysis.pp_print_param false)
          param
    | F_xml ->
        ...
        Format.fprintf !log_ppf
          "@.@.<AnalysisStart top=\"%a\" concrete=\"%a\" abstract=\"%a\" \
           assumptions=\"%a\"/>@.@."
    ...
    | F_json ->
        ...
        Format.fprintf !log_ppf
          ",@.{@[<v 1>@,\
           ...
           \"assumptions\" :%a@]@.}@."
          Scope.pp_print_scope sc
          (pp_print_list_attrib Scope.pp_print_scope)
          concrete
          (pp_print_list_attrib Scope.pp_print_scope)
          abstract
...
(** Logs the end of an analysis. [log_analysis_start result] logs the end of an
    analysis. *)
...
let log_post_analysis_start name title =
  ...
  | F_xml -> Format.fprintf !log_ppf "<PostAnalysisStart name=\"%s\"/>@.@." name
  | F_json ->
      Format.fprintf !log_ppf
        ",@.{@[<v 1>@,\
...
let log_post_analysis_end () =
  match get_log_format () with
  | F_pt -> Format.fprintf !log_ppf "%a@." Pretty.print_line ()
  | F_xml -> Format.fprintf !log_ppf "<PostAnalysisEnd/>@.@."
  | F_json ->
...
let log_timeout b =
  ...
  | F_xml -> log L_fatal "%s timeout." pref
  | F_json -> log L_fatal "%s timeout." pref
  | F_relay -> failwith "can only be called by supervisor"
...
let log_interruption signal =
  ...
  | F_xml -> log L_fatal "%s" txt
  | F_json -> log L_fatal "%s" txt
  | F_relay -> failwith "can only be called by supervisor"
...
let progress k =
  ...
  try
    ...
    EventMessaging.send_output_message (EventMessaging.Progress k)
    (* Don't fail if not initialized *)
  with Messaging.NotInitialized -> ()
...
let terminate () =
  try
    ...
    minisleep 0.1 (* Don't fail if not initialized *)
  with Messaging.NotInitialized -> ()
      ...
      (List.fold_left
         (function
           | accum -> (
               ...
               (* Output log message *)
               | _, EventMessaging.OutputMessage (EventMessaging.Log (lvl, msg))
                 ->
...
let update_child_processes_list new_process_list =
  try EventMessaging.update_child_processes_list new_process_list
  with Messaging.NotInitialized -> ()

let purge_im : messaging_setup -> unit =
 fun (ctx, _) ->
  try EventMessaging.purge_im_mailbox ctx with Messaging.NotInitialized -> ()

(* Terminates if a termination message was received. Does NOT modified
   received messages. *)
let check_termination () =
  if EventMessaging.check_termination () then raise Terminate else ()
  ...
  let rec update_trans_sys' trans_sys invars prop_status = function
    ...
    | (m, PropStatus (p, (Property.PropKTrue k as s))) :: tl -> (
        try
          ...
          update_trans_sys' trans_sys invars ((m, (p, s)) :: prop_status) tl
    ...
    | (m, PropStatus (p, (Property.PropInvariant cert as s))) :: tl -> (
        try
          ...
          let term =
            TransSys.props_list_of_bound trans_sys Numeral.zero |> List.assoc p
          in
    ...
    | (m, StepCex (p, cex)) :: tl ->
        ...
        let cex =
          List.map (function (_, []) as c -> c | sv, _ :: vl -> (sv, vl)) cex
        in
...
let top_invariants_of_invariants sys invariants =
  ...
  try SMap.find top invariants with Not_found -> (TSet.empty, TSet.empty)
