...
let rec seq_chain :
 ...
 fun f e -> function
  | [] -> ok e
  | h :: t -> f e h >>= fun e' -> seq_chain f e' t
...
(** Feeds a result to a function returning a result, propagates if argument's an
    error. *)
let chain ?(fmt = identity) f = function
  | Ok arg -> f arg |> map_err fmt
  | Error err -> Error err

(** Fold over a list of results. *)
let l_fold ?(fmt = identity) f init =
...
let l_iter ?(fmt = identity) f =
  let rec loop = function
    | head :: tail -> (
        match f head with Ok () -> loop tail | Error err -> Error (fmt err))
    | [] -> Ok ()
  in
  loop
