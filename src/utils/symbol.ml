...
type interpreted_symbol =
  ...
  | `UINT8_TO_INT
    (* Conversion from an unsigned integer 8 numeral to an integer *)
  | `UINT16_TO_INT
    (* Conversion from an unsigned integer 16 numeral to an integer *)
  | `UINT32_TO_INT
    (* Conversion from an unsigned integer 32 numeral to an integer *)
  | `UINT64_TO_INT
    (* Conversion from an unsigned integer 64 numeral to an integer *)
  | `INT8_TO_INT (* Conversion from a signed integer 8 numeral to an integer *)
  | `INT16_TO_INT
    (* Conversion from a signed integer 16 numeral to an integer *)
  | `INT32_TO_INT
    (* Conversion from a signed integer 32 numeral to an integer *)
  | `INT64_TO_INT
    (* Conversion from a signed integer 64 numeral to an integer *)
  ...
  | `BVUGE
    (* Arithmetic comparision greater than or equal to (unsigned binary) *)
  ...
  | `BVZEROEXT of Numeral.t (* Extend bitvector with zeros (unary) *)
...
module Symbol_node = struct
  ...
  let equal s1 s2 =
    ...
    | `STORE, `STORE -> true
    | `BVEXTRACT (i1, j1), `BVEXTRACT (i2, j2) ->
        Numeral.equal i1 i2 && Numeral.equal j1 j2
...
and pp_print_symbol ppf { Hashcons.node = n } = pp_print_symbol_node ppf n
...
let is_numeral = function { Hashcons.node = `NUMERAL _ } -> true | _ -> false

(* Return true if the symbol is a decimal *)
let is_decimal = function { Hashcons.node = `DECIMAL _ } -> true | _ -> false

(* Return true if the symbol is a bitvector *)
let is_bitvector = function { Hashcons.node = `BV _ } -> true | _ -> false

(* Return true if the symbol is an unsigned bitvector *)
let is_ubitvector = function { Hashcons.node = `UBV _ } -> true | _ -> false
...
let is_to_uint8 = function { Hashcons.node = `TO_UINT8 } -> true | _ -> false
...
let is_to_int8 = function { Hashcons.node = `TO_INT8 } -> true | _ -> false

(* Return true if the symbol is a to_int16 *)
let is_to_int16 = function { Hashcons.node = `TO_INT16 } -> true | _ -> false

(* Return true if the symbol is a to_int32 *)
let is_to_int32 = function { Hashcons.node = `TO_INT32 } -> true | _ -> false

(* Return true if the symbol is a to_int64 *)
let is_to_int64 = function { Hashcons.node = `TO_INT64 } -> true | _ -> false

(* Return true if the symbol is [`TRUE] or [`FALSE] *)
let is_bool = function
  | { Hashcons.node = `TRUE } | { Hashcons.node = `FALSE } -> true
  | _ -> false
...
let is_uf = function { Hashcons.node = `UF _ } -> true | _ -> false

(* Return true if the symbol is [`ITE] *)
let is_ite = function { Hashcons.node = `ITE } -> true | _ -> false
...
let mk_symbol sym = Hsymbol.hashcons ht sym ()
