...
(** Kind's symbols
    ...
    The term representation is modeled after the SMTLIB 2.0 standard and the
    symbols are a subset of the symbols defined in the SMTLIB theories
    {{:http://smtlib.cs.uiowa.edu/theories/Core.smt2} Core} and
    {{:http://smtlib.cs.uiowa.edu/theories/Reals_Ints.smt2} Reals_Ints} with the
    addition of uninterpreted symbols.

    Most symbols are variadic and associativity is to be understood as defined
    in the SMTLIB standard:
    ...
    A chainable symbol is to be read as the conjunction of successive pairs, for
    example [(<= 1 2 3)] is equivalent to [(and (<= 1 2) (<= 2 3))]. A pairwise
    symbol is to be read as the conjunction of each pair of arguments, for
    example [(distinct a b c)] is
    ...
    comparison, however, as of now there are no useful properties to be stored
...
type interpreted_symbol =
  ...
  | `INT8_TO_INT
    (** Conversion from a signed integer 8 numeral to an integer *)
  | `INT16_TO_INT
...
(** Adding uninterpreted function symbols separately for conversions from
    expressions in the SMT solver interface *)
