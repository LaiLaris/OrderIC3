...
let list_init f n =
  if n = 0 then []
    ...
    let rec init_aux i =
      if i = n - 1 then [ f i ] else f i :: init_aux (i + 1)
    in
    init_aux 0
...
let rec list_indexes' accum pos l = function
  | [] -> List.rev accum
  | h :: tl ->
      if List.mem h l then list_indexes' (pos :: accum) (succ pos) l tl
      else list_indexes' accum (succ pos) l tl

let list_indexes l1 l2 = list_indexes' [] 0 l1 l2

(* [list_filter_nth l [p1; p2; ...]] returns the elements [l] at positions [p1], [p2] etc. *)
let rec list_filter_nth' current_pos accum = function
  ...
  | h :: list_tl -> (
      ...
      | next_pos :: positions_tl when current_pos = next_pos -> (
          ...
          | _ ->
              list_filter_nth' (succ current_pos) (h :: accum) list_tl
                positions_tl)
      | positions -> list_filter_nth' (succ current_pos) accum list_tl positions
      )
...
let rec list_remove_nth n = function
  | [] -> []
  | h :: tl -> if n = 0 then tl else h :: list_remove_nth (n - 1) tl

let rec list_insert_at x n = function
  | [] -> [ x ]
  | h :: tl as l -> if n = 0 then x :: l else h :: list_insert_at x (n - 1) tl

let rec list_apply_at f n = function
  | [] -> []
  | h :: tl -> if n = 0 then f h :: tl else h :: list_apply_at f (n - 1) tl
...
let list_split k l =
  let l1, l2 = fold_until (fun acc h -> h :: acc) [] k l in
  (List.rev l1, l2)
...
(* Return a list containing all values in the first list that are not
    in the second list *)
let list_subtract l1 l2 = List.filter (function e -> not (List.mem e l2)) l1
...
let list_merge_uniq cmp l1 l2 =
  let rec list_merge_uniq cmp accum l1 l2 =
    ...
    | h1 :: tl1, h2 :: tl2 -> list_merge_uniq cmp (h2 :: h1 :: accum) tl1 tl2
  in
...
let list_inter_uniq cmp l1 l2 =
  let rec list_inter_uniq cmp accum l1 l2 =
    match (l1, l2) with
...
let list_diff_uniq cmp l1 l2 =
  let rec list_diff_uniq cmp accum l1 l2 =
    match (l1, l2) with
    ...
    (* First and second head elements are physically equal: remove
       both head elements *)
    | h1 :: tl1, h2 :: tl2 when h1 == h2 -> list_diff_uniq cmp accum tl1 tl2
    (* First head element is smaller than second: add first head
       element to accumulator *)
    | h1 :: tl, h2 :: _ when cmp h1 h2 < 0 ->
    ...
    (* First head element is greater than second: remove first head
       element from list *)
    | _ :: _, _ :: tl -> list_diff_uniq cmp accum l1 tl
  in
...
(* For two sorted lists without physical duplicates return true if the
   ...
   the second list *)
let rec list_subset_uniq cmp l1 l2 =
  match (l1, l2) with
...
let compare_pairs cmp_a cmp_b (a1, b1) (a2, b2) =
  let c_a = cmp_a a1 a2 in
  if c_a = 0 then cmp_b b1 b2 else c_a
...
let list_join equal l1 l2 =
  let rec list_join' equal accum l1 l2 =
    match (l1, l2) with
...
let rec list_apply : ('a -> 'b) list -> 'a -> 'b list =
 fun fs arg ->
  match fs with [] -> [] | f :: rest -> f arg :: list_apply rest arg

let rec find_map f = function
  | [] -> None
  | h :: tl -> ( match f h with None -> find_map f tl | v -> v)

let rec drop_last : 'a list -> 'a list = function
  | [] -> failwith "drop_last"
  | [ _ ] -> []
  | e :: r -> e :: drop_last r
...
module IntegerHashtbl = Hashtbl.Make (struct
  type t = int
...
let pp_print_arrayi pp sep ppf array =
  ...
  let print_element i =
    if i = n - 1 then pp ppf i array.(i)
    else (
      pp ppf i array.(i);
      fprintf ppf sep)
  in
...
let pp_print_if_not_empty s ppf = function [] -> () | _ -> fprintf ppf s
...
let paren_string_of_string_list list = string_of_t pp_print_paren_list list

(* Return the width of the string, meaning the wisth of it's longest line *)
let width_of_string s =
  let lines = Str.split (Str.regexp "\n") s in
  List.fold_left (fun max_width s -> max max_width (String.length s)) 0 lines
...
let log_to_file f =
  ...
  let oc =
    try open_out f with Sys_error _ -> failwith "Could not open logfile"
  in
...
let pp_print_kind_module ppf = function
  ...
  | `IC3QEl1l2 -> fprintf ppf "property directed reachability (QE) sorting clause l1l2"
  | `IC3QEl2l1 -> fprintf ppf "property directed reachability (QE) sorting clause l2l1"
...
let pp_print_process_status ppf = function
  ...
  | Unix.WSIGNALED s -> fprintf ppf "killed by signal %a" pp_print_signal s
  | Unix.WSTOPPED s -> fprintf ppf "stopped by signal %a" pp_print_signal s
...
let equal_pos { pos_fname = p1; pos_lnum = l1; pos_cnum = c1 }
    { pos_fname = p2; pos_lnum = l2; pos_cnum = c2 } =
  l1 = l2 && c1 = c2 && String.equal p1 p2

(* Comparision on positions *)
let compare_pos { pos_fname = p1; pos_lnum = l1; pos_cnum = c1 }
    { pos_fname = p2; pos_lnum = l2; pos_cnum = c2 } =
...
let pp_print_position ppf ({ pos_fname; pos_lnum; pos_cnum } as pos) =
  if pos = dummy_pos then fprintf ppf "(unknown)"
  else if pos_lnum = 0 && pos_cnum = -1 then fprintf ppf "%s" pos_fname
  else
    let fname = if pos_fname = "" then !stdin_id else pos_fname in
...
let position_of_lexing
  ...
  { pos_fname; pos_lnum; pos_cnum = pos_cnum - pos_bol + 1 }

(* Return true if position is a dummy position *)
let is_dummy_pos = function { pos_cnum = -1 } -> true | _ -> false
  ...
  let gc_c =
    {
      ...
      (* default 124000*)
      space_overhead = 100;
      (* default 80% des donnes vivantes *)
    }
  in
  Gc.set gc_c

let pp_print_bound_opt ppf bound =
  match bound with
