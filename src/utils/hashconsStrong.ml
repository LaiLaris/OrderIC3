...
type ('a, 'b) hash_consed = { hkey : int; tag : int; node : 'a; prop : 'b }
...
and add t d =
  ...
  if i >= Array.length bucket then (
    ...
    if newsz <= sz then failwith "Hashcons.add: hash bucket cannot grow more";
...
let hashcons t d p =
  ...
  let rec loop i =
    ...
    if i >= l then (
      ...
      let hnode = { hkey; tag = gentag (); node = d; prop = p } in
