...
module type S = sig
  ...
  (** Gets a subgraph along with appropriate edges of given graph from a given
      set of vertices *)
  ...
  (** Computes a topological ordering of vertices * or throws an
      [CyclicGraphException] if the graph is cyclic. * Implimentation is of this
      function is based on Kahn's algorithm *)
  ...
  (** Finds all the [vertices] that are rechable from the given [vertex] in a
      graph *)
...
module Make (Ord : OrderedType) = struct
  ...
  let pp_print_vertex : Format.formatter -> vertex -> unit =
   fun ppf v -> Ord.pp_print_t ppf v

  type edge = vertex * vertex
  (** directed edge type is between two vertices source and target *)

  let pp_print_edge : Format.formatter -> edge -> unit =
   fun ppf (s, t) ->
  ...
  (** Checks if the [vertex] is the source [vertex] *)
  let is_vertex_source : edge -> vertex -> bool =
   fun e v -> Ord.compare v (get_source_vertex e) = 0

  (** Checks if the [vertex] is the source [vertex] *)
  let is_vertex_target : edge -> vertex -> bool =
   fun e v -> Ord.compare v (get_target_vertex e) = 0

  let is_vertex_in_edge : edge -> vertex -> bool =
   fun (s, t) v -> Ord.compare v s = 0 || Ord.compare v t = 0
  ...
  let pp_print_vertices : Format.formatter -> vertices -> unit =
   fun ppf vs -> Lib.pp_print_list pp_print_vertex ", " ppf (VSet.elements vs)

  type edges = ESet.t
  (** type alias for set of edges *)

  let pp_print_edges : Format.formatter -> edges -> unit =
   fun ppf es -> Lib.pp_print_list pp_print_edge ", " ppf (ESet.elements es)

  type t = vertices * edges
  (** A graph is a set of vertices and set of edges *)

  let pp_print_graph : Format.formatter -> t -> unit =
   fun ppf (vs, es) ->
    Format.fprintf ppf "Vertices: %a\nEdges: %a" pp_print_vertices vs
      pp_print_edges es
  ...
  (** Check if the graph is empty *)
  let is_empty : t -> bool = fun (vs, _) -> VSet.is_empty vs

  (** returns a singleton graph *)
  let singleton : vertex -> t = fun v -> (VSet.singleton v, ESet.empty)

  let is_singleton : t -> bool =
   fun (vs, es) -> VSet.cardinal vs = 1 && ESet.cardinal es = 0

  (** Make an edge from two vertices *)
  let mk_edge : vertex -> vertex -> edge = fun v1 v2 -> (v1, v2)

  (** add a vertex to a graph *)
  let add_vertex : t -> vertex -> t = fun (vs, es) v -> (VSet.add v vs, es)

  let mem_vertex : t -> vertex -> bool = fun (vs, _) v -> VSet.mem v vs

  (** add an edge to a graph *)
  let add_edge : t -> edge -> t =
   fun (vs, es) (src, tgt) ->
    if VSet.mem src vs && VSet.mem tgt vs then (vs, ESet.add (src, tgt) es)
  ...
  *)
  ...
  let remove_vertex : t -> vertex -> t =
   fun (vs, es) v ->
    (VSet.remove v vs, ESet.filter (fun e -> not (is_vertex_in_edge e v)) es)
  ...
  (** Remove an [edge] from a graph *)
  let remove_edge : t -> edge -> t = fun (vs, es) e -> (vs, ESet.remove e es)

  let remove_edges : t -> edges -> t = fun (vs, es) es' -> (vs, ESet.diff es es')

  (** Returns a list of all vertices that have no incoming edge *)
  let non_target_vertices : t -> vertices =
   fun (vs, es) ->
    VSet.filter
      (fun v -> ESet.for_all (fun e -> not (is_vertex_target e v)) es)
      vs
  ...
  let non_source_vertices : t -> vertices =
   fun (vs, es) ->
    VSet.filter
      (fun v -> ESet.for_all (fun e -> not (is_vertex_source e v)) es)
      vs

  (** Connect [vertex] that is already in the graph to all the other vertices in
      the graph *)
  let connect : t -> vertex -> t =
   fun g v ->
    let vs = get_vertices g in
    ( VSet.add v vs,
      VSet.fold
        (fun v' es' -> ESet.add (mk_edge v v') es')
        (get_vertices g) (get_edges g) )

  (** Gets a subgraph with appropriate edges of given graph from a given set of
      vertices *)
  let sub_graph : t -> vertices -> t =
   fun g vs ->
    ( vs,
      ESet.filter
        (fun (src, tgt) -> VSet.mem src vs && VSet.mem tgt vs)
        (get_edges g) )

  let children : t -> vertex -> vertex list =
   fun g v ->
    let edges = ESet.filter (fun e -> is_vertex_source e v) (get_edges g) in
    ESet.elements edges |> List.map (fun (_, t) -> t)

  (** Returns true if the graph has no edges *)
  let is_point_graph : t -> bool = fun (_, es) -> ESet.is_empty es

  (** Unions two graphs *)
  let union : t -> t -> t =
   fun (v1s, e1s) (v2s, e2s) -> (VSet.union v1s v2s, ESet.union e1s e2s)
  ...
  (** Maps the [vertices] using the argument mapping, the structure should
      ...
      this is an injective mapping to make sure that the graph structure is
      preserved. *)
  let map : (vertex -> vertex) -> t -> t =
   fun f (vs, es) ->
    let map_edge : (vertex -> vertex) -> edge -> edge =
     fun f (s, t) -> (f s, f t)
    in
  ...
  (** Computes a topological ordering of vertices * or throws an
      [CyclicGraphException] if the graph is cyclic. * Implementation is based
      on Kahn's algorithm * https://en.wikipedia.org/wiki/Topological_sorting *)
  let topological_sort : t -> vertex list =
   fun g ->
    let rec find_cycle ((_, edges) as g) current seen =
      if List.mem current seen then seen
      else
        let current_edges =
          ESet.filter (fun e -> is_vertex_source e current) edges
        in
     ...
     fun ((vs, _) as g) sorted_vs ->
      ...
      Debug.parse
        ...
        \ new non source vertices: %a\n\
         -------------"
        pp_print_graph g
        (Lib.pp_print_list pp_print_vertex ",")
        sorted_vs pp_print_vertices no_outgoing_vs;
      (* graph is empty case *)
      if VSet.is_empty no_outgoing_vs then
        ...
        let new_g =
          VSet.fold (fun v g' -> remove_vertex g' v) no_outgoing_vs g
        in
        topological_sort_helper new_g (sorted_vs @ VSet.elements no_outgoing_vs)
    in
    topological_sort_helper g []

  (** Returns all the vertices rechable from the input vertex in the graph using
  ...
  let memoized_reachable : vertices VMap.t ref -> t -> vertex -> vertices =
   fun memo ((vs, _) as g) origin_v ->
     ...
     fun acc sv ((_, es) as g) ->
      ...
      | None ->
          Debug.parse
            ...
            pp_print_graph g
            (Lib.pp_print_list pp_print_vertex ",")
            (VSet.elements acc) pp_print_vertex sv;

          let result =
            if VSet.mem sv acc then acc
              (* we have already visited this vertex so skip *)
            else
                   ...
                   (fun v ->
                     VSet.add v
                       (reachable_from_aux
                          ...
                          v (remove_edges g new_edgs)))
                   (VSet.elements new_vs))
          in
    ...
    if VSet.mem origin_v vs then (
      ...
      Debug.parse "cumulative reachable from %a are %a" pp_print_vertex origin_v
