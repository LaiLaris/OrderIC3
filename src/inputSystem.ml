...
type _ t =
  | Lustre :
      (LustreNode.t S.t list * LustreGlobals.t * LustreAst.declaration list)
      -> LustreNode.t t
...
let ordered_scopes_of (type s) : s t -> Scope.t list = function
  | Lustre (main_subs, _, _) ->
      S.all_subsystems_of_list main_subs |> List.map (fun { S.scope } -> scope)
  | Native subsystem ->
      S.all_subsystems subsystem |> List.map (fun { S.scope } -> scope)
  | Horn _ -> assert false
...
let analyzable_subsystems (type s) : s t -> s SubSystem.t list = function
  ...
  | Native subsystem ->
      let subsystems' =
        if Flags.modular () then S.all_subsystems subsystem else [ subsystem ]
      in
      subsystems'
      |> List.filter (fun s ->
             Strategy.is_candidate_for_analysis (S.strategy_info_of s))
...
let maximal_abstraction_for_testgen (type s) :
  ...
  | Lustre (main_subs, _, _) -> (
      fun top assumptions ->
        ...
        match S.all_subsystems_of_list main_subs |> get_abstraction_for_top with
...
let next_analysis_of_strategy (type s) : s t -> 'a -> Analysis.param option =
  function
  | Lustre (main_subs, _, _) ->
      fun results ->
        let scope_and_strategy =
          List.map (fun ({ S.scope } as sub) -> (scope, S.strategy_info_of sub))
        in
        let all_syss =
  ...
  | Native subsystem ->
      fun results ->
        let scope_and_strategy =
          List.map (fun ({ S.scope } as sub) -> (scope, S.strategy_info_of sub))
        in
        let all_syss = scope_and_strategy (S.all_subsystems subsystem) in
        if Flags.modular () then
          let subs_of_scope scope =
...
let mcs_params (type s) (input_system : s t) =
          ...
          (fun abs_map { S.scope; S.has_impl; S.has_contract } ->
            if Flags.Contracts.compositional () then
              Scope.Map.add scope
                ...
                abs_map
            else Scope.Map.add scope (not has_impl) abs_map)
  ...
  | Lustre (main_subs, _, _) ->
      let subs =
        if Flags.modular () then S.all_subsystems_of_list main_subs |> List.rev
        else main_subs
      in
      subs
  ...
  | Native sub ->
      let subs =
        if Flags.modular () then S.all_subsystems sub |> List.rev else [ sub ]
      in
      subs
      |> List.filter (fun { S.has_impl } -> has_impl)
      |> List.map param_for_subsystem
    ...
    | Lustre (main_subs, _, _) ->
        let ({ S.scope } as sub) =
          ...
          | _ ->
              let msg =
                Format.asprintf
                  "two or more top nodes detected, please select one with \
                   --lus_main or a single --%%MAIN annotation"
              in
              failwith msg
        in
        ( scope,
...
let retrieve_lustre_nodes_of_scope (type s) :
    s t -> Scope.t -> LustreNode.t list =
 (function
 | Lustre (main_subs, _, _) ->
     fun scope ->
...
let pp_print_subsystems_debug (type s) : Format.formatter -> s t -> unit =
 fun fmt in_sys ->
  ...
  List.iter
    (Format.fprintf fmt "%a@." LustreNode.pp_print_node_debug)
    lustre_nodes

let pp_print_state_var_instances_debug (type s) :
...
let trans_sys_of_analysis (type s) ?(preserve_sig = false)
    ...
    ?slice_to_prop : s t -> Analysis.param -> TransSys.t * s t = function
      ...
      | analysis ->
          let t, s =
            LustreTransSys.(
              ...
              in
              trans_sys_of_nodes ~options globals main_subs analysis)
          in
          (t, Lustre ([ s ], globals, ast)))
  | Native sub -> fun _ -> (sub.SubSystem.source, Native sub)
  | Horn _ -> assert false

let pp_print_path_pt (type s) (input_system : s t) trans_sys first_is_init ppf
    model =
  match input_system with
  | Lustre (main_subs, globals, _) ->
      ...
      in
      LustrePath.pp_print_path_pt trans_sys globals sub first_is_init ppf model
  | Native _ ->
      Format.eprintf "pp_print_path_pt not implemented for native input@.";
      () (* assert false *)
  | Horn _ -> assert false

let pp_print_path_xml (type s) (input_system : s t) trans_sys first_is_init ppf
    model =
  match input_system with
  | Lustre (main_subs, globals, _) ->
      let sub =
        ...
        S.find_subsystem_of_list main_subs scope
      in
      LustrePath.pp_print_path_xml trans_sys globals sub first_is_init ppf model
  | Native _ ->
      Format.eprintf "pp_print_path_xml not implemented for native input@.";
      assert false
  | Horn _ -> assert false

let pp_print_path_json (type s) (input_system : s t) trans_sys first_is_init ppf
    model =
  match input_system with
  | Lustre (main_subs, globals, _) ->
      let sub =
        ...
        S.find_subsystem_of_list main_subs scope
      in
      LustrePath.pp_print_path_json trans_sys globals sub first_is_init ppf
        model
  | Native _ ->
...
let pp_print_path_in_csv (type s) (input_system : s t) trans_sys first_is_init
    ppf model =
  match input_system with
  | Lustre (main_subs, globals, _) ->
      let sub =
        ...
        S.find_subsystem_of_list main_subs scope
      in
      LustrePath.pp_print_path_in_csv trans_sys globals sub first_is_init ppf
        model
  | Native _ ->
  ...
  SVM.fold
    (fun sv l acc ->
      ...
      let scope =
        List.fold_left
          (fun acc (lid, n, _) ->
            Format.asprintf "%a[%d]" (LustreIdent.pp_print_ident true) lid n
            :: acc)
          [] path
      in
      let var_name = StateVar.name_of_state_var sv' in
      let full_name = String.concat "." (List.rev (var_name :: scope)) in
...
let pp_print_term_as_expr (type s) (in_sys : s t) sys =
  let var_map =
    let aux_vars =
      let usr_name =
...
let slice_to_abstraction (type s) (input_sys : s t) analysis trans_sys : s t =
  let scope = TransSys.scope_of_trans_sys trans_sys in
  ...
  | Lustre (main_subs, globals, ast) ->
      let subsystem' =
        ...
        LustreSlicing.slice_to_abstraction (Flags.slice_nodes ()) analysis sub
      in
...
let slice_to_abstraction_and_property (type s) (input_sys : s t) analysis
    trans_sys cex prop :
  ...
  in
  *)
  let trans_sys', instances, cex', prop' = (trans_sys, [], cex, prop) in
  ...
  let analysis' =
    Analysis.First
      { (Analysis.info_of_param analysis) with Analysis.top = scope }
  in
  ...
  ( trans_sys',
    ...
    | Lustre (main_subs, globals, ast) ->
        let vars =
          ...
          | _ -> Term.state_vars_of_term prop'.Property.prop_term
        in
        ...
        let subsystem' =
          ...
          if is_prop'_instantiated then
            LustreSlicing.slice_to_abstraction (Flags.slice_nodes ()) analysis'
              sub
          else
            LustreSlicing.slice_to_abstraction_and_property analysis' vars sub
        in
...
let compile_to_rust (type s) : s t -> Scope.t -> string -> unit =
 fun sys top_scope target ->
  ...
  | Native _ -> Format.printf "can't compile from native input: unsupported"
  | Horn _ -> Format.printf "can't compile from horn clause input: unsupported"

let compile_oracle_to_rust (type s) :
    s t ->
    Scope.t ->
    string ->
 ...
 fun sys top_scope target ->
  ...
  | Lustre (main_subs, _, _) ->
      LustreToRust.oracle_to_rust target
        (fun scope -> (S.find_subsystem_of_list main_subs scope).S.source)
        (S.find_subsystem_of_list main_subs top_scope).S.source
  | Native _ -> failwith "can't compile from native input: unsupported"
  | Horn _ -> failwith "can't compile from horn clause input: unsupported"

let contract_gen_param (type s) :
    s t -> Scope.t -> Analysis.param * (Scope.t -> N.t) =
 fun sys ->
  fun top ->
   match sys with
   | Lustre (main_subs, _, _) -> (
       ...
       in
       match
         Strategy.next_monolithic_analysis (Analysis.mk_results ())
           [
...
let state_var_dependencies (type s) :
    s t -> StateVar.StateVarSet.t StateVar.StateVarMap.t Scope.Map.t = function
  | Lustre (subsystem, _, _) ->
      S.all_subsystems_of_list subsystem
      |> List.rev (* Process leaves first *)
      |> List.fold_left
           (fun (map, deps) { S.scope; S.source } ->
