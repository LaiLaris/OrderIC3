...
let last_result results scope =
  try Ok (Analysis.results_last scope results)
  with Not_found ->
    Res.error (fun fmt ->
...
module type PostAnalysis = sig
  ...
  val run :
    ...
    Analysis.results (* Can fail. *) ->
    unit res
  (** Performs the treatment. *)
end

(** Test generation. Generates tests for a system if system's was proved correct
    under the maximal abstraction. *)
module RunTestGen : PostAnalysis = struct
  ...
  let gen_param_and_check in_sys param sys =
    ...
    match param with
    | Analysis.Interpreter _ ->
        error (fun fmt ->
            Format.fprintf fmt
              "%t@ Test generation is not compatible with interpreter mode."
              (head sys))
    (* Contract check, node must be abstract. *)
    | Analysis.ContractCheck _ ->
        error (fun fmt ->
            Format.fprintf fmt
              "%t@ Cannot generate tests unless implementation is safe."
              (head sys))
    | Analysis.Refinement _ ->
        error (fun fmt ->
            Format.fprintf fmt
              "%t@ Component was not proven correct under its maximal \
               abstraction."
              (head sys))
    | Analysis.First _ -> (
        if TSys.all_props_proved sys |> not then
          error (fun fmt ->
              Format.fprintf fmt "%t@ Component has not been proved safe."
                (head sys))
        else
          match
            InputSystem.maximal_abstraction_for_testgen in_sys top
              Analysis.assumptions_empty
          with
          | None ->
              error (fun fmt ->
                  Format.fprintf fmt
  ...
  let run in_sys param _ results =
    ...
    |> Res.chain (fun { Analysis.sys } ->
           ...
           | [] -> (
               ...
               | [] ->
                   error (fun fmt ->
                       Format.fprintf fmt "%t@ Component has no contract."
                         (head sys))
               | _ ->
                   error (fun fmt ->
                       Format.fprintf fmt "%t@ Component has no modes."
                         (head sys)))
           | _ -> Ok sys)
    ...
    |> Res.chain (fun param ->
           ...
           let sys, input_sys_sliced =
             InputSystem.trans_sys_of_analysis ~preserve_sig:true in_sys param
           in
           ...
           try
             ...
             mk_dir tests_target;
             KEvent.log_uncond "%sGenerating tests for node '%a' to '%s'."
             ...
             in
             KEvent.log_uncond "%sGenerating glue xml file to `%s/.`."
               TestGen.log_prefix target;
             testgen_xmls
             |> List.map (fun xml -> Format.sprintf "%s/%s" Paths.testgen xml)
             |> TestGen.log_test_glue_file target name
                  (Paths.oracle, guarantees, modes)
                  Paths.implem;
             KEvent.log_uncond "%sDone with test generation." TestGen.log_prefix;
             Ok ()
           with e ->
             TestGen.on_exit "T_T";
             error (fun fmt ->
  ...
  let run in_sys param analyze results =
    ...
    |> Res.chain (fun { Analysis.sys } ->
           ...
           let valid_rch =
             valid
             |> List.filter (fun p ->
                    match p.Property.prop_kind with
                    | Reachable _ -> true
                    | _ -> false)
           in
           let invalid_inv =
           ...
           | [] -> (
               ...
               | [], _ ->
                   error (fun fmt ->
                       Format.fprintf fmt
                         "No invalid properties, assumption generation \
                          disabled.")
               | _, [] -> Ok sys
               | _ ->
                   error (fun fmt ->
                       Format.fprintf fmt
                         "Unreachable property detected, assumption generation \
                          disabled."))
           | _ ->
               error (fun fmt ->
                   Format.fprintf fmt
                     "Unknown property detected, assumption generation \
                      disabled."))
    |> Res.chain (fun sys ->
           try
           ...
           when generated assumption is added to transition system *)
             ...
             mk_dir output_dir;
             let response =
               let one_state = not (Flags.Contracts.two_state_assumption ()) in
               Assumption.generate_assumption ~one_state analyze in_sys param
                 sys
             in
             (match response with
             | Assumption.Unknown ->
             ...
             | Assumption.Success assump ->
                 ...
                 let path =
                   Filename.concat output_dir
                     (Format.sprintf "%s.lus" contract_name)
                 in
                 let scope = TSys.scope_of_trans_sys sys in
                 KEvent.log L_note "Dumping assumption to `%s`..." path;
                 Assumption.dump_contract_for_assumption in_sys scope assump
                   path (Names.contract_name top);
...
(** Contract generation. Generates contracts by running invariant generation
    techniques. *)
module RunContractGen : PostAnalysis = struct
  ...
  let run in_sys param analyze _ =
    let top = (Analysis.info_of_param param).Analysis.top in
    KEvent.log L_note
      "Contract generation is a very experimental feature:@ in particular, the \
       modes it generates might not be exhaustive,@ which means that Kind 2 \
       will consider the contract unsafe.@ This can be dealt with by adding a \
       wild card mode:@ mode wildcard () ;";
    (* Building transition system and slicing info. *)
    let sys, in_sys_sliced =
      ISys.trans_sys_of_analysis ~preserve_sig:true ~slice_nodes:false in_sys
        param
    in
    let target = Flags.subdir_for top in
    (* Create directories if they don't exist. *)
    Flags.output_dir () |> mk_dir;
    mk_dir target;
    let target = Format.asprintf "%s/%s" target Names.contract_gen_file in
    ...
    Some (Flags.Contracts.contract_gen_depth ()) |> Flags.Invgen.set_max_depth;
    (* Disable logging. *)
    ...
    |> Res.chain (fun teks ->
           ...
           |> KEvent.log_uncond
                "  @[<v>Discovering invariants by running@   @[<v>%a@]@ to \
                 depth %d...@]"
                (pp_print_list
                   (fun fmt -> Format.fprintf fmt "- %a" pp_print_kind_module)
                   "@ ")
                teks;
           try
             analyze true false false teks
               (* [
              `INVGEN ; `INVGENOS ;
    ...
    |> Res.chain (fun () ->
        ...
        Invs.fmt (TSys.get_invariants top) ; *)
           try
             LustreContractGen.generate_contracts in_sys_sliced param sys target
               (Names.contract_name top);
             Ok ()
           with e ->
             error (fun fmt ->
...
(** Rust generation. Compiles lustre as Rust. *)
module RunRustGen : PostAnalysis = struct
  ...
  let run in_sys param _ _ =
    KEvent.log L_note
      "Compilation to Rust is still a rather experimental feature:@ in \
       particular, arrays are not supported.";
    let top = (Analysis.info_of_param param).Analysis.top in
    let target = Flags.subdir_for top in
    ...
    KEvent.log_uncond "  Compiling node '%a' to Rust in '%s'."
...
(** Invariant print. Prints invariants used in the proof. *)
  ...
  let run in_sys param _ results =
    ...
    |> Res.chain (fun { Analysis.sys } ->
           ...
           try
             ...
             | _ ->
                 KEvent.log_uncond
                   "Minimization result: @[<v>all properties valid by \
                    %d-induction@ using %d invariant(s)@]"
                 ...
                 Ok ()
           with
           | CertifChecker.CouldNotProve err ->
               error (fun fmt ->
                   Format.fprintf fmt "Could not minimize invariants:@ %a"
                     (fun fmt () -> err fmt)
                     ())
           | e ->
               error (fun fmt ->
                   Format.fprintf fmt "Could not minimize invariants:@ %s"
                     (Printexc.to_string e)))
end
          ...
          Term.print_term trans ; Format.printf "\n"
             ...
             let props =
               ...
               |> List.filter (function
                    | { Property.prop_status = Property.PropInvariant _ } ->
                        true
                    | { Property.prop_name } ->
                        KEvent.log L_info "Skipping unproved property %s"
                          prop_name;
                        false)
             in
             let props =
               if Flags.IVC.ivc_per_property () then
                 List.map (fun x -> [ x ]) props
               else [ props ]
             in
             ...
             let treat_props props =
               if List.length props > 0 then
                 let use_umivc =
                   Flags.IVC.ivc_all () || Flags.IVC.ivc_smallest_first ()
                 in
                 let stop_after = if Flags.IVC.ivc_all () then 0 else 1 in
                 let k =
                   if Flags.IVC.ivc_smallest_first () then -1
                 ...
                 let treat_ivc is_must_set ivc =
                   ...
                   KEvent.log_with_tag L_warn Pretty.success_tag
                     (Format.asprintf "%s generated after %.3fs."
                        (if is_must_set && not (IvcMcs.is_ivc_approx ivc) then
                           "MUST set"
                         else if is_must_set then "Approximate MUST set"
                         else if not (IvcMcs.is_ivc_approx ivc) then
                           "Minimal IVC"
                         else "Approximate minimal IVC")
                        elapsed);

                   let pt = ModelElement.pp_print_core_data in_sys param sys in
                   let xml =
                     ModelElement.pp_print_core_data_xml in_sys param sys
                   in
                   let json fmt =
                     Format.fprintf fmt ",\n%a"
                       (ModelElement.pp_print_core_data_json in_sys param sys)
                   in

                   (if Flags.IVC.print_ivc () then
                      let filtered_ivc, _ =
                        IvcMcs.separate_ivc_by_category top ivc
                      in
                      let cpd =
                        IvcMcs.ivc_to_print_data in_sys sys
                          (if is_must_set then "must" else "ivc")
                          (Some elapsed) filtered_ivc
                      in
                      KEvent.log_result pt xml json cpd);

                   (if Flags.IVC.print_ivc_compl () then
                      let not_ivc = IvcMcs.complement_of_ivc in_sys sys ivc in
                      let filtered_not_ivc, _ =
                        IvcMcs.separate_ivc_by_category top not_ivc
                      in
                      let cpd =
                        IvcMcs.ivc_to_print_data in_sys sys
                          (if is_must_set then "must complement"
                           else "ivc complement")
                          (Some elapsed) filtered_not_ivc
                      in
                      KEvent.log_result pt xml json cpd);

                   if
                     Flags.IVC.minimize_program () <> `DO_NOT_MINIMIZE
                     && not is_must_set
                   then (
                     let minimized =
                       ...
                       |> IvcMcs.minimize_lustre_ast
                            ~valid_lustre:
                              (Flags.IVC.minimize_program () = `VALID_LUSTRE)
                            in_sys ivc
                     in
                     let dir =
                       match Flags.IVC.minimized_program_dir () with
                       | "" -> Flags.input_file () |> Filename.remove_extension
                       | str -> str
                     in
                     (try Unix.mkdir dir 0o755 with _ -> ());
                     let filename =
                       Filename.concat dir
                         (Format.asprintf "%a_%n.lus" pp_print_properties props
                            !nb)
                     in
                 ...
                 let use_must_set =
                   if Flags.IVC.ivc_must_set () then Some (treat_ivc true)
                   else if Flags.IVC.ivc_disable_must_opt () then None
                   else if Flags.IVC.ivc_all () then Some (fun _ -> ())
                   else None
                 in
                 ...
                 in
                 let ivc_approximate =
                   Flags.IVC.ivc_approximate ()
                   && not (Flags.IVC.ivc_must_set ())
                 in
                 let complete, res =
                   match (use_umivc, ivc_approximate) with
                   | false, true ->
                       treat_and_return_lst
                         (IvcMcs.ivc_uc in_sys ~approximate:false sys props)
                   | false, false ->
                       treat_and_return_lst
                         (IvcMcs.ivc_ucbf in_sys ~use_must_set param analyze sys
                            props)
                   | true, _ ->
                       IvcMcs.umivc in_sys ~use_must_set ~stop_after param
                         analyze sys props k (treat_ivc false)
                 in
                 if Flags.IVC.ivc_all () then (
                   KEvent.log_with_tag L_note Pretty.note_tag
                     (Format.asprintf "Number of minimal IVCs found: %n"
                        (List.length res));
                   KEvent.log_result
                     (fun fmt b ->
                       if not b then
                         Format.fprintf fmt
                           "This enumeration might be incomplete (some IVCs \
                            might be missing).")
                     (fun fmt ->
                       Format.fprintf fmt
                         "<ModelSetEnumeration isComplete=\"%b\" />\n")
                     (fun fmt ->
                       Format.fprintf fmt
                         ",\n\
                          {\"objectType\":  \"modelSetEnumeration\", \
                          \"isComplete\": %b}")
                     complete)
             in
           ...
           with e ->
             error (fun fmt ->
                 Format.fprintf fmt "An error occured:@ %s"
                   (Printexc.to_string e)))
end
...
let run_mcs_post_analysis in_sys param analyze sys =
  ...
  try
    ...
    let props =
      if Flags.MCS.mcs_per_property () then (
        let props =
          IvcMcs.mcs_initial_analysis in_sys param analyze ~max_mcs_cardinality
            sys
        in
        (* Print proved properties *)
        let pt = ModelElement.pp_print_no_solution in
        let xml = ModelElement.pp_print_no_solution_xml "mcs" in
        let json ~unknown fmt =
          Format.fprintf fmt ",\n%a"
            (ModelElement.pp_print_no_solution_json "mcs" ~unknown)
        in
        let aux prop =
          match prop.Property.prop_status with
          | PropInvariant _ ->
              if Flags.MCS.print_mcs_legacy () then
                IvcMcs.pp_print_no_mcs_legacy prop sys
              else
                KEvent.log_result (pt ~unknown:false) (xml ~unknown:false)
                  (json ~unknown:false) prop
          | PropFalse _ -> ()
          | _ ->
              (* Unknown *)
              if Flags.MCS.print_mcs_legacy () then
                ...
                KEvent.log_result (pt ~unknown:true) (xml ~unknown:true)
                  (json ~unknown:true) prop
        in
    ...
    let treat_props (props, initial_solution) =
      if List.length props > 0 then
        let treat_mcs mcs =
          ...
          KEvent.log_with_tag L_warn Pretty.success_tag
            (Format.asprintf "%sMinimal Cut Set generated after %.3fs."
               (if not (IvcMcs.is_mcs_approx mcs) then "" else "Approximate ")
               elapsed);
          ...
          if Flags.MCS.print_mcs_legacy () then
            ...
            (if Flags.MCS.print_mcs () then
               let filtered_mcs, _ =
                 IvcMcs.separate_mcs_by_category scope mcs
               in
               let cpd =
                 IvcMcs.mcs_to_print_data in_sys sys "mcs" (Some elapsed)
                   filtered_mcs
               in
               KEvent.log_result pt xml json cpd);

            if Flags.MCS.print_mcs_compl () then
              let filtered_mua, _ = IvcMcs.separate_mcs_by_category scope mua in
              let cpd =
                IvcMcs.mcs_to_print_data in_sys sys "mcs complement"
                  (Some elapsed) filtered_mua
              in
              KEvent.log_result pt xml json cpd
        in
        ...
        let complete, res =
          IvcMcs.mcs in_sys param analyze sys props ~initial_solution
            ~max_mcs_cardinality mcs_all approx treat_mcs
        in
        if mcs_all then (
          KEvent.log_with_tag L_note Pretty.note_tag
            (Format.asprintf "Number of MCS found%s: %n"
               (match props with
               | [ { prop_name } ] -> " for property " ^ prop_name
               | _ -> "")
               (List.length res));
          KEvent.log_result
            (fun fmt b ->
              if not b then
                Format.fprintf fmt
                  "This enumeration might be incomplete (some MCS might be \
                   missing).")
            (fun fmt ->
              Format.fprintf fmt "<ModelSetEnumeration isComplete=\"%b\" />\n")
            (fun fmt ->
              Format.fprintf fmt
                ",\n\
  ...
  with e ->
    error (fun fmt ->
        Format.fprintf fmt "An error occured:@ %s" (Printexc.to_string e))
...
let run i_sys top analyze results =
  let log_exn e =
    match e with
    | Failure msg -> KEvent.log L_fatal "Failure: %s" msg
    | _ ->
        KEvent.log L_fatal "Caught %s in post-analysis treatment."
          (Printexc.to_string e)
  in
  ...
  try
    ...
    |> List.iter (fun m ->
           ...
           if Module.is_active () then (
             ...
             (try
                (match
                   Module.run i_sys (Analysis.param_clone param) analyze results
                 with
                | Ok () -> ()
