...
type num = Number of Numeral.t | Inf | NegInf | Nothing

let compare_expr = Term.compare

(* Total order on expressions *)
let compare { expr_init = init1; expr_step = step1 }
...
let equal { expr_init = init1; expr_step = step1 }
    { expr_init = init2; expr_step = step2 } =
  Term.equal init1 init2 && Term.equal step1 step2

(* Hashing of expressions *)
let hash { expr_init; expr_step } =
  Hashtbl.hash (Term.hash expr_init, Term.hash expr_step)
...
let map f { expr_init; expr_step } =
  ...
  let f' init i term =
    ...
    let expr =
      { expr_init = term; expr_step = term; expr_type = Term.type_of_term term }
    in
...
module LustreExprHashtbl = Hashtbl.Make (struct
  type z = t
...
let rec pp_print_lustre_type safe ppf t =
  ...
  | Type.UBV i -> (
      ...
      | _ ->
          raise (Invalid_argument "pp_print_lustre_type: UBV size not allowed"))
  | Type.BV i -> (
      match i with
      ...
      | _ ->
          raise (Invalid_argument "pp_print_lustre_type: BV size not allowed"))
  | Type.Array (s, _) ->
...
let string_of_symbol = function
  ...
  | `UBV b -> (
      ...
      | 8 ->
          "(uint8 " ^ Numeral.string_of_numeral (Bitvector.ubv8_to_num b) ^ ")"
      | 16 ->
          "(uint16 "
          ^ Numeral.string_of_numeral (Bitvector.ubv16_to_num b)
          ^ ")"
      | 32 ->
          "(uint32 "
          ^ Numeral.string_of_numeral (Bitvector.ubv32_to_num b)
          ^ ")"
      | 64 ->
          "(uint64 "
          ^ Numeral.string_of_numeral (Bitvector.ubv64_to_num b)
          ^ ")"
  ...
  | `BV b -> (
      ...
      | 16 ->
          "(int16 " ^ Numeral.string_of_numeral (Bitvector.bv16_to_num b) ^ ")"
      | 32 ->
          "(int32 " ^ Numeral.string_of_numeral (Bitvector.bv32_to_num b) ^ ")"
      | 64 ->
          "(int64 " ^ Numeral.string_of_numeral (Bitvector.bv64_to_num b) ^ ")"
      | _ -> raise Type_mismatch)
  | `MINUS -> "-"
  | `PLUS -> "+"
  | `TIMES -> "*"
...
let pp_print_symbol_as_type as_type ppf s =
  ...
  | _ -> Format.pp_print_string ppf (string_of_symbol s)

(* Pretty-print a symbol as is *)
let pp_print_symbol ppf s = Format.pp_print_string ppf (string_of_symbol s)
...
let pp_print_lustre_var_typed safe ppf state_var =
  Format.fprintf ppf "%t%a: %a"
    (function
      | ppf -> if StateVar.is_const state_var then Format.fprintf ppf "const ")
    (pp_print_lustre_var safe) state_var
    (pp_print_lustre_type safe)
  ...
  if Var.is_state_var_instance var then
    ...
    let pp =
      ...
      if Numeral.equal (Var.offset_of_state_var_instance var) cur_offset then
        Format.fprintf ppf "@[<hv 2>%a@]" pvar (* Variable at previous offset *)
      else if Numeral.equal (Var.offset_of_state_var_instance var) pre_offset
      then Format.fprintf ppf "@[<hv 2>(pre %a)@]" pvar
      (* Fail on other offsets *)
        else function _ -> raise (Invalid_argument "pp_print_var")
    in

    (* Pretty-print state variable with or without pre *)
    pp state_var (* Variable is constant *)
  else if Var.is_const_state_var var then
    (* Get state variable of variable *)
    let state_var = Var.state_var_of_state_var_instance var in

    Format.fprintf ppf "@[<hv 2>%a@]" pvar state_var
...
(* Pretty-print a term *)
and pp_print_term_node ?as_type safe pvar ppf t =
      ...
      | Term.T.Forall l -> (
          ...
          | Term.T.L (_, t) ->
              Format.fprintf ppf "@[<hv 1>forall@ @[<hv 1>(...)@ %a@]@]"
                (pp_print_term_node ?as_type safe pvar)
                t)
      | Term.T.Exists l -> (
          match Term.T.node_of_lambda l with
          | Term.T.L (_, t) ->
              Format.fprintf ppf "@[<hv 1>exists@ @[<hv 1>(...)@ %a@]@]"
                (pp_print_term_node ?as_type safe pvar)
                t)
...
and pp_print_app_left' safe pvar s ppf = function
  | h :: tl ->
      Format.fprintf ppf " %a@ %a%t" pp_print_symbol s
        (pp_print_term_node safe pvar) h (function ppf ->
          pp_print_app_left' safe pvar s ppf tl)
  | [] -> ()
...
and pp_print_app_left safe pvar s ppf = function
  ...
  | h :: tl ->
      Format.fprintf ppf "@[<hv 2>(%a%t)@]" (pp_print_term_node safe pvar) h
...
and pp_print_app_right' safe pvar s arity ppf = function
  ...
  | [ h ] ->
      ...
      let rec aux ppf = function
        | 0 -> ()
        | i -> Format.fprintf ppf "%t)@]" (function ppf -> aux ppf (pred i))
      in

      (* Print last argument and close all parentheses *)
      Format.fprintf ppf "%a%t" (pp_print_term_node safe pvar) h (function
          | ppf -> aux ppf arity)
  (* Second last or earlier argument *)
  | h :: tl ->
      (* Open parenthesis and print argument *)
      Format.fprintf ppf "@[<hv 2>(%a %a@ %t" (pp_print_term_node safe pvar)
        h pp_print_symbol s (function ppf ->
          pp_print_app_right' safe pvar s arity ppf tl)
...
and pp_print_app_chain safe pvar s ppf = function
  ...
  | [ l; r ] ->
      Format.fprintf ppf "@[<hv 2>(%a %a@ %a)@]"
        (pp_print_term_node safe pvar)
        l pp_print_symbol s
        (pp_print_term_node safe pvar)
        r
  (* Print function application of first pair, conjunction and continue *)
  | l :: r :: tl ->
      Format.fprintf ppf "@[<hv 2>(%a %a@ %a) and %a@]"
        (pp_print_term_node safe pvar)
        l pp_print_symbol s
        (pp_print_term_node safe pvar)
        r
        (pp_print_app_chain safe pvar s)
        (r :: tl)
...
and pp_print_app ?as_type safe pvar ppf = function
  (* Function application must have arguments, cannot have nullary
     symbols here *)
  | `TRUE | `FALSE | `NUMERAL _ | `DECIMAL _ | `UBV _ | `BV _ | `BV2NAT -> (
      function _ -> assert false)
  (* Unary symbols *)
  | ( `NOT | `BVNOT | `BVNEG | `TO_REAL | `TO_INT | `UINT8_TO_INT
    | `UINT16_TO_INT | `UINT32_TO_INT | `UINT64_TO_INT | `INT8_TO_INT
    | `INT16_TO_INT | `INT32_TO_INT | `INT64_TO_INT | `TO_UINT8 | `TO_UINT16
    | `TO_UINT32 | `TO_UINT64 | `TO_INT8 | `TO_INT16 | `TO_INT32 | `TO_INT64
    | `BVEXTRACT _ | `BVSIGNEXT _ | `BVZEROEXT _ | `ABS ) as s -> (
      function
      | [ a ] ->
          Format.fprintf ppf "@[<hv 2>(%a@ %a)@]" pp_print_symbol s
            (pp_print_term_node safe pvar)
            a
      | _ -> assert false)
  ...
  | `MINUS as s -> (
      ...
      | [] -> assert false
      | [ a ] ->
          Format.fprintf ppf "%a%a" pp_print_symbol s
            (pp_print_term_node safe pvar)
            a
      | _ as l -> pp_print_app_left safe pvar s ppf l)
  (* Binary left-associative symbols with two or more arguments *)
  | ( `AND | `OR | `XOR | `BVAND | `BVOR | `BVADD | `BVSUB | `BVMUL | `BVUDIV
    | `BVSDIV | `BVUREM | `BVSREM | `PLUS | `TIMES | `DIV | `INTDIV ) as s -> (
      function
      | [] | [ _ ] -> assert false
      | _ as l -> pp_print_app_left safe pvar s ppf l)
  ...
  | ( `EQ | `BVULT | `BVULE | `BVUGT | `BVUGE | `BVSLT | `BVSLE | `BVSGT
    | `BVSGE | `LEQ | `LT | `GEQ | `GT ) as s ->
      pp_print_app_chain safe pvar s ppf
  (* Binary non-associative symbols *)
  | (`BVSHL | `BVLSHR | `BVASHR) as s -> (
      function
      | [ a; b ] ->
          Format.fprintf ppf "@[<hv 2>(%a %a@ %a)@]"
            (pp_print_term_node safe pvar)
  ...
  | `DIVISIBLE n -> (
      ...
      | [ a ] ->
          ...
          pp_print_app safe pvar ppf `EQ
            [
              Term.T.mk_app (Symbol.mk_symbol `MOD)
  ...
  | `STORE -> (
      function
      | [ a; i; v ] ->
          Format.fprintf ppf "@[<hv 2>(%a with [%a] = %a)@]"
            (pp_print_term_node safe pvar)
            a
            (pp_print_term_node safe pvar)
            i
  ...
  | `UF sym as s when Symbol.is_select (Symbol.mk_symbol s) ->
      pp_print_app ?as_type safe pvar ppf
        (`SELECT (UfSymbol.res_type_of_uf_symbol sym))
  (* Unsupported functions symbols *)
  | `BVCONCAT | `DISTINCT | `IS_INT | `UF _ -> ( function _ -> assert false)
...
let pp_print_lustre_expr_pvar safe pvar ppf = function
  ...
  | { expr_init; expr_step; expr_type } when Term.equal expr_init expr_step ->
  ...
  | { expr_init; expr_step; expr_type } ->
      Format.fprintf ppf "@[<hv 1>(%a@ ->@ %a)@]"
        (pp_print_expr_pvar ~as_type:expr_type safe pvar)
        expr_init
        (pp_print_expr_pvar ~as_type:expr_type safe pvar)
        expr_step
...
let pp_print_term_as_expr_mvar ?as_type safe map ppf expr =
  pp_print_term_as_expr_pvar ?as_type safe
    (fun fmt sv ->
      Format.fprintf fmt "%s"
        (try StateVar.StateVarMap.find sv map
         with Not_found -> StateVar.name_of_state_var sv))
    ppf expr
...
let pp_print_lustre_expr safe ppf = function
  ...
  | { expr_init; expr_step; expr_type } ->
      Format.fprintf ppf "@[<hv 1>(%a@ ->@ %a)@]"
        (pp_print_expr ~as_type:expr_type safe)
        expr_init
        (pp_print_expr ~as_type:expr_type safe)
        expr_step
...
let pp_print_bound_or_fixed ppf = function
  ...
  | Unbound x ->
      Format.fprintf ppf "@[<hv 1>(Unbound %a)@]" (pp_print_expr true) x
...
let pre_base_var_of_state_var zero_offset state_var =
  Var.mk_state_var_instance state_var Numeral.(zero_offset |> pred)

(* Instance of state variable at instant zero *)
let base_var_of_state_var zero_offset state_var =
  Var.mk_state_var_instance state_var zero_offset
...
let base_term_of_state_var zero_offset state_var =
  base_var_of_state_var zero_offset state_var |> Term.mk_var

(* Term of instance of state variable at current instant *)
let cur_term_of_state_var zero_offset state_var =
  cur_var_of_state_var zero_offset state_var |> Term.mk_var

(* Term of instance of state variable at previous instant *)
let pre_term_of_state_var zero_offset state_var =
  pre_var_of_state_var zero_offset state_var |> Term.mk_var
...
(* Return the free variable of a variable *)
let var_of_expr { expr_init } =
  try Term.free_var_of_term expr_init (* Fail if any of the above fails *)
  with Invalid_argument _ -> raise (Invalid_argument "var_of_expr")
...
let base_state_vars_of_init_expr { expr_init } =
  Term.state_vars_at_offset_of_term base_offset
    (base_term_of_expr base_offset expr_init)

(* Return all state variables at the current instant in the initial
   state expression *)
let cur_state_vars_of_step_expr { expr_step } =
  Term.state_vars_at_offset_of_term cur_offset
...
let split_expr_list list =
  List.fold_left
    (fun (accum_init, accum_step) { expr_init; expr_step } ->
      ( (if expr_init == Term.t_true then accum_init else expr_init :: accum_init),
        if expr_step == Term.t_true then accum_step else expr_step :: accum_step
      ))
    ([], []) list
...
let mk_binary eval type_of expr1 expr2 =
  let res_type = type_of expr1.expr_type expr2.expr_type in
...
let mk_ternary eval type_of expr1 expr2 expr3 =
  let res_type = type_of expr1.expr_type expr2.expr_type expr3.expr_type in
...
let t_true =
  ...
  { expr_init = expr; expr_step = expr; expr_type = Type.t_bool }
...
let mk_constr c t =
  ...
  { expr_init = expr; expr_step = expr; expr_type = t }
...
let mk_uint8 d =
  ...
  { expr_init = expr; expr_step = expr; expr_type = Type.t_ubv 8 }

(* Unsigned integer16 constant *)
let mk_uint16 d =
  let expr = Term.mk_num d in

  { expr_init = expr; expr_step = expr; expr_type = Type.t_ubv 16 }

(* Unsigned integer32 constant *)
let mk_uint32 d =
  let expr = Term.mk_num d in

  { expr_init = expr; expr_step = expr; expr_type = Type.t_ubv 32 }

(* Unsigned integer64 constant *)
let mk_uint64 d =
  let expr = Term.mk_num d in

  { expr_init = expr; expr_step = expr; expr_type = Type.t_ubv 64 }

(* Integer8 constant *)
let mk_int8 d =
  let expr = Term.mk_num d in

  { expr_init = expr; expr_step = expr; expr_type = Type.t_bv 8 }

(* Integer16 constant *)
let mk_int16 d =
  let expr = Term.mk_num d in

  { expr_init = expr; expr_step = expr; expr_type = Type.t_bv 16 }

(* Integer32 constant *)
let mk_int32 d =
  let expr = Term.mk_num d in

  { expr_init = expr; expr_step = expr; expr_type = Type.t_bv 32 }

(* Integer64 constant *)
let mk_int64 d =
  let expr = Term.mk_num d in

  { expr_init = expr; expr_step = expr; expr_type = Type.t_bv 64 }

(* Real constant *)
let mk_real f =
  let expr = Term.mk_dec f in

  { expr_init = expr; expr_step = expr; expr_type = Type.t_real }
...
let mk_free_var v =
  ...
  { expr_init = t; expr_step = t; expr_type = Var.type_of_var v }
...
let mk_index_var i =
  ...
  { expr_init = v; expr_step = v; expr_type = Type.t_int }
...
let type_of_bool_bool_bool = function
  | t when Type.is_bool t -> (
      function t when Type.is_bool t -> Type.t_bool | _ -> raise Type_mismatch)
  | _ -> raise Type_mismatch
...
let num_to_numeral_opt = function Number n -> Some n | _ -> None

let numeral_opts_to_nums lo hi =
  match (lo, hi) with
...
let best_int_range is_div op t t' =
  ...
  | Some lo', Some hi'
    when is_div && Numeral.(equal lo' zero) && Numeral.(equal hi' zero) ->
      raise Division_by_zero
  | Some lo', _ when is_div && Numeral.(equal lo' zero) -> Type.t_int
  | _, Some hi' when is_div && Numeral.(equal hi' zero) -> Type.t_int
  | lo', hi' -> (
      let lo, hi = Type.bounds_of_int_range t in
      let (lo', hi'), (lo, hi) =
        (numeral_opts_to_nums lo' hi', numeral_opts_to_nums lo hi)
      in
      let bounds = [ op lo lo'; op hi lo'; op lo hi'; op hi hi' ] in
      let lower = min_opt bounds in
      let upper = max_opt bounds in
      match (lower, upper) with
      | Nothing, _ | _, Nothing -> Type.t_int
      | _ ->
          Type.mk_int_range (num_to_numeral_opt lower)
...
let type_of_numOrBV_numOrBV_numOrBV ?(is_div = false) op t t' =
  try best_int_range is_div op t t'
...
let type_of_num_num_bool = function
  | t when Type.is_int t || Type.is_int_range t -> (
      ...
      | _ -> raise Type_mismatch)
  | t when Type.is_real t -> (
      function t when Type.is_real t -> Type.t_bool | _ -> raise Type_mismatch)
...
let eval_uminus expr =
  ...
  | _ ->
      if
        Type.is_bitvector (Term.type_of_term expr)
        || Type.is_ubitvector (Term.type_of_term expr)
      then Term.mk_bvneg expr
      else Term.mk_minus [ expr ]
...
let type_of_uminus = function
  ...
  | t when Type.is_int_range t -> (
      ...
      | Some lbound, Some ubound ->
          Type.mk_int_range (Some Numeral.(-ubound)) (Some Numeral.(-lbound))
...
let eval_to_int expr =
  ...
  if Type.is_int tt || Type.is_int_range tt then expr
  else
    match Term.destruct expr with
    | Term.T.Const s when Symbol.is_decimal s ->
        Term.mk_num
    ...
    | Term.T.Const s when Symbol.is_ubv8 s ->
        Term.mk_num (Bitvector.ubv8_to_num (Symbol.ubitvector_of_symbol s))
    | Term.T.Const s when Symbol.is_ubv16 s ->
        Term.mk_num (Bitvector.ubv16_to_num (Symbol.ubitvector_of_symbol s))
    | Term.T.Const s when Symbol.is_ubv32 s ->
        Term.mk_num (Bitvector.ubv32_to_num (Symbol.ubitvector_of_symbol s))
    | Term.T.Const s when Symbol.is_ubv64 s ->
        Term.mk_num (Bitvector.ubv64_to_num (Symbol.ubitvector_of_symbol s))
    | Term.T.Const s when Symbol.is_bv8 s ->
        Term.mk_num (Bitvector.bv8_to_num (Symbol.bitvector_of_symbol s))
    | Term.T.Const s when Symbol.is_bv16 s ->
        Term.mk_num (Bitvector.bv16_to_num (Symbol.bitvector_of_symbol s))
    | Term.T.Const s when Symbol.is_bv32 s ->
        Term.mk_num (Bitvector.bv32_to_num (Symbol.bitvector_of_symbol s))
    | Term.T.Const s when Symbol.is_bv64 s ->
        Term.mk_num (Bitvector.bv64_to_num (Symbol.bitvector_of_symbol s))
    ...
    | exception Invalid_argument _ ->
        if Type.is_uint8 (Term.type_of_term expr) then Term.mk_uint8_to_int expr
...
let type_of_to_int = function
  ...
  | t when Type.is_int t || Type.is_int_range t -> Type.t_int
  | t
    when Type.is_uint8 t || Type.is_uint16 t || Type.is_uint32 t
         || Type.is_uint64 t ->
      Type.t_int
  | t
    when Type.is_int8 t || Type.is_int16 t || Type.is_int32 t || Type.is_int64 t
    ->
      Type.t_int
  | _ -> raise Type_mismatch
...
let eval_to_uint8 expr =
  ...
  | Term.T.App (_, [ sub_expr ]) when Term.is_negative_numeral expr ->
      Term.mk_ubv
        (Bitvector.num_to_ubv8 (Numeral.neg (Term.numeral_of_term sub_expr)))
  | _ ->
      let tt = Term.type_of_term expr in
      if Type.is_int tt then Term.mk_to_uint8 expr
      else if Type.is_ubitvector tt then
        if Type.is_uint8 tt then expr
        else Term.mk_bvextract (Numeral.of_int 7) (Numeral.of_int 0) expr
      else raise Type_mismatch
...
let type_of_to_uint8 = function
  | t when Type.is_real t -> Type.t_int
  | t when Type.is_uint8 t || Type.is_int t || Type.is_int_range t ->
      Type.t_ubv 8
  | t when Type.is_uint16 t || Type.is_uint32 t || Type.is_uint64 t ->
      Type.t_ubv 8
  | _ -> raise Type_mismatch
...
let eval_to_uint16 expr =
  ...
  | _ ->
      let tt = Term.type_of_term expr in
      if Type.is_int tt then Term.mk_to_uint16 expr
      else if Type.is_ubitvector tt then
        if Type.is_uint16 tt then expr
        else if Type.is_uint8 tt then
          Term.mk_bvconcat (Term.mk_ubv (Bitvector.zero 8)) expr
        else Term.mk_bvextract (Numeral.of_int 15) (Numeral.of_int 0) expr
      else raise Type_mismatch
...
let type_of_to_uint16 = function
  ...
  | t when Type.is_uint16 t || Type.is_int t || Type.is_int_range t ->
      Type.t_ubv 16
  | t when Type.is_uint8 t || Type.is_uint32 t || Type.is_uint64 t ->
      Type.t_ubv 16
  | _ -> raise Type_mismatch
...
let eval_to_uint32 expr =
  ...
  | _ ->
      let tt = Term.type_of_term expr in
      if Type.is_int tt then Term.mk_to_uint32 expr
      else if Type.is_ubitvector tt then
        if Type.is_uint32 tt then expr
        else if Type.is_uint8 tt then
        ...
        else Term.mk_bvextract (Numeral.of_int 31) (Numeral.of_int 0) expr
...
let type_of_to_uint32 = function
  ...
  | t when Type.is_uint32 t || Type.is_int t || Type.is_int_range t ->
      Type.t_ubv 32
  | t when Type.is_uint8 t || Type.is_uint16 t || Type.is_uint64 t ->
      Type.t_ubv 32
  | _ -> raise Type_mismatch
...
let eval_to_uint64 expr =
  ...
  | _ ->
      let tt = Term.type_of_term expr in
      if Type.is_int tt then Term.mk_to_uint64 expr
      else if Type.is_ubitvector tt then
        if Type.is_uint64 tt then expr
        ...
        else Term.mk_bvconcat (Term.mk_ubv (Bitvector.zero 56)) expr
      else raise Type_mismatch
...
let type_of_to_uint64 = function
  ...
  | t when Type.is_uint64 t || Type.is_int t || Type.is_int_range t ->
      Type.t_ubv 64
  | t when Type.is_uint8 t || Type.is_uint16 t || Type.is_uint32 t ->
      Type.t_ubv 64
  | _ -> raise Type_mismatch
...
let eval_to_int8 expr =
  ...
  | _ ->
      let tt = Term.type_of_term expr in
      if Type.is_int tt then Term.mk_to_int8 expr
      else if Type.is_bitvector tt then
        if Type.is_int8 tt then expr
        else Term.mk_bvextract (Numeral.of_int 7) (Numeral.of_int 0) expr
      else raise Type_mismatch
...
let eval_to_int16 expr =
  ...
  | _ ->
      let tt = Term.type_of_term expr in
      if Type.is_int tt then Term.mk_to_int16 expr
      else if Type.is_bitvector tt then
        if Type.is_int16 tt then expr
        else if Type.is_int8 tt then Term.mk_bvsignext (Numeral.of_int 8) expr
        else Term.mk_bvextract (Numeral.of_int 15) (Numeral.of_int 0) expr
      else raise Type_mismatch
...
let eval_to_int32 expr =
  ...
  | _ ->
      let tt = Term.type_of_term expr in
      if Type.is_int tt then Term.mk_to_int32 expr
      else if Type.is_bitvector tt then
        if Type.is_int32 tt then expr
        else if Type.is_int64 tt then
          Term.mk_bvextract (Numeral.of_int 31) (Numeral.of_int 0) expr
        else if Type.is_int8 tt then Term.mk_bvsignext (Numeral.of_int 24) expr
        else Term.mk_bvsignext (Numeral.of_int 16) expr
      else raise Type_mismatch
...
let eval_to_int64 expr =
  ...
  | _ ->
      let tt = Term.type_of_term expr in
      if Type.is_int tt then Term.mk_to_int64 expr
      else if Type.is_bitvector tt then
        if Type.is_int64 tt then expr
        else if Type.is_int8 tt then Term.mk_bvsignext (Numeral.of_int 56) expr
        else if Type.is_int16 tt then Term.mk_bvsignext (Numeral.of_int 48) expr
        else Term.mk_bvsignext (Numeral.of_int 32) expr
      else raise Type_mismatch
...
let eval_to_real expr =
  let tt = Term.type_of_term expr in
  if Type.is_real tt then expr
  else
    match Term.destruct expr with
    | Term.T.Const s when Symbol.is_numeral s ->
        Term.mk_dec
          (Decimal.of_big_int (Numeral.to_big_int (Symbol.numeral_of_symbol s)))
...
let mk_forall vars expr = mk_unary (eval_forall vars) type_of_forall expr

(* ********************************************************************** *)

(* Evaluate existential quantification *)
let eval_exists vars t =
  match (vars, t) with
  | [], _ -> Term.t_false
  | _, t when t == Term.t_true -> Term.t_true
  | _, t when t == Term.t_false -> Term.t_false
...
let eval_mod expr1 expr2 =
  ...
  | Term.T.Const c1, Term.T.Const c2
    when Symbol.is_numeral c1 && Symbol.is_numeral c2 ->
      Term.mk_num
        Numeral.(Symbol.numeral_of_symbol c1 mod Symbol.numeral_of_symbol c2)
  | _ ->
...
let type_of_mod = function
  | t when Type.is_int t || Type.is_int_range t -> (
      ...
      | t when Type.is_int_range t -> (
          match Type.bounds_of_int_range t with
          | Some l, Some u ->
              Type.mk_int_range (Some Numeral.zero)
                (Some Numeral.(pred (max (abs l) (abs u))))
  ...
  | t1 when Type.is_ubitvector t1 -> (
      ...
      | t2 when Type.is_ubitvector t2 -> (
          ...
          if s1 != s2 then raise Type_mismatch
          else
            match (s1, s2) with
            | 8, 8 -> Type.t_ubv 8
            | 16, 16 -> Type.t_ubv 16
...
let eval_minus expr1 expr2 =
  ...
  | Term.T.Const c1, Term.T.Const c2
    when Symbol.is_numeral c1 && Symbol.is_numeral c2 ->
      Term.mk_num
        Numeral.(Symbol.numeral_of_symbol c1 - Symbol.numeral_of_symbol c2)
  | Term.T.Const c1, Term.T.Const c2
    when Symbol.is_decimal c1 && Symbol.is_decimal c2 ->
      Term.mk_dec
        Decimal.(Symbol.decimal_of_symbol c1 - Symbol.decimal_of_symbol c2)
  | _ ->
      if
        Type.is_bitvector (Term.type_of_term expr1)
        || Type.is_ubitvector (Term.type_of_term expr1)
      then Term.mk_bvsub [ expr1; expr2 ]
      else Term.mk_minus [ expr1; expr2 ]
...
let eval_plus expr1 expr2 =
  ...
  | _ ->
      if
        ...
        || Type.is_ubitvector (Term.type_of_term expr1)
           && Type.is_ubitvector (Term.type_of_term expr1)
      then Term.mk_bvadd [ expr1; expr2 ]
      else Term.mk_plus [ expr1; expr2 ]
...
let eval_times expr1 expr2 =
  ...
  | _ ->
      if
        Type.is_bitvector (Term.type_of_term expr1)
        || Type.is_ubitvector (Term.type_of_term expr1)
      then Term.mk_bvmul [ expr1; expr2 ]
      else Term.mk_times [ expr1; expr2 ]
...
let type_of_intdiv t t' =
  try best_int_range true div_opt t t'
  with Invalid_argument _ -> (
    match t with
    | t when Type.is_int t || Type.is_int_range t -> (
        match t' with
...
let eval_bvshr expr1 expr2 =
  ...
  else Term.mk_bvlshr [ expr1; expr2 ]
...
let eval_eq expr1 expr2 =
  ...
  | _
    when Term.equal expr1 expr2
         && (not (has_pre_vars expr1))
         && not (has_pre_vars expr2) ->
      Term.t_true
  | _ -> (
      match (Term.destruct expr1, Term.destruct expr2) with
      | Term.T.Const c1, Term.T.Const c2
        when Symbol.is_numeral c1 && Symbol.is_numeral c2 ->
          if Numeral.(Symbol.numeral_of_symbol c1 = Symbol.numeral_of_symbol c2)
          then Term.t_true
          else Term.t_false
      | Term.T.Const c1, Term.T.Const c2
        when Symbol.is_decimal c1 && Symbol.is_decimal c2 ->
          if Decimal.(Symbol.decimal_of_symbol c1 = Symbol.decimal_of_symbol c2)
          then Term.t_true
          else Term.t_false
...
let eval_ite = function
  ...
  | expr1 -> (
      ...
      | expr2 -> (
          function
          | expr3 ->
              if Term.equal expr2 expr3 then expr2
              else Term.mk_ite expr1 expr2 expr3))
  ...
  | t when t = Type.t_bool -> (
      ...
      | type2 -> (
          ...
          | type3 -> (
              if
                (* If first type is subtype of second, choose second type *)
                Type.check_type type2 type3
              then type3
              else if
                (* If second type is subtype of first, choose first type *)
                Type.check_type type3 type2
              then type2
              else
                (* Extend integer ranges if one is not a subtype of the other *)
                ...
                | s, t
                  when (Type.is_int_range s && Type.is_int t)
                       || (Type.is_int s && Type.is_int_range t) ->
                    Type.t_int
...
(* If-then-else *)
let mk_ite expr1 expr2 expr3 = mk_ternary eval_ite type_of_ite expr1 expr2 expr3
...
let mk_arrow expr1 expr2 =
  ...
  let res_type = type_of_arrow expr1.expr_type expr2.expr_type in
...
let mk_pre ({ expr_init; expr_step } as expr) =
  if Term.equal expr_init expr_step then
    ...
    | t
      when t == Term.t_true || t == Term.t_false
           || Term.is_free_var t
              && Term.free_var_of_term t |> Var.is_const_state_var
           ||
           match Term.destruct t with
           | Term.T.Const c1 when Symbol.is_numeral c1 || Symbol.is_decimal c1
             ->
               true
           | _ -> false ->
        expr
    ...
    | t
      when Term.is_free_var t
           ...
           && Numeral.(
                Var.offset_of_state_var_instance (Term.free_var_of_term t)
                = base_offset) ->
...
let mk_pre_with_context mk_abs_for_expr mk_lhs_term ctx unguarded
  ...
  let abs_pre () =
    ...
    ({ expr_init = abs_t; expr_step = abs_t; expr_type }, ctx)
  in

  (* Stream is the same in the initial state and step (e -> e) *)
  if (not unguarded) && Term.equal expr_init expr_step then
    match expr_init with
    (* Expression is a constant not part of an unguarded pre expression *)
    | t
      when t == Term.t_true || t == Term.t_false
...
let type_of_store = function
  ...
  | s when Type.is_array s ->
      fun i v ->
        if
          (Type.is_int i || Type.is_int_range i)
...
let mk_of_expr ?as_type expr =
  let expr_type =
    match as_type with Some ty -> ty | None -> Term.type_of_term expr
  in

  { expr_init = expr; expr_step = expr; expr_type }
...
let push_select { expr_init; expr_step; expr_type } =
  ...
  { expr_init = expr_init'; expr_step = expr_step'; expr_type }

let mk_select_and_push e1 e2 = mk_select e1 e2 |> push_select
