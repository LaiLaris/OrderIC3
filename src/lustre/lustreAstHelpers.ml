...
let expr_is_id = function Ident (_, _) -> true | _ -> false
let expr_is_const = function Const (_, _) -> true | _ -> false
let expr_is_true = function Const (_, True) -> true | _ -> false
let expr_is_false = function Const (_, False) -> true | _ -> false

let pos_of_expr = function
  | Ident (pos, _)
  | ModeRef (pos, _)
  | RecordProject (pos, _, _)
  | TupleProject (pos, _, _)
...
let type_arity ty =
  let inner_types = function GroupType (_, es) -> List.length es | _ -> 1 in
  match ty with TArr (_, a, b) -> (inner_types a, inner_types b) | _ -> (0, 0)
...
let rec expr_contains_call = function
  ...
  | RecordExpr (_, _, _, expr_list) | Merge (_, _, expr_list) ->
      List.fold_left
        (fun acc (_, e) -> acc || expr_contains_call e)
        false expr_list
  | Activate (_, _, e1, e2, expr_list) ->
      expr_contains_call e1 || expr_contains_call e2
      || List.fold_left
           (fun acc x -> acc || expr_contains_call x)
           false expr_list
  | Call (_, _, _, _)
  | Condact (_, _, _, _, _, _)
  | RestartEvery (_, _, _, _)
  | AnyOp (_, _, _, _) ->
      true
...
let rec type_contains_array = function
  ...
  | RecordType (_, _, tys) ->
      List.fold_left
        (fun acc (_, _, ty) -> acc || type_contains_array ty)
        false tys
  | ArrayType _ -> true
  | _ -> false
...
let rec expr_contains_id id = function
  ...
  | RecordExpr (_, _, _, expr_list) | Merge (_, _, expr_list) ->
      List.fold_left
        (fun acc (_, e) -> acc || expr_contains_id id e)
        false expr_list
  | Activate (_, _, e1, e2, expr_list) ->
      expr_contains_id id e1 || expr_contains_id id e2
      || List.fold_left
           (fun acc x -> acc || expr_contains_id id x)
           false expr_list
  | AnyOp (_, (_, id2, _), e, None) ->
      if id != id2 then expr_contains_id id e else false
  | AnyOp (_, (_, id2, _), e1, Some e2) ->
      if id != id2 then expr_contains_id id e1 || expr_contains_id id e2
      else false
...
let rec substitute_naive (var : HString.t) t = function
  ...
  | RecordProject (pos, e, idx) ->
      RecordProject (pos, substitute_naive var t e, idx)
  | TupleProject (pos, e, idx) ->
      TupleProject (pos, substitute_naive var t e, idx)
  | Const (_, _) as e -> e
  ...
  | TernaryOp (pos, op, e1, e2, e3) ->
      TernaryOp
        ( pos,
          op,
          substitute_naive var t e1,
          substitute_naive var t e2,
          substitute_naive var t e3 )
  ...
  | RecordExpr (pos, ident, ps, expr_list) ->
      RecordExpr
        ( pos,
          ident,
          ps,
          List.map (fun (i, e) -> (i, substitute_naive var t e)) expr_list )
  | GroupExpr (pos, kind, expr_list) ->
      GroupExpr
        (pos, kind, List.map (fun e -> substitute_naive var t e) expr_list)
  | StructUpdate (pos, e1, idx, e2) ->
      StructUpdate
        (pos, substitute_naive var t e1, idx, substitute_naive var t e2)
  ...
  | Condact (pos, e1, e2, id, expr_list1, expr_list2) ->
      ...
      let expr_list1 =
        List.map (fun e -> substitute_naive var t e) expr_list1
      in
      let expr_list2 =
        List.map (fun e -> substitute_naive var t e) expr_list2
      in
      Condact (pos, e1, e2, id, expr_list1, expr_list2)
  ...
  | Arrow (pos, e1, e2) ->
      Arrow (pos, substitute_naive var t e1, substitute_naive var t e2)
  | Call (pos, ty_args, id, expr_list) ->
      Call
        ( pos,
          ty_args,
          id,
          List.map (fun e -> substitute_naive var t e) expr_list )
...
let rec apply_subst_in_expr sigma = function
  ...
  | ModeRef (_, _) as e -> e
  | RecordProject (pos, e, idx) ->
      RecordProject (pos, apply_subst_in_expr sigma e, idx)
  | TupleProject (pos, e, idx) ->
      TupleProject (pos, apply_subst_in_expr sigma e, idx)
  | Const (_, _) as e -> e
  | UnaryOp (pos, op, e) -> UnaryOp (pos, op, apply_subst_in_expr sigma e)
  | BinaryOp (pos, op, e1, e2) ->
      BinaryOp
        (pos, op, apply_subst_in_expr sigma e1, apply_subst_in_expr sigma e2)
  | TernaryOp (pos, op, e1, e2, e3) ->
      TernaryOp
        ( pos,
          op,
          apply_subst_in_expr sigma e1,
          apply_subst_in_expr sigma e2,
          apply_subst_in_expr sigma e3 )
  | ConvOp (pos, op, e) -> ConvOp (pos, op, apply_subst_in_expr sigma e)
  | CompOp (pos, op, e1, e2) ->
      CompOp
        (pos, op, apply_subst_in_expr sigma e1, apply_subst_in_expr sigma e2)
  | AnyOp _ ->
      assert false (* Not supported due to introduction of bound variables *)
  | Quantifier _ ->
      assert false (* Not supported due to introduction of bound variables *)
  | RecordExpr (pos, ident, ps, expr_list) ->
      RecordExpr
        ( pos,
          ident,
          ps,
          List.map (fun (i, e) -> (i, apply_subst_in_expr sigma e)) expr_list )
  | GroupExpr (pos, kind, expr_list) ->
      GroupExpr
        (pos, kind, List.map (fun e -> apply_subst_in_expr sigma e) expr_list)
  | StructUpdate (pos, e1, idx, e2) ->
      StructUpdate
        (pos, apply_subst_in_expr sigma e1, idx, apply_subst_in_expr sigma e2)
  | ArrayConstr (pos, e1, e2) ->
      ArrayConstr
        (pos, apply_subst_in_expr sigma e1, apply_subst_in_expr sigma e2)
  | ArrayIndex (pos, e1, e2) ->
      ArrayIndex
        (pos, apply_subst_in_expr sigma e1, apply_subst_in_expr sigma e2)
  ...
  | Condact (pos, e1, e2, id, expr_list1, expr_list2) ->
      let e1, e2 =
        (apply_subst_in_expr sigma e1, apply_subst_in_expr sigma e2)
      in
      let expr_list1 =
        List.map (fun e -> apply_subst_in_expr sigma e) expr_list1
      in
      let expr_list2 =
        List.map (fun e -> apply_subst_in_expr sigma e) expr_list2
      in
      Condact (pos, e1, e2, id, expr_list1, expr_list2)
  | Activate (pos, ident, e1, e2, expr_list) ->
      let e1, e2 =
      ...
      in
      let expr_list =
        List.map (fun e -> apply_subst_in_expr sigma e) expr_list
      in
      Activate (pos, ident, e1, e2, expr_list)
  | Merge (pos, ident, expr_list) ->
      Merge
  ...
  | Arrow (pos, e1, e2) ->
      Arrow (pos, apply_subst_in_expr sigma e1, apply_subst_in_expr sigma e2)
  | Call (pos, ty_args, id, expr_list) ->
      Call
        ( pos,
          ty_args,
...
let rec apply_type_subst_in_expr =
 fun sigma expr ->
  ...
  | Call (pos, ty_args, id, expr_list) ->
      ...
      Call
        (pos, ty_args, id, List.map (apply_type_subst_in_expr sigma) expr_list)
  | Quantifier (pos, q, tis, expr) ->
      let tis =
        List.map
          (fun (p, id, ty) -> (p, id, apply_type_subst_in_type sigma ty))
          tis
      in
      Quantifier (pos, q, tis, apply_type_subst_in_expr sigma expr)
  | AnyOp _ ->
      assert false (* Not supported due to introduction of bound variables *)
  | Ident _ | ModeRef _ -> expr
  | RecordProject (pos, e, idx) ->
      RecordProject (pos, apply_type_subst_in_expr sigma e, idx)
  | TupleProject (pos, e, idx) ->
      TupleProject (pos, apply_type_subst_in_expr sigma e, idx)
  | Const (_, _) as e -> e
  ...
  | BinaryOp (pos, op, e1, e2) ->
      BinaryOp
        ( pos,
          op,
          apply_type_subst_in_expr sigma e1,
          apply_type_subst_in_expr sigma e2 )
  | TernaryOp (pos, op, e1, e2, e3) ->
      TernaryOp
        ( pos,
          op,
          apply_type_subst_in_expr sigma e1,
          apply_type_subst_in_expr sigma e2,
          apply_type_subst_in_expr sigma e3 )
  ...
  | RecordExpr (pos, ident, ps, expr_list) ->
      RecordExpr
        ( pos,
          ...
          ps,
          List.map
            (fun (i, e) -> (i, apply_type_subst_in_expr sigma e))
            expr_list )
  | GroupExpr (pos, kind, expr_list) ->
      GroupExpr
        ( pos,
          kind,
          List.map (fun e -> apply_type_subst_in_expr sigma e) expr_list )
  | StructUpdate (pos, e1, idx, e2) ->
      StructUpdate
        ( pos,
          apply_type_subst_in_expr sigma e1,
  ...
  | Condact (pos, e1, e2, id, expr_list1, expr_list2) ->
      let e1, e2 =
        (apply_type_subst_in_expr sigma e1, apply_type_subst_in_expr sigma e2)
      in
      let expr_list1 =
        List.map (fun e -> apply_type_subst_in_expr sigma e) expr_list1
      in
      let expr_list2 =
        List.map (fun e -> apply_type_subst_in_expr sigma e) expr_list2
      in
      Condact (pos, e1, e2, id, expr_list1, expr_list2)
  | Activate (pos, ident, e1, e2, expr_list) ->
      let e1, e2 =
...
and apply_type_subst_in_type :
    (index * lustre_type) list -> lustre_type -> lustre_type =
 fun sigma ty ->
  match ty with
  ...
  | AbstractType (_, i) as ty -> (
      match List.assoc_opt i sigma with Some ty -> ty | None -> ty)
  | ArrayType (pos, (ty, expr)) ->
      ArrayType
        ( pos,
          ( apply_type_subst_in_type sigma ty,
            apply_type_subst_in_expr sigma expr ) )
  | TupleType (pos, tys) ->
  ...
  | TArr (pos, ty1, ty2) ->
      TArr
        ( pos,
          apply_type_subst_in_type sigma ty1,
          apply_type_subst_in_type sigma ty2 )
  ...
  | RefinementType (pos, (pos2, id, ty), expr) ->
      RefinementType
        ( pos,
          (pos2, id, apply_type_subst_in_type sigma ty),
          apply_type_subst_in_expr sigma expr )
  | ty -> ty
...
let rec has_unguarded_pre ung = function
  ...
  | AnyOp (pos, _, _, _) ->
      fail_at_position pos
        "'Any' operations are not supported in the old front end"
  | BinaryOp (_, _, e1, e2) | ArrayConstr (_, e1, e2) | CompOp (_, _, e1, e2) ->
  ...
  | StructUpdate (_, e1, li, e2) ->
      ...
      let us =
        List.map
          (function
            | Label _ -> false | Index (_, e) -> has_unguarded_pre ung e)
          li
      in
      let u2 = has_unguarded_pre ung e2 in
  ...
  | Pre (pos, e) as p ->
      (if ung then
         ...
         let err_or_warn =
           if Flags.lus_strict () then fail_at_position else warn_at_position
         in
...
let rec has_unguarded_pre_no_warn ung = function
  ...
  | TupleProject (_, e, _)
  | Quantifier (_, _, _, e) ->
      has_unguarded_pre_no_warn ung e
  ...
  | StructUpdate (_, e1, li, e2) ->
      ...
      let us =
        List.map
          (function
            | Label _ -> false | Index (_, e) -> has_unguarded_pre_no_warn ung e)
          li
      in
      let u2 = has_unguarded_pre_no_warn ung e2 in
      u1 || u2 || List.exists Lib.identity us
...
let has_unguarded_pre_no_warn e = has_unguarded_pre_no_warn true e

(** If second argument is `Some _`, returns that. Otherwise runs `f`. *)
let unwrap_or f = function None -> f () | res -> res

(** If input list contains `Some _`, returns that. Otherwise returns `None`. *)
let some_of_list =
  List.fold_left (function None -> Lib.identity | res -> fun _ -> res) None
...
let rec has_pre_or_arrow = function
  ...
  | ArrayIndex (_, e1, e2)
  | ArrayConstr (_, e1, e2) -> (
      match has_pre_or_arrow e1 with None -> has_pre_or_arrow e2 | res -> res)
  | TernaryOp (_, _, e1, e2, e3) ->
      has_pre_or_arrow e1
      |> unwrap_or (fun _ ->
             has_pre_or_arrow e2 |> unwrap_or (fun _ -> has_pre_or_arrow e3))
  | GroupExpr (_, _, l) | Call (_, _, _, l) ->
      List.map has_pre_or_arrow l |> some_of_list
  | Merge (_, _, l) ->
      List.map has_pre_or_arrow (List.map snd l) |> some_of_list
  | RestartEvery (_, _, l, e) ->
      List.map has_pre_or_arrow (e :: l) |> some_of_list
  | Activate (_, _, e, r, l) ->
      List.map has_pre_or_arrow (e :: r :: l) |> some_of_list
  | Condact (_, e, r, _, l1, l2) ->
      List.map has_pre_or_arrow ((e :: r :: l1) @ l2) |> some_of_list
  | RecordExpr (_, _, _, ie) ->
      List.map (fun (_, e) -> has_pre_or_arrow e) ie |> some_of_list
  | StructUpdate (_, e1, li, e2) ->
      has_pre_or_arrow e1
      |> unwrap_or (fun _ ->
             has_pre_or_arrow e2
             |> unwrap_or (fun _ ->
                    List.map
                      (function
                        | Label _ -> None | Index (_, e) -> has_pre_or_arrow e)
                      li
                    |> some_of_list))
  | Pre (pos, _) -> Some pos
...
let rec struct_item_has_pre_or_arrow = function
  ...
  | TupleStructItem (_, l) ->
      List.map struct_item_has_pre_or_arrow l |> some_of_list
...
let eq_lhs_has_pre_or_arrow = function
  | StructDef (_, l) -> List.map struct_item_has_pre_or_arrow l |> some_of_list
...
let rec node_item_has_pre_or_arrow = function
  ...
  | Body (Equation (_, lhs, e)) ->
      eq_lhs_has_pre_or_arrow lhs |> unwrap_or (fun _ -> has_pre_or_arrow e)
  | IfBlock (_, e, l1, l2) -> (
      match has_pre_or_arrow e with
      | Some pos -> Some pos
      | None -> (
          match node_item_list_has_pre_or_arrow l1 with
          | Some pos -> Some pos
...
and node_item_list_has_pre_or_arrow = function
  | ni :: nis -> (
      match node_item_has_pre_or_arrow ni with
      | Some pos -> Some pos
      | None -> node_item_list_has_pre_or_arrow nis)
  | [] -> None
...
let contract_node_equation_has_pre_or_arrow = function
  ...
  | GhostVars (_, _, e) | Assume (_, _, _, e) | Guarantee (_, _, _, e) ->
      has_pre_or_arrow e
  | Mode (_, _, reqs, enss) ->
      List.map (fun (_, _, e) -> has_pre_or_arrow e) reqs
      |> some_of_list
      |> unwrap_or (fun _ ->
             List.map (fun (_, _, e) -> has_pre_or_arrow e) enss |> some_of_list)
...
let contract_has_pre_or_arrow (_, l) =
  List.map contract_node_equation_has_pre_or_arrow l |> some_of_list
...
let rec vars_of_node_calls_h obs =
  ...
  | ModeRef (_, is) ->
      if obs then SI.singleton (mk_mode_ref_id is) else SI.empty
  ...
  | TernaryOp (_, _, e1, e2, e3) ->
      vars obs e1 |> SI.union (vars obs e2) |> SI.union (vars obs e3)
  ...
  | AnyOp (_, (_, i, _), e1, Some e2) ->
      SI.diff (SI.union (vars true e1) (vars true e2)) (SI.singleton i)
  (* Structured expressions *)
  | RecordExpr (_, _, _, flds) ->
      SI.flatten (List.map (vars obs) (snd (List.split flds)))
  ...
  | Quantifier (_, _, qs, e) ->
      SI.diff (vars obs e) (SI.flatten (List.map vars_of_ty_ids qs))
  (* Clock operators *)
  | When (_, e, clkE) -> SI.union (vars obs e) (vars_of_clock_expr clkE)
  | Condact (_, e1, e2, _, es1, es2) ->
      SI.flatten
        ((vars obs e1 :: vars obs e2 :: List.map (vars obs) es1)
        @ List.map (vars obs) es2)
  | Activate (_, _, e1, e2, es) ->
      SI.flatten (vars obs e1 :: vars obs e2 :: List.map (vars obs) es)
  | Merge (_, _, es) ->
      List.split es |> snd |> List.map (vars obs) |> SI.flatten
  | RestartEvery (_, _, es, e) ->
      SI.flatten (vars obs e :: List.map (vars obs) es)
  (* Temporal operators *)
  | Pre (_, e) -> vars obs e
...
let rec vars_without_node_call_ids : expr -> iset =
  ...
  | TernaryOp (_, _, e1, e2, e3) ->
      vars e1 |> SI.union (vars e2) |> SI.union (vars e3)
  | ConvOp (_, _, e) -> vars e
  | CompOp (_, _, e1, e2) -> vars e1 |> SI.union (vars e2)
  (* Structured expressions *)
  | RecordExpr (_, _, _, flds) ->
      SI.flatten (List.map vars (snd (List.split flds)))
  ...
  | AnyOp (_, (_, i, _), e1, Some e2) ->
      SI.diff (SI.union (vars e1) (vars e2)) (SI.singleton i)
  (* Temporal operators *)
  | Pre (_, e) -> vars e
...
let rec calls_of_expr : expr -> iset = function
  (* Node calls *)
  | Call (_, _, i, es) ->
      SI.union (SI.singleton i) (SI.flatten (List.map calls_of_expr es))
  | Condact (_, e1, e2, i, es1, es2) ->
      SI.union (SI.singleton i)
        (SI.flatten
           ((calls_of_expr e1 :: calls_of_expr e2 :: List.map calls_of_expr es1)
           @ List.map calls_of_expr es2))
  | Activate (_, i, e1, e2, es) ->
      SI.union (SI.singleton i)
        (SI.flatten
           (calls_of_expr e1 :: calls_of_expr e2 :: List.map calls_of_expr es))
  ...
  | TernaryOp (_, _, e1, e2, e3) ->
      calls_of_expr e1
      |> SI.union (calls_of_expr e2)
      |> SI.union (calls_of_expr e3)
  | ConvOp (_, _, e) -> calls_of_expr e
  | CompOp (_, _, e1, e2) -> calls_of_expr e1 |> SI.union (calls_of_expr e2)
  | RecordExpr (_, _, _, flds) ->
      SI.flatten (List.map calls_of_expr (snd (List.split flds)))
  | GroupExpr (_, _, es) -> SI.flatten (List.map calls_of_expr es)
  | StructUpdate (_, e1, _, e2) ->
      SI.union (calls_of_expr e1) (calls_of_expr e2)
  ...
  | Merge (_, _, es) ->
      List.split es |> snd |> List.map calls_of_expr |> SI.flatten
  | AnyOp (_, (_, i, _), e, None) -> SI.diff (calls_of_expr e) (SI.singleton i)
  | AnyOp (_, (_, i, _), e1, Some e2) ->
      SI.diff (SI.union (calls_of_expr e1) (calls_of_expr e2)) (SI.singleton i)
...
let rec vars_of_struct_item_with_pos = function
  ...
  | TupleStructItem (_, ts) ->
      List.flatten (List.map vars_of_struct_item_with_pos ts)
...
let rec vars_of_type = function
  | ArrayType (_, (ty, e)) ->
      SI.union (vars_of_type ty) (vars_without_node_call_ids e)
  | TupleType (_, tys) | GroupType (_, tys) ->
  ...
  | UInt64 _ | Bool _ | IntRange _ | Real _ | UserType _ | AbstractType _
  | EnumType _ ->
      SI.empty

let rec defined_vars_with_pos = function
  | Body (Equation (_, StructDef (_, ss), _)) ->
      List.flatten (List.map vars_of_struct_item_with_pos ss)
  | IfBlock (_, _, l1, l2) ->
      List.flatten (List.map defined_vars_with_pos l1)
      @ List.flatten (List.map defined_vars_with_pos l2)
  | FrameBlock (_, vars, _, _) -> vars
  | _ -> []

(** Return an ast that adds two expressions*)
let add_exp : Lib.position -> expr -> expr -> expr =
 fun pos e1 e2 -> BinaryOp (pos, Plus, e1, e2)

(** returns an ast which is the absolute difference of two expr ast*)
let abs_diff : Lib.position -> expr -> expr -> expr =
 fun pos e1 e2 ->
  TernaryOp
    ( pos,
      Ite,
...
let is_type_or_const_decl : declaration -> bool =
 fun ty -> match ty with TypeDecl _ | ConstDecl _ -> true | _ -> false
...
let split_program : declaration list -> declaration list * declaration list =
  List.fold_left
    (fun (ds, ds') d ->
      if is_type_or_const_decl d then (d :: ds, ds') else (ds, d :: ds'))
    ([], [])
...
let rec replace_with_constants : expr -> expr =
  ...
  | RecordExpr (p, i, ps, flds) ->
      RecordExpr
        (p, i, ps, List.map (fun (f, e) -> (f, replace_with_constants e)) flds)
  ...
  | ArrayIndex (p, e1, e2) ->
      ...
      ArrayIndex (p, e1', e2')
  (* Quantified expressions *)
  | Quantifier (p, q, qs, e) -> Quantifier (p, q, qs, replace_with_constants e)
  (* Clock operators *)
  ...
  | RestartEvery (p, i, es, e) ->
      RestartEvery
        (p, i, List.map replace_with_constants es, replace_with_constants e)
  (* Temporal operators *)
  | Pre (_, e) -> replace_with_constants e
  | Arrow (p, e1, e2) ->
      Arrow (p, replace_with_constants e1, replace_with_constants e2)
  (* Node calls *)
  | Call (p, ty_args, i, es) ->
      Call (p, ty_args, i, List.map replace_with_constants es)
...
let rec abstract_pre_subexpressions : expr -> expr = function
  ...
  | RecordProject (p, e, i) ->
      RecordProject (p, abstract_pre_subexpressions e, i)
  ...
  | RecordExpr (p, i, ps, flds) ->
      RecordExpr
        ( p,
          i,
          ps,
          List.map (fun (f, e) -> (f, abstract_pre_subexpressions e)) flds )
  | GroupExpr (p, g, es) ->
      GroupExpr (p, g, List.map abstract_pre_subexpressions es)
  (* Update of structured expressions *)
  | StructUpdate (p, e1, i, e2) ->
      let e1' = abstract_pre_subexpressions e1 in
      let e2' = abstract_pre_subexpressions e2 in
  ...
  | Merge (p, i, es) ->
      Merge
        (p, i, List.map (fun (i, e) -> (i, abstract_pre_subexpressions e)) es)
  | RestartEvery (p, i, es, e) ->
      RestartEvery
        ( p,
          i,
          List.map abstract_pre_subexpressions es,
          abstract_pre_subexpressions e )
  (* Temporal operators *)
  | Pre (p, e) -> Pre (p, replace_with_constants e)
  | Arrow (p, e1, e2) ->
      Arrow (p, abstract_pre_subexpressions e1, abstract_pre_subexpressions e2)
  (* Node calls *)
  | Call (p, ty_args, i, es) ->
      Call (p, ty_args, i, List.map abstract_pre_subexpressions es)

(** For every identifier, if that identifier is position n in locals1, replace
    it with position n in locals2 *)
let rec replace_idents locals1 locals2 expr =
  ...
  | Arrow (a, e1, e2) ->
      Arrow
        (a, replace_idents locals1 locals2 e1, replace_idents locals1 locals2 e2)
  | Const _ as e -> e
  | ModeRef _ as e -> e
  | RecordProject (a, e, b) ->
      RecordProject (a, replace_idents locals1 locals2 e, b)
  ...
  | TupleProject (a, e, b) ->
      TupleProject (a, replace_idents locals1 locals2 e, b)
  | BinaryOp (a, b, e1, e2) ->
      BinaryOp
        ( a,
          b,
          replace_idents locals1 locals2 e1,
  ...
  | ArrayConstr (a, e1, e2) ->
      ArrayConstr
        (a, replace_idents locals1 locals2 e1, replace_idents locals1 locals2 e2)
  | TernaryOp (a, b, e1, e2, e3) ->
      TernaryOp
        ( a,
          b,
          replace_idents locals1 locals2 e1,
          replace_idents locals1 locals2 e2,
          replace_idents locals1 locals2 e3 )
  | GroupExpr (a, b, l) ->
      GroupExpr (a, b, List.map (replace_idents locals1 locals2) l)
  | Call (a, b, c, l) ->
      Call (a, b, c, List.map (replace_idents locals1 locals2) l)
  | AnyOp _ -> assert false (* desugared in lustreDesugarAnyOps *)
  | Quantifier (a, b, tis, e) ->
      ...
      let locals1, locals2 =
        List.filter (fun (i, _) -> not (List.mem i is)) locals |> List.split
      in
  ...
  | RestartEvery (a, b, l, e) ->
      RestartEvery
        ( a,
          b,
          List.map (replace_idents locals1 locals2) l,
          replace_idents locals1 locals2 e )
  | Activate (a, b, e, r, l) ->
      Activate
        ( a,
          b,
          (replace_idents locals1 locals2) e,
          (replace_idents locals1 locals2) r,
          List.map (replace_idents locals1 locals2) l )
  | Condact (a, e, r, b, l1, l2) ->
      Condact
        ( a,
          ...
          (replace_idents locals1 locals2) r,
          b,
          List.map (replace_idents locals1 locals2) l1,
          List.map (replace_idents locals1 locals2) l2 )
...
let extract_node_equation : node_item -> (eq_lhs * expr) list = function
  | Body (Equation (_, lhs, expr)) -> [ (lhs, expr) ]
  | _ -> []

let get_last_node_name : declaration list -> ident option =
 fun ds ->
  let rec get_first_node_name : declaration list -> ident option = function
    | [] -> None
    | NodeDecl (_, (n, _, _, _, _, _, _, _, _)) :: _ -> Some n
    | _ :: rest -> get_first_node_name rest
  in
  get_first_node_name (List.rev ds)
...
let rec remove_node_in_declarations :
 ...
 fun n pres -> function
  ...
  | (NodeDecl (_, (n', _, _, _, _, _, _, _, _)) as mn) :: rest ->
      if HString.compare n' n = 0 then Some (mn, pres @ rest)
...
(** Sort identifiers *)
let sort_typed_ident : typed_ident list -> typed_ident list =
 fun ty_idents ->
  List.sort (fun (_, i1, _) (_, i2, _) -> HString.compare i1 i2) ty_idents

(** sort typed identifiers *)
let sort_idents : ident list -> ident list =
 fun ids -> List.sort (fun i1 i2 -> HString.compare i1 i2) ids

let rec syn_expr_equal depth_limit x y : (bool, unit) result =
  let ( >>= ) = Res.( >>= ) in
  let rec r depth x y =
    let rlist xl yl =
      if List.length xl = List.length yl then
    ...
    if Lib.is_some depth_limit && depth > Lib.get depth_limit then Error ()
      ...
      | BinaryOp (_, xop, xe1, xe2), BinaryOp (_, yop, ye1, ye2) ->
          ...
          r (depth + 1) xe2 ye2 >>= fun e2 -> Ok (e1 && e2 && xop = yop)
      | TernaryOp (_, xop, xe1, xe2, xe3), TernaryOp (_, yop, ye1, ye2, ye3) ->
          r (depth + 1) xe1 ye1 >>= fun e1 ->
          r (depth + 1) xe2 ye2 >>= fun e2 ->
          r (depth + 1) xe3 ye3 >>= fun e3 -> Ok (e1 && e2 && e3 && xop = yop)
      ...
      | When (_, x, ClockNeg xi), When (_, y, ClockNeg yi) ->
          r (depth + 1) x y >>= fun e -> Ok (e && HString.equal xi yi)
  ...
  let rec r depth x y =
    ...
    if Lib.is_some depth_limit && depth > Lib.get depth_limit then Error ()
      ...
      | UserType (_, ty_args1, x), UserType (_, ty_args2, y) ->
          ...
          Ok (r1 && r2)
      | AbstractType (_, x), AbstractType (_, y) -> Ok (HString.equal x y)
      ...
      | ArrayType (_, (xt, xe)), ArrayType (_, (yt, ye)) ->
          ...
          syn_expr_equal depth_limit xe ye >>= fun e -> Ok (t && e)
      | EnumType (_, xi, xl), EnumType (_, yi, yl) ->
          let t =
            if List.length xl = List.length yl then
              List.map2 HString.equal xl yl |> List.fold_left ( && ) true
            else false
          in
          Ok (t && HString.equal xi yi)
      | TArr (_, xt1, xt2), TArr (_, yt1, yt2) ->
          r (depth + 1) xt1 yt1 >>= fun t1 ->
          r (depth + 1) xt2 yt2 >>= fun t2 -> Ok (t1 && t2)
      | _ -> Ok false
  in
  r 0 x y
...
let rec rename_contract_vars = function
  ...
  | ModeRef (_, _) as e -> e
  | RecordProject (pos, e, idx) ->
      RecordProject (pos, rename_contract_vars e, idx)
  ...
  | TernaryOp (pos, op, e1, e2, e3) ->
      TernaryOp
        ( pos,
          op,
          rename_contract_vars e1,
          rename_contract_vars e2,
          rename_contract_vars e3 )
  ...
  | RecordExpr (pos, ident, ps, expr_list) ->
      RecordExpr
        ( pos,
          ident,
          ps,
          List.map (fun (i, e) -> (i, rename_contract_vars e)) expr_list )
  | GroupExpr (pos, kind, expr_list) ->
  ...
  | Arrow (pos, e1, e2) ->
      Arrow (pos, rename_contract_vars e1, rename_contract_vars e2)
  | Call (pos, ty_args, id, expr_list) ->
      Call
        (pos, ty_args, id, List.map (fun e -> rename_contract_vars e) expr_list)
...
let get_const_num_value = function
  | Const (_, Num x) -> int_of_string_opt (HString.string_of_hstring x)
  | _ -> None
