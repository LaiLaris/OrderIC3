...
type source = Input | Output | Local | Global | Ghost

type const_store = (LA.expr * tc_type option * source) IMap.t
(** A Store of constant identifier and their (const) values with types. * The
    values of the associated identifiers should be evaluated to a * Bool or an
    Int at constant propogation phase of type checking. *)
...
(** Mapping for all the exports of the contract, modes and contract ghost const
    and vars *)
...
type tc_context = {
  ty_syns : ty_alias_store; (* store of the type alias mappings *)
  ...
  node_param_attr : param_store;
      (* store of the parameter attributes of nodes *)
  vl_ctx : const_store; (* store of typed constants to its value *)
  u_types : ty_set;
      (* store of all declared user types,
                                                     this is poor mans kind (type of type) context *)
  contract_export_ctx :
    (* stores all the export variables  of the contract *)
    contract_exports;
  enum_vars : enum_variants;
  ty_vars :
    (* stores the type variables associated with each node *)
    ty_var_store;
  contract_ty_vars :
    (* stores  the type variables associated with each contract *)
    ty_var_store;
  ty_ty_vars : ty_var_store;
      (* stores the type variables associated with each user type *)
  ty_args :
    (* stores the type arguments associated with each (monomorphized) node *)
    ty_arg_store;
}
...
(** Picks out the type synonym from the context If it is user type then chases
    it (recursively looks up) the actual type. This chasing is necessary to
    check type equality between user defined types. *)
 ...
 fun ctx i ty_args ->
  ...
  | Some ty -> (
      ...
      | LA.UserType (_, ty_args, uid) ->
          if uid = i then Some ty else lookup_ty_syn ctx uid ty_args
      | _ -> Some ty)
  | None -> None
...
let lookup_node_param_attr :
    tc_context -> LA.ident -> (HString.t * bool) list option =
...
let lookup_const :
    tc_context -> LA.ident -> (LA.expr * tc_type option * source) option =
...
let add_ty_vars_node : tc_context -> LA.ident -> LA.ident list -> tc_context =
 fun ctx i ty_vars -> { ctx with ty_vars = IMap.add i ty_vars ctx.ty_vars }
...
let add_ty_args_node :
    tc_context -> LA.ident -> LA.lustre_type list -> tc_context =
 fun ctx i ty_args -> { ctx with ty_args = IMap.add i ty_args ctx.ty_args }
...
let add_ty_vars_contract : tc_context -> LA.ident -> LA.ident list -> tc_context
 ...
 fun ctx i contract_ty_vars ->
  {
    ctx with
    contract_ty_vars = IMap.add i contract_ty_vars ctx.contract_ty_vars;
  }

let add_node_param_attr :
    tc_context -> LA.ident -> LA.const_clocked_typed_decl list -> tc_context =
 fun ctx i args ->
  let v = List.map (function _, id, _, _, is_const -> (id, is_const)) args in
...
let is_enum_variant ctx id =
  ...
  | Some (_, Some (UserType (_, _, uid)), _) -> lookup_variants ctx uid != None
  | _ -> false
...
let add_const :
    tc_context -> LA.ident -> LA.expr -> tc_type -> source -> tc_context =
 fun ctx i e ty sc ->
  { ctx with vl_ctx = IMap.add i (e, Some ty, sc) ctx.vl_ctx }

let add_untyped_const :
    tc_context -> LA.ident -> LA.expr -> source -> tc_context =
...
let union : tc_context -> tc_context -> tc_context =
 fun ctx1 ctx2 ->
  {
    ty_syns = IMap.union (fun _ _ v2 -> Some v2) ctx1.ty_syns ctx2.ty_syns;
    ty_ctx = IMap.union (fun _ _ v2 -> Some v2) ctx1.ty_ctx ctx2.ty_ctx;
    contract_ctx =
      IMap.union (fun _ _ v2 -> Some v2) ctx1.contract_ctx ctx2.contract_ctx;
    node_ctx = IMap.union (fun _ _ v2 -> Some v2) ctx1.node_ctx ctx2.node_ctx;
    node_param_attr =
      IMap.union
        (fun _ _ v2 -> Some v2)
        ctx1.node_param_attr ctx2.node_param_attr;
    vl_ctx = IMap.union (fun _ _ v2 -> Some v2) ctx1.vl_ctx ctx2.vl_ctx;
    u_types = SI.union ctx1.u_types ctx2.u_types;
    contract_export_ctx =
      IMap.union
        (fun _ _ v2 -> Some v2)
        ctx1.contract_export_ctx ctx2.contract_export_ctx;
    enum_vars = IMap.union (fun _ _ v2 -> Some v2) ctx1.enum_vars ctx2.enum_vars;
    ty_vars = IMap.union (fun _ _ v2 -> Some v2) ctx1.ty_vars ctx2.ty_vars;
    contract_ty_vars =
      IMap.union
        (fun _ _ v2 -> Some v2)
        ctx1.contract_ty_vars ctx2.contract_ty_vars;
    ty_ty_vars =
      IMap.union (fun _ _ v2 -> Some v2) ctx1.ty_ty_vars ctx2.ty_ty_vars;
    ty_args = IMap.union (fun _ _ v2 -> Some v2) ctx1.ty_args ctx2.ty_args;
  }
...
let extract_consts : LA.const_clocked_typed_decl -> tc_context =
 fun (pos, i, ty, _, is_const) ->
  if is_const then singleton_const i (LA.Ident (pos, i)) ty Local
...
let add_contract_exports : tc_context -> LA.ident -> ty_store -> tc_context =
 fun ctx i exps ->
  { ctx with contract_export_ctx = IMap.add i exps ctx.contract_export_ctx }
...
(** Pretty print type bindings*)
let pp_print_ty_var_binding :
    Format.formatter -> LA.ident * HString.t list -> unit =
 fun ppf (i, ty_vars) ->
...
(** Pretty print type bindings*)
let pp_print_ty_arg_binding :
    Format.formatter -> LA.ident * LA.lustre_type list -> unit =
 fun ppf (i, ty_vars) ->
  Format.fprintf ppf "(%a:{%a})" LA.pp_print_ident i
    (Lib.pp_print_list LA.pp_print_lustre_type ",")
    ty_vars

(** Pretty print value bindings (used for constants)*)
let pp_print_val_binding :
    Format.formatter -> LA.ident * (LA.expr * tc_type option * source) -> unit =
 fun ppf (i, (v, ty, sc)) ->
  Format.fprintf ppf "(%a:%a :-> %a (%s))" LA.pp_print_ident i
    (Lib.pp_print_option LA.pp_print_lustre_type)
    ty LA.pp_print_expr v
...
let pp_print_vstore : Format.formatter -> const_store -> unit =
 fun ppf m ->
  Lib.pp_print_list
    (fun ppf (i, (e, ty, sc)) -> pp_print_val_binding ppf (i, (e, ty, sc)))
    ", " ppf (IMap.bindings m)
...
let pp_print_type_variables : Format.formatter -> ty_var_store -> unit =
 fun ppf m ->
  Lib.pp_print_list pp_print_ty_var_binding ", " ppf (IMap.bindings m)

(** Pretty print declared user types *)
let pp_print_type_arguments : Format.formatter -> ty_arg_store -> unit =
 fun ppf m ->
  Lib.pp_print_list pp_print_ty_arg_binding ", " ppf (IMap.bindings m)
...
let pp_print_contract_exports : Format.formatter -> contract_exports -> unit =
 fun ppf m ->
  Lib.pp_print_list
    (fun ppf (i, exm) ->
      Format.fprintf ppf "(contract %a -> [%a])" LA.pp_print_ident i
        pp_print_tymap exm)
    ", " ppf (IMap.bindings m)
...
let pp_print_enum_variants : Format.formatter -> enum_variants -> unit =
 fun ppf m ->
  Lib.pp_print_list
    (fun ppf (i, exm) ->
      Format.fprintf ppf "(enum %a -> [%a])" LA.pp_print_ident i
        (Lib.pp_print_list HString.pp_print_hstring ",")
        exm)
    ", " ppf (IMap.bindings m)
...
let pp_print_tc_context : Format.formatter -> tc_context -> unit =
 fun ppf ctx ->
  Format.fprintf ppf
   ...
   ^^ "Type arguments={%a}\n")
    pp_print_ty_syns ctx.ty_syns pp_print_tymap ctx.ty_ctx pp_print_tymap
    ctx.node_ctx pp_print_tymap ctx.contract_ctx pp_print_vstore ctx.vl_ctx
    pp_print_u_types ctx.u_types pp_print_contract_exports
    ctx.contract_export_ctx pp_print_enum_variants ctx.enum_vars
    pp_print_type_variables ctx.ty_vars pp_print_type_variables
    ctx.contract_ty_vars pp_print_type_variables ctx.ty_ty_vars
...
let rec traverse_group_expr_list f ctx proj es =
  ...
  | i, e :: es ->
      ...
      if a <= i then traverse_group_expr_list f ctx (i - a) es else f proj e
  | _ -> assert false

let rec is_type_num : tc_context -> tc_type -> (bool, HString.t) result =
 fun ctx ty ->
  match ty with
  | Int _ | UInt8 _ | UInt16 _ | UInt32 _ | UInt64 _ | Int8 _ | Int16 _
  | Int32 _ | Int64 _ | IntRange _ | Real _ ->
      Ok true
  | RefinementType (_, (_, _, ty), _) -> is_type_num ctx ty
  | History (_, id) -> (
      let ty = lookup_ty ctx id in
      match ty with None -> Error id | Some ty -> is_type_num ctx ty)
  | _ -> Ok false

let rec is_type_int : tc_context -> tc_type -> (bool, HString.t) result =
 fun ctx ty ->
  match ty with
  | Int _ | IntRange _ -> Ok true
  | RefinementType (_, (_, _, ty), _) -> is_type_int ctx ty
  | History (_, id) -> (
      let ty = lookup_ty ctx id in
      match ty with None -> Error id | Some ty -> is_type_int ctx ty)
  | _ -> Ok false
...
let rec is_type_real_or_int : tc_context -> tc_type -> (bool, HString.t) result
 ...
 fun ctx ty ->
  match ty with
  | Real _ | Int _ | IntRange _ -> Ok true
  | RefinementType (_, (_, _, ty), _) -> is_type_real_or_int ctx ty
  | History (_, id) -> (
      let ty = lookup_ty ctx id in
      match ty with None -> Error id | Some ty -> is_type_real_or_int ctx ty)
  | _ -> Ok false

let rec is_type_int_or_machine_int :
    tc_context -> tc_type -> (bool, HString.t) result =
 fun ctx ty ->
  match ty with
  | Int _ | UInt8 _ | UInt16 _ | UInt32 _ | UInt64 _ | Int8 _ | Int16 _
  | Int32 _ | Int64 _ | IntRange _ ->
      Ok true
...
let rec is_type_unsigned_machine_int :
    tc_context -> tc_type -> (bool, HString.t) result =
 fun ctx ty ->
  match ty with
  | UInt8 _ | UInt16 _ | UInt32 _ | UInt64 _ -> Ok true
...
let rec is_type_signed_machine_int :
    tc_context -> tc_type -> (bool, HString.t) result =
 fun ctx ty ->
  match ty with
  | Int8 _ | Int16 _ | Int32 _ | Int64 _ -> Ok true
...
let is_type_machine_int : tc_context -> tc_type -> (bool, HString.t) result =
 fun ctx ty ->
...
let rec is_machine_type_of_associated_width :
    tc_context -> tc_type * tc_type -> (bool, HString.t) result =
 fun ctx tys ->
  match tys with
  | Int8 _, UInt8 _
  | Int16 _, UInt16 _
  ...
  | UInt64 _, UInt64 _ ->
      Ok true
  | RefinementType (_, (_, _, ty1), _), ty2 ->
      is_machine_type_of_associated_width ctx (ty1, ty2)
  | ty1, RefinementType (_, (_, _, ty2), _) ->
      is_machine_type_of_associated_width ctx (ty1, ty2)
  | ty1, History (_, id) -> (
      let ty = lookup_ty ctx id in
      match ty with
...
let rec type_contains_subrange ctx = function
  ...
  | TupleType (_, tys) | GroupType (_, tys) ->
      List.fold_left
        (fun acc ty -> acc || type_contains_subrange ctx ty)
        false tys
  | RecordType (_, _, tys) ->
      List.fold_left
        (fun acc (_, _, ty) -> acc || type_contains_subrange ctx ty)
        false tys
  | ArrayType (_, (ty, _)) -> type_contains_subrange ctx ty
  | TArr (_, ty1, ty2) ->
      type_contains_subrange ctx ty1 || type_contains_subrange ctx ty2
  | History (_, id) -> (
  ...
  | UserType (_, ty_args, id) -> (
      ...
      | Some ty -> type_contains_subrange ctx ty
      | None -> assert false)
  | Bool _ | Int _ | Real _ | EnumType _ | UInt8 _ | UInt16 _ | UInt32 _
  | UInt64 _ | Int8 _ | Int16 _ | Int32 _ | Int64 _ | AbstractType _ ->
      false

let rec type_contains_enum_or_subrange ctx = function
  | LA.IntRange _ | EnumType _ -> true
  | RefinementType (_, (_, _, ty), _) -> type_contains_enum_or_subrange ctx ty
  | TupleType (_, tys) | GroupType (_, tys) ->
      List.fold_left
        (fun acc ty -> acc || type_contains_enum_or_subrange ctx ty)
        false tys
  | RecordType (_, _, tys) ->
      List.fold_left
        (fun acc (_, _, ty) -> acc || type_contains_enum_or_subrange ctx ty)
        false tys
  | ArrayType (_, (ty, _)) -> type_contains_enum_or_subrange ctx ty
  | TArr (_, ty1, ty2) ->
      type_contains_enum_or_subrange ctx ty1
      || type_contains_enum_or_subrange ctx ty2
  | History (_, id) -> (
      match lookup_ty ctx id with
  ...
  | UserType (_, ty_args, id) -> (
      ...
      | Some ty -> type_contains_enum_or_subrange ctx ty
      | None -> assert false)
  | Bool _ | Int _ | Real _ | UInt8 _ | UInt16 _ | UInt32 _ | UInt64 _ | Int8 _
  | Int16 _ | Int32 _ | Int64 _ | AbstractType _ ->
      false
...
let rec type_contains_ref ctx = function
  ...
  | RecordType (_, _, tys) ->
      List.fold_left
        (fun acc (_, _, ty) -> acc || type_contains_ref ctx ty)
        false tys
  ...
  | UserType (_, ty_args, id) -> (
      ...
      | Some ty -> type_contains_ref ctx ty
      | None -> false)
  | Bool _ | Int _ | Real _ | EnumType _ | IntRange _ | UInt8 _ | UInt16 _
...
let rec type_contains_enum_subrange_reftype ctx = function
  | LA.IntRange _ | EnumType _ | RefinementType _ -> true
  | TupleType (_, tys) | GroupType (_, tys) ->
      List.fold_left
        (fun acc ty -> acc || type_contains_enum_subrange_reftype ctx ty)
        false tys
  | RecordType (_, _, tys) ->
      List.fold_left
        (fun acc (_, _, ty) ->
          acc || type_contains_enum_subrange_reftype ctx ty)
        false tys
  | ArrayType (_, (ty, _)) -> type_contains_enum_subrange_reftype ctx ty
  | TArr (_, ty1, ty2) ->
      type_contains_enum_subrange_reftype ctx ty1
      || type_contains_enum_subrange_reftype ctx ty2
  | History (_, id) -> (
      match lookup_ty ctx id with
...
let rec type_contains_abstract ctx = function
  | LA.UserType (_, ty_args, id) -> (
      match lookup_ty_syn ctx id ty_args with
      | Some (AbstractType _) | None -> true
      | Some ty -> type_contains_abstract ctx ty)
  | RefinementType (_, (_, _, ty), _) -> type_contains_abstract ctx ty
  | TupleType (_, tys) | GroupType (_, tys) ->
      List.fold_left
        (fun acc ty -> acc || type_contains_abstract ctx ty)
        false tys
  | RecordType (_, _, tys) ->
      List.fold_left
        (fun acc (_, _, ty) -> acc || type_contains_abstract ctx ty)
        false tys
  | ArrayType (_, (ty, _)) -> type_contains_abstract ctx ty
  | TArr (_, ty1, ty2) ->
      type_contains_abstract ctx ty1 || type_contains_abstract ctx ty2
  | History (_, id) -> (
      match lookup_ty ctx id with
      | Some ty -> type_contains_abstract ctx ty
      | _ -> assert false)
...
let rec type_contains_array ctx = function
  ...
  | RecordType (_, _, tys) ->
      List.fold_left
        (fun acc (_, _, ty) -> acc || type_contains_array ctx ty)
        false tys
  | TArr (_, ty1, ty2) ->
      type_contains_array ctx ty1 || type_contains_array ctx ty2
  | History (_, id) -> (
      match lookup_ty ctx id with
...
let rec ty_vars_of_expr ctx node_name expr =
  let call = ty_vars_of_expr ctx node_name in
  match expr with
  (* Node calls *)
  | LA.Call (_, tys, _, es) ->
      SI.union
        (SI.flatten (List.map (ty_vars_of_type ctx node_name) tys))
  ...
  | Quantifier (_, _, qs, e) ->
      SI.diff (call e)
        (SI.flatten
           (List.map (fun (_, _, ty) -> ty_vars_of_type ctx node_name ty) qs))
  | Ident (_, id) -> (
      match lookup_ty ctx id with
  ...
  | TernaryOp (_, _, e1, e2, e3) ->
      call e1 |> SI.union (call e2) |> SI.union (call e3)
  | ConvOp (_, _, e) -> call e
  | CompOp (_, _, e1, e2) -> call e1 |> SI.union (call e2)
  (* Structured expressions *)
  | RecordExpr (_, _, _, flds) ->
      SI.flatten (List.map call (snd (List.split flds)))
  ...
  | Activate (_, _, e1, e2, es) ->
      SI.flatten (call e1 :: call e2 :: List.map call es)
  ...
  | AnyOp (_, (_, i, _), e1, Some e2) ->
      SI.diff (SI.union (call e1) (call e2)) (SI.singleton i)
  (* Temporal operators *)
  | Pre (_, e) -> call e
...
and ty_vars_of_type ctx node_name ty =
  ...
  | UserType (_, ty_args, id) -> (
      ...
      | Some ty -> ty_vars_of_type ctx node_name ty
      | None -> SI.empty)
  | RefinementType (_, (_, _, ty), e) | ArrayType (_, (ty, e)) ->
  ...
  | AbstractType (_, id) -> (
      match
        ( lookup_node_ty_vars ctx node_name,
          lookup_contract_ty_vars ctx node_name )
      with
      | None, None -> SI.empty
      | Some tvars, _ | _, Some tvars ->
          if List.mem id tvars then SI.singleton id else SI.empty)
  | History _ | Int _ | Int8 _ | Int16 _ | Int32 _ | Int64 _ | UInt8 _
  | UInt16 _ | UInt32 _ | UInt64 _ | Bool _ | IntRange _ | Real _ | EnumType _
    ->
      SI.empty
