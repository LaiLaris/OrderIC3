...
let error_message kind =
  match kind with
  | Unknown s -> s
  | ConstantOutOfSubrange i ->
      "Constant "
      ^ HString.string_of_hstring i
      ^ " is assigned a value outside of its subrange type"

let inline_error pos kind =
  Error (`LustreAbstractInterpretationError (pos, kind))

let unwrap result = match result with Ok r -> r | Error _ -> assert false

module IMap = HString.HStringMap

type context = LA.lustre_type IMap.t IMap.t
(** Context from a node identifier to a map of its variable identifiers to their
    inferred subrange bounds *)
...
let union a b =
  IMap.union
    (fun _ n1 n2 -> Some (IMap.union (fun _ _ i2 -> Some i2) n1 n2))
    a b

let get_type ctx node_name id =
  match IMap.find_opt node_name ctx with
  | Some node_ctx -> (
      match IMap.find_opt id node_ctx with Some ty -> Some ty | None -> None)
  | None -> None
...
let rec merge_types a b =
  ...
  | IntRange (_, l1, u1), IntRange (_, l2, u2) ->
      let lower =
        match (l1, l2) with
        | Some (Const (_, Num l1)), Some (Const (_, Num l2)) ->
            ...
            let l =
              HString.mk_hstring (Numeral.string_of_numeral (Numeral.min l1 l2))
            in
      ...
      let upper =
        ...
        | Some (Const (_, Num u1)), Some (Const (_, Num u2)) ->
            ...
            let u =
              HString.mk_hstring (Numeral.string_of_numeral (Numeral.max u1 u2))
            in
...
let rec restrict_type_by ty restrict =
  match (ty, restrict) with
  | LA.ArrayType (_, (t1, e)), LA.ArrayType (_, (t2, _)) ->
  ...
  | IntRange (_, l1, u1), IntRange (_, l2, u2) ->
      ...
      if lower = None && upper = None then (Int dpos, is_restricted)
...
let rec interpret_program ty_ctx gids = function
  | [] -> empty_context
  | h :: t ->
      union (interpret_decl ty_ctx gids h) (interpret_program ty_ctx gids t)

and interpret_contract node_id ctx ty_ctx contract =
  let ty_ctx =
    TC.tc_ctx_of_contract ~ignore_modes:true ty_ctx Ghost node_id contract
    |> unwrap |> fst
  in
  List.fold_left
    (fun acc eqn -> union acc (interpret_contract_eqn node_id acc ty_ctx eqn))
    ctx (snd contract)

and interpret_contract_eqn node_id ctx ty_ctx = function
  | LA.GhostConst _ -> empty_context
  | Assume _ | Guarantee _ | Mode _ | ContractCall _ | AssumptionVars _ ->
      empty_context
  | GhostVars (_, GhostVarDec (_, tis), rhs) ->
      let eqns = List.init (Ctx.arity_of_expr ty_ctx rhs) (fun p -> (rhs, p)) in
      List.fold_left2
        (fun acc (_, i, ty) (expr, p) ->
          let restrict_ty =
            interpret_expr_by_type node_id ctx ty_ctx ty p expr
          in
          let ty1, is_restricted = restrict_type_by ty restrict_ty in
          if is_restricted then add_type acc node_id i ty1 else acc)
        ctx tis eqns

and interpret_decl ty_ctx gids = function
  | LA.TypeDecl _ | ConstDecl _ -> empty_context
  | NodeDecl (_, decl) | FuncDecl (_, decl) -> interpret_node ty_ctx gids decl
...
and interpret_node ty_ctx gids (id, _, _, ps, ins, outs, locals, items, contract)
  ...
  let ty_ctx =
    GeneratedIdentifiers.StringMap.fold
      (fun id ty ctx -> Ctx.add_ty ctx id ty)
      gids_node.GeneratedIdentifiers.locals ty_ctx
  in
  let eqns =
    List.fold_left
      (fun acc -> function
        ...
        | AnnotMain _ | AnnotProperty _ -> acc
        (* Shouldn't be possible *)
        | LA.IfBlock _ | LA.FrameBlock _ -> assert false)
      [] items
  in
  let eqn_ctx =
    List.fold_left
...
and interpret_eqn node_id ctx ty_ctx lhs rhs =
  let struct_items = match lhs with StructDef (_, items) -> items in
  let eqns = List.init (Ctx.arity_of_expr ty_ctx rhs) (fun p -> (rhs, p)) in
  List.fold_left2
    (fun acc lhs (expr, p) ->
      match lhs with
      | LA.SingleIdent (_, id) ->
          ...
          let restrict_ty =
            interpret_expr_by_type node_id ctx ty_ctx ty1 p expr
          in
          let ty, is_restricted = restrict_type_by ty1 restrict_ty in
          if is_restricted then add_type acc node_id id ty else acc
      | LA.ArrayDef (_, array, indices) ->
          let array_type = Ctx.lookup_ty ty_ctx array |> get in
          let array_type =
            TC.expand_type_syn_reftype_history ty_ctx array_type |> unwrap
          in
          let ty_ctx, ty1, sizes =
            List.fold_left
              (fun (acc, ty, sizes) idx ->
                match ty with
                | LA.ArrayType (_, (idx_ty, size)) ->
                    (Ctx.add_ty acc idx (Int dpos), idx_ty, size :: sizes)
                | _ -> assert false)
              (ty_ctx, array_type, []) indices
          in
          let restrict_ty =
            interpret_expr_by_type node_id ctx ty_ctx ty1 p expr
          in
          ...
          let ty =
            List.fold_left
              ...
              ty sizes
          in
          if is_restricted then add_type acc node_id array ty else acc
      | _ -> assert false)
    ctx struct_items eqns
...
and interpret_expr_by_type node_id ctx ty_ctx ty proj expr : LA.lustre_type =
            ...
            let t =
              List.fold_left
                (fun acc e ->
                  ...
                  merge_types acc t')
                (interpret_expr_by_type node_id ctx ty_ctx t 0 e)
                es
            in
  ...
  | IntRange (pos, Some (Const (_, Num l1)), None) as t -> (
      ...
      | None, Some r2 -> subrange_from_bounds pos l1 r2
      | _ -> t)
  | IntRange (pos, None, Some (Const (_, Num r1))) as t -> (
      ...
      | Some l2, None -> subrange_from_bounds pos l2 r1
      | None, Some r2 ->
          let r = Numeral.min r1 r2 in
...
and interpret_structured_expr f node_id ctx ty_ctx ty proj expr =
  let infer e =
    let ty, _ = TC.infer_type_expr ty_ctx (Some node_id) e |> unwrap in
  ...
  | None -> (
      ...
      | RecordProject (_, e, idx) -> (
          ...
          let parent_ty =
            interpret_expr_by_type node_id ctx ty_ctx parent_ty proj e
          in
          let parent_ty =
            TC.expand_type_syn_reftype_history ty_ctx parent_ty |> unwrap
          in
          match parent_ty with
          | LA.RecordType (_, _, idents) ->
              let _, _, t =
                List.find (fun (_, i, _) -> HString.equal i idx) idents
              in
              t
          | Bool _ | Int _ | UInt8 _ | UInt16 _ | UInt32 _ | UInt64 _ | Int8 _
          | Int16 _ | Int32 _ | Int64 _ | IntRange _ | Real _ | UserType _
          | AbstractType _ | TupleType _ | GroupType _ | ArrayType _
...
and interpret_int_expr node_id ctx ty_ctx proj expr =
  ...
  | RecordProject (_, e, p) -> (
      ...
      | LA.RecordType (_, _, nested) ->
          let _, _, ty =
            List.find (fun (_, id, _) -> HString.equal id p) nested
          in
          extract_bounds_from_type ty
      | Bool _ | Int _ | UInt8 _ | UInt16 _ | UInt32 _ | UInt64 _ | Int8 _
      | Int16 _ | Int32 _ | Int64 _ | IntRange _ | Real _ | UserType _
      | AbstractType _ | TupleType _ | GroupType _ | ArrayType _ | EnumType _
      | TArr _ | RefinementType _ | History _ ->
          assert false)
  | TupleProject (_, e, idx) -> (
      match infer e with
      | TupleType (_, nested) ->
  ...
  | Const (_, const) -> (
      ...
      | Num x ->
          ...
          (Some v, Some v)
      | Dec _ -> assert false)
  | UnaryOp (_, op, e) -> interpret_int_unary_expr node_id ctx ty_ctx op proj e
  | BinaryOp (_, op, e1, e2) ->
...
and interpret_int_unary_expr node_id ctx ty_ctx op proj e =
  ...
  match op with
  | Uminus ->
      let l = match l with Some l -> Some (Numeral.neg l) | _ -> None in
      let r = match r with Some r -> Some (Numeral.neg r) | _ -> None in
      (r, l)
  | _ -> assert false
...
and interpret_int_binary_expr node_id ctx ty_ctx proj op e1 e2 =
  ...
  let template op =
    let l =
      match (l1, l2) with Some l1, Some l2 -> Some (op l1 l2) | _ -> None
    in
    let r =
      match (r1, r2) with Some r1, Some r2 -> Some (op r1 r2) | _ -> None
    in
    (l, r)
  in
  match op with
...
let expr_opt_lte e1 e2 =
  ...
  | Some (LA.Const (_, Num l1)) -> (
      ...
      | Some (LA.Const (_, Num l2)) ->
          int_of_string (HString.string_of_hstring l1)
          <= int_of_string (HString.string_of_hstring l2)
      | _ ->
          assert false
          (* Not possible as we require subranges to have concrete bounds *))
  | _ ->
      assert
        false (* Not possible as we require subranges to have concrete bounds *)
...
let rec compare_ranges id pos_map actual_ty inferred_range =
  ...
  | LA.IntRange (_, e1, e2) -> (
      ...
      | LA.IntRange (_, e3, e4) ->
          if expr_opt_lte e3 e1 && expr_opt_gte e4 e2 && expr_opt_lte e1 e2 then
            R.ok ()
          else error ()
      | _ -> R.ok ())
  | Int _ -> (
...
let rec most_general_int_ty = function
  ...
  | LA.GroupType (pos, types) | TupleType (pos, types) ->
      let types = List.map most_general_int_ty types in
      LA.TupleType (pos, types)
  | RecordType (pos, id, tis) ->
      let tis =
        List.map (fun (p, id, ty) -> (p, id, most_general_int_ty ty)) tis
      in
      RecordType (pos, id, tis)
...
and check_global_const_subrange ty_ctx ctx pos_map =
  ...
  in
  IMap.fold
    (fun id inferred_range acc ->
      let actual_ty = Ctx.lookup_ty ty_ctx id |> get in
      let actual_ty =
        TC.expand_type_syn_reftype_history ty_ctx actual_ty |> unwrap
      in
