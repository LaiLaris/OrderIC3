...
type error_kind = MisplacedNodeItemError of A.node_item
...
let error_message error =
  ...
  | MisplacedNodeItemError ni -> (
      ...
      | AnnotProperty _ ->
          "Property annotations are not allowed inside if blocks."
...
let unwrap = function Ok r -> r | Error _ -> assert false
...
let mk_fresh_ib_oracle pos expr_type =
  ...
  let gids = { (GI.empty ()) with ib_oracles = [ (name, expr_type) ] } in
  (nexpr, gids)

let rec update_if_position_info node_id ni =
  match ni with
...
let update_recursive_array_locals map lhs expr =
  ...
  | A.StructDef (_, [ ArrayDef (_, var1, inds1) ]) -> (
      let matching_lhs =
        LhsMap.bindings map |> List.map fst |> List.map fst
        |> List.find_opt (fun x ->
               match x with
               | A.StructDef (_, [ ArrayDef (_, var2, _) ]) when var1 = var2 ->
                   true
               | _ -> false)
      in
...
(** Converts an if block to a map of trees (creates a tree for each equation
    LHS) *)
  ...
  let rec helper ib trees conds =
    ...
    | A.IfBlock (pos, cond, ni :: nis, nis') -> (
        ...
        | A.Body (Equation (_, lhs, expr)) ->
            ...
            let trees =
              LhsMap.update
                (lhs, AH.pos_of_expr expr)
                (fun tree ->
                  match tree with
                  | Some tree ->
                      Some (add_eq_to_tree (conds @ [ (true, cond) ]) expr tree)
                  | None ->
                      Some
                        (add_eq_to_tree
                           (conds @ [ (true, cond) ])
        ...
        | A.IfBlock _ ->
            ...
            helper (A.IfBlock (pos, cond, nis, nis')) res conds
    ...
    | A.IfBlock (pos, cond, [], ni :: nis) -> (
            ...
            let trees =
              LhsMap.update
                ...
                (fun tree ->
                  ...
                  | Some tree ->
                      Some
                        (add_eq_to_tree (conds @ [ (false, cond) ]) expr tree)
                  | None ->
                      Some
                        (add_eq_to_tree
                           (conds @ [ (false, cond) ])
        ...
        | A.IfBlock _ ->
            ...
            helper (A.IfBlock (pos, cond, [], nis)) res conds
        (* Misplaced frame block, annot main, or annot property *)
        | A.FrameBlock (pos, _, _, _)
        | A.Body (Assert (pos, _))
        | A.AnnotProperty (pos, _, _, _)
...
let rec fill_ite_with_oracles ctx expr ty =
  ...
  | Ident (p, s) when s = ib_oracle_tree -> (
      ...
      | A.Ident (_, name) ->
          (expr, gids, [ A.NodeVarDecl (p, (p, name, ty, ClockTrue)) ])
      | _ -> assert false (* not possible *))
  | _ -> (expr, GI.empty (), [])

(** Helper function to determine if two trees are equal *)
let rec trees_eq node1 node2 =
  match (node1, node2) with
  | Leaf (Some i), Leaf (Some j) -> (
      match AH.syn_expr_equal None i j with Ok n -> n | Error _ -> false)
  | Node (l1, _, r1), Node (l2, _, r2) -> trees_eq l1 l2 && trees_eq r1 r2
  | _ -> false
...
let rec simplify_tree node =
  ...
  | Node (i, str, j) ->
      ...
      if trees_eq i j then i else Node (i, str, j)
...
(** Helper function for 'desugar_node_item' that converts IfBlocks to a list of
    ITEs. There are a number of steps in this process.

    Precondition: Multiple assignment has been removed from if blocks.

    Steps: 1. Converting the if block to a list of trees modeling the ITEs. 2.
    Doing any possible simplication on the above trees. 3. Converting the trees
    to ITE expressions. 4. Filling in the ITE expressions with oracles where
    variables are undefined. 5. Returning lists of new local declarations,
    generated equations, and gids *)
let extract_equations_from_if node_id ctx ib =
  ...
  let tys =
    List.map
      (fun x ->
        match x with Some y -> y | None -> assert false (* not possible *))
      tys
  in
  ...
  let eqs =
    List.map2
      (fun (a, b) c -> A.Body (A.Equation (a, b, c)))
      (List.combine lhs_poss lhss)
      ites
  in
...
let rec desugar_node_item node_id ctx ni =
  match ni with
...
let desugar_node_decl ctx decl =
  match decl with
  | A.FuncDecl (s, (node_id, b, opac, nps, cctds, ctds, nlds, nis, co)) ->
      ...
      R.ok
        ( A.FuncDecl
            (s, (node_id, b, opac, nps, cctds, ctds, new_decls @ nlds, nis, co)),
          GI.StringMap.singleton node_id gids )
...
let desugar_if_blocks ctx sorted_node_contract_decls gids =
  ...
  let* res =
    R.seq (List.map (desugar_node_decl ctx) sorted_node_contract_decls)
  in
  let decls, gids2 = List.split res in
  let gids2 =
    List.fold_left (GI.StringMap.merge GI.union_keys2) GI.StringMap.empty gids2
  in
