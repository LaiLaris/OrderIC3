...
type index = Val of int | Unk of Lib.position * A.expr (* Unknown *)

let is_unknown = function Val _ -> false | Unk _ -> true
...
module G = Graph.Make (struct
  ...
  let pp_print_t =
    Lib.pp_print_pair A.pp_print_ident (Lib.pp_print_list print_r_index " ") " "
end)
...
let error_message = function
  | Unknown e -> e
  | ComplicatedExpr e ->
      "Couldn't determine well-foundedness of array definition because of \
       index expression '"
...
let rec process_items ctx ns = function
  | A.Body eqn :: tail ->
      let* eqn_graph, eqn_pos_map, eqn_count, eqn_len =
        process_equation ctx ns eqn
      in
      let* tail_graph, tail_pos_map, tail_count, tail_len =
        process_items ctx ns tail
      in
      let graph = union eqn_graph tail_graph in
      let map =
        StringMap.union (fun _ x _ -> Some x) eqn_pos_map tail_pos_map
      in
...
and process_equation ctx ns = function
  | A.Equation (_, A.StructDef (_, lhs), expr) -> process_lhs ctx ns 0 expr lhs
...
and process_lhs ctx ns proj expr = function
  | A.ArrayDef (pos, id, indices) :: tail ->
      ...
      let* expr_graph =
        process_expr (Some (List.rev indices)) ctx ns proj [] expr
      in
      let expr_graph = G.connect expr_graph (id, zero_list) in
      let* tail_graph, tail_pos_map, count, len =
        process_lhs ctx ns (proj + 1) expr tail
      in
...
and process_expr ind_vars ctx ns proj indices expr =
  ...
  match expr with
  (* Identifiers *)
  | A.Ident (_, id) -> R.ok (G.singleton (id, indices))
  | ModeRef _ -> empty_
  ...
  | GroupExpr (_, _, es) -> es |> List.map r |> List.fold_left union_ empty_
  (* Update of structured expressions *)
  | StructUpdate (_, e1, _, e2) -> union_ (r e1) (r e2)
  | ArrayConstr (_, e1, e2) -> union_ (r e1) (r e2)
  | ArrayIndex (p, e, idx) ->
      let n = match ind_vars with Some iv -> List.length iv | None -> 0 in
      ...
      if n = index_layers then
        ...
        | Some ind_vars' -> (
            ...
            | Ok idx' ->
                ...
                if
                  A.SI.cardinal idx_vars = 1
                  && A.SI.mem ind_var idx_vars
                  && not (AH.expr_contains_call idx)
                then process_expr ind_vars ctx ns proj (Val idx' :: indices) e
                else
...
let extract_unknown ids =
  let unknowns =
    List.filter_map
      ...
      ids
  in
  match unknowns with [] -> None | unk :: _ -> Some unk
...
let rec check_inductive_array_dependencies ctx ns = function
  ...
  | _ :: tail -> check_inductive_array_dependencies ctx ns tail
  | [] -> Ok ()
...
and check_node_decl ctx ns decl =
  ...
  let* _ =
    try Res.ok (G.topological_sort graph)
    with G.CyclicGraphException ids -> (
      ...
      | None ->
          ...
          let ids =
            List.map
              (fun idx ->
                Format.asprintf "%a" G.pp_print_vertex idx |> HString.mk_hstring)
              ids
          in
...
and add_wellfounded_edges idx_len graph =
  ...
  let vertices =
    List.filter (fun (_, offsets) -> List.length offsets = idx_len) vertices
  in
  let rec partition_by_id vertices =
    if vertices = [] then []
      ...
      let id_vertices, other =
        List.partition (fun (id', _) -> HString.equal id id') vertices
      in
      let id_verts = (id, indices) :: id_vertices in
      let id_verts =
        List.sort (fun (_, i1) (_, i2) -> 1 - lexiographic_order i1 i2) id_verts
      in
...
and add_offset_edges count graph =
  ...
  let mk_edges ((id, offsets) as v : G.vertex) =
    ...
    let nhbd = G.children graph (id, List.mapi (fun _ _ -> Val 0) offsets) in
    let nhbd_offset =
      List.map
        (fun (id', offsets') ->
          ( id',
  ...
  let rec loop n (vertices : G.vertex list) graph =
    if n <= 0 then graph
      ...
      let new_edges =
        List.fold_left (fun acc v -> union acc (mk_edges v)) G.empty vertices
      in
...
and add_init_edges idx_len graph =
  let vertices = G.get_vertices graph |> G.to_vertex_list in
  let sub_length_vertices =
    List.filter (fun (_, idxs) -> List.length idxs < idx_len) vertices
  in
  let parts ((id, offset) as v) =
    let filled_offset =
      List.init idx_len (fun i ->
          match List.nth_opt offset i with Some x -> x | None -> Val 0)
    in
    let filled_graph = G.singleton (id, filled_offset) in
    G.connect filled_graph v
  in
  List.fold_left (fun acc v -> G.union acc (parts v)) graph sub_length_vertices
