  ...
  | head :: tail ->
      let quad =
        ...
        | Ast.GhostConst dec ->
            let pos, (id, expr, typ) =
              match dec with
              | Ast.FreeConst (pos, _, _) | Ast.UntypedConst (pos, _, _) ->
                  Format.asprintf
                    "[contracts translator] %a: untyped ghost consts are \
                     unsupported"
                    pp_print_position pos
                  |> failwith
              | Ast.TypedConst (pos, id, expr, typ) -> (pos, (id, expr, typ))
            in
            ( ( blah "Contract constant definition" pos,
                (Ast.GhostVarDec (pos, [ (pos, id, typ) ]), expr) )
              :: equations,
              (blah "Contract constant declaration" pos, (id, expr, typ))
              :: locals,
              asserts,
              props )
       ...
       to "equations" once *)
        ...
        | Ast.Guarantee (pos, name, _, expr) ->
            ( equations,
              locals,
              asserts,
              (blah_opt "Guarantee" name pos, [], expr) :: props )
...
let fmt_node_decl fmt (ident, params, ins, outs, locals, items)
    (c_equations, c_locals, c_asserts, c_properties) =
  (* Header. *)
  Format.fprintf fmt
    "node %a%a (@.  @[<hov>%a@]@.) returns (@.  @[<hov>%a@]@.) ;@.@?"
    Ast.pp_print_ident ident Ast.pp_print_node_param_list params
    (pp_print_list Ast.pp_print_const_clocked_typed_ident " ;@ ")
    ins
    (pp_print_list Ast.pp_print_clocked_typed_ident " ;@ ")
    outs;

  (* Locals. *)
  let locals_empty, c_locals_empty = (locals = [], c_locals = []) in
  if (not locals_empty) || not c_locals_empty then (
    if locals_empty then Format.fprintf fmt "var@."
    else Format.fprintf fmt "  @[<v>%a@]@." Ast.pp_print_node_local_decl locals;

    if not c_locals_empty then
      c_locals
      |> List.iter (fun (blah, (id, _, typ)) ->
             Format.fprintf fmt "  -- %s@.  %a: %a ;@." blah Ast.pp_print_ident
               id Ast.pp_print_lustre_type typ));

  (* body. *)
  Format.fprintf fmt "let@.";
  Format.fprintf fmt "  @[<v>%a@]@.@?"
    (pp_print_list Ast.pp_print_node_item "@ ")
    items;

  if items <> [] then Format.fprintf fmt "@.";

  c_equations
  |> List.iter (fun (blah, (lhs, expr)) ->
         Format.fprintf fmt "  -- %s@.  %a = %a ;@." blah
           Ast.pp_print_contract_eq_lhs lhs Ast.pp_print_expr expr);
  if c_equations <> [] then Format.fprintf fmt "@.";

  c_asserts
  |> List.iter (fun (blah, ass) ->
         Format.fprintf fmt "  -- %s@.  %a@." blah Ast.pp_print_node_item
           (LustreAst.Body ass));
  if c_asserts <> [] then Format.fprintf fmt "@.";

  c_properties
  |> List.iter (fun (blah, lhs, rhs) ->
         Format.fprintf fmt "  -- %s@.  --%%PROPERTY " blah;
         (match lhs with
         | [] -> ()
         | _ ->
             Format.fprintf fmt "(not (%a)) or "
               (pp_print_list Ast.pp_print_expr " and ")
               lhs);
...
let rec fmt_declarations fmt = function
  | dec :: tail ->
      ...
      | Ast.ContractNodeDecl ({ Ast.start_pos = spos }, _) ->
          Format.asprintf
            "[contracts translator] %a: contract node declarations are \
             unsupported"
...
let translate_file file target =
  ...
  | Error e ->
      LustreReporting.fail_at_position_pt (LE.error_position e)
        (LE.error_message e)
