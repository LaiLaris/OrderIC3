...
let error_message error =
  match error with
  | MisplacedNodeItemError ni -> (
      match ni with
      | Body (Assert _) -> "Asserts are not allowed inside frame blocks."
      | FrameBlock _ ->
          "Frame blocks are not allowed inside other frame blocks."
      | AnnotMain _ -> "Main annotations are not allowed inside frame blocks."
      | AnnotProperty _ ->
          "Property annotations are not allowed inside frame blocks."
...
let warning_message warning =
  match warning with
  | UninitializedVariableWarning id ->
      "Uninitialized frame block variable " ^ HString.string_of_hstring id
  ...
  List.map
    (fun ni ->
      ...
      | A.Body (Equation (_, StructDef (_, [ SingleIdent (_, id) ]), expr)) ->
          if AH.has_unguarded_pre_no_warn expr then
            [ mk_warning pos (UninitializedVariableWarning id) ]
          else []
...
(** Parses an expression and replaces any ITE oracles with the 'fill' expression
    (which is stuttering, ie, 'pre variable'). *)
let rec fill_ite_helper frame_pos node_id lhs id fill = function
  ...
  | A.Ident (pos, i) ->
      ...
      if GI.var_is_iboracle i then (
        ...
        let frame_info =
          match HString.HStringHashtbl.find_opt pos_list_map node_id with
  ...
  | Arrow (a, e1, e2) ->
      Arrow
        ( a,
          fill_ite_helper frame_pos node_id lhs id fill e1,
          fill_ite_helper frame_pos node_id lhs id fill e2 )
  | Const _ as e -> e
  | ModeRef _ as e -> e
  | RecordProject (a, e, b) ->
      RecordProject (a, fill_ite_helper frame_pos node_id lhs id fill e, b)
  | ConvOp (a, b, e) ->
      ConvOp (a, b, fill_ite_helper frame_pos node_id lhs id fill e)
  | UnaryOp (a, b, e) ->
      UnaryOp (a, b, fill_ite_helper frame_pos node_id lhs id fill e)
  | When (a, e, b) ->
      When (a, fill_ite_helper frame_pos node_id lhs id fill e, b)
  | TupleProject (a, e, b) ->
      TupleProject (a, fill_ite_helper frame_pos node_id lhs id fill e, b)
  | Quantifier (a, b, c, e) ->
      Quantifier (a, b, c, fill_ite_helper frame_pos node_id lhs id fill e)
  | BinaryOp (a, b, e1, e2) ->
      BinaryOp
        ( a,
          b,
  ...
  | TernaryOp (a, b, e1, e2, e3) ->
      TernaryOp
        ( a,
          ...
          fill_ite_helper frame_pos node_id lhs id fill e1,
          fill_ite_helper frame_pos node_id lhs id fill e2,
          fill_ite_helper frame_pos node_id lhs id fill e3 )
  | GroupExpr (a, b, l) ->
      GroupExpr
        (a, b, List.map (fill_ite_helper frame_pos node_id lhs id fill) l)
  | Call (a, b, c, l) ->
      Call (a, b, c, List.map (fill_ite_helper frame_pos node_id lhs id fill) l)
  | Merge (a, b, l) ->
      Merge
        ( a,
          b,
          List.combine (List.map fst l)
            (List.map
               (fill_ite_helper frame_pos node_id lhs id fill)
               (List.map snd l)) )
  | RecordExpr (a, b, c, l) ->
      RecordExpr
        ( a,
  ...
  | RestartEvery (a, b, l, e) ->
      RestartEvery
        ( a,
          b,
          List.map (fill_ite_helper frame_pos node_id lhs id fill) l,
          fill_ite_helper frame_pos node_id lhs id fill e )
  | Activate (a, b, e, r, l) ->
      Activate
        ( a,
          b,
          (fill_ite_helper frame_pos node_id lhs id fill) e,
          (fill_ite_helper frame_pos node_id lhs id fill) r,
          List.map (fill_ite_helper frame_pos node_id lhs id fill) l )
  | Condact (a, e, r, b, l1, l2) ->
      Condact
        ( a,
          ...
          (fill_ite_helper frame_pos node_id lhs id fill) r,
          b,
          List.map (fill_ite_helper frame_pos node_id lhs id fill) l1,
          List.map (fill_ite_helper frame_pos node_id lhs id fill) l2 )
        ...
        ( a,
          ...
          List.map
            (function
              ...
              | Index (a, e) ->
                  Index (a, fill_ite_helper frame_pos node_id lhs id fill e))
            li,
          fill_ite_helper frame_pos node_id lhs id fill e2 )

(** Helper function to generate node equations when an initialized variable in
    the frame block is left undefined in the frame block body. *)
let generate_undefined_nes f_pos node_id nis ne =
  match ne with
  | A.Equation (pos, (StructDef (_, [ SingleIdent (_, id) ]) as lhs), init) -> (
      ...
      let res =
        List.find_opt
          (fun ni ->
            match ni with
            | A.Body (A.Equation (_, StructDef (_, [ SingleIdent (_, i) ]), _))
              when id = i ->
                true
            | A.Body (A.Equation (_, StructDef (_, [ ArrayDef (_, i, _) ]), _))
              when id = i ->
                true
            | _ -> false)
          nis
      ...
      | None ->
          ...
          R.ok
            [
              A.Body
                (A.Equation
                   (pos, lhs, Arrow (pos2, init, Pre (pos2, Ident (pos2, id)))));
            ])
              ...
              A.Body
                (A.Equation
                   ( pos,
                     lhs,
                     Arrow
                       (pos2, init, Pre (pos2, build_array_index (List.rev id2)))
                   ));
            ])
...
(** Helper function to generate node equations when a variable in the frame
    block var list is left undefined in the frame block body AND has no
    initialization. *)
  ...
  | _ -> (
      ...
      match
        List.find_opt
          (fun ne ->
            match ne with
            | A.Equation (_, StructDef (_, [ SingleIdent (_, i) ]), _)
              when i = var ->
                true
            | A.Equation (_, StructDef (_, [ ArrayDef (_, i, _) ]), _)
              when i = var ->
                true
            | _ -> false)
          nes
...
let fill_ite_oracles f_pos node_id nes ni =
  match ni with
  | A.Body
      (Equation (pos, (StructDef (_, [ SingleIdent (_, i) ]) as lhs), rhs_expr))
    -> (
      ...
      let lhs_init_e =
        Lib.find_map
          (fun ne ->
            match ne with
            | A.Equation (_, StructDef (_, [ SingleIdent (_, id) ]), init_expr)
              when id = i ->
         ...
         the body equation is not. So, we have to make the whole desugared equation recursive. *)
            | A.Equation
                ( _,
                  (StructDef (_, [ ArrayDef (_, id, inds1) ]) as lhs),
                  init_expr )
              when id = i ->
                let pos = AH.pos_of_expr init_expr in
                let rhs_expr =
                  List.fold_left
                    (fun acc ind -> A.ArrayIndex (pos, acc, Ident (pos, ind)))
                    rhs_expr inds1
                in
                Some (lhs, init_expr, rhs_expr)
            | _ -> None)
               ...
               (Equation
                  ( pos,
                    ...
                    A.Arrow
                      ( pos2,
                        init,
                        fill_ite_helper f_pos node_id lhs i
  ...
  | A.Body
      (Equation
         (pos, (StructDef (_, [ ArrayDef (_, i1, inds1) ]) as lhs), rhs_expr))
    -> (
      let pos2 = AH.pos_of_expr rhs_expr in
      (* Find initialization value *)
      let array_index =
        List.fold_left
          (fun expr j -> A.ArrayIndex (pos2, expr, A.Ident (pos2, j)))
          (A.Ident (pos2, i1))
          inds1
      in
      let init =
        Lib.find_map
          (fun ne ->
            match ne with
            | A.Equation (_, StructDef (_, [ ArrayDef (_, id, inds2) ]), expr)
              when id = i1 ->
         ...
         the initialization is not. So, we have to make the whole desugared equation recursive. *)
            | A.Equation (_, StructDef (_, [ SingleIdent (_, id) ]), expr)
              when id = i1 ->
               ...
               (Equation
                  ( pos,
                    ...
                    A.Arrow
                      ( pos2,
                        init,
                        fill_ite_helper f_pos node_id lhs i1
  ...
  | A.AnnotMain (pos, _) ->
      mk_error pos (MisplacedNodeItemError ni)

(** For each node item in frame block body: Fill in ITE oracles and initialize
    equations (RHS) when an initialization value is specified. For each
    initialization: Fill in an equation if one doesn't exist. For each variable
    that is neither initialized nor defined: Fill in an equation of the form 'y
    = pre y' (initially undefined) *)
let desugar_node_item node_id ni =
  match ni with
  ...
  | A.FrameBlock (pos, vars, nes, nis) ->
      ...
      let* nis2 =
        R.seq (List.map (generate_undefined_nes pos node_id nis) nes)
      in
      let nis2 = List.flatten nis2 in
      let* nis3 =
        R.seq
          (List.map (generate_undefined_nes_no_init node_id pos nes nis) vars)
      in
...
(** Desugars a declaration list to remove frame blocks. Node equations in the
    body are initialized with the provided initializations. If a frame block
    node equation has if statements with undefined branches, it fills the
    branches in by setting the variable equal to its value in the previous
    timestep. *)
let desugar_frame_blocks sorted_node_contract_decls =
  HString.HStringHashtbl.clear pos_list_map;
  let desugar_node_decl decl =
    match decl with
          ...
          ( A.NodeDecl
              ( s,
                ( node_id,
                  ...
                  cctds,
                  ctds,
                  List.flatten decls @ nlds,
                  List.flatten nis,
                  co ) ),
            warnings )
    (* Make sure there are no frame blocks in functions *)
    | A.FuncDecl (_, (_, _, _, _, _, _, _, nis, _)) -> (
        let contains_frame_block =
          List.find_opt
            (fun ni -> match ni with A.FrameBlock _ -> true | _ -> false)
            nis
        in
        match contains_frame_block with
        | Some (FrameBlock (pos, _, _, _) as fb) ->
            mk_error pos (MisplacedFrameBlockError fb)
        | _ -> R.ok (decl, []))
    | _ -> R.ok (decl, [])
  in
