...
let print_init_debug init =
  Debug.assump "  @[<hov 2>Initial predicate:@ %a@]@."
    (LustreExpr.pp_print_term_as_expr true)
    init
...
let map_vars f { init; trans } =
  { init = Term.map_state_vars f init; trans = Term.map_state_vars f trans }

let lustre_assumption { init; trans } =
  let mk_expr_of_term t =
    LustreExpr.mk_of_expr (LustreExpr.unsafe_expr_of_term t)
  in
  let bump_minus_one term = Term.bump_state (Numeral.of_int (-1)) term in
...
type 'a variable_filter_func =
  'a InputSystem.t -> Scope.t -> Var.t list -> Var.t list

type 'a pair_of_filters = 'a variable_filter_func * 'a variable_filter_func
type response = Success of t | Failure | Unknown
...
let get_output_svars in_sys scope =
  ...
  | Some { LustreNode.outputs } -> List.map snd (LustreIndex.bindings outputs)
  | None -> []
...
let has_assumptions in_sys scope =
  ...
  | Some { LustreNode.contract } -> (
      match contract with Some { assumes } -> assumes <> [] | None -> false)
  | None -> false
...
let filter_non_output in_sys scope =
  ...
  List.filter (fun v ->
      ...
      List.exists (fun o -> StateVar.equal_state_vars o sv) output_svars |> not)
...
let dump_footer fmt = Format.fprintf fmt "@]@.tel@.@."
...
let dump_assumption ?(prefix = "assume") fmt { init; trans } =
  ...
  if init == trans' then
    Format.fprintf fmt "@[<hov 2>%s@ %a;@]" prefix pp_print_lustre_expr init
  else
    Format.fprintf fmt "@[<hov 2>%s@ (%a)@ ->@ (%a);@]" prefix
...
let create_assumpion_trans fmt_assump sys solver vars fp prop =
  ...
  KEvent.log_uncond "  @[<hov 2>Transition predicate:@ %a@]" fmt_assump
    assump_trans';
...
let get_uf_symbol sv_to_ufs sv o =
  ...
  | Some lst -> (
      match List.nth_opt lst o with Some uf -> uf | None -> assert false)
  | None -> assert false
...
let mk_equalities ?(no_prev = false) (sys_svars, env_svars) sv_to_ufs f i j =
  ...
  if i = 0 then env_eqs
    ...
    in
    if no_prev then
      if sys_eqs = Term.t_true then env_eqs
      else Term.mk_and [ sys_eqs; env_eqs ]
    else
      let prev_env_eqs =
        env_svars
        |> List.map (fun sv -> mk_equality sv_to_ufs f sv (i - 1) (j - 1))
        |> Term.mk_and
      in
      if sys_eqs = Term.t_true then Term.mk_and [ prev_env_eqs; env_eqs ]
      else Term.mk_and [ sys_eqs; prev_env_eqs; env_eqs ]
    ...
    (fun acc sv ->
      let vars =
        List.init last (fun i ->
            Var.mk_state_var_instance sv (Numeral.of_int (i + c)))
      in
      List.rev_append vars acc)
    [] svars
  |> List.rev

let mk_forall_term ?disj ?(init_conds = Term.t_true) init assump_svars one_state
    sv_to_ufs k abduct =
  let mk_equalities' =
  ...
  in
  let cterms =
    if one_state then oterms' (k + 1) 0
    else if init then mk_equalities' 0 0 :: oterms' k 1
    else init_conds :: oterms' k 1
  in
  Term.mk_forall forall_vars (Term.mk_implies [ Term.mk_and cterms; abduct ])

let init_soln one_state solver assump_svars sv_to_ufs k system_unrolling abduct
    =
  let full_term =
    let qterm = mk_forall_term true assump_svars one_state sv_to_ufs k abduct in
    let sigma =
      SVM.fold
        (fun sv ufs acc ->
          let sv_sigma =
  ...
  let soln =
    let all_ufs =
      SVM.fold (fun _ ufs acc -> List.rev_append ufs acc) sv_to_ufs []
    ...
    SMTSolver.check_sat_and_get_term_values solver
      (fun _ term_values ->
        ...
        if one_state then
          let pred_at_0 =
            List.init (k + 1) (fun j -> mk_equalities' true 0 j) |> Term.mk_or
          in
          ...
          let trans =
            List.init k (fun j -> mk_equalities' false 1 (j + 1)) |> Term.mk_or
          in
          Some { init; trans })
      (fun _ -> None) (* If unsat. *)
      all_ufs
...
let iso_decomp one_state sys abd_solver uf_solver assump_svars sv_to_ufs pred
    init_conds k abduct =
  let sys_svars, env_svars = assump_svars in
  let all_forall_vars =
    List.rev_append
  ...
  let generalize_step_predicate pred_unrolling' init_conds' i =
    let forall_vars =
      ...
      |> List.filter (fun v ->
             ...
             if one_state then Numeral.equal offset (Numeral.of_int i) |> not
             else
  ...
  let distribute_and_generalize pred_unrolling init_conds' =
    let pred_unrolling, init_conds' =
      if one_state then (pred_unrolling, init_conds')
      else
        List.fold_left
            ...
            | QE.Invalid conds ->
                let pred_unrolling', init_conds' =
                  if i = 0 then
                    ...
                    let conds = conds |> Term.bump_state (Numeral.of_int i) in
                    ( Lib.list_apply_at
                        (fun p ->
                          Term.mk_and [ p; conds ]
                          |> SMTSolver.simplify_term abd_solver)
    ...
    in
    ( pred_unrolling
      |> List.mapi (fun i t -> Term.bump_state (Numeral.of_int (-i)) t)
      |> Term.mk_and
      |> SMTSolver.simplify_term abd_solver,
      init_conds' )
  in
  ...
  let rec loop pred' init_conds' iter =
            ...
            SVM.fold
              (fun sv ufs acc ->
                let add o j m =
                  ...
                  (v, Term.mk_uf uf []) :: m
                in
                if one_state then add 0 i acc else add 0 i (add 1 (i + 1) acc))
              sv_to_ufs []
          in
    ...
    let qterm =
      mk_forall_term ~disj:pred' ~init_conds:init_conds' false assump_svars
        one_state sv_to_ufs k abduct
    in

    SMTSolver.trace_comment uf_solver
      (Format.sprintf "Looking for new assignments (k=%d)" k);
    ...
    let res =
      let all_ufs =
        ...
        |> List.map (fun uf -> Term.mk_uf uf [])
      in
      SMTSolver.check_sat_and_get_term_values uf_solver
        (fun _ term_values ->
          (* If sat. *)
          let f t =
            match List.assoc_opt t term_values with
            ...
            | None -> assert false
          in
          let mk_equalities' = mk_equalities assump_svars sv_to_ufs f in
          let l, s, c = if one_state then (k + 1, 0, 0) else (k, 1, 1) in
          Some (List.init l (fun j -> mk_equalities' s (j + c)) |> Term.mk_or))
        (fun _ ->
          (* If unsat. *)
          None)
        all_ufs
    ...
    | Some sol ->
        ...
        if pred' == pred'' then (pred'', init_conds'')
        else (
          if one_state then (
          ...
          if max_iters > 0 && iter >= max_iters then (pred'', init_conds'')
          else loop pred'' init_conds'' (iter + 1))
  in
...
let cart_decomp one_state abd_solver assump_svars sys k system_unrolling abduct
  ...
  let sv_to_ufs, _ =
    ...
    List.fold_left
      (fun (map, id) sv ->
        let lst = List.init (k + 1) (fun o -> mk_uf_symbol sv id o) in
        (SVM.add sv lst map, id + 1))
      (SVM.empty, 0) (sys_svars @ env_svars)
  in

  let decomp { init; trans } =
    if one_state then
      let init, _ =
        iso_decomp one_state sys abd_solver uf_solver assump_svars sv_to_ufs
          init Term.t_true k abduct
      in
      ...
      let init =
        let premises =
          Term.mk_and
            (List.init k (fun i -> Term.bump_state (Numeral.of_int i) trans))
        in
        let forall_vars =
          List.rev_append
            (mk_forall_vars 0 sys_svars k |> List.rev)
  ...
  let soln =
    init_soln one_state uf_solver assump_svars sv_to_ufs k system_unrolling
      abduct
  in
...
let no_abduct =
  { term = Term.t_true; forall_varset = VS.empty; system_unrolling = [] }

let get_length { system_unrolling } = List.length system_unrolling

let generate_assumption_for_k_and_below one_state assump_svars last_abduct sys
    props k =
  ...
  let props_conj =
    props |> List.map (fun { Property.prop_term } -> prop_term) |> Term.mk_and
  in
  ...
  let compute_next_abduct { term; forall_varset; system_unrolling } =
    ...
    let forall_varset =
      let forall_varset =
        if one_state || n = 0 then forall_varset
        else
    ...
    let abduct =
      ...
      Abduction.abduce abd_solver forall_vars system_unrolling_term props_at_n
    in
    if abduct = Term.t_false then
      { term = Term.t_false; forall_varset; system_unrolling }
    else
      let term =
        Term.mk_and [ term; abduct ] |> SMTSolver.simplify_term abd_solver
      in
  ...
  let last_abduct =
    let rec iterate n current_abduct =
      if n <= 0 then current_abduct
      else
        let new_abduct = compute_next_abduct current_abduct in
        if new_abduct.term = Term.t_false then new_abduct
        else iterate (n - 1) new_abduct
    in
  ...
  Debug.assump "@[<hv>Initial abduct:@ @[<hv>%a@]@]@." Term.pp_print_term
    abduct_term;

  let res =
    if abduct_term = Term.t_false then Failure
    else if k = 0 then Success { init = abduct_term; trans = Term.t_true }
    else
      let system_unrolling_term =
        let { system_unrolling } = last_abduct in
        Term.mk_and system_unrolling
      in
      cart_decomp one_state abd_solver assump_svars sys k system_unrolling_term
        abduct_term
  in
...
let satisfy_input_requirements in_sys param top =
  let model_contains_assert =
    ...
    |> List.exists (fun { LustreNode.asserts } -> asserts <> [])
  in
...
let generate_assumption ?(one_state = false) analyze in_sys param sys =
  ...
  | invalid, _ ->
    ...
    *)
      let c_sys, const_svars = TSys.enforce_constantness_via_equations sys in
      ...
      let assump_svars =
        ...
        in
        let input_svars =
          if SVS.is_empty user_selection then input_svars
          else List.filter (fun sv -> SVS.mem sv user_selection) input_svars
        in
        if one_state then ([], input_svars)
        else
          let output_svars =
            ...
            |> List.filter (fun sv ->
                   List.exists
                     (fun o -> StateVar.equal_state_vars o sv)
                     in_sys_output_svars)
          in
          let output_svars =
            if SVS.is_empty user_selection then output_svars
            else List.filter (fun sv -> SVS.mem sv user_selection) output_svars
          in
      ...
      let map_back_const_input v =
        let sv = Var.state_var_of_state_var_instance v in
        if List.mem sv const_svars then Var.mk_const_state_var sv else v
      in

      let rec loop props last_abduct k =
        List.iter (fun sv -> StateVar.set_const false sv) const_svars;
        let res, last_abduct =
          generate_assumption_for_k_and_below one_state assump_svars last_abduct
            c_sys props k
        in
        ...
        | Success { init; trans } -> (
            ...
            props
            |> List.iter (fun { Property.prop_name = n } ->
            ...
            | _, [], [] ->
                ...
                if
                  (not (only_current_state assump))
                  && not (satisfy_input_requirements in_sys param scope)
                then
                  KEvent.log L_warn
                    "Assumption may contain constraints over underspecified \
                     outputs";

                KEvent.log L_note "Generated assumption:@,%a"
                  (dump_assumption ~prefix:"")
                  assump;
               ...
               TODO: Allow previous assumptions with only current values *)
                ...
                if only_current_state assump && no_previous_temporal_assumption
                then Success assump
                else (
                  ...
                  let result =
                    let vars_at_0 =
                      TSys.vars_of_bounds ~with_init_flag:true c_sys'
                    ...
                    realizability_check Term.vars_of_term
                      (* It doesn't matter since all variables are controllable *)
                      c_sys' controllable_vars_at_0 vars_at_1
                      controllable_vars_at_1
                  in
...
let generate_assumption_vg in_sys sys var_filters prop =
  ...
  *)
  ...
  let sys' =
    let _, init_eq, trans_eq = TSys.init_trans_open sys in
    let init_eq = Term.mk_and [ init_eq; prop.Property.prop_term ] in
    let trans_eq =
      let prop_at_1 = Term.bump_state Numeral.one prop.Property.prop_term in
  ...
  let result =
    let include_invariants =
      ...
      *)
      not (has_assumptions in_sys scope)
    in
    realizability_check ~include_invariants Term.vars_of_term
      (* It doesn't matter since all variables are controllable *)
  ...
  | Realizable fp ->
      ...
      let init =
        create_assumpion_init fmt_assump sys solver init_vars fp prop
      in
      ...
      if Term.equal init Term.t_false then Unknown
        ...
        let trans_vars =
          List.rev_append
            (vf_pre in_sys scope vars_at_0)
            (vf_curr in_sys scope vars_at_1)
        in

        let trans =
          create_assumpion_trans fmt_assump sys solver trans_vars fp prop
        in
