...
let raise_no_new_definition_exc = function
  (* Raise exception *)
  | { definitions_allowed = Some (pos, msg) } -> fail_at_position pos msg
...
let push_scope ({ scope; ident_expr_map } as ctx) ident =
  {
    ctx with
    scope = ident :: scope;
...
let push_contract_scope ({ contract_scope; ident_expr_map } as ctx) ident =
  {
    ctx with
...
let pop_scope = function
  ...
  | { scope = [] } | { ident_expr_map = [] } ->
      raise (Invalid_argument "pop_scope")
...
let pop_contract_scope = function
  ...
  | {
      ...
      { ctx with contract_scope; ident_expr_map }
...
let bounds_of_index index =
  List.fold_left
    (fun acc -> function
      ...
      | D.ArrayIntIndex i -> E.Fixed (E.mk_int_expr (Numeral.of_int i)) :: acc
      | _ -> acc)
    [] index
...
let create_node = function
  ...
  | { node = Some _ } -> (
      (* Fail *)
      function
      | _ -> raise (Invalid_argument "create_node"))
...
let current_node_map = function
  | { node = Some node } as ctx -> fun f -> { ctx with node = Some (f node) }
  | ctx -> fun _ -> ctx

(** Returns the modes of the current node. *)
let current_node_modes = function
  | { node = None } -> None
  | { node = Some { N.contract } } ->
      Some (match contract with None -> None | Some { C.modes } -> Some modes)
...
let create_function = function
  ...
  | { node = Some _ } -> (
      (* Fail *)
      function
      | _ -> raise (Invalid_argument "create_function"))
  (* Not in a node or function *)
  | { ident_type_map; ident_expr_map; expr_abs_map } as ctx -> (
      function
      | _ (* ident *) ->
...
let add_free_constant ctx ident vt = IT.add ctx.free_constants ident vt

let get_free_constants ctx =
  IT.fold (fun i vt acc -> (i, vt) :: acc) ctx.free_constants []

let free_constant_expr_of_ident ctx ident =
  IT.find ctx.free_constants ident |> D.map E.mk_free_var
...
let add_expr_for_ident ?(shadow = false) ({ ident_expr_map } as ctx) ident expr
  ...
  if IT.mem (List.hd ident_expr_map) ident then
  ...
  then
    raise
      (Invalid_argument ("add_expr_for_ident: " ^ I.string_of_ident false ident))
  else
...
(* Add a binding of an identifier to an expression to context *)
let[@ocaml.warning "-27"] add_expr_for_indexed_ident ?(shadow = false)
    ({ ident_expr_map } as ctx) ident index expr =
  try
    (* Must have at least a map for the top level *)
    assert (ident_expr_map <> []);
...
let rec get_nodes ({ prev; nodes } as ctx) =
  if ctx == prev then nodes else nodes @ get_nodes prev
...
let contract_node_decl_of_ident { contract_nodes } ident =
  try
    ...
    List.find (function _, (i, _, _, _, _) -> i = ident) contract_nodes
...
let add_contract_node_decl_to_context ({ contract_nodes } as ctx)
    (pos, ((ident, _, _, _, _) as contract_node_decl)) =
  if
    (* Check if contract of with the same identifier exists *)
    List.exists (function _, (i, _, _, _, _) -> i = ident) contract_nodes
  then fail_at_position pos "Contract node already defined"
  else
...
let set_state_var_source = function
  ...
  | { node = None } -> (
      function
      | _ -> ( function _ -> raise (Invalid_argument "set_state_var_source")))

(* Create a state variable for from an indexed state variable in a
   scope *)
let mk_state_var ?is_input ?is_const ?for_inv_gen ?(shadow = false) ctx scope
    ident index state_var_type state_var_source =
  (* Concatenate identifier and indexes *)
  let state_var_name =
    Format.asprintf "%a%a" (I.pp_print_ident true) ident (D.pp_print_index true)
      (* Filter out array indexes *)
      (List.filter
         (function
           | D.ArrayVarIndex _ | D.ArrayIntIndex _ -> false
           | D.RecordIndex _ | D.TupleIndex _ | D.ListIndex _
           | D.AbstractTypeIndex _ ->
               true)
         index)
  in
  ...
  *)
  ...
  let ctx =
    ...
    if index = D.empty_index then
      ...
      D.singleton D.empty_index expr |> add_expr_for_ident ~shadow ctx ident
    else
...
let expr_of_ident { ident_expr_map } ident = expr_of_ident' ident ident_expr_map

(* Return true if the identifier denotes an output or local variable *)
let assignable_state_var_of_ident = function
  (* No node in context *)
  | { node = None } -> (
      function _ -> raise (Invalid_argument "assignable_state_var_of_ident"))
            ...
            D.map
              (fun e ->
                ...
                (* Expression is not a variable *)
                  else raise (Invalid_argument "assignable_state_var_of_ident"))
              expr
...
let add_node_oracle = function
  ...
  | { node = Some node } as ctx ->
      fun state_var ->
        ...
        {
          ctx with
          node = Some { node with N.oracles = state_var :: node.N.oracles };
        }

(* Create a fresh state variable as an oracle input *)
let mk_fresh_oracle ?is_input ?is_const ?for_inv_gen ?(bounds = [])
    ({ node; definitions_allowed; fresh_oracle_index } as ctx) state_var_type =
  ...
  | _ -> (
      ...
      | Some _ ->
          ...
          let state_var, ctx =
            mk_state_var ?is_input ?is_const ?for_inv_gen ctx
              (scope_of_node ctx @ I.reserved_scope)
              (I.push_index I.oracle_ident !fresh_oracle_index)
              D.empty_index state_var_type (Some N.Oracle)
          in
...
let mk_fresh_oracle_for_state_var ?bounds ctx state_var =
  (* Create fresh oracle *)
  let state_var', ctx =
    mk_fresh_oracle ~is_const:true (* ?bounds *)
      ~bounds:
        (let b =
           try SVT.find ctx.state_var_bounds state_var with Not_found -> []
         in
...
let[@ocaml.warning "-27"] close_expr ?(bounds = []) ?original pos
      ...
      E.map_vars_expr
        (fun var ->
          ...
          then (
            ...
            let state_var', ctx =
              mk_fresh_oracle_for_state_var !rctx state_var
            in
            rctx := ctx;
...
let has_var_index_t vi t = Var.VarSet.mem vi (Term.vars_of_term t)
let has_var_index vi expr = Var.VarSet.mem vi (E.vars_of_expr expr)
  ...
  List.map
    (fun bnd ->
      ...
      | E.Bound _ | E.Fixed _ | E.Unbound _ -> (
          ...
          try
            let t =
              Term.TermSet.filter (has_var_index_t vi) sel_terms
              |> Term.TermSet.choose
            in
            ...
            List.fold_left2
              (fun acc b ti -> if has_var_index_t vi ti then (vi, b) else acc)
              (vi, bnd) bnds_sv tind
          with Not_found | Invalid_argument _ -> (vi, bnd))
      (* if has_var_index vi expr then bnd :: acc else acc *))
    bounds

let fresh_state_var_for_expr ?(is_input = false) ?(is_const = false)
    ?(for_inv_gen = true) ?(reuse = true) bounds present_bounds present_bounds'
    ({ fresh_local_index } as ctx) after_mk expr expr_type =
...
let mk_abs_for_expr ?(is_input = false) ?(is_const = false)
    ?(for_inv_gen = true) ?(bounds = []) ?(reuse = true)
    ({ expr_abs_map } as ctx) after_mk ({ E.expr_type } as expr) =
  ...
  let expr_type, expr, present_bounds, present_bounds', _ =
    List.fold_left2
      (fun (ty, expr, bounds_acc, bounds_acc', i) ->
        let vi = E.mk_index_var i |> E.var_of_expr in
        fun b1 bp ->
          match b1 with
          | ev, ((E.Bound b | E.Fixed b) as bnd) when Var.equal_vars ev vi ->
          ...
          | ev, ((E.Unbound b | E.Bound b | E.Fixed b) as bnd) ->
              if not (has_var_index ev expr) then
                ...
                ( Type.mk_array ty (E.type_of_expr b),
                  ...
                  succ i ))
      (expr_type, expr, [], [], 0)
      (List.rev bounds') (List.rev bounds)
  in
  ...
  if not reuse then
    fresh_state_var_for_expr ~is_input ~is_const ~for_inv_gen ~reuse bounds
      present_bounds present_bounds' ctx after_mk expr expr_type
  else
    try
...
(* Define the expression with a state variable *)
let mk_local_for_expr ?is_input ?is_const ?for_inv_gen ?bounds ?(reuse = true)
    ?(is_ghost = false) ?original pos ({ node; definitions_allowed } as ctx)
    expr =
  match definitions_allowed with
  (* Fail with error if no new definitions allowed *)
  | Some (pos, msg) -> fail_at_position pos msg
  ...
  | _ -> (
      ...
      | Some _ ->
          ...
          let ((state_var, _) as abs), ctx =
            mk_abs_for_expr ?is_input ?is_const ?for_inv_gen ?bounds ~reuse
              ...
              with Not_found ->
                {
                  ctx with
                  node =
                    Some (N.set_state_var_source node state_var N.Generated);
                }
            else ctx
          in
...
let mk_fresh_local ?is_input ?is_const ?for_inv_gen ?(bounds = [])
    ({ node; fresh_local_index } as ctx) state_var_type =
  (* Are we in a node? *)
  match node with
  (* Fail if not inside a node *)
  ...
  | Some { N.locals } ->
      ...
      let state_var, ctx =
        mk_state_var ?is_input ?is_const ?for_inv_gen ctx
          (scope_of_node ctx @ I.reserved_scope)
          (I.push_index I.abs_ident !fresh_local_index)
          D.empty_index state_var_type None
      in
...
let rec node_of_name ({ prev; nodes } as ctx) ident =
  ...
  with Not_found ->
    if ctx == prev then raise Not_found else node_of_name prev ident

(* Return the output variables of a node call in the context with the
   same parameters *)
let call_outputs_of_node_call { node } ident cond_var input_state_vars defaults
    =
  (* Are we in a node? *)
  match node with
  (* Fail if not inside a node *)
      ...
      let inputs', ctx =
        D.fold
          (fun index index_type (accum, ctx) ->
            ...
            let state_var, ctx =
              mk_state_var ~is_input:true ?is_const ctx
      ...
      let outputs', ctx =
        D.fold
          (fun index index_type (accum, ctx) ->
            (* Create state variable as input and contant *)
            let state_var, ctx =
              mk_state_var ~is_input:false ctx
                (scope_of_node ctx @ I.user_scope)
                ident index index_type (Some N.Output)
            in

            let index' =
              if is_single then index else D.ListIndex next_top_idx :: index
            in
      ...
      let local, ctx =
        D.fold
          (fun index index_type (accum, ctx) ->
            (* Create state variable as input and contant *)
            let state_var, ctx =
              mk_state_var ~is_input:false ~shadow:ghost ctx
                (scope_of_context ctx @ I.user_scope)
                ident index index_type
...
(** The svars in the COI of the input expression, in the current node.
    ...
    Returns [None] if there's no current node. Raises [Not_found] if some svars
    in the COI do not have an equation and are not outputs of node calls.

    Used to check that the assumes and requires of a contract do not mention the
    outputs. *)
let trace_svars_of ctx expr =
  match ctx with
  | { node = None } -> None
  | { node = Some node } ->
      ...
      (* Set of an index. *)
      let to_set idx =
        D.fold (fun _ elm set -> SVS.add elm set) idx SVS.empty
      in
      (* $(a \setminus b) \cup c$ *)
      let diff_union a b c = SVS.diff a b |> SVS.union c in

      let rec loop (mem, to_do) =
        if SVS.is_empty to_do then mem
        else
          (mem, SVS.empty)
          |> SVS.fold
               (fun svar (mem, to_do) ->
                 ...
                 match
                   ...
                   try N.get_state_var_source node svar
                   with Not_found -> N.Local
                 with
                 | N.Oracle | N.Input | N.Output -> (mem, to_do)
                 | N.Local | N.Ghost | N.Generated
                 (* | N.Alias (_,_) *)
                 | N.Call ->
                     let svars =
          ...
          |> loop
      in
      Some ((SVS.empty, svars_of expr) |> loop)
  ...
  | { node = Some _ } -> (
      ...
      let state_var, ctx =
        ...
        then
          ...
          let abs, ctx = mk_abs_for_expr ctx add_state_var_to_locals expr in

          let state_var = match abs with sv, [] -> sv | _ -> assert false in
      ...
      | { node = Some ({ N.equations; N.props } as n) } ->
          ...
          let equations', prop', ctx =
            ...
            then
              ...
              let state_var', ctx = mk_fresh_local ctx Type.t_bool in
             ...
             added *)
          {
            ctx with
            node =
              Some { n with N.equations = equations'; N.props = prop' :: props };
          })
  ...
  | { node = Some { N.equations; N.calls } } -> (
      ...
      then
        fail_at_position pos
          (Format.asprintf "Duplicate definition for %a"
      ...
      let expr, expr_type, bounds, _ (* indexes *) =
        if Type.is_array expr_type then
          ...
          let expr, _ =
            List.fold_left
              (fun (e, i) _ -> (E.mk_select e (E.mk_index_var i), succ i))
              (expr, indexes) eitys
          in
        ...
        (*   expr, elty, List.rev_append extra_bnds bounds, i *)
          else (expr, expr_type, bounds, indexes)
      in
            ...
            (fun t -> function
              | E.Bound _ | E.Fixed _ | E.Unbound _ ->
                  if Type.is_array t then Type.elem_type_of_array t
                    ...
                    fail_at_position pos
                      (Format.asprintf "Type mismatch in equation: %a and %a"
      ...
      | { node = Some node; original_int_type } ->
          {
            ...
            original_int_type =
              (if convert_type then
               ...
               else original_int_type);
            node =
              Some
                {
                  node with
                  N.equations = ((state_var, bounds), expr) :: equations;
                };
          })
        ...
        D.exists
          (fun _ state_var ->
            ...
            List.exists
              ...
              calls)
          call_outputs
      then fail_at_position pos "Duplicate definition for output of node call";
...
let node_of_context = function
  (* Fail if not in a node *)
  | { node = None } -> raise (Invalid_argument "node_of_context")
  ...
  | { expr_abs_map; node = Some _ } as ctx -> (
      match
        ...
        ET.fold
          (fun e (sv, b) ctx ->
            add_node_equation ctx dummy_pos sv b (List.length b) e)
          expr_abs_map ctx
      with
      | { node = Some n } -> n
      | _ -> assert false)
...
let add_assumption_variable = function
  (* No node in context *)
  | { node = None } -> (
      function _ -> raise (Invalid_argument "add_assumption_variable"))
            ...
            D.fold
              (fun _ e ctx ->
                ...
                if E.is_var e || E.is_const_var e then
                  (* Get state variable of expression *)
                  let state_var =
                    let v = E.var_of_expr e in
                    (* Works for constant inputs too *)
                  ...
                  then
...

                    {
                      ...
                      node =
                        Some
                          { node with N.assumption_svars = assumption_svars' };
                    }
                  (* State variable is not a input or output variable *)
                    else
                    fail_at_position pos
                      (Format.asprintf
                         "State variable '%a' is not an input or an output."
...
let get_node_function_flag ctx =
  match ctx with
...
let solve_fref { deps' } decl (f_type, f_ident, f_pos) decls =
  ...
  if Deps.mem deps' (f_type, f_ident) (typ, ident) then
    Format.asprintf "circular dependency between %a '%a' and %a '%a'"
    ...
    try Deps.insert_decl decl (f_type, f_ident) decls
    with Not_found ->
      (* Forward reference to unknown declaration. *)
      Format.asprintf "unknown %a '%a' referenced in %a '%a'"
        (fun ppf -> function
        ...
        f_type (I.pp_print_ident false) f_ident Deps.pp_print_decl typ
        (I.pp_print_ident false) ident
...
let position_of_state_variable ctx svar =
  let eq (sv, _, _) = StateVar.equal_state_vars sv svar in
