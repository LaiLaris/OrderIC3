...
exception Cycle
(* Exception raised locally when a cycle in contract imports is detected *)
...
let error_message kind =
  ...
  | Unknown s -> s
  | UndefinedLocal id ->
      "Local variable '" ^ HString.string_of_hstring id ^ "' has no definition"
  | DuplicateLocal (id, pos) ->
      "Local variable '"
      ^ HString.string_of_hstring id
      ^ "' has already been defined at position "
      ^ Lib.string_of_t Lib.pp_print_position pos
  | DuplicateOutput (id, pos) ->
      "Output variable '"
      ^ HString.string_of_hstring id
      ^ "' has already been defined at position "
      ^ Lib.string_of_t Lib.pp_print_position pos
  ...
  | UndefinedNode id ->
      "Node or function '" ^ HString.string_of_hstring id ^ "' is undefined"
  | UndefinedContract id ->
      "Contract '" ^ HString.string_of_hstring id ^ "' is undefined"
  | DanglingIdentifier id ->
      "Unknown identifier '" ^ HString.string_of_hstring id ^ "'"
  | QuantifiedVariableInPre var ->
      "Quantified variable '"
      ^ HString.string_of_hstring var
      ^ "' is not allowed in an argument to pre operator"
  | QuantifiedVariableInNodeArgument (var, node) ->
      "Quantified variable '"
      ^ HString.string_of_hstring var
      ^ "' is not allowed in an argument of a call to node or non-inlinable \
         function '"
      ^ HString.string_of_hstring node
      ^ "'"
  | SymbolicArrayIndexInNodeArgument (idx, node) ->
      "Symbolic array index '"
      ^ HString.string_of_hstring idx
      ^ "' is not allowed in an argument of a call to node or non-inlinable \
         function '"
      ^ HString.string_of_hstring node
      ^ "'"
  | AnyOpInFunction -> "Illegal any operator in function"
  | NodeCallInFunction node ->
      "Illegal call to node '"
      ^ HString.string_of_hstring node
      ^ "', functions and function contracts can only call other functions, \
         not nodes"
  | NodeCallInConstant id ->
      "Illegal node call or 'any' operator in definition of constant '"
      ^ HString.string_of_hstring id
      ^ "'"
  | NodeCallInGlobalTypeDecl id ->
      "Illegal node call or 'any' operator in definition of global type '"
      ^ HString.string_of_hstring id
      ^ "'"
  | IllegalTemporalOperator (kind, variant) ->
      "Illegal " ^ kind ^ " in " ^ variant ^ " definition, " ^ variant
      ^ "s cannot have state"
  | IllegalImportOfStatefulContract contract ->
      "Illegal import of stateful contract '"
      ^ HString.string_of_hstring contract
      ^ "'. Functions can only be specified by stateless contracts"
  | UnsupportedClockedInputOrOutput ->
      "Clocked inputs or outputs are not supported"
  | UnsupportedClockedLocal id ->
      "Clocked node local variable not supported for '"
      ^ HString.string_of_hstring id
      ^ "'"
  | UnsupportedExpression e ->
      "The expression '" ^ LA.string_of_expr e ^ "' is not supported"
  | UnsupportedOutsideMerge e ->
      "The expression '" ^ LA.string_of_expr e
      ^ "' is only supported inside a merge"
  | UnsupportedWhen e ->
      "The `when` expression '" ^ LA.string_of_expr e
      ^ "' can only be the top most expression of a merge case"
  | UnsupportedParametricDeclaration ->
      "Parametric nodes and functions are not supported"
  | UnsupportedAssignment -> "Assignment not supported"
  | AssumptionVariablesInContractNode ->
      "Assumption variables not supported in contract nodes"
  | ClockMismatchInMerge -> "Clock mismatch for argument of merge"
  | MisplacedVarInFrameBlock id ->
      "Variable '"
      ^ HString.string_of_hstring id
      ^ "' is defined in the frame block but not declared in the frame block \
         header"
  | MisplacedAssertInFrameBlock ->
      "Assertion not allowed in frame block initialization"
  | IllegalClockExprInActivate e ->
      "Illegal clock expression '" ^ LA.string_of_expr e ^ "' in activate"
  | OpaqueWithoutContract n ->
      "An opaque annotation found for a node/function without a contract: "
      ^ HString.string_of_hstring n
  | TransparentWithoutBody n ->
      "A transparent annotation found for an imported node/function: "
      ^ HString.string_of_hstring n
  | IllegalHistoryVar id ->
      "History type constructor uses illegal quantified variable '"
      ^ HString.string_of_hstring id
      ^ "'"
...
let warning_message warning =
  ...
  | UnusedBoundVariableWarning id ->
      "Unused 'any' operator bound variable " ^ HString.string_of_hstring id
...
type contract_data = { stateful : bool; imports : StringSet.t }
...
let ctx_add_node ctx i ty = { ctx with nodes = StringMap.add i ty ctx.nodes }
...
let ctx_add_const ctx i ty = { ctx with consts = StringMap.add i ty ctx.consts }
let ctx_add_local ctx i ty = { ctx with locals = StringMap.add i ty ctx.locals }
...
let ctx_add_symbolic_array_index ctx i ty =
  {
    ctx with
    symbolic_array_indices = StringMap.add i ty ctx.symbolic_array_indices;
  }

(* Expression contains a pre, an arrow, or a call to a node *)
let rec has_stateful_op ctx = function
  | LA.Pre _ | Arrow _ -> true
  | RestartEvery _ | AnyOp _ -> true
  | Condact (_, e, r, i, l1, l2) ->
      StringMap.mem i ctx.nodes || has_stateful_op ctx e
      || has_stateful_op ctx r
      || List.fold_left (fun acc e -> acc || has_stateful_op ctx e) false l1
      || List.fold_left (fun acc e -> acc || has_stateful_op ctx e) false l2
  | Activate (_, i, e, r, l) ->
      StringMap.mem i ctx.nodes || has_stateful_op ctx e
      || has_stateful_op ctx r
      || List.fold_left (fun acc e -> acc || has_stateful_op ctx e) false l
  | Call (_, _, i, l) ->
  ...
  | GroupExpr (_, _, l) ->
      List.fold_left (fun acc e -> acc || has_stateful_op ctx e) false l
  | Merge (_, _, l) | RecordExpr (_, _, _, l) ->
      List.fold_left (fun acc (_, e) -> acc || has_stateful_op ctx e) false l
...
let build_global_ctx (decls : LustreAst.t) =
  ...
  let over_contract_eq (stateful, imports) = function
    ...
    | Mode (_, _, reqs, enss) ->
        ...
        let stateful' =
          stateful
          || req_or_ens_has_stateful_op reqs
          || req_or_ens_has_stateful_op enss
        in
        (stateful', imports)
    | ContractCall (_, i, _, ins, _) ->
        let arg_has_stateful_op ins =
          List.fold_left (fun acc e -> acc || has_stateful_op ctx e) false ins
        in
        (stateful || arg_has_stateful_op ins, StringSet.add i imports)
    | AssumptionVars _ -> (stateful, imports)
  in
  let over_contract_decls acc = function
    | LA.ContractNodeDecl (_, (i, _, _, _, (_, eqns))) ->
        let stateful, imports =
          List.fold_left over_contract_eq (false, StringSet.empty) eqns
        in
...
let build_contract_ctx ctx eqns =
  let over_eqns acc = function
    ...
    | LA.GhostVars (_, GhostVarDec (_, l), _) ->
        List.fold_left
          (fun acc (_, i, ty) -> ctx_add_const acc i (Some ty))
          acc l
    | _ -> acc
  in
...
let build_local_ctx ctx locals inputs outputs =
  let over_locals acc = function
    | LA.NodeConstDecl (_, FreeConst (_, i, ty)) ->
        ctx_add_local acc i (Some ty)
  ...
  | LA.StructDef (_, items) ->
      let over_items ctx = function
        | LA.ArrayDef (_, i, indices) ->
            ...
            let is_symbolic =
              match output_type_opt with
              | Some ty -> (
                  match ty with
                  | ArrayType (_, (_, e)) ->
                      let vars = LAH.vars_without_node_call_ids e in
                      let check_var e =
                        StringMap.mem e ctx.free_consts
            ...
            let over_indices acc id =
              if is_symbolic then
                ctx_add_symbolic_array_index acc id output_type_opt
...
let rec find_var_def_count id = function
  | LA.Body eqn -> (
      ...
      | LA.Equation (_, lhs, _) ->
          (match lhs with
          | LA.StructDef (_, vars) -> List.map (find_var_def_count_lhs id) vars)
          |> List.flatten)
  | LA.IfBlock (_, _, l1, l2) ->
      let x1 = List.map (find_var_def_count id) l1 |> List.flatten in
      let x2 = List.map (find_var_def_count id) l2 |> List.flatten in
      let len1 = List.length x1 in
      let len2 = List.length x2 in
      (* Detect duplicate definitions in the same branch *)
      if len1 > 1 then x1
      else if len2 > 1 then x2 (* Local is defined in at least one branch *)
  ...
  | LA.FrameBlock (pos, vars, nes, nis) ->
      ...
      let x2 = List.map (find_var_def_count id) nis |> List.flatten in
      let x3 = List.map (find_var_def_count id) nes |> List.flatten in
      let len1 = List.length x1 in
      let len2 = List.length x2 in
  ...
  let over_locals = function
    ...
    | LA.NodeVarDecl (_, (pos, id, _, _)) -> (
        ...
        | _ ->
            ...
            syntax_error
              (List.hd (List.tl poss))
              (DuplicateLocal (id, List.hd poss)))
  in
  ...
  let over_outputs = function
    | _, id, _, _ -> (
        ...
        | _ ->
            ...
            syntax_error
              (List.hd (List.tl poss))
              (DuplicateOutput (id, List.hd poss)))
  in
...
let no_dangling_identifiers ctx = function
  | LA.Ident (pos, i) -> no_a_dangling_identifier ctx pos i
  | _ -> Ok ()

let no_node_calls_in_constant i e =
  if LAH.expr_contains_call e then
    syntax_error (LAH.pos_of_expr e) (NodeCallInConstant i)
  else Ok ()
...
let no_calls_to_node ctx = function
  ...
  | Call (pos, _, i, _) ->
      let check_nodes = StringMap.mem i ctx.nodes in
      if check_nodes then syntax_error pos (NodeCallInFunction i) else Ok ()
...
let no_temporal_operator decl_ctx expr =
  match expr with
  | LA.Pre (pos, _) ->
      syntax_error pos (IllegalTemporalOperator ("pre", decl_ctx))
  | Arrow (pos, _, _) ->
      syntax_error pos (IllegalTemporalOperator ("arrow", decl_ctx))
  | _ -> Ok []
    ...
    let rec check_import_stateful visited pos initial i =
      ...
      | Some { stateful; imports } ->
          if not stateful then
            ...
            StringSet.fold
              (fun j a -> a >> check_import_stateful visited pos initial j)
              imports (Ok ())
...
let rec expr_only_supported_in_merge observer expr =
  ...
  | Merge (_, _, e) ->
      Res.seq_
        (List.map
           (fun (_, e) -> match e with LA.When (_, e, _) | e -> r true e)
           e)
  ...
  | ArrayIndex (_, e1, e2)
  | ArrayConstr (_, e1, e2) ->
      r observer e1 >> r observer e2
  | TernaryOp (_, _, e1, e2, e3) ->
      r observer e1 >> r observer e2 >> r observer e3
  | GroupExpr (_, _, e) | Call (_, _, _, e) -> r_list observer e
  ...
  | Activate (pos, _, _, _, _) as e ->
      if observer then Ok () else syntax_error pos (UnsupportedOutsideMerge e)
  | RestartEvery (_, _, e1, e2) -> r_list observer e1 >> r observer e2

let check_opacity pos node_id contract is_ext = function
  | LA.Opaque when contract = None ->
      syntax_error pos (OpaqueWithoutContract node_id)
...
and check_ty_node_calls i ty =
  ...
  | LA.RefinementType (_, _, e) ->
      if LAH.expr_contains_call e then
        syntax_error (LAH.pos_of_expr e) (NodeCallInGlobalTypeDecl i)
      else Ok ()
  | TupleType (_, tys) | GroupType (_, tys) ->
      Res.seq_ (List.map (check_ty_node_calls i) tys)
  | RecordType (_, _, tis) ->
      Res.seq_ (List.map (fun (_, _, ty) -> check_ty_node_calls i ty) tis)
  | ArrayType (_, (ty, e)) ->
  ...
  | Bool _ | Int _ | IntRange _ | Real _ | EnumType _ | UInt8 _ | UInt16 _
  | UInt32 _ | UInt64 _ | Int8 _ | Int16 _ | Int32 _ | Int64 _ | AbstractType _
  | History _ | TArr _ ->
      Ok ()

and check_declaration :
    context ->
    LA.declaration ->
    ([> warning ] list * LA.declaration, [> error ]) result =
 fun ctx -> function
  | TypeDecl (span, FreeType (pos, id)) ->
      Ok ([], LA.TypeDecl (span, FreeType (pos, id)))
  | TypeDecl (span, AliasType (pos, id, ps, ty)) ->
      check_ty_node_calls id ty
      >> Ok ([], LA.TypeDecl (span, AliasType (pos, id, ps, ty)))
  | ConstDecl (span, decl) ->
      let* warnings =
        match decl with
        | LA.FreeConst _ -> Ok []
  ...
  | NodeParamInst (span, _) ->
      syntax_error span.start_pos UnsupportedParametricDeclaration

and check_const_expr_decl :
    H.t -> context -> LA.expr -> ([> warning ] list, [> error ]) result =
 fun i ctx expr ->
  let composed_checks i ctx e =
    no_dangling_identifiers ctx e >> no_node_calls_in_constant i e >> Ok []
  in
...
and no_reachability_modifiers item =
  match item with
  | LA.AnnotProperty (pos, _, _, Reachable (Some _)) ->
      syntax_error pos
        (IllegalTemporalOperator ("pre", "reachability query modifier"))
  | _ -> Ok ()
...
and check_local_items :
    context -> LA.node_local_decl -> ([> warning ] list, [> error ]) result =
 fun ctx local ->
  match local with
...
and check_node_decl ctx span
    (id, ext, opac, params, inputs, outputs, locals, items, contract) =
  let decl =
    LA.NodeDecl
  ...
  let* warnings3 =
    check_items
      ...
      common_node_equations_checks items
  in
  Ok (warnings1 @ List.flatten warnings2 @ warnings3, decl)

and check_func_decl ctx span
    (id, ext, opac, params, inputs, outputs, locals, items, contract) =
  let ctx =
    (* Locals are not visible in contracts *)
    build_local_ctx ctx [] inputs outputs
...
and check_contract_node_decl ctx span (id, params, inputs, outputs, contract) =
  ...
  let decl =
    LA.ContractNodeDecl (span, (id, params, inputs, outputs, contract))
  in
  Res.seq_ (List.map check_input_items inputs)
  >> Res.seq_ (List.map check_output_items outputs)
  >>
...
and check_items :
    context ->
    (context -> LA.expr -> ([> warning ] list, ([> error ] as 'a)) result) ->
    LA.node_item list ->
    ([> warning ] list, 'a) result =
 fun ctx f items ->
  ...
  let check_item :
   ...
   fun ctx f -> function
     ...
     | LA.FrameBlock (pos, vars, nes, nis) ->
         ...
         >> Res.seq_
              (List.map (fun (p, v) -> no_a_dangling_identifier ctx p v) vars)
         >>
         ...
         let* warnings1 =
           check_items ctx
             (fun _ e -> no_temporal_operator "frame block initialization" e)
             nes
         in
         let* warnings2 = check_items ctx f nes in
         let* warnings3 = check_items ctx f nis in
         Ok (warnings1 @ warnings2 @ warnings3)
     | Body (Assert (_, e)) | AnnotProperty (_, _, e, _) -> check_expr ctx f e
     | AnnotMain _ -> Ok []
  in
...
and check_frame_vars pos vars ni =
  ...
  let unlisted =
    H.HStringSet.diff
      (H.HStringSet.of_list vars_of_ni)
      (H.HStringSet.of_list vars)
  in
 ...
 fun is_contract_node ctx f (_, contract) ->
  ...
  let check_contract_item ctx f = function
    ...
    | GhostConst decl -> (
        match decl with
        | LA.FreeConst _ -> Ok []
        | UntypedConst (_, i, e) | TypedConst (_, i, e, _) ->
            check_const_expr_decl i ctx e)
    | ContractCall (pos, i, _, args, outputs) ->
        if StringMap.mem i ctx.contracts then
          Res.seqM
            (fun x _ -> x)
            ()
            (List.map (no_a_dangling_identifier ctx pos) outputs)
 ...
 fun ctx f (expr : LustreAst.expr) ->
  ...
  let check = function
    ...
    | Quantifier (_, _, vars, e) ->
        ...
        check_quantified_vars ctx vars >> check_expr ctx f e
    | BinaryOp (_, _, e1, e2)
    | CompOp (_, _, e1, e2)
    | ArrayConstr (_, e1, e2)
    | ArrayIndex (_, e1, e2)
    ...
    | TernaryOp (_, _, e1, e2, e3) ->
        ...
        Ok (warnings1 @ warnings2 @ warnings3)
    | GroupExpr (_, _, e) | Call (_, _, _, e) -> check_expr_list ctx f e
    | RecordExpr (_, _, _, e) | Merge (_, _, e) ->
        let e = List.map (fun (_, e) -> e) e in
    ...
    | StructUpdate (_, e1, l, e2) ->
        ...
        let l =
          List.filter_map
            (function LA.Label _ -> None | Index (_, e) -> Some e)
            l
        in
    ...
    | AnyOp (pos, (_, i, ty), e1, Some e2) ->
        ...
        let warnings1 =
          ...
          if
            (not (LAH.expr_contains_id i e1)) && not (i = HString.mk_hstring "_")
          then [ mk_warning pos (UnusedBoundVariableWarning i) ]
          else []
        in
...
let no_mismatched_clock is_bool e =
  ...
  let clocks_match_result pos c1 c2 =
    if clocks_match c1 c2 then Ok [] else syntax_error pos ClockMismatchInMerge
  in
  let check_clocks clock = function
    ...
    | _ -> Ok []
  in
  let check_merge : LA.expr -> ([> warning ] list, [> error ]) result = function
    | LA.Merge (_, clock, exprs) -> (
        if not is_bool then
          let case (i, e) =
            check_expr ctx (fun _ -> check_clocks (ClockConstr (i, clock))) e
          in
...
let ovq_check_expr inlinable_funcs ctx = function
  | LA.Call (pos, _, i, args) ->
      ...
      let over_vars j =
        ...
        in
        match
          (found_quant_in_non_inlinable, found_symbolic_index_in_non_inlinable)
        with
        | true, _ -> syntax_error pos (QuantifiedVariableInNodeArgument (j, i))
        | _, true -> syntax_error pos (SymbolicArrayIndexInNodeArgument (j, i))
        | false, false -> Ok []
...
let oqv_check_node_decl inlinable_funcs ctx
    (_, _, _, _, inputs, outputs, locals, items, contract) =
  let* warnings1 =
    match contract with
    | Some c ->
        let ctx =
...
let oqv_check_contract_node_decl inlinable_funcs ctx
    (_, _, inputs, outputs, contract) =
  let ctx = build_local_ctx ctx [] inputs outputs in
  let* warnings =
...
let oqv_check_decl :
 ...
 fun inlinable_funcs ctx -> function
  | NodeDecl (_, decl) -> oqv_check_node_decl inlinable_funcs ctx decl
  | FuncDecl (_, decl) -> oqv_check_node_decl inlinable_funcs ctx decl
