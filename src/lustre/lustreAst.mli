...
(** Minimally simplified Lustre abstract syntax tree
    ...
    The types in this module closely represent the abstract syntax of Lustre. No
    type checking or simplification is performed when constructing the abstract
    syntax tree, this is done when producing the intermediate Lustre
    representation in {!LustreDeclarations}.

    Some values are reserved for future use and will cause the translation to
    intermediate Lustre to fail.

    A Lustre file is parsed into a {!declaration} list, where a declaration is
    either
    ...
    Almost all types are annotated with the position in the input file for
    better error reporting in the translation.
...
(** An identifier, possibly flagged as constant, with a type and a clock as used
...
(** An instance of a parametric node as a tuple of the identifier for the
    instance, the identifier of the parametric node and the list of type
    parameters *)

type span = { start_pos : position; end_pos : position }

(** A declaration of a type, a constant, a node, a function or an instance of a
    parametric node *)
type declaration =
