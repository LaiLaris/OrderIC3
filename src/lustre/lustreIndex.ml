...
let pp_print_one_index' db = function
  | false -> (
      ...
      | ppf -> (
          ...
          | ArrayVarIndex _ ->
              ()
              (* Format.fprintf ppf "[X%d(%a)]" db (E.pp_print_expr false) v ) *)
          | AbstractTypeIndex _ -> ()))
  | true -> (
      function
      | ppf -> (
...
let compare_one_index a b =
  match (a, b) with
  (* Use polymorphic comparison on strings and integers *)
  | RecordIndex a, RecordIndex b | AbstractTypeIndex a, AbstractTypeIndex b ->
      String.compare a b
  | TupleIndex a, TupleIndex b
  | ListIndex a, ListIndex b
  | ArrayIntIndex a, ArrayIntIndex b ->
      Int.compare a b
  ...
  | ListIndex _, RecordIndex _ | ListIndex _, TupleIndex _ -> -1
  ...
  | ArrayIntIndex _, ListIndex _ ->
      -1
  | ArrayIntIndex _, ArrayVarIndex _ | ArrayIntIndex _, AbstractTypeIndex _ -> 1
...
let compare_indexes a b =
  ...
  | a :: atl, b :: btl -> (
      match compare_one_index a b with
...
let array_bounds_of_index idx =
  List.fold_left (fun a -> function ArrayVarIndex e -> e :: a | _ -> a) [] idx
    ...
    (fun a -> function
      | ArrayVarIndex e ->
          StateVar.mk_state_var
            (List.length a |> I.push_index I.index_ident
...
let top_max_index t =
  try
    LustreIndexTrie.max_binding t |> fst |> function
    | ListIndex i :: _ -> i
    | [] -> -1
    | _ -> raise (Invalid_argument "top_max_index")
  with Not_found -> -1

let filter_array_indices index =
  List.filter
    (function
      | ArrayVarIndex _ | ArrayIntIndex _ -> false
      | RecordIndex _ | TupleIndex _ | ListIndex _ | AbstractTypeIndex _ -> true)
    index

let compatible_one_index i1 i2 =
  match (i1, i2) with
  ...
  | ArrayVarIndex _, ArrayIntIndex _
  | ArrayVarIndex _, ArrayVarIndex _ ->
      true
...
let pp_print_index_trie safe pp_e ppf t =
  ...
  |> pp_print_list
       (fun ppf (i, e) ->
         if i = empty_index then pp_e ppf e
         else Format.fprintf ppf "%a: %a" (pp_print_index safe) i pp_e e)
       ";@ " ppf

let pp_print_trie_expr safe ppf expr =
  pp_print_index_trie safe (E.pp_print_lustre_expr safe) ppf expr

let mk_scope_for_index index =
  List.rev_map
    (fun i ->
      string_of_t (pp_print_one_index true) i
      |> String.length |> string_of_int |> Ident.of_string)
