...
let array_select_of_indexes_expr indexes e =
  List.fold_left
    (fun e i -> E.mk_select_and_push e (E.mk_index_var i))
    e indexes
...
let coalesce_array2 e1 e2 =
  let t1 = E.type_of_lustre_expr e1 and t2 = E.type_of_lustre_expr e2 in
...
let mk_state_var_name ident index =
  Format.asprintf "%a%a" (I.pp_print_ident true) ident (X.pp_print_index true)
  ...
  try
    let state_var =
      StateVar.state_var_of_string
        (state_var_name, List.map Ident.to_string (scope @ flatten_scopes))
    in
    H.replace !map.array_literal_index sv_ident
      (compute_expr (E.mk_var state_var))
  with Not_found -> assert false

(* Create a state variable for from an indexed state variable in a
  scope *)
let mk_state_var ?is_input ?is_const ?for_inv_gen ?expr_ident
    ?(force_return = false) map scope sv_ident index state_var_type source =
  let expr_ident = match expr_ident with Some id -> id | None -> sv_ident in
  ...
  *)
  ...
  let state_var, fresh =
    ...
    with Not_found ->
      let state_var =
        StateVar.mk_state_var ?is_input ?is_const ?for_inv_gen state_var_name
          (scope @ flatten_scopes) state_var_type
      in
      (state_var, true)
  in
...
let flatten_list_indexes (e : 'a X.t) =
  let top_is_list =
    try X.top_max_index e >= 0 with Invalid_argument _ -> false
  in
  if not top_is_list then e
  else
    let rec extract_list_prefix acc = function
      | X.ListIndex i :: tl -> extract_list_prefix (X.ListIndex i :: acc) tl
    ...
    let m =
      List.fold_left
        (fun acc (indices, e) ->
          ...
          XMap.update prefix
            (function
              | None -> Some [ (other, e) ] | Some l -> Some ((other, e) :: l))
            acc)
        XMap.empty (X.bindings e)
    in
    XMap.fold
      (fun _ l (acc, i) ->
        let acc =
          List.fold_left
            (fun acc (indices, e) -> X.add (X.ListIndex i :: indices) e acc)
            acc l
        in
        (acc, i + 1))
      m (X.empty, 0)
...
let rec expand_tuple' pos accum bounds lhs rhs =
  ...
  | _, [] | [], _ ->
      ...
      assert false (* All indexes consumed *)
  ...
  | ( (X.ArrayIntIndex idx :: lhs_index_tl, state_var) :: lhs_tl,
      ...
      if Type.is_array expr_type then
        ...
        let index_arg =
          E.mk_of_expr ~as_type:index_type (E.mk_int_expr (Numeral.of_int idx))
        in
        ...
        let rec mk_bounds ty acc =
          if Type.is_array ty then
            ...
            let u =
              match Type.bounds_of_int_range index_type with
              | _, Some u -> u
              | _, None -> assert false
            in
  ...
  | ( (X.ArrayVarIndex b :: lhs_index_tl, state_var) :: lhs_tl,
      ...
      let b =
        if
          E.is_numeral b && E.is_numeral br
      ...
      in
      let start =
        List.length lhs_index_tl + 1 - List.length array_index_types
      in
      let expr, _ =
        List.fold_left over_index_types (expr, start) array_index_types
      in
      expand_tuple' pos accum (E.Bound b :: bounds)
        ((lhs_index_tl, state_var) :: lhs_tl)
            ...
            if j = j' then
              let accum =
                expand_tuple' pos accum bounds
                  ...
                  [ (rhs_index_tl, expr) ]
              in
              expand_tuple' pos accum bounds lhs_tl
                ((X.ListIndex j :: rhs_index_tl, expr) :: rhs_tl)
            else
              expand_tuple' pos accum bounds
                ((lhs_index_tl, state_var) :: lhs_tl)
  ...
  | ( (X.ArrayIntIndex i :: lhs_index_tl, state_var) :: lhs_tl,
      ...
      let expr_type = E.type_of_lustre_expr expr in
      if Type.is_array expr_type then
        expand_tuple' pos accum bounds [ ([], state_var) ] [ ([], expr) ]
      else if i = j then
...
let expand_tuple pos lhs rhs =
  ...
  expand_tuple' pos [] [] (X.bindings lhs) (X.bindings rhs)
...
let rec compile ctx gids decls =
  ...
  let free_constants =
    output.free_constants |> List.map (fun (_, id, v) -> (mk_ident id, v))
  in
  ( output.nodes,
    {
      G.free_constants;
...
and compile_ast_type ?(expand = false) (cstate : compiler_state)
    (ctx : Ctx.tc_context) map = function
  ...
  | A.IntRange (_, lbound, ubound) ->
      ...
      let lvalue =
        match lbound with
        | Some (A.Const (_, Num x)) ->
            Some (Numeral.of_string (HString.string_of_hstring x))
        | None -> None
        | _ -> assert false
      in
      let uvalue =
  ...
  | A.UserType (_, _, ident) -> StringMap.find ident cstate.type_alias
  ...
  | A.ArrayType (_, (type_expr, size_expr)) ->
      ...
      if expand then (
        ...
        let over_element_type j t a =
          X.add
            (j @ [ X.ArrayVarIndex array_size ])
            (Type.mk_array t
               (if E.is_numeral array_size then
                  let array_size =
                    Numeral.(max zero (E.numeral_of_expr array_size))
                  in
  ...
  | A.History _ | A.TArr _ -> assert false
...
and vars_of_quant cstate ctx map avars =
  let avars =
    List.map (fun (p, s, ty) -> (p, HString.string_of_hstring s, ty)) avars
  in
...
and compile_ast_expr (cstate : compiler_state) (ctx : Ctx.tc_context)
    (bounds : E.expr E.bound_or_fixed list) map expr :
  ...
  let rec compile_id_string bounds id_str =
    ...
    with Not_found -> (
      ...
      with Not_found -> (
        ...
        with Not_found -> (
          try H.find !map.quant_vars ident
          with Not_found -> (
            try H.find !map.array_index ident
            with Not_found -> (
              let id_str = HString.string_of_hstring id_str in
  ...
  and compile_mode_reference path' =
    ...
    let path2 =
      List.map (fun (_, s) -> HString.string_of_hstring s) !map.contract_scope
    in
    ...
    let rec find_mode = function
      | { C.path; C.requires } :: tail ->
          if path = path' then
            ...
            |> E.mk_and_n |> X.singleton X.empty_index
          else find_mode tail
      | [] -> assert false
    in
    find_mode !map.modes
  ...
  and compile_quantifier bounds mk avars expr =
    ...
    let bounds =
      bounds
      @ List.map
          (fun v -> E.Unbound (E.unsafe_expr_of_term (Term.mk_var v)))
          vars
    in
  ...
  and compile_equality bounds polarity expr1 expr2 =
    let mk_binary, mk_seq, const_expr =
      match polarity with
      | true -> (E.mk_eq, E.mk_and, E.t_true)
      | false -> (E.mk_neq, E.mk_or, E.t_false)
    in
  ...
  and compile_ite bounds expr1 expr2 expr3 =
    ...
    let expr1 = compile_ast_expr cstate ctx bounds map expr1 in
    let expr1 =
      match X.bindings expr1 with [ (_, expr) ] -> expr | _ -> assert false
    in
    let mk e1 e2 =
  ...
  and compile_pre bounds expr =
    ...
    in
    X.fold over_indices cexpr X.empty
  and compile_merge bounds clock_ident merge_cases =
    let merge_cases =
      List.map (fun (s, e) -> (HString.string_of_hstring s, e)) merge_cases
    in
    let clock_expr =
      compile_id_string bounds clock_ident |> X.values |> List.hd
    in
    let clock_type = E.type_of_lustre_expr clock_expr in
    let cond_expr_clock_value clock_value =
      match clock_value with
      | "true" -> clock_expr
      | "false" -> E.mk_not clock_expr
      | _ -> E.mk_eq clock_expr (E.mk_constr clock_value clock_type)
    in
    let compile_merge_case = function
      | A.When (_, expr, _) -> compile_ast_expr cstate ctx bounds map expr
      | expr -> compile_ast_expr cstate ctx bounds map expr
    in
    let merge_cases_r =
      let over_cases =
       fun acc (case_value, e) ->
        let e = compile_merge_case e in
        (cond_expr_clock_value case_value, e) :: acc
      in
      List.fold_left over_cases [] merge_cases
    in
    let default_case, other_cases_r =
      match merge_cases_r with (_, d) :: l -> (d, l) | _ -> assert false
    in
    let over_other_cases =
     fun acc (cond, e) -> X.map2 (fun _ -> E.mk_ite cond) e acc
    in
    List.fold_left over_other_cases default_case other_cases_r
  and compile_projection bounds expr = function
    | (X.RecordIndex _ | X.TupleIndex _ | X.ArrayIntIndex _) as index ->
  ...
  and compile_record_expr bounds expr_list =
    let expr_list =
      List.map (fun (s, e) -> (HString.string_of_hstring s, e)) expr_list
    in
    let over_exprs =
     fun accum (i, expr) ->
      let compiled_expr = compile_ast_expr cstate ctx bounds map expr in
  ...
  and compile_struct_update expr1 index expr2 =
    ...
    let rec aux accum = function
      ...
      | A.Label (_, index) :: tl ->
          ...
          if X.mem_prefix (List.rev accum') cexpr1 then aux accum' tl
      ...
      | A.Index (_, index_expr) :: tl ->
          ...
          let i =
            if Term.is_numeral index_term then
              ...
              | X.ArrayVarIndex _ :: _, _ | X.ArrayIntIndex _ :: _, _ ->
                  X.ArrayIntIndex value
      ...
      with Not_found -> (
        try
          ...
          let old_v =
            ...
            |> fst
          in
          let new_v = X.find cindex cexpr2' in
  ...
  and compile_array_ctor bounds expr size_expr =
    ...
    let over_indices =
     fun j (e : LustreExpr.t) a ->
      let e' = state_var_of_expr e |> E.mk_var in
      X.add (j @ [ X.ArrayVarIndex array_size ]) e' a
    in
    let result = X.fold over_indices cexpr X.empty in
    result
  and compile_array_index bounds expr i =
    ...
    let compiled_expr = compile_ast_expr cstate ctx bounds map expr in
    let rec push expr =
      match X.choose expr with
      | X.ArrayVarIndex _ :: _, v | X.ArrayIntIndex _ :: _, v ->
          let over_expr =
           fun e ->
            match e with
            | X.ArrayVarIndex _ :: tl | X.ArrayIntIndex _ :: tl -> X.add tl
            | _ -> assert false
          in
  ...
  (* Other Operators                                                    *)
  (* ****************************************************************** *)
  | A.CompOp (_, A.Eq, expr1, expr2) -> compile_equality bounds true expr1 expr2
  ...
  | A.RecordExpr (_, _, _, expr_list) -> compile_record_expr bounds expr_list
...
and compile_node node_scope pos ctx cstate map outputs cond restart ident args
    defaults inlined =
  ...
  let oracles =
    ...
    |> List.mapi (fun i sv ->
           let propagated_oracle =
             let sv' =
               mk_state_var ~is_const:true map
                 ...
                 (Some N.Oracle)
             in
             match sv' with Some sv' -> sv' | None -> assert false
           in
  ...
  let node_inputs_of_exprs inputs ast =
    ...
    let over_indices i input_sv expr accum =
      ...
      let i' = match i with X.ListIndex _ :: idx -> idx | idx -> idx in
  ...
  let node_act_cond_of_expr cond defaults =
    let cond_test =
      match cond with A.Const (_, A.True) -> true | _ -> false
    in
    if cond_test then (None, None)
    else
      ...
      let defaults' =
        ...
        | Some d ->
            Some
              (compile_ast_expr cstate ctx [] map
                 (A.GroupExpr (dummy_pos, A.ExprList, d)))
        | None -> None
      in
      (Some state_var, defaults')
  in
  let restart_cond_of_expr restart =
    let restart_test =
      match restart with A.Const (_, A.False) -> true | _ -> false
    in
    if restart_test then None
    else
      let state_var = restart |> extract_normalized |> H.find !map.state_var in
      Some state_var
  in
  ...
  let cond_state_var =
    match (act_state_var, restart_state_var) with
    | None, None -> []
...
and compile_contract_variables cstate gids ctx map contract_scope node_scope
    contract =
  ...
  let gconsts, gvars, modes, contract_calls =
    ...
    in
    List.fold_left over_items ([], [], [], []) contract
  in
  (* ****************************************************************** *)
  ...
  let ghost_locals, ghost_equations =
    let extract_namespace name =
      ...
      | _ -> assert false
    in
    let over_vars (gvar_accum, eq_accum) =
     fun (pos, A.GhostVarDec (_, tis), expr) ->
      let extract_local (_, id, ty) =
        ...
        let over_indices =
         fun index index_type accum ->
          let possible_state_var =
            mk_state_var ~is_input:false ~expr_ident map
              (node_scope @ contract_namespace @ I.user_scope)
              ident index index_type (Some N.Ghost)
          in
      ...
      let struct_items =
        List.map (fun (pos, id, _) -> A.SingleIdent (pos, id)) tis
      in
      let eq_lhs = A.StructDef (pos, struct_items) in
      let eq_rhs = expr in
      ( List.map extract_local tis @ gvar_accum,
        (pos, eq_lhs, eq_rhs) :: eq_accum )
    in
    List.fold_left over_vars ([], []) gvars
    (* ****************************************************************** *)
    (* Contract Modes                                                     *)
    (* ****************************************************************** *)
  in
  let modes =
    let over_modes (pos, id, reqs, enss) =
      ...
      mode
    in
    List.map over_modes modes
    (* ****************************************************************** *)
    (* Contract Calls                                                     *)
    (* ****************************************************************** *)
  in
  let cstate, ghost_locals2, ghost_equations2, modes2 =
    let over_calls (cstate, gls, ges, ms) (_, cref, _, _, _) =
      ...
      let params =
        match Ctx.lookup_contract_ty_vars ctx cname with
        | None -> []
        | Some params -> params
      in
      let cstate =
        List.fold_left
          (fun acc param ->
            ...
            let t =
              compile_ast_type cstate ctx empty_map
                (A.AbstractType (Lib.dummy_pos, param))
            in
      ...
      let cstate, gl, ge, m =
        compile_contract_variables cstate gids ctx map contract_scope node_scope
          contract_eqns
      in
      (cstate, gl @ gls, ge @ ges, m @ ms)
    in
    List.fold_left over_calls (cstate, [], [], []) contract_calls
  in
  ( cstate,
    ghost_locals @ ghost_locals2,
...
and compile_contract cstate gids ctx map contract_scope node_scope contract =
  ...
  let assumes2, guarantees2 =
    let over_calls (ams, gs) (_, id, _, _, _) =
      ...
      let a, g =
        compile_contract cstate gids ctx map scope node_scope contract_eqns
      in
      (a @ ams, g @ gs)
    in
    List.fold_left over_calls ([], []) contract_calls
    (* ****************************************************************** *)
  ...
  let guarantees =
    let over_guarantees (pos, name, soft, expr) =
      ...
      compile_contract_item map (i + 1) contract_scope kind pos name expr
    in
    List.map over_guarantees guarantees |> List.map (fun g -> (g, false))
  in
  (assumes @ assumes2, guarantees @ guarantees2)

and compile_node_decl gids_map is_function opac cstate ctx i ext params inputs
    outputs locals items contract =
  ...
  in
  let instance =
    StateVar.mk_state_var ~is_const:true
  ...
  let inputs =
    ...
    let over_inputs =
     fun compiled_input (_pos, i, ast_type, clock, is_const) ->
      ...
      | A.ClockTrue ->
          ...
          let index_types = compile_ast_type cstate ctx map ast_type in
          let over_indices =
           fun index index_type (accum1, accum2) ->
            let possible_state_var =
              mk_state_var ~is_input:true ~is_const map
                (node_scope @ I.user_scope)
                ident index index_type (Some N.Input)
            in
            match possible_state_var with
            | Some state_var ->
  ...
  let outputs =
    ...
    let over_outputs =
     fun is_single compiled_output (_, i, ast_type, clock) ->
      let indexed_state_var = X.empty in
      match clock with
      | A.ClockTrue ->
          ...
          let over_indices =
           fun index index_type (accum1, accum2) ->
            let possible_state_var =
              mk_state_var ~is_input:false map
                (node_scope @ I.user_scope)
                ident index index_type (Some N.Output)
            in
            let index' = if is_single then index else X.ListIndex n :: index in
            match possible_state_var with
            | Some state_var ->
                (X.add index' state_var accum1, X.add index state_var accum2)
            | None -> (accum1, accum2)
          in
          ...
          compiled_output
      | _ -> assert false (* Guaranteed by LustreSyntaxChecks *)
    and is_single = List.length outputs = 1 in
    List.fold_left (over_outputs is_single) X.empty outputs
    (* ****************************************************************** *)
    (* User Locals                                                        *)
    (* ****************************************************************** *)
  in
           ...
           fun index index_type accum ->
            let possible_state_var =
              mk_state_var ~is_input:false map
                ...
                ident index index_type (Some N.Local)
            in
            match possible_state_var with
            | Some state_var -> X.add index state_var accum
            | None -> accum
          in
          let indexed_state_var = X.fold over_indices index_types X.empty in
          H.replace !map.usr_state_var ident indexed_state_var;
          (indexed_state_var :: locals, cstate)
      | A.NodeConstDecl (_, decl) ->
          ( locals,
            compile_const_decl cstate ctx map (node_scope @ [ "impl" ]) decl )
  ...
  let glocals =
    ...
    let over_generated_locals glocals (id, expr_type) =
      ...
      (if GI.StringSet.mem id gids.GI.array_literal_vars then
         ...
         let index_types' =
           compile_ast_type ~expand:true cstate ctx map expr_type
         in
         X.iter
           (fun index _ ->
             update_array_literal_index map
               (node_scope @ I.reserved_scope)
               ident index)
           index_types');
      result :: glocals
    in
    List.fold_left over_generated_locals [] locals_list
    (* ****************************************************************** *)
  ...
  let glocals =
    let over_generated_locals glocals (_, _, _, _, id, _) =
      ...
      let over_indices =
       fun index index_type accum ->
        let possible_state_var =
          mk_state_var map
            (node_scope @ I.reserved_scope)
            ident index index_type (Some N.Generated)
        in
        match possible_state_var with
        | Some state_var -> X.add index state_var accum
        | None -> accum
      ...
      result :: glocals
    in
    List.fold_left over_generated_locals glocals gids.GI.subrange_constraints
    (* ****************************************************************** *)
  ...
  let glocals =
    let over_generated_locals glocals (_, _, id, _) =
      ...
      result :: glocals
    in
    List.fold_left over_generated_locals glocals
  ...
  let glocals =
    let over_generated_locals glocals (id, is_const, expr_type, _) =
      ...
      result :: glocals
    in
    List.fold_left over_generated_locals glocals gids.GI.node_args
    (* ****************************************************************** *)
      ...
      let over_indices =
       fun index index_type accum ->
        let possible_state_var =
          mk_state_var map
            (node_scope @ I.reserved_scope)
            ident index index_type None
        in
        match possible_state_var with
        | Some state_var ->
            if not (StateVar.is_input state_var) then
              N.add_state_var_def ~is_dep:true state_var
                (N.GeneratedEq (pos, index));
  ...
  let oracles, oracle_state_var_map =
    let over_oracles (oracles, osvm) (id, expr_type, expr) =
      ...
      let closed_sv =
        match expr with
        | A.Ident (_, id') | A.ArrayIndex (_, A.Ident (_, id'), _) ->
            let ident = mk_ident id' in
      ...
      let over_indices =
       fun index index_type accum ->
        let possible_state_var =
          mk_state_var ~is_const:true map
            (node_scope @ I.reserved_scope)
            oracle_ident index index_type (Some N.Oracle)
        in
        match possible_state_var with
        | Some state_var ->
  ...
  let ib_oracles =
    let over_ib_oracles ib_oracles (id, expr_type) =
      ...
      let over_indices =
       fun index index_type accum ->
        let possible_state_var =
          mk_state_var ~is_const:false map
            (node_scope @ I.reserved_scope)
            oracle_ident index index_type (Some N.Oracle)
        in
        match possible_state_var with
  ...
  let calls, glocals =
    ...
    let over_calls =
     fun (calls, glocals)
         (pos, var, cond, restart, ident, args, defaults, inlined) ->
      let node_id = mk_ident ident in
      let called_node = N.node_of_name node_id cstate.nodes in
      ...
      let outputs =
        let over_vars =
         fun index sv compiled_vars ->
          let var_id = mk_ident var in
          let possible_state_var =
            mk_state_var ~force_return:true ~is_input:false map
              (node_scope @ I.reserved_scope)
              var_id index
              (StateVar.type_of_state_var sv)
              (Some N.Call)
          ...
          | Some state_var ->
              let result =
                if SVS.mem state_var !seen_calls then compiled_vars
                else (
      ...
      in
      let node_call =
        compile_node node_scope pos ctx cstate map outputs cond restart node_id
          args defaults inlined
      in
      let glocals' =
        H.fold (fun _ v a -> X.singleton X.empty_index v :: a) local_map []
      in
  ...
  let oracles =
    ...
    (* ****************************************************************** *)
  in
  let node_props, node_eqs, node_asserts, is_main =
    let over_items =
     fun (props, eqs, asserts, is_main) item ->
      match item with
      | A.Body e -> (
          match e with
      ...
      | A.AnnotProperty (p, n, e, k) ->
          ((p, n, e, k) :: props, eqs, asserts, is_main)
      | A.IfBlock _ | A.FrameBlock _ ->
    ...
    in
    List.fold_left over_items ([], [], [], false) items
    (* ****************************************************************** *)
      ...
      let name, src =
        ...
        | Some n ->
            ( HString.string_of_hstring n,
              ...
              else Property.PropAnnot pos )
      in
      let kind =
        match kind with
        | A.Invariant -> Property.Invariant
        | A.Reachable (Some (FromWithin (ts1, ts2))) ->
        ...
        | A.Reachable (Some (Within ts)) ->
            Property.Reachable (Some (Within ts))
        | A.Reachable None -> Property.Reachable None
        | A.Provided _ -> assert false
        (* Should be desugared into one invariant and one reachable property *)
      in
      (sv, name, src, kind)
    in
    List.map op node_props
  in
  let asserts =
    let op (pos, expr) =
  ...
  let asserts =
    let op (pos, expr) =
      ...
      (pos, sv)
    in
    asserts @ List.map op gids.GI.asserts
    (* ****************************************************************** *)
    (* Helpers for generated and user equations                           *)
    (* ****************************************************************** *)
  in
  let compile_struct_item struct_item =
    ...
    | A.ArrayDef (_, i, l) ->
        ...
        List.iteri
          ...
          l;
        (result, indexes)
    | A.TupleStructItem _ | A.TupleSelection _ | A.FieldSelection _
    | A.ArraySliceStructItem _ ->
        assert false (* guaranteed by LustreSyntaxChecks *)
  in
  let rm_array_var_index lst =
    List.filter (function X.ArrayVarIndex _ -> false | _ -> true) lst
  in
  let gen_lhs_bounds is_generated eq_lhs expr indexes =
    List.fold_left
      (fun acc (i, sv) ->
        let result =
          List.fold_left
            (fun (acc, cpt) -> function
              | X.ArrayVarIndex b ->
  ...
  let gequations =
    let over_equations =
     fun eqns (qvars, contract_scope, lhs, ast_expr, _) ->
      map := { !map with contract_scope };
      let eq_lhs, indexes =
        match lhs with
        | A.StructDef (_, []) -> (X.empty, 0)
        | A.StructDef (_, [ A.SingleIdent (_, i) ])
          when GI.StringSet.mem i gids.GI.array_literal_vars ->
        ...
        | A.StructDef (_, l) ->
            ...
            in
            let _, res = List.fold_left over_items (0, X.empty) l in
            (res, 0)
      in
      ...
      equations @ eqns
    in
    List.fold_left over_equations [] gids.GI.equations
    (* ****************************************************************** *)
    (* Node Equations                                                     *)
    (* ****************************************************************** *)
  in
  ...
  let equations =
    let over_equations =
     fun eqns (pos, lhs, ast_expr) ->
      match lhs with
      | A.StructDef (_, []) -> eqns
      | _ ->
          let (eq_lhs, indexes), is_generated =
            match lhs with
          ...
          in
          let lhs_bounds =
            gen_lhs_bounds is_generated eq_lhs ast_expr indexes
          in
  ...
  let assumes, guarantees =
    match contract with
    | Some (_, contract) ->
        compile_contract cstate gids ctx map [] node_scope contract
    | None -> ([], [])
    (* ****************************************************************** *)
  ...
  let assumes, guarantees, props =
    ...
    in
    let over_subrange_constraints (a, ac, g, gc, p)
      ...
      if is_original then
        let scope =
          List.map
            (fun (i, s) -> (i, HString.string_of_hstring s))
            contract_scope
        in
        ...
        | Some N.Assumption ->
            ...
            N.add_state_var_def sv
              (N.ContractItem (pos, contract_sv, N.Assumption));
            (contract_sv :: a, ac + 1, g, gc, p)
        | Some N.Guarantee ->
            let name = create_constraint_name rexpr in
            let contract_sv = C.mk_svar pos gc (Some name) sv scope in
            N.add_state_var_def sv
              (N.ContractItem (pos, contract_sv, N.Guarantee));
  ...
  let assumes, guarantees, props =
    ...
    (* ****************************************************************** *)
  in
  let contract, sofar_local, sofar_equation =
    if assumes != [] || guarantees != [] || modes != [] then
      let sofar_assumption =
        get
          (mk_state_var ~is_input:false map
             (node_scope @ I.reserved_scope)
             (mk_ident (HString.mk_hstring "sofar"))
             X.empty_index Type.t_bool None)
      in
      let assumes =
        List.sort
      ...
      in
      let modes =
        List.sort (fun { C.pos = a } { C.pos = b } -> compare_pos a b) modes
      in
      let sofar_local = X.singleton X.empty_index sofar_assumption in
      let conj_of_assumes =
        assumes |> List.map (fun { C.svar } -> E.mk_var svar) |> E.mk_and_n
      in
      ...
      List.iter
        (fun (pos, def) ->
          ...
          | FCond (A.StructDef (_, [ e ])) ->
              let lhs, _ = compile_struct_item e in
              List.iter
                (fun (_, sv) -> N.add_state_var_def sv (N.FrameBlock pos))
                (X.bindings lhs)
          | _ -> assert false)
        frame_infos
  | None -> ());
  ...
  | Some if_infos ->
      ...
      List.iter
        (fun (pos, lhs) ->
          ...
          List.iter
            (fun (i, sv) ->
              N.add_state_var_def sv (N.ProperEq (pos, rm_array_var_index i)))
            (X.bindings lhs))
        if_infos
  | None -> ());
  (* ****************************************************************** *)
  ...
  let state_var_source_map =
    SVT.fold (fun k v a -> SVM.add k v a) !map.source SVM.empty
  in
    ...
    List.fold_left
      (fun acc (id, h_id) ->
        ...
        List.fold_left2
          ...
          acc (X.bindings svars) (X.bindings h_svars))
      TM.empty
      (StringMap.bindings gids.GI.history_vars)
  in
  let ty_args =
    match Ctx.lookup_node_ty_args ctx i with
    | None -> []
  ...
  in
  { cstate with nodes = node :: cstate.nodes }
...
and compile_const_decl ?(ghost = false) cstate ctx map scope = function
  | A.FreeConst (p, i, ty) ->
      ...
      let over_index =
       fun i ty vt ->
        let possible_state_var =
          mk_state_var ?is_input:(Some false) ?is_const:(Some true)
            ?for_inv_gen:(Some true) map (scope @ I.user_scope) ident i ty None
        in
        match possible_state_var with
        | Some state_var -> X.add i (Var.mk_const_state_var state_var) vt
        | None -> vt
      in
      ...
      if ghost then cstate
        ...
        let global_constraints =
          ...
          if has_subrange || has_ref_type then
            ...
            in
            let ref_type_exprs =
              if has_ref_type then AN.mk_ref_type_expr ctx (A.Ident (p, i)) ty
              else []
            in
            List.map
              (fun expr ->
              ...
              (range_exprs @ ref_type_exprs)
            @ cstate.global_constraints
  ...
  | A.UntypedConst (_, id, expr) | A.TypedConst (_, id, expr, _) ->
      if ghost then (
...
and compile_type_decl pos ctx cstate = function
  | A.AliasType (_, ident, ps, ltype) ->
      ...
      { cstate with type_alias }
  | A.FreeType (_, ident) ->
      let empty_map = ref (empty_identifier_maps None) in
      let t =
        compile_ast_type cstate ctx empty_map (A.AbstractType (pos, ident))
      in
      let type_alias = StringMap.add ident t cstate.type_alias in
      { cstate with type_alias }
...
and compile_declaration :
 ...
 fun cstate gids ctx decl ->
  ...
  | A.FuncDecl
      (_, (i, ext, opac, params, inputs, outputs, locals, items, contract)) ->
      compile_node_decl gids true opac cstate ctx i ext params inputs outputs
        locals items contract
  | A.NodeDecl
      (_, (i, ext, opac, params, inputs, outputs, locals, items, contract)) ->
      compile_node_decl gids false opac cstate ctx i ext params inputs outputs
        locals items contract
