...
type decl = NodeOrFun | Type | Contract | Const
...
let dep_add dep decl ident =
  (try DeclMap.find dep decl with Not_found -> ISet.empty)
  |> ISet.add ident |> DeclMap.replace dep decl

(** Checks if a dependency map mentions a declaration. *)
let dep_mem dep decl ident =
  try DeclMap.find dep decl |> ISet.mem ident with Not_found -> false
...
let add deps (key_type, key_ident) (val_type, val_ident) =
  let deps =
    try DeclMap.find deps key_type
    with Not_found ->
...
let info_of_decl = function
  ...
  | decl ->
      Format.asprintf "info requested on unsupported declaration: %a"
