...
let mk_fresh_temp_var ty =
  ...
  let gids2 = { (GI.empty ()) with locals = GI.StringMap.singleton name ty } in
  (name, gids2)

(** Takes in an equation LHS and returns * updated gids with temp variables, *
    equations setting original variable equal to temp variables, * updated
    context *)
  ...
  let convert_struct_item = function
    ...
    *)
    | ArrayDef (p, i, js) as si ->
        ...
        let array_index =
          List.fold_left
            (fun expr j -> A.ArrayIndex (rhs_pos, expr, A.Ident (rhs_pos, j)))
            (A.Ident (rhs_pos, temp))
            js
        in
        ( [ gids ],
...
let remove_mult_assign_from_ni ctx ni =
  let rec helper ctx ni =
    ...
    | A.Body (Equation (_, lhs, expr)) ->
        ...
        if List.length lhs_vars = 1 then ([ ni ], [])
    ...
    | IfBlock (pos, e, l1, l2) ->
        ...
        ( [ A.IfBlock (pos, e, List.flatten nis1, List.flatten nis2) ],
          List.flatten gids1 @ List.flatten gids2 )
    | FrameBlock (pos, vars, nes, nis) ->
        ...
        let nis1 =
          ...
          |> List.map (fun x ->
                 match x with
                 | A.Body (Equation _ as e) -> e
                 | _ -> assert false)
        in
        (* nis1 and nis3 are the temp variables need to get pulled outside the if block *)
        ( [ A.FrameBlock (pos, vars, nis1, List.flatten nis2) ],
          List.flatten gids1 @ List.flatten gids2 )
    ...
    | A.Body (Assert _) | A.AnnotProperty _ | A.AnnotMain _ -> ([ ni ], [])
  in
...
let desugar_node_item ctx ni =
  match ni with
  | A.IfBlock _ | A.FrameBlock _ -> remove_mult_assign_from_ni ctx ni
  | _ -> (ni, GI.empty ())
...
let remove_mult_assign ctx gids sorted_node_contract_decls =
  let decls, gids2 =
    List.map (desugar_node_decl ctx) sorted_node_contract_decls |> List.split
  in
