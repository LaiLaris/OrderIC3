...
(** Internal reperesentation of a Lustre expression
    ...
    A {!LustreExpr.t} does not contain node calls, temporal operators or
    expressions under a [pre] operator.
    ...
    expression [pre x] can be represented by the variable [x] at the previous
    state.

    A non-variable expression under a [pre] has to be abstracted to a fresh
    variable that is defined by this expression. There are no node calls in a
    Lustre expression. They have to be abstracted out and the results are
    captured in fresh variables. See {!LustreSimplify} for details about how the
    input file is translates

    The offsets of state variable instances are zero for the initial state and
    zero for the current state, see the constants {!init_offset} and
    {!trans_offset} and {!pre_offset}. These are different from the offsets in
    the transition system, because here we want to know if the initial and the
    step expressions are equal without bumping offsets. Use the functions
    {!base_term_of_expr}, {!cur_term_of_expr}, and {!pre_term_of_expr} to take
    the expression on one side of the [->] operator and adjust to the variable
    offsets to the given base.

    Expressions can only be constructed with the constructors which do type
    checking and some easy simplifications with constants.
...
(** A {!Term.t} representing a Lustre expression, state variable offsets refer
    to the current and the previous instant.

    Cannot be constructed outside this module to enforce invariants about
    allowed offsets of state variable instants, see the constructors below. *)

(** Type of index in an equation for an array *)
type 'a bound_or_fixed =
  | Bound of 'a
      (** Equation is for each value of the index variable between zero and the
          upper bound *)
...
(** A Lustre expression
    ...
    The [->] operator is moved to the top of the expression, the expression is
    to be interpreted as [expr_init -> expr_step]. *)
...
(** {1 Pretty-printers}
    ...
    All pretty-printers take a Boolean flag as first argument, indicating if
    identifiers should be valid Lustre identifiers. If the flag is [false],
    indexed identifiers are printed with [\[], [\]] and [.]. These characters
    are replaced with [_] if the flag is [true]. *)
...
(** Pretty-print a Lustre expression. The optional parameter [as_type] indicates
    that the expression should be printed as if it had this type (used for
    encoded enumerated datatypes). *)
...
val pp_print_term_as_expr_pvar :
  ...
  Term.t ->
  unit
(** Pretty-print a term as an expr using the given printing function for state
    vars **)
...
val pp_print_term_as_expr_mvar :
  ...
  Term.t ->
  unit
(** Pretty-print a term as an expr using the given map from state vars to
    strings If a state variable is not in the map, the name of the state
...
(** These offsets are different from the offsets in the transition system,
    because here we want to know if the initial and the step expressions are
    equal without bumping offsets. Use the constants {!base_offset},
    {!cur_offset} or {!pre_offset} of this module, or the constants
    {!TransSys.init_base} and {!TransSys.trans_base} for the zero offsets. *)
...
(** Term of instance of state variable at first instant with the given offset as
    zero *)

val cur_term_of_state_var : Numeral.t -> StateVar.t -> Term.t
(** Term of instance of state variable at current instant with the given offset
    as zero *)

val pre_term_of_state_var : Numeral.t -> StateVar.t -> Term.t
(** Term of instance of state variable at previous instant with the given offset
    as zero *)
...
(** Return the state variable of a variable
    ...
    Fail with [Invalid_argument "state_var_of_expr"] if the expression is not a
    variable at the current or previous offset. *)

val var_of_expr : t -> Var.t
(** Return the free variable of a expresion

    Fail with [Invalid_argument "var_of_expr"] if the expression is not a free
    variable. *)

val var_of_init_expr : t -> Var.t
(** Return the free variable of the initial state expression

    Fail with [Invalid_argument "var_of_init_expr"] if the initial state
    expression is not a free variable. *)
...
(** Return all state variables of the initial state expression at the base
    instant *)
...
(** Split a list of Lustre expressions into a list of pairs of expressions for
    the initial step and the transition steps, respectively *)
...
(** The constructors do some easy simplifcations, such as arithmetic and Boolean
    operations on constants. If the types of the arguments are do not match the
    signature of the symbol, the {!Type_mismatch} exception is raised. *)
...
val mk_pre : t -> t
(** [mk_pre e] returns the expression [e] if it is a constant, and the previous
    state variable if the expression is a current state variable. *)

val mk_pre_with_context :
  ('a -> t -> 'b * 'a) -> ('b -> Term.t) -> 'a -> bool -> t -> t * 'a
(** Apply the [pre] operator to the expression, abstract the expression to a
    fresh variable if it is not a variable at the current state.

    [mk_pre_with_context f c b e] returns the expression [e] and context [c]
    unchanged if it is a constant, and the previous state variable if the
    expression is a current state variable, again together with [c] unchanged.

    Otherwise the expression [e] is abstracted to a fresh variable obtained by
    calling the function [f], which returns a fresh state variable and a changed
    context [c] that records the association between the fresh variable and the
    expression. Then return an expression of the fresh state variable and the
    changed context.

    [b] is used to denote that we're in a context where there are unguarded pres
...
val push_select : t -> t
(** Return an expression where the top-level selects of the initial and/or the
    step expressions (if any) have been pushed *)
