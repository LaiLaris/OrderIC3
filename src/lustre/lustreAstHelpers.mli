...
(** Returns whether or not the expression is a Bool Const variant with the True
    value *)

val expr_is_false : expr -> bool
(** Returns whether or not the expression is a Bool Const variant with the False
    value *)
...
(** Returns the arity of a type, a function (TArr) has arity `(a, b)` where `a`
    is the number of inputs and `b` is the number of outputs, every other type
    has arity `(0, 0)` *)

val type_contains_array : lustre_type -> bool
(** Returns true if the lustre type expression contains an array or if it is an
    array *)

val substitute_naive : HString.t -> expr -> expr -> expr
(** Substitute second param for first param in third param. AnyOp and Quantifier
    are not supported due to introduction of bound variables. *)

val apply_subst_in_expr : (HString.t * expr) list -> expr -> expr
(** [apply_subst_in_expr s e] applies the substitution defined by association
    list [s] to the expression [e] AnyOp and Quantifier are not supported due to
    introduction of bound variables. *)

val apply_subst_in_type : (HString.t * expr) list -> lustre_type -> lustre_type
(** [apply_subst_in_type s t] applies the substitution defined by association
    list [s] to the expressions of (possibly dependent) type [t] AnyOp and
    Quantifier are not supported due to introduction of bound variables. *)

val apply_type_subst_in_type :
  (HString.t * lustre_type) list -> lustre_type -> lustre_type
(** [apply_type_subst_in_type s t] applies the (type-level) substitution defined
    by association list [s] to type [t]. *)
...
(** Returns true iff a contract mentions a `pre` or a `->`. Does not (cannot)
    check contract calls recursively, checks only inputs and outputs. *)
...
(** [vars_of_node_calls e] returns all variable identifiers within arguments of
    node calls that appear in the expression [e] (while excluding node call
    identifiers) *)

val vars_without_node_call_ids : expr -> SI.t
(** [vars_without_node_call_ids e] returns all variable identifiers that appear
    in the expression [e] while excluding node call identifiers *)
...
(** returns all variables that appear in a [struct_item] (the lhs of an
    equation) with associated positions *)
...
(** returns all the variables that appear in the lhs of the equation of the node
    body with associated positions *)

val vars_of_ty_ids : typed_ident -> SI.t
(** returns a singleton set with the only identifier in a typed identifier
    declaration *)

val calls_of_expr : expr -> SI.t
(** [calls_of_expr e] returns all node/function names for those nodes/functions
    called in [e] *)

val vars_of_type : lustre_type -> SI.t
(** [vars_of_type ty] returns all variable identifiers that appear in the type
    [ty] while excluding node call identifiers and refinement type bound
    variables *)
...
(** Splits the program into two. First component are the type and constant
    declarations and Second component are the nodes, contract and function
    declarations. *)

val abstract_pre_subexpressions : expr -> expr
(** Abstracts out the pre expressions into a constant so that the built graph
...
(** Extracts out all the node equations as an associated list of rhs and lhs of
    the equation *)
...
(** Check syntactic equality o Lustre expressions (ignoring positions) up to a
    certain optional depth. If the depth is reached, then [Error ()] is
    returned, otherwise [Ok false] if the two expressions are unequal and
    [Ok true] if they are equal. *)

val syn_type_equal :
  int option -> lustre_type -> lustre_type -> (bool, unit) result
(** Check syntactic equality of Lustre types (ignoring positions) up to a
    certain optional depth. If the depth is reached, then [Error ()] is
    returned, otherwise [Ok false] if the two expressions are unequal and
    [Ok true] if they are equal.*)

val hash : int option -> expr -> int
(** Compute the hash of an AST expression to the given depth. After the depth
    limit is reached the same hash value is assigned to every sub expression.
    This function does not include position information in the hash. *)

val rename_contract_vars : expr -> expr
(** Rename contract variables from internal names (with format #_contract_var)
    to syntax names *)

val name_of_prop :
  Lib.position -> HString.t option -> LustreAst.prop_kind -> HString.t
