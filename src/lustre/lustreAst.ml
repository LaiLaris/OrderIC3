...
module SI = struct
  ...
  let flatten : t list -> t = fun sets -> List.fold_left union empty sets
end
...
type unary_operator = Not | Uminus | BVNot
...
type ternary_operator = Ite
type comparison_operator = Eq | Neq | Lte | Lt | Gte | Gt
type constant = True | False | Num of HString.t | Dec of HString.t
...
and label_or_index = Label of position * index | Index of position * expr
...
type eq_lhs = StructDef of position * struct_item list

(* The left-hand side of an equation in a contract *)
type contract_eq_lhs = GhostVarDec of position * typed_ident list
...
type prop_kind = Invariant | Reachable of prop_bound option | Provided of expr
...
type node_item =
  ...
  | FrameBlock of
      position * (position * ident) list * node_equation list * node_item list
...
(* A contract call. *)
type contract_call =
  position * ident * lustre_type list * expr list * ident list
...
type opacity = Default | Opaque | Transparent
...
let rec pp_print_expr ppf =
  let ppos ppf p = Format.ifprintf ppf "%a" pp_print_position p in
  ...
  let p1 p s e =
    Format.fprintf ppf "@[<hv 2>%a(%s %a)@]" ppos p s pp_print_expr e
  in

  (* Pretty-print a binary infix operator *)
  let p2 p s e1 e2 =
    Format.fprintf ppf "@[<hv 2>%a(%a %s@ %a)@]" ppos p pp_print_expr e1 s
      pp_print_expr e2
  in

  (* Pretty-print a ternary infix operator *)
  let p3 p s1 s2 s3 e1 e2 e3 =
    Format.fprintf ppf "@[<hv 2>%a(%s@ %a@;<1 -1>%s@ %a@;<1 -1>%s@ %a)@]" ppos p
      s1 pp_print_expr e1 s2 pp_print_expr e2 s3 pp_print_expr e3
  in
  ...
  | ModeRef (p, ids) ->
      Format.fprintf ppf "%a::%a" ppos p (pp_print_list pp_print_ident "::") ids
  | GroupExpr (p, ExprList, l) ->
      Format.fprintf ppf "%a@[<hv 1>(%a)@]" ppos p pl l
  | GroupExpr (p, TupleExpr, l) ->
      Format.fprintf ppf "%a@[<hv 1>{%a}@]" ppos p pl l
  | GroupExpr (p, ArrayExpr, l) ->
      Format.fprintf ppf "%a@[<hv 1>[%a]@]" ppos p pl l
  | StructUpdate (_, e1, i, e2) ->
      Format.fprintf ppf "@[<hv 1>(%a@ with@ @[<hv>%a@] =@ %a)@]" pp_print_expr
        e1
        (pp_print_list pp_print_label_or_index "")
        i pp_print_expr e2
  | ArrayConstr (p, e1, e2) ->
      Format.fprintf ppf "%a@[<hv 1>(%a^%a)@]" ppos p pp_print_expr e1
        pp_print_expr e2
  | ArrayIndex (p, e, l) ->
      Format.fprintf ppf "%a@[<hv 1>%a@[<hv 1>[%a]@]@]" ppos p pp_print_expr e
        pp_print_expr l
  | RecordProject (p, e, f) ->
      Format.fprintf ppf "%a%a.%a" ppos p pp_print_expr e pp_print_index f
  | RecordExpr (p, t, ty_args, l) ->
      Format.fprintf ppf "%a@[<hv 1>%a%t {%a}@]" ppos p pp_print_ident t
        (function
          | ppf -> (
              match ty_args with
              | [] -> ()
              | _ ->
                  Format.fprintf ppf "<<%a>>"
                    (pp_print_list pp_print_lustre_type ";")
                    ty_args))
        (pp_print_list pp_print_field_assign ";@ ")
        l
  | TupleProject (p, e, f) ->
      Format.fprintf ppf "%a%a.%%%a" ppos p pp_print_expr e Format.pp_print_int
        f
  ...
  | Quantifier (_, Forall, vars, e) ->
      Format.fprintf ppf "@[<hv 2>forall@ @[<hv 1>(%a)@]@ %a@]"
        (pp_print_list pp_print_typed_decl ";@ ")
        vars pp_print_expr e
  ...
  | When (p, e1, e2) ->
      Format.fprintf ppf "%a%a %a" ppos p pp_print_expr e1 pp_print_clock_expr
        e2
  | Condact (p, e1, er, n, e2, e3) ->
      Format.fprintf ppf "%acondact(%a,(restart %a every %a)(%a),%a)" ppos p
        pp_print_expr e1 pp_print_ident n pp_print_expr er
        (pp_print_list pp_print_expr ",@ ")
        e2
        (pp_print_list pp_print_expr ",@ ")
        e3
  | Activate (_, i, c, r, l) ->
      Format.fprintf ppf "(activate (restart %a every %a) every %a) (%a)"
        pp_print_ident i pp_print_expr r pp_print_expr c
        (pp_print_list pp_print_expr ",@ ")
        l
  | Merge (_, c, l) ->
      Format.fprintf ppf "merge(%a,@ %a)" pp_print_ident c
        (pp_print_list
           (fun fmt (c, e) ->
             Format.fprintf fmt "%a -> %a" pp_print_ident c pp_print_expr e)
           ",@ ")
        l
  | RestartEvery (_, i, l, c) ->
      Format.fprintf ppf "(restart %a every %a)(%a)" pp_print_ident i
        pp_print_expr c
        (pp_print_list pp_print_expr ",@ ")
        l
  | Pre (p, e) -> p1 p "pre" e
  ...
  | Call (p, [], id, l) ->
      Format.fprintf ppf "%a%a(%a)" ppos p pp_print_ident id
        (pp_print_list pp_print_expr ",@ ")
        l
  | Call (p, ty_args, id, l) ->
      Format.fprintf ppf "%a%a<<%a>>(%a)" ppos p pp_print_ident id
        (pp_print_list pp_print_lustre_type ";")
        ty_args
        (pp_print_list pp_print_expr ",@ ")
        l
  | AnyOp (p, id, e1, Some e2) ->
      Format.fprintf ppf "%aany { %a | %a } assuming %a" ppos p
        pp_print_typed_ident id pp_print_expr e1 pp_print_expr e2
  | AnyOp (p, id, e, None) ->
      Format.fprintf ppf "%aany { %a | %a }" ppos p pp_print_typed_ident id
        pp_print_expr e
...
and pp_print_field_assign ppf (i, e) =
  Format.fprintf ppf "@[<hv 2>%a =@ %a@]" pp_print_index i pp_print_expr e
...
and pp_print_lustre_type ppf = function
  ...
  | IntRange (_, l, u) ->
      let pp_print_opt ppf expr_opt =
        match expr_opt with
        | Some expr -> pp_print_expr ppf expr
        | None -> Format.fprintf ppf "%s" unbounded_limit_string
      in
      Format.fprintf ppf "subrange [%a,%a] of int" pp_print_opt l pp_print_opt u
  | Real _ -> Format.fprintf ppf "real"
  | UserType (_, [], s) -> Format.fprintf ppf "%a" pp_print_ident s
  | UserType (_, tys, s) ->
      Format.fprintf ppf "%a<<%a>>" pp_print_ident s
        (pp_print_list pp_print_lustre_type "; ")
        tys
  | AbstractType (_, s) -> Format.fprintf ppf "%a" pp_print_ident s
  | TupleType (_, l) ->
  ...
  | GroupType (_, l) ->
      Format.fprintf ppf "@[<hv 1>(%a)@]"
        ...
        l
  | RecordType (_, i, l) ->
      Format.fprintf ppf "%a @[<hv 2>{ %a }@]" pp_print_ident i
        (pp_print_list pp_print_typed_ident ";@ ")
        l
  | ArrayType (_, (t, e)) ->
      Format.fprintf ppf "%a^%a" pp_print_lustre_type t pp_print_expr e
  | EnumType (_, _, l) ->
      Format.fprintf ppf "enum @[<hv 2>{ %a }@]"
        (pp_print_list HString.pp_print_hstring ",@ ")
        l
  | TArr (_, arg_ty, ret_ty) ->
      Format.fprintf ppf "@[%a->@,%a@]" pp_print_lustre_type arg_ty
        pp_print_lustre_type ret_ty
  | RefinementType (_, (_, id, ty), expr1) ->
      Format.fprintf ppf "subtype { %a: %a | %a }" pp_print_ident id
        pp_print_lustre_type ty pp_print_expr expr1
  | History (_, i) -> Format.fprintf ppf "history(%a)" pp_print_ident i

(* Pretty-print a typed identifier *)
and pp_print_typed_ident ppf (_, s, t) =
  Format.fprintf ppf "@[<hov 2>%a:@ %a@]" HString.pp_print_hstring s
...
and pp_print_clocked_typed_ident ppf (_, s, t, c) =
  Format.fprintf ppf "@[<hov 2>%a:@ %a%a@]" pp_print_ident s
    pp_print_lustre_type t pp_print_clock_expr c
...
and pp_print_const_clocked_typed_ident ppf (_, s, t, c, o) =
  Format.fprintf ppf "@[<hov 2>%t%a:@ %a%a@]"
    ...
    pp_print_ident s pp_print_lustre_type t pp_print_clock_expr c
...
let pp_print_type_decl ppf = function
  | AliasType (_, s, [], t) ->
      Format.fprintf ppf "@[<hv 2>%a =@ %a@]" pp_print_ident s
        pp_print_lustre_type t
  | AliasType (_, s, p, t) ->
      Format.fprintf ppf "@[<hv 2>%a<<%a>> =@ %a@]" pp_print_ident s
        (pp_print_list pp_print_ident ";")
        p pp_print_lustre_type t
  | FreeType (_, t) -> Format.fprintf ppf "%a" pp_print_ident t
...
let pp_print_const_decl ppf = function
  | FreeConst (_, s, t) ->
      Format.fprintf ppf "@[<hov 2>const %a:@ %a;@]" pp_print_ident s
        pp_print_lustre_type t
  | UntypedConst (_, s, e) ->
      Format.fprintf ppf "@[<hov 2>const %a =@ %a;@]" pp_print_ident s
        pp_print_expr e
  | TypedConst (_, s, e, t) ->
      Format.fprintf ppf "@[<hov 2>const %a:@ %a =@ %a;@]" pp_print_ident s
        pp_print_lustre_type t pp_print_expr e

(* Pretty-print a single static node parameter *)
let pp_print_node_param ppf t = Format.fprintf ppf "%a" pp_print_ident t
...
let pp_print_node_param_list ppf = function
  ...
  | l ->
      Format.fprintf ppf "@[<hv 2><<%a>>@]"
        (pp_print_list pp_print_node_param ";@ ")
        l
...
let pp_print_node_local_decl ppf l =
  ...
  if c = [] then ()
    ...
    Format.fprintf ppf "%a@ "
      (pp_print_list pp_print_node_local_decl_const "@;<1 -2>")
      c;

  if v = [] then ()
  else
    Format.fprintf ppf "@[<hv 2>var@ %a@]@ "
      (pp_print_list pp_print_node_local_decl_var "@ ")
      v

let pp_print_array_def_index ppf ident =
  Format.fprintf ppf "[%a]" pp_print_ident ident

let rec pp_print_struct_item ppf = function
  | SingleIdent (_, s) -> Format.fprintf ppf "%a" pp_print_ident s
  | TupleStructItem (_, l) ->
      Format.fprintf ppf "@[<hv 1>[%a]@]"
        (pp_print_list pp_print_struct_item ",@ ")
        l
  | TupleSelection (_, e, i) ->
      Format.fprintf ppf "%a[%a]" pp_print_ident e pp_print_expr i
  | FieldSelection (_, e, i) ->
      Format.fprintf ppf "%a.%a" pp_print_ident e pp_print_ident i
  | ArraySliceStructItem (_, e, i) ->
      Format.fprintf ppf "%a@[<hv 1>[%a]@]" pp_print_ident e
        (pp_print_list pp_print_array_slice ",@ ")
        i
  | ArrayDef (_, i, l) ->
      Format.fprintf ppf "%a%a" pp_print_ident i
        (pp_print_list pp_print_array_def_index "")
        l

let pp_print_eq_lhs ppf = function
  | StructDef (_, [ l ]) -> pp_print_struct_item ppf l
  | StructDef (_, l) ->
      Format.fprintf ppf "(%a)" (pp_print_list pp_print_struct_item ",") l

let pp_print_contract_eq_lhs ppf = function
  | GhostVarDec (_, [ (_, l, _) ]) -> pp_print_ident ppf l
  | GhostVarDec (_, l) ->
      Format.fprintf ppf "(%a)"
        (pp_print_list pp_print_ident ", ")
        (List.map (fun (_, i, _) -> i) l)

let pp_print_typed_contract_eq_lhs ppf = function
  | GhostVarDec (_, l) ->
      Format.fprintf ppf "%a" (pp_print_list pp_print_typed_ident ", ") l

let rec pp_print_node_body ppf = function
  | Assert (_, e) -> Format.fprintf ppf "assert %a;" pp_print_expr e
  | Equation (_, lhs, e) ->
      Format.fprintf ppf "@[<hv 2>%a =@ %a;@]" pp_print_eq_lhs lhs pp_print_expr
        e
...
and pp_print_node_item ppf = function
  ...
  | IfBlock (_, e, l1, []) ->
      Format.fprintf ppf "if %a then %a fi" pp_print_expr e
        (pp_print_list pp_print_node_item " ")
        l1
  | IfBlock (_, e, l1, l2) ->
      Format.fprintf ppf "if %a then %a else  %a fi" pp_print_expr e
        (pp_print_list pp_print_node_item " ")
        l1
        (pp_print_list pp_print_node_item " ")
        l2
  | FrameBlock (_, vars, nes, nis) ->
      Format.fprintf ppf "frame (%a) %a let %a tel"
        (pp_print_list pp_print_ident ", ")
        (List.map snd vars)
        (pp_print_list pp_print_node_body " ")
        nes
        (pp_print_list pp_print_node_item " ")
        nis
  ...
  | AnnotProperty (_, None, e, Reachable (Some (From b))) ->
      Format.fprintf ppf "--%%PROPERTY reachable %a from %d;" pp_print_expr e b
  | AnnotProperty (_, None, e, Reachable (Some (Within b))) ->
      Format.fprintf ppf "--%%PROPERTY reachable %a within %d;" pp_print_expr e
        b
  | AnnotProperty (_, None, e, Reachable (Some (At b))) ->
      Format.fprintf ppf "--%%PROPERTY reachable %a at %d;" pp_print_expr e b
  ...
  | AnnotProperty (_, None, e1, Provided e2) ->
      Format.fprintf ppf "--%%PROPERTY %a provided %a;" pp_print_expr e1
        pp_print_expr e2
  | AnnotProperty (_, Some name, e, Invariant) ->
      Format.fprintf ppf "--%%PROPERTY \"%a\" %a;" HString.pp_print_hstring name
        pp_print_expr e
  | AnnotProperty (_, Some name, e, Reachable (Some (From b))) ->
      Format.fprintf ppf "--%%PROPERTY reachable \"%a\" %a from %d;"
        HString.pp_print_hstring name pp_print_expr e b
  ...
  | AnnotProperty (_, Some name, e, Reachable (Some (FromWithin (l, u)))) ->
      Format.fprintf ppf "--%%PROPERTY reachable \"%a\" %a from %d within %d;"
        HString.pp_print_hstring name pp_print_expr e l u
  | AnnotProperty (_, Some name, e, Reachable None) ->
      Format.fprintf ppf "--%%PROPERTY reachable \"%a\" %a;"
        HString.pp_print_hstring name pp_print_expr e
  | AnnotProperty (_, Some name, e1, Provided e2) ->
      Format.fprintf ppf "--%%PROPERTY \"%a\" %a provided %a;"
        HString.pp_print_hstring name pp_print_expr e1 pp_print_expr e2

let pp_print_contract_ghost_const ppf = function
  | FreeConst (_, s, t) ->
      Format.fprintf ppf "@[<hv 3>const %a:@ %a;@]" pp_print_ident s
        pp_print_lustre_type t
  | UntypedConst (_, s, e) ->
      Format.fprintf ppf "@[<hv 3>const %a =@ %a;@]" pp_print_ident s
        pp_print_expr e
  | TypedConst (_, s, e, t) ->
      Format.fprintf ppf "@[<hv 3>const %a:@ %a =@ %a;@]" pp_print_ident s
        pp_print_lustre_type t pp_print_expr e

let pp_print_contract_ghost_vars ppf =
 fun (_, lhs, e) ->
  Format.fprintf ppf "@[<hv 3>var %a =@ %a;@]" pp_print_typed_contract_eq_lhs
    lhs pp_print_expr e

let pp_print_contract_assume ppf (_, n, s, e) =
  Format.fprintf ppf "@[<hv 3>%sassume%s@ %a;@]"
    (if s then "weakly " else "")
    (match n with
    | None -> ""
    | Some s -> " \"" ^ HString.string_of_hstring s ^ "\"")
    pp_print_expr e

let pp_print_contract_guarantee ppf (_, n, s, e) =
  Format.fprintf ppf "@[<hv 3>%sguarantee%s@ %a;@]"
    (if s then "weakly " else "")
    (match n with
    | None -> ""
    | Some s -> " \"" ^ HString.string_of_hstring s ^ "\"")
...
let pp_print_contract_require ppf (_, n, e) =
  Format.fprintf ppf "@[<hv 3>require%s@ %a;@]"
    (match n with
    | None -> ""
    | Some s -> " \"" ^ HString.string_of_hstring s ^ "\"")
    pp_print_expr e
...
let pp_print_contract_ensure ppf (_, n, e) =
  Format.fprintf ppf "@[<hv 3>ensure%s@ %a;@]"
    (match n with
    | None -> ""
    | Some s -> " \"" ^ HString.string_of_hstring s ^ "\"")
    pp_print_expr e
...
let pp_print_contract_mode ppf (_, id, reqs, enss) =
  Format.fprintf ppf "@[<hv 2>mode %a (%a%a%a%a@]%a) ;" pp_print_ident id
    (cond_new_line (reqs <> []))
    ()
    (pp_print_list pp_print_contract_require "@ ")
    reqs
    (cond_new_line (enss <> []))
    ()
    (pp_print_list pp_print_contract_ensure "@ ")
    enss
...
let pp_print_contract_call fmt (_, id, tys, in_params, out_params) =
  match tys with
  | [] ->
      Format.fprintf fmt "@[<hov 2>import %a (@,%a@,) returns (@,%a@,) ;@]"
        pp_print_ident id
        (pp_print_list pp_print_expr ", ")
        in_params
        (pp_print_list pp_print_ident ", ")
        out_params
  | tys ->
      Format.fprintf fmt "@[<hov 2>import %a<<%a>>(@,%a@,) returns (@,%a@,) ;@]"
        pp_print_ident id
        (pp_print_list pp_print_lustre_type "; ")
        tys
        (pp_print_list pp_print_expr ", ")
...
let pp_print_contract_assump_vars fmt (_, vars) =
  Format.fprintf fmt "@[<hov 2>assumption_vars %a ;@]"
    (pp_print_list pp_print_ident ", ")
...
let pp_print_contract fmt contract =
  Format.fprintf fmt "@[<v>%a@]"
    (pp_print_list pp_print_contract_item "@ ")
    contract

let pp_print_contract_spec ppf = function
  | None -> ()
  | Some (_, contract) ->
      Format.fprintf ppf "@[<v 2>(*@contract@ %a@]@ *)@ " pp_print_contract
        contract

(* Pretty-prints a contract node. *)
let pp_print_contract_node_decl ppf (n, p, i, o, (_, e)) =
  Format.fprintf ppf
    "@[<hv>@[<hv 2>contract %a%t@ @[<hv 1>(%a)@]@;\
     <1 -2>returns@ @[<hv 1>(%a)@];@]@.@[<hv 2>let@ %a@;\
     <1 -2>tel;@]@]@?"
    pp_print_ident n
    (function ppf -> pp_print_node_param_list ppf p)
    (pp_print_list pp_print_const_clocked_typed_ident ";@ ")
    i
    (pp_print_list pp_print_clocked_typed_ident ";@ ")
    o pp_print_contract e

let pp_print_node_or_fun_decl is_fun ppf (_, (n, ext, opac, p, i, o, l, e, r)) =
  if e = [] then
    Format.fprintf ppf
      "@[<hv>@[<hv 2>%s%s%s %a%t@ @[<hv 1>(%a)@]@;\
       <1 -2>returns@ @[<hv 1>(%a)@];@]@.%a@?%a@?@]@?"
      ...
      i
      (pp_print_list pp_print_clocked_typed_ident ";@ ")
      o pp_print_contract_spec r pp_print_node_local_decl l
  else
    Format.fprintf ppf
      "@[<hv>@[<hv 2>%s%s %a%t@ @[<hv 1>(%a)@]@;\
       <1 -2>returns@ @[<hv 1>(%a)@];@]@.%a@?%a@?@[<v 2>let@ %a@;\
       <1 -2>tel;@]@]@?"
      (if is_fun then "function" else "node")
      (if ext then " imported" else "")
      pp_print_ident n
...
let pp_print_declaration ppf = function
  | TypeDecl (_, t) -> Format.fprintf ppf "type %a;" pp_print_type_decl t
  | ConstDecl (_, c) -> pp_print_const_decl ppf c
  | NodeDecl (span, decl) -> pp_print_node_or_fun_decl false ppf (span, decl)
  | FuncDecl (span, decl) -> pp_print_node_or_fun_decl true ppf (span, decl)
  | ContractNodeDecl (_, decl) -> pp_print_contract_node_decl ppf decl
  | NodeParamInst (_, (n, _, p)) ->
      Format.fprintf ppf "@[<hv>@[<hv 2>node %a =@ %a@[<hv 2><<%a>>@];@]"
        pp_print_ident n pp_print_ident n
        (pp_print_list pp_print_lustre_type "@ ")
        p

let pp_print_program ppf p =
  Format.fprintf ppf "@[<v>%a@]" (pp_print_list pp_print_declaration "@ ") p
