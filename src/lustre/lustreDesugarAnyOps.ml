 ...
 fun ctx node_name fun_ids expr ->
  ...
  | A.AnyOp (pos, (_, id, ty), expr1, expr2_opt) ->
      ...
      let contract =
        ...
        | Some expr2 ->
            [
              A.Assume (AH.pos_of_expr expr2, None, false, expr2);
      ...
      let inputs =
        ...
        let vars_of_exprs =
          ...
          | Some expr2 ->
              ...
              in
              Ctx.SI.diff vars_of_expr1_and_expr2 (Ctx.SI.singleton id)
        in
      ...
      let inputs =
        List.map
          (fun input -> (pos, input, Ctx.lookup_ty ctx input, A.ClockTrue))
          inputs
      in
      let inputs =
        List.map
          (fun (p, inp, opt, cl) ->
            ...
            | Some ty ->
                let is_const =
                  match Ctx.lookup_const ctx inp with
                  | Some _ -> true
                  | None -> false
                in
    ...
    Otherwise, we generate an imported function. *)
      let has_pre_arrow_or_node_call =
        ...
        | Some expr2 ->
            let node_calls1 =
              AH.calls_of_expr expr1 |> Ctx.SI.elements
              |> List.filter (fun i -> not (List.mem i fun_ids))
            in
            let node_calls2 =
              AH.calls_of_expr expr2 |> Ctx.SI.elements
              |> List.filter (fun i -> not (List.mem i fun_ids))
            in
            AH.has_pre_or_arrow expr1 != None
            || node_calls1 != []
            || AH.has_pre_or_arrow expr2 != None
            || node_calls2 != []
        | None ->
            let node_calls1 =
              AH.calls_of_expr expr1 |> Ctx.SI.elements
      ...
      let ty_params =
        Ctx.SI.union
          (Ctx.ty_vars_of_type ctx node_name ty)
      ...
      let ty_params =
        ...
        | Some expr2 ->
            ty_params
            @ Ctx.SI.elements (Ctx.ty_vars_of_expr ctx node_name expr2)
        | None -> ty_params
      in
  ...
  | RecordExpr (pos, ident, ps, expr_list) ->
      ...
      ( RecordExpr (pos, ident, ps, List.combine id_list expr_list),
        List.flatten gen_nodes )
  ...
  | Condact (pos, e1, e2, id, expr_list1, expr_list2) ->
      ...
      ( Condact (pos, e1, e2, id, expr_list1, expr_list2),
        gen_nodes1 @ gen_nodes2 @ List.flatten gen_nodes3
        @ List.flatten gen_nodes4 )
  ...
  | Merge (pos, ident, expr_list) ->
      ...
      in
      let expr_list, gen_nodes = List.split exprs_gen_nodes in
      ( Merge (pos, ident, List.combine id_list expr_list),
        List.flatten gen_nodes )
  | RestartEvery (pos, ident, expr_list, e) ->
      ...
      ( RestartEvery (pos, ident, expr_list, e),
        List.flatten gen_nodes1 @ gen_nodes2 )
  | Pre (pos, e) ->
      let e, gen_nodes = rec_call e in
      (Pre (pos, e), gen_nodes)
  | Arrow (pos, e1, e2) ->
...
let desugar_contract_item :
    Ctx.tc_context ->
    HString.t ->
    HString.t list ->
    A.contract_node_equation ->
    A.contract_node_equation * A.declaration list =
 fun ctx node_name fun_ids ci ->
  ...
  | Mode (pos, i, reqs, enss) ->
      ...
      in
      ( Mode (pos, i, reqs, enss),
        List.flatten gen_nodes1 @ List.flatten gen_nodes2 )
  ...
  | (GhostConst _ | AssumptionVars _) as ci -> (ci, [])

let desugar_contract :
    Ctx.tc_context ->
    HString.t ->
    HString.t list ->
    A.contract option ->
    A.contract option * A.declaration list =
 fun ctx node_name fun_ids contract ->
  match contract with
  | Some (pos, contract_items) ->
      let items, gen_nodes =
        List.map (desugar_contract_item ctx node_name fun_ids) contract_items
        |> List.split
      in
      (Some (pos, items), List.flatten gen_nodes)
  | None -> (None, [])

let rec desugar_node_item :
    Ctx.tc_context ->
    HString.t ->
    HString.t list ->
    A.node_item ->
 ...
 fun ctx node_name fun_ids ni ->
  ...
  | IfBlock (pos, cond, nis1, nis2) ->
      ...
      ( A.IfBlock (pos, cond, nis1, nis2),
        List.flatten gen_nodes1 @ List.flatten gen_nodes2 @ gen_nodes3 )
 ...
 fun ctx decls ->
  let fun_ids =
    List.filter_map
      (fun decl ->
        match decl with
        | A.FuncDecl (_, (id, _, _, _, _, _, _, _, _)) -> Some id
        | _ -> None)
      decls
  in
  let decls =
    List.fold_left
      (fun decls decl ->
        ...
        | A.NodeDecl
            ...
            let ctx =
              Chk.add_full_node_ctx ctx id params inputs outputs locals
            in
            let items, gen_nodes =
              List.map (desugar_node_item ctx id fun_ids) items |> List.split
            in
            let contract, gen_nodes2 =
              desugar_contract ctx id fun_ids contract
            in
        ...
        | A.ContractNodeDecl (span, (id, params, inputs, outputs, contract)) ->
            ...
            let contract, gen_nodes =
              desugar_contract ctx id fun_ids (Some contract)
            in
            let contract =
              match contract with
