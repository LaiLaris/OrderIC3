...
let select_from_arrayintindex pos bound_e index expr =
  ...
  if
    bound_e <> None
    && E.is_numeral (get bound_e)
    && E.numeral_of_expr (get bound_e) |> Numeral.to_int > size
  then
    fail_at_position pos
      (Format.asprintf
         "Size of indexes on left of equation (%a) is larger than size on the \
          right (%d)"
         (E.pp_print_expr false) (get bound_e) size);

  let last, vals =
    match vals with (_, x) :: r -> (x, r) | _ -> assert false
  in
...
*)
let rec eval_ast_expr bounds ctx = function
  (* ****************************************************************** *)
  (* Identifier                                                         *)
  (* ****************************************************************** *)
  ...
  | A.ModeRef (pos, p4th) -> (
      ...
      let fail () =
        fail_at_position pos
          (Format.asprintf "reference to unknown mode '::%a'"
      ...
      let rec find_mode = function
        | { Contract.path; Contract.requires } :: tail ->
            if path = p4th then
              ...
              |> E.mk_and_n |> D.singleton D.empty_index
            else find_mode tail
        | [] -> fail ()
      in
  ...
  (* Integer constant [d] *)
  | A.Const (_, A.Num d) ->
      eval_nullary_expr ctx
        (E.mk_int (d |> HString.string_of_hstring |> Numeral.of_string))
  (* Real constant [f] *)
  | A.Const (_, A.Dec f) ->
      eval_nullary_expr ctx
        (E.mk_real (f |> HString.string_of_hstring |> Decimal.of_string))
  (* ****************************************************************** *)
  (* Unary operators                                                    *)
  (* ****************************************************************** *)

  (* Conversion to an integer number [int expr] *)
  | A.ConvOp (pos, A.ToInt, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_to_int expr
  (* Conversion to unsigned fixed-width integer numbers [uint8-uint64 expr] *)
  | A.ConvOp (pos, A.ToUInt8, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_to_uint8 expr
  | A.ConvOp (pos, A.ToUInt16, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_to_uint16 expr
  | A.ConvOp (pos, A.ToUInt32, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_to_uint32 expr
  | A.ConvOp (pos, A.ToUInt64, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_to_uint64 expr
  (* Conversion to signed fixed-width integer numbers [int8-int64 expr] *)
  | A.ConvOp (pos, A.ToInt8, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_to_int8 expr
  | A.ConvOp (pos, A.ToInt16, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_to_int16 expr
  | A.ConvOp (pos, A.ToInt32, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_to_int32 expr
  | A.ConvOp (pos, A.ToInt64, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_to_int64 expr
  (* Conversion to a real number [real expr] *)
  | A.ConvOp (pos, A.ToReal, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_to_real expr
  (* Boolean negation [not expr] *)
  | A.UnaryOp (pos, A.Not, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_not expr
  (* Unary minus [- expr] *)
  | A.UnaryOp (pos, A.Uminus, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_uminus expr
  (* Bitwise negation [! expr] *)
  | A.UnaryOp (pos, A.BVNot, expr) ->
      eval_unary_ast_expr bounds ctx pos E.mk_bvnot expr
  (* ****************************************************************** *)
  (* Binary operators                                                   *)
  (* ****************************************************************** *)
  ...
  | A.Arrow (pos, expr1, expr2) ->
      let mk e1 e2 =
        ...
        E.mk_arrow e1' e2'
      in
      eval_binary_ast_expr bounds ctx pos mk expr1 expr2
  (* ****************************************************************** *)
  (* Other operators                                                    *)
  (* ****************************************************************** *)
  ...
  | A.CompOp (pos, A.Eq, expr1, expr2) ->
      ...
      let expr, ctx = eval_binary_ast_expr bounds ctx pos E.mk_eq expr1 expr2 in

      (* We don't want to do extensional array equality, because that
       would need quantifiers already at this level *)
      D.iter
        (fun i _ ->
          if List.exists (function D.ArrayVarIndex _ -> true | _ -> false) i
          then
            (* Expression has array bounds *)
            fail_at_position pos "Extensional array equality not supported")
        expr;

      (* Conjunction of equations *)
      let res =
        D.singleton D.empty_index
          (List.fold_left E.mk_and E.t_true (D.values expr))
      in
  ...
  | A.TernaryOp (pos, A.Ite, expr1, expr2, expr3) ->
      ...
      let expr1', ctx = eval_bool_ast_expr bounds ctx pos expr1 in
        ...
        D.fold
          (fun index expr' (accum, ctx) ->
            ...
            let mk_local ctx expr' =
              let (((sv, _), _) as res) =
                C.mk_local_for_expr ~original ~bounds pos ctx expr'
              in
            ...
            in*)
              ...
              then
                N.add_state_var_def sv ~is_dep:true (N.GeneratedEq (pos, index));
              res
            in
            let expr', ctx =
              E.mk_pre_with_context mk_local mk_lhs_term ctx (C.guard_flag ctx)
                expr'
            in
  ...
  *)
  | A.Merge (pos, clock_ident, merge_cases) ->
      let merge_cases =
        List.map (fun (s, c) -> (HString.string_of_hstring s, c)) merge_cases
      in
      ...
      if
        List.sort String.compare cases <> List.sort String.compare cases_to_have
      then fail_at_position pos "Cases of merge must be exhaustive and unique";

      let cond_of_clock_value clock_value =
      ...
      let eval_merge_case ctx = function
        ...
        | clock_value, A.When (pos, expr, case_clock) ->
            ...
            | A.ClockConstr (cs, c)
              when clock_value = HString.string_of_hstring cs && c = clock_ident
              ->
                ()
        ...
        | clock_value, A.Activate (pos, ident, case_clock, restart_clock, args)
          ->
            ...
            try_eval_node_call bounds ctx pos
              (I.mk_string_ident (HString.string_of_hstring ident))
              case_clock restart_clock args None
        ...
        | _, A.Call (p, _ :: _, _, _) ->
            fail_at_position p
              "Node calls with type arguments not supported in old front-end."
      ...
      let merge_cases_r, ctx =
        List.fold_left
          (fun (acc, ctx) ((case_value, _) as case) ->
      ...
      let res =
        ...
        with
        | Invalid_argument _ ->
            fail_at_position pos "Index mismatch for expressions in merge"
        | E.Type_mismatch ->
            fail_at_position pos "Type mismatch for expressions in merge"
      in
  ...
  | A.RecordProject (pos, expr, field) ->
      eval_ast_projection bounds ctx pos expr
  ...
  | A.TupleProject (pos, expr, field) ->
      eval_ast_projection bounds ctx pos expr (D.TupleIndex field)
          ...
          (fun (i, accum, ctx) expr ->
            ...
            ( succ i,
              (* Push current index to indexes in expression and add
               to accumulator trie *)
              D.fold (fun j e a -> D.add (D.ListIndex i :: j) e a) expr' accum,
        ...
        List.fold_left
          (fun (i, accum, ctx) expr ->
            ...
            ( succ i,
              (* Push current index to indexes in expression and add
               to accumulator trie *)
              D.fold (fun j e a -> D.add (D.TupleIndex i :: j) e a) expr' accum,
              (* Continue with added definitions *)
              ctx ))
          (* Start counting index at zero, with given abstractions and
           add to the empty trie *)
  ...
  | A.RecordExpr (pos, record_type, _, expr_list) ->
      let record_ident =
        I.mk_string_ident (HString.string_of_hstring record_type)
      in
        ...
        List.fold_left
          (fun (accum, ctx) (i, expr) ->
            ...
            (* Push field index to indexes in expression and add to
               accumulator trie *)
            ( D.fold (fun j e t -> D.add (D.RecordIndex i :: j) e t) expr' accum,
              ctx ))
          (D.empty, ctx) expr_list
      in
      ...
      if List.for_all2 D.equal_index (D.keys record_indexes) (D.keys res) then
        (res, ctx)
      else fail_at_position pos "Type mismatch in record"
  ...
  | A.StructUpdate (_, expr1, index, expr2) ->
      ...
      let rec aux accum = function
        ...
        | A.Label (pos, index) :: tl ->
            (* Add to accumulator *)
            let accum' =
              D.RecordIndex (HString.string_of_hstring index) :: accum
            in
            ...
            if D.mem_prefix (List.rev accum') expr1' then
              ...
              fail_at_position pos
                (Format.asprintf "Invalid index %s for expression"
                   (HString.string_of_hstring index))
        ...
        | A.Index (pos, index_expr) :: tl -> (
            ...
            | D.ArrayVarIndex _ :: _, _ ->
                ...
                let i =
                  ...
                  else D.ArrayVarIndex index_expr'
                in
            ...
            | D.ArrayIntIndex _ :: _, _ ->
                ...
                if Term.is_numeral index_term then
                  ...
                  aux
                    ...
                    :: accum)
                    tl
                else fail_at_position pos "Invalid index for expression"
            ...
            | [], _ ->
                fail_at_position pos "Invalid index for expression")
      in
      ...
      let expr2'' =
        D.fold (fun i v a -> D.add (index' @ i) v a) expr2' D.empty
      in
      ...
      let rec mk_cond_indexes (acc, cpt) li ri =
        ...
        | D.ArrayVarIndex _ :: li', D.ArrayVarIndex vi :: ri' ->
            let acc = E.mk_eq (E.mk_index_var cpt) (E.mk_of_expr vi) :: acc in
            mk_cond_indexes (acc, cpt + 1) li' ri'
            ...
            with Not_found -> (
              try
                ...
                if Flags.Arrays.smt () then
                  ...
                  let v' =
                    E.mk_ite (mk_cond_indexes ([], 0) i index') new_v old_v
                  in

                  (* We've added the index variables so we can forget this
                  one *)
                  D.add [] v' a
              with Not_found -> D.add i v a))
          expr1' D.empty
      in
  ...
  | A.Condact (pos, cond, restart, ident, args, defaults) ->
      try_eval_node_call bounds ctx pos
        (I.mk_string_ident (HString.string_of_hstring ident))
        cond restart args (Some defaults)
          ...
          (fun (i, accum, ctx) expr ->
            ...
            ( succ i,
              (* Push current index to indexes in expression and add
               to accumulator trie *)
              D.fold
                (fun j e a -> D.add (j @ [ D.ArrayIntIndex i ]) e a)
                expr' accum,
              (* Continue with added definitions *)
              ctx ))
          (* Start counting index at zero, with given abstractions and
  ...
  | A.ArrayConstr (pos, expr, size_expr) ->
      ...
      if
        (not (C.are_definitions_allowed ctx))
        && Term.is_numeral (E.unsafe_term_of_expr array_size)
      then
        let l_expr =
          array_size |> E.unsafe_term_of_expr |> Term.numeral_of_term
          |> Numeral.to_int
          |> Lib.list_init (fun _ -> expr)
        in
      ...
      let rec push expr' =
        ...
        (*                       is larger than size on the right (%a)" *)
            ...
            let expr' =
              D.fold
                (function
                  | D.ArrayVarIndex _ :: tl -> D.add tl | _ -> assert false)
                expr' D.empty
            in
  ...
  | A.Activate (pos, _, _, _, _) ->
      fail_at_position pos "Activate operator only supported in merge"
  | A.AnyOp (pos, _, _, _) ->
...
and var_of_quant (ctx, vars) (_, v, ast_type) =
  ...
  let vars, d =
    D.fold
      ...
      index_types (vars, D.empty)
  in

  (* Bind identifier to the index variable, shadow previous
     bindings *)
  let ctx = C.add_expr_for_ident ~shadow:true ctx (I.mk_string_ident v) d in
...
and const_int_of_ast_expr ctx pos expr =
  match
    ...
    let expr', _ =
      eval_ast_expr []
        (C.fail_on_new_definition ctx pos "Expression must be constant")
        expr
    in
  ...
  | [ (index, { E.expr_init = ei; E.expr_step = es }) ]
    when index = D.empty_index
      ...
      if Term.is_numeral ti then Term.numeral_of_term ti
      else fail_at_position pos "Expression must be an integer"
  (* Expression is not a constant integer *)
  | _ -> fail_at_position pos "Expression must be constant"
...
and eval_bool_ast_expr bounds ctx pos expr =
  ...
  | [ (index, ({ E.expr_type = t } as expr)) ]
    when index = D.empty_index && Type.equal_types t Type.t_bool ->
      (expr, ctx)
  (* Expression is not Boolean or is indexed *)
  | _ -> fail_at_position pos "Expression is not of Boolean type"
...
and eval_clock_ident ctx pos ident =
  ...
  (* Boolean expression without indexes *)
  | [ (index, ({ E.expr_type = t } as expr)) ]
    when index = D.empty_index && (Type.is_bool t || Type.is_enum t) ->
      (expr, ctx)
  (* Expression is not Boolean or is indexed *)
  | _ ->
      fail_at_position pos
        "Clock identifier is not Boolean or of an enumerated datatype"
...
and eval_unary_ast_expr bounds ctx pos mk expr =
  try
    ...
    (D.map mk expr', ctx)
  with E.Type_mismatch -> fail_at_position pos "Type mismatch for expression"
...
and eval_binary_ast_expr bounds ctx pos mk expr1 expr2 =
  ...
  let res =
    ...
    | Invalid_argument _ ->
        fail_at_position pos
          (Format.asprintf "Index mismatch for expressions %a and %a"
             A.pp_print_expr expr1 A.pp_print_expr expr2)
    | E.Type_mismatch ->
        fail_at_position pos
          (Format.asprintf "Type mismatch for expressions %a and %a"
             A.pp_print_expr expr1 A.pp_print_expr expr2)
  in
...
and eval_ast_projection bounds ctx pos expr = function
  ...
  | (D.RecordIndex _ | D.TupleIndex _ | D.ArrayIntIndex _) as index -> (
      ...
      with Not_found ->
        fail_at_position pos
          (Format.asprintf "Expression %a does not have index %a"
             A.pp_print_expr expr
             (D.pp_print_one_index false)
             index))
  (* Cannot project array field this way, need to use select
     operator *)
  | D.ListIndex _ | D.ArrayVarIndex _ | D.AbstractTypeIndex _ ->
      raise (Invalid_argument "eval_ast_projection")
...
and try_eval_node_call bounds ctx pos ident cond restart args defaults =
  ...
  | Some node ->
      ...
      let rec unroll_array_bounds sigma bounds' index_var ctx =
        match bounds' with
        | [] ->
            eval_node_call sigma bounds ctx pos ident node cond restart args
              defaults
        (* Do the expansion for arrays of known size *)
        | E.Bound b :: bounds' when Term.is_numeral (E.unsafe_term_of_expr b) ->
            let array_size =
              E.unsafe_term_of_expr b |> Term.numeral_of_term |> Numeral.to_int
            in
            ...
            in
            unroll_indices 0 ctx
        | _ :: bounds' -> unroll_array_bounds sigma bounds' (succ index_var) ctx
      in
      unroll_array_bounds [] bounds 0 ctx
...
and eval_node_call sigma bounds ctx pos ident
    {
  ...
  let node_inputs_of_exprs bounds ctx node_inputs pos ast =
    try
      ...
      let expr', ctx =
        ...
        let expr', ctx =
          eval_ast_expr bounds ctx (A.GroupExpr (dummy_pos, A.ExprList, ast))
        in
        ...
        then
          ...
          ( D.fold
              (function
                | D.ListIndex 0 :: tl -> D.add tl | _ -> raise E.Type_mismatch)
              expr' D.empty,
            ctx )
      in
      ...
      D.fold2
        (fun i state_var ({ E.expr_type } as expr) (accum, ctx) ->
          if E.has_indexes expr then
            fail_at_position pos
              "Not supported: node or function call for dynamically sized \
               array.";
          (* Expression must be of a subtype of input type *)
          if
            Type.check_type expr_type (StateVar.type_of_state_var state_var)
            &&
          ...
          then (
            ...
            let (state_var', _), ctx =
              C.mk_local_for_expr
                ...
                ~bounds:bounds_state_var pos ctx expr
            in
            ...
            let pos', i' =
              try
                match i with
                | ListIndex i :: indexes ->
                    (H.pos_of_expr (List.nth ast i), indexes)
            ...
            if
              ...
              (*is_a_def*)
              not (StateVar.is_input state_var')
            then
              N.add_state_var_def ~is_dep:true state_var'
                (N.GeneratedEq (pos', i'));
            let ctx =
              C.current_node_map ctx (fun node ->
  ...
  let node_act_cond_of_expr ctx node_outputs pos cond defaults =
    ...
    if E.equal cond' E.t_true then
      ...
      let defaults', ctx =
        ...
        | Some d ->
            let d', ctx =
              eval_ast_expr bounds ctx (A.GroupExpr (dummy_pos, A.ExprList, d))
            in
            (Some d', ctx)
        (* No defaults, skip *)
        | None -> (None, ctx)
  ...
  match
    ...
    if node_oracles <> [] then None
      ...
      C.call_outputs_of_node_call ctx ident cond_state_var input_state_vars
        defaults
  with
  ...
  | None ->
      ...
      let ctx, oracle_state_vars =
        node_oracles |> List.rev (* Preserve order of oracles. *)
        |> List.fold_left
             (fun (ctx, accum) sv ->
               let bounds =
                 try
                   StateVar.StateVarHashtbl.find (C.get_state_var_bounds ctx) sv
                 with Not_found -> []
               in
               let sv', ctx =
                 C.mk_fresh_oracle ~is_input:true
                   ~is_const:(StateVar.is_const sv) ~bounds ctx
      ...
      let output_state_vars, ctx =
        D.fold
          (fun i sv (accum, ctx) ->
            ...
            let ctx =
              C.current_node_map ctx (fun node ->
                  N.set_state_var_node_call node sv')
            in
      ...
      let node_call =
        {
          N.call_id = -1;
          (* Call_id is not implemented in old frontend *)
          N.call_pos = pos;
  ...
  let _, e =
    List.fold_left
      (fun (i, t) -> function
        | E.Bound _ ->
            ( succ i,
              Term.mk_select t (Term.mk_var @@ E.var_of_expr @@ E.mk_index_var i)
            )
        | E.Unbound v -> (i, Term.mk_select t (E.unsafe_term_of_expr v))
        | _ -> assert false)
      (0, e) bounds
  in
  e
...
and eval_ast_type_flatten flatten_arrays ctx = function
  ...
  | A.IntRange (pos, Some lbound, Some ubound) as t ->
      ...
      D.singleton D.empty_index
        (Type.mk_int_range (Some const_lbound) (Some const_ubound))
  ...
  | A.IntRange (_, None, None) ->
      D.singleton D.empty_index (Type.mk_int_range None None)
  ...
  | A.UserType (pos, _ :: _, _) ->
      fail_at_position pos
        "UserTypes with type arguments not supported in old frontend"
  ...
  | A.AbstractType (_, ident) ->
      D.singleton
        [ D.AbstractTypeIndex (HString.string_of_hstring ident) ]
        Type.t_int
  ...
  | A.RecordType (_, _, record_fields) ->
      ...
      List.fold_left
        ...
        (fun a (_, i, t) ->
          ...
          D.fold (fun j t a -> D.add (D.RecordIndex i :: j) t a) expr a)
        (* Start with empty trie *)
        D.empty record_fields
      ...
      List.fold_left
        ...
        (fun (i, a) t ->
          ...
          (* Take all indexes and their defined types and add index of tuple
            field to index of type and add to trie *)
          let d =
            D.fold (fun j t a -> D.add (D.TupleIndex i :: j) t a) expr a
          in
  ...
  | A.ArrayType (pos, (type_expr, size_expr)) ->
      ...
      if flatten_arrays then (
        ...
        for ix = 0 to Numeral.to_int upper - 1 do
          result :=
            D.fold
              (fun j t a -> D.add (j @ [ D.ArrayIntIndex ix ]) t a)
              element_type !result
        done;
        !result)
      else
        ...
        D.fold
          (fun j t a ->
            D.add
              (j @ [ D.ArrayVarIndex array_size ])
              (Type.mk_array t
                 (if E.is_numeral array_size then
                    Type.mk_int_range (Some Numeral.zero)
                      (Some (E.numeral_of_expr array_size))
                  else Type.t_int))
              a)
          element_type D.empty
  | A.History (_, ident) ->
  ...
  | RefinementType (pos, _, _) ->
      fail_at_position pos "Refinement types not supported in old front-end"
