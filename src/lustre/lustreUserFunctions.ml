...
let valid_locals ctx locals =
  ...
  |> List.for_all (function
       ...
       | NodeVarDecl (_, (_, _, ty, _)) -> not (Ctx.type_contains_array ctx ty))
...
let valid_items set items =
  ...
  |> List.for_all (function
       ...
       | A.Body (Equation (_, _, _)) | Body (Assert _) | AnnotMain _ -> false
       | FrameBlock _ | IfBlock _ ->
           assert false (* desugared earlier in pipeline *))

let is_output_defined outputs items =
  let output_id =
    match outputs with [ (_, id, _, _) ] -> id | _ -> assert false
  in
  items
...
let have_ref_type_or_subrange ctx outputs =
  outputs
  |> List.exists (fun (_, _, ty, _) -> Ctx.type_contains_ref_or_subrange ctx ty)
...
let is_inlinable set contracts ctx opac contract outputs locals items =
  (opac = A.Transparent
  || not (can_be_abstracted ctx contracts outputs contract))
  && valid_outputs ctx outputs && valid_locals ctx locals
  && valid_items set items
  && is_output_defined outputs items

let inlinable_functions ctx decls =
  List.fold_left
    (fun (set, contracts) dcl ->
      match dcl with
      ...
      | A.FuncDecl
          (_, (id, false, opac, [], _, outputs, locals, items, contract)) ->
          if is_inlinable set contracts ctx opac contract outputs locals items
          then (IdSet.add id set, contracts)
          else (set, contracts)
      | _ -> (set, contracts))
