...
let[@ocaml.warning "-27"] contract_check_no_output ctx pos expr =
  ...
  let outputs = D.fold (fun _ elm set -> SVS.add elm set) outputs SVS.empty in
...
let eval_const_decl ?(ghost = false) ctx = function
  ...
  | A.FreeConst (_, i, ty) ->
      ...
      let vt, ctx =
        D.fold
          (fun i ty (vt, ctx) ->
            let state_var, ctx =
              C.mk_state_var ?is_input:(Some false) ?is_const:(Some true)
                ?for_inv_gen:(Some true) ~shadow:ghost ctx
  ...
  | (A.UntypedConst (pos, i, expr) | A.TypedConst (pos, i, expr, _)) as
    const_decl ->
      (* Identifier of AST identifier *)
      let ident = I.mk_string_ident (HString.string_of_hstring i) in
      ...
      then
        ...
        fail_at_position pos
          (Format.asprintf "Identifier %a is redeclared as constant"
             (I.pp_print_ident false) ident);

      (* Evaluate constant expression *)
      let res, _ =
        S.eval_ast_expr []
          (C.fail_on_new_definition ctx pos "Invalid constant expression")
          expr
      in
      ...
      | A.TypedConst (_, _, _, type_expr) -> (
          ...
          with Invalid_argument _ | E.Type_mismatch ->
            fail_at_position pos "Type mismatch in constant declaration")
      ...
      D.iter
        (fun _ e ->
          if not (E.is_const e) then
            fail_at_position pos
              (Format.asprintf "Invalid constant expression for %a"
                 (I.pp_print_ident false) ident))
        res;
...
let rec eval_node_inputs ctx = function
  ...
  | (pos, i, ast_type, A.ClockTrue, is_const) :: tl ->
      ...
      if
        try C.expr_in_context ctx ident
        with Invalid_argument e -> fail_at_position pos e
      then
        fail_at_position pos
          (Format.asprintf "Node input %a already declared"
             (I.pp_print_ident false) ident);
      ...
      let ctx = C.add_node_input ~is_const ctx ident pos index_types in
...
let rec eval_node_outputs ?is_single ctx = function
  ...
  | (pos, i, ast_type, A.ClockTrue) :: tl ->
      ...
      then
        fail_at_position pos
          (Format.asprintf "Node output %a already declared"
             (I.pp_print_ident false) ident);
...
let rec eval_node_locals ?(ghost = false) ctx = function
  ...
  | A.NodeConstDecl (pos, A.TypedConst (_, i, _, _)) :: _
    when (* Ghost variables shadow already declared variables *)
         (not ghost)
         &&
         (* Identifier of AST identifier *)
         let ident = I.mk_string_ident (HString.string_of_hstring i) in
         ...
         with Invalid_argument e -> fail_at_position pos e ->
      fail_at_position pos
        (Format.asprintf "Node local variable or constant %a already declared"
           A.pp_print_ident i)
  ...
  | A.NodeVarDecl (_, (pos, i, _, _)) :: _ ->
      fail_at_position pos
        (Format.asprintf "Clocked node local variables not supported for %a"
           A.pp_print_ident i)
  (* Local constant *)
...
let[@ocaml.warning "-27"] eval_struct_item ctx pos = function
  ...
  | A.ArrayDef (pos, i, l) ->
      ...
      let res =
        ...
        (* Identifier cannot be assigned to *)
        | Invalid_argument _ ->
            fail_at_position pos "Assignment to identifier not possible"
        (* Identifier not declared *)
        | Not_found ->
            fail_at_position pos "Assignment to undeclared identifier"
      in
        ...
        List.fold_left
          (fun (i, ctx) v ->
            ...
            (* Bind identifier to the index variable, shadow previous
              bindings *)
            let ctx =
              C.add_expr_for_ident ~shadow:true ctx (I.mk_string_ident v)
      ...
      let ctx =
        List.fold_left
          (fun ctx v ->
            (* Bind identifier to the index variable, shadow previous
              bindings *)
            let ctx = C.remove_expr_for_ident ctx (I.mk_string_ident v) in
            ctx)
          ctx
        ...
        List.fold_left
          (fun (ctx, i, accum) e ->
            ...
            ( ctx,
              ...
              D.fold (fun j e a -> D.add (D.ListIndex i :: j) e a) t accum ))
...
let rec eval_node_equation _inputs _outputs _locals ctx = function
  ...
  | A.Equation (pos, lhs, ast_expr) ->
      ...
      let rm_array_var_index lst =
        List.filter (function D.ArrayVarIndex _ -> false | _ -> true) lst
      in
      let lhs_bounds =
        List.fold_left
          (fun acc (i, sv) ->
...
and eval_ghost_var ?(no_defs = false) is_postponed inputs outputs locals ctx =
  function
  ...
  | (A.UntypedConst (pos, i, expr) | A.TypedConst (pos, i, expr, _)) as
    const_decl -> (
      ...
      then
        (* Fail if identifier already declared *)
        fail_at_position pos
          (Format.asprintf "Identifier %a is redeclared as ghost"
             (I.pp_print_ident false) ident);

      if H.has_unguarded_pre expr then
        fail_or_warn pos "Illegal unguarded pre in ghost variable definition.";
      ...
      | A.TypedConst (_, _, _, type_expr) -> (
          try
            ...
            let ctx =
              eval_node_equation inputs outputs locals ctx
                (A.Equation
                   (pos, A.StructDef (pos, [ A.SingleIdent (pos, i) ]), expr))
            in
          ...
          | E.Type_mismatch ->
              fail_at_position pos
                (Format.sprintf
                   "Type mismatch in declaration of ghost variable %s"
                   (HString.string_of_hstring i))
      ...
      | _ ->
          ...
          let expr, ctx' =
            S.eval_ast_expr []
              (if
                 ...
                 no_defs
               then
                 C.fail_on_new_definition ctx pos
          ...
          (* Add ghost to context. *)
          let ctx = C.add_node_local ~ghost:true ctx ident pos type_expr in
          let ctx = C.add_expr_for_ident ~shadow:true ctx ident expr in
...
and eval_contract_item check ~typ scope (ctx, accum, count) (pos, iname, expr) =
      ...
      | svars ->
        ...
        *)
          warn_at_position pos
            (Format.asprintf "@[<v>%s mentions output%s%s %a%s@]" desc s pref
               (pp_print_list
                  (fun fmt sv ->
  ...
  let (svar, _), ctx =
    C.mk_local_for_expr ~reuse:false ~is_ghost:true pos ctx expr
  in
...
and eval_node_mode scope ctx is_candidate (pos, id, reqs, enss) =
  ...
  let ctx, enss, _ =
    enss
    |> List.fold_left (eval_contract_item None ~typ:N.Ensure scope) (ctx, [], 1)
  in
  let path = scope |> List.fold_left (fun l (_, name) -> name :: l) [ id ] in
  (* Done. *)
...
and eval_node_contract_call known ctx scope inputs outputs locals is_candidate
    (call_pos, id, tys, in_params, out_params) =
  if tys <> [] then
    fail_at_position call_pos
      "Contract calls with type parameters not supported in old frontend"
  else
    let id' = HString.string_of_hstring id in
    ...
    |> List.iter (fun expr ->
           if H.has_unguarded_pre expr then
             fail_or_warn call_pos
               "Illegal unguarded pre in input parameters of contract call.");
    ...
    | [] -> ()
    | _ ->
        fail_at_position pos "type parameters in contract node is not supported");
    in_formals
    |> List.iter (function
         | _ (* pos *), _ (* id *), _ (* typ *), A.ClockTrue, _ (* is_const *)
           ->
             () (* pos, id, typ, is_const *)
         | _ ->
             fail_at_position pos
      ...
      try
        List.fold_left2
          (fun ctx expr (_, in_id, typ, _, _) ->
              ...
              (fun _ expr ->
                ...
                | svars ->
                    ...
                    let pref =
                      ...
                      | None -> ""
                      | Some name ->
                          Format.asprintf " in node %a" (I.pp_print_ident false)
                            name
                    in
                    let suff =
                      match scope with
            ...
            C.add_expr_for_ident ~shadow:true ctx
              (LustreIdent.mk_string_ident in_id)
              expr)
          ctx in_params in_formals
      with Invalid_argument _ ->
        fail_at_position call_pos
          (Format.asprintf
             "arity mismatch for the input parameters of import of contract \
              %s: expected %d but got %d"
             (HString.string_of_hstring id)
             (List.length in_formals) (List.length in_params))
    in
          ...
          (fun ctx expr (_, in_id, typ, _) ->
            ...
            (try
             ...
             with Invalid_argument _ | E.Type_mismatch ->
               fail_at_position call_pos
                 (Format.asprintf
                    "type mismatch in import of contract %s for formal output \
                     %s"
                    (HString.string_of_hstring id)
                    in_id));
            ...
            C.add_expr_for_ident ~shadow:true ctx
              ...
              expr)
          ctx
          (List.map (fun i -> LustreAst.Ident (pos, i)) out_params)
          out_formals
      with Invalid_argument _ ->
        fail_at_position call_pos
          (Format.asprintf
             "arity mismatch for the output parameters of import of contract \
              %s: expected %d but got %d"
             (HString.string_of_hstring id)
             (List.length in_formals) (List.length in_params))
    in
    ...
    (if C.get_node_function_flag ctx then
       ...
       | Some _ ->
           fail_at_position call_pos
             (Format.asprintf
                "@[<v>in contract of function %a@ illegal import of stateful \
                 contract %s@ functions can only be specified by stateless \
                 contracts@]"
                (I.pp_print_ident false)
                (match C.current_node_name ctx with
                | Some id -> id
    ...
    let ctx =
      contract |> snd
      |> List.map (fun item -> (item, is_candidate))
      |> eval_node_contract_spec known ctx call_pos svar_scope inputs outputs
           locals
    in
...
and eval_node_contract_item known scope inputs outputs locals is_candidate
    is_postponed (ctx, cpt_a, cpt_g) = function
  ...
  | A.GhostVars (pos, _, _) ->
      fail_at_position pos
        "Multiple ghost variable declaration is not supported in old frontend"
  (* Evaluate assumption *)
  | A.Assume (pos, name, soft, expr) ->
      let ctx, assumes, cpt_a =
        eval_contract_item
          (Some (if soft then "weakly assume" else "assume"))
          ~typ:(if soft then N.WeakAssumption else N.Assumption)
          scope (ctx, [], cpt_a) (pos, name, expr)
      in
      (C.add_node_ass ctx assumes, cpt_a, cpt_g)
  (* Evaluate guarantee *)
  | A.Guarantee (pos, name, soft, expr) ->
      let ctx, guarantees, cpt_g =
        eval_contract_item None
          ~typ:(if soft then N.WeakGuarantee else N.Guarantee)
          scope (ctx, [], cpt_g) (pos, name, expr)
      in
  ...
  | A.ContractCall call ->
      ...
      ( eval_node_contract_call known ctx scope inputs outputs locals
          is_candidate call,
        cpt_a,
        cpt_g )
...
and eval_node_contract_spec known ctx pos scope inputs outputs locals contract =
    ...
    | [] -> (
        ...
        | (head, is_candidate, is_postponed) :: _
          when prev_postponed_size = List.length postponed -> (
            try
              (* eval_node_contract_item is expected to fail with Deps.Unknown_decl *)
              eval_node_contract_item known scope inputs outputs locals
                is_candidate is_postponed acc head
            with Deps.Unknown_decl (s_type, s_ident, s_pos) ->
            ...
            identifier. *)
              ...
              let msg =
                if sc = [] then
                  Format.asprintf "unknown %a '%a'" pp_print_type s_type
                    (I.pp_print_ident false) s_ident
                else
                  Format.asprintf "unknown %a '%a' referenced in contract '%a'"
  ...
  | Some { N.oracles; N.calls } ->
      ...
      | _ ->
          (* PEBCAK. *)
          fail_or_warn pos
            "Illegal unguarded pre under a node call in this contract.");
      ...
      let node_of_name ctx name =
        try C.node_of_name ctx name
        with Not_found ->
          Format.asprintf "unreachable, node %a called in contract undefined"
            (I.pp_print_ident false) name
          |> failwith
      in
      let rec loop known = function
        | [] -> ()
        | ({ N.name; N.calls; N.is_extern } as node) :: tail
          when is_extern || not (N.has_effective_contract node) ->
...
and eval_node_items inputs outputs locals ctx = function
  ...
  | A.IfBlock (pos, _, _, _) :: _ ->
      fail_at_position pos "If blocks not supported in old front-end"
  | A.FrameBlock (pos, _, _, _) :: _ ->
      fail_at_position pos "Frame blocks not supported in old front-end"
  (* Property annotation, reachability query *)
  | A.AnnotProperty (pos, _, _, Reachable _) :: _ ->
      fail_at_position pos
        "Reachability queries are not supported in the old frontend."
  (* Property annotation *)
      ...
      let name =
        ...
        | Some n ->
            if C.prop_name_in_context ctx (HString.string_of_hstring n) then
              fail_at_position pos
                (Format.asprintf "Name '%s' already used by another property"
                   (HString.string_of_hstring n))
            else n
        | None ->
            HString.mk_hstring
              (Format.asprintf "@[<h>%a@]" A.pp_print_expr ast_expr)
      in

      (* Add property to node *)
      let ctx =
        C.add_node_property ctx (Property.PropAnnot pos)
  ...
  | A.AnnotMain (_, true) :: tl ->
      eval_node_items inputs outputs locals (C.set_node_main ctx) tl
  (* Annotation for main node *)
  | A.AnnotMain (_, false) :: tl -> eval_node_items inputs outputs locals ctx tl

(* Add declarations of node to context *)
and eval_node_decl ctx pos inputs outputs locals items contract_spec =
  (* Add inputs to context: as state variable to ident_expr_map, and
     to inputs *)
  let inputs' =
    List.map
      (fun (a, b, c, d, e) -> (a, HString.string_of_hstring b, c, d, e))
      inputs
  in
  ...
  let contract_spec =
    ...
    | None -> None
    | Some (_, spec) -> Some (spec |> List.map (fun item -> (item, false)))
  in
  ...
  let ctx =
    ...
    | Some contract ->
        ...
        (* Eval contracts. *)
        let ctx =
          eval_node_contract_spec I.Set.empty ctx pos [] inputs outputs locals
            contract
        in
  ...
  let ctx = eval_node_items inputs outputs locals ctx items in
  ...
  let assume_svars, guarantee_svars, prop_svars =
    ...
    | Some ({ N.is_extern; N.state_var_source_map } as node) ->
        let assume_svars, others =
          ...
          |> StateVar.StateVarMap.partition (fun _ src ->
                 match src with N.Input -> true | _ -> false)
        in
        let prop_svars, guarantee_svars =
          if is_extern then (StateVar.StateVarMap.empty, others)
          else
        ...
        List.fold_left
          (fun acc (ty, iv) ->
            ...
            | Type.IntRange (Some i, None) ->
                E.mk_forall [ iv ]
                  (E.mk_impl (E.mk_lte (E.mk_int i) (E.mk_free_var iv)) acc)
            | Type.IntRange (None, Some j) ->
                E.mk_forall [ iv ]
                  (E.mk_impl (E.mk_lt (E.mk_free_var iv) (E.mk_int j)) acc)
            | Type.IntRange (None, None) -> E.t_true
            | _ -> E.mk_forall [ iv ] acc)
          ct indices
      in
      qct
    else
      ...
      List.fold_left
        (fun ctx svar ->
          ...
          let source =
            ...
            | None -> src
          in
          C.add_node_property ctx source
    ...
    if guarantee_svars = [] then ctx
      ...
      let ctx, guarantees, _ =
        List.fold_left
          ...
          (ctx, [], cpt_g) guarantee_svars
      in
      List.map (fun g -> (g, false)) guarantees |> C.add_node_gua ctx
  in
...
and declaration_to_context ctx = function
  ...
  | A.TypeDecl ({ A.start_pos = pos }, type_rhs) ->
      let i, type_expr =
        ...
        (* Replace type aliases with their right-hand-side *)
        | A.AliasType (pos, _, _, _) ->
            fail_at_position pos
              "AliasTypes with type parameters not supported in old frontend"
      ...
      if C.type_in_context ctx ident then
        fail_at_position pos
          (Format.asprintf "Type %a is redeclared" (I.pp_print_ident false)
             ident);
  ...
  | A.FuncDecl
      ( { A.start_pos = pos },
        (i, ext, _, [], inputs, outputs, locals, items, contracts) ) ->
      (* Identifier of AST identifier *)
      let ident = I.mk_string_ident (HString.string_of_hstring i) in
      ...
      if C.node_in_context ctx ident then
        fail_at_position pos
          (Format.asprintf "Function %a is redeclared" (I.pp_print_ident false)
             ident);
      ...
      let pre_or_arrow_fail desc = function
        | Some illegal_pos ->
            fail_at_position pos
              (Format.asprintf
                 "@[<v>in declaration of function %a:@ in %s at %a:@ illegal \
                  pre or arrow in function declaration@ functions cannot have \
                  any state@]"
                 (I.pp_print_ident false) ident desc pp_print_position
                 illegal_pos)
        | None -> ()
      in
      ...
      (locals
       ...
       items
       |> List.iter (fun item ->
              H.node_item_has_pre_or_arrow item |> pre_or_arrow_fail "item");
      ...
      let fun_ctx =
        eval_node_decl fun_ctx pos inputs outputs locals items contracts
      in

      (* Check that all there's no (non-function) node call. *)
      C.current_node_calls fun_ctx
      |> List.iter (fun { N.call_pos; N.call_node_name } ->
             let node = C.node_of_name fun_ctx call_node_name in
             if not node.N.is_function then
               fail_at_position call_pos
                 (Format.asprintf
                    "@[<v>in function %a@ illegal call to node %a@ functions \
                     can only call other functions, not nodes@]"
                    (I.pp_print_ident false) ident (I.pp_print_ident false)
                    call_node_name));
  ...
  | A.NodeDecl
      ...
      if C.node_in_context ctx ident then
        fail_at_position pos
          (Format.asprintf "Node %a is redeclared" (I.pp_print_ident false)
             ident);
      ...
      let node_ctx =
        eval_node_decl node_ctx pos inputs outputs locals items contracts
      in
...
let rec declarations_to_context ctx = function
  ...
  | decl :: tail ->
      ...
      let ctx, tail =
        ...
        try
          (declaration_to_context ctx decl, tail)
          (* Otherwise, something unknown was found. Let's check if this something is
    forward referenced. *)
        with Deps.Unknown_decl (typ, ident, pos) ->
