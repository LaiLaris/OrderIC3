...
let string_of_tc_type : tc_type -> string =
 fun t -> Lib.string_of_t LA.pp_print_lustre_type t
...
let error_message kind =
  match kind with
  | Unknown s -> s
  | Impossible s -> "This should be impossible! " ^ s
  | MergeCaseExtraneous (case, ty) ->
      "Merge case "
      ^ HString.string_of_hstring case
      ^ " does not exist in type " ^ string_of_tc_type ty
  | MergeCaseMissing case ->
      "Merge case "
      ^ HString.string_of_hstring case
      ^ " is missing from merge expression"
  | MergeCaseNotUnique case ->
      "Merge case " ^ HString.string_of_hstring case ^ " must be unique"
  | UnboundIdentifier id ->
      "Unbound identifier '" ^ HString.string_of_hstring id ^ "'"
  | UnboundModeReference id ->
      "Unbound mode reference '" ^ HString.string_of_hstring id ^ "'"
  | UnboundNodeName id ->
      "Unbound node identifier '" ^ HString.string_of_hstring id ^ "'"
  | NotAFieldOfRecord id ->
      "No field name '" ^ HString.string_of_hstring id ^ "' in record type"
  | AssumptionOnCurrentOutput id ->
      "Refinement type references an illegal variable '"
      ^ HString.string_of_hstring id
      ^ "' (either an assumption on the current value of an output, or an \
         out-of-scope type argument)"
  | NoValueForRecordField id ->
      "No value given for field '" ^ HString.string_of_hstring id ^ "'"
  | IlltypedRecordProjection ty ->
      "Cannot project field out of non record expression type "
      ^ string_of_tc_type ty
  | TupleIndexOutOfBounds (id, ty) ->
      "Index " ^ string_of_int id ^ " is out of bounds for tuple type "
      ^ string_of_tc_type ty
  | IlltypedTupleProjection ty ->
      "Cannot project field out of non tuple type " ^ string_of_tc_type ty
  | UnequalIteBranchTypes (ty1, ty2) ->
      "Expected equal types of each if-then-else branch but found: "
      ^ string_of_tc_type ty1 ^ " on the then-branch and "
      ^ string_of_tc_type ty2 ^ " on the the else-branch"
  | ExpectedBooleanExpression ty ->
      "Expected a boolean expression but found expression of type "
      ^ string_of_tc_type ty
  | ExpectedIntegerExpression ty ->
      "Expected an integer expression but found expression of type "
      ^ string_of_tc_type ty
  | Unsupported s -> "Unsupported: " ^ s
  | UnequalArrayExpressionType ->
      "All expressions must be of the same type in an Array"
  | TypeMismatchOfRecordLabel (label, ty1, ty2) ->
      "Type mismatch. Type of record label '"
      ^ HString.string_of_hstring label
      ^ "' is of type " ^ string_of_tc_type ty1
      ^ " but the type of the expression is " ^ string_of_tc_type ty2
  | IlltypedUpdateWithLabel ty ->
      "Expected a record type but found " ^ string_of_tc_type ty
  | IlltypedUpdateWithIndex ty ->
      "Expected a tuple type or an array type but found " ^ string_of_tc_type ty
  | ExpectedLabel e ->
      "Only labels can be used for record expressions but found "
      ^ LA.string_of_expr e
  | ExpectedIntegerLiteral e ->
      "Expected an integer literal but found " ^ LA.string_of_expr e
  | IlltypedArraySlice ty ->
      "Slicing can only be done on an array type but found "
      ^ string_of_tc_type ty
  | ExpectedIntegerTypeForSlice -> "Slicing should have integer types"
  | IlltypedArrayIndex ty ->
      "Indexing can only be done on an array type but found "
      ^ string_of_tc_type ty
  | ExpectedIntegerTypeForArrayIndex ty ->
      "Array index should have an integer type but found "
      ^ string_of_tc_type ty
  | IlltypedArrayConcat (both_array_types, ty1, ty2) -> (
      "Cannot concat "
      ^
      match both_array_types with
      | true -> (
          "array of two different types" ^ string_of_tc_type ty1
          ^
          match ty2 with
          | Some ty2 -> " and " ^ string_of_tc_type ty2
          | None -> "")
      | false -> "non-array type " ^ string_of_tc_type ty1)
  | IlltypedDefaults -> "Defaults do not have the same type as node call"
  | IlltypedMerge ty ->
      "All expressions in merge expected to be the same type "
      ^ string_of_tc_type ty
  | IlltypedFby (ty1, ty2) ->
      "Both the expressions in Fby should be of the same type." ^ "Found types "
      ^ string_of_tc_type ty1 ^ " and " ^ string_of_tc_type ty2
  | IlltypedArrow (ty1, ty2) ->
      "Arrow types do not match " ^ string_of_tc_type ty1 ^ " and "
      ^ string_of_tc_type ty2
  | IlltypedCall (ty1, ty2) ->
      "Node arguments at call expect to have type " ^ string_of_tc_type ty1
      ^ " but found type " ^ string_of_tc_type ty2
  | ExpectedFunctionType ty ->
      "Expected node type to be a function type, but found type "
      ^ string_of_tc_type ty
  | IlltypedIdentifier (id, ty1, ty2) ->
      "Identifier '"
      ^ HString.string_of_hstring id
      ^ "' does not match expected type " ^ string_of_tc_type ty1
      ^ " with inferred type " ^ string_of_tc_type ty2
  | UnificationFailed (ty1, ty2) ->
      "Cannot unify type " ^ string_of_tc_type ty1 ^ " with inferred type "
      ^ string_of_tc_type ty2
  | ExpectedType (ty1, ty2) ->
      "Expected type " ^ string_of_tc_type ty1 ^ " but found type "
      ^ string_of_tc_type ty2
  | EmptyArrayExpression -> "Array expression cannot be empty"
  | ExpectedArrayType ty ->
      "Expected an array type but found type " ^ string_of_tc_type ty
  | MismatchedNodeType (id, ty1, ty2) ->
      "Node '"
      ^ HString.string_of_hstring id
      ^ "' of type " ^ string_of_tc_type ty1 ^ " does not match expected type "
      ^ string_of_tc_type ty2
  | IlltypedBitNot ty ->
      "Cannot apply the bit-value not operator to a non machine integer value \
       of type " ^ string_of_tc_type ty
  | IlltypedUnaryMinus ty ->
      "Unary minus cannot be applied to non number expression of type "
      ^ string_of_tc_type ty
  | ExpectedIntegerTypes (ty1, ty2) ->
      "Expected both arguments of operator to be of same integer type but \
       found " ^ string_of_tc_type ty1 ^ " and " ^ string_of_tc_type ty2
  | ExpectedNumberTypes (ty1, ty2) ->
      "Expected both arguments of operator to be of same integer type (or type \
       real) but found " ^ string_of_tc_type ty1 ^ " and "
      ^ string_of_tc_type ty2
  | ExpectedMachineIntegerTypes (ty1, ty2) ->
      "Expected both arguments of operator to be of machine integer type but \
       found " ^ string_of_tc_type ty1 ^ " and " ^ string_of_tc_type ty2
  | ExpectedBitShiftConstantOfSameWidth ty ->
      "Expected second argument of shit opperator to be a constant of type "
      ^ "unsigned machine integer of the same width as first argument but \
         found type " ^ string_of_tc_type ty
  | ExpectedBitShiftMachineIntegerType ty ->
      "Expected first argument of shit operator to be of type signed "
      ^ "or unsigned machine integer but found type " ^ string_of_tc_type ty
  | InvalidConversion (ty1, ty2) ->
      "Cannot convert type " ^ string_of_tc_type ty1 ^ " to type "
      ^ string_of_tc_type ty2
  | NodeArgumentOnLHS v ->
      "Input '" ^ HString.string_of_hstring v ^ "' can not be defined"
  | MismatchOfEquationType (items, ty) ->
      "Term structure on left hand side of the equation "
      ^ (match items with
        | Some items ->
            Lib.string_of_t
              ...
              items
        | None -> "")
      ^ " does not match expected type " ^ string_of_tc_type ty
      ^ " on right hand side of the node equation"
  | DisallowedReassignment vars ->
      "Cannot reassign value to a constant or enum but found reassignment to \
       identifier(s): "
      ^ Lib.string_of_t
          (Lib.pp_print_list LA.pp_print_ident ", ")
          (LA.SI.elements vars)
  | AssumptionMustBeInputOrOutput id ->
      "Assumption variable must be either an input or an output variable, "
      ^ "but found '"
      ^ HString.string_of_hstring id
      ^ "'"
  ...
  | UndeclaredType id ->
      "Type '" ^ HString.string_of_hstring id ^ "' is undeclared"
  | EmptySubrange (v1, v2) ->
      "Range can not be empty, but found range: [" ^ string_of_int v1 ^ ", "
      ^ string_of_int v2 ^ "]"
  | SubrangeArgumentMustBeConstantInteger e ->
  ...
  | ExpectedRecordType ty ->
      "Expected record type but found " ^ string_of_tc_type ty
  | GlobalConstRefType id ->
      "Definition of global constant '"
      ^ HString.string_of_hstring id
      ^ "' has refinement type (not yet supported)"
  | QuantifiedAbstractType id ->
      "Variable '"
      ^ HString.string_of_hstring id
      ^ "' with type that contains an abstract type (or type variable) cannot \
         be quantified"
  | UnsupportedQuantifiedArray id ->
      "Quantified variable '"
      ^ HString.string_of_hstring id
      ^ "' has a type that includes an array, which is not currently supported"
  | InvalidPolymorphicCall id ->
      "Call to node, contract, or user type '"
      ^ HString.string_of_hstring id
      ^ "' passes an incorrect number of type parameters"
  | InvalidNumberOfIndices id ->
      "Recursive definition of array '"
      ^ HString.string_of_hstring id
      ^ "' must use one (and only one) index for every array dimension"

type warning_kind = UnusedBoundVariableWarning of HString.t

let warning_message warning =
  match warning with
  | UnusedBoundVariableWarning id ->
      "Unused refinement type bound variable " ^ HString.string_of_hstring id
...
let add_ty_params_node_ctx ctx nname params =
  ...
  List.fold_left
    (fun acc p -> add_ty_syn acc p (LA.AbstractType (Lib.dummy_pos, p)))
    ctx params

let add_io_node_ctx ctx nname params inputs outputs =
  let ctx = add_ty_params_node_ctx ctx nname params in
  let ctx = inputs |> List.map extract_consts |> List.fold_left union ctx in
  let ctx = inputs |> List.map extract_arg_ctx |> List.fold_left union ctx in
  let ctx = outputs |> List.map extract_ret_ctx |> List.fold_left union ctx in
  ctx

let add_local_node_ctx ctx locals =
  locals |> List.map extract_loc_ctx |> List.fold_left union ctx
...
let infer_type_const : Lib.position -> LA.constant -> tc_type =
 fun pos -> function Num _ -> Int pos | Dec _ -> Real pos | _ -> Bool pos

let check_merge_clock : LA.expr -> LA.lustre_type -> (unit, [> error ]) result =
 fun e ty ->
  match ty with
...
let check_merge_exhaustive :
 ...
 fun ctx pos ty cases ->
  match ty with
  | EnumType (_, enum_id, _) -> (
      match lookup_variants ctx enum_id with
      | Some variants ->
          let check_cases_containment =
            R.seq_
              (List.map
                 (fun i ->
          ...
          let check_cases_cover =
            R.seq_
              (List.map
                 ...
                 variants)
          in
          let check_cases_unique =
            R.seq_
              (List.map
                 (fun i ->
                   if
                     List.length
                       (List.filter (fun x -> HString.equal x i) cases)
                     > 1
                   then type_error pos (MergeCaseNotUnique i)
                   else Ok ())
                 variants)
          in
  ...
  | _ ->
      type_error pos
        (Impossible
           ("Type " ^ string_of_tc_type ty ^ " must be a bool or an enum type"))
...
let rec infer_const_attr ctx exp =
  ...
  | GroupExpr (_, ArrayExpr, es) | GroupExpr (_, TupleExpr, es) ->
      List.fold_left (fun l1 l2 -> combine l1 l2) [ R.ok () ] (List.map r es)
  ...
  | Quantifier (_, _, _, _) -> [ error exp "quantified expression" ]
  (* Clock operators *)
  | When (_, e, _) -> List.map (fun _ -> error exp "when operator") (r e)
  | Merge (_, _, es) ->
      List.map
        (fun _ -> error exp "merge operator")
        (r (List.hd (snd (List.split es))))
  (* Temporal operators *)
  | Pre (_, e) -> List.map (fun _ -> error exp "pre operator") (r e)
  | Arrow (_, e1, _) -> List.map (fun _ -> error exp "arrow operator") (r e1)
  (* Node calls *)
...
let check_constant_args ctx i arg_exprs =
  let check param_attr =
    let arg_attr = List.map (infer_const_attr ctx) arg_exprs |> List.flatten in
    R.seq_chain
      (fun _ ((id, is_const_param), res) ->
        match (is_const_param, res) with
        | true, Error (pos, exn_fn) ->
            let where =
              "argument for constant parameter '"
              ^ HString.string_of_hstring id
              ^ "'"
            in
            type_error pos (exn_fn where)
...
let rec type_extract_array_lens ctx ty =
  match ty with
...
let update_ty_with_ctx node_ty call_params ctx arg_exprs =
  let call_param_len_idents =
    ...
    |> List.filter (fun id ->
           (not (member_val ctx id)) || List.mem id call_params)
  in
  ...
  | _ ->
      let find_matching_len_params array_param_lens =
        List.map
          (fun len -> Lib.list_index (fun id2 -> len = id2) call_params)
          array_param_lens
      in
      ...
      LH.apply_subst_in_type
        (List.combine call_param_len_idents array_len_exprs)
        node_ty

let instantiate_type_variables :
    tc_context ->
    Lib.position ->
    HString.t ->
    tc_type ->
 ...
 fun ctx pos nname ty ty_args ->
  ...
  match
    ( lookup_node_ty_vars ctx nname,
      lookup_contract_ty_vars ctx nname,
      lookup_ty_ty_vars ctx nname )
  with
  | None, None, None ->
      let* substitution =
        try R.ok (List.combine [] ty_args)
        with Invalid_argument _ ->
          type_error pos (InvalidPolymorphicCall nname)
      in
      R.ok (LustreAstHelpers.apply_type_subst_in_type substitution ty)
  | Some ty_vars, _, _ | _, Some ty_vars, _ | _, _, Some ty_vars ->
      let* substitution =
        try R.ok (List.combine ty_vars ty_args)
        with Invalid_argument _ ->
          type_error pos (InvalidPolymorphicCall nname)
      in
      R.ok (LustreAstHelpers.apply_type_subst_in_type substitution ty)

let rec instantiate_type_variables_expr :
    tc_context ->
    HString.t ->
    tc_type list ->
    LA.expr ->
 ...
 fun ctx nname ty_args expr ->
  ...
  | AnyOp _ ->
      assert false (* Polymorphism is handled after any ops are desugared *)

(** Chases the type (and nested types) to its base form to resolve type
    synonyms. Also simplifies refinement types and history types to their base
    types. In addition, chases int ranges to their base types (int) if
    [expand_subrange] is true. *)
...
(** Chases the type (and nested types) to its base form to resolve type
    synonyms. Also simplifies refinement types, history types, __and subrange
    types__ to their base types. *)
let expand_type_syn_reftype_history_subrange ctx =
  expand_type_syn_reftype_history ~expand_subrange:true ctx

(** Infer the type of a [LA.expr] with the types of free variables given in
    [tc_context] *)
let rec infer_type_expr :
    tc_context ->
    HString.t option ->
    LA.expr ->
 ...
 fun ctx nname -> function
  ...
  | LA.ModeRef (pos, ids) ->
      let lookup_mode_ty ctx (ids : HString.t list) =
        ...
        | rest -> (
            let i = HString.concat (HString.mk_hstring "::") rest in
            match lookup_ty ctx i with
  ...
  | LA.TupleProject (pos, e1, i) -> (
      ...
      | LA.TupleType (pos, tys) as ty ->
          if List.length tys <= i then
            type_error pos (TupleIndexOutOfBounds (i, ty))
          else
  ...
  | LA.TernaryOp (pos, top, con, e1, e2) -> (
      match top with
      | Ite -> (
          infer_type_expr ctx nname con >>= function
          | Bool _, warnings1 ->
      ...
      | Some ty -> (
          ...
          | LA.RecordType (_, r_name, fld_tys) ->
              ...
              if List.length flds > List.length matches then
                let open HString in
                let fnames1 = HStringSet.of_list (List.map fst flds) in
                let fnames2 =
                ...
                let infer_field ctx (p, i, exp, ty) =
                  ...
                  if eq then R.ok ((p, i, ty), warnings)
                    ...
                    type_error (LH.pos_of_expr exp) (ExpectedType (ty, exp_ty))
                in
                let* fld_tys_warns =
                  R.seq (List.map (infer_field ctx) matches)
                in
                let fld_tys, warnings = List.split fld_tys_warns in
                R.ok
                  (LA.RecordType (pos, r_name, fld_tys), List.flatten warnings)
  ...
  | LA.GroupExpr (pos, struct_type, exprs) -> (
      ...
      | LA.ArrayExpr ->
          ...
          R.ifM
            (R.seqM ( && ) true (List.map (eq_lustre_type ctx elty) tys))
            (let arr_ty = List.hd tys in
             let arr_size =
               LA.Const
                 ( pos,
                   Num (List.length tys |> string_of_int |> HString.mk_hstring)
                 )
             in
  ...
  | LA.StructUpdate (pos, ue, i_or_ls, e) -> (
      if List.length i_or_ls != 1 then
        type_error pos
          (Unsupported
             "List of labels or indices for structure update is not supported")
      else
        match List.hd i_or_ls with
        | LA.Label (pos, l) -> (
            infer_type_expr ctx nname ue >>= function
            | (RecordType (_, _, flds) as r_ty), warnings1 -> (
                ...
                | Some f_ty ->
                    ...
                    R.ifM
                      (eq_lustre_type ctx f_ty e_ty)
                      (R.ok (r_ty, warnings1 @ warnings2))
                      (type_error pos
                         (TypeMismatchOfRecordLabel (l, f_ty, e_ty)))
  ...
  | LA.Quantifier (_, _, qs, e) ->
      let* warnings1 =
        R.seq
          (List.map
             (fun (_, _, ty) -> check_type_well_formed ctx Local nname true ty)
             qs)
      in
      let extn_ctx =
        List.fold_left union ctx
          (List.map (fun (_, i, ty) -> singleton_ty i ty) qs)
      in
  ...
  | LA.Condact (pos, c, _, node, args, defaults) ->
      ...
      in
      let* d_tys_warns =
        R.seq (List.map (infer_type_expr ctx nname) defaults)
      in
      let d_tys, warnings2 = List.split d_tys_warns in
      R.ifM
        (eq_lustre_type ctx r_ty (GroupType (pos, d_tys)))
  ...
  | LA.Merge (pos, i, mcases) as e ->
      ...
      let* case_tys_warns =
        R.seq (List.map (infer_type_expr ctx nname) mcases_exprs)
      in
      ...
      R.ifM
        (R.seqM ( && ) true (List.map (eq_lustre_type ctx main_ty) case_tys))
  ...
  | LA.Call (pos, ty_args, i, arg_exprs) -> (
      ...
      let* warnings1 =
        R.seq (List.map (check_type_well_formed ctx Input nname true) ty_args)
      in
      let infer_type_node_args :
          tc_context ->
          LA.expr list ->
          (tc_type * [> warning ] list, [> error ]) result =
       fun ctx args ->
        let* arg_tys_warns =
          R.seq (List.map (infer_type_expr ctx nname) args)
        in
        let arg_tys, warnings = List.split arg_tys_warns in
        if List.length arg_tys = 1 then
          R.ok (List.hd arg_tys, List.flatten warnings)
      ...
      | Some call_params, Some node_ty ->
          ...
          let exp_arg_tys, exp_ret_tys =
            match node_ty with
            | LA.TArr (_, exp_arg_tys, exp_ret_tys) ->
                ( expand_type_syn ctx exp_arg_tys,
          ...
          if are_equal then
            check_constant_args ctx i arg_exprs
            >> R.ok (exp_ret_tys, List.flatten warnings1 @ warnings2)
          else type_error pos (IlltypedCall (exp_arg_tys, given_arg_tys))
...
and check_array_dimensions pos ctx base_e idxs =
  ...
  let rec calc_number_of_array_dimensions ty =
    match ty with
    | LustreAst.ArrayType (_, (ty, _)) -> 1 + calc_number_of_array_dimensions ty
    | _ -> 0
  in
...
(** Type checks an expression and returns [ok] * if the expected type is the
    given type [tc_type] * returns an [Error of string] otherwise *)
and check_type_expr :
    tc_context ->
    HString.t option ->
    LA.expr ->
 ...
 fun ctx nname expr exp_ty ->
  ...
  | ModeRef (pos, ids) ->
      let id =
        match ids with
        | [] -> failwith "empty mode name"
        | rest -> HString.concat (HString.mk_hstring "::") rest
      in
  ...
  | RecordExpr (pos, name, ty_args, flds) ->
      ...
      let inf_r_ty =
        LA.RecordType (pos, name, List.map2 (mk_ty_ident pos) ids inf_tys)
      in
      let* exp_ty = instantiate_type_variables ctx pos name exp_ty ty_args in
      R.ifM
        (eq_lustre_type ctx exp_ty inf_r_ty)
  ...
  | GroupExpr (pos, group_ty, es) -> (
      ...
      | ExprList ->
          let* inf_tys_warns =
            R.seq (List.map (infer_type_expr ctx nname) es)
          in
          let inf_tys, warnings = List.split inf_tys_warns in
          let inf_ty = LA.GroupType (pos, inf_tys) in
          R.ifM
            (eq_lustre_type ctx exp_ty inf_ty)
            (R.ok (List.flatten warnings))
      ...
      | ArrayExpr ->
          ...
          in
          let inf_tys, warnings = List.split inf_tys_warns in
          if List.length inf_tys < 1 then type_error pos EmptyArrayExpression
          else
            let elty = List.hd inf_tys in
            R.ifM
              (R.seqM ( && ) true (List.map (eq_lustre_type ctx elty) inf_tys))
              (let arr_size =
                 LA.Const
                   ( pos,
                     Num
                       (List.length inf_tys |> string_of_int
                      |> HString.mk_hstring) )
               in
               let arr_ty = LA.ArrayType (pos, (elty, arr_size)) in
               R.ifM
                 (eq_lustre_type ctx exp_ty arr_ty)
                 (R.ok (List.flatten warnings))
  ...
  | Condact (pos, c, _, node, args, defaults) ->
      ...
      let* warnings2 =
        check_type_expr ctx nname (Call (pos, [], node, args)) exp_ty
      in
      let* dy_tys_warns =
        R.seq (List.map (infer_type_expr ctx nname) defaults)
      in
      ...
      R.ifM
        (eq_lustre_type ctx exp_ty (GroupType (pos, d_tys)))
  ...
  | RestartEvery (pos, node, args, cond) ->
      ...
      let* warnings2 =
        check_type_expr ctx nname (LA.Call (pos, [], node, args)) exp_ty
      in
      R.ok (warnings1 @ warnings2)
  (* Temporal operators *)
  ...
  | Call (pos, ty_args, i, args) -> (
      ...
      let arg_tys, warnings = List.split arg_tys_warns in
      let arg_ty =
        if List.length arg_tys = 1 then List.hd arg_tys
        else GroupType (pos, arg_tys)
      in
      match (lookup_node_ty ctx i, lookup_node_param_ids ctx i) with
      | None, _ | _, None -> type_error pos (UnboundNodeName i)
      | Some ty, Some call_params ->
          ...
          if b then R.ok (List.flatten warnings)
            ...
            type_error pos
              (MismatchedNodeType (i, TArr (pos, arg_ty, exp_ty), ty)))

(** Infers type of unary operator application *)
and infer_type_unary_op :
    tc_context ->
    HString.t option ->
    Lib.position ->
    LA.expr ->
 ...
 fun ctx pos ty1 ty2 ->
  ...
  let num_tys =
    [
      ...
      LA.Int64 pos;
      LA.IntRange
        (pos, Some (Const (pos, Num num1)), Some (Const (pos, Num num1)));
      LA.Real pos;
    ]
  in
  let are_equal_types :
      tc_context -> tc_type -> tc_type -> tc_type -> (bool, [> error ]) result =
   fun ctx ty1 ty2 ty ->
    R.seqM ( && ) true [ eq_lustre_type ctx ty1 ty; eq_lustre_type ctx ty2 ty ]
  in
  R.seqM ( || ) false (List.map (are_equal_types ctx ty1 ty2) num_tys)

(** infers the type of binary operators *)
and infer_type_binary_op :
    tc_context ->
    HString.t option ->
    Lib.position ->
    LA.binary_operator ->
 ...
 fun ctx nname pos op e1 e2 ->
  ...
  | LA.And | LA.Or | LA.Xor | LA.Impl ->
      R.ifM
        (eq_lustre_type ctx ty1 (Bool pos))
        (R.ifM
           (eq_lustre_type ctx ty2 (Bool pos))
  ...
  | LA.Mod -> (
      match
        (is_type_int_or_machine_int ctx ty1, is_type_int_or_machine_int ctx ty2)
      with
      | Ok true, Ok true ->
          R.ifM
            (eq_lustre_type ctx ty1 ty2)
  ...
  | LA.Plus | LA.Minus | LA.Times | LA.Div ->
      ...
      if is_num then R.ok (ty2, warnings1 @ warnings2)
  ...
  | LA.BVShiftL | LA.BVShiftR -> (
      match
        ( is_type_signed_machine_int ctx ty1,
          is_type_unsigned_machine_int ctx ty1 )
      with
      | Ok b1, Ok b2 when b1 || b2 -> (
          match
            ( is_type_unsigned_machine_int ctx ty2,
              is_machine_type_of_associated_width ctx (ty1, ty2) )
          with
          | Ok true, Ok true -> R.ok (ty1, warnings1 @ warnings2)
          | Ok _, Ok _ ->
              type_error pos (ExpectedBitShiftConstantOfSameWidth ty1)
          | Error id, _ | _, Error id -> type_error pos (UnboundIdentifier id))
      | Ok _, Ok _ -> type_error pos (ExpectedBitShiftMachineIntegerType ty1)
      | Error id, _ | _, Error id -> type_error pos (UnboundIdentifier id))

(** Converts from given type to the intended type aka casting *)
and infer_type_conv_op :
    tc_context ->
    HString.t option ->
    Lib.position ->
    LA.expr ->
    LA.conversion_operator ->
    (tc_type * [> warning ] list, [> error ]) result =
 fun ctx nname pos e op ->
  let* ty, warnings = infer_type_expr ctx nname e in
  match op with
...
and infer_type_comp_op :
    tc_context ->
    HString.t option ->
    Lib.position ->
    LA.expr ->
    LA.expr ->
    LA.comparison_operator ->
    (tc_type * [> warning ] list, [> error ]) result =
  ...
  | Neq | Eq ->
      R.ifM
        ...
        (if LH.type_contains_array ty1 then
           type_error pos
             (Unsupported "Extensional array equality is not supported")
         else R.ok (LA.Bool pos, warnings1 @ warnings2))
        (type_error pos (UnificationFailed (ty1, ty2)))
  | Lte | Lt | Gte | Gt ->
      are_args_num ctx pos ty1 ty2 >>= fun is_num ->
      if is_num then R.ok (LA.Bool pos, warnings1 @ warnings2)
      else type_error pos (ExpectedIntegerTypes (ty1, ty2))

and check_type_record_proj :
    Lib.position ->
    tc_context ->
    HString.t option ->
    LA.expr ->
 ...
 fun pos ctx nname expr idx exp_ty ->
  infer_type_expr ctx nname expr >>= function
  | RecordType (_, _, flds), warnings ->
      ...
      >>= fun (_, _, fty) ->
      R.ifM
        (eq_lustre_type ctx fty exp_ty)
        (R.ok warnings)
        (type_error pos (UnificationFailed (exp_ty, fty)))
  | rec_ty, _ ->
      type_error (LH.pos_of_expr expr) (IlltypedRecordProjection rec_ty)

and check_type_tuple_proj :
    Lib.position ->
    tc_context ->
    HString.t option ->
    LA.expr ->
 ...
 fun pos ctx nname expr idx exp_ty ->
  ...
  | (TupleType (_, tys) as ty), warnings ->
      if List.length tys <= idx then
        type_error pos (TupleIndexOutOfBounds (idx, ty))
      else
        R.ok (List.nth tys idx) >>= fun ity ->
        R.ifM
          (eq_lustre_type ctx ity exp_ty)
          (R.ok warnings)
          (type_error pos (UnificationFailed (exp_ty, ity)))
  | ty, _ -> type_error (LH.pos_of_expr expr) (IlltypedTupleProjection ty)

and check_type_const_decl :
    tc_context ->
    HString.t option ->
    LA.const_decl ->
    tc_type ->
...
and check_type_node_decl :
 ...
 fun pos ctx
  ...
  let arg_ids =
    LA.SI.of_list (List.map (fun a -> LH.extract_ip_ty a |> fst) input_vars)
  in
  let ret_ids =
    LA.SI.of_list (List.map (fun a -> LH.extract_op_ty a |> fst) output_vars)
  in
  let loc_ids =
    LA.SI.of_list
      (List.map (fun a -> LH.extract_loc_ty a |> fun (id, _, _) -> id) ldecls)
  in
  ...
  R.seq_chain
    (fun _ i ->
      if member_ty ctx i then type_error pos (Redeclaration i) else R.ok ())
    ()
  ...
  R.seq_chain
    (fun _ i ->
      if member_ty_syn ctx i then type_error pos (Redeclaration i) else R.ok ())
    () params
  >>
  let ctx =
    List.fold_left
      (fun acc p -> add_ty_syn acc p (LA.AbstractType (pos, p)))
      ctx params
  in
  (* (Debug.parse "Params: %a (skipping)" LA.pp_print_node_param_list params; *)
  (* store the input constants passed in the input *)
  let ip_constants_ctx =
    List.fold_left union ctx (List.map extract_consts input_vars)
  in
  (* These are inputs to the node *)
  let ctx_plus_ips =
    List.fold_left union ip_constants_ctx (List.map extract_arg_ctx input_vars)
  in
  (* These are outputs of the node *)
  let ctx_plus_ops_and_ips =
    List.fold_left union ctx_plus_ips (List.map extract_ret_ctx output_vars)
  in
  ...
  | None -> R.ok []
  | Some c ->
      let* con_ctx, warnings =
        tc_ctx_of_contract ctx_plus_ops_and_ips Ghost node_name c
      in
      Debug.parse "Checking node contract with context %a" pp_print_tc_context
        con_ctx;
  ...
  if is_extern then
    R.ok
      (Debug.parse "External Node, no body to type check.";
    ...
    let* warnings1 =
      R.seq
        (List.map
           (fun local_decl ->
             match local_decl with
             | LA.NodeConstDecl (_, TypedConst (_, i, e, ty)) ->
                 let* _ =
                   check_expr_is_constant local_ctx "constant definition" e
                 in
                 let* warnings2 =
                   check_type_expr (add_ty local_ctx i ty) (Some node_name) e ty
                 in
                 let* warnings3 =
                   check_type_well_formed local_ctx Local (Some node_name) true
                     ty
                 in
                 R.ok (warnings2 @ warnings3)
             | LA.NodeVarDecl (_, (_, _, ty, _)) ->
                 check_type_well_formed local_ctx Local (Some node_name) false
                   ty
           ...
           ldecls)
    in
    Debug.parse "Local Typing Context with local state: {%a}"
    ...
    let check_lhs_eqns =
      List.map
        (fun (pos, v) ->
          if SI.mem v arg_ids then type_error pos (NodeArgumentOnLHS v)
          else R.ok ())
...
and do_node_eqn :
 ...
 fun ctx nname -> function
  ...
  | LA.Equation (_, lhs, e) as eqn ->
      ...
      let get_array_def_context : LA.struct_item -> tc_context = function
        | ArrayDef (pos, _, is) ->
            List.fold_left
              (fun c i -> add_ty c i (LA.Int pos))
              empty_tc_context is
        | _ -> empty_tc_context
      in
      ...
      Debug.parse "Checking node equation lhs=%a; rhs=%a" LA.pp_print_eq_lhs lhs
        LA.pp_print_expr e;
      let* ty, warnings1 = infer_type_expr new_ctx (Some nname) e in
      Debug.parse "RHS has type %a for lhs %a" LA.pp_print_lustre_type ty
 ...
 fun ctx nname -> function
  ...
  | LA.IfBlock (pos, e, l1, l2) -> (
      ...
      | Bool _ ->
          let* warnings2 =
            R.seq
              (List.map (do_item ctx nname) l1 @ List.map (do_item ctx nname) l2)
          in
          R.ok (warnings1 @ List.flatten warnings2)
      | e_ty -> type_error pos (ExpectedBooleanExpression e_ty))
  | LA.FrameBlock (pos, vars, nes, nis) ->
      let vars = List.map snd vars in
      let reassigned_consts =
        SI.filter (fun e -> member_val ctx e) (SI.of_list vars)
      in
      let* warnings =
        R.seq
          ((if SI.cardinal reassigned_consts = 0 then R.ok []
            else type_error pos (DisallowedReassignment reassigned_consts))
           :: List.map (do_node_eqn ctx nname) nes
          @ List.map (do_item ctx nname) nis)
      in
      R.ok (List.flatten warnings)
  | LA.AnnotMain _ as ann ->
      Debug.parse "Node Item Skipped (Main Annotation): %a"
        LA.pp_print_node_item ann;
      R.ok []
  | LA.AnnotProperty (_, _, e1, Provided e2) as ann ->
      ...
      let* warnings1 =
        check_type_expr ctx (Some nname) e1 (Bool (LH.pos_of_expr e1))
      in
      let* warnings2 =
        check_type_expr ctx (Some nname) e2 (Bool (LH.pos_of_expr e2))
      in
      R.ok (warnings1 @ warnings2)
  ...
  | SingleIdent (pos, i) ->
      let* inf_ty =
        ...
        | None ->
            type_error pos
              (Impossible
                 ("Could not find Identifier " ^ HString.string_of_hstring i))
        | Some ty -> R.ok ty
      in
      let* inferred_is_expected1 = eq_lustre_type ctx exp_ty inf_ty in
      let* inferred_is_expected2 =
...
(** The structure of the left hand side of the equation * should match the type
    of the right hand side expression *)
and check_type_struct_def :
    tc_context ->
    HString.t ->
    LA.eq_lhs ->
    tc_type ->
 ...
 fun ctx nname (StructDef (pos, lhss)) exp_ty ->
  ...
  Debug.parse
    "Checking if structure definition: %a has type %a \nwith local context %a"
    (Lib.pp_print_list LA.pp_print_struct_item ",")
    lhss LA.pp_print_lustre_type exp_ty pp_print_tc_context ctx;
  ...
  if SI.for_all (fun i -> not (member_val ctx i)) lhs_vars then
    match exp_ty with
    | GroupType (_, exp_ty_lst') ->
        ...
        then check_type_struct_item ctx nname (List.hd lhss) exp_ty
          ...
          (* Case 2. LHS is a compound statment *)
          List.length lhss = List.length exp_ty_lst
        then
          let* warnings =
            R.seq (List.map2 (check_type_struct_item ctx nname) lhss exp_ty_lst)
          in
          R.ok (List.flatten warnings)
    ...
    | _ ->
        if List.length lhss != 1 then
          type_error pos (MismatchOfEquationType (None, exp_ty))
        else
          let lhs = List.hd lhss in
          check_type_struct_item ctx nname lhs exp_ty
  else
    type_error pos
      (DisallowedReassignment (SI.filter (fun e -> member_val ctx e) lhs_vars))

and tc_ctx_contract_eqn :
    tc_context ->
    HString.t ->
    LA.contract_node_equation ->
 ...
 fun ctx cname -> function
  ...
  | ContractCall (_, cc, _, _, _) -> (
      ...
      | None ->
          failwith
            ("Cannot find exports for contract " ^ HString.string_of_hstring cc)
      | Some m ->
          R.ok
            ( List.fold_left
                (fun c (i, ty) ->
                  add_ty c
                    (HString.concat (HString.mk_hstring "::") [ cc; i ])
                    ty)
                ctx (IMap.bindings m),
              [] ))

and check_type_contract_decl :
    tc_context ->
    LA.contract_node_decl ->
    ([> warning ] list, [> error ]) result =
 fun ctx (cname, _, args, rets, (p, contract)) ->
  let arg_ids =
    LA.SI.of_list (List.map (fun arg -> LH.extract_ip_ty arg |> fst) args)
  in
  let ret_ids =
    LA.SI.of_list (List.map (fun ret -> LH.extract_op_ty ret |> fst) rets)
  in
  ...
  let local_const_ctx =
    List.fold_left union ret_ctx (List.map extract_consts args)
  in
  ...
  let* warnings2 =
    check_type_contract (arg_ids, ret_ids) local_ctx cname (p, contract)
  in
  R.ok
    (Debug.parse "TC Contract Decl %a done }" LA.pp_print_ident cname;
     List.flatten warnings1 @ warnings2)

and check_type_contract :
    LA.SI.t * LA.SI.t ->
    tc_context ->
    HString.t ->
    LA.contract ->
 ...
 fun node_params ctx nname (_, eqns) ->
  let* warnings =
    R.seq (List.map (check_contract_node_eqn node_params ctx nname) eqns)
  in
  R.ok (List.flatten warnings)

and check_contract_node_eqn :
    LA.SI.t * LA.SI.t ->
    tc_context ->
    HString.t ->
    LA.contract_node_equation ->
 ...
 fun node_params ctx nname eqn ->
  Debug.parse "Checking node's contract equation: %a" LA.pp_print_contract_item
    eqn;
  match eqn with
  | AssumptionVars (_, ids) -> (
      ...
      match
        List.find_opt (fun (_, id) -> LA.SI.mem id io_params |> not) ids
      with
      | Some (pos, id) -> type_error pos (AssumptionMustBeInputOrOutput id)
      | None -> R.ok [])
  ...
  | GhostVars v ->
      ...
      do_node_eqn ctx nname node_eqn
  | Assume (pos, _, _, e) -> check_type_expr ctx (Some nname) e (Bool pos)
  ...
  | Mode (pos, _, reqs, ensures) ->
      let* warnings =
        R.seq
          (Lib.list_apply
             (List.map
                (check_type_expr ctx (Some nname))
  ...
  | ContractCall (pos, cname, ty_args, args, rets) -> (
      let* ret_tys_warns =
        R.seq
          (List.map
             (infer_type_expr ctx (Some nname))
             (List.map (fun i -> LA.Ident (pos, i)) rets))
      in
      let ret_tys, warnings1 = List.split ret_tys_warns in
      let ret_ty =
        if List.length ret_tys = 1 then List.hd ret_tys
        else LA.GroupType (pos, ret_tys)
      in
      let* arg_tys_warns =
        R.seq (List.map (infer_type_expr ctx (Some nname)) args)
      in
      ...
      | Some inf_ty ->
          let* inf_ty =
            instantiate_type_variables ctx pos cname inf_ty ty_args
          in
          R.ifM
            (eq_lustre_type ctx inf_ty exp_ty)
      ...
      | None ->
          type_error pos
            (Impossible
               ("Undefined or not in scope contract name "
...
and contract_eqn_to_node_eqn : LA.contract_ghost_vars -> LA.node_equation =
 fun (pos1, GhostVarDec (pos2, tis), expr) ->
  let lhs =
    LA.StructDef
      (pos2, List.map (fun (pos, i, _) -> LA.SingleIdent (pos, i)) tis)
  in
 ...
 fun ctx src nname -> function
  | LA.FreeConst (pos, i, ty) ->
      ...
      if member_ty ctx i then type_error pos (Redeclaration i)
        ...
        R.ok (add_ty (add_const ctx i (LA.Ident (pos, i)) ty src) i ty, warnings)
...
and tc_ctx_contract_vars :
    tc_context ->
    HString.t ->
    LA.contract_ghost_vars ->
...
and tc_ctx_of_ty_decl :
    tc_context -> LA.type_decl -> (tc_context, [> error ]) result =
 fun ctx -> function
  | LA.AliasType (pos, i, ps, ty) -> (
      ...
      let* _ =
        R.seq_chain
          (fun _ i ->
            if member_ty_syn ctx i then type_error pos (Redeclaration i)
            else R.ok ())
          () ps
      in
      ...
      check_type_well_formed ctx' Global None false ty
      >>
      match ty with
      | LA.EnumType (pos, ename, econsts) ->
          if
            List.for_all (fun e -> not (member_ty ctx e)) econsts
          ...
          then
            ...
            let enum_const_bindings =
              Lib.list_apply
                (List.map2 (Lib.flip singleton_const)
                   (List.map mk_ident econsts)
                   econsts)
            ...
            type_error pos
              (Redeclaration (HString.mk_hstring "Enum value or constant"))
...
and tc_ctx_of_node_decl :
    Lib.position ->
    tc_context ->
    LA.node_decl ->
    (tc_context * [> warning ] list, [> error ]) result =
 fun pos ctx (nname, _, _, ps, ip, op, _, _, _) ->
  Debug.parse "Extracting type of node declaration: %a" LA.pp_print_ident nname;
  if member_node ctx nname then type_error pos (Redeclaration nname)
  else
...
and tc_ctx_contract_node_eqn ?(ignore_modes = false) src cname (ctx, warnings) =
  function
  | LA.GhostConst c -> tc_ctx_const_decl ctx src (Some cname) c
  | LA.GhostVars vs ->
      let* ctx = tc_ctx_contract_vars ctx cname vs in
      R.ok (ctx, warnings)
  | LA.Mode (pos, mname, _, _) ->
      if ignore_modes then R.ok (ctx, warnings)
      else if member_ty ctx mname then type_error pos (Redeclaration mname)
      else R.ok (add_ty ctx mname (Bool pos), warnings)
  | LA.ContractCall (p, cc, _, _, _) -> (
      match lookup_contract_exports ctx cc with
      | None ->
          type_error p
            (Impossible ("Cannot find contract " ^ HString.string_of_hstring cc))
      | Some m ->
          R.ok
            ( List.fold_left
                (fun c (i, ty) ->
...
and extract_exports :
    ...
    LA.contract ->
    (tc_context * [> warning ] list, [> error ]) result =
  let exports_from_eqn :
      tc_context ->
      HString.t ->
      LA.contract_node_equation ->
      ((LA.ident * tc_type) list * [> warning ] list, [> error ]) result =
   fun ctx nname -> function
     ...
     | LA.GhostConst (UntypedConst (_, i, e)) ->
         ...
         R.ok ([ (i, ty) ], warnings)
     | LA.GhostConst (TypedConst (_, i, _, ty)) -> R.ok ([ (i, ty) ], [])
     | LA.GhostVars (_, GhostVarDec (_, tis), _) ->
         R.ok (List.map (fun (_, i, ty) -> (i, ty)) tis, [])
     | LA.Mode (pos, mname, _, _) ->
         if member_ty ctx mname then type_error pos (Redeclaration mname)
     ...
     | LA.ContractCall (p, cc, _, _, _) -> (
         ...
         | None ->
             type_error p
               (Impossible
                  ("Cannot find contract " ^ HString.string_of_hstring cc))
         | Some m ->
             R.ok
               ( List.map
                   (fun (k, v) ->
                     (HString.concat (HString.mk_hstring "::") [ cc; k ], v))
                   (IMap.bindings m),
                 [] ))
     | _ -> R.ok ([], [])
  in
  fun cname ctx (_, contract) ->
    let* (exports, _), warnings =
      R.seq_chain
...
and tc_ctx_of_contract_node_decl :
    Lib.position ->
    tc_context ->
    LA.contract_node_decl ->
    (tc_context * [> warning ] list, [> error ]) result =
 fun pos ctx (cname, params, inputs, outputs, contract) ->
  Debug.parse "Extracting type of contract declaration: %a" LA.pp_print_ident
    cname;
  if member_contract ctx cname then type_error pos (Redeclaration cname)
  else
    let ctx = add_ty_vars_contract ctx cname params in
    let* fun_ty, warnings1 =
      build_node_fun_ty pos ctx cname params inputs outputs
    in
...
and tc_ctx_of_declaration :
    tc_context * [> warning ] list ->
    LA.declaration ->
    (tc_context * [> warning ] list, [> error ]) result =
 fun (ctx', warnings) -> function
  | LA.ConstDecl (_, const_decl) ->
      tc_ctx_const_decl ctx' Global None const_decl
...
and tc_context_of :
    ...
    LA.t ->
    (tc_context * [> warning ] list, [> error ]) result =
 fun ctx decls -> R.seq_chain tc_ctx_of_declaration ctx decls

(** Process top level type declarations and make a type context with * user
    types, enums populated *)
and build_type_and_const_context :
...
and check_const_integer_expr :
 ...
 fun ctx nname kind e ->
  ...
  | Error (`LustreTypeCheckerError (pos, UnboundNodeName _)) ->
      type_error pos (ExpectedConstant (kind, "node call or any operator"))
  | Ok (ty, warnings) ->
      let* eq = eq_lustre_type ctx ty (LA.Int (LH.pos_of_expr e)) in
      if eq then check_expr_is_constant ctx kind e >> R.ok warnings
      else type_error (LH.pos_of_expr e) (ExpectedIntegerExpression ty)
  | Error err -> Error err
...
(** Does it make sense to have this type i.e. is it inhabited? * We do not want
    types such as int^true to creep in the typing context *)
and check_type_well_formed :
    tc_context ->
    source ->
    HString.t option ->
    bool ->
    tc_type ->
    ([> warning ] list, [> error ]) result =
 fun ctx src nname is_const -> function
  ...
  | LA.RefinementType (pos, (_, i, ty), e) ->
      ...
      (if is_const then
         ...
         check_expr_is_constant ctx
           "type of constant or refinement type argument" e
       else R.ok ())
      >>
      ...
      >>
      let warnings2 =
        if not (LH.expr_contains_id i e) then
          [ mk_warning pos (UnusedBoundVariableWarning i) ]
        else []
      in
  ...
  | LA.TupleType (_, tys) ->
      let* warnings =
        R.seq (List.map (check_type_well_formed ctx src nname is_const) tys)
      in
      R.ok (List.flatten warnings)
  | LA.GroupType (_, tys) ->
      let* warnings =
  ...
  | LA.UserType (pos, ty_args, i) as ty -> (
      if member_ty_syn ctx i || member_u_types ctx i then
        ...
        | Some nname -> (
            match
              (lookup_node_ty_vars ctx nname, lookup_contract_ty_vars ctx nname)
            with
            | Some ty_vars, _ | _, Some ty_vars ->
                if List.mem i ty_vars then R.ok []
                else type_error pos (UndeclaredType i)
            | None, None -> type_error pos (UndeclaredType i)))
  | LA.IntRange (pos, e1, e2) -> (
      ...
      | Some e1, Some e2 ->
          check_range_bound ctx nname e1
          >> check_range_bound ctx nname e2
          >>
  ...
  | Int32 _ | Int64 _ | Real _ | AbstractType _ | EnumType _ | History _ ->
      R.ok []

(** Function type for nodes will be [TupleType ips] -> [TupleTy outputs] *)
and build_node_fun_ty :
    Lib.position ->
    tc_context ->
    HString.t ->
    HString.t list ->
 ...
 fun pos ctx nname params args rets ->
  ...
  let fun_const_ctx =
    List.fold_left
      (fun ctx (i, ty) -> add_const ctx i (LA.Ident (pos, i)) ty Local)
      fun_ty_vars_ctx
      (List.filter LH.is_const_arg args |> List.map LH.extract_ip_ty)
  in
  let fun_ctx =
    List.fold_left
      (fun ctx (i, ty) -> add_ty ctx i ty)
      fun_const_ctx
      (List.map LH.extract_ip_ty args)
  in
  let fun_ctx =
    List.fold_left
      (fun ctx (i, ty) -> add_ty ctx i ty)
      fun_ctx
      (List.map LH.extract_op_ty rets)
  in
  let ops = List.map snd (List.map LH.extract_op_ty rets) in
  let ips = List.map snd (List.map LH.extract_ip_ty args) in
  let ret_ty =
    if List.length ops = 1 then List.hd ops else LA.GroupType (pos, ops)
  in
  let arg_ty =
    if List.length ips = 1 then List.hd ips else LA.GroupType (pos, ips)
  in
  let* warnings1 =
    check_type_well_formed fun_ctx Output (Some nname) false ret_ty
  in
  let* warnings2 =
    check_type_well_formed fun_ctx Input (Some nname) false arg_ty
  in
...
and eq_lustre_type :
 ...
 fun ctx t1 t2 ->
  ...
  | UserType (_, ty_args1, i1), UserType (_, ty_args2, i2) ->
      if List.length ty_args1 = List.length ty_args2 then
        let* r1 =
          R.seqM ( && ) true (List.map2 (eq_lustre_type ctx) ty_args1 ty_args2)
        in
        let r2 = i1 = i2 in
        R.ok (r1 && r2)
      else R.ok false
  ...
  | TupleType (_, tys1), TupleType (_, tys2) ->
      if List.length tys1 = List.length tys2 then
        R.seqM ( && ) true (List.map2 (eq_lustre_type ctx) tys1 tys2)
      else R.ok false
  (* For Equality for group types, flatten out the structures  *)
  | GroupType (_, tys1), GroupType (_, tys2) ->
      let ftys1, ftys2 =
        (LH.flatten_group_types tys1, LH.flatten_group_types tys2)
      in
      if List.length ftys1 = List.length ftys2 then
        R.seqM ( && ) true (List.map2 (eq_lustre_type ctx) ftys1 ftys2)
      else R.ok false
  | RecordType (_, n1, tys1), RecordType (_, n2, tys2) ->
      if List.length tys1 = List.length tys2 then
        let* isEqs =
          R.seq
            (List.map2 (eq_typed_ident ctx) (LH.sort_typed_ident tys1)
  ...
  | EnumType (_, n1, is1), EnumType (_, n2, is2) ->
      if List.length is1 = List.length is2 then
        R.ok
          (n1 = n2
          && List.fold_left ( && ) true
               (List.map2 ( = ) (LH.sort_idents is1) (LH.sort_idents is2)))
      else R.ok false
  (* node/function type *)
  | TArr (_, arg_ty1, ret_ty1), TArr (_, arg_ty2, ret_ty2) ->
      R.seqM ( && ) true
        [
          eq_lustre_type ctx arg_ty1 arg_ty2; eq_lustre_type ctx ret_ty1 ret_ty2;
        ]
  (* special case for type synonyms *)
  | UserType (pos, ty_args, u), ty | ty, UserType (pos, ty_args, u) ->
      if member_ty_syn ctx u then
        let* ty_alias =
          match lookup_ty_syn ctx u ty_args with
          | None ->
              type_error pos
                (Impossible
                   ("Cannot find definition of Identifier "
  ...
  | GroupType (_, tys), t | t, GroupType (_, tys) ->
      if List.length tys = 1 then eq_lustre_type ctx (List.hd tys) t
      else R.ok false
  | History (pos, v), ty | ty, History (pos, v) -> (
      match lookup_ty ctx v with
      | Some hist_ty -> eq_lustre_type ctx hist_ty ty
      | None -> type_error pos (UnboundIdentifier v))
  | _, _ -> R.ok false

(** Checks if the expr is of type Int. This will be useful * in evaluating array
    sizes that we need to have as constant integers * while declaring the array
    type *)
and is_expr_int_type : tc_context -> HString.t option -> LA.expr -> bool =
 fun ctx nname e ->
  R.safe_unwrap false
    ( infer_type_expr ctx nname e >>= fun (ty, _) ->
      eq_lustre_type ctx ty (LA.Int (LH.pos_of_expr e)) )

(** Compute type equality for [LA.typed_ident] *)
and eq_typed_ident :
    tc_context -> LA.typed_ident -> LA.typed_ident -> (bool, [> error ]) result
    =
 fun ctx (_, _, ty1) (_, _, ty2) -> eq_lustre_type ctx ty1 ty2

(** Compute equality for [LA.ArrayType]. If there are free constants in the
    size, the eval function will fail, but we want to pass such cases, as there
    might be some value assigment to the free constant that satisfies the type
    checker. Hence, silently return true with a leap of faith. *)
and eq_type_array :
    tc_context ->
    LA.lustre_type * LA.expr ->
    LA.lustre_type * LA.expr ->
    (bool, [> error ]) result =
...
(** By this point, all the circularity should be resolved, * the top most
    declaration should be able to access * the types of all the forward
    referenced indentifiers from the context*)
let rec type_check_group :
    tc_context -> LA.t -> ([> warning ] list, [> error ]) result list =
 fun global_ctx -> function
  | [] -> [ R.ok [] ]
  (* skip over type declarations and const_decls*)
  | LA.TypeDecl _ :: rest | LA.ConstDecl _ :: rest ->
  ...
  | LA.ContractNodeDecl (_, contract_decl) :: rest ->
      ...
      :: type_check_group global_ctx rest
  | LA.NodeParamInst _ :: rest -> type_check_group global_ctx rest

(** Typecheck a list of independent groups using a global context*)
let type_check_decl_grps :
    tc_context -> LA.t list -> ([> warning ] list, [> error ]) result list =
 fun ctx decls ->
  Debug.parse
    ("@.===============================================@."
...
let type_check_infer_nodes_and_contracts :
    tc_context -> LA.t -> (tc_context * [> warning ] list, [> error ]) result =
 fun ctx prg ->
  ...
  Debug.parse
    ("@.===============================================@."
   ^^ "Type checking declaration Groups@." ^^ "with TC Context@.%a@."
   ^^ "===============================================@.")
