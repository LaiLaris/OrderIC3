...
let get_assumption_vars in_sys sys =
  ...
  | Some { LN.contract } -> (
      ...
      | Some { LC.sofar_assump } -> (
          let svar_deps =
            match
              Scope.Map.find_opt scope (ISys.state_var_dependencies in_sys)
            with
            | Some deps -> deps
            | None -> assert false
          in
          try SVM.find sofar_assump svar_deps |> SVS.add sofar_assump
...
let compute_input_system_svars in_sys sys =
  ...
  | Some { LN.inputs; LN.locals; LN.outputs } ->
      ...
      in
      let inputs = List.map snd (LustreIndex.bindings inputs) |> SVS.of_list in
      let locals =
        locals
        |> List.map (fun l -> List.map snd (LustreIndex.bindings l))
    ...
    (fun acc_vars { TSys.pos; TSys.map_down } ->
      ...
      | Some { LN.call_outputs } ->
          ...
          if SVS.inter call_outputs assumption_vars |> SVS.is_empty then
            ...
            in
            let trans_svars =
              call_vars |> SVS.filter (fun sv -> SVS.mem sv in_sys_svars |> not)
            in
...
let compute_controllable_vars in_sys sys vars_at_1 =
  ...
  let controllable_vars_at_0 =
    ...
    |> List.filter (fun v ->
           ...
           (not (StateVar.is_input sv)) && not (SVS.mem sv assumption_vars))
  in
...
let check_contract_realizability in_sys sys =
  ...
  let call_output_args =
    let instances = TSys.get_subsystem_instances sys |> List.map fst in
    List.fold_left
      (fun acc called_sys ->
  ...
  realizability_check vars_of_term sys controllable_vars_at_0 vars_at_1
...
let compute_unviable_trace_and_core analyze in_sys param sys u_result =
  ...
  Realizability.compute_deadlocking_trace_and_conflict analyze in_sys param sys
    controllable_vars_at_0 controllable_vars_at_1 u_result

let core_desc = "conflicting constraints"

let pp_print_viable_states in_sys param fmt fp =
  if fp = Term.t_true then Format.fprintf fmt "@[<hov>true@]"
  else
    let top_node =
    ...
    let ncsv_to_ln_map =
      ...
      *)
      ...
      if SVS.is_empty call_svars then SVM.empty
      else
        (*T*)
          ...
          List.fold_left
            (fun (with_eq, call_svars) ((sv', _), rhs) ->
              try
                ...
                else (with_eq, call_svars)
              with Invalid_argument _ -> (with_eq, call_svars))
            ([], call_svars) top_node.LN.equations
        in
      ...
      | None -> (
          ...
          | Some expr ->
              ...
              use (pre expr_j) instead *)
                if has_oracle then LE.mk_of_expr (LE.step_expr expr) else expr
              in
              Format.fprintf fmt "%a"
                (LE.pp_print_lustre_expr_pvar false pvar)
                expr)
    in
    Format.fprintf fmt "@[<hov>%a@]"
      (LE.pp_print_term_as_expr_pvar false pvar)
      fp

let pp_print_realizability_result_pt analyze in_sys param sys fmt result =
  (* Update time *)
  ...
  let print_not_unknown_result tag =
    ...
    in
    Format.fprintf fmt "@[<hov>%t %s %s was proven %s after %.3fs.@]@.@." tag
  ...
  | Unknown ->
      Format.fprintf fmt
        "@[<hov>%t Could not determine whether the contract of %a is \
         realizable or not after %.3fs.@]@.@."
        Pretty.warning_tag Scope.pp_print_scope scope
        (Stat.get_float Stat.analysis_time)
  | Realizable fp ->
      ...
      if Flags.Contracts.print_viable_states () then
        Format.fprintf fmt "@{<b>Viable States@}:@,%a@.@."
          (pp_print_viable_states in_sys param)
          fp
  | Unrealizable u_res ->
      print_not_unknown_result Pretty.failure_tag;

      if Flags.Contracts.print_deadlock () || Flags.Contracts.dump_deadlock ()
      then (
        KEvent.log L_note "Computing deadlocking trace and conflict...";
        let trace, core =
          compute_unviable_trace_and_core analyze in_sys param sys u_res
        in
        let cpd = ME.loc_core_to_print_data in_sys sys core_desc None core in
        ...
        Format.fprintf fmt "@[<v>%a@]@."
          (KEvent.pp_print_trace_pt ~title:"Deadlocking trace" ~color:"red"
             (Flags.dump_cex ()) L_warn in_sys param sys None true)
          trace;
        (* Restore dump_cex value *)
        Flags.set_dump_cex dump_cex;

        Format.fprintf fmt "%a" (ME.pp_print_core_data in_sys param sys) cpd;

        Format.pp_print_flush fmt ())

let pp_print_realizability_result_json analyze in_sys param sys fmt result =
  (* Update time *)
  Stat.update_time Stat.total_time;
  Stat.update_time Stat.analysis_time;
  let pp_print_additional_info =
    ...
    | Realizable fp ->
        if Flags.Contracts.print_viable_states () then fun fmt ->
          let fp_str =
            Format.asprintf "%a" (pp_print_viable_states in_sys param) fp
            |> Str.global_replace (Str.regexp "\n") " "
            |> Str.global_replace (Str.regexp "  +") " "
          in
          Format.fprintf fmt ",@,\"viableStates\" : \"%s\"" fp_str
        else fun _ -> ()
    | Unrealizable u_res ->
        if Flags.Contracts.print_deadlock () then
          try
            ...
            fun fmt ->
              ...
              in
              Format.fprintf fmt ",@,%a,@,\"conflictingSet\" : %a"
                (KEvent.pp_print_trace_json ~object_name:"deadlockingTrace"
                   in_sys param sys None true)
                trace
                (ME.pp_print_core_data_json in_sys param sys)
                cpd
  ...
  Format.fprintf fmt
    ",@.{@[<v 1>@,\
     ...
     \"runtime\" : {\"unit\" : \"sec\", \"timeout\" : false, \"value\" : \
     %.3f},@,\
     \"result\" : \"%s\"%t@]@.}@."
    (Stat.get_float Stat.analysis_time)
    (Realizability.result_to_string result)
...
let pp_print_realizability_result_xml analyze in_sys param sys fmt result =
  ...
  let pp_print_additional_info =
    ...
    | Realizable fp ->
        if Flags.Contracts.print_viable_states () then fun fmt ->
          Format.fprintf fmt "@,<ViableStates><![CDATA[@,%a@,]]></ViableStates>"
            (pp_print_viable_states in_sys param)
            fp
        else fun _ -> ()
    | Unrealizable u_res ->
          ...
          try
            ...
            fun fmt ->
              ...
              Format.fprintf fmt "@,%a@,%a"
                (KEvent.pp_print_trace_xml ~tag:"DeadlockingTrace" in_sys param
                   sys None true)
                trace
                (ME.pp_print_core_data_xml ~tag:"ConflictingSet" in_sys param
                   sys)
                cpd
          with Realizability.Trace_or_conflict_computation_failed _ ->
            fun _ -> ()
...
let check_contract_satisfiability sys =
  let sys, const_svars = TSys.enforce_constantness_via_equations sys in
...
let pp_print_satisfiability_result_pt param fmt result =
  ...
  | Unknown ->
      Format.fprintf fmt
        "@[<hov>%t Could not determine whether the %s %s is satisfiable or not \
         after %.3fs.@]@."
        Pretty.warning_tag
        (match node_type with
        | Environment -> "Environment of"
