...
let is_index_var v =
  ...
  try Scanf.sscanf s "__index_%s" (fun _ -> true)
...
let offset_of_index expr =
  ...
  let tv =
    match
      Term.vars_of_term t |> Var.VarSet.elements |> List.filter is_index_var
    with
    | [ v ] -> Term.mk_var v
    | _ -> Var.mk_fresh_var Type.t_int |> Term.mk_var
  in
...
let sum_offsets_negative offsets =
  let neg_offset_test =
    Term.mk_lt [ Term.mk_plus offsets; Term.mk_num_of_int 0 ]
  in
  (* The simplification is disabled until it supports bitvector variables *)
  (*let neg_offset_test = Simplify.simplify_term [] neg_offset_test in*)
  Term.equal neg_offset_test Term.t_true
...
let gather_offsets_on_cycles state_var parents =
  let rec gather_up_to big_acc small_acc sv parents =
    ...
    | (sv', None) :: _ when StateVar.equal_state_vars sv' sv -> raise Exit
...
let has_cycle_path state_var path =
  try
    List.exists
      (fun offsets_cycle -> not (sum_offsets_negative offsets_cycle))
...
let rec describe_cycle node accum = function
  | (state_var, _) :: tl -> (
      ...
      (*| N.Alias (_,_)*)
      | N.Oracle ->
          describe_cycle node accum tl (* State variable from abstraction *)
      | exception Not_found -> (
          try
            ...
            describe_cycle node
              (Format.asprintf "<call to %a>" (I.pp_print_ident false)
                 call_node_name
              :: accum)
              tl
...
let add_dep_to_parents sv indgrps parents =
  if indgrps = [] then
    ...
    let offsets_inds =
      List.map
        (fun inds ->
          List.map offset_of_index inds |> Term.mk_plus
          (* The simplification is disabled until it supports bitvector variables *)
    ...
    List.fold_left
      (fun acc i ->
        let np =
          ...
          else List.map (fun path -> (sv, Some i) :: path) parents
        in
...
(* Strategy for merging dependencies on indexes of array accesses (we keep them
   all). *)
let merge_deps _ oind1 oind2 =
  match (oind1, oind2) with
...
let rec state_vars_of_bounds acc = function
  ...
  | (E.Bound e | E.Fixed e | E.Unbound e) :: l ->
      let acc =
        Term.state_vars_of_term (E.unsafe_term_of_expr e) |> SVS.union acc
      in
...
let rec node_state_var_dependencies' init output_input_deps
    ({ N.equations; N.calls } as node) accum = function
  ...
  | (state_var, parents) :: tl ->
        ...
        variable? *)
         ...
         | Some path ->
          ...
          that are not visible in the origial source *)
             let str_path =
               describe_cycle node [] ((state_var, None) :: path)
             in
             ...
             fail_no_position
               (Format.asprintf
                  ...
                  state_var (I.pp_print_ident false) node.N.name
      ...
      let children =
        ...
        |> List.fold_left
             ...
             (fun accum ((_, bnds), expr) ->
               ...
               (* (\* add bounds *\) *)
               let state_vars =
                 state_vars_of_bounds bnds |> SVS.union state_vars
               in
               ...
               SVS.fold
                 (fun sv acc ->
                   let indexes =
                     ...
                     else E.indexes_of_state_vars_in_step sv expr
                   in
              ...
              have dependencies computed *)
               ...
               |> List.fold_left
                    (fun accum i ->
                      ...
                      try
                        SVM.add (D.find i call_inputs) [] accum
                        (* Invalid map *)
                      with Not_found -> assert false)
                    SVM.empty
               ...
               List.fold_left
                 (fun children -> function
                   (* | N.CNone -> children *)
                   | N.CActivate clk | N.CRestart clk -> SVM.add clk [] children)
                 children call_cond
      ...
      if SVM.is_empty children_not_visited then
        ...
        node_state_var_dependencies' init output_input_deps node
          ((state_var, children) :: accum)
          tl
      else
        (* First get dependencies of all dependent variables *)
        node_state_var_dependencies' init output_input_deps node accum
          (SVM.fold
             (fun sv ind a ->
...
let rec order_state_vars accum seen = function
  ...
  | (h, d) :: tl ->
      ...
      if
        SVM.for_all
          (fun sv _ -> List.exists (StateVar.equal_state_vars sv) accum)
          d
          ...
          SVM.fold
            (fun sv _ a ->
              try
                ...
                List.find (fun (sv', _) -> StateVar.equal_state_vars sv sv') tl
                :: a
      ...
      SVM.fold
        (fun sv _ a ->
          match
            ...
            D.filter (fun _ sv' -> StateVar.equal_state_vars sv sv') inputs
            |> D.keys
          with
...
let state_var_dependencies' init output_input_deps
    ({ N.outputs; N.equations; N.calls } as node) roots =
...
let state_var_dependencies init output_input_deps
  ...
  let dependencies = state_var_dependencies' init output_input_deps node [] in
...
let order_equations init output_input_deps
...
let slice_all_of_node ?(keep_props = true) ?(keep_contracts = true)
    ?(keep_asserts = true)
    {
      N.name;
      N.is_extern;
...
let add_roots_of_node_call roots
    { N.call_cond; N.call_inputs; N.call_oracles; N.call_defaults } =
  ...
  let roots' =
    ...
    | Some d ->
        D.fold
          (fun _ e a -> (E.state_vars_of_expr e |> SVS.elements) @ a)
          d roots
  in
  ...
  let roots' =
    ...
    List.fold_left
      (fun roots' -> function
        (* | N.CNone -> roots' *)
        | N.CActivate c | N.CRestart c -> c :: roots')
      roots' call_cond
  in
...
let roots_of_props props =
  List.map (fun (sv, _, _, _) -> sv) props |> SVS.of_list
...
let add_roots_of_asserts asserts roots =
  List.fold_left (fun accum (_, sv) -> SVS.add sv accum) roots asserts
...
let rec slice_nodes preserve_sig init_slicing_of_node nodes accum = function
  (* All nodes are sliced and in the accumulator *)
  | [] -> accum
  (* Node is sliced to all equations *)
  | ( [],
      leaves,
      ({ N.inputs; N.outputs; N.locals } as node_sliced),
      _ (* node_unsliced *) )
    :: tl ->
      ...
      slice_nodes preserve_sig init_slicing_of_node nodes (node_sliced :: accum)
        tl
  ...
  | (state_var :: roots_tl, leaves, node_sliced, node_unsliced) :: tl
    when List.exists (StateVar.equal_state_vars state_var) leaves ->
      slice_nodes preserve_sig init_slicing_of_node nodes accum
    ...
    :: tl as l -> (
      try
        ...
        let { N.call_node_name } =
          List.find
            (function
              | { N.call_node_name; N.call_outputs } ->
      ...
      with Not_found ->
        ...
        let equations_in_coi', equations_not_in_coi', roots' =
          List.fold_left
            (fun (equations_in_coi, equations_not_in_coi, roots')
                 (((sv, _), _) as eq) ->
        ...
        let calls_in_coi', calls_not_in_coi', roots' =
          List.fold_left
            (fun (calls_in_coi, calls_not_in_coi, roots')
...
let root_and_leaves_of_impl is_top roots
    ({ N.outputs; N.contract; N.props; N.calls; N.asserts } as node) =
  (* Slice everything from node *)
  let node_sliced =
    slice_all_of_node ~keep_props:true ~keep_contracts:true node
  in
  ...
  let node_roots =
    ...
    | None ->
        ...
        *)
        roots_of_contract_ass contract |> SVS.union (roots_of_props props)
    (* Use instead of roots from properties and contracts *)
    | Some r -> roots_of_contract_ass contract |> SVS.union r)
...
(* Slice a node to its contracts, starting from contracts, stopping at
   outputs *)
let root_and_leaves_of_contracts is_top roots
    ({ N.outputs; N.calls; N.contract } as node) =
  (* Slice everything from node *)
  let node_sliced =
    slice_all_of_node ~keep_props:false ~keep_contracts:true ~keep_asserts:false
      node
  in
...
let node_is_abstract analysis { N.name } =
  [ I.string_of_ident false name ] |> Analysis.param_scope_is_abstract analysis
...
(* Return roots for slicing to contracts or implementation as
   ...
   not in the map. *)
let root_and_leaves_of_abstraction_map is_top roots abstraction_map node =
  if node_is_abstract abstraction_map node then (* Node is to be abstract *)
...
let no_slice { N.inputs; N.outputs; N.locals; N.contract; N.props } is_impl =
  let vars =
    if is_impl then
      ...
      |> SVS.union (List.concat (List.map D.values locals) |> SVS.of_list)
    else roots_of_contract ~with_sofar_var:true contract
  in
  Some vars

(* Slice nodes to abstraction or implementation as indicated in
   [abstraction_map] *)
let slice_to_abstraction ?(preserve_sig = false) reduce_to_coi analysis
    subsystem =
  let roots = if reduce_to_coi then fun _ _ -> None else no_slice in
  slice_to_abstraction' ~preserve_sig analysis roots subsystem

(* Slice nodes to abstraction or implementation as indicated in
   [abstraction_map] *)
let slice_to_abstraction_and_property ?(preserve_sig = false) analysis vars
    subsystem =
  let roots { N.contract } is_impl =
    if is_impl then Some vars
    else
