...
(** Internal representation of a Lustre node
    ...
    Nodes are normalized for easy translation into a transition system, mainly
    by introducing new variables.

    The node equations taken together become a map of state variables to
    expressions. All node calls are factored out with fresh state variables as
    inputs and outputs.

    The node signature as input and output variables as well as its local
    variables is in [inputs], [outputs] and [locals], respectively. Local
    constants are propagated and do not need to be stored. The inputs of a node
    can be extended by constant state variables in [oracles] for the initial
    value of unguarded pre operations.

    Assertions, properties to prove and contracts as assumptions and guarantees
    are lists of expressions in [asserts], [props], contracts fo into
    [global_contracts] and [mode_contracts].

    The flag [node_is_main] is set if the node has been annotated as main, it is
    not checked if more than one node or no node at all may have that
    annotation.
...
type node_call = {
  call_id : int; (* A unique numerical id *)
  call_pos : position;  (** Position of node call in input file *)
  ...
  call_inputs : StateVar.t LustreIndex.t;
      (** Variables for actual input parameters
          ...
          The keys of the index match those in the {!t.inputs} field of the
          called node. *)
  call_oracles : StateVar.t list;
      (** Variables providing non-deterministic inputs
          ...
          {!t.oracles} field of the called node. *)
  ...
  call_defaults : LustreExpr.t LustreIndex.t option;
      (** Expressions for initial return values
          ...
          This value should be [None] for node calls on the base clock, and
          [Some l] for node calls with a clock. A node call with a clock may
          only have [None] here if it occurs directly under a [merge] operator.

          If the option value is not [None], the keys of the index match those
          in the {!t.outputs} field of the called node. *)
  call_inlined : bool;  (** Whether this call was inlined or not *)
}
(** A call to a node

    Calls are uniquely identified by the position, no two calls may share the
    same position, therefore the [call_pos] must not be a dummy position. *)
...
(** Type of left hand side of equations.
    ...
    An array can be defined either only at a given index, or at all indexes,
    when the expression on the right-hand side is interpreted as a function of
    the running variable of the index. *)

type equation = equation_lhs * LustreExpr.t
(** An equation is a tuple [(eqlhs, expr)] that defines a possibly indexed state
    variable as an expression. *)

type t = {
  name : LustreIdent.t;  (** Name of the node *)
  ...
  ty_args : LustreAst.lustre_type list;
      ...
      *)
  instance : StateVar.t;
      (** Distinguished constant state variable uniquely identifying the node
          instance *)
  ...
  inputs : StateVar.t LustreIndex.t;
      (** Input streams defined in the node
          ...
          corresponding to their position in the formal parameters if there is
          more than one input parameter. If there is only one input parameter,
          the list index is omitted, the index is empty if there are no input
          parameters. *)
  oracles : StateVar.t list;
      (** Oracle inputs added to the node inputs

          Input streams added to the node to obtain non-deterministic values for
          the initial values of unguarded pre operators. The state variables are
          constant. *)
  ...
  locals : StateVar.t LustreIndex.t list;
      (** Local variables of node
          ...
          The order of the list is irrelevant, we are doing dependency analysis
          and cone of influence reduction later. *)
  equations : equation list;  (** Equations for local and output variables *)
  calls : node_call list;  (** Node calls inside the node *)
  asserts : (position * StateVar.t) list;  (** Assertions of node *)
  props :
    (StateVar.t * string * Property.prop_source * Property.prop_kind) list;
  ...
  state_var_source_map : state_var_source StateVar.StateVarMap.t;
      (** Map from a state variable to its source
          ...
          Variables that were introduced to abstract expressions do not have a
          source. *)
...
(** A Lustre node
    ...
    Every state variable occurs exactly once in {!t.inputs}, {!t.outputs}, and
    {!t.oracles}, and at most once on the left-hand side of {!t.calls}. If the
    state variable is of array type, there may be more than one occurrence of it
    in {!t.equations}, each defining the index variable at a different value
    with {!bound_or_fixed.Fixed}. If the state variable is not an array, or all
    its bounds are {!bound_or_fixed.Bound}, then it occurs at most once on the
    left-hand side of {!t.equations}. *)
...
type contract_item_type =
  ...
  | Require
  | Ensure
      (** A definition of a state variable in the initial Lustre program. For a
          given state var s, it indicates the position p of an expression e that
          defines s as well as the corresponding index i, such that s defined by
          the value of e at index i. *)
...
(** Pretty-print a node equation in Lustre format
    ...
    If the flag in the first argument is [true], print identifiers in Lustre
    syntax. *)
...
(** Pretty-prints the signature of a node in Lustre format, WITHOUT NODE KEYWORD
    AND NAME. (Used in contract generation.) *)
...
(** Return name of all nodes annotated with --%MAIN. Raise [Not_found] if no
    node has a --%MAIN annotation. If the processing mode does not support
    multiple main nodes, then it is the caller's responsibility to ensure there
    is only a single main node. *)
...
(** Return true if the node has a contract with at least one guarantee or one
    mode *)
...
(** Return a tree-like subsystem hierarchy from a flat list of nodes, where the
    name of the top node is given as first argument. *)

val nodes_of_subsystem : t SubSystem.t -> t list
(** Return list of topologically ordered list of nodes from subsystem. The top
...
(** [ordered_equations_of_node n stateful init] Returns the equations of [n],
    topologically sorted by their base (step) expression if [init] ([not init]).
*)
...
(** Returns [true] if the node is partially defined, that is, there is an output
    without an equational definition *)
...
(** Fold bottom-up over node calls together with the transition system
    ...
    [fold_node_calls_with_trans_sys l f n t] evaluates [f m s i a] for each node
    call in the node [n], including [n] itself. The list of nodes [l] must at
    least contain all sub-nodes of [n], and [n] itself, the transition system
    [t] must at least contain subsystem instances for all node calls. Both [l]
    and [t] may contain more nodes and subsystems, respectively, only the node
    calls in [n] are relevant.
...
(** Every state variable is either defined in a node, or was introduced in
    pre-processing, see {!state_var_source}.

    - [Input], [Output] or [Local] state variables correspond to input, output
      and local streams defined in a node, respectively.

    - [Oracle] state variables are additional input variables introduced to
      non-deterministivcally give a value to unguarded [pre] expressions, or to
      unconstrained streams.

    - A [Ghost] state variables are is a local variable defined in a contract.
*)
...
(** Get the definitions (with positions in the Lustre program) of a state
    variable. In the return type, the first list contains state var defs where
    the state variable is explicitly mentioned. The second list contains state
    var defs that are dependencies (the node item does not explicitly reference
    the state variable, but the state variable depends on it) *)

val add_state_var_def : ?is_dep:bool -> StateVar.t -> state_var_def -> unit
(** Add a definition (with positions in the Lustre program) for a state
    variable. is_dep should be true iff the state_var_def is a dependency (where
    the state variable is not explicitly referenced by the definition, but
...
(** Return true if the state variable should be visible to the user, false if it
    was created internally

    Return [true] if the source of the state variable is either [Input],
    [Output], or [Local], and [false] otherwise. *)

val node_is_visible : t -> bool
(** Return true if the node should be visible to the user, false if it was
    created internally. *)
