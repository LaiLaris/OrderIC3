...
val add_ty_args_node :
  tc_context -> HString.t -> LA.lustre_type list -> tc_context
(** Add node/function type arguments into the typing context *)

val add_ty_vars_contract :
...
val traverse_group_expr_list :
  (int -> LA.expr -> 'a) -> tc_context -> int -> LA.expr list -> 'a
(** Traverse a group expr list *)

val is_type_num : tc_context -> LA.lustre_type -> (bool, HString.t) result
(** returns [true] if the type is a number type i.e. Int, Real, IntRange, or
    Machine Integer *)
...
val is_type_real_or_int :
  tc_context -> LA.lustre_type -> (bool, HString.t) result
(** returns [true] if the type is a real or integer type, i.e, Real, Int, or
    IntRange *)

val is_type_int_or_machine_int :
  tc_context -> LA.lustre_type -> (bool, HString.t) result
(** returns [true] if the type is an integer type or machine int, i.e. Int,
    IntRange, or Machine Integer *)

val is_type_unsigned_machine_int :
  tc_context -> LA.lustre_type -> (bool, HString.t) result
(** returns [true] if the type is an unsigned machine int. i.e. UInt, UInt32
    etc. *)
...
val is_machine_type_of_associated_width :
  tc_context -> LA.lustre_type * LA.lustre_type -> (bool, HString.t) result
(** returns [true] if the first component of the type is of the same width as
    the second component. eg. Int8 and UInt8 returns [true] but Int16 and UInt8
    return [false] *)

val type_contains_subrange : tc_context -> LA.lustre_type -> bool
(** Returns true if the lustre type expression contains an IntRange or if it is
    an IntRange *)

val type_contains_enum_or_subrange : tc_context -> LA.lustre_type -> bool
(** Returns true if the lustre type expression contains an EnumType/IntRange or
    if it is an EnumType/IntRange *)

val type_contains_ref : tc_context -> LA.lustre_type -> bool
(** Returns true if the lustre type expression contains a RefinementType or if
    it is an RefinementType *)
...
(** Returns true if the lustre type expression contains an abstract type
    (including polymorphic type variable) or if it is an abstract type *)
...
(** [ty_vars_of_type ctx node_name e] returns all type variable identifiers that
    appear in the expression [e] *)

val ty_vars_of_type : tc_context -> LA.index -> LA.lustre_type -> SI.t
(** [ty_vars_of_type ctx node_name ty] returns all type variable identifiers
    that appear in the type [ty] *)
