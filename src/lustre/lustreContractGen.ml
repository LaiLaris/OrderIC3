...
(** Helper functions to analyze invariants and decide whether they qualify as
    ...
    TERM CONVENTION: one-state terms have offset [0], two-state terms have
    offset [-1,0].*)
module Contract = struct
  type dep = {
    node : Node.t;  (** Original node, for dependency tracing. *)
    mutable cache : (bool * SvSet.t) option term_map;
        (** Dependency tracing cache. Boolean indicates whether term can be
            traced to current version of the outputs ([None] if no svar). *)
    ass : term_set;  (** Assumptions. *)
    gua : term_set;  (** Guarantees. *)
    modes : term_set term_map;  (** Modes: maps requires to lists of ensures. *)
  }
  (** Contract building and dependency tracing context. *)

  (** Returns [None] if term contains an oracle. Otherwise, returns true iff
      term only contains input variables, along with the set of svars in the
      term. First parameter yields the source of a state variable of the node.
  ...
  let add_inv dep term =
    ...
    | None ->
        ( (* KEvent.log L_info "discarding %a" Term.pp_print_term term ; *)
          dep,
          SvSet.empty )
    | Some (true, locals) -> ({ dep with ass = TSet.add term dep.ass }, locals)
    | Some (false, locals) -> ({ dep with gua = TSet.add term dep.gua }, locals)
  ...
  let add_inv dep inv =
    ...
    | Tree.App (sym, [ lhs; rhs ]) when Symbol.node_of_symbol sym = `IMPLIES ->
        add_impl_inv dep lhs rhs
    | _ -> add_inv dep inv
  ...
  (** Splits some invariants into assumptions, guarantees, and modes. Second
      parameter is normal invariants, then invariant implications.
  ...
  let fmt_mode (two_state, count) f fmt (req, ens) =
    TSet.elements ens |> List.map f
    |> Format.fprintf fmt
         "mode mode_%d (@   @[<v>require @[<hov 2>%a@] ;@ %a@]@ ) ;@ " count
         (fmt_lus two_state) (f req)
         (fmt_list (fmt_ens two_state f) "@ ")
  ...
  let fmt (two_state, count) f fmt { ass; gua; modes } =
    ...
    TSet.elements ass
    |> Format.fprintf fmt "%a" (fmt_list (fmt_ass two_state f) "@ ");

    if ass_cnt > 0 && gua_cnt + mod_cnt > 0 then Format.fprintf fmt "@ @ ";

    TSet.elements gua
    |> Format.fprintf fmt "%a" (fmt_list (fmt_gua two_state f) "@ ");

    if gua_cnt > 0 && mod_cnt > 0 then Format.fprintf fmt "@ @ ";
...
let fmt_sys_name fmt sys = scope_of sys |> Scope.pp_print_scope fmt

(** Name of a trans sys as a string. *)
let sys_name sys = scope_of sys |> Scope.to_string

let get_node_of_sys in_sys sys =
  let scope = scope_of sys in
  let _, node_of_scope = InputSystem.contract_gen_param in_sys scope in
  try scope |> node_of_scope
  with Not_found ->
    Format.asprintf "unknown system %a" fmt_sys_name sys |> failwith

(** Ghost instance carries the info to identify and generate a ghost variable
    that is necessary for the specification of a node, and that represents a
    variable defined in that node, or a variable defined in a called subnode. *)
module GhostInstance = struct
  ...
  let get_name (context, svar) =
    ...
    let pp_print_svar_lustre_name ppf (n, s, c) =
      if s = [] then Format.fprintf ppf "%s%a" n pp_print_cxt c
      else
  ...
  let get_name_st (cxt_stack, svar) =
    match cxt_stack with cxt :: _ -> get_name (cxt, svar) | [] -> assert false
  ...
  let mk_ghost_var_st cxt_stack sv =
    match cxt_stack with cxt :: _ -> mk_ghost_var cxt sv | [] -> assert false

  module IdSet = Set.Make (String)

  (** Returns a list of pending ghost instances and a set of the ghost variables
      included in the pending list. The list is created from the given context
      and state variable set. *)
  ...
  (** Add the ghost instance to the pending list if it is not known yet, and
      returns the updated list and set *)
  ...
  (** Creates a list of equations defining all necessary ghost variables from a
  ...
  let create_ghost_definitions in_sys cxt svar_set =
    ...
    let process_equation pending known cxt_stack eq =
      let eq_svars = Expr.state_vars_of_expr (snd eq) in
      let pending, known =
        SvSet.fold
      ...
      let rec find_var_in_instance v = function
        ...
        | inst :: tail -> (
            try
              ...
              Some (inst, sub_svar)
            with Not_found -> find_var_in_instance v tail)
      in
    ...
    let create_argument_eq pending known (lhs_cxt, lhs_svar)
        (rhs_cxt_stack, rhs_svar) =
      ...
      let pending, known =
        add_to_pending pending known (rhs_cxt_stack, rhs_svar)
      in
      (ghost_eq, pending, known)
    in
                        ...
                        try
                          ...
                          let ghost_eq, pending, known =
                            if is_top_node_input svar_arg stack then
                              ( create_input_eq (cxt, svar) svar_arg,
                                pending,
                                known )
                            else
                              create_argument_eq pending known (cxt, svar)
                                svar_arg_inst
                          in
                          ghost_eq :: loop known pending
...
let fmt_ghost_def fmt ((var, bounds), expr) =
  Format.fprintf fmt "@[<hv 2>var %a%a: %a = %a ;@]"
    (Expr.pp_print_lustre_var false)
    var
    (pp_print_listi
       (fun ppf i -> function
       ...
       "")
    bounds
    (Expr.pp_print_lustre_type true)
    (SVar.type_of_state_var var)
    (Expr.pp_print_lustre_expr true)
    expr

let generate_contract_for in_sys sys path invs name =
  let node = get_node_of_sys in_sys sys in
  let contract, locals = TSet.of_list invs |> Contract.build node in
  ...
  Format.fprintf fmt
    "(*@[<v>@ The contract below was generated automatically by the Kind 2@ \
     model-checker.@ @ http://kind2-mc.github.io/kind2/@]@ *)@.@.@.@.";

  Format.fprintf fmt "(* Contract for node %s. *)@.contract %s %a@.let@[<v 2>"
