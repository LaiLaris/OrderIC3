...
type t = {
  node_args :
    (HString.t (* abstracted variable name *)
    * bool (* whether the variable is constant *)
    * LustreAst.lustre_type
    * LustreAst.expr)
    list;
  array_constructors :
    (LustreAst.lustre_type * LustreAst.expr * LustreAst.expr) StringMap.t;
  locals : LustreAst.lustre_type StringMap.t;
  ...
  calls :
    (Lib.position (* node call position *)
  ...
  equations :
    ...
    * LustreAst.expr
    * source option)
    (* Record the source of the equation if generated before normalization step *)
    list;
...
let union_keys key id1 id2 =
  match (key, id1, id2) with
  | _, None, None -> None
...
let union ids1 ids2 =
  {
    ...
    array_constructors =
      StringMap.merge union_keys ids1.array_constructors ids2.array_constructors;
    ...
    contract_calls =
      StringMap.merge union_keys ids1.contract_calls ids2.contract_calls;
    subrange_constraints = ids1.subrange_constraints @ ids2.subrange_constraints;
    refinement_type_constraints =
      ids1.refinement_type_constraints @ ids2.refinement_type_constraints;
    expanded_variables =
      StringSet.union ids1.expanded_variables ids2.expanded_variables;
    equations = ids1.equations @ ids2.equations;
    nonvacuity_props =
      StringSet.union ids1.nonvacuity_props ids2.nonvacuity_props;
    array_literal_vars =
      StringSet.union ids1.array_literal_vars ids2.array_literal_vars;
    history_vars =
      StringMap.union
        (fun _ h_sv _ -> Some h_sv)
        ids1.history_vars ids2.history_vars;
  }

(* Same as union_keys, but we don't assume that identifiers are unique *)
let union_keys2 key id1 id2 =
  match (key, id1, id2) with
  | _, None, None -> None
  | _, Some v, None -> Some v
  | _, None, Some v -> Some v
