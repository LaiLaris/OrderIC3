...
let error_message error =
  match error with
  | Unknown s -> s
  | IdentifierRedeclared i ->
      "Identifier '" ^ HString.string_of_hstring i ^ "' is already declared"
  | WidthLengthsUnequal (l, r) ->
      "Width lengths are not equal for expressions: '"
      ^ Lib.string_of_t LA.pp_print_expr l
      ^ "' and '"
      ^ Lib.string_of_t LA.pp_print_expr r
      ^ "'"
  | EquationWidthsUnequal -> "Width lengths of equation are not equal"
  | ContractDependencyOnCurrentOutput ids ->
      "Contract assumption or import argument cannot depend on "
      ^ "current values of output parameters but found: "
      ^ Lib.string_of_t
          (Lib.pp_print_list LA.pp_print_ident ", ")
          (SI.elements ids)
  | CyclicDependency ids ->
      "Cyclic dependency detected in definition of identifiers: "
      ^ Lib.string_of_t (Lib.pp_print_list LA.pp_print_ident " -> ") ids

type error = [ `LustreAstDependenciesError of Lib.position * error_kind ]
...
(** The node summary contains the positions of the input streams of a node that
    are used in their current value for each output stream.

    Eg. Assume a node N(a1, a2, a3, a4) returns (r1, r2, r3) where r1 does not
    depend on any input stream's current value, r2 depends on the current value
    of a4 and a3 and r3 depends on the current value of a1.

    The node summary's entry that is generated for N will be
    [0:->[], 1:-> [2, 3], 2:->[1]].

    The reason as to why we store it using indices is because we would
    substitute the indices with the actual call parameters during the
    circularity analysis for equations.

    For imported nodes and functions we make the conservative assumption that
    each output stream is dependent on the current values of all the arguments.

    We generate the node summary entry by doing a rechablility analysis for each
    of the output streams equations. *)

type contract_summary = LA.ident list IMap.t
(** The contract summary contains the symbols that are exported from a contract
    i.e. mode names and ghost constants and variables. which can be referenced
    after the contract is imported *)

type dependency_analysis_data = {
  graph_data : G.t; (* How are the ids related  *)
  graph_data2 : G.t;
      (* Relation of ids where imported node outputs depend on all inputs *)
  ...
  nsummary2 : node_summary;
      (* Node summaries where imported node outputs depend on all inputs *)
}
...
let pp_print_node_summary ppf m =
  let pp_print_val ppf m =
    Lib.pp_print_list
      (fun ppf (i, b) ->
        Format.fprintf ppf "(%a:%a)" Format.pp_print_int i
          (Lib.pp_print_list Format.pp_print_int ", ")
          b)
      ", " ppf (IntMap.bindings m)
  in
  Lib.pp_print_list
    (fun ppf (i, b) ->
      Format.fprintf ppf "(%a->%a)" LA.pp_print_ident i pp_print_val b)
    ", " ppf (IMap.bindings m)
...
let pp_print_contract_summary ppf m =
  Lib.pp_print_list
    (fun ppf (i, b) ->
      Format.fprintf ppf "(%a :-> %a)" LA.pp_print_ident i
        (Lib.pp_print_list LA.pp_print_ident ", ")
        b)
    ", " ppf (IMap.bindings m)
...
let pp_print_analysis_data ppf ad =
  Format.fprintf ppf
    ("Node Summary: %a\n" ^^ "Contract Summary: %a\n")
    pp_print_node_summary ad.nsummary pp_print_contract_summary ad.csummary
...
let add_pos : id_pos_map -> LA.ident -> Lib.position -> id_pos_map =
 fun m i p ->
  IMap.update i (function None -> Some [ p ] | Some ps -> Some (p :: ps)) m

let union_pos : id_pos_map -> id_pos_map -> id_pos_map =
 fun m1 m2 -> IMap.union (fun _ v1 v2 -> Some (v1 @ v2)) m1 m2

let singleton_pos : LA.ident -> Lib.position -> id_pos_map =
 fun i p -> IMap.singleton i [ p ]

let find_id_pos : id_pos_map -> LA.ident -> Lib.position option =
 fun m i ->
...
let singleton_dependency_analysis_data :
    HString.t -> LA.ident -> Lib.position -> dependency_analysis_data =
 fun prefix i p ->
  {
...
let union_dependency_analysis_data :
...
let map_g_pos :
 ...
 fun f ad ->
  let pos_info' =
    List.fold_left
      (fun m (k, v) -> IMap.add k v m)
      IMap.empty
      (List.map (fun (k, v) -> (f k, v)) (IMap.bindings ad.id_pos_data))
  in
  {
    ad with
    graph_data = G.map f ad.graph_data;
...
let rec mk_graph_type : LA.lustre_type -> dependency_analysis_data = function
  ...
  | UserType (pos, ty_args, i) ->
      ...
      List.fold_left union_dependency_analysis_data usr_g
        (List.map mk_graph_type ty_args)
  | AbstractType (pos, i) -> singleton_dependency_analysis_data ty_prefix i pos
  | TupleType (_, tys) ->
      List.fold_left union_dependency_analysis_data
        empty_dependency_analysis_data
        (List.map (fun t -> mk_graph_type t) tys)
  | GroupType (_, tys) ->
      List.fold_left union_dependency_analysis_data
        empty_dependency_analysis_data
        (List.map (fun t -> mk_graph_type t) tys)
  | RecordType (_, _, ty_ids) ->
      List.fold_left union_dependency_analysis_data
        empty_dependency_analysis_data
        (List.map (fun (_, _, t) -> mk_graph_type t) ty_ids)
  | ArrayType (_, (ty, e)) ->
      union_dependency_analysis_data (mk_graph_type ty) (mk_graph_expr e)
  | History _ -> empty_dependency_analysis_data
  | TArr (_, aty, rty) ->
      union_dependency_analysis_data (mk_graph_type aty) (mk_graph_type rty)
...
(** This graph is useful for analyzing top level constant and type declarations
*)
and mk_graph_expr ?(only_modes = false) = function
  | LA.Ident (pos, i) ->
  ...
  | LA.ModeRef (pos, ids) ->
      if List.length ids > 1 then
        singleton_dependency_analysis_data empty_hs
          (List.fold_left HString.concat2 contract_prefix (Lib.drop_last ids))
          pos
      else singleton_dependency_analysis_data mode_prefix (List.hd ids) pos
  | LA.Call (_, _, _, es) ->
      List.fold_left union_dependency_analysis_data
        empty_dependency_analysis_data
        (List.map (mk_graph_expr ~only_modes) es)
...
let mk_graph_const_decl : LA.const_decl -> dependency_analysis_data = function
  | LA.FreeConst (pos, i, ty) ->
      connect_g_pos (mk_graph_type ty) (HString.concat2 const_prefix i) pos
  | LA.UntypedConst (pos, i, e) ->
      connect_g_pos (mk_graph_expr e) (HString.concat2 const_prefix i) pos
  | LA.TypedConst (pos, i, e, ty) ->
      connect_g_pos
        (union_dependency_analysis_data (mk_graph_expr e) (mk_graph_type ty))
        (HString.concat2 const_prefix i)
        pos

let mk_graph_type_decl : LA.type_decl -> dependency_analysis_data = function
  | FreeType (pos, i) -> singleton_dependency_analysis_data ty_prefix i pos
  | AliasType (pos, i, _, ty) ->
      connect_g_pos (mk_graph_type ty) (HString.concat2 ty_prefix i) pos
...
let rec get_node_call_from_expr : LA.expr -> (LA.ident * Lib.position) list =
  ...
  | RecordProject (_, e, _) | TupleProject (_, e, _) ->
  ...
  | LA.Condact (pos, e1, e2, i, e3, e4) ->
      ((HString.concat2 node_prefix i, pos) :: get_node_call_from_expr e1)
...
let rec extract_node_calls_type :
    LA.lustre_type -> (LA.ident * Lib.position) list = function
  | RefinementType (_, (_, _, ty), e) ->
      extract_node_calls_type ty @ get_node_call_from_expr e
  ...
  | RecordType (_, _, tis) ->
      List.map (fun (_, _, ty) -> extract_node_calls_type ty) tis
      |> List.flatten
  | Int _ | Int8 _ | Int16 _ | Int32 _ | Int64 _ | UInt8 _ | UInt16 _ | UInt32 _
  | UInt64 _ | Bool _ | Real _ | IntRange _ | UserType _ | AbstractType _
  | EnumType _ | History _ ->
      []

(** This builds a graph with all the node call dependencies from the equations
    of the contract *)
let mk_graph_contract_node_eqn :
    HString.t -> LA.contract_node_equation -> dependency_analysis_data =
 fun node_name -> function
  ...
  | LA.ContractCall (pos, i, _, es, _) ->
      union_dependency_analysis_data
        ...
        (List.fold_left union_dependency_analysis_data
           empty_dependency_analysis_data
           (List.map
              (fun (i, p) -> singleton_dependency_analysis_data node_prefix i p)
              (List.flatten (List.map get_node_call_from_expr es))))
  | LA.Assume (_, _, _, e) ->
      List.fold_left union_dependency_analysis_data
        empty_dependency_analysis_data
  ...
  | LA.Mode (_, _, reqs, ensures) ->
      let calls_ps =
        List.flatten
          (List.map
             (fun (_, _, e) -> get_node_call_from_expr e)
             (reqs @ ensures))
      in
      let sub_graphs =
        List.map
      ...
      in
      List.fold_left union_dependency_analysis_data
        empty_dependency_analysis_data sub_graphs
  | LA.GhostConst c -> (
      match c with
      ...
      | UntypedConst (_, _, e) ->
          List.fold_left union_dependency_analysis_data
            ...
            (List.map
               (fun (i, p) ->
                 singleton_dependency_analysis_data node_prefix i p)
               (get_node_call_from_expr e))
          ...
          List.fold_left
            (fun g (nr, p) ->
              union_dependency_analysis_data g
                (connect_g_pos
                   (singleton_dependency_analysis_data node_prefix nr p)
                   node_name pos))
            ad node_refs)
...
(** This builds a graph with all the node call dependencies from the equations
    of the contract *)
let mk_graph_contract_decl :
    Lib.position -> LA.contract_node_decl -> dependency_analysis_data =
 fun pos (i, _, ips, ops, (_, c)) ->
  let i = HString.concat2 contract_prefix i in
  let node_refs =
    List.map (fun (_, _, ty, _, _) -> extract_node_calls_type ty) ips
  in
  let node_refs =
    node_refs @ List.map (fun (_, _, ty, _) -> extract_node_calls_type ty) ops
    |> List.flatten
  in
  let ad =
    connect_g_pos
...
let rec extract_node_calls_item : LA.node_item -> (LA.ident * Lib.position) list
  ...
  | IfBlock (_, e, l1, l2) ->
      get_node_call_from_expr e @ extract_node_calls l1 @ extract_node_calls l2
...
(** Builds a dependency graph between the node in question to node calls * seen
    in the definition of the node and the inlined contract *)
let mk_graph_node_decl :
    Lib.position -> LA.node_decl -> dependency_analysis_data =
...
let check_and_add :
    ...
    LA.ident ->
    'a ->
    ('a IMap.t, [> error ]) result =
 fun m pos prefix i tyd ->
  let i' = HString.concat2 prefix i in
  if IMap.mem i' m then graph_error pos (IdentifierRedeclared i)
  else R.ok (add_decl m i' tyd)

(** builds an id :-> decl map *)
let rec mk_decl_map :
    LA.declaration option IMap.t ->
    LA.t ->
    (LA.declaration option IMap.t, [> error ]) result =
 fun m -> function
  | [] -> R.ok m
...
let mk_graph_decls : LA.t -> dependency_analysis_data =
  let mk_graph : LA.declaration -> dependency_analysis_data = function
    ...
    | ContractNodeDecl ({ LA.start_pos = pos }, cdecl) ->
        mk_graph_contract_decl pos cdecl
    | NodeParamInst _ -> Lib.todo __LOC__
  in
  fun decls ->
    List.fold_left union_dependency_analysis_data empty_dependency_analysis_data
      (List.map mk_graph decls)

(** Given a list of ids, finds the associated payload from the playload map. It
    ignores ids that it cannot find as they may be globals. *)
let extract_decls :
    'a option IMap.t -> LA.ident list -> ('a list, [> error ]) result =
 fun decl_map ids ->
  R.ok
    (List.concat
       (List.map
...
let split_contract_equations : LA.contract -> LA.contract * LA.contract =
 fun (p, eqns) ->
  let split_eqns :
      LA.contract * LA.contract ->
      LA.contract_node_equation ->
      LA.contract * LA.contract =
   fun ((p1, ps), (p2, qs)) ->
    fun e ->
     match e with
     | LA.GhostConst _ | LA.GhostVars _ | LA.ContractCall _ | LA.Mode _ ->
         ((p1, e :: ps), (p2, qs))
     | LA.Guarantee _ | LA.Assume _ | LA.AssumptionVars _ ->
         ((p1, ps), (p2, e :: qs))
  in
...
(** get all the variables and flatten node calls using the node summary for an
    expression *)
...
let rec mk_contract_eqn_map :
    LA.contract_node_equation option IMap.t ->
    LA.contract_node_equation list ->
    (LA.contract_node_equation option IMap.t, [> error ]) result =
 fun m -> function
  | [] -> R.ok m
...
(** This graph is useful for analyzing equations assuming that the
    nodes/contract call recursive calling has been resolved already. The
    generated graph would be useful only if the pre's are abstracted out first
    and then passed into this function using [LH.abstract_pre_subexpressions] *)
let rec mk_graph_expr2 :
    node_summary ->
    LA.expr ->
    (dependency_analysis_data list, [> error ]) result =
 fun m -> function
  | LA.Ident (pos, i) ->
      R.ok [ singleton_dependency_analysis_data empty_hs i pos ]
  | LA.ModeRef (pos, ids) ->
      R.ok
        [
          singleton_dependency_analysis_data mode_prefix
            (List.nth ids (List.length ids - 1))
            pos;
        ]
  | LA.Const _ -> R.ok [ empty_dependency_analysis_data ]
  | LA.RecordExpr (pos, i, _, ty_ids) ->
      R.seq (List.map (fun ty_id -> mk_graph_expr2 m (snd ty_id)) ty_ids)
      >>= fun gs ->
      R.ok
        [
          List.fold_left union_dependency_analysis_data
  ...
  | LA.UnaryOp (_, _, e) | LA.ConvOp (_, _, e) -> mk_graph_expr2 m e
  ...
  | LA.TernaryOp (p, _, e1, e2, e3) ->
      ...
      let g1 =
        List.fold_left union_dependency_analysis_data
          empty_dependency_analysis_data g1
      in
      mk_graph_expr2 m e2 >>= fun g2 ->
      ...
      if List.length g3 != List.length g2 then (
        Debug.parse
          ("e2: %a" ^^ "\ne3: %a" ^^ "\ng2 length %a: %a"
         ^^ "\ng3 length %a: %a")
          LA.pp_print_expr e2 LA.pp_print_expr e3 Format.pp_print_int
          (List.length g2)
          (Lib.pp_print_list G.pp_print_graph ", ")
          (List.map (fun g -> g.graph_data) g2)
          Format.pp_print_int (List.length g3)
          (Lib.pp_print_list G.pp_print_graph ", ")
          (List.map (fun g -> g.graph_data) g3);
        graph_error p (WidthLengthsUnequal (e2, e3)))
      else
        R.ok
          (List.map
             (fun g -> union_dependency_analysis_data g1 g)
             (List.map2 (fun g g' -> union_dependency_analysis_data g g') g2 g3))
  | LA.RecordProject (_, e, _) | LA.TupleProject (_, e, _) -> mk_graph_expr2 m e
  | LA.ArrayConstr (_, e1, e2) ->
      mk_graph_expr2 m e1 >>= fun g1 ->
      mk_graph_expr2 m e2 >>= fun g2 ->
      R.ok
  ...
  | LA.GroupExpr (_, _, es) ->
      ...
      R.ok
        [
          List.fold_left union_dependency_analysis_data
            empty_dependency_analysis_data (List.concat gs);
        ]
  | LA.AnyOp _ -> assert false (* Already desugared in lustreDesugarAnyOps *)
  ...
  | LA.Condact (pos, _, _, n, e1s, e2s) ->
      ...
      if List.length gs != List.length default_gs then
        graph_error pos
          (WidthLengthsUnequal
             (node_call, LA.GroupExpr (Lib.dummy_pos, LA.ExprList, e2s)))
  ...
  | LA.Merge (pos, clk_id, cs) -> (
      ...
      try
        let gs' =
          List.tl gs
          |> List.fold_left
               (fun acc gs' -> List.map2 union_dependency_analysis_data acc gs')
               (List.hd gs)
        in
  ...
  | LA.RestartEvery (p, n, es, clk_exp) ->
      ...
      let clk_g =
        List.fold_left union_dependency_analysis_data
          empty_dependency_analysis_data clk_g
      in
  ...
  | LA.Pre (_, e) ->
      ...
      R.ok
        (List.map
           (map_g_pos (fun v -> HString.concat2 v (HString.mk_hstring "$p")))
           g)
  | LA.Arrow (p, e1, e2) ->
      mk_graph_expr2 m e1 >>= fun g1 ->
      mk_graph_expr2 m e2 >>= fun g2 ->
      if List.length g1 != List.length g2 then (
        Debug.parse
         ...
         ^^ " RHS: graphs %a\n")
          LA.pp_print_expr e1
          (Lib.pp_print_list G.pp_print_graph ",")
          (List.map (fun g -> g.graph_data) g1)
          LA.pp_print_expr e2
          (Lib.pp_print_list G.pp_print_graph ",")
          (List.map (fun g -> g.graph_data) g2);
  ...
  | LA.Call (_, _, i, es) -> (
      ...
      | Some summary ->
          ...
          R.ok
            (List.map
               (fun (_, b) ->
                 if List.length b = 0 then empty_dependency_analysis_data
                 else
                   List.fold_left union_dependency_analysis_data
                     empty_dependency_analysis_data
                     (List.map
  ...
  | e ->
      Lib.todo
        (__LOC__ ^ " "
        ^ Lib.string_of_t Lib.pp_print_position (LH.pos_of_expr e))

(** all the variables whose current value is used in the expression *)
let expression_current_streams :
    dependency_analysis_data -> LA.expr -> (LA.ident list, [> error ]) result =
 fun ad e ->
  let vars =
    vars_with_flattened_nodes ad.nsummary2 0 (LH.abstract_pre_subexpressions e)
  in
  ...
  let rechable_vs =
    List.concat
      (List.map
         (fun v ->
           G.to_vertex_list (G.memoized_reachable memo ad.graph_data2 v))
         vs)
  in
  Debug.parse "Current Stream Usage for %a: %a"
    (Lib.pp_print_list G.pp_print_vertex ", ")
    vs
    (Lib.pp_print_list G.pp_print_vertex ", ")
    rechable_vs;
  R.ok rechable_vs
...
let mk_graph_contract_node_eqn2 :
    dependency_analysis_data ->
    LA.contract_node_equation ->
 ...
 fun ad -> function
      ...
      R.ok
        (connect_g_pos
           (List.fold_left union_dependency_analysis_data ad
              (List.map
                 (fun e -> mk_graph_expr (LH.abstract_pre_subexpressions e))
                 es))
           (HString.concat2 contract_prefix i)
           pos)
  | LA.Assume (_, _, _, e) | LA.Guarantee (_, _, _, e) ->
      R.ok
        (union_dependency_analysis_data ad
           (mk_graph_expr (LH.abstract_pre_subexpressions e)))
  | LA.Mode (pos, i, reqs, ensures) ->
      let mgs =
        List.fold_left union_dependency_analysis_data ad
          (List.map
             (fun (_, _, e) ->
               mk_graph_expr ~only_modes:true (LH.abstract_pre_subexpressions e))
             (reqs @ ensures))
      in
  ...
  | LA.GhostConst c -> (
      match c with
      | FreeConst _ -> R.ok ad
      | UntypedConst (pos, i, e) | TypedConst (pos, i, e, _) ->
          let union g v =
            union_dependency_analysis_data g
              (singleton_dependency_analysis_data empty_hs v pos)
          in
          let vars =
            vars_with_flattened_nodes ad.nsummary 0
              (LH.abstract_pre_subexpressions e)
          in
          let effective_vars = LA.SI.elements vars in
          let vars2 =
            vars_with_flattened_nodes ad.nsummary2 0
              (LH.abstract_pre_subexpressions e)
          in
          let effective_vars2 = LA.SI.elements vars2 in
          let ad =
            connect_g_pos_biased false
              (List.fold_left union ad effective_vars)
              i pos
          in
          R.ok
            (connect_g_pos_biased true
               (List.fold_left union ad effective_vars2)
               i pos))
...
let check_eqn_no_current_vals :
    LA.SI.t -> dependency_analysis_data -> LA.expr -> (unit, [> error ]) result
    =
 fun node_out_streams ad e ->
  let* s = expression_current_streams ad e in
  let* assume_vars_out_streams =
    R.ok (SI.inter node_out_streams (LA.SI.of_list s))
  in
  Debug.parse "node_params: %a non pre vars of e: %a"
    (Lib.pp_print_list LA.pp_print_ident ", ")
    (SI.elements node_out_streams)
    (Lib.pp_print_list LA.pp_print_ident ", ")
    (SI.elements
       (LH.vars_without_node_call_ids (LH.abstract_pre_subexpressions e)));
  R.guard_with
    (R.ok (SI.is_empty assume_vars_out_streams))
...
(** Check if any of the out stream vars of the node is being used at its current
    value in assumption *)
let validate_contract_equation :
    LA.SI.t ->
    dependency_analysis_data ->
    LA.contract_node_equation ->
    (unit, [> error ]) result =
 fun ids ad -> function
  | LA.Assume (_, _, _, e) -> check_eqn_no_current_vals ids ad e
...
let check_no_input_output_duplicate ips ops =
  ...
  let* map = R.seq_chain (fun m (p, i, _, _, _) -> add m p i) IMap.empty ips in
  R.seq_chain (fun m (p, i, _, _) -> add m p i) map ops

(** This function does two things: 1. Sort the contract equations according to
    their dependencies
    - The assumptions and guarantees are added to the bottom of the list as no
      other contract equation can depend on it. 2. Ensures the assumptions do
      not use the current value of the output streams. *)
let sort_and_check_contract_eqns :
    dependency_analysis_data ->
 ...
 fun ad ((i, params, ips, ops, (p, contract)) as decl) ->
  ...
  let* sorted_ids =
    try R.ok (G.topological_sort ad'.graph_data)
    with G.CyclicGraphException ids -> (
      match find_id_pos ad'.id_pos_data (List.hd ids) with
      | None ->
          assert
            false (* LustreSyntaxChecks should guarantee this is impossible *)
      | Some p -> graph_error p (CyclicDependency ids))
  in
  let equational_vars =
    List.filter (fun i -> not (SI.mem i ids_to_skip)) sorted_ids
  in
  ...
  Debug.parse "sorted contract equations for contract %a %a" LA.pp_print_ident i
    (Lib.pp_print_list LA.pp_print_contract_item "\n")
    contract';
  R.seq_
    (List.map (validate_contract_equation (SI.of_list op_ids) ad') contract)
  >> R.ok (i, params, ips, ops, (p, contract' @ rest))

(** Accepts a function to generate a declaration map, a function to generate the
    graph of the declarations, runs a topological sort on the ids extracted from
    the map and then returns the sorted declarations (or an error if circular
 ...
 fun decls ->
  ...
  let* sorted_ids =
    ...
    with G.CyclicGraphException ids -> (
      match find_id_pos ad.id_pos_data (List.hd ids) with
      | None ->
          assert false (* SyntaxChecks should guarantee this is impossible *)
      | Some p -> graph_error p (CyclicDependency ids))
  in
  let dependency_sorted_ids = sorted_ids in
  let is_contract_node node_name =
    try String.equal (HString.sub node_name 0 9) "contract " with _ -> false
  in
  let toplevel_nodes =
    ad.graph_data |> G.non_target_vertices |> G.to_vertex_list
    |> List.filter (fun s -> not (is_contract_node s))
  in
  Debug.parse "sorted ids: %a"
    (Lib.pp_print_list LA.pp_print_ident ",")
    dependency_sorted_ids;
...
let summarize_ip_vars : LA.ident list -> SI.t -> int list =
 fun ips critial_ips ->
  (List.fold_left
     (fun (acc, nums) i ->
       if SI.mem i critial_ips then (nums :: acc, nums + 1) else (acc, nums + 1))
     ([], 0))
    ips
  |> fst
...
(** Computes the node call summary of the node to the input stream of the node.
    ...
    For imported nodes and imported functions we assume that output streams
    depend on every input stream. *)
let mk_node_summary : bool -> node_summary -> LA.node_decl -> node_summary =
 fun connect_imported s (i, imported, _, _, ips, ops, _, items, _) ->
  if not imported then (
    ...
    let node_equations =
      List.concat (List.map LH.extract_node_equation items)
    in
    let process_one_eqn =
     fun (LA.StructDef (_, lhs), e) ->
      let ms =
        List.mapi
          (fun idx item ->
    ...
    Debug.parse "Node equation dependency map for node %a {\n %a \n}"
      LA.pp_print_ident i
      (Lib.pp_print_list
         (Lib.pp_print_pair LA.pp_print_ident
            (Lib.pp_print_list LA.pp_print_ident ", ")
            "->")
         "\n")
    ...
    let mk_g =
     fun (lhs, vars) ->
      G.connect (List.fold_left G.union G.empty (List.map G.singleton vars)) lhs
    in
    let g =
      List.fold_left G.union G.empty
        (List.map mk_g (IMap.bindings node_equation_dependency_map))
    in

    Debug.parse "Node equation graph: %a" G.pp_print_graph g;
    let memo = ref G.VMap.empty in
    let vars_op_depends_on =
      List.map
        (fun i -> G.to_vertex_list (G.memoized_reachable memo g i))
        op_vars
    in
    let critical_ips =
      List.map
        (fun vs ->
          SI.inter (SI.of_list vs) (SI.of_list ip_vars)
...
let get_contract_exports :
    contract_summary -> LA.contract_node_equation -> LA.ident list =
 fun m -> function
  ...
  | LA.ContractCall (_, cc, _, _, _) -> (
      ...
      | Some ids ->
          List.map
            (fun i -> HString.concat (HString.mk_hstring "::") [ cc; i ])
            ids
      | None ->
          failwith
            ("Undeclared contract "
...
*)
let mk_contract_summary :
    contract_summary -> LA.contract_node_decl -> contract_summary =
 fun m (i, _, _, _, (_, eqns)) ->
  let export_ids = List.concat (List.map (get_contract_exports m) eqns) in
  IMap.add i export_ids m

(** Make a dependency graph from the equations. Each LHS has an edge that goes
    into its RHS definition. *)
let mk_graph_eqn :
    node_summary ->
    LA.node_equation ->
    (dependency_analysis_data, [> error ]) result =
  let handle_one_lhs :
      dependency_analysis_data -> LA.struct_item -> dependency_analysis_data =
   fun rhs_g lhs ->
    match lhs with
  ...
  fun m -> function
    | Equation (pos, LA.StructDef (_, lhss), e) ->
        ...
        Debug.parse "For lhss=%a: width RHS=%a, width LHS=%a"
          ...
          lhss Format.pp_print_int (List.length rhs_g) Format.pp_print_int
          (List.length lhss);
        if List.length rhs_g = List.length lhss then
          R.ok
            (List.fold_left union_dependency_analysis_data
               empty_dependency_analysis_data
...
(** Check for node equations, we need to flatten the node calls using
    [node_summary] generated *)
let analyze_circ_node_equations :
    node_summary -> LA.node_item list -> (unit, [> error ]) result =
 fun m eqns ->
...
let analyze_circ_node_locals :
    LA.node_local_decl list -> (unit, [> error ]) result =
 fun locals ->
  ...
  let ad =
    List.fold_left
      (fun acc local ->
        ...
        | NodeVarDecl _ -> acc)
      empty_dependency_analysis_data locals
  in
  (try R.ok (G.topological_sort ad.graph_data)
   with G.CyclicGraphException ids -> (
     match find_id_pos ad.id_pos_data (List.hd ids) with
...
let check_no_input_output_local_duplicate ips ops locals =
  let add m pos i = check_and_add m pos empty_hs i () in
  let* map = R.seq_chain (fun m (p, i, _, _, _) -> add m p i) IMap.empty ips in
  let* map = R.seq_chain (fun m (p, i, _, _) -> add m p i) map ops in
  R.seq_chain
    (fun m local ->
      match local with
...
(** Check if node equations do not have circularity and also, if the node has a
    contract sort the contract equations. *)
let check_node_equations :
    dependency_analysis_data ->
    LA.node_decl ->
    (LA.node_decl, [> error ]) result =
 fun ad
     ((i, imported, opac, params, ips, ops, locals, items, contract_opt) as
      ndecl) ->
  (if not imported then
  ...
  | Some c ->
      let* _, _, _, _, c' =
        sort_and_check_contract_eqns ad
          ( HString.concat (HString.mk_hstring "inline$") [ contract_prefix; i ],
            params,
            ips,
            ops,
            c )
...
(** This function generates the node summary and contract summary data This
    function requires that the program does not have any forward references. *)
let rec generate_summaries :
    dependency_analysis_data -> LA.t -> dependency_analysis_data =
 fun ad -> function
  | [] -> ad
  ...
  | LA.ContractNodeDecl (_, cdecl) :: decls ->
      ...
      generate_summaries
        { ad with csummary = IMap.union (fun _ _ v2 -> Some v2) ad.csummary cs }
        decls
  | _ :: decls -> generate_summaries ad decls

(** Sort equations for contracts and check if node and function equations have
    circular dependencies *)
let rec sort_and_check_equations :
    dependency_analysis_data -> LA.t -> (LA.t, [> error ]) result =
 fun ad -> function
...
(** Returns a topological order to resolve forward references of global
    constants and type definitions. *)
let sort_globals decls =
  let* sorted_decls, _ = sort_declarations decls in
  Debug.parse
    "Sorting types and constants declarations done.\n\
    \    \n\
     ============\n\
     %a\n\
     ============\n"
...
(** Returns a topological order of declarations to resolve all forward refernce.
    It also reorders contract equations and checks for circularity of node
    equations *)
let sort_and_check_nodes_contracts decls =
  ...
  Debug.parse
    "Sorting functions, nodes and contracts done.\n\
    \    \n\
     ============\n\
     %a\n\
     ============\n"
  ...
  let analysis_data =
    generate_summaries empty_dependency_analysis_data sorted_decls
  in
  Debug.parse
    "Generated contract and node summaries.\n\
    \    \n\
     ============\n\
     %a\n\
     ============\n"
