...
let error_message error =
  ...
  | EnvRealizabilityCheckModeRefAssumption ->
      "Environment realizability checks do not currently support assumptions \
       containing mode references. To disable environment realizability \
       checking, pass --check_realizability false"
...
let mk_fresh_id id =
  ...
  let prefix =
    HString.concat2
      (HString.mk_hstring (string_of_int !i))
      (HString.mk_hstring "_")
  in
...
let get_node_type_and_name : string -> node_type * string =
 fun name ->
  ...
  if
    String.length name > inputs_tag_len
    && String.sub name 0 inputs_tag_len = inputs_tag
  then
    ( Environment,
      String.sub name inputs_tag_len (String.length name - inputs_tag_len) )
  else if
    String.length name > contract_tag_len
    && String.sub name 0 contract_tag_len = contract_tag
  then
    ( Contract,
      String.sub name contract_tag_len (String.length name - contract_tag_len)
    )
  else if
    String.length name > type_tag_len
    && String.sub name 0 type_tag_len = type_tag
  then (Type, String.sub name type_tag_len (String.length name - type_tag_len))
  else if
    String.length name > poly_gen_node_tag_len
    && String.sub name 0 poly_gen_node_tag_len = poly_gen_node_tag
  then
    let s =
    ...
    let len_prefix =
      if Str.string_match re s 0 then String.length (Str.matched_string s) + 1
      else 1
    in
  ...
  let gids =
    {
      ...
      equations =
        [
          ( [],
            [],
            A.StructDef (pos, [ SingleIdent (pos, name) ]),
            rhs,
            Some GI.Ghost );
        ];
    }
...
let rec expr_contains_mode_ref expr =
  ...
  | Activate (_, _, e1, e2, expr_list) ->
      r e1 || r e2 || List.fold_left (fun acc x -> acc || r x) false expr_list
  | Call (_, _, _, _)
  | Condact (_, _, _, _, _, _)
      ...
      (List.map
         (fun ci ->
           ...
           | A.Assume (pos, name, b, expr) ->
               if expr_contains_mode_ref expr then
                 mk_error pos EnvRealizabilityCheckModeRefAssumption
               else R.ok (Some (A.Guarantee (pos, name, b, expr), GI.empty ()))
           | A.ContractCall (pos, name, ty_args, ips, ops) ->
               if Flags.Contracts.check_environment () then
                 let name =
                   HString.concat2 (HString.mk_hstring inputs_tag) name
                 in
          ...
          to store the output *)
                       | expr ->
                           let called_contract_ty =
                             Ctx.lookup_contract_ty ctx name
                           in
                           (* Option.get should not fail because we know the check_environment flag is enabled *)
                           let ghost_var_ty =
                             match Option.get called_contract_ty with
                             | TArr (_, _, GroupType (_, tys)) -> List.nth tys i
                             | TArr (_, _, ty) when i = 0 -> ty
                             | _ -> assert false
                           in
                           let gen_ghost_var, gids =
                             mk_fresh_ghost_var pos ghost_var_ty expr
                           in
                           (gen_ghost_var, gids))
                     ips
                   |> List.split
...
let mk_swapped_inputs_and_outputs ctx inputs outputs =
  ...
  let inputs2 =
    List.map
      (fun (p, id, ty, cl, b) ->
        let ty =
          Chk.expand_type_syn_reftype_history_subrange ctx ty |> unwrap
        in
        (p, id, ty, cl, b))
      inputs2
  in
...
let contract_node_decl_to_contracts =
 fun ctx (id, params, inputs, outputs, (pos, base_contract)) ->
  ...
  let environment =
    (gen_node_id, params, inputs2, outputs2, (pos, contract'))
  in
  let gids =
    List.fold_left GI.union (GI.empty ()) gids
    |> GI.StringMap.singleton gen_node_id
  in
  if Flags.Contracts.check_environment () then
    (* Update ctx with info about the generated contract *)
    let ctx, _ =
      LustreTypeChecker.tc_ctx_of_contract_node_decl pos ctx environment
      |> unwrap
    in
...
let node_decl_to_contracts =
 fun pos ctx (id, extern, _, params, inputs, outputs, locals, _, contract) ->
  let base_contract =
    match contract with None -> [] | Some (_, contract) -> contract
  in
  let* contract', gids = mk_generated_env_contract_eqs ctx base_contract in
  let locals_as_outputs =
    List.map
      (fun local_decl ->
        match local_decl with
        | A.NodeConstDecl (pos, FreeConst (_, id, ty))
        | A.NodeConstDecl (pos, TypedConst (_, id, _, ty)) ->
            Some (pos, id, ty, A.ClockTrue)
        | A.NodeVarDecl (pos, (_, id, ty, cl)) -> Some (pos, id, ty, cl)
        | _ -> None)
      locals
  ...
  if extern then
    ...
    if Flags.Contracts.check_environment () then
      ...
      let ctx, _ =
        LustreTypeChecker.tc_ctx_of_node_decl pos ctx environment |> unwrap
      in
      R.ok ([ environment ], ctx, gids)
    else R.ok ([], ctx, gids)
  else
    ...
    if Flags.Contracts.check_environment () then
      ...
      in
      let ctx, _ =
        LustreTypeChecker.tc_ctx_of_node_decl pos ctx contract |> unwrap
      in
...
(* NOTE: If "ty" is a refinement type that includes a constant with
   ...
   (see commit 6ac7eee).
*)
let type_to_contract :
    Lib.position ->
    HString.t ->
    A.lustre_type ->
    HString.t list ->
    A.declaration option =
 fun pos id ty ps ->
...
let gen_imp_nodes :
 ...
 fun ctx decls ->
  let* decls, ctx, gids =
    R.seq_chain
      (fun (acc_decls, acc_ctx, acc_gids) decl ->
        match decl with
        | A.ConstDecl (_, FreeConst _) | A.ConstDecl (_, TypedConst _) ->
            R.ok (acc_decls, acc_ctx, acc_gids)
        ...
        | A.TypeDecl (_, FreeType _) | A.ConstDecl (_, UntypedConst _) ->
            R.ok (acc_decls, acc_ctx, acc_gids)
        | A.NodeDecl
            (span, ((p, e, opac, ps, ips, ops, locs, _, c) as node_decl)) ->
            ...
            let node_decl =
              if e then
                ( p,
                  ...
                  ops,
                  locs,
                  [ A.AnnotMain (span.start_pos, true) ],
                  c )
              else node_decl
            in
            let* decls, acc_ctx, gids =
              node_decl_to_contracts span.start_pos acc_ctx node_decl
            in
            let decls = List.map (fun decl -> A.NodeDecl (span, decl)) decls in
            R.ok
              ( (A.NodeDecl (span, node_decl) :: decls) @ acc_decls,
                acc_ctx,
                GI.StringMap.merge GI.union_keys2 gids acc_gids )
        | A.FuncDecl
            (span, ((p, e, opac, ps, ips, ops, locs, _, c) as func_decl)) ->
            ...
            in
            let* decls, acc_ctx, gids =
              node_decl_to_contracts span.start_pos acc_ctx func_decl
            in
            let decls = List.map (fun decl -> A.FuncDecl (span, decl)) decls in
            R.ok
              ( (A.FuncDecl (span, func_decl) :: decls) @ acc_decls,
                acc_ctx,
                GI.StringMap.merge GI.union_keys2 gids acc_gids )
        | A.ContractNodeDecl (span, contract_node_decl) ->
            let* decls, acc_ctx, gids =
              contract_node_decl_to_contracts acc_ctx contract_node_decl
            in
            let decls =
              List.map (fun decl -> A.ContractNodeDecl (span, decl)) decls
            in
            R.ok
              ( (A.ContractNodeDecl (span, contract_node_decl) :: decls)
                @ acc_decls,
                acc_ctx,
                GI.StringMap.merge GI.union_keys2 gids acc_gids )
        | A.NodeParamInst _ -> R.ok (decl :: acc_decls, acc_ctx, acc_gids))
      ([], ctx, GI.StringMap.empty)
      decls
  in
