...
let rec map_top_and_add instances model model' i state_var =
  if SVT.length model == 0 then SVT.add model' state_var []
  else
    try
...
let safe_map_top_and_add instances model model' i state_var =
  try map_top_and_add instances model model' i state_var
  with Not_found -> () (* No state variable is added to model' *)

let add_init_values equations_of_init trans_sys instances model' first_model
    to_reconstruct =
  equations_of_init
  |> List.iter (fun ((sv, _), def) ->
         if SVS.mem sv to_reconstruct then (
          ...
          not fail if they are not available *)
             ...
             |> Eval.eval_term (TransSys.uf_defs trans_sys) first_model
             (* Return term *)
             |> Eval.term_of_value
           in
           let var = Var.mk_state_var_instance sv Model.path_offset in
...
let add_step_values equations_of_step trans_sys instances model' curr_model
    prev_model to_reconstruct =
  equations_of_step
  |> List.iter (fun ((sv, _), def) ->
         if SVS.mem sv to_reconstruct then (
           (* Value for state variable at step *)
...
let reconstruct_and_add first_is_init node trans_sys instances model'
    step_models to_reconstruct =
  let equations_of_init =
    (* Equations must be topologically sorted *)
    N.ordered_equations_of_node node (TransSys.state_vars trans_sys) true
  in
  let equations_of_step =
    ...
    | _ ->
        (* Equations must be topologically sorted *)
        N.ordered_equations_of_node node (TransSys.state_vars trans_sys) false
  in
  let rec aux = function
    ...
    | [ first_model ] when first_is_init ->
       ...
       from their (initial) Lustre expressions *)
        add_init_values equations_of_init trans_sys instances model' first_model
          to_reconstruct
    | [ curr_model ] ->
        ...
        let prev_model =
         ...
         we use an empty model *)
          Model.create 0
        in
        add_step_values equations_of_step trans_sys instances model' curr_model
          prev_model to_reconstruct
    | curr_model :: tl ->
        (* Reconstruct the rest of values for the variables
       from their (step) Lustre expressions *)
        add_step_values equations_of_step trans_sys instances model' curr_model
          (List.hd tl) to_reconstruct;
...
let map_top_or_reconstruct_and_add first_is_init node trans_sys instances model
    model' locals =
  ...
  if not (SVS.is_empty to_reconstruct) then
    ...
    reconstruct_and_add first_is_init node trans_sys instances model'
      step_models to_reconstruct
...
let active_modes_of_instances model_top instances = function
  ...
  | Some { C.modes } ->
      ...
      let trace_of_req { C.svar } =
        map_top instances svar |> SVT.find model_top
        |> List.map (function
             | Model.Term t -> t == Term.t_true
             | _ ->
                 failwith "evaluating mode requirement: value should be a term")
      in

      (* Trace of active modes has the same length as the model. Originally
  empty. *)
      let empty_trace =
        let rec loop acc n = if n <= 0 then acc else loop ([] :: acc) (n - 1) in
      ...
      let merge_req_traces t1 t2 =
        let rec loop acc = function
          ...
          | _ ->
              failwith
                "while constructing the trace of active modes:@ tried to merge \
                 two traces of inconsistent length"
        in
        loop [] (t1, t2)
      in
      ...
      let add_mode_to_trace mode_trace = function
        ...
        | { C.requires = head :: tail } as m ->
            ...
            let rec loop pref = function
              ...
              | _ ->
                  failwith
                    "while adding mode to trace of active modes:@ tried to \
                     merge two traces of inconsistent length"
            in
...
let active_modes_to_strings =
  let rec loop acc current rest width = function
    | [] :: tail -> loop acc ("" :: current) ([] :: rest) width tail
    | (mode :: mode_tail) :: tail ->
        let str = Format.asprintf "%a" (I.pp_print_ident false) mode.C.name in
        loop acc (str :: current) (mode_tail :: rest)
          (max width (String.length str))
          tail
    | [] ->
        if rest |> List.for_all (fun l -> l = []) then
          (List.rev current :: acc, width)
        else loop (List.rev current :: acc) [] [] width (List.rev rest)
  in
  loop [] [] []

let get_constants const_map scope =
  match Scope.Map.find_opt scope const_map with None -> [] | Some l -> l

(* Reconstruct model for the instance given the models for the subnodes 

   Use with [TransSys.fold_subsystem_instances]. *)
let node_path_of_instance const_map first_is_init model_top
    ({ N.inputs; N.outputs; N.locals; N.contract } as node) trans_sys instances
    subnodes =
  ...
  let call_conds =
    List.fold_left
      (fun acc (_, _, call_cond) -> List.rev_append call_cond acc)
      [] instances
    |> List.rev
  in
  ...
  let model =
    Model.create_path
      (D.cardinal inputs + D.cardinal outputs + List.length call_conds
  ...
  *)
  ...
  List.iter
    (function
      | N.CActivate c | N.CRestart c -> (
          try
            map_top_and_add (List.tl instances) model_top model D.empty_index c
  ...
  map_top_or_reconstruct_and_add first_is_init node trans_sys instances
    model_top model
  ...
  else List.iter (fun sv -> SVT.add model sv (SVT.find model_top sv)) constants;
...
let node_path_of_subsystems globals first_is_init trans_sys model
    ({ S.scope } as subsystems) =
  (* Format.printf "trans_sys@.@." ; *)

  (* Get transition system of top scope of subsystems *)
  let trans_sys' = TransSys.find_subsystem_of_scope trans_sys scope in
  ...
  let const_map =
      ...
      List.fold_left
        (fun acc (_, vt) ->
          let l =
            D.bindings vt
            |> List.map (fun (i, v) ->
                   (i, Var.state_var_of_state_var_instance v))
          in
    ...
    List.fold_left
      (fun acc (i, sv) ->
        ...
        in
        Scope.Map.update scope
          (function None -> Some [ (i, sv) ] | Some l -> Some ((i, sv) :: l))
          acc)
      Scope.Map.empty constants
  in

  try
    let r =
      (* Create models for all subnodes *)
      N.fold_node_calls_with_trans_sys nodes
...
let pp_print_stream_value_pt ty val_width ppf = function
  | None -> Format.fprintf ppf "%*s" val_width "_"
  | Some v ->
...
let pp_print_stream_ident_pt ppf (index, state_var) =
  Format.fprintf ppf "%a%a" pp_print_lustre_var state_var
...
let pp_print_call_pt ppf (name, pos) =
  ...
  let _, name = LGR.get_node_type_and_name name in
  Format.fprintf ppf "%s%a" name pp_print_pos_pt pos
...
let rec act_to_bool acc = function
  | [] -> List.rev acc
  | Model.Term t :: tl -> act_to_bool (Term.bool_of_term t :: acc) tl
  | _ :: _ -> assert false
...
let rec and_lists acc ls =
  let cur, rs, one_empty =
    List.fold_left
      ...
      (true, [], false) ls
  in
  if one_empty then List.rev acc else and_lists (cur :: acc) rs
...
let act_stream path l =
  let streams =
    List.fold_left
      (fun acc -> function
        | N.CActivate c -> (SVT.find path c |> act_to_bool []) :: acc | _ -> acc)
      [] l
  in
  match streams with [] -> None | _ -> Some (and_lists [] streams)

(* Sample a stream of values (as strings) on a Boolean clock *)
let sample_stream_on_clock =
  List.map2 (fun s b -> match (s, b) with Some _, true -> s | _ -> None)

(* Sample a set of streams *)
let sample_streams_on_clock clock streams =
  List.map
    (fun (name, ty, stream) -> (name, ty, sample_stream_on_clock stream clock))
    streams

(* Convert identifiers and values of streams to strings and update
   maximal lenght of the strings *)
let rec streams_to_values path ident_width val_width streams = function
  ...
  | (index, state_var) :: tl -> (
      try
        ...
        let ident_width = max ident_width (String.length stream_name) in

        (* Add string representation of stream and continue with
           remaining streams *)
        streams_to_values path ident_width val_width
      ...
      with Not_found ->
        ...
        *)
        streams_to_values path ident_width val_width streams tl)

(* Output a stream value with given width for the identifier and
   values *)
let pp_print_stream_pt ident_width val_width ppf (stream_name, ty, stream_values)
    =
  (* Break lines if necessary and indent correctly *)
  Format.fprintf ppf "@[<hov %d>@{<blue_b>%-*s@} %a@]" (ident_width + 1)
    ident_width stream_name
    (pp_print_list (pp_print_stream_value_pt ty val_width) "@ ")
    stream_values

(* Output a stream value with given width for the identifier and
   values *)
let pp_print_stream_string_pt ident_width val_width stream_name ppf
...
let pp_print_modes_section_pt ident_width val_width mode_ident ppf = function
  | None -> ()
  | Some vals ->
      Format.fprintf ppf "== @{<b>%s@} ==@,%a@," mode_ident
        (pp_print_list
           (pp_print_stream_string_pt ident_width val_width "")
           "@,")
        vals
...
let filter_locals is_visible locals =
  ...
  List.fold_left
    (fun acc (i, sv) ->
      match is_visible sv with true -> (i, sv) :: acc | false -> acc)
    [] locals
  ...
  | ( trace,
      Node
        ( ({ N.name; N.inputs; N.outputs; N.locals; N.is_function } as node),
          ...
          subnodes ) )
    :: tl
    when N.node_is_visible node ->
      let is_visible = N.state_var_is_visible node in

      let node_type, node_name =
        LGR.get_node_type_and_name (I.string_of_ident true name)
      in
      ...
      let title =
        if is_function then "Function"
          ...
          | LGR.Environment -> "Environment of"
     ...
     call *)
      ...
      let ident_width, val_width, inputs' =
        D.bindings inputs |> List.map pop_head_index
      ...
      let ident_width, val_width, outputs' =
        D.bindings outputs |> List.map pop_head_index
        |> List.filter (fun (_, sv) -> is_visible sv)
        |> streams_to_values model ident_width val_width []
      in

      let mode_ident = "Mode(s)" in
      let _ (* ident_witdth *), val_width, modes =
        match active_modes with
     ...
     original order again. *)
      ...
      let globals', constants', inputs', outputs', ghosts', locals' =
        match clock with
      ...
      Format.fprintf ppf
        "@[<v>@{<b>%s@} @{<blue>%s@} (%a)@,  @[<v>%a%a%a%a%a%a%a@]@,@]" title
        node_name
        (pp_print_list pp_print_call_pt " / ")
        (List.rev trace)
        (pp_print_modes_section_pt ident_width val_width mode_ident)
        modes
        (pp_print_stream_section_pt ident_width val_width "Global Constants")
        globals'
        (pp_print_stream_section_pt ident_width val_width "Constants")
        constants'
        (pp_print_stream_section_pt ident_width val_width "Inputs")
        inputs'
        (pp_print_stream_section_pt ident_width val_width "Outputs")
        outputs'
        (pp_print_stream_section_pt ident_width val_width "Ghosts")
        ghosts'
        (pp_print_stream_section_pt ident_width val_width "Locals")
        locals';

      (* Recurse depth-first to print subnodes *)
      pp_print_lustre_path_pt' false const_map ppf (subnodes @ tl)
  | _ :: tl -> pp_print_lustre_path_pt' false const_map ppf tl
...
let pp_print_path_pt trans_sys globals subsystems first_is_init ppf model =
  (* Create the hierarchical model *)
  node_path_of_subsystems globals first_is_init trans_sys model subsystems
...
let pp_print_file_xml ppf pos_file =
  if pos_file = "" then () else Format.fprintf ppf "@ file=\"%s\"" pos_file

(* Output a position as XML attributes *)
let pp_print_pos_xml ppf pos =
  (* Do not print anything for a dummy position *)
  if is_dummy_pos pos then ()
  else
    (* Get file, line and column of position *)
    let pos_file, pos_lnum, pos_cnum = file_row_col_of_pos pos in

    (* Print attributes *)
    Format.fprintf ppf "%a@ line=\"%d\"@ column=\"%d\"" pp_print_file_xml
      pos_file pos_lnum pos_cnum
...
let pp_print_call_xml ppf = function
  ...
  | (_, pos) :: _ -> Format.fprintf ppf "%a" pp_print_pos_xml pos
...
let pp_print_stream_value ty ppf i show v =
  if show then
    Format.fprintf ppf
      "@,@[<hv 2><Value instant=\"%d\">@,@[<hv 2>%a@]@,@]</Value>" i
      (Model.pp_print_value_xml ~as_type:ty)
      v
...
let pp_print_stream_xml node model clock ppf (index, state_var) =
  let pp_print_type ppf stream_type =
    ...
    | Type.Bool -> Format.pp_print_string ppf "type=\"bool\""
    | Type.Int -> Format.pp_print_string ppf "type=\"int\""
    | Type.UBV i -> (
        match i with
        ...
        | _ -> raise (Invalid_argument "pp_print_type: BV size not allowed"))
    | Type.BV i -> (
        match i with
    ...
    | Type.IntRange (i, j) ->
        let pp_print_bound_opt attr bound ppf =
          ...
          | None -> ()
          | Some x ->
              Format.fprintf ppf " %s=\"%a\"" attr Numeral.pp_print_numeral x
        in
        Format.fprintf ppf "type=\"subrange\"%t%t"
          (pp_print_bound_opt "min" i)
          (pp_print_bound_opt "max" j)
    | Type.Real -> Format.pp_print_string ppf "type=\"real\""
    ...
    | Type.Enum _ ->
        ...
        in
        Format.fprintf ppf "type=\"enum\"@ %tvalues=\"%a\"" pp_print_enum_name
          (pp_print_list Format.pp_print_string ", ")
          (Type.constructors_of_enum stream_type)
    | Type.Array (_, _) -> Format.pp_print_string ppf "type=\"array\""
  in
  ...
  try
    ...
    let stream_type = StateVar.type_of_state_var state_var in
    Format.fprintf ppf
      "@,@[<hv 2>@[<hv 1><Stream@ name=\"%a\" %a%a>@]%a@]@,</Stream>"
      pp_print_stream_ident_xml (index, state_var) pp_print_type stream_type
      (pp_print_stream_prop_xml node)
      state_var
      (pp_print_stream_values clock stream_type)
      stream_values
  with Not_found -> assert false
  ...
  | Some mode_trace ->
      Format.fprintf ppf "@,@[<v>%a@]"
        (pp_print_list
           (fun _ (k, tree) ->
             Format.fprintf ppf
               "<ActiveModes instant=\"%d\">@   %a</ActiveModes>" k
...
let rec pp_print_lustre_path_xml' is_top const_map ppf = function
  ...
  | ( trace,
    ...
    when N.node_is_visible node ->
      ...
      let title =
        if is_function then "Function" else if is_state then "State" else "Node"
      in
      ...
      Format.fprintf ppf "@,@[<hv 2>@[<hv 1><%s@ name=\"%s\"%a>@]" title name
...
let pp_print_path_xml trans_sys globals subsystems first_is_init ppf model =
  (* Create the hierarchical model *)
  node_path_of_subsystems globals first_is_init trans_sys model subsystems
  (* Output as XML *)
...
let pp_print_call_json ppf = function
  ...
  | (_, pos) :: _ ->
      let pp_print_file_json ppf pos_file =
        if pos_file = "" then ()
        else Format.fprintf ppf ",@,\"file\" : \"%s\"" pos_file
      in
      ...
      if is_dummy_pos pos then ()
        ...
        (* Print attributes *)
        Format.fprintf ppf "%a,@,\"line\" : %d,@,\"column\" : %d"
          pp_print_file_json pos_file pos_lnum pos_cnum
...
let pp_print_stream_value_json ty ppf i v =
  Format.fprintf ppf "@,[%d, %a]" i (Model.pp_print_value_json ~as_type:ty) v
...
let pp_print_stream_values_json clock ty ppf l =
  ...
  | None ->
      (* Show all values if no clock *)
      pp_print_listi
        (fun ppf i -> pp_print_stream_value_json ty ppf i)
        "," ppf l
  | Some c ->
      ...
      in
      pp_print_list
        (fun ppf (i, v, _c) -> pp_print_stream_value_json ty ppf i v)
        "," ppf values_on_clock

let rec pp_print_type_json field ppf stream_type =
  match Type.node_of_type stream_type with
  | Type.Bool | Type.Int | Type.UBV _ | Type.BV _ | Type.Real ->
      Format.fprintf ppf "\"%s\" : \"%a\",@," field
        (E.pp_print_lustre_type false)
        stream_type
  | Type.Abstr s ->
      Format.fprintf ppf
        "\"%s\" : \"abstr\",@,\"%sInfo\" :@,{@[<v 1>@,\"name\" : %s@]@,},@,"
        field field s
  | Type.IntRange (i, j) ->
      Format.fprintf ppf
        "\"%s\" : \"subrange\",@,\"%sInfo\" :@,{@[<v 1>@,%t@]@,},@," field field
        (fun ppf ->
          match (i, j) with
          | Some i, Some j ->
          ...
          | Some i, None ->
              Format.fprintf ppf "\"min\" : %a" Numeral.pp_print_numeral i
          | None, Some j ->
              Format.fprintf ppf "\"max\" : %a" Numeral.pp_print_numeral j
          | None, None -> ())
  | Type.Enum (_, _) ->
      let pp_print_qstring ppf s = Format.fprintf ppf "\"%s\"" s in
      let pp_print_enum_name ppf =
        Format.fprintf ppf "\"name\" : \"%s\",@,"
          (Type.name_of_enum stream_type)
      in
      Format.fprintf ppf
        "\"%s\" : \"enum\",@,\
         \"%sInfo\" :@,\
        ...
        field field pp_print_enum_name
...
let pp_print_stream_json node model clock ppf (index, state_var) =
  try
    ...
    let stream_type = StateVar.type_of_state_var state_var in
    Format.fprintf ppf
      "@,{@[<v 1>@,\"name\" : \"%a\",@,%a%a\"instantValues\" :%t@]@,}"
      pp_print_stream_ident_json (index, state_var) (pp_print_type_json "type")
      stream_type (pp_print_stream_prop_json node) state_var (function ppf ->
        if stream_values = [] then Format.fprintf ppf " []"
        else
...
let pp_print_active_modes_json ppf = function
  ...
  | Some mode_trace ->
      Format.fprintf ppf ",@,\"activeModes\" :@,[@[<v 1>%a@]@,]"
        (pp_print_list
           (fun _ (k, tree) ->
             Format.fprintf ppf "@,{@[<v 1>@,\"instant\" : %d,@,%a@]@,}" k
...
let pp_print_streams_json node model clock ppf = function
  ...
  | streams ->
      Format.fprintf ppf ",@,\"streams\" :@,[@[<v 1>%a@]@,]"
        (pp_print_list (pp_print_stream_json node model clock) ",")
        streams
...
let pp_print_streams_json is_top const_map ppf
  ...
  let streams =
    [] |> List.rev_append globals' |> List.rev_append constants'
    |> List.rev_append inputs' |> List.rev_append outputs'
    |> List.rev_append ghosts' |> List.rev_append locals' |> List.rev
  in

  Format.fprintf ppf "%a" (pp_print_streams_json node model clock) streams
...
let rec pp_print_lustre_path_json' is_top const_map ppf = function
  ...
  | ( trace,
    ...
    when N.node_is_visible node ->
      ...
      let pp_print_subnodes_json ppf = function
        ...
        | subnodes ->
            Format.fprintf ppf ",@,\"subnodes\" :@,[@[<v 1>%a@]@,]"
              (pp_print_lustre_path_json' false const_map)
              subnodes
      in
      ...
      (* Pretty-print this node *)
      Format.fprintf ppf
        "@,{@[<v 1>@,\"blockType\" : \"%s\",@,\"name\" : \"%s\"%a%a%a%a@]@,}%s"
        title name pp_print_call_json trace pp_print_active_modes_json
        active_modes
        (pp_print_streams_json is_top const_map)
        (node, model, call_conds) pp_print_subnodes_json subnodes comma;
...
let pp_print_lustre_path_json ppf (path, const_map) =
  (* Delegate to recursive function *)
  Format.fprintf ppf "@,[@[<v 1>%a@]@,]"
    (pp_print_lustre_path_json' true const_map)
    [ path ]

(* Ouptut a hierarchical model as JSON *)
let pp_print_path_json trans_sys globals subsystems first_is_init ppf model =
  (* Create the hierarchical model *)
  node_path_of_subsystems globals first_is_init trans_sys model subsystems
  (* Output as JSON *)
...
let pp_print_stream_csv model ppf (index, sv) =
  try
    ...
    Format.fprintf ppf "%a,%a,%a" pp_print_stream_ident_xml (index, sv)
      (E.pp_print_lustre_type false)
      typ3
...
let pp_print_lustre_path_in_csv ppf = function
  | (_, Node ({ N.inputs }, model, _, _, _)), _ ->
      ...
      (* Print inputs in CSV. *)
      Format.fprintf ppf "@[<v>%a@]"
        (pp_print_list (pp_print_stream_csv model) "@ ")
        inputs

(* Outputs a model for the inputs of a system in CSV. *)
let pp_print_path_in_csv trans_sys globals subsystems first_is_init ppf model =
  (* Create the hierarchical model. *)
  node_path_of_subsystems globals first_is_init trans_sys model subsystems
  (* Output as CSV. *)
...
let rec add_to_callpos abstr_map acc pos cond args calls =
  ...
  | ((pos', nb', _, args') as x) :: r ->
      ...
      if same_args abstr_map args args' then
        ...
        if c_pos > 0 then List.rev_append acc (x :: (pos, nb', cond, args) :: r)
        else List.rev_append acc ((pos, nb', cond, args) :: calls)
...
let pos_to_numbers abstr_map nodes =
  ...
  let node_by_lid lid = List.find (fun n -> I.equal n.N.name lid) nodes in
  ...
  let rec fold parents node =
    List.iter
      (fun ({
        ...
        (* ; *)
        register_callpos_for_nb abstr_map hc lid parents pos cond (inputs, defs);
...
let get_pos_number hc lid pos =
  let find_in_cat cat =
    Hashtbl.iter
      (fun _ l ->
        List.iter
          (fun (p, n, c, _) -> if Lib.equal_pos p pos then raise (Found (n, c)))
          l)
      cat;
    raise Not_found
  in
    ...
    (fun acc node ->
      SVT.fold
        (fun oracle sv acc ->
          ...
          Debug.fec "inverse oracle: %a ->>> %a" StateVar.pp_print_state_var
            oracle StateVar.pp_print_state_var sv;
          SVM.add oracle (sv :: l) acc)
        (N.get_oracle_state_var_map node)
        acc)
    SVM.empty nodes
...
let inverse_expr_map nodes =
  List.fold_left
    (fun acc node ->
      SVT.fold
        (fun sv e acc ->
          Debug.fec "inverse expr: %a ->>> %a" StateVar.pp_print_state_var sv
            (LustreExpr.pp_print_lustre_expr false)
            e;
          SVM.add sv e acc)
        (N.get_state_var_expr_map node)
        acc)
    SVM.empty nodes
...
let reconstruct_lustre_streams subsystems state_vars =
  let nodes =
    ...
    |> List.map (fun { SubSystem.source } -> source)
  in
