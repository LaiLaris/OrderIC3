...
(** Resets the flag indicating there are unguarded pre's in the lustre code, and
    we need to guard them. *)
...
(** Return a copy of the context with an empty node of the given name and is
    extern flag in the context *)
...
(** Return a copy of the context with an empty function of the given name in the
    context *)

val add_node_to_context : t -> t -> t
(** Return a context that is identical to the previous context with the node
    constructed in the current context added *)
...
(** Add a binding of an identifier to an expression to context
    ...
    If the labeled argument [shadow] is true, allow overwriting a previous
    binding to the identifier. Otherwise raise the exception
...
(** Remove the binding of an identifier from the context
    ...
    [Invalid_argument "remove_expr_for_ident"]. Otherwise, remove the most
    recent binding to the identifier, which may make a previous binding visible,
    which was shadowed by a call to [add_expr_for_ident] with the labeled
    argument [shadow] set to true. *)
...
(** Return a context that raises an error when defining an expression.
    ...
    [fail_on_new_definition ctx pos msg] will evaluate {!fail_at_position} with
    the location [pos] and the message [msg] as arguments *)

(** Raise exception if no new definitions allowed

    Raise {!A.ParseError} with the message and position set by
    {!fail_on_new_definition}. Raise an assertion failure if the context does
...
(** Return the respective state variable if the expression denotes an output or
    a local variable of the node in the context *)
...
(** Return the original type of a state variable whose type has changed, or
    [None] if the type of the state variable has not changed *)
...
val mk_fresh_oracle_for_state_var :
  ...
  StateVar.t ->
  StateVar.t * t
(** Create a fresh oracle state variable for the pre-initial value of the given
    state variable in the context, or return a previously created oracle for
    this state variable. *)
...
val call_outputs_of_node_call :
  ...
  LustreExpr.t LustreIndex.t option ->
  StateVar.t LustreIndex.t option
(** Return variables capturing outputs of node call if a node call with the same
    input parameters and activation condition is in the context.
...
val close_expr :
  ...
  LustreExpr.t * t ->
  LustreExpr.t * t
(** Replace unguarded pre operators with oracle constants and return expression
    and modified context.

    [close_expr pos (expr, ctx)] introduces or uses a previously introduced
    fresh oracle constant for each [pre] operator applied to a variable that is
    not under a [->].
