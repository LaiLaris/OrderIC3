...
let lift_state_var state_var_map state_var =
  ...
  with Not_found ->
    Format.printf "state_var_map: @[<v>%a@]@."
      (pp_print_list
         (fun fmt (k, b) ->
           Format.fprintf fmt "%a -> %a" StateVar.pp_print_state_var k
    ...
    raise
      (Invalid_argument
         (Format.asprintf "lift_term: state variable %a not found in map"
...
let lift_prop_name node_name pos prop_name =
  ...
  let pp_print_pos ppf pos =
    ...
    if is_dummy_pos pos then () else Lib.pp_print_line_and_column ppf pos
  in
  ...
  let _, node_name =
    LustreGenRefTypeImpNodes.get_node_type_and_name node_name
  in
  let _, node_name =
    LustreGenRefTypeImpNodes.get_node_type_and_name node_name
  in
  ...
  string_of_t
    (fun ppf prop_name ->
      Format.fprintf ppf "%s%a.%s" node_name pp_print_pos pos prop_name)
    prop_name
...
let property_of_expr ?(prop_kind = P.Invariant) candidate prop_name prop_status
    prop_source { E.expr_step; E.expr_init } =
...
let ass_and_mode_requires_of_contract = function
  | Some { C.assumes; C.modes } ->
      ( (match assumes with [] -> None | _ -> Some (term_conj_of assumes)),
        modes
        |> List.map (fun { C.path; C.requires } ->
               (path, term_conj_of requires)) )
  | None -> (None, [])
  ...
  | { C.scope = s } :: _ ->
      let name =
        Format.asprintf "%a%s%a"
          (pp_print_list
             (fun fmt (pos, call) ->
               Format.fprintf fmt "%s%a." call Lib.pp_print_line_and_column pos)
             "")
          s name Lib.pp_print_line_and_column pos
      in
      let guard = conj_of requires in
      property_of_expr ~prop_kind:(P.Reachable None) false name P.PropUnknown
...
let subrequirements_of_contract call_pos scope svar_map { C.assumes } =
  assumes
  |> List.map (fun { C.pos; C.name; C.svar } ->
         let prop_term =
           Var.mk_state_var_instance svar TransSys.prop_base
           |> Term.mk_var |> lift_term svar_map
         in
         let prop_name =
           match name with
           | None ->
               Format.asprintf "%a%a.assume%a" Scope.pp_print_scope
                 (Analysis.clean_polymorphic_info scope)
                 pp_print_line_and_column call_pos pp_print_line_and_column pos
           | Some n ->
               Format.asprintf "%a%a.%s" Scope.pp_print_scope
                 (Analysis.clean_polymorphic_info scope)
...
let abstraction_of_contract assumption_assumed
  ...
  (* LHS of the implication. *)
  let lhs =
    if assumes <> [] && not assumption_assumed then E.mk_var sofar_assump
    else E.t_true
  in
  (* Guarantee. *)
...
let one_mode_active scope { C.modes } =
  if modes = [] then []
    ...
    let name =
      ...
      Format.asprintf "%a" (pp_print_list Format.pp_print_string ".") l
    in
    (* Disjunction of mode requirements. *)
    let prop =
      modes
      |> List.map (fun { C.requires } -> conj_of requires)
      |> E.mk_or_n
      (* Building property. *)
      |> property_of_expr false name P.PropUnknown
           (P.GuaranteeOneModeActive (pos, scope))
    in
    [ prop ]
...
let add_constraints_of_type init terms state_var =
  ...
  if Type.is_array state_var_type then
      ...
      List.fold_left
        (fun acc (ty, iv) ->
          ...
          | Type.IntRange (None, Some j) ->
              let bounds =
                Term.mk_leq [ Term.mk_var iv; Term.mk_num Numeral.(j - one) ]
              in
              Term.mk_forall [ iv ] (Term.mk_implies [ bounds; acc ])
          | Type.IntRange (Some i, None) ->
              let bounds = Term.mk_leq [ Term.mk_num i; Term.mk_var iv ] in
              Term.mk_forall [ iv ] (Term.mk_implies [ bounds; acc ])
          | _ -> Term.mk_forall [ iv ] acc)
        ct indices
      |> Term.convert_select
    in
    ...
    let l, u =
      if Type.is_int_range state_var_type then (None, None)
    ...
    match (l, u) with
    | Some l, Some u ->
        Term.mk_leq [ Term.mk_num l; var; Term.mk_num u ] (* Add to terms *)
        :: terms
    | Some l, None ->
        Term.mk_leq [ Term.mk_num l; var ] (* Add to terms *) :: terms
    | None, Some u ->
        Term.mk_leq [ var; Term.mk_num u ] (* Add to terms *) :: terms
...
let rec add_subsystem' trans_sys instance accum = function
  ...
  | ((trans_sys', inst) as h) :: tl ->
      ...
      if TransSys.equal_scope trans_sys trans_sys' then
        ...
        List.rev_append ((trans_sys, instance :: inst) :: accum) tl
      else
        (* Keep searching for transition system in tail of list *)
        add_subsystem' trans_sys instance (h :: accum) tl

(* Add instance of called node to list of subsystems *)
let add_subsystem trans_sys uid pos map_up map_down guard_clock assumes
    subsystems =
  let instance =
    {
      TransSys.uid;
  ...
  add_subsystem' trans_sys instance [] subsystems
    ...
    List.map
      (fun b ->
        ...
        | E.Bound e ->
            ...
            let sigma =
              ...
              let v = Var.mk_const_state_var s in*)
                ...
                let tv' = Var.mk_const_state_var sv' |> Term.mk_var in*)
                    let tv' =
                      Var.mk_state_var_instance sv' Numeral.zero |> Term.mk_var
                    in
                    (v, tv') :: acc
                  with Not_found -> acc)
                [] svs
            ...
            in
            if sigma = [] then b
            else E.Bound (Term.apply_subst sigma t |> E.unsafe_expr_of_term))
      bounds
  in
...
let call_terms_of_node_call mk_fresh_state_var globals
    {
      ...
      N.call_oracles;
      N.call_outputs;
    } node_locals node_props node_hist_svars node_crt_svars
    {
      init_uf_symbol;
      trans_uf_symbol;
      node = { N.inputs; N.oracles; N.outputs; N.contract };
      stateful_locals;
      properties;
      history_svars;
      ctr_svars;
  ...
  let node_locals, call_locals, (state_var_map_up, state_var_map_down) =
    ...
    List.fold_right
      (fun state_var
           (locals, call_locals, (state_var_map_up, state_var_map_down)) ->
        (* Create a fresh state variable in the caller *)
        let inst_state_var =
          let name =
            let row, col = row_col_of_pos call_pos in
            Format.asprintf "inst_%a_l%dc%d_%a_%d" (I.pp_print_ident true)
              call_node_name row col StateVar.pp_print_state_var state_var
              call_id
          in
          mk_fresh_state_var
            ?is_const:(Some (StateVar.is_const state_var))
            ?for_inv_gen:(Some true) ?inst_for_sv:(Some state_var) name
            (StateVar.type_of_state_var state_var)
        in

        N.set_state_var_instance inst_state_var call_pos call_node_name
          state_var;
  ...
  let node_hist_svars =
    ...
    in
    TM.union
      (fun _ m1 m2 -> Some (SVM.union (fun _ l1 l2 -> Some (l1 @ l2)) m1 m2))
      lifted_hist_svars node_hist_svars
  in

  let node_crt_svars =
    List.map (lift_state_var state_var_map_up) ctr_svars @ node_crt_svars
  in
  ...
  let node_props =
    ...
    |> List.fold_left
         (fun a ({ P.prop_name = n; P.prop_term = t; P.prop_kind } as p) ->
           ...
           let prop_name = lift_prop_name call_node_name call_pos n in
  ...
  let node_assume_props =
    ...
    | None -> []
    | Some contract ->
        subrequirements_of_contract call_pos
          (I.to_scope call_node_name)
          state_var_map_up contract
  in
  ...
  let init_params_of_bound term_of_state_var =
    List.map term_of_state_var
      (D.values call_inputs @ call_oracles @ D.values call_outputs @ call_locals)
  in
  ...
  let init_call_term =
    init_params_of_bound (E.base_term_of_state_var TransSys.init_base)
    |> Term.mk_uf init_uf_symbol
  in

  (* Term for initial state constraint at current state *)
  let init_call_term_trans =
    init_params_of_bound (E.cur_term_of_state_var TransSys.trans_base)
    |> Term.mk_uf init_uf_symbol
  in
  ...
  LustreIndex.iter
    (fun _ -> register_call_bound globals state_var_map_up)
    call_inputs;
  LustreIndex.iter
    (fun _ -> register_call_bound globals state_var_map_up)
    call_outputs;
...
(* Add constraints from node calls to initial state constraint and
   transition relation *)
let rec constraints_of_node_calls mk_fresh_state_var globals trans_sys_defs
    node_locals node_init_flags node_props node_hist_svars node_crt_svars
    subsystems init_terms trans_terms = function
  ...
  | ({ N.call_id; N.call_pos; N.call_node_name; N.call_cond = [] } as node_call)
    :: tl ->
      ...
      let ( state_var_map_up,
            ...
            trans_term ) =
        (* Create node call *)
        call_terms_of_node_call mk_fresh_state_var globals node_call node_locals
          node_props node_hist_svars node_crt_svars node_def
      in

      (* Add node instance to list of subsystems *)
      let subsystems =
        add_subsystem trans_sys call_id call_pos state_var_map_up
          state_var_map_down
          (* No guarding necessary when instantiating term, because
           this node instance does not have an activation
           condition *)
          (fun _ t -> t)
          node_assumes subsystems
      in

      (* Continue with next node calls *)
      constraints_of_node_calls mk_fresh_state_var globals trans_sys_defs
        node_locals node_init_flags node_props node_hist_svars node_crt_svars
        subsystems (init_term :: init_terms)
  ...
  | ({
       ...
       N.call_pos;
       N.call_node_name;
       N.call_cond = [ N.CRestart restart ];
     } as node_call)
    :: tl ->
      (* Get generated transition system of callee *)
      let ({ trans_sys } as node_def) =
      ...
      let subsystems =
        add_subsystem trans_sys call_id call_pos state_var_map_up
          ...
          (fun i t ->
            Term.mk_implies
              [
                Var.mk_state_var_instance restart i
      ...
      let trans_term =
        Term.mk_ite restart_trans
          (Term.bump_state
             Numeral.(TransSys.trans_base - E.cur_offset)
             init_term)
          trans_term
      in
  ...
  | ({
    ...
    :: tl ->
      ...
      let
         ...
         and equation with previous state value *)
          ( shadow_inputs,
            ...
            interpolate_inputs ) =
        D.fold2
          (fun formal_idx formal_sv actual_sv
               ( shadow_inputs,
                 node_locals,
                 propagate_inputs_init,
                 propagate_inputs_trans,
            ...
            if StateVar.is_const formal_sv then
              ...
              let shadow_sv =
                let name =
                  let row, col = row_col_of_pos call_pos in
                  Format.asprintf "shw_%a_l%dc%d_%a" (I.pp_print_ident true)
                    call_node_name row col StateVar.pp_print_state_var formal_sv
                in
                mk_fresh_state_var ?is_const:None ?for_inv_gen:(Some false)
                  ?inst_for_sv:(Some formal_sv) name
      ...
      let ( state_var_map_up,
            ...
            init_term_trans,
            trans_term ) =
        call_terms_of_node_call mk_fresh_state_var globals
          (* Modify node call to use shadow inputs *)
          { node_call with N.call_inputs = shadow_inputs }
          node_locals node_props node_hist_svars node_crt_svars node_def
      in
      ...
      let has_ticked =
        let name =
          let row, col = row_col_of_pos call_pos in
          Format.asprintf "tck_%a_l%dc%d_%a" (I.pp_print_ident true)
            call_node_name row col StateVar.pp_print_state_var clock
        in
        mk_fresh_state_var ?is_const:None ?for_inv_gen:(Some false)
          ?inst_for_sv:(Some clock) name Type.t_bool
      in
      ...
      let init_flags_init =
        List.map (E.base_term_of_state_var TransSys.init_base) init_flags
      in
      ...
      let trans_term =
        ...
        | [ N.CRestart restart ] ->
            let restart_trans =
              E.cur_term_of_state_var TransSys.trans_base restart
            in
      ...
      let init_term =
        Term.mk_and
          ([
             ...
             tick *)
             Term.mk_implies [ clock_init; Term.mk_and propagate_inputs_init ];
          ...
          | None ->
              ...
              [
                Term.mk_implies
                  [ Term.mk_not clock_init; Term.mk_and init_flags_init ];
              ]
                    ...
                    Term.mk_and
                      (D.fold2
                         (fun _ sv { E.expr_init } accum ->
                           ...
                           :: accum)
                         call_outputs d init_flags_init);
                  ];
              ])
      in
        ...
        Term.mk_and
          [
            ...
            Term.mk_eq
              [
                has_ticked_trans;
                Term.mk_or [ has_ticked_trans_pre; clock_trans_pre ];
              ];
            (* Propagate input values to shadow variables on clock
             tick *)
            Term.mk_implies [ clock_trans; Term.mk_and propagate_inputs_trans ];
            ...
            Term.mk_implies
              [
                Term.mk_and [ clock_trans; Term.negate has_ticked_trans ];
                init_term_trans;
              ];
            (* Transition relation with true activation condition
             on following clock ticks *)
            Term.mk_implies
              [ Term.mk_and [ clock_trans; has_ticked_trans ]; trans_term ];
                     ...
                     (fun accum state_var ->
                       Term.mk_eq
                         [
                           ...
                           E.pre_term_of_state_var TransSys.trans_base state_var;
                         ]
                       :: accum)
                     [] call_locals
                  |> D.fold
      ...
      let guard_prop one_state =
        ...
        | [ N.CRestart restart ] ->
            let restart_prop =
              E.cur_term_of_state_var TransSys.prop_base restart
            in
      ...
      let guard_clock =
        ...
        | [] ->
            fun i t ->
              Term.mk_implies
                [ Var.mk_state_var_instance clock i |> Term.mk_var; t ]
        | [ N.CRestart restart ] ->
            fun i t ->
              Term.mk_implies
                [
      ...
      let subsystems =
        add_subsystem trans_sys call_id call_pos state_var_map_up
          state_var_map_down guard_clock node_assumes subsystems
      in

      constraints_of_node_calls mk_fresh_state_var globals trans_sys_defs
        node_locals
        (init_flags @ node_init_flags)
        node_props node_hist_svars node_crt_svars subsystems
...
let constraints_of_ctr ctr_svars trans_terms =
  ...
  match eqs with [] -> trans_terms | _ -> Term.mk_and eqs :: trans_terms

let constraints_of_history_congruence mk_fresh_state_var history_svars
  ...
  let pairs =
              ...
              (fun acc (sv2, h_sv2) ->
                let cond =
                  if StateVar.equal_state_vars sv1 sv2 then None
                    ...
                    let name =
                      Format.sprintf "eq_vars_%d_%d" (StateVar.uid sv1)
                        (StateVar.uid sv2)
                    in
    ...
    TM.fold
      (fun _ m acc ->
        let l = SVM.bindings m |> List.map (fun (sv, l) -> (sv, List.hd l)) in
        SVM.fold
          (fun _ l' acc ->
            match l' with
            | [] -> assert false
  ...
  *)
  ...
  let add_constraints_for_vars_equal init_terms trans_terms =
    let init_terms =
      all_inputs_equal
      |> List.fold_left
           (fun init_terms o_base -> o_base :: init_terms)
           init_terms
    in
  ...
  let add_constraints_for_rest init_terms trans_terms =
    let local_and_terms =
      rest
      |> List.map (fun (name, terms) ->
             let fresh_svar =
               mk_fresh_state_var ?is_const:(Some false)
                 ?for_inv_gen:(Some true) ?inst_for_sv:None name Type.t_bool
             in
             (fresh_svar, terms))
    in
    ...
    let init_terms =
      local_and_terms
      |> List.fold_left
           (fun init_terms (l, (i_base, o_base)) ->
             let vi =
    ...
    let trans_terms =
      local_and_terms
      |> List.fold_left
           (fun trans_terms (l, (i_base, o_base)) ->
             let vt =
...
let rec constraints_of_asserts init_terms trans_terms = function
  ...
  | (_, sv) :: tl ->
      ...
      let init_terms =
        (expr |> E.base_term_of_t TransSys.init_base) :: init_terms
      in
      let trans_terms =
        (expr |> E.cur_term_of_t TransSys.trans_base) :: trans_terms
      in
...
module MBounds = Map.Make (struct
  ...
  let compare_bounds b1 b2 =
    ...
    | E.Unbound e1, E.Unbound e2 ->
        E.compare_expr e1 e2
    | E.Fixed _, _ -> -1
    | _, E.Fixed _ -> 1
    | E.Unbound _, _ -> 1
    | _, E.Unbound _ -> -1
...
(* Add constraints from equations to initial state constraint and
   transition relation *)
let rec constraints_of_equations_wo_arrays transfer_defs node eq_bounds init
    stateful_vars terms (lets, lets_dependencies) = function
  ...
  | ((state_var, []), { E.expr_init; E.expr_step }) :: tl
    when List.exists (StateVar.equal_state_vars state_var) stateful_vars ->
      ...
      constraints_of_equations_wo_arrays transfer_defs node eq_bounds init
        stateful_vars (def :: terms) (lets, lets_dependencies) tl
  ...
  | ((state_var, []), ({ E.expr_init; E.expr_step } as expr)) :: tl ->
      (if transfer_defs then
         ...
         let add_dependency sv acc =
           let old = try SVM.find sv acc with Not_found -> SVS.empty in
      ...
      to all the state variables that depend on it or one of its dependencies *)
         ...
         let depends_on_this_sv expr =
           SVS.inter dependencies (svs_in_expr expr) |> SVS.is_empty |> not
         in
         let transfer_defs_to_eq_if_needed ((sv, _), eq) =
           if
             (not (StateVar.equal_state_vars sv state_var))
             && depends_on_this_sv eq
          ...
          (if init then
             ...
             (if
              ...
              then (
                ...
                (* No binding for the variable at the previous instant
                    necessary *)
                []))
      in
      ...
      constraints_of_equations_wo_arrays transfer_defs node eq_bounds init
        stateful_vars terms
        (let_closure :: lets, lets_dependencies)
        tl
  (* Array state variable *)
  | (((_, bounds), _) as eq) :: tl ->
      let other_eqs =
        try MBounds.find bounds eq_bounds with Not_found -> []
      in
      ...
      constraints_of_equations_wo_arrays transfer_defs node eq_bounds init
        stateful_vars terms (lets, lets_dependencies) tl
    ...
    let term, quant_v, _ =
      List.fold_left
        (fun (term, quant_v, i) ->
          ...
          function
          | E.Fixed e ->
              ( Term.mk_let [ (v, E.unsafe_term_of_expr e) ] term,
                quant_v,
                pred i )
          ...
          | E.Bound e ->
              let term =
                if Flags.Arrays.recdef () then term
                  ...
                  let te =
                    E.unsafe_term_of_expr e |> fun t ->
                    if init then t else Term.bump_state Numeral.one t
                  in
                  Term.(
                    mk_implies
                      [
  ...
  MBounds.fold
    (fun bounds eqs terms ->
      ...
      (* group constraints under same quantifier when not using recursive
         encoding *)
      let cstrs =
        if Flags.Arrays.recdef () then cstrs_eqs else [ Term.mk_and cstrs_eqs ]
      in

      (* Wrap equations in let binding and/or quantifiers for indexes and add
         definitions to terms *)
      List.fold_left
        (fun terms cstr -> add_bounds cstr bounds :: terms)
        terms cstrs)
    eq_bounds terms
...
let constraints_of_equations node init stateful_vars terms equations =
  ...
  let terms, lets, eq_bounds =
    ...
    in
    constraints_of_equations_wo_arrays transfer_defs node MBounds.empty init
      stateful_vars terms ([], SVM.empty) equations
  in
  ...
  | node_name :: tl -> (
      if
       ...
       accumulator meanwhile? *)
        I.Map.mem node_name trans_sys_defs
      then
        (* Continue with next transition systems *)
        trans_sys_of_node' options globals top_name analysis_param
          trans_sys_defs output_input_dep nodes tl
      (* Transition system has not been created *)
        ...
        let ({
          ...
          with Not_found ->
            ...
            raise
              (Invalid_argument
                 (Format.asprintf "trans_sys_of_node: node %a not found"
        ...
        let mk_fresh_state_var ?is_const ?for_inv_gen ?inst_for_sv name
            state_var_type =
          ...
          let fsv =
            StateVar.mk_state_var ~is_input:false ?is_const ?for_inv_gen name
        ...
        | _ :: _ ->
            ...
            trans_sys_of_node' options globals top_name analysis_param
              trans_sys_defs output_input_dep nodes
              (tl' @ (node_name :: tl))
        ...
        | [] ->
          ...
          `trans`. *)
            let function_ufs, function_constraints_at_0 =
              if (not is_function) || not options.add_functional_constraints
              then ([], [])
              else
                ...
                |> List.fold_left
                     (fun (ufs, eqs) output ->
                       let uf_name =
                         Format.asprintf "%a.%s.%s" Scope.pp_print_scope scope
                           (StateVar.name_of_state_var output)
                           Lib.ReservedIds.function_of_inputs
                       in
                       let uf =
                         UfSymbol.mk_uf_symbol uf_name input_types
                           (type_of output)
                       in
                       ( uf :: ufs,
                         Term.mk_eq
                           [ term_0_of output; Term.mk_uf uf input_terms_at_0 ]
                         :: eqs ))
                     ([], [])
            in
            ...
            let node_assumptions =
              ...
              if A.param_scope_is_abstract analysis_param scope then
                Invs.empty ()
              else A.param_assumptions_of_scope analysis_param scope
            in
            ...
            let contract_asserts, properties =
              ...
              | Some contract ->
                 ...
                 otherwise add ensures as properties *)
                  if A.param_scope_is_abstract analysis_param scope then
                    ( abstraction_of_contract include_assumption contract
                      :: contract_asserts,
                      properties )
                  else
                    ( contract_asserts,
            ...
            let init_terms =
              ...
              :: (* Add invariants from contracts as assertions *)
                 List.map (E.base_term_of_t TransSys.init_base) contract_asserts
            ...
            let trans_terms =
              ...
              |> Term.negate)
              :: (* Add invariants from contracts as assertions *)
                 List.map (E.cur_term_of_t TransSys.trans_base) contract_asserts
              (* Add functional constraints on ouputs if any. *)
              |> List.rev_append
                   ((* Bump to `1`. *)
            ...
            let all_state_vars =
              D.values inputs @ oracles @ D.values outputs
            ...
            let node_assumptions =
              node_assumptions
              |> Invs.filter (fun _ term _ ->
                     Term.state_vars_of_term term
                     |> SVS.for_all (fun svar -> List.mem svar all_state_vars))
            in
            ...
            let filter_enum_svars =
              List.filter (fun state_var ->
                  ...
                  if Type.is_array state_var_type then
                    let base_type =
                      Type.last_elem_type_of_array state_var_type
                    in
                    Type.is_enum base_type
                  else Type.is_enum state_var_type)
            in
            ...
            *)
              let subrange_state_vars =
                let svars =
                  if A.param_scope_is_abstract analysis_param scope then oracles
                  else List.rev_append undefined_outputs oracles
                in
                svars
                |> List.filter (fun state_var ->
                       let state_var_type =
                       ...
                       else Type.is_int_range state_var_type)
              in
            ...
            let init_terms =
              List.fold_left
                ...
                init_terms subrange_and_enum_state_vars
            in

            let trans_terms =
              List.fold_left
                (add_constraints_of_type false)
                trans_terms subrange_and_enum_state_vars
            in
            ...
            let ( subsystems,
                  ...
                  init_terms,
                  trans_terms ) =
              constraints_of_node_calls mk_fresh_state_var globals
                trans_sys_defs [] (* No lifted locals *)
                [ init_flag ] [] (* No lifted properties *)
                TM.empty (* No lifted history_svars *)
                [] (* No lifted crt_vars *)
                [] (* No subsystems *)
                init_terms trans_terms calls
            in
                            ...
                            (fun acc (sv, h_sv) ->
                              if
                                List.exists
                                  ...
                                  all_state_vars
                                && List.exists
                                     (StateVar.equal_state_vars h_sv)
                                     all_state_vars
                              then SVM.add sv [ h_sv ] acc
                              else acc)
                            SVM.empty)
              in
              TM.union
                (fun _ m1 m2 ->
                  Some (SVM.union (fun _ l1 l2 -> Some (l1 @ l2)) m1 m2))
                history_svars' lifted_hist_svars
            in

            let equiv_locals, init_terms, trans_terms =
              if I.equal node_name top_name then
                constraints_of_history_congruence mk_fresh_state_var
                  history_svars init_terms trans_terms
              else ([], init_terms, trans_terms)
            in
            ...
            let global_constraints =
              ...
              List.fold_left
                (add_constraints_of_type true)
                global_constraints enum_consts
            in

            let stateful_vars =
              let global_const_svars = SVS.of_list global_const_svars in
              List.filter
            ...
            let init_terms, svar_dep_init, node_output_input_dep_init =
              ...
              |> fun (e, sv_d, io_d) ->
              ( constraints_of_equations node true stateful_vars init_terms
                  (List.rev e),
                sv_d,
                io_d )
            in
            ...
            let trans_terms, svar_dep_trans, node_output_input_dep_trans =
              ...
              |> fun (e, sv_d, io_d) ->
              ( constraints_of_equations node false stateful_vars trans_terms
                  (List.rev e),
                sv_d,
                io_d )
            in
          ...
          *)
            let constrained_svars =
              let roots =
                (* assume and assert state variables *)
                List.map snd asserts |> SVS.of_list |> fun roots' ->
                match contract with
                | None -> roots'
          ...
          *)
            let unconstrained_inputs =
              SVS.diff (SVS.of_list (D.values inputs)) constrained_svars
            in
            ...
            let properties =
              ...
              List.map
                (fun (state_var, prop_name, prop_source, prop_kind) ->
                  ...
                  let prop_term =
                    E.cur_term_of_state_var TransSys.prop_base state_var
                  in
              ...
              then
                ...
                | Some contract when contract.C.assumes <> [] ->
                    let sofar_assump offset =
                      Term.mk_var
                        (Var.mk_state_var_instance contract.C.sofar_assump
                           offset)
                    in
            ...
            let init_terms, trans_terms =
              ...
              List.fold_left
                (fun (init_terms, trans_terms) prop_term ->
                  ...
                  let prop_term_init =
                    Term.bump_state
                      ...
                      (Term.mk_implies
                         (List.rev_append assumption [ prop_term ]))
                  in
                  ...
                  (prop_term_init :: init_terms, prop_term_trans :: trans_terms))
                (init_terms, trans_terms) valid_prop_terms
            in
            ...
            (* State variables in the signature of the initial state constraint
               in correct order *)
            let signature_state_vars = signature_state_vars @ stateful_locals in

            (* Formal parameters of initial state constraint *)
            let init_formals =
              List.map
                (fun sv -> Var.mk_state_var_instance sv TransSys.init_base)
                signature_state_vars
            in
            ...
            let init_uf_symbol =
              UfSymbol.mk_uf_symbol
                (Format.asprintf "%s_%a_%d" Ids.init_uf_string
                   (LustreIdent.pp_print_ident false)
                   node_name (A.info_of_param analysis_param).A.uid)
            ...
            let trans_formals =
              ...
              List.map
                (fun sv -> Var.mk_state_var_instance sv TransSys.trans_base)
                signature_state_vars
              @
              (* Non-constant state variables at the previous instant *)
              List.map
                (fun sv ->
                  Var.mk_state_var_instance sv
            ...
            let trans_uf_symbol =
              UfSymbol.mk_uf_symbol
                (Format.asprintf "%s_%a_%d" Ids.trans_uf_string
                   (LustreIdent.pp_print_ident false)
                   node_name (A.info_of_param analysis_param).A.uid)
            ...
            let trans_sys, _ =
              TransSys.mk_trans_sys
                ...
                ty_args None (* instance_state_var *)
                init_flag
                (* [] *)
                (* global_state_vars *)
                signature_state_vars unconstrained_inputs
                globals.G.state_var_bounds global_consts global_constraints ufs
                init_uf_symbol init_formals (Term.mk_and init_terms)
                trans_uf_symbol trans_formals (Term.mk_and trans_terms)
                subsystems properties mode_requires node_assumptions
            in
            trans_sys_of_node' options globals top_name analysis_param
              (I.Map.add node_name
                 {
...
let trans_sys_of_nodes ?(options = default_settings) globals subsystems
    analysis_param =
  ...
  (match analysis_param with
  | A.Interpreter _ | A.ContractCheck _ -> ()
  | _ ->
  ...
  let ({ SubSystem.source = { N.name = top_name } } as subsystem') =
    ...
    | None ->
        S.slice_to_abstraction ~preserve_sig slice_nodes analysis_param
          subsystem'
    | Some prop ->
        let vars = Term.state_vars_of_term prop.P.prop_term in
        S.slice_to_abstraction_and_property ~preserve_sig analysis_param vars
          subsystem'
  in
  ...
  let { trans_sys } =
    try
      ...
      trans_sys_of_node' options globals top_name analysis_param I.Map.empty []
        nodes [ top_name ]
  ...
  | A.Refinement (_, result) ->
      ...
      |> List.iter (function
           | _, P.PropUnknown ->
               (* Unknown is still unknown, do nothing. *)
               ()
           | name, (P.PropKTrue _ as status) ->
               (* K-true is still k-true. *)
               TransSys.set_prop_status trans_sys name status
           | name, P.PropInvariant cert ->
