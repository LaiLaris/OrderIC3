...
let mk_svar pos num name svar scope = { pos; num; name; svar; scope }
      ...
      Format.asprintf "%a%s"
        (pp_print_list
           (fun fmt (pos, call) ->
             ...
             let _, call =
               LustreGenRefTypeImpNodes.get_node_type_and_name call
             in
...
let add_ass t assumes = { t with assumes = t.assumes @ assumes }
let add_gua t guarantees = { t with guarantees = t.guarantees @ guarantees }
let add_modes t modes = { t with modes = t.modes @ modes }

let svars_of_list l set =
  l |> List.fold_left (fun set { svar } -> SVarSet.add svar set) set

let svars_of_plist l set =
  l |> List.fold_left (fun set ({ svar }, _) -> SVarSet.add svar set) set

let svars_of_modes modes set =
  modes
  |> List.fold_left
       (fun set { requires; ensures } ->
         svars_of_list requires set |> svars_of_list ensures)
       set

let svars_of ?(with_sofar_var = true)
    { assumes; sofar_assump; guarantees; modes } =
  let initial_set =
    if with_sofar_var then SVarSet.singleton sofar_assump else SVarSet.empty
  in
  svars_of_list assumes initial_set
  |> svars_of_plist guarantees |> svars_of_modes modes

let pp_print_svar fmt { pos; num; svar } =
  Format.fprintf fmt "[%d] [%a] (%a)" num pp_print_position pos
    SVar.pp_print_state_var svar

let pp_print_svar_debug fmt { pos; num; name; svar; scope } =
  Format.fprintf fmt "[%d] [%a] (name:%a) (sv:%a) (scope:%a)" num
    pp_print_position pos
    (pp_print_option Format.pp_print_string)
    name SVar.pp_print_state_var svar
    (pp_print_list
       (pp_print_pair pp_print_position Format.pp_print_string ":")
       ",")
    scope
...
let pp_print_mode_debug safe fmt
    { name; pos; path; requires; ensures; candidate } =
  Format.fprintf fmt
    "@[<v 2>mode %a (candidate? %b) (%a) (path:%a) (@ %a@ %a@ ) ;@]"
    (I.pp_print_ident safe) name candidate
    (pp_print_list Format.pp_print_string ",")
    path pp_print_position pos
    (pp_print_list
       (fun fmt req -> Format.fprintf fmt "  require%a" pp_print_svar req)
       "@,")
    requires
...
let pp_print_contract_debug safe fmt
    { sofar_assump; assumes; guarantees; modes } =
  Format.fprintf fmt "@[<v 2>(*@contract@ %a@ %a@ %a@ %a@]@ *)"
    StateVar.pp_print_state_var_debug sofar_assump
    (pp_print_list
       (fun fmt ass -> Format.fprintf fmt "  assume%a" pp_print_svar_debug ass)
       "@ ")
    assumes
    (pp_print_list
       (fun fmt (gua, _) ->
    ...
    (pp_print_list
       (fun fmt mode ->
         Format.fprintf fmt "  %a" (pp_print_mode_debug safe) mode)
       "@ ")
    modes
...
module ModeTrace = struct
  ...
  Semantics of the triplet's elements:
    ...
    - sub contracts.
  *)
  type mode_tree_rec = Contract of string * string list * mode_tree_rec list
  ...
  *)
  ...
  let insert (top_modes, subs) path =
    ...
    let zip_up zipper trees mode =
      let rec loop subs = function
        | (pref, (contract, modes), suff) :: tail ->
            let subs =
              Contract (contract, modes, subs) :: suff |> List.rev_append pref
            in
            loop subs tail
        | [] -> subs
      in
    ...
    let rec loop zipper path trees =
      match path with
  ...
  let mode_paths_to_tree paths =
    paths |> List.map (fun { path } -> path) |> List.fold_left insert empty_tree
  ...
  let fmt_as_cex_step_xml fmt (top_mods, trees) =
    ...
    and go_up = function
      | head :: tail -> (
          ...
          match head with [] -> go_up tail | _ -> loop tail head)
      | [] -> ()
    in

    top_mods |> List.iter (Format.fprintf fmt "<Mode name=\"%s\"/>");
  ...
  let fmt_as_cex_step_json fmt (top_mods, contract_modes) =
    ...
    let pp_print_mode_list pp ppf = function
      | [] -> Format.fprintf ppf " []"
      | lst ->
          Format.fprintf ppf "@,[@[<v 1>@,%a@]@,]" (pp_print_list pp ",@,") lst
    in
    ...
    let rec pp_contract_modes ppf (Contract (name, modes, subs)) =
      Format.fprintf ppf
        ...
        name
        (pp_print_mode_list pp_print_qstring)
        modes pp_print_contract_modes_list subs
    and pp_print_contract_modes_list ppf = function
      | [] -> Format.fprintf ppf " []"
      | lst ->
          Format.fprintf ppf "@,[@[<v 1>@,%a@]@,]"
            (pp_print_list pp_contract_modes ",@,")
            lst
    in

    Format.fprintf fmt "\"topModes\" :%a,@,\"contractModes\" :%a"
      (pp_print_mode_list pp_print_qstring)
      top_mods pp_print_contract_modes_list contract_modes
end
