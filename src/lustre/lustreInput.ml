...
let build_parse_error_msg env =
  ...
  | (lazy (Cons (LPMI.Element (state, _, _, p), _))) ->
      ...
      let error_msg =
        try LPE.message pstate with Not_found -> "Syntax Error!"
      in
...
let rec parse lexbuf (chkpnt : LA.t LPMI.checkpoint) =
  ...
  | LPMI.InputNeeded _ ->
      ...
      parse lexbuf chkpnt
  | LPMI.Shifting _ | LPMI.AboutToReduce _ ->
      let chkpnt = LPMI.resume chkpnt in
      parse lexbuf chkpnt
  | LPMI.HandlingError env -> fail env lexbuf
  | LPMI.Accepted v -> Ok (success v)
  | LPMI.Rejected ->
      Error
        (`LustreParserError
           (Lib.dummy_pos, "Parser Error: Parser rejected the input."))
...
let ast_of_channel (in_ch : in_channel) =
  ...
  LustreLexer.lexbuf_init in_ch
    (try Filename.dirname input_source with Failure _ -> Sys.getcwd ());
  ...
  try parse lexbuf (LPI.main lexbuf.lex_curr_p)
  with LustreLexer.Lexer_error err ->
...
let type_check declarations =
  let tc_res =
    ...
    let* ctx, warnings2 =
      TC.type_check_infer_globals TCContext.empty_tc_context
        sorted_const_type_decls
    in

    (* Step 5: Inline type toplevel decls *)
    let* inlined_ctx, const_inlined_type_and_consts =
    ...
    let* sorted_node_contract_decls, toplevel_nodes, node_summary =
      AD.sort_and_check_nodes_contracts node_contract_src
    in

    (* Step 8. Type check nodes and contracts *)
    let* global_ctx, warnings3 =
      TC.type_check_infer_nodes_and_contracts inlined_ctx
    ...
    let sorted_node_contract_decls, gids =
      RMA.remove_mult_assign global_ctx gids sorted_node_contract_decls
    in

    (* Step 11. Desugar imperative if block to ITEs *)
    let* sorted_node_contract_decls, gids =
      LDI.desugar_if_blocks global_ctx sorted_node_contract_decls gids
    in

    (* Step 12. Desugar frame blocks by adding node equations and guarding oracles. *)
    let* sorted_node_contract_decls, warnings4 =
      LDF.desugar_frame_blocks sorted_node_contract_decls
    in
    ...
    let* _ =
      LAD.check_inductive_array_dependencies inlined_global_ctx node_summary
        const_inlined_nodes_and_contracts
    in

    (* Step 15. Infer tighter subrange constraints with abstract interpretation *)
    let* _ =
      LIA.interpret_global_consts inlined_global_ctx
        const_inlined_type_and_consts
    in
    let abstract_interp_ctx =
      LIA.interpret_program inlined_global_ctx gids
        const_inlined_nodes_and_contracts
    in

    (* Step 16. Instantiate polymorphic nodes with concrete types *)
    let inlined_global_ctx, gids, const_inlined_nodes_and_contracts =
      LIP.instantiate_polymorphic_nodes inlined_global_ctx gids
        const_inlined_nodes_and_contracts
    in

    (* Step 17. Flatten refinement types *)
    let const_inlined_type_and_consts =
      LFR.flatten_ref_types inlined_global_ctx const_inlined_type_and_consts
    in
    let const_inlined_nodes_and_contracts =
      LFR.flatten_ref_types inlined_global_ctx const_inlined_nodes_and_contracts
    in

    (* Step 18. Check no quantified variable in argument of non-inlinable function *)
    let inlinable_funcs =
      LUF.inlinable_functions inlined_global_ctx
        const_inlined_nodes_and_contracts
    in
    let* warnings5 =
      LS.no_quant_vars_in_calls_to_non_inlinable_funcs inlinable_funcs
        declarations
    in
        ...
        List.map
          (fun warning ->
            if Flags.lus_strict () then
              fail_at_position
                (LW.warning_position warning)
                (LW.warning_message warning)
            else (
              warn_at_position
                (LW.warning_position warning)
                (LW.warning_message warning);
...
let print_nodes_and_globals nodes globals =
  Debug.parse
   ...
   ^^ "===============================================\n")
    ...
    (pp_print_list
       (pp_print_pair StateVar.pp_print_state_var
          (pp_print_list LustreExpr.pp_print_bound_or_fixed ";@ ")
          " = ")
       ";@ ")
    (StateVar.StateVarHashtbl.fold
       (fun k v acc -> (k, v) :: acc)
       globals.LG.state_var_bounds [])
    (pp_print_list LustreNode.pp_print_node_debug ";@ ")
    nodes
    (pp_print_list LustreNode.pp_print_state_var_instances_debug ";@ ")
    nodes
    (pp_print_list LustreNode.pp_print_state_var_defs_debug ";@ ")
    nodes
    (pp_print_list StateVar.pp_print_state_var_debug ";@ ")
    (nodes
    |> List.map (fun n -> LustreNode.get_all_state_vars n @ n.oracles)
    |> List.flatten)
...
let of_channel old_frontend only_parse in_ch =
  ...
  if old_frontend then Log.log L_note "Old front-end enabled";

  if only_parse then
    if old_frontend then
      let _ = LD.declarations_to_nodes declarations in
      Ok None
    else
      match type_check declarations with Ok _ -> Ok None | Error e -> Error e
  else
    let result =
      if old_frontend then
        ...
        let nodes, globals, main_nodes =
          ...
          | Some _ ->
              raise
                (NoMainNode
                   "Flag --lus_main_type not supported in old front-end")
        ...
        let main_nodes =
          ...
          | Some s -> (
              ...
              try
                ...
                let main_nodes =
                  if
                    (not main_lustre_node.is_extern)
                    && List.mem `CONTRACTCK (Flags.enabled ())
                  then [ LustreIdent.mk_string_ident (LGI.contract_tag ^ s) ]
                  else [ s_ident ]
                in
                let main_nodes =
                  if
                    Flags.Contracts.check_environment ()
                    && List.mem `CONTRACTCK (Flags.enabled ())
                  then
        ...
        let main_nodes =
          ...
          | Some s -> (
              ...
              if not (List.mem `CONTRACTCK (Flags.enabled ())) then
                let msg =
                  Format.asprintf
                    "Option --lus_main_type can only be used when \
                     realizability checking is enabled (--enable CONTRACTCK)"
                in
                raise (MainTypeWithoutRealizability msg)
              else
                try
                  let s_ident =
                    LustreIdent.mk_string_ident (LGI.type_tag ^ s)
                  in
                  let _ = LN.node_of_name s_ident nodes in
                  match Flags.lus_main () with
                ...
                with Not_found ->
                  let msg =
                    Format.asprintf
                      "No refinement type with alias '%a' found in input"
                      (LustreIdent.pp_print_ident false)
                      s_ident
                  in
                  raise (NoMainNode msg))
    ...
    | Ok (nodes, globals, main_nodes) ->
        let nodes =
          List.map
            (fun ({ LustreNode.name } as n) ->
              if List.exists (fun id -> LustreIdent.equal id name) main_nodes
              then { n with is_main = true }
              else n)
            nodes
        in
...
let ast_of_file filename =
  ...
  let in_ch = match filename with "" -> stdin | _ -> open_in filename in
