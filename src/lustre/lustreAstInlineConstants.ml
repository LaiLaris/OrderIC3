...
type error = [ `LustreAstInlineConstantsError of Lib.position * error_kind ]

let error_message kind =
  match kind with
  | Unknown s -> s
  | FreeIntIdentifier i ->
      "Cannot evaluate free constant '"
      ^ HString.string_of_hstring i
      ^ "' to an int constant"
  | ConstantMustBeInt e ->
      "Cannot evaluate non-int constant '" ^ LA.string_of_expr e
      ^ "' to an int constant"
  | UnaryMustBeInt e ->
      "Cannot evaluate non-int unary expression '" ^ LA.string_of_expr e
      ^ "' to an int constant"
  | BinaryMustBeInt e ->
      "Cannot evaluate non-int binary expression '" ^ LA.string_of_expr e
      ^ "' to an int constant"
  | FreeBoolIdentifier i ->
      "Cannot evaluate free constant '"
      ^ HString.string_of_hstring i
      ^ "' to a bool constant"
  | ConstantMustBeBool e ->
      "Cannot evaluate non-bool constant '" ^ LA.string_of_expr e
      ^ "' to a bool constant"
  | UnaryMustBeBool e ->
      "Cannot evaluate non-bool unary expression '" ^ LA.string_of_expr e
      ^ "' to a bool constant"
  | BinaryMustBeBool e ->
      "Cannot evaluate non-bool binary expression '" ^ LA.string_of_expr e
      ^ "' to a bool constant"
  | IdentifierMustBeConstant i ->
      "Not a constant identifier '" ^ HString.string_of_hstring i ^ "'"
  | UnableToEvaluate e ->
      "Cannot evaluate expression '" ^ LA.string_of_expr e ^ "'"
...
let int_value_of_const : LA.expr -> (int, [> error ]) result = function
  | LA.Const (_, LA.Num n) ->
      R.ok (n |> HString.string_of_hstring |> int_of_string)
  | e -> inline_error (LH.pos_of_expr e) (ConstantMustBeInt e)

let bool_value_of_const : LA.expr -> (bool, [> error ]) result = function
...
let rec is_normal_form : TC.tc_context -> LA.expr -> bool =
 fun ctx -> function
  | Const _ -> true
  | RecordExpr (_, _, _, id_exprs) ->
      List.for_all (fun (_, e) -> is_normal_form ctx e) id_exprs
  | RecordProject (_, e, _) | TupleProject (_, e, _) -> is_normal_form ctx e
  | _ -> false

(** try and evalutate expression to int, return error otherwise *)
let rec eval_int_expr : TC.tc_context -> LA.expr -> (int, [> error ]) result =
 fun ctx -> function
  | LA.Ident (pos, i) -> (
      match TC.lookup_const ctx i with
      | Some (const_expr, _, _) -> (
          if is_normal_form ctx const_expr then int_value_of_const const_expr
          else
            match const_expr with
            | LA.Ident (_, i') as e ->
                if HString.compare i i' = 0 then
                  inline_error pos (FreeIntIdentifier i)
  ...
  | LA.TernaryOp (pos, top, e1, e2, e3) ->
      eval_int_ternary_op ctx pos top e1 e2 e3
...
and eval_bool_expr : TC.tc_context -> LA.expr -> (bool, [> error ]) result =
 fun ctx -> function
  | LA.Ident (pos, i) -> (
      match TC.lookup_const ctx i with
      | Some (const_expr, _, _) -> (
          if is_normal_form ctx const_expr then bool_value_of_const const_expr
          else
            match const_expr with
            | LA.Ident (_, i') as e ->
                if HString.compare i i' = 0 then
                  inline_error pos (FreeBoolIdentifier i)
...
and eval_bool_ternary_op :
 ...
 fun ctx _ top b1 e1 e2 ->
  ...
  eval_bool_expr ctx e1 >>= fun v1 ->
  eval_bool_expr ctx e2 >>= fun v2 ->
  match top with LA.Ite -> if c then R.ok v1 else R.ok v2

(** try and evalutate ternary op expression to int, return error otherwise *)
and eval_int_ternary_op :
    TC.tc_context ->
    Lib.position ->
    LA.ternary_operator ->
    LA.expr ->
 ...
 fun ctx _ top b1 e1 e2 ->
  ...
  | LA.Ite ->
      eval_bool_expr ctx b1 >>= fun c ->
      if c then eval_int_expr ctx e1 else eval_int_expr ctx e2
...
and simplify_array_index :
    TC.tc_context -> Lib.position -> LA.expr -> LA.expr -> LA.expr =
 fun ctx pos e1 idx ->
...
and simplify_tuple_proj :
    TC.tc_context -> Lib.position -> LA.expr -> int -> LA.expr =
 fun ctx pos e1 idx ->
  match simplify_expr ctx e1 with
  | LA.GroupExpr (_, _, es) ->
      if List.length es > idx then List.nth es idx
...
(** Assumptions: These constants are arranged in dependency order, all of the
    constants have been type checked *)
and simplify_expr ?(is_guarded = false) ctx = function
  | LA.Const _ as c -> c
  | LA.Ident (_, i) as ident -> (
      ...
      | Some (const_expr, _, _) -> (
          match const_expr with
          | LA.Ident (_, i') as ident' ->
              if HString.compare i i' = 0 (* If This is a free constant *) then
                ident
  ...
  | LA.UnaryOp (pos, op, e1) -> (
      ...
      match op with
      | LA.Uminus -> (
          match eval_int_unary_op ctx pos op e1' with
          | Ok v ->
              LA.Const (pos, Num (v |> string_of_int |> HString.mk_hstring))
          | Error _ -> e')
      | LA.Not -> (
          match eval_bool_unary_op ctx pos op e1' with
  ...
  | LA.Pre (pos, e) ->
      ...
      else if Flags.lus_push_pre () then push_pre is_guarded pos e'
      else Pre (pos, e')
  | Arrow (pos, e1, e2) ->
      let e1' = simplify_expr ~is_guarded ctx e1 in
  ...
  | LA.TernaryOp (pos, top, cond, e1, e2) -> (
      ...
      | Ite -> (
          ...
          | Ok v ->
              if v then simplify_expr ~is_guarded ctx e1
  ...
  | LA.RecordExpr (pos, i, ps, fields) ->
      let fields' =
        List.map (fun (f, e) -> (f, simplify_expr ~is_guarded ctx e)) fields
      in
      LA.RecordExpr (pos, i, ps, fields')
...
let rec inline_constants_of_lustre_type ctx ty =
  match ty with
  | LA.IntRange (pos, lbound, ubound) ->
      let lbound' =
        match lbound with
        | None -> None
        | Some lbound -> Some (simplify_expr ctx lbound)
      in
      let ubound' =
        match ubound with
        | None -> None
        | Some ubound -> Some (simplify_expr ctx ubound)
      in
      LA.IntRange (pos, lbound', ubound')
  | LA.TupleType (pos, types) ->
      let types' =
        List.map (fun t -> inline_constants_of_lustre_type ctx t) types
      in
      LA.TupleType (pos, types')
  ...
  | LA.RecordType (pos, name, types) ->
      let types' =
        List.map
          (fun (p, i, t) -> (p, i, inline_constants_of_lustre_type ctx t))
          types
      in
      LA.RecordType (pos, name, types')
  ...
  | RefinementType (pos, (pos2, id, ty), expr) ->
      let ty' = inline_constants_of_lustre_type ctx ty in
      RefinementType (pos, (pos2, id, ty'), expr)
  | History _ | Int _ | Bool _ | UInt8 _ | UInt16 _ | UInt32 _ | UInt64 _
  | Int8 _ | Int16 _ | Int32 _ | Int64 _ | Real _ | UserType _ | AbstractType _
  | EnumType _ ->
      ty

let inline_constants_of_node_equation :
    TC.tc_context -> LA.node_equation -> LA.node_equation =
 fun ctx -> function
...
let rec inline_constants_of_node_locals ctx = function
  ...
  | LA.NodeVarDecl (pos, decl) :: t ->
      let decl' =
        inline_constants_of_clocked_type_decl ctx [ decl ] |> List.hd
      in
      let ctx', t' = inline_constants_of_node_locals ctx t in
      (ctx', LA.NodeVarDecl (pos, decl') :: t')

let rec inline_constants_of_node_items :
    TC.tc_context -> LA.node_item list -> LA.node_item list =
 fun ctx -> function
  | [] -> []
  | Body b :: items ->
  ...
  | IfBlock _ :: _ | FrameBlock _ :: _ -> assert false
...
let rec inline_constants_of_contract :
    TC.tc_context ->
...
let substitute :
    TC.tc_context -> LA.declaration -> TC.tc_context * LA.declaration =
 fun ctx -> function
  ...
  | LA.NodeDecl
      ...
      ( ctx,
        LA.NodeDecl
          ( span,
            (i, imported, opac, params, ips', ops', ldecls', items', contract')
          ) )
  | LA.FuncDecl
      (span, (i, imported, opac, params, ips, ops, ldecls, items, contract)) ->
      let ips' = inline_constants_of_const_clocked_type_decl ctx ips in
  ...
  | LA.ContractNodeDecl (span, (i, params, ips, ops, (p, contract))) ->
      ( ctx,
        LA.ContractNodeDecl
          ( span,
            (i, params, ips, ops, (p, inline_constants_of_contract ctx contract))
          ) )
  | e -> (ctx, e)

(** Best effort at inlining constants *)
let rec inline_constants :
    TC.tc_context -> LA.t -> (TC.tc_context * LA.t, [> error ]) result =
 fun ctx decl ->
  match decl with
  | [] -> R.ok (ctx, [])
  | c :: rest ->
      let* ctx', c' =
        try R.ok (substitute ctx c)
        with Out_of_bounds (pos, err) -> inline_error pos (OutOfBounds err)
      in
