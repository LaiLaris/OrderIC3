...
type warning_kind = UnguardedPreWarning of A.expr | UseOfAssertionWarning

let warning_message warning =
  match warning with
  | UnguardedPreWarning expr ->
      "Unguarded pre in expression " ^ A.string_of_expr expr
  | UseOfAssertionWarning ->
      "Assertions are not checked; please consider using a contract assumption \
       instead"
...
module AstCallHash = struct
  ...
  let hash (i, c, r, a, d) =
    ...
    let d_hash =
      match d with Some l -> List.map (AH.hash (Some 6)) l | None -> [ 0 ]
    in
...
module LocalHash = struct
  ...
  let equal x y =
    match AH.syn_expr_equal (Some 6) x y with Ok true -> true | _ -> false
...
module NodeArgHash = struct
  ...
  let equal x y =
    match AH.syn_expr_equal None x y with Ok true -> true | _ -> false
...
let pp_print_generated_identifiers ppf gids =
  ...
  let array_ctor_list =
    StringMap.bindings gids.array_constructors
    |> List.map (fun (x, (y, z, w)) -> (x, y, z, w))
  in
  let contract_calls_list =
    StringMap.bindings gids.contract_calls
    |> List.map (fun (x, (y, z, w)) -> (x, y, z, w))
  in
  let pp_print_local ppf (id, ty) =
    Format.fprintf ppf "(%a, %a)" HString.pp_print_hstring id
      LustreAst.pp_print_lustre_type ty
  in
  let pp_print_array_ctor ppf (id, ty, e, s) =
    Format.fprintf ppf "(%a, %a, %a, %a)" HString.pp_print_hstring id
      LustreAst.pp_print_lustre_type ty LustreAst.pp_print_expr e
      LustreAst.pp_print_expr s
  in
  let pp_print_node_arg ppf (id, b, ty, e) =
    Format.fprintf ppf "(%a, %b, %a, %a)" HString.pp_print_hstring id b
      LustreAst.pp_print_lustre_type ty LustreAst.pp_print_expr e
  in
  let pp_print_oracle ppf (id, ty, e) =
    Format.fprintf ppf "(%a, %a, %a)" HString.pp_print_hstring id
      LustreAst.pp_print_lustre_type ty LustreAst.pp_print_expr e
  in
  let pp_print_call =
   fun ppf (pos, output, cond, restart, ident, args, defaults, inlined) ->
    Format.fprintf ppf "%a: %a = call(%a,(restart %a every %a)(%a),%a)%s"
      pp_print_position pos HString.pp_print_hstring output A.pp_print_expr cond
      HString.pp_print_hstring ident A.pp_print_expr restart
      (pp_print_list A.pp_print_expr ",@ ")
      args
      (pp_print_option (pp_print_list A.pp_print_expr ",@"))
      defaults
      (if inlined then " %inlined" else "")
  in
  let pp_print_source ppf source =
    Format.fprintf ppf
      (match source with
      | Local -> "local"
      | Input -> "input"
  ...
  let pp_print_subrange_constraint ppf (source, _, is_original, pos, id, rexpr)
      =
    Format.fprintf ppf "(%a, %b, %a, %a, %a)" pp_print_source source is_original
      Lib.pp_print_position pos HString.pp_print_hstring id A.pp_print_expr
      rexpr
  in
  let pp_print_refinement_type_constraint ppf (source, pos, id, rexpr) =
    Format.fprintf ppf "(%a, %a, %a, %a)" pp_print_source source
      Lib.pp_print_position pos HString.pp_print_hstring id A.pp_print_expr
      rexpr
  in
  let pp_print_contract_call ppf (ref, pos, scope, decl) =
    Format.fprintf ppf "%a := (%a, %a): %a" HString.pp_print_hstring ref
      pp_print_position pos
      (pp_print_list
         (pp_print_pair Lib.pp_print_position HString.pp_print_hstring ":")
         "::")
      scope
      (pp_print_list A.pp_print_contract_item ";")
      decl
  in
  let pp_print_equation ppf (_, _, lhs, expr, _) =
    Format.fprintf ppf "%a = %a" A.pp_print_eq_lhs lhs A.pp_print_expr expr
  in
...
let compute_node_input_constant_mask decls =
  let over_decl map = function
    ...
    | _ -> map
  in
  List.fold_left over_decl StringMap.empty decls
...
let union_list ids = List.fold_left (fun x y -> union x y) (empty ()) ids
...
let expr_has_inductive_var ind_vars expr =
  match get_inductive_vars ind_vars expr with [] -> false | _ -> true
...
let extract_array_size_opt = function
  | A.ArrayType (_, (_, size)) -> (
      match size with
      | A.Const (_, Num x) ->
          Some (x |> HString.string_of_hstring |> int_of_string)
      | _ -> None)
  | _ -> None

let extract_array_size t =
  match extract_array_size_opt t with Some s -> s | None -> assert false
...
let get_inline_func_expr inlinable_funcs name args =
  let (_, _, _, _, inputs, _, _, items, _) : A.node_decl =
    match StringMap.find_opt name inlinable_funcs with
    | Some nd -> nd
    | None -> assert false
  in
  let var_map =
    ...
    |> List.fold_left
         (fun acc item ->
           ...
           | A.Body (Equation (_, StructDef (_, [ lhs ]), rhs)) -> (
               ...
               | ArrayDef _ -> assert false (* rejected earlier in pipeline *)
               | TupleStructItem _ | TupleSelection _ | FieldSelection _
               | ArraySliceStructItem _ ->
           ...
           | A.Body (Equation (_, StructDef (_, _), _)) ->
               assert
                 false (* rejected earlier in pipeline, should we support it? *))
         []
  in
  let input_map = List.map2 (fun (_, id, _, _, _) e -> (id, e)) inputs args in
  match var_map with
  ...
  let gids =
    {
      ...
      equations =
        [ (info.quantified_variables, info.contract_scope, eq_lhs, expr, None) ];
    }
  in
  (nexpr, gids)
...
let mk_range_expr ctx node_id expr_type expr =
  let rec mk ctx n expr_type expr =
    let expr_type =
      Chk.expand_type_syn_reftype_history ctx expr_type |> unwrap
    in
    match expr_type with
    | A.IntRange (_, l, u) -> (
        let original_ty, _ = Chk.infer_type_expr ctx node_id expr |> unwrap in
        let original_ty =
          Chk.expand_type_syn_reftype_history ctx original_ty |> unwrap
        in
        let user_prop, is_original =
          match original_ty with
          | A.IntRange (_, l', u') ->
              let eval_int_expr_opt expr =
                match expr with
              ...
              let is_original =
                ...
                let l', u' = (eval_int_expr_opt l', eval_int_expr_opt u') in
                match (l, u, l', u') with
                | Some (Ok l), Some (Ok u), Some (Ok l'), Some (Ok u') ->
                    l = l' && u = u'
              ...
              let user_prop =
                if is_original then []
                  ...
                  | Some l', None ->
                      [ (A.CompOp (dpos, A.Lte, l', expr), true) ]
                  | None, Some u' ->
                      [ (A.CompOp (dpos, A.Lte, expr, u'), true) ]
    ...
    | A.ArrayType (_, (ty, upper_bound)) ->
        let id_str =
          HString.concat2 (HString.mk_hstring "x")
            (HString.mk_hstring (string_of_int n))
        in
        ...
        let l =
          A.CompOp
            (dpos, A.Lte, A.Const (dpos, A.Num (HString.mk_hstring "0")), id)
        in
        ...
        List.map
          (fun (e, is_original) ->
            (A.Quantifier (dpos, A.Forall, [ var ], body e), is_original))
          rexpr
    | TupleType (_, tys) ->
        let mk_proj i = A.TupleProject (dpos, expr, i) in
        let tys =
          List.filter (fun ty -> Ctx.type_contains_subrange ctx ty) tys
        in
    ...
    | RecordType (_, _, tys) ->
        ...
        let tys =
          List.filter (fun (_, _, ty) -> Ctx.type_contains_subrange ctx ty) tys
        in
...
let mk_enum_range_expr ctx node_id expr_type expr =
  let rec mk ctx n expr_type expr =
    ...
    | A.EnumType (_, _, values) -> (
        let eqs =
          List.map
            (fun v -> A.CompOp (dpos, A.Eq, expr, A.Ident (dpos, v)))
            values
        in
        match eqs with
        | [] -> assert false
        ...
        | e :: eqs ->
            let disj =
              List.fold_left
                (fun acc e' -> A.BinaryOp (dpos, A.Or, acc, e'))
                e eqs
            in
            [ (disj, true) ])
    | A.IntRange (_, l, u) -> (
    ...
    | TupleType (_, tys) ->
        let mk_proj i = A.TupleProject (dpos, expr, i) in
        let tys =
          List.filter (fun ty -> Ctx.type_contains_enum_or_subrange ctx ty) tys
        in
        let tys = List.mapi (fun i ty -> mk ctx n ty (mk_proj i)) tys in
        List.fold_left ( @ ) [] tys
    | RecordType (_, _, tys) ->
        ...
        let tys =
          List.filter
            (fun (_, _, ty) -> Ctx.type_contains_enum_or_subrange ctx ty)
            tys
        in
        let tys = List.map (fun (_, i, ty) -> mk ctx n ty (mk_proj i)) tys in
        List.fold_left ( @ ) [] tys
  ...
  let gids =
    List.map
      (fun (range_expr, is_original) ->
        ...
        let nexpr = A.Ident (pos, name) in
        let eq_lhs, _ =
          generalize_to_array_expr name StringMap.empty range_expr nexpr
        in
        let gids =
          {
            (empty ()) with
...
let rec mk_ref_type_expr :
    Ctx.tc_context -> A.expr -> A.lustre_type -> A.expr list =
 fun ctx id ty ->
  ...
  | TupleType (pos, tys) | GroupType (pos, tys) ->
      List.mapi
        (fun i ty -> mk_ref_type_expr ctx (A.TupleProject (pos, id, i)) ty)
        tys
      |> List.flatten
  | RecordType (p, _, tis) ->
      List.map
  ...
  | ArrayType (_, (ty, len)) ->
      ...
      let exprs =
        mk_ref_type_expr ctx
          (A.ArrayIndex (pos, id, Ident (pos, dummy_index)))
          ty
      in
      List.map
        (fun expr ->
          ...
          let expr =
            A.BinaryOp (pos, Impl, A.BinaryOp (pos, And, bound1, bound2), expr)
          in
...
let mk_enum_subrange_reftype_constraints node_id info vars =
  ...
  | c :: cs ->
      let conj =
        List.fold_left (fun acc c' -> A.BinaryOp (dpos, A.And, c', acc)) c cs
      in
      Some conj
...
let mk_fresh_oracle expr_type expr =
  ...
  let gids = { (empty ()) with oracles = [ (name, expr_type, expr) ] } in
  (nexpr, name, gids)
...
let mk_fresh_frozen_local node_id info pos ind_vars expr_type =
  ...
  let constraints =
    ...
    let info =
      { info with context = Ctx.add_ty info.context oracle_id expr_type }
    in
  ...
  let gids =
    List.map
      (fun ref_type_expr ->
        ...
        let nexpr = A.Ident (pos, name) in
        let eq_lhs, _ =
          generalize_to_array_expr name StringMap.empty ref_type_expr nexpr
        in
        let gids =
          {
            (empty ()) with
...
let mk_fresh_call ?(inlined = false) info id map pos cond restart ty_args args
    defaults =
  ...
  let check_cache =
    CallCache.find_opt call_cache (id, cond, restart, args, defaults)
  in
  ...
  | _ ->
      ...
      let proj =
        if info.local_group_projection < 0 then HString.mk_hstring ""
        else
          HString.concat2
      ...
      if not has_ty_args then
        CallCache.add call_cache (id, cond, restart, args, defaults) nexpr;
      (nexpr, gids)

(** Add local step 'counter' and an equation setting counter = 0 -> pre counter
    \+ 1 *)
...
let get_type_of_id info node_id id =
  let ty =
    match
      ( AI.get_type info.abstract_interp_context node_id id,
        Ctx.lookup_ty info.context id |> get )
    with
    | _, (RefinementType _ as ty) ->
        ty (* don't discard refinement type in favor of inferred subrange *)
    | Some ty, _ -> ty
    | None, ty -> ty
  in
...
let should_not_abstract info force = function
  | A.Ident (_, id) -> (not (Ctx.is_enum_variant info.context id)) && not force
  | _ -> false
...
let add_subrange_constraints info node_id kind vars =
  ...
  |> List.fold_left
       (fun acc (p, id) ->
         ...
         union acc
           (mk_fresh_subrange_constraint kind info p (Some node_id) id ty))
       (empty ())

let add_ref_type_constraints info kind vars =
  vars
  |> List.filter (fun (_, _, ty) -> Ctx.type_contains_ref info.context ty)
  |> List.fold_left
       (fun acc (p, id, ty) ->
         let ty = AIC.inline_constants_of_lustre_type info.context ty in
         union acc
           (mk_fresh_refinement_type_constraint kind info p
              (A.Ident (p, id))
              ty))
       (empty ())
      ...
      let prev_hist =
        A.Arrow
          ( dpos,
            A.Ident (dpos, id),
            A.ArrayIndex
              (dpos, A.Pre (dpos, A.Ident (dpos, h_id)), A.Ident (dpos, index))
          )
      in
...
let normalize_list f list =
  ...
  in
  let list, gids, warnings = List.fold_left over_list ([], empty (), []) list in
  (List.rev list, gids, warnings)

(** [desugar_history_in_expr c p e] desugars type constructors of the form
    history(x) occurring in [e] using [c] for the name of the counter variable
    and [p] as a prefix for the name of the history variables. It returns the
    set of variables passed as argument to the type constructors history and an
    expression that is the result of desugaring the type constructors ocurring
    in [e] *)
let desugar_history_in_expr ctx ctr_id prefix expr =
  ...
  let rec r map expr =
    ...
    | A.Quantifier (pos, kind, idents, e) ->
            ...
            (fun (vars, map, idents, constrs) (pos, bv, ty) ->
              ...
              | A.History (_, i) -> (
                  ...
                  | Exists ->
                      ...
                      let bound_ty =
                        A.RefinementType (pos, (pos, bv, A.Int pos), rng)
                      in
                  ...
                  | Forall ->
                      ...
                      let c =
                        ...
                        let bound_ty =
                          A.RefinementType
                            (pos, (pos, idx_varid, A.Int pos), rng)
                        in
                        let eq =
                          ...
                          let rhs =
                            A.ArrayIndex
                              ( pos,
                                Ident (pos, hist_varid),
                                Ident (pos, idx_varid) )
                          in
                          A.CompOp (pos, A.Eq, lhs, rhs)
                        in
                        A.Quantifier
                          (pos, A.Exists, [ (pos, idx_varid, bound_ty) ], eq)
                      in
        ...
        in
        (StringSet.union vars vars', Quantifier (pos, kind, List.rev idents, e'))
    | Ident (pos, id) -> (
    ...
    | BinaryOp (pos, op, e1, e2) ->
        ...
        (StringSet.union vars1 vars2, BinaryOp (pos, op, e1', e2'))
    ...
    | CompOp (pos, op, e1, e2) ->
        let vars1, e1' = r map e1 in
        let vars2, e2' = r map e2 in
        (StringSet.union vars1 vars2, CompOp (pos, op, e1', e2'))
    ...
    | StructUpdate (pos, e1, idx_list, e2) ->
        let vars1, e1' = r map e1 in
        let vars2, e2' = r map e2 in
        (StringSet.union vars1 vars2, StructUpdate (pos, e1', idx_list, e2'))
    | ArrayConstr (pos, e1, e2) ->
        let vars1, e1' = r map e1 in
        let vars2, e2' = r map e2 in
        (StringSet.union vars1 vars2, ArrayConstr (pos, e1', e2'))
    | ArrayIndex (pos, e1, e2) ->
        let vars1, e1' = r map e1 in
        let vars2, e2' = r map e2 in
        (StringSet.union vars1 vars2, ArrayIndex (pos, e1', e2'))
    | When (pos, e, c) ->
        let vars, e' = r map e in
        (vars, When (pos, e', c))
    | Pre (pos, e) ->
    ...
    | Arrow (pos, e1, e2) ->
        let vars1, e1' = r map e1 in
        let vars2, e2' = r map e2 in
        (StringSet.union vars1 vars2, Arrow (pos, e1', e2'))
    ...
    | RestartEvery (pos, ident, expr_list, e) ->
        ...
        (StringSet.union vars1 vars2, RestartEvery (pos, ident, expr_list', e'))
  and desugar_expr_list map expr_list =
    let vars, expr_list' =
      expr_list |> List.map (fun e -> r map e) |> List.split
    in
    ( List.fold_left
  ...
  List.fold_left
    (fun acc decl ->
      ...
      | A.FuncDecl (_, nd) ->
          ...
          if A.SI.mem id inlinable_funcs then StringMap.add id nd acc else acc
      | _ -> acc)
...
let rec normalize ctx ai_ctx inlinable_funcs (decls : LustreAst.t) gids =
  ...
  let over_declarations (nitems, accum, warnings_accum) item =
    clear_cache ();
    let normal_item, accum, warnings = normalize_declaration info accum item in
    ( (match normal_item with Some ni -> ni :: nitems | None -> nitems),
      accum,
  ...
  Debug.parse
   ...
   ^^ "===============================================\n")
    (pp_print_list
       (pp_print_pair HString.pp_print_hstring pp_print_generated_identifiers
          ":")
       "\n")
    (StringMap.bindings map) A.pp_print_program ast;
...
and normalize_gid_equations info gids_map node_id =
  ...
  | Some gids ->
      ...
      let gids_list, warnings, eqs =
        List.map
          (fun (tis, sc, lhs, expr, source) ->
            match source with
            (* Generated equations created during the normalization step don't need to be normalized *)
            | None -> (empty (), [], (tis, sc, lhs, expr, source))
         ...
         info.interpretation and info.contract_scope *)
            | Some Ghost ->
                let nexpr, gids, warnings =
                  normalize_expr info node_id gids_map expr
                in
                ( gids,
                  warnings,
                  ( info.quantified_variables,
...
and normalize_node_contract info node_id map cref inputs outputs
    (id, _, ivars, ovars, body) =
  (* Normalize types *)
  let ivars, gids1, warnings1 =
    List.map
      (fun (p, id, ty, cl, c) ->
  ...
  let ctx =
    List.fold_left
      (fun c (id, ty) -> Ctx.add_ty c id ty)
      info.context
      (Ctx.IMap.bindings type_exports)
  in
  let ctx =
    List.fold_left (fun c (_, id, ty, _, _) -> Ctx.add_ty c id ty) ctx ivars
  in
  let ctx =
    List.fold_left (fun c (_, id, ty, _) -> Ctx.add_ty c id ty) ctx ovars
  in
  let info =
    { info with context = ctx; interpretation = interp; contract_ref }
  in
  ...
  *)
  ...
  in
  let nbody, gids7, _, warnings3 =
    normalize_contract info node_id map ivars ovars body
  in
  let gids =
    List.fold_left union (empty ())
      [ union_list gids1; union_list gids2; gids3; gids4; gids5; gids6; gids7 ]
  in
  ( nbody,
    gids,
    List.flatten (warnings1 @ warnings2) @ warnings3,
    StringMap.empty )
...
and normalize_ghost_declaration info node_id map = function
  | A.UntypedConst (pos, id, expr) ->
      ...
      let nexpr, map, warnings =
        normalize_expr ?guard:None info node_id map expr
      in
  ...
  | TypedConst (pos, id, expr, ty) ->
      ...
      let new_id = StringMap.find id info.interpretation in
      let nexpr, map2, warnings2 =
        normalize_expr ?guard:None info node_id map expr
      in
      ( A.TypedConst (pos, new_id, nexpr, ty),
        union map1 map2,
        warnings1 @ warnings2 )
  | FreeConst (pos, id, ty) ->
...
and normalize_node info map
  ...
  let ncontracts, gids6, interpretation, warnings4 =
    match contract with
    | Some contract ->
        let ctx =
          Chk.tc_ctx_of_contract info.context Ghost node_id contract
          |> unwrap |> fst
        in
  ...
  let gids7 =
    ...
    |> List.filter (function
         ...
         | A.NodeConstDecl (_, FreeConst _) | A.NodeConstDecl (_, UntypedConst _)
           ->
             false)
    |> List.fold_left
         (fun acc l ->
           ...
           | A.NodeConstDecl (p, TypedConst (_, id, _, _)) ->
               ...
               let gids =
                 union acc
                   (mk_fresh_subrange_constraint Local info p (Some node_id) id
                      ty)
               in
               union gids
                 (mk_fresh_refinement_type_constraint Local info p
                    (A.Ident (p, id))
                    ty)
           | A.NodeConstDecl (_, FreeConst _)
           | A.NodeConstDecl (_, UntypedConst _) ->
               assert false)
         (empty ())
  in
  let items =
    if Flags.check_reach () then items
    else
      items
      |> List.filter (function
  ...
  let nitems, gids8, warnings5 =
    normalize_list (normalize_item info node_id map) items
  in
  let gids6_8 = union gids6 gids8 in
  let gids9 =
    if
      exists_reachability_prop_with_bounds
      || not (StringMap.is_empty gids6_8.history_vars)
    then add_step_counter info
    else empty ()
  in
  let gids10 =
    StringMap.fold
      (fun id h_id acc -> union acc (add_history_var_and_equation info id h_id))
      gids6_8.history_vars (empty ())
  in
  let new_gids =
    union_list
      [
  ...
  in
  let old_gids, warnings6 =
    normalize_gid_equations { info with interpretation } map node_id
  in
...
and normalize_item info node_id map = function
  | A.Body equation ->
      let nequation, gids, warnings =
        normalize_equation info node_id map equation
      in
      ([ A.Body nequation ], gids, warnings)
  (* shouldn't be possible *)
  | IfBlock _ | FrameBlock _ -> assert false
  ...
  | AnnotProperty (pos, name, expr, k) ->
      ...
      let decls, gids, warnings =
        ...
        | Reachable (Some (From b)) ->
            ...
            in
            let nexpr, gids, warnings =
              abstract_expr false info node_id map expr
            in
                    ...
                    ( pos,
                      ...
                      A.CompOp
                        ( pos,
                          ...
                          Ident (dpos, ctr_id),
                          Const
                            (dpos, Num (HString.mk_hstring (string_of_int b1)))
                        ),
                      A.CompOp
                        ( pos,
                          A.Gt,
                          Ident (dpos, ctr_id),
                          Const
                            (dpos, Num (HString.mk_hstring (string_of_int b2)))
                        ) ) )
            in
            let nexpr, gids, warnings =
              abstract_expr false info node_id map expr
        ...
        | Provided expr2 ->
            ...
            let nexpr1, gids1, warnings1 =
              abstract_expr false info node_id map expr1
            in
            ...
            if Flags.check_nonvacuity () then
              ...
              let nexpr2, gids2, warnings2 =
                abstract_expr false info node_id map expr2
              in
              let name'', gids2 =
                match name' with
                | Some name ->
                    let name'' =
                      HString.concat2 (HString.mk_hstring "Guard of ") name
                    in
                    ( Some name'',
                      {
                        gids2 with
                        nonvacuity_props =
                          StringSet.add name'' gids2.nonvacuity_props;
                      } )
                | None -> (None, gids2)
              in
              ( [
...
and rename_ghost_variables info contract =
  ...
  | ( A.GhostConst (UntypedConst (_, id, _))
    | GhostConst (TypedConst (_, id, _, _)) )
    :: t ->
  ...
  | GhostVars (pos1, A.GhostVarDec (pos2, (_, id, ty) :: tis), e) :: t ->
      ...
      let info = { info with context = Ctx.add_ty info.context new_id ty } in
      let tail, info =
        rename_ghost_variables info
          (A.GhostVars (pos1, A.GhostVarDec (pos2, tis), e) :: t)
      in
      (StringMap.singleton id new_id :: tail, info)
...
and normalize_contract info node_id map ivars ovars (p, items) =
  ...
  let ghost_interp =
    List.fold_left (StringMap.merge union_keys) StringMap.empty ghost_interp
  in
  ...
  for j = 0 to List.length items - 1 do
    ...
    let nitem, gids', warnings', interpretation' =
      match item with
      | Assume (pos, name, soft, expr) ->
          let info, h_gids, expr = desugar_history info expr in
          let nexpr, gids, warnings =
            abstract_expr force_fresh info node_id map expr
          in
          ( A.Assume (pos, name, soft, nexpr),
            union h_gids gids,
            warnings,
            StringMap.empty )
      ...
      | Mode (pos, name, requires, ensures) ->
        ...
        let info = { info with interpretation } in *)
          let over_property info map (pos, name, expr) =
            let info, h_gids, expr = desugar_history info expr in
            let nexpr, gids, warnings =
              abstract_expr true info node_id map expr
            in
            ((pos, name, nexpr), union h_gids gids, warnings)
          in
          let nrequires, gids1, warnings1 =
            normalize_list (over_property info map) requires
          in
          let nensures, gids2, warnings2 =
            normalize_list (over_property info map) ensures
          in
          ( Mode (pos, name, nrequires, nensures),
            union gids1 gids2,
            warnings1 @ warnings2,
            StringMap.empty )
      | ContractCall (pos, name, ty_args, inputs, outputs) ->
          let ninputs, gids1, warnings1 =
            normalize_list (abstract_expr false info node_id map) inputs
          in
          let noutputs =
            List.map
              (fun id ->
                let ty =
                  ...
                  match
                    List.find_opt
                      (fun (_, oid, _, _) -> HString.equal id oid)
                      ovars
                  with
                  | None -> None
              ...
              (fun e ->
                ...
                | A.Ident (_, i) ->
                    let ty =
                      ...
                      match
                        List.find_opt
                          (fun (_, id, _, _, _) -> HString.equal i id)
                          ivars
                      with
                      | None -> None
          ...
          let (_, normalized_call), gids2, warnings2, interp =
            normalize_node_contract info name map cref ninputs noutputs
              called_node
          in
          ...
          let gids =
            {
              gids with
              contract_calls =
                StringMap.add info.contract_ref
                  (pos, info.contract_scope, normalized_call)
                  gids.contract_calls;
            }
          in
          ( ContractCall (pos, cref, ty_args, inputs, outputs),
            gids,
            warnings,
            interp )
      | GhostConst decl ->
          let ndecl, map, warnings =
            normalize_ghost_declaration info node_id map decl
          in
      ...
      | GhostVars (pos, (GhostVarDec (pos2, tis) as lhs), expr) ->
          ...
          let expand =
            array_sizes <> []
            && array_sizes
               |> List.for_all (fun (_, sz) ->
                      match sz with Some _ -> true | None -> false)
          in
          let (nexpr, gids1, warnings), expanded =
            if expand && lhs_arity <> rhs_arity then
              ...
              | Some (ivar, ty) ->
                  ...
                  let expanded_expr =
                    expand_node_calls_in_place info node_id ivar size expr
                  in
                  let exprs, gids, warnings =
                    Lib.split3
                      (List.init lhs_arity (fun i ->
                           let info =
                             { info with local_group_projection = i }
                           in
                           normalize_expr ?guard:None info node_id map
                             expanded_expr))
                  in
                  let gids =
                    List.fold_left (fun acc g -> union g acc) (empty ()) gids
                  in
          ...
          let gids2 =
            if expanded then
              ...
              let ids = List.map (function _, i, _ -> i) items in
                ...
                (fun (pos, i, ty) ->
                  ...
                  then
                    ( (pos, i, ty),
                      union gids
                        (union
                           (mk_fresh_subrange_constraint Ghost info pos
                              (Some node_id) new_id ty)
                           (mk_fresh_refinement_type_constraint Ghost info pos
                              (A.Ident (pos, new_id))
                              ty)),
                      warnings )
                  else ((pos, i, ty), gids, []))
        ...
        List.fold_left
          (fun info item ->
            ...
            | A.ArrayDef (pos, v, is) ->
                ...
                in
                let ty =
                  match Ctx.lookup_ty info.context v with
                  | Some t -> t
                  | None -> assert false
                in
                let ivars =
                  List.fold_left
                    (fun m i -> StringMap.add i ty m)
                    StringMap.empty is
                in
                { info with inductive_variables = ivars }
            | _ -> info)
          info items
      in
      let lhs_arity = List.length items in
...
and abstract_expr ?guard force info node_id map expr =
  ...
  if should_not_abstract info force nexpr then (nexpr, gids1, warnings)
  else
...
and expand_node_call info node_id expr var count =
  let ty =
    Chk.infer_type_expr info.context (Some node_id) expr |> unwrap |> fst
  in
  let mk_index i = A.Const (dpos, Num (HString.mk_hstring (string_of_int i))) in
  let expr_array =
    List.init count (fun i -> AH.substitute_naive var (mk_index i) expr)
  in
  match ty with
  ...
  | _ ->
      List.fold_left
        (fun acc (i, e) ->
          ...
          let cond =
            A.CompOp (pos, A.Eq, A.Ident (pos, var), A.Const (pos, A.Num i))
          in
...
and combine_args_with_const info args flags =
  ...
  let over_args_arity (i, acc) (e, arity) =
    if arity > 1 then (i + arity, (e, false) :: acc)
    else (succ i, (e, List.nth flags i) :: acc)
  in
...
and normalize_expr ?guard info node_id map =
  let abstract_array_literal info expr nexpr =
    ...
    let id = match nexpr with A.Ident (_, name) -> name | _ -> assert false in
    let gids =
      {
        gids with
  ...
  | Call (pos, ty_args, id, args) ->
      let is_inlinable = StringMap.mem id info.inlinable_funcs in
      let info, vmap, gids0 =
      ...
      let handle_call inlined args =
        ...
        let nargs, gids1, warnings =
          normalize_list
            (fun (arg, is_const) ->
              abstract_node_arg ?guard:None false is_const info map arg)
      ...
      in
      if is_inlinable && vmap <> [] then
        let nargs, gids1, warnings1 =
          normalize_list
            (fun arg -> normalize_expr ?guard info node_id map arg)
            args
        in
        let nexpr = get_inline_func_expr info.inlinable_funcs id nargs in
        let args = List.map (fun a -> AH.apply_subst_in_expr vmap a) args in
        let _, gids2, warnings2 = handle_call true args in
        (nexpr, union_list [ gids0; gids1; gids2 ], warnings1 @ warnings2)
      else handle_call false args
  | Condact (pos, cond, restart, id, args, defaults) ->
      let flags = StringMap.find id info.node_is_input_const in
      let ncond, gids1, warnings1 =
        if AH.expr_is_true cond then (cond, empty (), [])
        else abstract_expr ?guard true info node_id map cond
      in
      let nrestart, gids2, warnings2 =
        if AH.expr_is_const restart then (restart, empty (), [])
        else abstract_expr ?guard true info node_id map restart
      in
      let nargs, gids3, warnings3 =
        normalize_list
          (fun (arg, is_const) ->
            abstract_node_arg ?guard:None false is_const info map arg)
          (combine_args_with_const info args flags)
      in
      let ndefaults, gids4, warnings4 =
        normalize_list (normalize_expr ?guard info node_id map) defaults
      in
      let nexpr, gids5 =
        mk_fresh_call info id map pos ncond nrestart [] nargs (Some ndefaults)
      in
  ...
  | RestartEvery (pos, id, args, restart) ->
      ...
      let nrestart, gids1, warnings1 =
        if AH.expr_is_const restart then (restart, empty (), [])
        else abstract_expr ?guard true info node_id map restart
      in
      let nargs, gids2, warnings2 =
        normalize_list
          (fun (arg, is_const) ->
            abstract_node_arg ?guard:None false is_const info map arg)
          (combine_args_with_const info args flags)
      in
      let nexpr, gids3 =
        mk_fresh_call info id map pos cond nrestart [] nargs None
      in
  ...
  | Merge (pos, clock_id, cases) ->
      let normalize' info map ?guard = function
        | clock_value, A.Activate (pos, id, cond, restart, args) ->
            ...
            let ncond, gids1, warnings1 =
              if AH.expr_is_true cond then (cond, empty (), [])
              else abstract_expr ?guard false info node_id map cond
            in
            let nrestart, gids2, warnings2 =
              if AH.expr_is_const restart then (restart, empty (), [])
              else abstract_expr ?guard false info node_id map restart
            in
            let nargs, gids3, warnings3 =
              normalize_list
                (fun (arg, is_const) ->
            ...
            in
            let nexpr, gids4 =
              mk_fresh_call info id map pos ncond nrestart [] nargs None
            in
        ...
        | clock_value, A.Call (pos, _, id, args) ->
            let flags = StringMap.find id info.node_is_input_const in
            let cond_expr =
              match HString.string_of_hstring clock_value with
              ...
              | _ ->
                  A.CompOp
                    ( pos,
                      A.Eq,
                      A.Ident (pos, clock_id),
                      A.Ident (pos, clock_value) )
            in
            let ncond, gids1, warnings1 =
              abstract_expr ?guard false info node_id map cond_expr
            in
            let restart = A.Const (Lib.dummy_pos, A.False) in
            let nargs, gids2, warnings2 =
              normalize_list
            ...
            in
            let nexpr, gids3 =
              mk_fresh_call info id map pos ncond restart [] nargs None
            in
            let gids = union_list [ gids1; gids2; gids3 ] in
            let warnings = warnings1 @ warnings2 in
            ((clock_value, nexpr), gids, warnings)
        | clock_value, expr ->
            ...
            ((clock_value, nexpr), gids, warnings)
      in
      let ncases, gids, warnings =
        normalize_list (normalize' ?guard info map) cases
      in
  ...
  | Arrow (pos, expr1, expr2) ->
      let nexpr1, gids1, warnings1 =
        normalize_expr ?guard info node_id map expr1
      in
      let nexpr2, gids2, warnings2 =
        normalize_expr ?guard:(Some nexpr1) info node_id map expr2
      in
      let gids = union gids1 gids2 in
      let warnings = warnings1 @ warnings2 in
  ...
  | Pre (pos, expr) ->
      ...
      let nexpr, gids1, warnings1 =
        abstract_expr ?guard:None force info node_id map expr
      in
      let guard, gids2, warnings2, previously_guarded =
        match guard with
        | Some guard -> (guard, empty (), [], true)
        | None ->
            let guard, _, gids = mk_fresh_oracle ty nexpr in
            let warnings =
              [ mk_warning pos (UnguardedPreWarning (Pre (pos, expr))) ]
            in
  ...
  | ArrayConstr (pos, expr, size_expr) ->
      ...
      let ivars = info.inductive_variables in
      let iexpr, gids2 =
        mk_fresh_array_ctor info pos ivars ty nexpr size_expr
      in
      (ArrayConstr (pos, iexpr, size_expr), union gids1 gids2, warnings)
  | GroupExpr (pos, ArrayExpr, expr_list) as expr ->
      let nexpr_list, gids1, warnings =
        normalize_list (normalize_expr ?guard:None info node_id map) expr_list
      in
  ...
  | BinaryOp (pos, op, expr1, expr2) ->
      ...
      in
      let nexpr2, gids2, warnings2 =
        normalize_expr ?guard info node_id map expr2
      in
      ( BinaryOp (pos, op, nexpr1, nexpr2),
        union gids1 gids2,
        warnings1 @ warnings2 )
  | TernaryOp (pos, op, expr1, expr2, expr3) ->
      ...
      let nexpr3, gids3, warnings3 =
        normalize_expr ?guard info node_id map expr3
      in
  ...
  | RecordExpr (pos, id, ps, id_expr_list) ->
      ...
      in
      let nid_expr_list, gids, warnings =
        normalize_list (normalize' ?guard info map) id_expr_list
      in
      (RecordExpr (pos, id, ps, nid_expr_list), gids, warnings)
  | GroupExpr (pos, kind, expr_list) ->
      let nexpr_list, gids, warnings =
        normalize_list (normalize_expr ?guard info node_id map) expr_list
      in
  ...
  | StructUpdate (pos, expr1, i, expr2) ->
      ...
      let nexpr2, gids2, warnings2 =
        normalize_expr ?guard info node_id map expr2
      in
      ( StructUpdate (pos, nexpr1, i, nexpr2),
        union gids1 gids2,
        warnings1 @ warnings2 )
  ...
  | Activate (pos, id, expr1, expr2, expr_list) ->
      ...
      let nexpr2, gids2, warnings2 =
        normalize_expr ?guard info node_id map expr2
      in
      let nexpr_list, gids3, warnings3 =
        normalize_list (normalize_expr ?guard info node_id map) expr_list
      in
      let gids = union (union gids1 gids2) gids3 in
      let warnings = warnings1 @ warnings2 @ warnings3 in
...
and expand_node_calls_in_place info node_id var count expr =
  ...
  | Call (p, ty_args, n, expr_list) ->
      let expr_list = List.map (fun e -> r e) expr_list in
      expand_node_call info node_id
        (A.Call (p, ty_args, n, expr_list))
        var count
...
and normalize_ty info node_id map id ty =
  ...
  | UInt64 _ | History _ | Bool _ | Real _ | IntRange _ | UserType _
  | AbstractType _ | TupleType _ | GroupType _ | RecordType _ | ArrayType _
  | EnumType _ | TArr _ ->
      (ty, empty (), [])
