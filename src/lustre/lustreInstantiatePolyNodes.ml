...
let unwrap res = match res with Ok res -> res | Error _ -> assert false

let instantiate_type_variables_ni =
 fun ctx nname ty_args ni ->
  match ni with
...
let instantiate_type_variables_loc =
 fun ctx nname ty_args loc ->
  match loc with
  | A.NodeConstDecl (p, FreeConst (p2, id, ty)) ->
      let ty =
        Chk.instantiate_type_variables ctx p nname ty ty_args |> unwrap
      in
      A.NodeConstDecl (p, FreeConst (p2, id, ty))
  ...
  | Mode (p, id, reqs, enss) ->
      let reqs =
        List.map
          (fun (p, id, expr) ->
            let expr =
              Chk.instantiate_type_variables_expr ctx nname ty_args expr
              |> unwrap
            in
            (p, id, expr))
          reqs
...
let build_node_fun_ty =
 fun pos args rets ->
  ...
  let ret_ty =
    if List.length ops = 1 then List.hd ops else A.GroupType (pos, ops)
  in
  let arg_ty =
    if List.length ips = 1 then List.hd ips else A.GroupType (pos, ips)
  in
...
let rec gen_poly_decl :
    Ctx.tc_context ->
    GI.t GI.StringMap.t ->
    HString.t option ->
    (A.declaration * A.lustre_type list list) HString.HStringMap.t ->
    HString.t ->
    A.lustre_type list ->
    Ctx.tc_context
    * GI.t GI.StringMap.t
 ...
 fun ctx gids caller_nname node_decls_map nname ty_args ->
  ...
  | Some i ->
      let prefix =
        LustreGenRefTypeImpNodes.poly_gen_node_tag ^ string_of_int i ^ "_"
      in
  ...
  | None ->
      let span, is_function, is_contract, ext, opac, ips, ops, locs, nis, c =
        ...
        | ( A.NodeDecl
              (span, (_, ext, opac, __FUNCTION__, ips, ops, locs, nis, c)),
            _ ) ->
      ...
      let nis =
        List.filter
          (fun ni -> match ni with A.AnnotMain _ -> false | _ -> true)
          nis
      in
      (* Instantiate type variables *)
      let nis =
        List.map (instantiate_type_variables_ni ctx nname ty_args) nis
      in
      let locs =
        List.map (instantiate_type_variables_loc ctx nname ty_args) locs
      in
      let c =
        match c with
        | Some (pos, cis) ->
            Some
              ( pos,
                List.map (instantiate_type_variables_ci ctx nname ty_args) cis
              )
        | None -> c
      in
      let ips =
        List.map
          (fun (pos, id, ty, cl, const) ->
      ...
      let ops =
        List.map
          (fun (pos, id, ty, cl) ->
            let ty = Chk.instantiate_type_variables ctx pos nname ty ty_args in
            match ty with Ok ty -> (pos, id, ty, cl) | Error _ -> assert false)
          ops
      in
      ...
      let ps =
        ...
        | Some caller_nname ->
            let ty_vars =
              List.map (Ctx.ty_vars_of_type ctx caller_nname) ty_args
            in
      ...
      let prefix =
        LustreGenRefTypeImpNodes.poly_gen_node_tag
        ^ (List.length tyss |> string_of_int)
        ^ "_"
      in
      let pnname = HString.concat2 (HString.mk_hstring prefix) nname in
      (* Remember new instantiation *)
      let node_decls_map =
        GI.StringMap.add nname (decl, tyss @ [ ty_args ]) node_decls_map
      in
      let ctx, called_decl =
        if is_function then
          ...
          let ctx, _ =
            match Chk.extract_exports pnname ctx c with
            | Ok ctx -> ctx
            | Error _ -> assert false
          in
          (ctx, ContractNodeDecl (span, (pnname, ps, ips, ops, c)))
        else
          let ctx = Ctx.add_ty_vars_node ctx pnname ps in
          let ctx = Ctx.add_node_param_attr ctx pnname ips in
          let ctx = Ctx.add_ty_args_node ctx pnname ty_args in
      ...
      let ctx, gids, decls, node_decls_map =
        match GI.StringMap.find_opt nname gids with
        ...
        | Some polymorphic_gids ->
            ...
            let glocals =
              GI.StringMap.map
                (fun ty ->
                  Chk.instantiate_type_variables ctx Lib.dummy_pos nname ty
                    ty_args
                  |> unwrap)
                polymorphic_gids.locals
            in
            let ib_oracles =
              List.map
                (fun (id, ty) ->
                  ...
                  in
                  (id, ty))
                polymorphic_gids.ib_oracles
            in
            let geqs =
              List.map
                (fun (q_vars, sc, lhs, expr, source) ->
                  let q_vars =
                    List.map
                      (fun (pos, id, ty) ->
                        let ty =
                          Chk.instantiate_type_variables ctx pos nname ty
                            ty_args
                          |> unwrap
                        in
                        (pos, id, ty))
                ...
                polymorphic_gids.equations
            in

            (* Recursively create new polymorphic instantiations, e.g. if the gids contain call M<<int>> *)
            let ctx, gids, decls, glocals, node_decls_map =
              GI.StringMap.fold
                (fun id ty
                     (ctx, gids, acc_decls, acc_glocals, acc_node_decls_map) ->
                  let ctx, gids, ty, decls, node_decls_map =
                    gen_poly_decls_ty ctx gids (Some nname) acc_node_decls_map
                      ty
                  in
                  ( ctx,
                    gids,
                    decls @ acc_decls,
                    GI.StringMap.add id ty acc_glocals,
                    node_decls_map ))
                glocals
                (ctx, gids, [], GI.StringMap.empty, node_decls_map)
            in
            let ctx, gids, decls, ib_oracles, node_decls_map =
              List.fold_left
                (fun (ctx, gids, acc_decls, acc_ib_oracles, acc_node_decls_map)
                     (id, ty) ->
                  let ctx, gids, ty, decls, node_decls_map =
                    gen_poly_decls_ty ctx gids (Some nname) acc_node_decls_map
                      ty
                  ...
                  ( ctx,
                    gids,
                    decls @ acc_decls,
                    (id, ty) :: acc_ib_oracles,
                    node_decls_map ))
                (ctx, gids, decls, [], node_decls_map)
                ib_oracles
            in
            let ctx, gids, decls, geqs, node_decls_map =
              List.fold_left
                (fun (ctx, gids, acc_decls, acc_geqs, acc_node_decls_map)
                     (q_vars, sc, lhs, expr, source) ->
                  let ctx, gids, expr, decls, node_decls_map =
                    gen_poly_decls_expr ctx gids (Some nname) acc_node_decls_map
                      expr
                  in
                  ( ctx,
                    gids,
                    decls @ acc_decls,
                    (q_vars, sc, lhs, expr, source) :: acc_geqs,
                    node_decls_map ))
                (ctx, gids, decls, [], node_decls_map)
                geqs
            in
    ...
    instantiation to call another polymorphic node *)
      let ctx, gids, decls, node_decls_map =
        gen_poly_decls_decls ctx gids node_decls_map (decls @ [ called_decl ])
      in

      (ctx, gids, pnname, decls, node_decls_map)

and gen_poly_decls_ty :
    Ctx.tc_context ->
    GI.t GI.StringMap.t ->
    HString.t option ->
    (A.declaration * A.lustre_type list list) HString.HStringMap.t ->
    ...
    * A.declaration list
    * (A.declaration * A.lustre_type list list) HString.HStringMap.t =
 fun ctx gids nname node_decls_map ty ->
  match ty with
  | A.TupleType (p, tys) ->
      let ctx, gids, tys, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_tys, acc_decls, acc_node_decls_map) ty ->
            let ctx, gids, ty, decls, node_decls_map =
              gen_poly_decls_ty ctx gids nname acc_node_decls_map ty
            in
  ...
  | RecordType (p, id, tis) ->
      let ctx, gids, tis, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_tys, acc_decls, acc_node_decls_map) (p, id, ty)
             ->
            let ctx, gids, ty, decls, node_decls_map =
              gen_poly_decls_ty ctx gids nname acc_node_decls_map ty
            in
  ...
  | ArrayType (p, (ty, expr)) ->
      let ctx, gids, ty, decls1, node_decls_map =
        gen_poly_decls_ty ctx gids nname node_decls_map ty
      in
      let ctx, gids, expr, decls2, node_decls_map =
        gen_poly_decls_expr ctx gids nname node_decls_map expr
      in
      (ctx, gids, ArrayType (p, (ty, expr)), decls1 @ decls2, node_decls_map)
  | TArr (p, ty1, ty2) ->
      let ctx, gids, ty1, decls1, node_decls_map =
        gen_poly_decls_ty ctx gids nname node_decls_map ty1
      in
      let ctx, gids, ty2, decls2, node_decls_map =
        gen_poly_decls_ty ctx gids nname node_decls_map ty2
      in
  ...
  | Int32 _ | Int64 _ | IntRange _ | Real _ | UserType _ | AbstractType _
  | EnumType _ | History _ ->
      (ctx, gids, ty, [], node_decls_map)

and gen_poly_decls_expr :
    Ctx.tc_context ->
    GI.t GI.StringMap.t ->
    HString.t option ->
    (A.declaration * A.lustre_type list list) HString.HStringMap.t ->
 ...
 fun ctx gids caller_nname node_decls_map expr ->
  ...
  | A.Call (pos, ty :: tys, nname, exprs) ->
      let ctx, gids, exprs, decls1, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_exprs, acc_decls, acc_node_decls_map) expr ->
            let ctx, gids, expr, decls, node_decls_map =
              gen_poly_decls_expr ctx gids caller_nname acc_node_decls_map expr
            in
      ...
      let ctx, gids, pnname, decls2, node_decls_map =
        gen_poly_decl ctx gids caller_nname node_decls_map nname (ty :: tys)
      in
      ...
      let ty_args =
        ...
        | Some caller_nname ->
            let ty_vars =
              List.map (Ctx.ty_vars_of_type ctx caller_nname) (ty :: tys)
            in
      ...
      ( ctx,
        gids,
        Call (pos, ty_args, pnname, exprs),
        decls1 @ decls2,
        node_decls_map )
  | Ident _ | Call _ | Const _ | ModeRef _ ->
      (ctx, gids, expr, [], node_decls_map)
  ...
  | BinaryOp (p, op, expr1, expr2) ->
      ...
      let ctx, gids, expr2, decls2, node_decls_map =
        gen_poly_decls_expr ctx gids caller_nname node_decls_map expr2
      in
      ( ctx,
        gids,
        BinaryOp (p, op, expr1, expr2),
  ...
  | TernaryOp (p, op, expr1, expr2, expr3) ->
      ...
      let ctx, gids, expr2, decls2, node_decls_map =
        gen_poly_decls_expr ctx gids caller_nname node_decls_map expr2
      in
      let ctx, gids, expr3, decls3, node_decls_map =
        gen_poly_decls_expr ctx gids caller_nname node_decls_map expr3
      in
      ( ctx,
        gids,
        TernaryOp (p, op, expr1, expr2, expr3),
  ...
  | AnyOp (p, (p2, id, ty), expr1, Some expr2) ->
      ...
      let ctx, gids, expr2, decls2, node_decls_map =
        gen_poly_decls_expr ctx gids caller_nname node_decls_map expr2
      in
      let ctx, gids, ty, decls3, node_decls_map =
        gen_poly_decls_ty ctx gids caller_nname node_decls_map ty
      in
      ( ctx,
        gids,
        AnyOp (p, (p2, id, ty), expr1, Some expr2),
  ...
  | AnyOp (p, (p2, id, ty), expr, None) ->
      ...
      let ctx, gids, ty, decls2, node_decls_map =
        gen_poly_decls_ty ctx gids caller_nname node_decls_map ty
      in
      ( ctx,
        gids,
  ...
  | Quantifier (p, q, tis, expr) ->
      let ctx, gids, expr, decls1, node_decls_map = rec_call expr in
      let ctx, gids, tis, decls2, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_tis, acc_decls, acc_node_decls_map) (p, id, ty)
             ->
            let ctx, gids, ty, decls, node_decls_map =
              gen_poly_decls_ty ctx gids caller_nname acc_node_decls_map ty
            in
            ( ctx,
              gids,
              acc_tis @ [ (p, id, ty) ],
              decls @ acc_decls,
              node_decls_map ))
          (ctx, gids, [], decls1, node_decls_map)
          tis
      in
      (ctx, gids, Quantifier (p, q, tis, expr), decls1 @ decls2, node_decls_map)
  | Merge (p, id, id_exprs) ->
      let ctx, gids, id_exprs, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_id_exprs, acc_decls, acc_node_decls_map)
               (id, expr) ->
            let ctx, gids, expr, decls, node_decls_map =
              gen_poly_decls_expr ctx gids caller_nname acc_node_decls_map expr
            in
  ...
  | GroupExpr (p, ge, exprs) ->
      let ctx, gids, exprs, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_exprs, acc_decls, acc_node_decls_map) expr ->
            let ctx, gids, expr, decls, node_decls_map =
              gen_poly_decls_expr ctx gids caller_nname acc_node_decls_map expr
  ...
  | Condact (p, expr1, expr2, id, exprs1, exprs2) ->
      ...
      let ctx, gids, expr2, decls2, node_decls_map =
        gen_poly_decls_expr ctx gids caller_nname node_decls_map expr2
      in
      let ctx, gids, exprs1, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_exprs, acc_decls, acc_node_decls_map) expr ->
            let ctx, gids, expr, decls, node_decls_map =
              gen_poly_decls_expr ctx gids caller_nname acc_node_decls_map expr
            in
            (ctx, gids, acc_exprs @ [ expr ], decls @ acc_decls, node_decls_map))
          (ctx, gids, [], decls1 @ decls2, node_decls_map)
          exprs1
      in
      let ctx, gids, exprs2, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_exprs, acc_decls, acc_node_decls_map) expr ->
            let ctx, gids, expr, decls, node_decls_map =
              gen_poly_decls_expr ctx gids caller_nname acc_node_decls_map expr
...
and gen_poly_decls_ni =
 fun ctx gids nname node_decls_map ni ->
  match ni with
  ...
  | IfBlock (p, expr, nis1, nis2) ->
      let ctx, gids, expr, decls, node_decls_map =
        gen_poly_decls_expr ctx gids nname node_decls_map expr
      in
      let ctx, gids, nis1, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_nis, acc_decls, acc_node_decls_map) ni ->
            let ctx, gids, ni, decls, node_decls_map =
              gen_poly_decls_ni ctx gids nname acc_node_decls_map ni
            in
            (ctx, gids, acc_nis @ [ ni ], decls @ acc_decls, node_decls_map))
          (ctx, gids, [], decls, node_decls_map)
          nis1
      in
      let ctx, gids, nis2, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_nis, acc_decls, acc_node_decls_map) ni ->
            let ctx, gids, ni, decls, node_decls_map =
              gen_poly_decls_ni ctx gids nname acc_node_decls_map ni
  ...
  | FrameBlock (p, vars, nes, nis) ->
      let ctx, gids, nis, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_nis, acc_decls, acc_node_decls_map) ni ->
            let ctx, gids, ni, decls, node_decls_map =
              gen_poly_decls_ni ctx gids nname acc_node_decls_map ni
          ...
          nis
      in
      let ctx, gids, nes, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_nes, acc_decls, acc_node_decls_map) ne ->
            let ctx, gids, ni, decls, node_decls_map =
              gen_poly_decls_ni ctx gids nname acc_node_decls_map (A.Body ne)
            in
...
and gen_poly_decls_loc =
 fun ctx gids nname node_decls_map loc ->
  match loc with
  | A.NodeConstDecl (p, FreeConst (p2, id, ty)) ->
      let ctx, gids, ty, decls, node_decls_map =
        gen_poly_decls_ty ctx gids nname node_decls_map ty
  ...
  | NodeVarDecl (p, var_decl) ->
      let ctx, gids, var_decl, decls, node_decls_map =
        gen_poly_decls_op ctx gids nname node_decls_map var_decl
      in
...
and gen_poly_decls_ci =
 fun ctx gids nname node_decls_map ci ->
  match ci with
  | A.ContractCall (p, cname, ty_arg :: ty_args, ips, ops) ->
      let ctx, gids, pcname, decls, node_decls_map =
        gen_poly_decl ctx gids nname node_decls_map cname (ty_arg :: ty_args)
      in
      ( ctx,
        gids,
        A.ContractCall (p, pcname, ty_arg :: ty_args, ips, ops),
        decls,
        node_decls_map )
  ...
  | Mode (p, id, creqs, cens) ->
      let ctx, gids, creqs, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_creqs, acc_decls, acc_node_decls_map)
               (p, id, expr) ->
            let ctx, gids, expr, decls, node_decls_map =
              gen_poly_decls_expr ctx gids nname acc_node_decls_map expr
            in
            ( ctx,
              gids,
              acc_creqs @ [ (p, id, expr) ],
          ...
          creqs
      in
      let ctx, gids, cens, decls, node_decls_map =
        List.fold_left
          (fun (ctx, gids, acc_cens, acc_decls, acc_node_decls_map)
               (p, id, expr) ->
            let ctx, gids, expr, decls, node_decls_map =
              gen_poly_decls_expr ctx gids nname acc_node_decls_map expr
            in
...
and gen_poly_decls_decls =
 fun ctx gids node_decls_map decls ->
  let ctx, gids, decls, node_decls_map =
    List.fold_left
      (fun (ctx, gids, acc_decls, acc_node_decls_map) decl ->
        match decl with
        | A.FuncDecl (p, (nname, ext, opac, ps, ips, ops, locs, nis, c)) -> (
            let ctx = Chk.add_ty_params_node_ctx ctx nname ps in
            let ctx, gids, ips, decls, node_decls_map =
              List.fold_left
                (fun (ctx, gids, acc_ips, acc_decls, acc_node_decls_map) ip ->
                  let ctx, gids, ip, decls, node_decls_map =
                    gen_poly_decls_ip ctx gids (Some nname) acc_node_decls_map
                      ip
                  in
                  ( ctx,
                    gids,
                    ...
                    decls @ acc_decls,
                    node_decls_map ))
                (ctx, gids, [], acc_decls, acc_node_decls_map)
                ips
            in
            let ctx, gids, ops, decls, node_decls_map =
              List.fold_left
                (fun (ctx, gids, acc_ops, acc_decls, acc_node_decls_map) op ->
                  let ctx, gids, op, decls, node_decls_map =
                    gen_poly_decls_op ctx gids (Some nname) acc_node_decls_map
                      op
                  in
                  ( ctx,
                    gids,
                ...
                ops
            in
            let ctx, gids, locs, decls, node_decls_map =
              List.fold_left
                (fun (ctx, gids, acc_locs, acc_decls, acc_node_decls_map) loc ->
                  let ctx, gids, loc, decls, node_decls_map =
                    gen_poly_decls_loc ctx gids (Some nname) acc_node_decls_map
                      loc
                  in
                  ( ctx,
                    gids,
            ...
            let ctx, gids, nis, decls, node_decls_map =
              List.fold_left
                (fun (ctx, gids, acc_nis, acc_decls, acc_node_decls_map) ni ->
                  let ctx, gids, ni, decls, node_decls_map =
                    gen_poly_decls_ni ctx gids (Some nname) acc_node_decls_map
                      ni
                  in
                  ( ctx,
                    gids,
            ...
            | Some (p3, c) ->
                let ctx, gids, c, decls, node_decls_map =
                  List.fold_left
                    (fun (ctx, gids, acc_cis, acc_decls, acc_node_decls_map) ip
                       ->
                      let ctx, gids, ci, decls, node_decls_map =
                        gen_poly_decls_ci ctx gids (Some nname)
                          acc_node_decls_map ip
                      in
                      ( ctx,
                        gids,
                        acc_cis @ [ ci ],
                ...
                let decl =
                  A.FuncDecl
                    ( p,
                      (nname, ext, opac, ps, ips, ops, locs, nis, Some (p3, c))
                    )
                in
                (ctx, gids, decl :: decls, node_decls_map))
        | NodeDecl (p, (nname, ext, opac, ps, ips, ops, locs, nis, c)) -> (
            ...
            | None ->
                let decl =
                  A.NodeDecl (p, (nname, ext, opac, ps, ips, ops, locs, nis, c))
                in
                (ctx, gids, decl :: decls, node_decls_map)
            | Some (p3, c) ->
                let ctx, gids, c, decls, node_decls_map =
        ...
        | ContractNodeDecl (p, (cname, ps, ips, ops, (p3, c))) ->
            ...
            let ctx, gids, ips, decls, node_decls_map =
              List.fold_left
                (fun (ctx, gids, acc_ips, acc_decls, acc_node_decls_map) ip ->
                  let ctx, gids, ip, decls, node_decls_map =
                    gen_poly_decls_ip ctx gids (Some cname) acc_node_decls_map
                      ip
                  in
                  ( ctx,
                    gids,
            ...
            let ctx, gids, ops, decls, node_decls_map =
              List.fold_left
                (fun (ctx, gids, acc_ops, acc_decls, acc_node_decls_map) op ->
                  let ctx, gids, op, decls, node_decls_map =
                    gen_poly_decls_op ctx gids (Some cname) acc_node_decls_map
                      op
                  in
                  ( ctx,
                    gids,
            ...
            let ctx, gids, c, decls, node_decls_map =
              List.fold_left
                (fun (ctx, gids, acc_cis, acc_decls, acc_node_decls_map) ip ->
                  let ctx, gids, ci, decls, node_decls_map =
                    gen_poly_decls_ci ctx gids (Some cname) acc_node_decls_map
                      ip
                  in
                  ( ctx,
                    gids,
            ...
            (ctx, gids, decl :: decls, node_decls_map)
        | TypeDecl _ | ConstDecl _ | NodeParamInst _ ->
            (ctx, gids, decl :: acc_decls, node_decls_map))
      (ctx, gids, [], node_decls_map)
      decls
  in
  (ctx, gids, decls, node_decls_map)

let instantiate_polymorphic_nodes :
    Ctx.tc_context ->
    GI.t GI.StringMap.t ->
    A.declaration list ->
 ...
 fun ctx gids decls ->
  ...
  let ctx, gids, decls, _ =
    gen_poly_decls_decls ctx gids node_decls_map decls
  in
