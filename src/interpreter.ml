...
let on_exit _ =
  ...
  try
    ...
    | None -> ()
  with e ->
    KEvent.log L_error "Error deleting solver_init: %s" (Printexc.to_string e)
...
let main input_file input_sys _ trans_sys =
  ...
  let input_scope =
    TransSys.scope_of_trans_sys trans_sys @ LustreIdent.user_scope
  in
       ...
       | (sv, _), head :: tail when StateVar.is_const sv ->
           ...
           |> List.fold_left
                (fun acc value ->
                  if acc != value then (
                    KEvent.log L_warn
                      "Input %s is constant, but input values differ: got %a \
                       and, later, %a."
                      (StateVar.name_of_state_var sv)
                      Term.pp_print_term acc Term.pp_print_term value;
                    Failure "main" |> raise);
                  acc)
                head
           |> ignore
  ...
  List.iter
    (fun ((state_var, _), inputs) ->
      ...
      if List.length inputs > input_length then
        (* Output warning *)
        KEvent.log L_warn "Input for %s is longer than other inputs"
  ...
  let steps =
    ...
    | s ->
        ...
        if s > input_length && nb_inputs > 0 then
          KEvent.log L_warn
            "Input is not long enough to simulate %d steps. Simulation is \
             nondeterministic."
            input_length;
  ...
  let solver =
    Flags.Smt.solver () |> SMTSolver.create_instance ~produce_models:true logic
  in
      ...
      List.iteri
        (fun instant instant_value ->
          ...
          if instant < steps then
            ...
            let var =
              Var.mk_state_var_instance state_var (Numeral.of_int instant)
              |> Term.mk_var
            in
            ...
            let var =
              List.fold_left
                (fun acc i -> Term.mk_select acc (Term.mk_num_of_int i))
                var indexes
              |> Term.convert_select
            in

            (* Constrain variable to its value at instant *)
            let equation = Term.mk_eq [ var; instant_value ] in
  ...
  KEvent.log L_info "Parsing interpreter input file %s" (Flags.input_file ());
  ...
  if SMTSolver.check_sat solver then
    ...
    KEvent.execution_path input_sys trans_sys (Model.path_to_list path)
  else
