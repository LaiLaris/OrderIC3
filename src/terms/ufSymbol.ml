...
let rec pp_print_uf_symbol_node ppf s = Format.pp_print_string ppf s

(* Pretty-print a hashconsed uninterpreted function symbol *)
and pp_print_uf_symbol ppf { H.node = n } = pp_print_uf_symbol_node ppf n
  ...
  try
    ...
    then
      ...
      raise
        (Invalid_argument
           (Format.asprintf
              "@[<v>Uninterpreted symbol %s with signature %a -> %a redeclared \
               with different signature %a -> %a @]"
              s
              (pp_print_list Type.pp_print_type " -> ")
              (arg_type_of_uf_symbol u) Type.pp_print_type
              (res_type_of_uf_symbol u)
              (pp_print_list Type.pp_print_type " -> ")
              a Type.pp_print_type r))
    (* Uninterpreted symbol is not in the hashcons table *)
  with Not_found ->
    (* Hashcons uninterpreted symbol *)
    let s = Huf_symbol.hashcons ht s { uf_arg_type = a; uf_res_type = r } in
    uf_symbs := s :: !uf_symbs;
    s
...
let import u =
  mk_uf_symbol (name_of_uf_symbol u)
...
let fold_uf_declarations f a =
  Huf_symbol.fold
    (fun u a ->
      let s, t, r =
        (function
          | { H.node = s; H.prop = { uf_arg_type = t; uf_res_type = r } } ->
              (s, t, r))
          u
      in
      f s t r a)
