...
(** Term representation
    ...
    Terms are hashconsed for maximal sharing, comparison with physical equality
    and to store type information.

    Terms are lambda trees, see {!Ltree}, with symbols of type {!Symbol.t}, free
    variables of type {!Var.t} and types {!Type.t}.

    The type {!t} is private and cannot be constructed outside this module in
    order to ensure that all equal subterms are physically equal for
    hashconsing.

    Use the constructor functions like {!mk_true}, {!mk_num} etc. to construct
    terms. An exception will be raised if an incorrectly typed term is
    constructed.
...
(** Negates a term by modifying the top node if it is a bvneg or an machine
    integer constant. *)
...
(** [set_inter_group t g] associates term [t] with interpolation group [g] and
    return the corresponing interpolation group term *)

val mk_named_unsafe : t -> string -> int -> t
(** Name term with the given integer in a given namespace

    This is a basic function, the caller has to generate the name, and ensure
    the name is used only once. Use with caution, or better only use
    {!mk_named}, which will create a unique name.

    The namespace ["t"] will be rejected, because this is the namespace used by
    {!mk_named}. *)
...
(** Negates a term by modifying the top node if it is a not, true, false, or an
...
val is_atom : t -> bool
(** Return true if the term is a simple Boolean atom, that is, has type Boolean
    and does not contain subterms of type Boolean *)
...
(** Return the indexes and the array variable of the select operator
    ...
    The array argument of a select is either another select operation or a
    variable. For the expression [(select (select A j) k)] return the pair [A]
    and [[j; k]]. *)
...
(** Evaluate the term bottom-up and right-to-left. The evaluation function is
    called at each node of the term with the the term being evaluated, and the
    list of values computed for the subterms. Let bindings are lazily unfolded.
*)
...
(** Tail-recursive bottom-up right-to-left map on the term
    ...
    Not every subterm is a proper term, since the de Bruijn indexes are shifted.
    Therefore, the function [f] is called with the number of let bindings the
    subterm is under as first argument, so that the indexes can be adjusted in
    the subterm if necessary. *)
...
(** Return a new term with each state variable replaced
    ...
    [map_state_vars t f] returns a new term of [t] with each occurring state
    variable [s] replaced by the result of the evaluation [f s]. *)
