...
module Make (T : BaseTypes) :
  ...
  type flat = Var of var | Const of symbol | App of symbol * t list
  (* | Attr of t * attr *)
  ...
  *)
  ...
  module T_node = struct
    ...
    let equal t1 t2 =
      ...
      (* Equality of quantifiers: lambda abstractions are physically
         equal *)
      | Exists l1, Exists l2 | Forall l1, Forall l2 -> l1 == l2
    ...
    let hash = function
      ...
      | Let ({ H.hkey = hl }, l) ->
          safe_hash_interleave
            (Hashtbl.hash (hl :: List.map (function { H.hkey = h } -> h) l))
            8 4
  ...
  let rec pp_print_lambda' pp_symbol pp_var pp_sort db ppf = function
    | { H.node = L (l, t) } ->
        ...
        Format.fprintf ppf "@[<hv 1>(lambda@ (%a)@ (%a))@]"
          (pp_print_var_seq db) l
          (pp_print_term' pp_symbol pp_var pp_sort (db + List.length l))
          t
  ...
  and pp_print_let_bindings pp_symbol pp_var pp_sort i db ppf = function
    ...
    | t :: tl ->
        ...
        Format.fprintf ppf "@[<hv 1>(X%i@ %a)@]" db'
          (pp_print_term' pp_symbol pp_var pp_sort (db - i))
          t;
  ...
  and pp_print_term' pp_symbol pp_var pp_sort db ppf = function
    ...
    | { H.node = Node (s, a) } ->
        Format.fprintf ppf "@[<hv 1>(%a@ %a)@]"
          (pp_symbol ?arity:(Some (List.length a)))
          s
          (pp_print_term_list pp_symbol pp_var pp_sort db)
          a
    (* Print a let binding *)
    | { H.node = Let ({ H.node = L (_, t) }, b) } ->
        Format.fprintf ppf "@[<hv 1>(let@ @[<hv 1>(%a)@]@ %a)@]"
          (pp_print_let_bindings pp_symbol pp_var pp_sort 0 db)
          b
          (pp_print_term' pp_symbol pp_var pp_sort (db + List.length b))
          t
    (* Print an existential quantification *)
    | { H.node = Exists { H.node = L (x, t) } } ->
        Format.fprintf ppf "@[<hv 1>(exists@ @[<hv 1>(%a)@ %a@])@]"
          (pp_print_typed_var_list pp_sort db)
          x
          (pp_print_term' pp_symbol pp_var pp_sort (db + List.length x))
          t
    (* Print a universal quantification *)
    | { H.node = Forall { H.node = L (x, t) } } ->
        Format.fprintf ppf "@[<hv 1>(forall@ @[<hv 1>(%a)@ %a@])@]"
          (pp_print_typed_var_list pp_sort db)
          x
          (pp_print_term' pp_symbol pp_var pp_sort (db + List.length x))
          t
    ...
    | { H.node = Annot (t, a) } ->
        Format.fprintf ppf "@[<hv 1>(!@ @[<hv 1>%a@] @[<hv 1>%a@])@]"
          (pp_print_term' pp_symbol pp_var pp_sort db)
          t T.pp_print_attr a
  ...
  *)
  ...
  let map f term =
    ...
    let rec push db trees st =
      match trees with [] -> st | h :: t -> push db t ((db, MTree h) :: st)
    in
  ...
  let rec fold fail_quant f subst accum = function
    ...
    | FTree (db, { H.node = Node (op, args) }) :: tl ->
        (* Push the argument terms as FTree on the instruction stack in
         reverse order *)
        let rec push trees st =
          match trees with [] -> st | h :: t -> push t (FTree (db, h) :: st)
        in
    ...
    | FTree
        ( db,
          {
            H.node = Forall { H.node = L (n, l) } | Exists { H.node = L (n, l) };
          } )
      :: tl ->
        if fail_quant then
          raise (Invalid_argument "Ltree.fold : quantified term");
    ...
    | FCtx i :: itl -> (
        ...
        | [ t ] :: atl ->
            let rec aux accum = function
              | [] -> List.rev accum
              | (v, _) :: tl when v = i ->
                  List.rev_append ((v, E t) :: accum) tl
              | h :: tl -> aux (h :: accum) tl
            in
            (*
...
*)
        ...
        (* Result stack is never empty and has a singleton list as
             first element *)
        | x :: _ ->
            Format.eprintf "%d@." (List.length x);
            assert false
        | [] -> assert false)
  ...
  *)
  ...
  let rec import_lambda = function
    | { H.node = L (i, t) } ->
  ...
  *)
  ...
  let mk_lambda x t =
    ...
    hl_lambda (List.map T.sort_of_var x) (bind dbm t)

  (* Beta-evaluate a lambda expression *)
  let eval_lambda ({ Hashcons.node = L (v, _) } as l) b =
    if List.length v = List.length b then ht_let l b
    else raise (Invalid_argument "eval_lambda")
  ...
  let mk_app s a = match a with [] -> assert false | _ -> ht_node s a
  ...
  let[@ocaml.warning "-27"] trim_let_domain offset bvar_types bvar_terms
    ...
    let _, bound_var_map =
      List.fold_left
        (fun (c, a) i ->
          if i > List.length bvar_terms then (c, a) else (succ c, (i, c) :: a))
        (1, []) bound_vars
    in

    Debug.ltree
      "@[<hv>trim_let_domain:@ %a@ terms@ @[<hv>%a@]@ bound vars@ @[<hv>%a@]@ \
       map@ @[<hv>%a@]@]"
      (pp_print_term ~db:0) term
      (pp_print_list pp_print_term ",@ ")
      bvar_terms
      (pp_print_list Format.pp_print_int ",@ ")
      bound_vars
      (pp_print_list (fun ppf (i, j) -> Format.fprintf ppf "%d:%d" i j) ",@ ")
      bound_var_map;
    ...
    let bvar_terms' =
      List.rev (list_extract_indexes bound_vars (List.rev bvar_terms))
    in

    (* Eliminate types of variables not bound in the term *)
    let bvar_types' =
      List.rev (list_extract_indexes bound_vars (List.rev bvar_types))
    in
    ...
    | _ ->
        ...
        ht_let (hl_lambda bvar_types' term') bvar_terms'
  ...
  let rec destruct' ofs = function
    ...
    | { H.node = Let ({ H.node = L (i, { H.node = Node (s, l) }) }, b) } ->
        ...
        ht_node s (List.map (function r -> trim_let_domain ofs i b r) l)
    ...
    | { H.node = Let ({ H.node = L (i, { H.node = Annot (t, a) }) }, b) } ->
        ...
        ht_annot (ht_let (hl_lambda i t) b) a
    (* Quantified term *)
    | { H.node = Exists _ }
    | { H.node = Forall _ }
  ...
  let has_quantifier term =
    (* Add the subterms in reverse order to the instruction stack *)
    let rec push trees st =
      match trees with [] -> st | h :: t -> push t (MTree h :: st)
    in
    ...
    let rec has_quantifier has n =
      ...
      (* Existential quantifier *)
      | MTree { H.node = Exists _ } :: _ | MTree { H.node = Forall _ } :: _ ->
          true
      (* Function application *)
