...
let default_of_type t =
  ...
  (* No defaults *)
  | Type.Abstr _ | Type.Array _ -> invalid_arg "default_of_type"
...
let remove_top_level_quantifier t =
  ...
  | Term.T.Forall lam | Term.T.Exists lam ->
      ...
      |> List.map (fun t ->
             (UfSymbol.mk_fresh_uf_symbol [] t |> Term.mk_uf) @@ [])
...
let rec logic_of_sort ty =
  ...
  | Real -> singleton RA
  | Array (ta, tr) -> union (logic_of_sort ta) (logic_of_sort tr) |> add A
...
let at_most_one_non_num l =
  let rec aux found = function
    | [] -> true
    | t :: l when Term.is_numeral t || Term.is_decimal t -> aux found l
    | _ :: l -> if found then raise Exit else aux true l
  in
...
let logic_of_flat fun_symbols t acc =
  ...
  | Const s | App (s, []) ->
      if Symbol.is_uf s then
        Symbol.uf_of_symbol s |> UfSymbol.res_type_of_uf_symbol |> logic_of_sort
  ...
  | App (s, l) when s == Symbol.s_times && at_most_one_non_num l ->
      add LA (sup_logics acc)
  | App (s, _ :: l)
    when Symbol.(s == s_div || s == s_intdiv || s == s_mod)
         && List.for_all (fun t -> Term.is_numeral t || Term.is_decimal t) l ->
      add LA (sup_logics acc)
  | App (s, [ n ]) when s == s_abs && (Term.is_numeral n || Term.is_decimal n)
    ->
      add LA (sup_logics acc)
  | App (s, _)
    when Symbol.(
           s == s_div || s == s_times || s == s_abs || s == s_intdiv
           || s == s_mod) ->
      add NA (sup_logics acc)
  | App (s, _) when Symbol.(s == s_lt || s == s_gt || s == s_leq || s == s_geq)
    ->
      add LA (sup_logics acc)
  ...
  | App (s, _)
    when Symbol.(
           is_to_uint8 s || is_to_uint16 s || is_to_uint32 s || is_to_uint64 s
           || is_to_int8 s || is_to_int16 s || is_to_int32 s || is_to_int64 s)
    ->
      add BV (sup_logics acc)
  | App _ -> sup_logics acc

let check_add_Q t l = if Term.has_quantifier t then FeatureSet.add Q l else l
...
let pp_print_features ?(enforce_logic = false) fmt l =
  ...
  let smt_arrays = L.mem A l && (enforce_logic || Flags.Arrays.smt ()) in
...
module Signals = struct
  ...
  let pp_print_signals fmt () =
    Format.fprintf fmt
      "Signals: @[<v>sigalrm | %a@ sigint  | %a@ sigquit | %a@ sigterm | %a@ \
       timeout | %a@ break   | %b@]"
      pp_print_handler signals.sigalrm pp_print_handler signals.sigint
      pp_print_handler signals.sigquit pp_print_handler signals.sigterm
      (pp_print_option Format.pp_print_float)
      signals.timeout signals.break
  ...
  let ignore_sig s = Sys.set_signal s Sys.Signal_ignore
...
*)
  ...
  let set_sig s f = Sys.set_signal s (Sys.Signal_handle f)
  ...
  let set_timeout_value ?(interval = 0.) value =
    ...
    Unix.setitimer Unix.ITIMER_REAL
      { Unix.it_interval = interval; Unix.it_value = value }
    |> ignore
...
let add_quantifiers = function
  ...
  | `SMTLogic s as l -> (
      try
        let s =
          if String.sub s 0 3 = "QF_" then String.sub s 3 (String.length s - 3)
          else s
        in
        `SMTLogic s
