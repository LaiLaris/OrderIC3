...
let pp_print_var_node ppf = function
  ...
  | StateVarInstance (v, o) ->
      Format.fprintf ppf "%a@%a" StateVar.pp_print_state_var v
        Numeral.pp_print_numeral o
  (* Pretty-print a constant state variable *)
  | ConstStateVar v -> Format.fprintf ppf "%a" StateVar.pp_print_state_var v
  (* Pretty-print a free variable *)
  | FreeVar (s, _) -> Format.fprintf ppf "%a" HString.pp_print_hstring s
...
let is_free_var = function { Hashcons.node = FreeVar _ } -> true | _ -> false
...
let mk_const_state_var v =
  ...
  else raise (Invalid_argument "mk_const_state_var")
...
let rec next_fresh_var_node var_type =
  let fresh_var_id =
    try Type.TypeHashtbl.find fresh_var_ids var_type with Not_found -> 1
  in
  ...
  let fresh_var_name =
    HString.mk_hstring
      (Format.asprintf "__X_%a_%d" Type.pp_print_type var_type fresh_var_id)
  in

  (* Candidate name for next fresh symbol *)
  let v = FreeVar (fresh_var_name, var_type) in
...
let set_offset_of_state_var_instance v i =
  match v with
  ...
  | ({ Hashcons.node = ConstStateVar _ } | { Hashcons.node = FreeVar _ }) as v
    ->
      v
...
let bump_offset_of_state_var_instance v i =
  match v with
  (* State variable instance *)
  | { Hashcons.node = StateVarInstance (v, o) } ->
...
let find_unrolled_var_map string = StringMap.find string !unrolled_var_map
...
let unrolled_uf_of_state_var_instance = function
  ...
  | { Hashcons.node = FreeVar (_, ty) } as var ->
      ...
      let uf = UfSymbol.mk_fresh_uf_symbol [] ty in
...
let rec declare_vars declare = function
  | ({ Hashcons.node = StateVarInstance (_, _) } as var) :: tail ->
      (* Declaring the uf. *)
      declare (unrolled_uf_of_state_var_instance var);
