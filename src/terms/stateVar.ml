...
module State_var_node = struct
  ...
  let equal (s1, l1) (s2, l2) =
    String.equal s1 s2
    && try List.for_all2 String.equal l1 l2 with Invalid_argument _ -> false
end
...
let pp_print_state_var_prop ppf prop =
  Format.fprintf ppf
    ...
    Type.pp_print_type prop.var_type UfSymbol.pp_print_uf_symbol prop.uf_symbol
    prop.is_input prop.is_const prop.for_inv_gen

(* Pretty-print a scoped name of a state variable *)
let pp_print_state_var_name ppf (n, s) =
  if s = [] then Format.fprintf ppf "%s" n
  else Format.fprintf ppf "%a.%s" (pp_print_list Format.pp_print_string ".") s n

(* Return a string representation of the name of a state variable *)
let string_of_state_var_name (n, s) = string_of_t pp_print_state_var_name (n, s)

(* Pretty-print a state variable *)
let pp_print_state_var_node ppf (n, s) = pp_print_state_var_name ppf (n, s)
...
let pp_print_state_var_debug ppf state_var =
  Format.fprintf ppf "node %a; prop %a" pp_print_state_var_name
    state_var.Hashcons.node pp_print_state_var_prop state_var.Hashcons.prop
...
(* Uninterpreted function symbol of a state variable *)
let state_var_of_uf_symbol u = UfSymbol.UfSymbolHashtbl.find uf_symbols_map u
...
let gen_uf =
  ...
  fun a s ->
    incr r;
    UfSymbol.mk_uf_symbol (Format.sprintf "%%f%d" !r) a s

(* Hashcons a state variable *)
let mk_state_var ?(is_input : bool = false) ?(is_const : bool = false)
    ?(for_inv_gen : bool = true) state_var_name state_var_scope state_var_type =
  try
    (* Get previous declaration of identifier *)
    let v = Hstate_var.find ht (state_var_name, state_var_scope) in
      ...
      raise
        (Invalid_argument
           (Format.asprintf
              ...
              Type.pp_print_type state_var_type Type.pp_print_type
              (type_of_state_var v)))
  ...
  with Not_found -> (
    try
      ...
      raise
        (Invalid_argument
           (Format.asprintf
              "State variable %a conflicts with uninterpreted function symbol"
              pp_print_state_var_name
              (state_var_name, state_var_scope)))
    with Not_found ->
      (* Create an uninterpreted function symbol for the state variable *)
      let state_var_uf_symbol =
        (if Flags.Smt.short_names () && not is_const then gen_uf
         else
      ...
      (* Remember association of uninterpreted function symbol with
          state variable *)
      UfSymbol.UfSymbolHashtbl.add uf_symbols_map state_var_uf_symbol state_var;
...
let mk_init_flag scope =
  mk_state_var ~is_input:true ~is_const:false ~for_inv_gen:false
    init_flag_string scope Type.t_bool

(* Returns a scoped depth input. *)
let mk_depth_input scope =
  mk_state_var ~is_input:true ~is_const:true ~for_inv_gen:false
    depth_input_string scope Type.t_int

(* Returns a scoped max depth input. *)
let mk_max_depth_input scope =
  mk_state_var ~is_input:true ~is_const:true ~for_inv_gen:false
    max_depth_input_string scope Type.t_int

(* Import a state variable from a different instance into this
   hashcons table *)
let import v =
  mk_state_var ~is_input:(is_input v) ~is_const:(is_const v)
    ~for_inv_gen:(for_inv_gen v) (name_of_state_var v) (scope_of_state_var v)
...
let state_var_of_long_string s = state_var_of_string (Lib.extract_scope_name s)
...
(* Fold all variables in the hash-cons table *)
let iter f = Hstate_var.iter f ht
