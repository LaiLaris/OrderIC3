...
let has_prefix s = String.length s > 2 && s.[0] = '0' && s.[1] = 'x'
...
let to_numeral s =
  if has_prefix s then
    let rec scan value s k =
      if k < String.length s then scan (add_hexdigit value s.[k]) s (succ k)
      else Some value
    in
    scan Big_int.zero_big_int s 2
  else None

let to_decimal s =
  if has_prefix s then
    let rec vexp n s k =
      if k < String.length s then vexp ((n * 10) + hex_digit s.[k]) s (succ k)
      else n
    in

    let expo s k =
      if k < String.length s then
        if s.[k] = '+' then +vexp 0 s (succ k)
        else if s.[k] = '-' then -vexp 0 s (succ k)
        else vexp 0 s k
      else 0
    in
    ...
    let exponent fk k p =
      ...
      if fk > 0 then p - (4 * (k - fk - 1)) else p
    in
