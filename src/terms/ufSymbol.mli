...
(** Declare an uninterpreted symbol
    ...
    [mk_uf_symbol s a r] constructs and returns an uninterpreted symbol with its
    name, the type of its arguments and the type of its result.

    Uninterpreted symbols can only be constructed with this function in order to
    ensure that they are properly declared.

    Declaring an uninterpreted function again with the same signature is
    harmless and will simply return the previously declared symbol. However,
    re-declaring an uninterpreted function with a different signature will raise
...
val import : t -> t
(** Import an uninterpreted symbol from a different instance into this hashcons
    table

    We may have clashes if we import fresh uninterpreted symbols from one
    instance to another. *)
...
val fold_uf_declarations :
  (string -> Type.t list -> Type.t -> 'a -> 'a) -> 'a -> 'a
(** [fold_declarations f a] computes
    [(f sN aN rN ... (f s2 a2 r2 (f s1 a1 r1 a))...)], where [sI], [aI] and
    [rI], respectively are the name of the uninterpreted symbol, the types of
    its arguments and the type of its value. *)

val iter_uf_declarations : (string -> Type.t list -> Type.t -> unit) -> unit
(** [iter_declarations f t] iterates [f] over all declarations, calling
    [f s a r] on each, where [s], [a] and [r], respectively are the name of the
    uninterpreted symbol, the types of its arguments and the type of its value.
*)
