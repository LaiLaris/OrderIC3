...
let pp_print_positive_decimal_as_json ppf = function
  ...
  | Num.Big_int n ->
      Format.fprintf ppf "{\"num\": %s, \"den\": 1}"
  ...
  | Num.Ratio r ->
      ...
      (* Print with division as prefix operator *)
      Format.fprintf ppf "@[<hv 1>{\"num\": %s, \"den\": %s}@]"
        (Big_int.string_of_big_int rn)
        (Big_int.string_of_big_int rd)
...
let pp_print_decimal_as_float fmt = function
  ...
  | N d -> (
      ...
      | Num.Big_int n ->
          Format.fprintf fmt "%sf64" (Big_int.string_of_big_int n)
      | Num.Ratio r ->
          (* Normalize rational number *)
          let r' = Ratio.normalize_ratio r in
...
let of_decimal_string s =
  ...
  let rec scan_exp start_pos pos =
    ...
    if start_pos + pos >= String.length s then ()
    else
      ...
      | c ->
          raise
            (Invalid_argument
               (Format.sprintf "of_string: invalid character %c at position %d"
                  c (start_pos + pos)))
  in
  ...
  let int_num =
    if Buffer.length int_buf = 0 then Num.num_of_int 0
    else Num.num_of_string (Buffer.contents int_buf)
  in

  (* Convert fractional buffer to numeral, default to zero if empty *)
  let frac_num =
    if Buffer.length frac_buf = 0 then Num.num_of_int 0
    else Num.num_of_string (Buffer.contents frac_buf)
  in

  (* Convert exponent buffer to numeral, default to one if empty *)
  let exp_num =
    if Buffer.length exp_buf = 0 then Num.num_of_int 0
    else Num.num_of_string (Buffer.contents exp_buf)
  in

  (* Exponent *)
  let exp = Num.power_num (Num.num_of_int 10) exp_num in
  ...
  let frac =
    Num.div_num
      ...
      (Num.power_num (Num.num_of_int 10)
...
let is_int = function N (Num.Int _) | N (Num.Big_int _) -> true | _ -> false
...
(* Equality *)
let equal l r = match (l, r) with N l, N r -> Num.eq_num l r | _ -> l = r
...
let geq l r = match (l, r) with N l, N r -> Num.ge_num l r | _ -> not (lt l r)

(* Greater than predicate *)
let gt l r = match (l, r) with N l, N r -> Num.gt_num l r | _ -> not (leq l r)
...
let succ = function N n -> Num.succ_num n |> of_num | nan -> nan

(* Decrement a decimal by one *)
let pred = function N n -> Num.pred_num n |> of_num | nan -> nan
...
let mult l r =
  ...
  | InfPos, n | n, InfPos ->
      if n = zero then Undef else if lt n zero then InfNeg else InfPos
  | InfNeg, n | n, InfNeg ->
      if n = zero then Undef else if lt n zero then InfPos else InfNeg
...
let div l r =
  ...
  | N l', N r' ->
      if r = zero then
        if l = zero then Undef else if lt l zero then InfNeg else InfPos
      else Num.div_num l' r' |> of_num
  | N _, InfNeg | N _, InfPos -> zero
  | InfPos, N _ -> if lt r zero then InfNeg else InfPos
  | InfNeg, N _ -> if lt r zero then InfPos else InfNeg
  | _ -> Undef

(* Remainder *)
let rem l r =
  match (l, r) with N l, N r -> Num.mod_num l r |> of_num | _ -> Undef

(* Computes the rational 2^p with signed p *)
let epsilon_ratio p =
  if p > 0 then Ratio.ratio_of_big_int (Big_int.power_int_positive_int 2 p)
  else if p < 0 then
    Ratio.ratio_of_big_int (Big_int.power_int_positive_int 2 (-p))
    |> Ratio.div_int_ratio 1
  else Ratio.ratio_of_int 1
...
let pp_print_decimal_approximation fmt dec =
  ...
  | N (Num.Ratio _ as r) -> (
      try
        ...
        if s = 0 then Format.pp_print_string fmt "0.0"
        else
          ...
          if p = 0 then Format.fprintf fmt "%s%s" sr approx
          else
