...
(** Abstract syntax trees with binders and quantifiers
    ...
    An abstract syntax tree with binders extends a basic first-order term
    structure with let bindings and quantifiers. The abstract syntax tree is
    parametrized by the three types of its symbols, its free variables and its
    sorts.

    A basic term is a tree structure, where a term is either a leaf containing a
    symbol or a variable. A term can also be a node containing a symbol with one
    or more subterms. All symbols are variadic and arity constraints are not
    checked or enforced in this module.

    For let bindings and quantifiers we add typed lambda abstractions and
    distinguish between free and bound variables. A typed lambda abstraction is
    a term where one or more variables are bound. We do nameless abstraction
    with de Bruijn indexes, hence a bound variables is just its index, whereas
    free variables are values of the type of free variables.

    A let binding is a lambda abstraction of [n] bound variables together with
    [n] terms that are to be substituted for the bound variables. Quantifiers
    are just lambda abstractions.

    In order to maintain invariants about de Bruijn indexes, the type of an
    abstract syntax term is private to this module and terms must be created
    with the appropriate constructors.

    In addition, there is a type of flat terms, where the topmost let binding
    has been evaluated. An abstract syntax term can be converted to a flat term
    with the {!S.destruct} function, which distributes let bindings over nodes
    and ensures that the top symbols of the term is a node, a leaf or a
    variable. Subterms of a nodes are abstract syntax terms with binders and
    {!S.destruct} can be repeatedly applied to these subterms.

    Tail-recursive fold and map functions are provided. The {!S.eval_t} function
    presents the subterms bottom-up and right-to-left to the folding function
    and lazily evaluates all let bindings. It fails when the term contains
    quantifiers. The {!S.map} function presents all subterms to the function,
    again bottom-up and right-to-left, let bindings are not unfolded. Hence, not
    every subterm is a proper abstract syntax term and the mapping function is
    given the number of let binding the subterm is under as an argument.
...
module type S = sig
  ...
  (** Lambda abstraction over symbols, variables and sort of the types given.
      Values of the type cannot be constructed outside this module in order to
      maintain invariants about the data type. *)
  ...
  (** Abstract syntax term over symbols, variables and sort of the types given.
      Values of the type cannot be constructed outside this module in order to
      maintain invariants about the data type. Use the constructors {!mk_var},
      {!mk_const}, {!mk_app}, {!mk_let}, {!mk_exists} and {!mk_forall}. *)
  and t_node = private
    | FreeVar of var
    | BoundVar of int
    | Leaf of symbol
  ...
  (** Term over symbols, variables and sort of the types given where the topmost
      symbol is not a binding

      This type must remain private, because {!construct} does not check the
      invariants and would be a backdoor to construct unsafe terms. *)
  and flat = private Var of var | Const of symbol | App of symbol * t list
  (* | Attr of t * attr *)
  ...
  (** Constructor for an existential quantification over an indexed free
      variable *)
  ...
  *)
  ...
  (** Evaluate the term bottom-up and right-to-left. The evaluation function is
      called at each node of the term with the term being evaluated and the list
      of values computed for the subterms. Let bindings are lazily unfolded. *)

  val map : (int -> t -> t) -> t -> t
  (** Tail-recursive bottom-up right-to-left map on the term

      Not every subterm is a proper term, since the de Bruijn indexes are
      shifted. Therefore, the function [f] is called with the number of let
      bindings the subterm is under as first argument, so that the indexes can
      be adjusted in the subterm if necessary. *)

  val destruct : t -> flat
  (** Return the top symbol of a term along with its subterms

      If the top symbol of a term is a let binding, the binding is distributed
      over the subterms. *)
  ...
  (** Import a lambda abstraction into the hashcons table by rebuilding it
      bottom up *)
