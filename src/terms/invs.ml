...
let copy { os; ts } = { os = TMap.copy os; ts = TMap.copy ts }

(** The empty collection of invariants. *)
let empty () = { os = TMap.create 107; ts = TMap.create 107 }

(** True if no invariants. *)
let is_empty { os; ts } = TMap.length os = 0 && TMap.length ts = 0

(** Number of invariants. *)
let len { os; ts } = (TMap.length os, TMap.length ts)

(** Bumps invariants. *)
let of_bound { os; ts } include_two_state offset =
  let bump_add inv _ acc = Term.bump_state offset inv :: acc in
  TMap.fold bump_add os []
  |> if include_two_state then TMap.fold bump_add ts else identity

(** Filters some invariants. *)
let filter f { os; ts } =
  let res = empty () in
  TMap.iter
    (fun inv cert -> if f false inv cert then TMap.add res.os inv cert)
    os;
  TMap.iter
    (fun inv cert -> if f true inv cert then TMap.add res.ts inv cert)
    ts;
  res
...
let add_ts { os; ts } inv cert =
  if TMap.mem os inv |> not && TMap.mem ts inv |> not then TMap.add ts inv cert
...
let get_os { os } =
  TMap.fold (fun inv _ acc -> Term.TermSet.add inv acc) os Term.TermSet.empty

(** The two-state invariants. *)
let get_ts { ts } =
  TMap.fold (fun inv _ acc -> Term.TermSet.add inv acc) ts Term.TermSet.empty

(** Checks if a term is a known invariant. *)
let mem { os; ts } term = TMap.mem os term || TMap.mem ts term

(** Returns [Some cert] if [term] is a known invariant, or [None] otherwise. *)
let find { os; ts } term =
  try Some (TMap.find os term)
  with Not_found -> ( try Some (TMap.find ts term) with Not_found -> None)
...
let fmt fmt { os; ts } =
  let os_empty, ts_empty = (TMap.length os = 0, TMap.length ts = 0) in
  if (not os_empty) || not ts_empty then (
    Format.fprintf fmt "@[<v>";
    if not os_empty then (
      Format.fprintf fmt "one state:";
      os |> TMap.iter (fun inv _ -> Format.fprintf fmt "@   %a" fmt_term inv);
      if not ts_empty then Format.fprintf fmt "@ ");
    if not ts_empty then (
      Format.fprintf fmt "two state:";
      ts |> TMap.iter (fun inv _ -> Format.fprintf fmt "@   %a" fmt_term inv));
    Format.fprintf fmt "@]")
