...
(** [num_to_ubv s n] returns size [s] unsigned bitvector converted from numeral
    [n] *)
...
val bv_arsh : t -> t -> t
(** Arithmetic right shift of bitvector by n times, where n is specified as an
    unsigned BV *)
...
val pp_yices_print_bitvector_d :
  Format.formatter -> Numeral.t -> Numeral.t -> unit
(** Pretty-print a constant bitvector in Yices' binary format given the decimal
    value and size *)
...
(** Convert a string to a bitvector Binary and hexadecimal notation is accepted
    as #b[01]+ and #x[0-9a-fA-F]+ as in the SMTLIB standard *)

val bitvector_of_hstring : HString.t -> t
(** Convert a hashconsed string to a bitvector, store all converted values in a
    cache *)
