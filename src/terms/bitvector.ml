...
let rec repeat_bit (b : bool) (n : int) : t =
  match n with 0 -> [] | n -> b :: repeat_bit b (n - 1)

(* Bit-vector representing decimal 0 *)
let zero (len : int) : t = repeat_bit false len

(* Bit-vector representing decimal 1 *)
let one (len : int) : t = repeat_bit false (len - 1) @ [ true ]

(* Bit-vector representing hexadecimal F - all bit's are 1 *)
let f (len : int) : t = repeat_bit true len
...
let bvsignext (m : int) (b : t) : t =
  ...
  let rec repeat (m : int) (b : bool) : t =
    match m with 0 -> [] | m' -> b :: repeat (m' - 1) b
  in
  List.append (repeat m sign) b

let bvzeroext (m : int) (b : t) : t = zero m @ b

(* Function that concatenates the input bitvectors *)
let bvconcat (b1 : t) (b2 : t) : t = List.append b1 b2
...
let modulo (x : Numeral.t) (y : Numeral.t) : Numeral.t =
  ...
  if Numeral.geq result Numeral.zero then result else Numeral.add result y
...
let rec pow2 (n : Numeral.t) : Numeral.t =
  if Numeral.equal n Numeral.zero then Numeral.one
    ...
    Numeral.mult (Numeral.succ Numeral.one) (pow2 (Numeral.sub n Numeral.one))
...
let num_to_ubv (size : Numeral.t) (i : Numeral.t) : t =
  ...
  let rec convert acc (l : Numeral.t) (n : Numeral.t) =
    if Numeral.gt n Numeral.zero then
      convert
        (Numeral.equal (Numeral.rem n (Numeral.of_int 2)) Numeral.one :: acc)
        (Numeral.add l Numeral.one)
        (Numeral.div n (Numeral.of_int 2))
    else (acc, l)
  in
...
let bool_to_bin (b : bool) : Numeral.t =
  match b with false -> Numeral.zero | true -> Numeral.one
...
let rec ubv_to_num' (size : Numeral.t) (b : t) : Numeral.t =
  ...
  | h :: t ->
      let prod =
        Numeral.mult (bool_to_bin h) (pow2 (Numeral.sub size Numeral.one))
      in
...
let signed_modulo (n : Numeral.t) (range_size : Numeral.t) : Numeral.t =
  ...
  let pos_lim =
    Numeral.sub (Numeral.div range_size (Numeral.of_int 2)) Numeral.one
  in
  if Numeral.lt n neg_lim then
    let diff = Numeral.sub neg_lim n in
    let diff_mod = Numeral.rem diff range_size in
    if Numeral.equal diff_mod Numeral.zero then neg_lim
    else Numeral.sub pos_lim (Numeral.sub diff_mod Numeral.one)
  else if Numeral.gt n pos_lim then
    let diff = Numeral.sub n pos_lim in
    let diff_mod = Numeral.rem diff range_size in
    if Numeral.equal diff_mod Numeral.zero then pos_lim
    else Numeral.add neg_lim (Numeral.sub diff_mod Numeral.one)
  else n
...
let rec ones_comp (n : t) : t =
  ...
  | [] -> []
  | h :: t -> (
      match h with true -> false :: ones_comp t | false -> true :: ones_comp t)

(* Return a binary version of "size"-bit 1 *)
let rec bin_one (size : int) : t =
  if size > 1 then false :: bin_one (size - 1) else [ true ]
...
let num_to_bv (size : Numeral.t) (i : Numeral.t) : t =
  ...
  if Numeral.geq n Numeral.zero then num_to_ubv size n
  else
...
let bv_to_num' (size : Numeral.t) (b : t) : Numeral.t =
  if List.nth b 0 = false then ubv_to_num' size b
  else
    Numeral.neg
      (ubv_to_num' size
         (plus_one (ones_comp b) (bin_one (Numeral.to_int size))))
...
let sbv_add (bv1 : t) (bv2 : t) : t =
  if List.length bv1 != List.length bv2 then raise ComparingUnequalBVs
  else
...
let sbv_div (bv1 : t) (bv2 : t) : t =
  if List.length bv1 != List.length bv2 then raise ComparingUnequalBVs
    ...
    let q =
      ...
      | n1, n2 when Numeral.geq n1 Numeral.zero && Numeral.geq n2 Numeral.zero
        ->
          Numeral.div n1 n2
      | n1, n2 when Numeral.lt n1 Numeral.zero && Numeral.geq n2 Numeral.zero ->
          let neg_n1 = Numeral.neg n1 in
          let q_aux = Numeral.div neg_n1 n2 in
          Numeral.neg q_aux
      | n1, n2 when Numeral.geq n1 Numeral.zero && Numeral.lt n2 Numeral.zero ->
          let neg_n2 = Numeral.neg n2 in
          let q_aux = Numeral.div n1 neg_n2 in
          Numeral.neg q_aux
      | n1, n2 when Numeral.lt n1 Numeral.zero && Numeral.lt n2 Numeral.zero ->
          let neg_n1 = Numeral.neg n1 in
          let neg_n2 = Numeral.neg n2 in
...
let sbv_rem (bv1 : t) (bv2 : t) : t =
  if List.length bv1 != List.length bv2 then raise ComparingUnequalBVs
    ...
    let num2 = bv_to_num' (Numeral.of_int (List.length bv2)) bv2 in
    let r =
      match (num1 < Numeral.zero, num2 < Numeral.zero) with
      ...
      | true, true ->
          Numeral.neg (Numeral.rem (Numeral.neg num1) (Numeral.neg num2))
    in
...
let rec create_one (size : Numeral.t) : t =
  ...
  else if Numeral.equal size Numeral.one then [ true ]
  else false :: create_one (Numeral.sub size Numeral.one)

(* Using ones_comp and bitwise_add from Numeral -> Signed BV conversion *)
let sbv_neg (bv : t) : t =
  let bv_ones_comp = ones_comp bv in
  let res =
    bitwise_add (List.rev bv_ones_comp)
...
let bv_or (bv1 : t) (bv2 : t) : t =
  if List.length bv1 != List.length bv2 then raise ComparingUnequalBVs
  else bv_or_aux bv1 bv2 []

(* Negation *)
let rec bv_not_aux (bv : t) (acc : t) : t =
  match bv with [] -> acc | h :: t -> bv_not_aux t (List.append acc [ not h ])

let bv_not (bv : t) : t = bv_not_aux bv []
...
let rec shift_left (bv : t) (n : Numeral.t) : t =
  if Numeral.equal n Numeral.zero then bv
  else
    match bv with
    | [] -> []
    | _ :: t -> shift_left (List.append t [ false ]) (Numeral.sub n Numeral.one)
...
let rec remove_last (bv : t) : t =
  match bv with [] -> [] | [ _ ] -> [] | h :: t -> h :: remove_last t
...
let rec shift_right (bv : t) (n : Numeral.t) : t =
  if Numeral.equal n Numeral.zero then bv
    ...
    | [] -> []
    | h :: t ->
        shift_right (false :: h :: remove_last t) (Numeral.sub n Numeral.one)
...
let rec list_first_n' a l n =
  if n = 0 then a
    ...
    list_first_n'
      ((try List.nth l (pred n) with Failure _ -> invalid_arg "list_first_n")
      :: a)
      l (pred n)
...
let rec pp_print_bitvector_x' ppf = function
  ...
  | d :: ([] as tl) | d :: ([ _ ] as tl) | d :: ([ _; _ ] as tl) ->
      pp_print_bitvector_x' ppf (false :: d :: tl)
  (* Print a hexadecimal digit for the first four bits *)
  | d1 :: d2 :: d3 :: d4 :: tl ->
      pp_print_string ppf (List.assoc [ d1; d2; d3; d4 ] bv_hex_table);
  ...
  if i <= 1 then a
    ...
    try
      bitvector_of_string_x
        ...
        (pred i) s
    with Not_found -> raise (Invalid_argument "bitvector_of_string")
...
let bitvector_of_string s =
  match
    ...
    with Invalid_argument _ -> raise (Invalid_argument "bitvector_of_string")
  with
  ...
  | "_ " ->
      ...
      let n, s =
        try
          Scanf.sscanf s "(_ bv%d %d)"
            f (*use %Ld and %u here to account for 64 bit ints*)
        with Scanf.Scan_failure _ ->
          raise (Invalid_argument "bitvector_of_string")
      in
      num_to_bv (Numeral.of_int s) (Numeral.of_int n)
  (*with
...
let bitvector_of_hstring s =
  (* Return cached value if available *)
  try HString.HStringHashtbl.find hstring_bitvector_cache s
  with Not_found ->
...
let first_bit (b : t) : bool =
  match b with h :: _ -> h | _ -> raise NonStandardBVSize
