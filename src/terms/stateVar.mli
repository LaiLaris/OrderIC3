...
(** State variables
    ...
    Every state variable is associated with an uninterpreted function symbol
    that is substituted for the state variable when creating expressions for the
    SMT solvers.

    State variables do not occur directly in terms, but as instances in a
    particular state, see {!Var}.
...
(** State variable
    ...
    A state state variable is a pair of its identifier and its scope, which is a
    list of identifiers. *)
...
(** [mk_state_var n s t] declares a state variable of name [n] with scope [s],
    ...
    [?for_inv_gen] flag the variable as an input, as constant and as usable as a
    candidate for invariant generation, respectively. Their defaults are
...
val fold : (t -> 'a -> 'a) -> 'a -> 'a
(** [fold f a] computes [(f sN tN uN ... (f s2 t2 u2 (f s1 t1 u1 a))...)], where
    [sI], [tI] and [uI], respectively are the name of the state variable, its
    types and its associated uninterpreted function symbol. *)

val iter : (t -> unit) -> unit
(** [iter f] calls [f s t u] for every state variable with [s] being the name of
    the variable, [t] its type and [u] its associated uninterpreted function
    symbol. *)
