...
let model_equal lhs rhs =
  try
    ...
    |> VHT.iter (fun var va1 ->
           try
             if VHT.find rhs var == va1 then ()
             else raise Not_equal (* [rhs] does not feature rhs. *)
...
let update_colors n white grey black nu_white nu_grey nu_black =
  ...
  let white =
    nu_white
    |> List.fold_left (fun white w -> (model_split n w |> fst) :: white) white
  in

  (* Updating black. *)
  let black =
    nu_black
    |> List.fold_left (fun black b -> (model_split n b |> fst) :: black) black
  in

  (* Updating white, grey and black with the new grey models. *)
  nu_grey
  |> List.fold_left
       (fun (white, grey, black) g ->
