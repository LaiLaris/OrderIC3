...
module type SubCandidate = sig
  ...
  (** Creates a new, default sub candidate. [mk disj_count conj_count vars vals]
      creates a sub candidate for a DNF with [disj_count] disjuncts, each having
      [conj_count] atoms in the sub cube corresponding to this sub candidate.
      Each atom is a an equation over [vars] with coefficients and constants
      taking their values in [vals].
  ...
  (** [guided_move sc i j] moves the [j]^th atom of the [i]^th sub cube of a sub
      candidate [sc]. *)
  ...
  (** [to_term sub_candidate dnf] adds the atoms of each sub cube of the sub
      ...
      /!\ SUPER IMPORTANT /!\ For the rating to work, terms must be prepended to
      each cube. For instance, assuming the DNF only has one cube, and if the
      atoms stored in the sub candidate for this cube are [ a1 ; a2 ; a3 ] then
      [to_term t (cube :: [])] returns [( a3 :: a2 :: a1 :: cube) :: []]. Also,
      the ordering of the DNF itself should not be changed.

      Note that this is what you're naturally going to get by doing a [map2]
      followed by a [fold2]. See [BoolSC.to_term] for a concrete example, or the
      arithmetic subcandidates. *)
end
...
module BoolSC : SubCandidate = struct
  ...
  let len { vals } = match vals with [] -> 0 | head :: _ -> List.length head

  let mk disj_count _ vars _ =
    (* All bool variables are false in the default sub cube. *)
    let vals = vars |> List.map (fun _ -> zero) |> mk_list disj_count in
    { vars; vals }
...
(** Sub candidate for modes. A mode sub candidate is not moveable. If the system
    has [n] modes, then [n+1] sub disjuncts are created. One for each mode and
    one empty sub disjunct (covers every mode).
...
module ModeSC : SubCandidate = struct
  ...
  let mk disj_count _ terms _ =
    ...
    if disj_count - 1 != List.length terms then
      Format.sprintf
        "[ModeSC] dimension mismatch: %d disjunct requested but found %d mode \
         terms"
...
module IntSCInfo :
  ArithSCInfo (* Exposing type info for [value], it is abstracted otherwise. *)
...
module ToArithSC (Info : ArithSCInfo) :
  SubCandidate with type value = Info.value =
  ...
  let reset { vars; vals; pool } =
    {
      ...
      vals =
        vals
        |> List.map
             (List.map (fun (lhs, _) -> (lhs |> List.map (fun _ -> zero), zero)));
      pool;
    }
   ...
   fun (coefs, rhs) ->
    ...
    | 3 ->
        ...
        then
          ...
          (coefs |> List.map (fun _ -> zero), zero)
...
type sc_list =
  | Cons : ((module SubCandidate with type t = 'a) * 'a * sc_list) -> sc_list
  | Nil : sc_list
...
let terms_of_sc_list d_count sc_list =
  let rec loop term_ll = function
    ...
    | _ -> term_ll
  in
  loop (mk_list d_count []) sc_list |> List.map List.rev
...
let unmoveable_of_sc_list sc_list =
  let rec delta_cons offset delta l =
    if delta > 0 then (offset + delta) :: l |> delta_cons offset (delta - 1)
    else l
  in
...
(* Prepends a bool sub candidate with [d_count] disjuncts if [vars] is not
   empty. *)
let add_bool_sub d_count vars sc_list =
  match vars with
  | [] -> sc_list
  | _ ->
...
let add_int_sub d_count c_count vars (vals : Numeral.t list) sc_list =
  match vars with
  | [] -> sc_list
  | _ ->
      (* Term building and type checking. *)
...
(* Prepends a rat sub candidate with [d_count] disjuncts and [c_count] atoms
   in each subcube if [vars] is not empty. *)
let add_rat_sub d_count c_count vars vals sc_list =
  match vars with
  | [] -> sc_list
  | _ ->
      (* Term building and type checking. *)
...
let add_mode_sub terms sc_list =
  match terms with
  | [] -> sc_list
  | _ ->
      Cons
...
let int_init_set, rat_init_set =
  ...
  ( IntSet.empty |> IntSet.add Numeral.zero |> IntSet.add Numeral.one
    |> IntSet.add Numeral.(one + one)
    |> IntSet.add Numeral.(one + one + one),
...
let op_combinate_arith_sets (ints, rats) =
  let rec loop un_ops bin_ops add set = function
    | e1 :: tail ->
        ...
        let set =
          un_ops |> List.fold_left (fun set unary -> add (unary e1) set) set
        in
...
let mk sys =
  ...
  Debug.c2i "[mk] %d bool vars, %d int vars, %d rat vars@." (List.length b_vars)
    (List.length i_vars) (List.length r_vars);
  ...
  let rec eval_conj conj_val rating_prefix_rev = function
    | atom :: a_tail, rating :: r_tail ->
        if eval_term sys model atom then
          ...
          (a_tail, r_tail) |> eval_conj conj_val (rating :: rating_prefix_rev)
        else
          (* Participates in the dnf being false. *)
          (a_tail, r_tail) |> eval_conj false ((rating + 1) :: rating_prefix_rev)
  ...
  let rec eval_disj rating_prefix_rev = function
    | conj :: c_tail, rating :: r_tail ->
        ...
        if conj_val then
          ...
          (true, dnf_rating)
        else (c_tail, r_tail) |> eval_disj (rating :: rating_prefix_rev)
    | [], [] ->
...
let eval_rate_true sys dnf dnf_rating model =
  let rec eval_conj rating_prefix_rev = function
    | atom :: a_tail, rating :: r_tail ->
        if eval_term sys model atom then
          ...
          (a_tail, r_tail) |> eval_conj ((rating + 1) :: rating_prefix_rev)
        else
  ...
  let rec eval_disj disj_val rating_prefix_rev = function
    ...
    | [], [] ->
        if disj_val then (true, List.rev rating_prefix_rev)
        else (false, dnf_rating)
    | [], _ -> failwith "[eval_rate_false] rating longer than disjunction"
    | _, [] -> failwith "[eval_rate_false] disjunction longer than rating"
  in
...
let neg = function
  | 0 -> 1
  | 1 -> 0
  | n -> Format.sprintf "illegal bool as int value %d" n |> failwith

(* Type returned by the rated cost function. *)
type rated_t = { cost : int; mutable rating : int list list; candidate : t }
...
let rated_cost_function ({ sys } as candidate) white grey black =
  ...
  let c3, black_vals, rating =
    black
    |> List.fold_left
         (fun (c3, vals, rating) black_model ->
  ...
  let c1, c2, rating =
    white
    |> List.fold_left
         (fun (c1, c2, rating) white_model ->
           (* DNF should be true on a white model, thus we rate false. *)
           let white_val, rating' =
             eval_rate_false sys dnf rating white_model
           in
  ...
  let c4, rating =
    ...
    |> List.fold_left
         ...
         We start by eval-rating [s] for true. *)
           ...
           | false, _ ->
               (* Evaluates to false, don't care about rating or [s']. *)
               (c4, rating)
           | true, rating' -> (
               (* Evaluates to true, let's check [s']. *)
               match eval_rate_false sys dnf rating' s' with
               | true, _ ->
                   (* Implication holds, [rating'] irrelevant. *)
                   (c4, rating)
               | false, rating' ->
                   ( c4 + 1,
                     rating'
                     (* Implication does not hold, [rating'] relevant. *) )))
         (0, rating)
  in
  ...
  let rec max_of_dnf cpt max_atom_rating max_conjuncts = function
    | rating_list :: tail ->
        ...
        if max_atom_rating' < max_atom_rating then
          ...
          max_of_dnf (cpt + 1) max_atom_rating
            ((cpt, max_atoms) :: max_conjuncts)
            tail
        else
...
let rating_decrement i j = nth_do i (nth_do j (fun v -> max (v - 1) 0))
...
let rated_move ({ rating; candidate } as rated) =
  ...
  let disjunct_index, atom_index =
    ...
    in
    (d_index, Array.length atom_ratings |> Random.int |> Array.get atom_ratings)
  in
