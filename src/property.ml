...
let is_candidate = function
  | Candidate _ -> true
  | PropAnnot _ | Generated _ | Instantiated _ | Assumption _ | Guarantee _
  | GuaranteeOneModeActive _ | GuaranteeModeImplication _ | NonVacuityCheck _ ->
      false
...
let pp_print_prop_source ppf = function
  | PropAnnot pos -> Format.fprintf ppf "%a" pp_print_position pos
  | Generated (_, []) -> Format.fprintf ppf "generated"
  | Generated _ -> Format.fprintf ppf "subrange constraint"
  | Candidate _ -> Format.fprintf ppf "candidate invariant"
  | Instantiated (scope, _) ->
      Format.fprintf ppf "instantiated from %s" (String.concat "." scope)
...
let pp_print_prop_quiet ppf { prop_name; prop_source } =
  Format.fprintf ppf "'%s': %a" prop_name pp_print_prop_source prop_source
...
let pp_print_property ppf { prop_name; prop_source; prop_term; prop_status } =
  Format.fprintf ppf "@[<hv 1>(%s@ %a@ %a@ %a)@]" prop_name Term.pp_print_term
    prop_term pp_print_prop_source prop_source pp_print_prop_status_pt
    prop_status

(* Property status is known? *)
let prop_status_known = function
  (* Property may become invariant or false *)
  | PropUnknown | PropKTrue _ -> false
  (* Property is invariant or false *)
  | PropInvariant _ | PropFalse _ -> true
...
let set_prop_invariant p cert =
  ...
  p.prop_status <-
    ...
    | PropKTrue _ | PropInvariant _ | PropUnknown -> PropInvariant cert
...
let set_prop_false p cex =
  ...
  p.prop_status <-
    ...
    | PropKTrue l when l > length_of_cex cex ->
        let msg =
          Format.sprintf
            "Property '%s' proven %d-true before, now falsified with cex of \
             length %d!"
...
let set_prop_ktrue p k =
  ...
  p.prop_status <-
    ...
    | PropFalse cex ->
        let msg =
          Format.sprintf
            "Falsified property '%s' (l=%d) was proven %d-true too!" p.prop_name
            (length_of_cex cex) k
        in
        failwith msg)
...
let set_prop_unknown p = p.prop_status <- PropUnknown
