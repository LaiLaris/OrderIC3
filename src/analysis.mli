...
(** Interface between strategy and low-level analysis
    ...
    An analysis distinguishes exactly one system as the top system, and looks at
    its properties and its contract. Subsystems are view either as abstract with
    their contract or as concrete with their implementations.

    The result of an analysis indicates which properties of the top system are
    invariant, whether the system conforms to its contract and whether it
    conforms to the preconditions of all its subsystems.

    Values of type {!param} are produced by a strategy and parameterize the
    input-specific transitions system generators, in particular the slicing ot
    the cone of influence.

    Values of type {!result} are produced by running an analysis of a generated
    transition system. The accumulated results are used by a strategey to decide
    the next steps.

    {e NB:} The [uid] stored in an [info] must be unique because it is used
    during transition system generation to avoid name clashes in UFs and svars.
...
val assumptions_fold :
  ('a -> Scope.t -> Invs.t -> 'a) -> 'a -> assumptions -> 'a
...
type param =
  | Interpreter of info  (** Simulation of a system *)
  | ContractCheck of info  (** Analysis of the contract of a system. *)
  | First of info  (** First analysis of a system. *)
...
(** Return [true] if no system is flagged abstract in the [abstraction_map] of a
    [param]. *)
