...
let handle_events input_sys aparam trans_sys =
  ...
  List.iter
    (function
      | m, e ->
          KEvent.log L_debug "Message received from %a: %a" pp_print_kind_module
            m KEvent.pp_print_event e)
    events;

  (* Update transition system from events *)
  let _ = KEvent.update_trans_sys input_sys aparam trans_sys events in

  ()

let print_stats trans_sys =
  KEvent.log L_debug "@[<v>%a@,Final statistics:@]" Pretty.print_line ();
  ...
  | Some trans_sys ->
      let status_kinds =
        TransSys.get_prop_status_and_kind_all_nocands trans_sys
      in
...
(* modules for which other modules needs them to be active to function
   properly *)
let needed_by = function `BMC -> [ `IND; `IND2 ] | `IND -> [ `BMC ] | _ -> []

(* Set of core modules. The analysis goes on if at least one of them is
   active *)
let core_module = function `IND | `BMC | `IC3QE | `IC3QEl1l2 | `IC3QEl2l1 | `IC3IA -> true | _ -> false
...
let term_kill (pid, dep) =
  KEvent.log L_warn "Terminating useless %a (PID %d)" pp_print_kind_module dep
    pid;
...
let check_pending_processes run_process pending_processes sys child_pids =
  ...
  | _ ->
      let num_of_ic3ia_modules =
        !child_pids
        |> List.filter (function _, `IC3IA -> true | _ -> false)
        |> List.length
      in
      let diff = Flags.IC3IA.max_processes () - num_of_ic3ia_modules in
      if diff > 0 then (
        let to_run, pending = list_split diff !pending_processes in
...
let rec wait_for_children run_process pending_processes sys child_pids =
  ...
  | child_pid, (Unix.WEXITED 0 as status) ->
      KEvent.log L_info "Child process %d (%a) %a" child_pid
        pp_print_kind_module
        (List.assoc child_pid !child_pids)
  ...
  | child_pid, status ->
      KEvent.log L_warn "Child process %d (%a) %a" child_pid
        pp_print_kind_module
        (List.assoc child_pid !child_pids)
        pp_print_process_status status;
...
(* Polling loop *)
let rec loop run_process pending_processes ignore_props stop_if_falsified
  ...
  let done_at' =
    ...
    if
      (TransSys.all_props_proved trans_sys && not ignore_props)
    ...
    then
      ...
      | None ->
          ...
          KEvent.log L_info
            "<Done> @[<v>All properties proved or disproved in %.3fs.@ Waiting \
      ...
      | None ->
          ...
          KEvent.log L_info
            "<Done> @[<v>Reached analysis timeout (%1.0f)@ Waiting for \
  ...
  if
    wait_for_children run_process pending_processes trans_sys child_pids
    ||
    match done_at with
    | None -> false
  ...
  then (
    ...
    loop run_process pending_processes ignore_props stop_if_falsified done_at'
      timeout_analysis_reached child_pids input_sys aparam trans_sys)

(* Entry point *)
let main run_process pending_processes ignore_props stop_if_falsified child_pids
    input_sys aparam trans_sys =
  (* Building the function checking whether we reached the analysis timeout. *)
  let timeout_analysis = Flags.timeout_analysis () in
  let timeout_analysis_reached =
    if timeout_analysis > 0.0 then fun () ->
  ...
  (* Run main loop *)
  loop run_process pending_processes ignore_props stop_if_falsified None
    timeout_analysis_reached child_pids input_sys aparam trans_sys
