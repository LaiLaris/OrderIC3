...
let init_svar () =
  StateVar.mk_state_var ~is_input:false ~is_const:false ~for_inv_gen:false
    "%init" [] Type.t_bool

let get_init offset =
  Var.mk_state_var_instance (init_svar ()) offset |> Term.mk_var
...
let sys_def_guard () = UfSymbol.mk_uf_symbol "%sys_def" [] Type.t_bool
...
type solving_options = Default | ExtractModel | NoInd
...
let handle_events in_sys param sys prop_name =
  ...
  let _ = KEvent.update_trans_sys in_sys param sys messages in

  match TransSys.get_prop_status sys prop_name with
  | Property.PropFalse _ | Property.PropInvariant _ -> raise Terminate
  | _ -> ()

let sys_def solver sys offset =
  let init_term_curr = get_init offset in
  let init_term_prev = get_init (Numeral.pred offset) in
  let init =
    Term.mk_and
      [
        TransSys.init_of_bound (Some (SMTSolver.declare_fun solver)) sys offset;
  ...
  let trans =
    Term.mk_and
      [
        TransSys.trans_of_bound (Some (SMTSolver.declare_fun solver)) sys offset;
        Term.mk_not init_term_curr;
        Term.mk_not init_term_prev;
      ]
  in
  Term.mk_and (TransSys.global_constraints sys @ [ Term.mk_or [ init; trans ] ])
  ...
  | `MathSAT_SMTLIB -> (
    ...
    *)
      ...
      | `Inferred l when mem RA l && not (mem IA l) ->
          added_logic := Some IA;
          `Inferred (FeatureSet.add IA l)
      | l -> l)
  | _ -> TransSys.get_logic sys

let mk_solver sys =
  let solver =
    SMTSolver.create_instance ~produce_models:true ~produce_unsat_cores:true
      (get_logic sys) (Flags.Smt.solver ())
  in
  ...
  TransSys.define_and_declare_of_bounds sys
    ...
    Numeral.zero (Numeral.succ max_offset);
  ...
  SMTSolver.assert_term solver
    (Term.mk_implies
       [ Term.mk_uf guard []; sys_def solver sys prime_abstr_offset ]);
...
let get_R frames k =
  list_suffix frames k |> List.map (fun f -> Frame.to_term f) |> Term.mk_and
...
let extract_cube predicates predicate_evaluations =
  ...
  let cube =
    List.fold_left
      ...
      Cube.empty (TS.elements predicates)
  in
  cube
...
let add_predicates solver old_predicates new_predicates =
  let new_predicates = TS.diff new_predicates old_predicates in
  TS.iter
    (fun p ->
      ...
      SMTSolver.assert_term solver
        (Term.mk_eq
           [
             Term.bump_state base_offset p; Term.bump_state base_abstr_offset p;
           ]);
...
let refine fwd solver sys predicates cubes =
  ...
  let intrpo =
    ...
    | Some s ->
        if len > !max_unrolling then (
          TransSys.declare_vars_of_bounds sys (SMTSolver.declare_fun s)
            (Numeral.of_int (!max_unrolling + 1))
            (Numeral.of_int len);

          declare_init_var (SMTSolver.declare_fun s)
            (Numeral.of_int (!max_unrolling + 1))
            (Numeral.of_int len);
    ...
    | None ->
        let produce_interpolants, logic =
          ...
          | _ -> (true, get_logic sys)
        in

        let solver =
          SMTSolver.create_instance ~produce_models:true ~produce_interpolants
            logic
  ...
  if SMTSolver.check_sat intrpo then
    let cex_path =
      Model.path_from_model (TransSys.state_vars sys)
    ...
    let interpolants =
      ...
      | _ ->
          SMTSolver.get_interpolants intrpo names |> fun itps ->
          SMTSolver.pop intrpo;
          itps
    in
    ...
    let atoms =
      List.mapi (fun i t -> Term.bump_state (Numeral.of_int ~-i) t) interpolants
...
let is_blocked solver frames s =
  ...
  let blocked =
    ...
    |> List.exists (fun f ->
           Frame.cubes f |> CS.elements
           |> List.exists (fun c -> Cube.subsumes c (TCube.cube s)))
  in
  if blocked then true
  else
    (* Semantic subsumption thru SAT: *)
    let query =
      Term.mk_and
        [ TCube.cube s |> Cube.to_term; get_R frames (TCube.frame_index s) ]
    in
    match check_sat solver TS.empty query with None -> true | _ -> false

let is_initial init c = not (Cube.contains (Term.mk_not init) c)
...
let get_minimal_non_initial_cube init p_names unsat_core =
  let cube =
    List.fold_left
      (fun c (l, l') -> if TS.mem l' unsat_core then Cube.add l c else c)
      Cube.empty p_names
  in
  if is_initial init cube then Cube.add (Term.mk_not init) cube else cube
...
let solve_relative solver predicates frames s option =
  ...
  SMTSolver.assert_term solver (Term.mk_uf (sys_def_guard ()) []);
  ...
  let named_frames, last_frame =
    ...
    (named_frames, List.rev named_frames |> List.hd)
  in
  ...
  let p_names =
    List.map
      (fun l ->
        let l' = Term.bump_state Numeral.(prime_offset - base_offset) l in
  ...
  let res =
    SMTSolver.check_sat_and_get_term_values solver
      (fun _ predicate_evaluations -> Some predicate_evaluations)
      (fun _ -> None)
      (TS.elements predicates)
  in
  ...
  | Some predicate_evaluations ->
      let c =
        match option with
        | ExtractModel -> extract_cube predicates predicate_evaluations
        | _ -> Cube.empty
      in
      SMTSolver.pop solver;
      TCube.mk c TCube.FrameNull
  | None ->
      let unsat_core = SMTSolver.get_unsat_core_of_names solver |> TS.of_list in
      SMTSolver.pop solver;
  ...
  let c' =
    List.fold_left
      (fun c l ->
        ...
        if is_initial init c' then c
        else
          let s' = TCube.mk c' f in
          let z = solve_relative solver predicates frames s' Default in
          match TCube.frame z with TCube.FrameNull -> c | _ -> c')
      c (Cube.literals c)
  in
  TCube.mk c' f
...
let get_invariant init c =
  List.fold_left
    (fun disj l ->
      if not (l == Term.mk_not init) then Term.negate l :: disj else disj)
    [] (Cube.literals c)
  |> Term.mk_or
...
let report_invariant sys inv =
  ...
  if broadcast_inv then
    let cert = (1, inv) in
    KEvent.invariant (TransSys.scope_of_trans_sys sys) inv cert false
...
let add_blocked_cube solver sys init frames s =
       ...
       |> List.map (fun f ->
              ...
              | Frame.Fi cubes ->
                  let cubes' =
                    ...
                    |> List.fold_left
                         (fun acc c ->
                           if Cube.subsumes s_cube c then CS.remove c acc
                           else acc)
                         cubes
                  in
                  Frame.Fi cubes'
  ...
  let frames =
    list_apply_at
      (function
        | Frame.Fi cubes -> Frame.Fi (CS.add s_cube cubes) | _ -> assert false)
      k frames1'
    @ frames2
  in
...
let block_cube fwd solver in_sys param sys prop_name predicates next_cube frames
    s0 =
  let init = get_init base_offset in

  let insert q c = PriorityQueue.(insert q (TCube.frame_index c) c) in
            ...
            let next_cube, frames, q =
              if not (is_blocked solver frames s) then (
                ...
                | _ ->
                    ...
                    let z = generalize solver init predicates frames z in
                    ...
                    let q =
                      if
                        TCube.frame_index s < depth frames
                        && TCube.frame z <> TCube.FrameInf
                      then insert q (TCube.next s)
                      else q
                    in
                    (next_cube, frames, q))
              else (next_cube, frames, q)
            in
...
let add_frame f frames = Lib.list_insert_at f (List.length frames - 1) frames

let get_invariants init frames k =
  List.fold_left
    (fun invs f ->
      CS.fold (fun c invs -> get_invariant init c :: invs) (Frame.cubes f) invs)
...
let main_ic3ia fwd prop in_sys param sys =
  ...
  let predicates =
    add_predicates solver TS.empty (Term.get_atoms prop |> TS.add init)
  in

  SMTSolver.trace_comment solver
    (Format.sprintf "Checking property: %s" prop_name);

  let frames = Frame.mk_frame init :: [ Frame.empty ] in
  ...
  try
    let rec loop predicates next_cube frames =
      ...
      | Some cube ->
          let s = TCube.mk cube (TCube.Frame (depth frames)) in
          let predicates, next_cube, frames =
            block_cube fwd solver in_sys param sys prop_name predicates
              next_cube frames s
          in
      ...
      | None -> (
          ...
          match
            propogate_blocked_cubes solver in_sys param sys prop_name predicates
              frames
          with
    ...
    invariants |> List.iter (fun i -> report_invariant sys i);

    let cert = (1, Term.mk_and invariants) in

    KEvent.prop_status (Property.PropInvariant cert) in_sys param sys prop_name
  with
  | Counterexample cex_path ->
      KEvent.prop_status
        (Property.PropFalse (Model.path_to_list cex_path))
        in_sys param sys prop_name;

      KEvent.log L_info "Property %s disproved by IC3IA" prop_name
  | Terminate ->
      ();
...
let main fwd slice_to_prop prop in_sys param sys =
  ...
  let sys =
    if Flags.slice_nodes () && slice_to_prop then
      ...
      | _ ->
          fst
            (InputSystem.trans_sys_of_analysis ~slice_to_prop:prop in_sys param)
    else sys
  in
  ...
  | `Inferred l when mem A l ->
      let msg =
        Format.sprintf
          "IC3IA disabled for property %s: arrays are not supported."
  ...
  let check_system_is_supported itp_solver =
    if TransSys.subsystem_includes_function_symbol sys then
      let msg =
        Format.sprintf
          "IC3IA (%s) disabled for property %s: system includes an abstract \
           function. Use MathSAT or SMTInterpol instead."
