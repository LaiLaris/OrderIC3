...
type loc = { pos : Lib.position; index : LustreIndex.index }

type term_cat =
  | NodeCall of string * SVSet.t
  | ContractItem of
      StateVar.t * LustreContract.svar * LustreNode.contract_item_type
...
let scmap_size c = ScMap.fold (fun _ lst acc -> acc + List.length lst) c 0
...
let aux_vars sys =
  ...
  in
  List.filter
    (fun sv -> not (List.mem usr_name (StateVar.scope_of_state_var sv)))
    (TS.state_vars sys)

let compute_var_map in_sys sys =
  let aux_vars =
    TS.fold_subsystems ~include_top:true
      (fun acc sys -> aux_vars sys @ acc)
      [] sys
  in
...
let lustre_name_of_sv var_map sv =
  let usr_name =
    ...
    List.hd LustreIdent.user_scope
  in
  if List.mem usr_name (StateVar.scope_of_state_var sv) then
...
type core_print_data = {
  ...
  counterexample : (StateVar.t * Model.value list) list option;
      (* Only for MCSs *)
  time : float option;
  size : int;
  approx : bool option;
    ...
    Some
      (List.map
         (fun (loc, cat) ->
           ...
           | Unknown -> None
           | NodeCall (name, _) ->
               Some { name; category = "Node Call"; position = loc.pos }
           | Equation sv -> (
               try
                 Some
                   {
           ...
           | Assertion _ ->
               Some
                 {
                   name =
                     Format.asprintf "assertion%a" Lib.pp_print_line_and_column
                       loc.pos;
                   category = "Assertion";
                   position = loc.pos;
                 }
...
let printable_elements_of_core in_sys sys core =
  ...
  let aux lst =
    ...
    |> List.filter (function Some _ -> true | None -> false)
    |> List.map (function Some x -> x | None -> assert false)
    |> List.sort (fun { position = p1 } { position = p2 } ->
           Lib.compare_pos p1 p2)
    ...
    |> List.rev
  in
  core |> ScMap.map aux (* Build map *) |> ScMap.filter (fun _ lst -> lst <> [])
...
let attach_approx_to_print_data data approx = { data with approx = Some approx }

let print_mcs_counterexample in_sys param sys typ fmt (prop, cex) =
  try
    if Flags.MCS.print_mcs_cex () || Flags.dump_cex () then
      match typ with
      | `PT ->
          let dump = Flags.dump_cex () in
          KEvent.pp_print_trace_pt dump L_warn in_sys param sys (Some prop) true
            fmt cex
      | `XML ->
...
let format_name_for_json str =
  String.uncapitalize_ascii str |> Str.global_replace (Str.regexp " ") ""

let format_name_for_xml str =
  String.lowercase_ascii str |> Str.global_replace (Str.regexp " ") "_"

let pp_print_core_data in_sys param sys fmt cpd =
  let print_elt elt =
    Format.fprintf fmt "%s @{<blue_b>%s@} at position %a@ "
      (format_name_for_pt elt.category)
      elt.name Lib.pp_print_line_and_column elt.position
  in
  let print_node scope lst =
    let node_type, node_name =
      LustreGenRefTypeImpNodes.get_node_type_and_name (Scope.to_string scope)
    in
  ...
  | None ->
      Format.fprintf fmt "@{<b>%s (%i elements):@}@."
        (String.uppercase_ascii cpd.core_class)
        cpd.size
  | Some n ->
      Format.fprintf fmt
        "@{<b>%s (%i elements)@} for property @{<blue_b>%s@}:@."
        (String.uppercase_ascii cpd.core_class)
        cpd.size n);
  Format.fprintf fmt "  @[<v>";
...
let pp_print_json fmt json =
  Yojson.Basic.pretty_to_string json |> Format.fprintf fmt "%s"
...
let pp_print_core_data_json in_sys param sys fmt cpd =
  ...
  let assoc =
    assoc
    @
    match cpd.property with None -> [] | Some n -> [ ("property", `String n) ]
  in
  let assoc =
    assoc
    @
  ...
  let assoc =
    assoc
    @
    match cpd.approx with None -> [] | Some b -> [ ("approximate", `Bool b) ]
  in
  let assoc =
    assoc
    @ [
               ...
               (fun (scope, elts) ->
                 `Assoc
                   [
                     ( "name",
                       `String
                         (LustreGenRefTypeImpNodes.get_node_type_and_name
  ...
  let assoc =
    ...
    | Some cex, Some p -> (
        let str =
          Format.asprintf "%a"
            (print_mcs_counterexample in_sys param sys `JSON)
            (p, cex)
        in
...
let pp_print_core_data_xml ?(tag = "ModelElementSet") in_sys param sys fmt cpd =
  ...
  let print_node scope elts =
    ...
    let print_elt elt =
      ...
      let file, row, col = Lib.file_row_col_of_pos elt.position in
      Format.fprintf fmt
        "<Element category=\"%s\" name=\"%s\" line=\"%s\" column=\"%s\"%s/>"
        (format_name_for_xml elt.category)
        elt.name (string_of_int row) (string_of_int col)
  ...
  in
  Format.fprintf fmt "<%s class=\"%s\" size=\"%i\"%s%s>@.  @[<v>" tag
    cpd.core_class cpd.size
    (match cpd.property with
    | None -> ""
    | Some n -> Format.asprintf " property=\"%s\"" n)
    (match cpd.approx with
    | None -> ""
    | Some b -> Format.asprintf " approximate=\"%b\"" b);
  (match cpd.time with
  | None -> ()
  | Some f ->
      Format.fprintf fmt
        "<Runtime unit=\"sec\" timeout=\"false\">%.3f</Runtime>@ " f);
  ScMap.iter print_node cpd.elements;
  (match (cpd.counterexample, cpd.property) with
  | Some cex, Some p ->
      Format.fprintf fmt "@ ";
...
let pp_print_no_solution ~unknown fmt prop =
  Format.fprintf fmt "%s for property @{<blue_b>%s@}.@.@."
    (if unknown then "Unknown result" else "No solution")
    ...
    [
      ...
      ( "runtime",
        `Assoc
          [
            ("unit", `String "sec");
            ("value", `Float (Stat.get_float Stat.analysis_time));
            ("timeout", `Bool unknown);
          ] );
    ]
  in
  pp_print_json fmt (`Assoc assoc)
...
let pp_print_no_solution_xml clas ~unknown fmt prop =
  Format.fprintf fmt "<NoModelElementSet class=\"%s\" property=\"%s\">@.  @[<v>"
    clas prop.Property.prop_name;
  ...
  Format.fprintf fmt "<Runtime unit=\"sec\" timeout=\"%s\">%.3f</Runtime>@ "
    (if unknown then "true" else "false")
...
let fresh_actsv_name () = Printf.sprintf "__model_elt_%i" (actsvs_counter ())
...
let get_sv_of_actlit (_, mapping) actlit = SyMap.find actlit mapping |> snd
...
let eq_of_actlit_sv core ?(with_act = false) actlit =
  ...
  if with_act then
    ...
    let guard t =
      (* Term.mk_eq *)
      Term.mk_implies
        [ Term.mk_not (Term.mk_var (Var.mk_const_state_var sv)); t ]
    in
    {
      init_opened = guard eq.init_opened;
      init_closed = guard eq.init_closed;
      trans_opened = guard eq.trans_opened;
      trans_closed = guard eq.trans_closed;
    }
  else eq
...
let scopes_of_core (scmap, _) = ScMap.bindings scmap |> List.map fst
...
let add_new_ts_equation_to_core scope eq ((scmap, mapping) as core) =
  ...
  let sv =
    StateVar.mk_state_var ~is_input:false ~is_const:true (fresh_actsv_name ())
      [] (Type.mk_bool ())
  in
  (ScMap.add scope actlits scmap, SyMap.add actlit (eq, sv) mapping)

let add_from_other_core (_, src_mapping) scope actlit ((scmap, mapping) as core)
    =
  let actlits = get_actlits_of_scope core scope in
  if List.exists (fun a -> UfSymbol.equal_uf_symbols a actlit) actlits then core
  else
...
let sy_union sy1 sy2 =
  SySet.union (SySet.of_list sy1) (SySet.of_list sy2) |> SySet.elements

let sy_inter sy1 sy2 =
  SySet.inter (SySet.of_list sy1) (SySet.of_list sy2) |> SySet.elements

let sy_diff sy1 sy2 =
  SySet.diff (SySet.of_list sy1) (SySet.of_list sy2) |> SySet.elements
...
let filter_core_svs state_vars ((scmap, mapping) as core) =
  ...
  let aux actlits =
    List.filter (fun a -> SVSet.mem (get_sv_of_actlit core a) svs) actlits
  in
...
let equal_model_elements (eq1, _) (eq2, _) = Equation.equal eq1 eq2

let get_model_elements_of_scope core scope =
  try ScMap.find scope core with Not_found -> []

let loc_core_size = scmap_size
let scopes_of_loc_core core = ScMap.bindings core |> List.map fst
let normalize_positions lst = List.sort_uniq Lib.compare_pos lst
...
let locs_of_node_call in_sys output_svs =
  output_svs |> SVSet.elements
  |> List.map (fun sv ->
         InputSystem.lustre_definitions_of_state_var in_sys sv
         |> fst
         |> List.filter (function
              | LustreNode.CallOutput _ -> true
              | _ -> false)
         |> List.map (fun d ->
                {
                  pos = LustreNode.pos_of_state_var_def d;
...
let name_and_svs_of_node_call in_sys s args =
  ...
  let regexp =
    Printf.sprintf "^\\(%s\\|%s\\)_\\(.+\\)_[0-9]+$"
      Lib.ReservedIds.init_uf_string Lib.ReservedIds.trans_uf_string
    |> Str.regexp
  in
  let name = Symbol.string_of_symbol s in
  let name =
    if Str.string_match regexp name 0 then Str.matched_group 2 name else name
  in
  (* Retrieve number of inputs/outputs *)
  let node =
    match
      InputSystem.get_lustre_node in_sys
        (Scope.mk_scope [ Ident.of_string name ])
    with
    | Some node -> node
    | None -> assert false
  in
  ...
  let svs =
    sublist (nb_inputs + nb_oracles) nb_outputs args
    |> List.map (fun t ->
...
let rec first_var t =
  ...
  | App (_, lst) ->
      let rec loop = function
        | [] -> None
        | t :: ts -> (
            match first_var t with None -> loop ts | Some v -> Some v)
      in
      loop lst
...
let locs_of_eq_term in_sys t =
  try
    ...
    |> List.map (fun def ->
           ...
           | LustreNode.ContractItem (_, svar, typ) ->
               ({ pos = p; index = i }, ContractItem (sv, svar, typ))
           | LustreNode.FrameBlock _ -> ({ pos = p; index = i }, FrameBlock sv)
           | _ -> ({ pos = p; index = i }, Equation sv))
  with _ -> assert false

let compare_loc { pos; index } { pos = pos'; index = index' } =
...
let normalize_locs_cats locs_cats =
  let f (loc, cat) acc =
    match
      List.find_opt (fun loc' -> compare_loc loc loc' = 0) (List.map fst acc)
    with
    | None -> (loc, cat) :: acc
    | Some _ -> acc
  in
...
let add_loc in_sys eq =
  try
    ...
    | Term.T.App (s, ts)
      when match Symbol.node_of_symbol s with `UF _ -> true | _ -> false ->
        (* Case of a node call *)
        let name, svs = name_and_svs_of_node_call in_sys s ts in
        let locs = locs_of_node_call in_sys svs in
        let locs_cats =
          List.map (fun x -> (x, NodeCall (name, svs))) (normalize_locs locs)
        in
        (eq, locs_cats)
    | _ ->
...
let add_to_loc_core ?(check_already_exists = false) scope elt core =
  ...
  if
    ...
    && List.exists (fun e -> equal_model_elements e elt) elts
  then core
...
let is_model_element_in_categories (_, locs_cats) is_main_node cats =
  ...
  | _ ->
      let cat =
        match snd (List.hd locs_cats) with
        ...
        | ContractItem (_, _, LustreNode.Ensure) when not is_main_node ->
            [ `CONTRACT_ITEM ]
        | ContractItem (_, _, _) -> []
        | FrameBlock _ | Equation _ -> [ `EQUATION ]
...
module TermId = struct
  ...
  let compare (a, b) (a', b') =
    match compare b b' with 0 -> SVSet.compare a a' | n -> n
end
...
let extract_toplevel_equations in_sys sys =
  ...
  let init = List.combine oinit cinit and trans = List.combine otrans ctrans in
          ...
          let eq =
            {
              init_opened = oi;
              init_closed = ci;
              trans_opened = Term.t_true;
              trans_closed = Term.t_true;
            }
          in
          Some eq
      | None, Some (ot, ct) ->
          let eq =
            {
              init_opened = Term.t_true;
              init_closed = Term.t_true;
              trans_opened = ot;
              trans_closed = ct;
            }
          in
...
let partition_loc_core_elts_by_guarantees_and_mode_elts loc_core =
  let f = function
    ...
    | ContractItem (_, _, LustreNode.Ensure) ->
        true
    | _ -> false
  in
  ScMap.fold
