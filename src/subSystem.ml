...
let strategy_info_of { opacity; has_contract; has_modes; has_impl } =
  {
    Strategy.opacity;
...
let rec all_subsystems' accum = function
  ...
  | { scope } :: tl when accum |> List.exists (fun { scope = s } -> scope = s)
    ->
        ...
        |> List.fold_left
             (fun tl' ({ scope } as subsystem) ->
               if
                 ...
                 List.exists (fun { scope = s } -> scope = s) accum
               then
                 (* Do not add twice. *)
                 tl'
...
let rec find_subsystem' scope visited = function
  ...
  | ({ scope = scope'; subsystems } as subsystem) :: tl ->
      ...
      if scope = scope' then subsystem
      else if
        (* System already seen? *)
        List.mem scope' visited
      then
...
let find_subsystem_of_list subsystems scope =
  find_subsystem' scope [] subsystems
