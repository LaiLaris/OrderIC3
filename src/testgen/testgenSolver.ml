...
(** Wraps a solver and provides a convenient interface for test generation.
    ...
    that, in the solver: a0 => Init(0) a1 => (and a0 T(0,1)) a2 => (and a1
    T(1,2)) ... aN => (and aN-1 T(N-1,N))
...
type t = { sys : Sys.t; solver : S.t; mutable actlits : actlit list }
...
let mk sys =
  ...
  (* Creating solver. *)
  let solver =
    S.create_instance ~produce_models:true (Sys.get_logic sys)
      (Flags.Smt.solver ())
  in
  (* println "declaring vars@0" ; *)
  (* Declaring variables at 0. *)
  TransSys.define_and_declare_of_bounds sys (S.define_fun solver)
    (S.declare_fun solver) (S.declare_sort solver) zero zero;
  ...
  Term.mk_implies
    [ actlit; Sys.init_of_bound (Some (S.declare_fun solver)) sys zero ]
...
let nth_actlit_of t n =
  ...
  let rec loop map_prefix cpt = function
    ...
    | [] ->
        ...
        | [] ->
            failwith
              "unreachable: list of activation literals can never be empty");
        if Num.(cpt = n) then (
...
let checksat ({ solver } as t) n term actlits terms f =
  ...
  |> S.check_sat_assuming_and_get_term_values solver
       ...
       (fun solver values ->
         ...
         Some
           ( terms |> List.map (fun (key, term) -> (key, List.assq term values)),
             whatever ))
       (* If unsat. *)
       (fun _ ->
         (* Deactivating actlit. *)
