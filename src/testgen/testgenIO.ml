...
let mk input_sys sys root name title =
  ...
  let class_file =
    if Flags.Testgen.graph_only () |> not then (
      (* |===| XML class file. *)
      let class_file = Format.sprintf "%s.xml" dir |> openfile in
      let class_fmt = fmt_of_file class_file in
      Format.fprintf class_fmt
        "<?xml version=\"1.0\"?>@.<data system=\"%a\" name=\"%s\">@.@.@?"
  ...
  (* |===| Graph file. *)
  let graph_file = Format.sprintf "%s.dot" dir |> openfile in
  let graph_fmt = fmt_of_file graph_file in
  Format.fprintf graph_fmt
    "strict digraph mode_graph {@.@[<v 2>graph [bgcolor=black margin=0.0] ;@ \
     node [@ style=filled@ fillcolor=black@ fontcolor=\"#1e90ff\"@ \
     color=\"#666666\"@ ] ;@ edge [color=\"#1e90ff\" fontcolor=\"#222222\"] ;@ \
     @]@.@.";
...
let init_error (type s) : s t -> unit =
 fun ({ sys; dir; edir } as t) ->
  ...
  Format.fprintf error_fmt "<?xml version=\"1.0\"?>@.<data system=\"%a\">@.@.@?"
    Scope.pp_print_scope
    (TransSys.scope_of_trans_sys sys);
...
let rm (type s) : s t -> unit =
 fun { class_file; graph_file; error_file } ->
  ...
  let graph_fmt = fmt_of_file graph_file in
  Format.fprintf graph_fmt "}@.@?";
  Unix.close class_file;
  Unix.close graph_file;
  ()

(* The number of testcases generated. *)
let testcase_count (type s) : s t -> int = fun { uid } -> uid

(* The number of errors logged. *)
let error_count (type s) : s t -> int = fun { euid } -> euid
...
let pp_print_tc fmt path name modes =
  let rec loop cpt = function
    | modes :: tail ->
        Format.fprintf fmt "    at step %d, activates @[<v>%a@]@." cpt
          (pp_print_list Scope.pp_print_scope " and ")
          modes;
        loop (cpt + 1) tail
    | [] -> ()
  in
  Format.fprintf fmt "  <testcase path=\"%s\" name=\"%s\" format=\"csv\">@."
    path name;
...
let pp_print_deadlock fmt path name modes =
  ...
  in
  Format.fprintf fmt "  <deadlock path=\"%s\" name=\"%s\" format=\"csv\">@."
    path name;
  List.rev modes |> loop 0;
...
let log_test_glue_file target sys (oracle_path, guarantees, modes) implem_path
    test_files =
  let target = Format.sprintf "%s/tests.xml" target |> openfile in
  let fmt = fmt_of_file target in
  Format.fprintf fmt
    "<?xml version=\"1.0\"?>@.<data system=\"%s\">@.@.  @[<v><oracle@   \
     @[<v>path=\"%s/target/release/%s_oracle\"@ setup=\"%a\"@]@ >@   @[<v><!-- \
     Guarantees -->@ %a@ <!-- Mode ensures -->@ %a@]@ </oracle>@]@.@.@.  \
     @[<v>%a%t@]@.@.</data>@.@?"
    sys oracle_path sys fmt_cargo_build oracle_path
    (pp_print_list
       (fun fmt (pos, cnt) ->
         let file, row, col = file_row_col_of_pos pos in
         Format.fprintf fmt
           "<output count=\"%d\" file=\"%s\" row=\"%d\" col=\"%d\"></output>"
           cnt file row col)
       "@ ")
    guarantees
    (pp_print_list
       (fun fmt (mode, pos, cnt) ->
         let file, row, col = file_row_col_of_pos pos in
         Format.fprintf fmt
           "<output mode=\"%s\" count=\"%d\" file=\"%s\" row=\"%d\" \
            col=\"%d\"></output>"
    ...
    (fun fmt ->
      ...
      if Flags.lus_compile () then
        Format.fprintf fmt
          "@ @ @[<v><binary@   @[<v>name=\"Rust implementation generated by \
           Kind 2\"@ setup=\"%a\"@]@ >@   %s/target/release/%s_implem@ \
           </binary>@]"
          fmt_cargo_build implem_path implem_path sys);
