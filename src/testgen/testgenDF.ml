...
let on_exit _ =
  ...
  print_stats ();
  (match !solver_ref with None -> () | Some solver -> Solver.rm solver);
  (match !close_io_ref with None -> () | Some close -> close ());
  ()
...
let rec enumerate target io solver tree modes contract_term =
  ...
  let rec loop () =
    ...
    let mode_path = Term.mk_and [ contract_term; Tree.blocking_path_of tree ] in
...
and forward target io solver tree modes contract_term =
  ...
  let solver =
    if Actlit.fresh_actlit_count () >= 10 then (
  ...
  let rec loop () =
    ...
    if Flags.Testgen.len () > Num.to_int k then (
      ...
      | None ->
          (* Deadlock. *)
          KEvent.log L_warn "%sDeadlock found (%a)" log_prefix
            Tree.pp_print_tree tree;
          let k = Num.pred k in
          (match Solver.checksat solver k mode_path [] [] get_model with
          | None -> assert false
          | Some (_, model) ->
...
(* Entry point. *)
let main (type s) : Analysis.param -> s Sys.t -> TSys.t -> string -> string list
    =
 fun param input_sys sys target ->
  (* Separating abstract and concrete systems. *)
  let abstract, concrete =
    Scope.Map.fold
      (fun key value (a, c) -> if value then (key :: a, c) else (a, key :: c))
      (Analysis.info_of_param param).Analysis.abstraction_map ([], [])
  in
  KEvent.log_uncond
    "%s@[<v>Launching on %a.@ concrete subsystems: [ @[<hov>%a@] ]@ abstract \
     subsystems: [ @[<hov>%a@] ]@]"
    log_prefix Scope.pp_print_scope
  ...
  let globals, modes =
    match TestgenModes.modes_of sys with
  ...
  let mode_term = (mode_terms |> Term.mk_or) :: global_terms |> Term.mk_and in

  let init_modes =
    match Solver.checksat solver Numeral.zero mode_term [] modes unit_of with
    | None -> failwith "no mode activable in init"
    | Some (map, ()) -> active_modes_of_map map
  in

  let tree = Tree.mk (fun name -> List.assoc name modes) init_modes in

  (* Starting the timer. *)
  Stat.start_timer Stat.testgen_total_time;

  (try forward target io solver tree modes mode_term
   with TestgenTree.TopReached -> (
     ...
     | 0 -> ()
     | n -> KEvent.log L_warn "%s/!\\ %d deadlocks found /!\\" log_prefix n));
  Stat.testgen_stop_timers ();
  Stat.smt_stop_timers ();
